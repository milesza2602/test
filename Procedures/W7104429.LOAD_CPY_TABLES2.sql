-- ****** Object: Procedure W7104429.LOAD_CPY_TABLES2 Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "LOAD_CPY_TABLES2"  (l_owner in varchar2, l_table_name in varchar2, l_pfx in varchar2) authid  current_user    --definer as  --
as
...
--   IF l_gn <> 'DWHUAT.DWHDBUAT.WOOLWORTHS.CO.ZA'
   ...
   BEGIN
       select count(*) into c from user_tables where table_name = upper(l_pfx || l_table_name);
       if c = 1 then
           l_sql_i := 'drop table W7104429.' || l_pfx || l_table_name || ' ';
...
--       DBMS_OUTPUT.put_line ('INS   ' || SUBSTR (l_sql_i, 1, 180));
   ...
   l_sql_i := 'update /*+ parallel (t,4)  */ W7104429.' || l_pfx || l_table_name || ' t set sys_process_code = ''N'' ';
   EXECUTE IMMEDIATE l_sql_i;
   g_recs_updated := SQL%ROWCOUNT;
       
   l_text :=  '>> ' || 'Records Updated: '||g_recs_updated;
   insert into w7104429.sys_dwh_log values
  (localtimestamp,l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   COMMIT;
--   DBMS_OUTPUT.put_line ('UPD  ' || SUBSTR (l_sql_i, 1, 180));

END  "LOAD_CPY_TABLES2";
/
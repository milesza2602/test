-- ****** Object: Procedure W7104429.OLAP_EMULATE_CUBE Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "OLAP_EMULATE_CUBE" (AWX_P1 in varchar2, AWX_P2 in varchar2, AWX_P3 in varchar2, AWX_P4 in varchar2) authid  current_user    
as
   ...
   l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'OLAP_EMULATE_CUBE';
...
--    dbms_lock.sleep(l_sleep_time);   
    ...
    -- log it
    insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'SSAS', AWX_P1, l_JOB_NM, 'Success', l_sleep_time, AWX_P2, AWX_P3, AWX_P4, l_start_time, l_end_time, 'OLAP_EMULATE_CUBE');

--    g_recs_inserted := SQL%ROWCOUNT;
--    l_text :=  '>> ' || 'Records Inserted: '||g_recs_inserted;
--    insert into w7104429.sys_dwh_log values
--    (localtimestamp,l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    commit;
   ...
   EXCEPTION
       ...
       THEN                                                -- logit and exit
          ...
          l_end_time := sysdate;
          insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'SSAS', AWX_P1, l_JOB_NM, 'Failure', l_sleep_time, AWX_P2, AWX_P3, AWX_P4, l_start_time, l_end_time, 'LOG Failed');
   END;
   commit;

END  "OLAP_EMULATE_CUBE";
/
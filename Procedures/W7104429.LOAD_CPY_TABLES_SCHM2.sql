-- ****** Object: Procedure W7104429.LOAD_CPY_TABLES_SCHM2 Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "LOAD_CPY_TABLES_SCHM2"  (l_owner in varchar2, l_table_name in varchar2, l_pfx in varchar2) authid  current_user    --definer as  --
as
...
--        (localtimestamp,l_name,SYS_CONTEXT('userenv','current_schema'),l_script_name,l_procedure_name,l_text, g_recs_inserted1, 'INc');
--         insert into w7104429.sys_dwh_log values
--        (localtimestamp,l_name,l_system_name,l_script_name,l_procedure_name,l_text, g_recs_inserted1, substr(l_table_name,1,3));
--        commit;
...
--   l_script_name := '';
   ...
   BEGIN
    --   The following for creation of tables in local schema ...
      select count(*) into c from sys.all_tab_cols where owner = l_owner and table_name = l_table_name||'_CPY';

      if c > 0 then
         l_sql_i :=
             'create table W7104429.TH_'
             || l_table_name
         ...
         l_sql_i := 'update W7104429.TH_'|| l_table_name || ' set SYS_PROCESS_CODE = ''N'' ';      
...
--     DBMS_OUTPUT.put_line ('INS   ' || SUBSTR (l_sql_i, 1, 180));
   ...
   EXCEPTION
       ...
       THEN                                                -- logit and exit
--        l_text := DBMS_UTILITY.format_error_backtrace;
--        insert into w7104429.sys_dwh_log values
--        (localtimestamp,l_name,l_system_name,l_script_name,l_procedure_name,l_text, 0, 'ERR');

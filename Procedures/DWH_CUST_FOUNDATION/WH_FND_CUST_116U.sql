--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_116U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_116U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_egn_cust_basket_aux_cpy
...
g_location_no    stg_egn_cust_basket_aux_cpy.location_no%type;
g_tran_date      stg_egn_cust_basket_aux_cpy.tran_date%type;
g_till_no        stg_egn_cust_basket_aux_cpy.till_no%type;
g_tran_no        stg_egn_cust_basket_aux_cpy.tran_no%type;
g_aux_seq_no     stg_egn_cust_basket_aux_cpy.aux_seq_no%type;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_116U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD BASKET_AUX EX ENGEN';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

cursor stg_dup is
select * from stg_egn_cust_basket_aux_cpy
where (location_no,
tran_date,
...
tran_date,
till_no,tran_no,aux_seq_no
from stg_egn_cust_basket_aux_cpy
...
cursor c_stg_egn_cust_basket_aux is
select /*+ FULL(cpy)  parallel (cpy,2) */
              cpy.*
      from    stg_egn_cust_basket_aux_cpy cpy,
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.location_no   = g_location_no and
        ...
        update stg_egn_cust_basket_aux_cpy stg
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      select /*+ FULL(cpy)  parallel (cpy,2) */
             ...
             '','',
              cpy.	promotion_discount_amount AS PROMOTION_DISCOUNT_AMT_LOCAL,
              cpy.	wreward_sales_value       AS WREWARD_SALES_VALUE_LOCAL,'',''
       from  stg_egn_cust_basket_aux_cpy cpy
       where  not exists
...
FOR upd_rec IN c_stg_egn_cust_basket_aux
   loop
     ...
     set    fnd.	tran_type_code	          =	upd_rec.	tran_type_code	,
            ...
            fnd.	loyalty_group	            =	upd_rec.	loyalty_group	,
            ...
            fnd.	promotion_discount_amt_local	=	upd_rec.	promotion_discount_amount	,
            fnd.	wreward_sales_value_local    	=	upd_rec.	wreward_sales_value	,
     ...
     where  fnd.	location_no	      =	upd_rec.	location_no and
            ...
            nvl(fnd.atg_customer_no	          ,0) <>	upd_rec.	atg_customer_no
...
begin
      ...
      insert /*+ APPEND parallel (hsp,2) */ into stg_egn_cust_basket_aux_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */
...
--    update stg_egn_cust_basket_aux_cpy

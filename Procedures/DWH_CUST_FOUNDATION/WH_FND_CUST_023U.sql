--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_023U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_023U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        January 2013
--  Author:      Alastair de Wet
--  Purpose:     Create customer online product dimention table in the foundation layer
--               with input ex staging table from Customer Central.
--  Tables:      Input  - stg_c2_customer_atg_cpy
--               Output - fnd_customer_online_product
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_c2_customer_atg_hsp.sys_process_msg%type;
g_rec_out            fnd_customer_online_product%rowtype;
g_rec_in             stg_c2_customer_atg_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

L_MESSAGE            SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_023U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CUTOMER ONLINE PRODUCT DIM EX CUSTOMER CENTRAL';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_c2_customer_atg_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_customer_online_product%rowtype index by binary_integer;
type tbl_array_u is table of fnd_customer_online_product%rowtype index by binary_integer;
...
type staging_array1 is table of stg_c2_customer_atg_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_c2_customer_atg_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_c2_customer_atg is
   select *
   from stg_c2_customer_atg_cpy
...
begin
   ...
   insert into stg_c2_customer_atg_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_customer_online_product values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).product_no ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_customer_online_product
       ...
       where  product_no             = a_tbl_update(i).product_no  ;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).product_no ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_c2_customer_atg_cpy
...
begin
   g_found := dwh_cust_valid.fnd_customer_online_product (g_rec_out.product_no );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).product_no  = g_rec_out.product_no  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_customer_online_product EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_c2_customer_atg;
    fetch c_stg_c2_customer_atg bulk collect into a_stg_input limit g_forall_limit;

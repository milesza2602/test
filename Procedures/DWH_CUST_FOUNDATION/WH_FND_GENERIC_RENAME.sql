...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_GENERIC_RENAME" ( p_table_name VARCHAR2,
...
--
    ...
    dbms_stats.gather_table_stats('DWH_CUST_FOUNDATION',g_cpy_table_name);
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_OUTPUT.PUT_LINE('24=');
    
    l_text := 'LOCK TABLE STATS on '||  g_cpy_table_name;
    dbms_stats.lock_table_stats('DWH_CUST_FOUNDATION',g_cpy_table_name);
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

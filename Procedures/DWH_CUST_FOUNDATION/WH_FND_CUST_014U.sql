--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_014U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_014U" (p_forall_limit in integer,p_success out boolean) as
...
FOR upd_rec IN c_stg_vsn_all_prod
   loop
     ...
     set    fnd.	IDENTITY_NO	      =	upd_rec.	IDENTITY_NO	,
            ...
            fnd.	WRITE_OFF_VALUE	=	upd_rec.	WRITE_OFF_VALUE/100	,
     ...
     where  fnd.	WFS_ACCOUNT_NO	  =	upd_rec.	WFS_ACCOUNT_NO	and
            ...
            (
              ...
              nvl(fnd.write_off_value                 ,0) <> upd_rec.write_off_value / 100 or
...
--    if TO_CHAR(sysdate, 'DAY') LIKE 'SUNDAY%' or
--       TO_CHAR(sysdate, 'DAY') LIKE 'MONDAY%' then
...
--    end if;
    ...
    if  g_recs_read > 0 then
      l_text := 'Purge data on the allprod master daily when new input in staging:- '||g_date;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'truncate table fnd_wfs_all_prod';
    end if;
...
--    set    sys_process_code = 'Y';
   ...
   DBMS_STATS.gather_table_stats ('DWH_CUST_FOUNDATION','FND_WFS_ALL_PROD',estimate_percent=>1, DEGREE => 32); 

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_086U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_086U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        Sep 2014
--  Author:      Alastair de Wet
--  Purpose:     Create LS_ITEM dimention table in the foundation layer
--               with input ex staging table from SAS!
--  Tables:      Input  - stg_sas_item_cpy
--               Output - fnd_ls_item
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_sas_item_hsp.sys_process_msg%type;
g_rec_out            fnd_ls_item%rowtype;
g_rec_in             stg_sas_item_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_086U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ITEM DIM EX SAS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_sas_item_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_ls_item%rowtype index by binary_integer;
type tbl_array_u is table of fnd_ls_item%rowtype index by binary_integer;
...
type staging_array1 is table of stg_sas_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_sas_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_sas_item is
   select *
   from stg_sas_item_cpy
...
begin
   ...
   g_rec_out.ls_product_family         := g_rec_in.ls_product_family;
...
begin
   ...
   insert into stg_sas_item_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_ls_item values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_ls_item
       set    ls_product_family            = a_tbl_update(i).ls_product_family  ,
              last_updated_date            = a_tbl_update(i).last_updated_date
       where  item_no                      = a_tbl_update(i).item_no and
              nvl(ls_product_family,'  ') <> a_tbl_update(i).ls_product_family ;
...
begin
    ...
    l_text := 'LOAD OF fnd_ls_item EX SAS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_sas_item;
    fetch c_stg_sas_item bulk collect into a_stg_input limit g_forall_limit;

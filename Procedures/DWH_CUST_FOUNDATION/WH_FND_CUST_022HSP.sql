--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_022HSP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_022HSP" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_c2_customer_card
...
g_card_no        stg_c2_customer_card.card_no%type;
...
cursor stg_dup is
select * from stg_c2_customer_card
where (card_no)
in
(select card_no
from stg_c2_customer_card
group by card_no
having count(*) > 1)
order by card_no,
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.card_no    = g_card_no  then
        update stg_c2_customer_card stg

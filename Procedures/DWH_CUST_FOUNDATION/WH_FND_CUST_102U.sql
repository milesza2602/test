--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_102U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_102U" (p_forall_limit in integer,p_success out boolean) as
...
g_location_no       stg_pos_cust_basket_tender_cpy.location_no%type;
g_tran_date         stg_pos_cust_basket_tender_cpy.tran_date%type;
g_till_no           stg_pos_cust_basket_tender_cpy.till_no%type;
g_tran_no           stg_pos_cust_basket_tender_cpy.tran_no%type;
g_tender_type_code  stg_pos_cust_basket_tender_cpy.tender_type_code%type;
g_tender_seq_no     stg_pos_cust_basket_tender_cpy.tender_seq_no%type;
...
cursor stg_dup is
select * from stg_pos_cust_basket_tender_cpy
where (location_no,
tran_date,
...
select /*+ FULL(cpy)  parallel (cpy,2) */
              cpy.*
      from    stg_pos_cust_basket_tender_cpy cpy,
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.location_no       = g_location_no and
        ...
        update stg_pos_cust_basket_tender_cpy stg
...
FOR upd_rec IN c_stg_pos_cust_basket_tender
   loop
     ...
     set    fnd.	tran_time	                =	upd_rec.	tran_time	,
            ...
            fnd.	TENDER_SELLING_LOCAL    =	upd_rec.	TENDER_SELLING_LOCAL	,
...
begin
      ...
      insert /*+ APPEND parallel (hsp,2) */ into stg_pos_cust_basket_tender_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */

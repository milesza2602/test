--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_ARCHIVE_CLEAN
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_ARCHIVE_CLEAN" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_ARCHIVE_CLEAN';
...
cursor c_stg_archive is
      SELECT user AS SCHEMA_NAME, table_name,NUM_ROWS
      FROM   user_tables
      WHERE  partitioned = 'NO'
      AND table_name LIKE 'STG_C2%ARC' 
...
for arc_rec in c_stg_archive
   loop
      ...
      G_STMT :=  'delete from DWH_CUST_FOUNDATION. '
--           || arc_rec.SCHEMA_NAME
--           || '.'
           || arc_rec.table_name
      ...
      l_text := ARC_REC.NUM_ROWS||' '||G_STMT||' AT '||

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_100U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_100U_OLD" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Sean Le Roux
--  Purpose:     Create cust POS Basket fact table in the foundation layer
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_pos_cust_basket_hsp.sys_process_msg%type;
g_rec_out            fnd_cust_basket%rowtype;
g_rec_in             stg_pos_cust_basket_cpy%rowtype;
g_found              boolean;
...
type stg_array is table of stg_pos_cust_basket_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_cust_basket%rowtype index by binary_integer;
type tbl_array_u is table of fnd_cust_basket%rowtype index by binary_integer;
...
type staging_array1 is table of stg_pos_cust_basket_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_pos_cust_basket_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
 ...
 -- additional validations for TRAN_NO will take place once data is available
...
begin
   ...
   insert into stg_pos_cust_basket_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_cust_basket values a_tbl_insert(i);
...
begin
    ...
    l_text := 'LOAD OF FND_cust_BASKET EX POS STARTED AT '||

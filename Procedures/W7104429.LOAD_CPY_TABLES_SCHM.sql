-- ****** Object: Procedure W7104429.LOAD_CPY_TABLES_SCHM Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "LOAD_CPY_TABLES_SCHM"  (l_owner in varchar2, l_table_name in varchar2, l_pfx in varchar2) authid  current_user    --definer as  --
as
...
--   IF l_gn <> 'DWHUAT.DWHDBUAT.WOOLWORTHS.CO.ZA'
   ...
   BEGIN
    --   The following for creation of tables in local schema ...
      select count(*) into c from all_tab_cols where owner = l_owner and table_name = l_table_name||'_CPY';
      ...
      if c > 0 then
         ...
         l_sql_i := 'update W7104429.'|| l_table_name || ' set SYS_PROCESS_CODE = ''N'' ';      
         EXECUTE IMMEDIATE l_sql_i;
         commit; 
      else       

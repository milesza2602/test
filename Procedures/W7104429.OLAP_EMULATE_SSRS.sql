-- ****** Object: Procedure W7104429.OLAP_EMULATE_SSRS Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "OLAP_EMULATE_SSRS" (AWX_P1 in varchar2) authid  current_user    
as
   ...
   l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'OLAP_EMULATE_SSRS';
...
--    dbms_lock.sleep(l_sleep_time);   
    ...
    -- log it
    insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'SSRS', AWX_P1, l_JOB_NM, 'Success', 0, null, null, null, l_start_time, l_end_time, 'OLAP_EMULATE_SSRS');
    commit;
   ...
   EXCEPTION
       ...
       THEN                                                -- logit and exit
          ...
          l_end_time := sysdate;
          insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'SSRS', AWX_P1, l_JOB_NM, 'Failure', 0, null, null, null, l_start_time, l_end_time, 'LOG Failed');
   END;
   commit;

END  "OLAP_EMULATE_SSRS";
/
-- ****** Object: Procedure W7104429.OLAP_EMULATE Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "OLAP_EMULATE"  (AWX_P1 in varchar2, AWX_P2 in varchar2, AWX_P3 in varchar2, AWX_P4 in varchar2) authid  current_user    
as
   ...
   l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'OLAP_EMULATE';
...
--   IF l_gn <> 'DWHUAT.DWHDBUAT.WOOLWORTHS.CO.ZA'
   ...
   BEGIN
    ...
    if AWX_P3 is not null then     
        -- 
        select   AVG_SECS 
        into     l_sleep_time 
        from     w7104429.AWX_UPGRD_OLAP_LOOKUP t
        where    t.CUBE_NM = AWX_P1
        and      t.JOB_NM  = AWX_P3;

        l_JOB_NM := AWX_P3;
    else
        if AWX_P2 is not null then 
            -- IDX (2 parms passed)...
            select   AVG_SECS 
            into     l_sleep_time 
            from     w7104429.AWX_UPGRD_OLAP_LOOKUP t
...
--    dbms_lock.sleep(l_sleep_time);   
    ...
    -- log it
    insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'SSAS', AWX_P1, l_JOB_NM, 'Success', l_sleep_time, AWX_P2, AWX_P3, AWX_P4, l_start_time, l_end_time, 'LOG Success');

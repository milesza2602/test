-- ****** Object: Procedure W7104429.OLAP_EMULATE_QV Script Date: 04/12/2018 11:29:27 AM ******
CREATE OR REPLACE PROCEDURE "OLAP_EMULATE_QV" (AWX_P1 in varchar2) authid  current_user    
as
   ...
   l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'OLAP_emulate_QV';
...
--   IF l_gn <> 'DWHUAT.DWHDBUAT.WOOLWORTHS.CO.ZA'
   ...
   BEGIN
    -- QV lookup ...
    if AWX_P1 is not null then     
       l_JOB_NM := AWX_P1;

       l_sleep_time := floor(dbms_random.value(4,80));
    l_sleep_time := 4;
       l_start_time    := sysdate; commit;
       dbms_lock.sleep(l_sleep_time);   
--       dbms_lock.sleep(floor(dbms_random.value(4,60)));    
       l_end_time      := sysdate; commit;

    -- log it
        insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'QV', AWX_P1, l_JOB_NM, 'Success', l_sleep_time, null, null, null, l_start_time, l_end_time, 'OLAP_EMULATE_QV');
    end if;
    commit;
   ...
   EXCEPTION
       ...
       THEN                                                -- logit and exit
          ...
          l_end_time := sysdate;
          insert into w7104429.AWX_UPGRD_OLAP_LOG values (l_start, 'QV', AWX_P1, l_JOB_NM, 'Failure', l_sleep_time, null, null, null, l_start_time, l_end_time, 'LOG Failed');
   END;
   commit;

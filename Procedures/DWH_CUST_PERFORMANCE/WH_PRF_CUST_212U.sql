--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_212U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_212U" (p_forall_limit in integer,p_success out boolean) as
...
--**************************************************************************************************
    ...
    select count(*) 
    into g_table_count
    from CUST_BASKET_BU_TENDER_MTH_INT
    where fin_year_no = g_last_mn_fin_year_no
    and  fin_month_no = g_last_mn_fin_month_no;
    
    if g_table_count > 0 then

       g_stmt   := 'Alter table  DWH_CUST_PERFORMANCE.CUST_BASKET_BU_TENDER_MTH_INT truncate  subpartition for ('||g_last_mn_fin_year_no||','||g_last_mn_fin_month_no||') update global indexes';
       l_text   := g_stmt;
       dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

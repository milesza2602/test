--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_347U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_347U_NEW" (p_forall_limit in integer,p_success out boolean) AS
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_347U_NEW';
...
---------------------------------------------------------------------------------------------    
    ...
    MERGE /*+ parallel(6) */ INTO CUST_MART_STAFF_DISC_DETAIL   SD 
    USING
    (
    SELECT /*+ parallel(6) */
          LOCATION_NO,

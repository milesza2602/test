--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_327H
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_327H" (p_forall_limit in integer,p_success out boolean) as
...
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_BASKET_ITEM to HOME&AWAY DEPT MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_home_and_away_dept EX cust_basket item LEVEL STARTED AT '||
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    DELETE FROM cust_mart_home_and_away_dept
...
insert /*+ APPEND parallel(csm,4)*/ into cust_mart_home_and_away_dept csm
     ...
     group by bih.primary_customer_identifier, di.department_no),          
...
order by home.location_no,
   ...
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','cust_mart_home_and_away_dept',estimate_percent=>1, DEGREE => 32);

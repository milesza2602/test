--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_348TO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_348TO" (p_forall_limit in integer,p_success out boolean) AS
...
--**************************************************************************************************
             ...
             using (
               select /*+ parallel (csd,8) parallel(di,4) full(di) parallel(pck,4) full(pck) full(dl)*/
                      ...
                      sum(round(item_tran_selling * 100/(100 + di.vat_rate_perc),2)) staff_sales,
--                      sum(item_tran_selling * 100/(100 + di.vat_rate_perc)) staff_sales,
                      sum(round((company_discount_selling) * 100/(100 + di.vat_rate_perc),2))   staff_discount_selling,
                      sum (case when till_no  = 999 then 
                      round((item_tran_selling * 100/(100 + di.vat_rate_perc)),2) else 0 end) online_staff_sales ,
                      sum (case when till_no  = 999 then 
                      round(((company_discount_selling ) * 100/(100 + di.vat_rate_perc)),2) else 0 end) online_staff_discount_selling 
               from   cust_mart_staff_disc_detail csd,
                      dim_item di,
                      dim_location dl,
                      fnd_pack_item_detail pck
               where  tran_date       > g_date - 100
--               where tran_date = '1 feb 2018' 
               and    csd.item_no     = di.item_no
               and    csd.location_no = dl.location_no
               and    csd.item_no     = pck.pack_item_no(+)

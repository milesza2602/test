--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_602FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_602FIX" (p_forall_limit in integer,p_success out boolean) AS 
...
--**************************************************************************************************
   ...
   (
    SELECT /*+ parallel (fnd,8) */  *
    FROM   dwh_cust_foundation.fnd_svoc_campaign_feedback fnd
    WHERE last_updated_date = g_date - 1
    AND BATCH_ID < 200
   ) MER_REC

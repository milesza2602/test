--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_102U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_102U_OLD" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        AUGUST 2011
--  Author:      Alastair de Wet
--  Purpose:     Create cust_basket_tender performance fact table in the performance layer
--               with added value ex foundation layer fnd_cust_basket_tender.
...
g_rec_out            cust_basket_tender%rowtype;
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE cust_basket_tender EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For output arrays into bulk load forall statements --
type tbl_array_i is table of cust_basket_tender%rowtype index by binary_integer;
type tbl_array_u is table of cust_basket_tender%rowtype index by binary_integer;
...
cursor c_fnd_cust_basket_tender is
   select fcb.*,
          dl.sk1_location_no
   from  fnd_cust_basket_tender fcb,
         dim_location    dl
   where fcb.last_updated_date = g_date and
         fcb.location_no       = dl.location_no ;


g_rec_in             c_fnd_cust_basket_tender%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_cust_basket_tender%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into cust_basket_tender values a_tbl_insert(i);
...
begin
    ...
    l_text := 'LOAD OF cust_basket_tender EX fnd_cust_basket_tender STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_cust_basket_tender;
    fetch c_fnd_cust_basket_tender bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_cust_basket_tender;
--**************************************************************************************************

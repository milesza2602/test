--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_282T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_282T" (p_forall_limit in integer,p_success out boolean) AS 

--**************************************************************************************************
--  Date:        Dec 2015
--  Author:      Alastair de Wet
--  Purpose:    Generate product family FOR C&H BU 51,52,54,55 for items that do not have one for Lifestyle Segmentation
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_282U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD PRODUCT FAMILY CODE FOR C&H ';
...
--***********************************************************************************************************   
    ...
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_LSS_PRODUCT_FAMILY_CH';
    execute immediate g_stmt;  
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_LSS_FINAL_CALCS_CH';
    execute immediate g_stmt;
--    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_LSS_PFC_EXCEPTIONS';
--    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_LSS_PFC_NULL_CH';
    execute immediate g_stmt;
 
    insert /*+ APPEND parallel (tmp,12) */ into temp_lss_product_family_ch tmp
    with     filter_recs as ( 
    ...
    where    cbi.tran_date between g_date - 730 and g_date   
    and      di.business_unit_no IN (51,52,54,55)
    ...
    from     cust_basket_item cbi, 
             dim_item_cust_lss di 
    where    cbi.tran_date between g_date - 730  and g_date   --** change to 365 for live version **
    and      di.business_unit_no IN (51,52,54,55)
    and      cbi.primary_customer_identifier <> 998 
    and      cbi.tran_type in ('S','V','R')
    and      cbi.primary_customer_identifier is not null  
    ...
    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_LSS_PRODUCT_FAMILY_CH',estimate_percent=>1, DEGREE => 32);
...
--************************************************************************************************************************   
    ...
    from   dwh_cust_performance.temp_lss_product_family_ch;

-- CREATE A TEMP TABLE WITH PFC'S THAT NEED TO BE CALCULATED AT CUST/ITEM_L1 LEVEL --   
    insert /*+ APPEND parallel(tmp,12) */ into dwh_cust_performance.temp_lss_pfc_null_ch tmp
    select /*+ FULL(pff2)   parallel(pff2,12) */
             pff2.primary_customer_identifier,
             pff2.item_level1_no 
    from     dwh_cust_performance.temp_lss_product_family_ch pff2 
    where    pff2.product_family_code is null;

    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_LSS_PFC_NULL_CH',estimate_percent=>1, DEGREE => 32);
...
--************************************************************************************************************************       
    ...
    INSERT /*+ APPEND parallel(tmp,12) */ into dwh_cust_performance.temp_lss_final_calcs_ch tmp   
    with     assoc_var as ( 
    select /*+ FULL(pff)   parallel(pff,12) */
             pff.association_variable,
             count(distinct pff.primary_customer_identifier) num_customers 
    from     dwh_cust_performance.temp_lss_product_family_ch pff 
    ...
    from     dwh_cust_performance.temp_lss_product_family_ch pff1 
...
-- MATCH UP AT CUSTOMER LEVEL THOSE IL1'S THAT HAVE A NULL PFC WITH ASSOCIATED PURCHASES WHERE THE PFC IS PRESENT --
    ...
    from     temp_lss_pfc_null_ch l1,
--             dwh_cust_performance.temp_lss_product_family_ch l1, ** Alternative join method back to same table is slightly slower **
             dwh_cust_performance.temp_lss_product_family_ch pff3,
             assoc_var av,
             prod_fam pf 
...
--YULE SCORE CALC --
    ...
    select   /*+ FULL(pf)   parallel(12) */   
             ...
             ''
    from     pfc pf,
-- add to eleiminate iems where Category code at IL1 level <> PFC first digit.
             dim_item_cust_lss di
    where    pf.item_level1_no = di.item_no and
             substr(pf.product_family_code,1,1)  = di.category_code 
--             substr(pf.product_family_code,1,1)  = nvl(di.category_code, substr(pf.product_family_code,1,1))
-- end of added 15 Mar 2016
),
...
--************************************************************************************************************************    
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
--************************************************************************************************************************ 
    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_LSS_FINAL_CALCS_CH',estimate_percent=>1, DEGREE => 32);
    commit;
...
-- CALCULATE PREDOMINENT PFC AND PLACE ON TABLE --
       MERGE INTO  temp_lss_final_calcs_ch tmp USING
       (
       with iteml1 as (
       select  
              tmp.item_level1_no,
              ...
              di.class_no,
              di.subclass_no
      from    temp_lss_final_calcs_ch tmp,
              dim_item di

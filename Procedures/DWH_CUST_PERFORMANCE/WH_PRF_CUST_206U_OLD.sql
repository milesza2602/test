--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_206U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_206U_OLD" (p_forall_limit in integer,p_success out boolean) as
 ...
 --     g_rec_out.last_updated_date         := g_date;
      ...
      with    tmp as  
              (select /*+ FULL(cb)  parallel (cb,4) */ 
               ...
               from  cust_basket cb
               WHERE tran_date           BETWEEN g_process_date AND G_DATE
      ...
      from     cust_basket_item bi, basket bk
               where  bi.location_no = bk.location_no and
                      ...
                      bi.tran_date   BETWEEN g_process_date AND G_DATE ;
...
--**************************************************************************************************
    ...
    delete /*+ parallel(4) */ from cust_basket_item_sample
    where tran_date = g_date ;
    
    commit;

    l_text := 'BULK DELETE -1 STARTED AT '||
    to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

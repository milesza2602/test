--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_292U_THEO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_292U_THEO" (p_forall_limit in integer,p_success out boolean) AS
--**************************************************************************************************
--  Date:        Aug 2017
--  Author:      Theo Filander
--  Purpose:     Create Group Involvement Scores History Update for the last 2 years.
--  Tables:      Input  - cust_db_group_month
--                      - dim_calendar
--                      - temp_cust_grp_item_ranking
--                      - temp_cust_grp_sale_ranking
--                      - temp_item_sales_grp_involve
--               Output - cust_db_group_month_involve
...
--**************************************************************************************************
  ...
  g_max_groups          NUMBER DEFAULT 0;
  do_loop               NUMBER(3) DEFAULT 3;
  g_involve_column      VARCHAR2(30);
  g_item_column         VARCHAR2(30);
  g_sale_column         VARCHAR2(30);
  g_exit_tru            BOOLEAN := false;

  type array_t is table of varchar2(4000);
  g_sqls     array_t := array_t();
  
  type array_grp is table of number(6);
  g_groups array_grp := array_grp(); -- Initialise it
  
  

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_292U_THEO';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CUST_DB_GROUP_MONTH_INVOLVE EX CUST_DB_GROUP_MONTH';
...
--    dbms_output.put_line('PROCESSING populate_item_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_grp_item_ranking b (fin_year_no,fin_month_no,group_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    with customer_cnt as 
                 (select /*+ Parallel(cust,8) Full(cust) */
                    ...
                    from dwh_cust_performance.temp_item_sales_grp_involve
                   group by fin_year_no, fin_month_no, group_no
                 ),
         customer_item_cnt as 
                 (select /*+ Parallel(c,8) Parallel(d,8) Full(c) Full(d) */
                    ...
                    from dwh_cust_performance.temp_item_sales_grp_involve c
                   inner join customer_cnt d
                      on c.fin_year_no = d.fin_year_no and 
                         c.fin_month_no = d.fin_month_no and 
                         c.group_no = d.group_no
                   group by c.fin_year_no,c.fin_month_no,c.group_no,c.item_sum,d.customer_cnt
                 ),
         customer_item_ranking as 
                 (select /*+ Parallel(a,8) Full(a) */
                         ...
                         a.customer_cnt,
                         a.customer_item_cnt,
                         sum(a.customer_item_cnt) over (partition by a.fin_year_no, a.fin_month_no, a.group_no order by a.fin_year_no, a.fin_month_no, a.group_no, a.item_sum rows between unbounded preceding and current row) - a.customer_item_cnt as diff_customer_item_cnt
...
--    dbms_output.put_line('PROCESSING populate_sale_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '    
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_grp_sale_ranking (fin_year_no,fin_month_no,group_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH customer_cnt AS 
                 (SELECT /*+ Parallel(cust,8) Full(cust) */
                         fin_year_no,
                         fin_month_no,
                 ...
                 (select /*+ Parallel(c,8) Parallel(d,8) Full(c) Full(d) */
                   ...
                   inner join customer_cnt d
                      on c.fin_year_no = d.fin_year_no and 
                         c.fin_month_no = d.fin_month_no and 
                         c.group_no = d.group_no
                   group by c.fin_year_no,c.fin_month_no,c.group_no,c.sales_sum,d.customer_cnt
                 ),
         customer_item_ranking as 
                 (select /*+ Parallel(a,8) Full(a) */
                         a.fin_year_no,
                         ...
                         a.customer_cnt,
                         a.customer_item_cnt,
                         sum(a.customer_item_cnt) over (partition by a.fin_year_no, a.fin_month_no, a.group_no order by a.fin_year_no, a.fin_month_no, a.group_no, a.sales_sum rows between unbounded preceding and current row) - a.customer_item_cnt as diff_customer_item_cnt
                    from customer_item_cnt a 
                 )  
    select /*+ Parallel(a,8) Full(a) */
           '||f_year_no||','||f_month_no||',
...
l_text := 'LOAD OF CUST_DB_GROUP_MONTH_INVOLVE EX CUST_DB_GROUP_MONTH STARTED AT '||
...
g_stmt := 'truncate table dwh_cust_performance.temp_cust_grp_sale_ranking';
--execute immediate g_stmt; 

g_stmt := 'truncate table dwh_cust_performance.temp_cust_grp_item_ranking';
--execute immediate g_stmt;
--commit;
...
----------------------------------------------------------------------------------------------     
        case when g_loop = 1 then
                  ...
                  g_stmt := 'alter table dwh_cust_performance.cust_db_group_month_involve truncate subpartition for ('||g_year_no||','||g_month_no||')';
                  l_text := g_stmt;
                  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--                  execute immediate g_stmt;
                  commit;
                  ...
                  g_stmt_main := 'select  /*+ Parallel(cma,8) */
                                          distinct
                                          cma.fin_year_no,
                                          cma.fin_month_no,
        ...
        g_stmt := ' create table dwh_cust_performance.temp_item_sales_grp_involve nologging as 
                         (select /*+ Parallel(a,8) Full(a) */ '||
                        ...
                        from dwh_cust_performance.cust_db_group_month a
...
--        execute immediate g_stmt;
        ...
        l_text := '   TEMP_ITEM_SALES_GRP_INVOLVE Inserted '|| SQL%ROWCOUNT;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        commit;
        ...
        ---------------------------------------------------------------------------------------------- 
        g_sqls.extend(); -- Extend it
        g_sqls(g_loop) := '
                          merge /*+ Parallel(di,8) Append */ into dwh_cust_performance.temp_cust_db_grp_month_involve di
                          using (select /*+ Parallel(ir,8) Parallel(sr,8) */ ir.group_no,
                                        ir.item_sum,
                                        sr.sales_sum,
                                        ir.ranking item_rank ,
                                        sr.ranking sale_rank
                                   from dwh_cust_performance.temp_cust_grp_item_ranking ir
                                  inner join dwh_cust_performance.temp_cust_grp_sale_ranking sr
                                     on (ir.fin_year_no  = sr.fin_year_no and
                                         ir.fin_month_no = sr.fin_month_no and
                                         ir.group_no  = sr.group_no 
                                        )
                                  where ir.fin_year_no  = '||i.fin_year_no||' and 
                                        ir.fin_month_no = '||i.fin_month_no||' and
                                        ir.group_no     = this_group_no 
                                ) tt
                             on (
                                  di.group_no                = tt.group_no and
...
--            execute immediate 'DROP TABLE TEMP_ITEM_SALES_GRP_INVOLVE';
--            commit;

     end loop;

----------------------------------------------------------------------------------------------
--  Create the temp involve table. The table has to be without indexes etc as this has a huge
--  impact the involvement score updates happen.
----------------------------------------------------------------------------------------------
--    execute immediate 'drop table dwh_cust_performance.temp_cust_db_grp_month_involve';

--    execute immediate 'create table dwh_cust_performance.temp_cust_db_grp_month_involve  nologging as 
--                       ( select * from dwh_cust_performance.cust_db_group_month_involve where 1=0)';
--    commit;

    l_text := 'LOAD DWH_CUST_PERFORMANCE.TEMP_CUST_DB_GRP_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    g_stmt_main := 'insert /*+ Append Parallel(a,8) */ into dwh_cust_performance.temp_cust_db_grp_month_involve a 
                   ...
                   to_date(trunc(sysdate)) last_updated_date from dwh_cust_performance.cust_db_group_month cma where cma.fin_year_no = '||g_year_no||' and cma.fin_month_no = '||g_month_no;

--    execute immediate g_stmt_main;

    l_text := ' * GROUP INVOLVE RECORDS INSERTED INTO TEMP '|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    commit;
    ...
    --  Get all Groups
    ---------------------------------------------------------------------------------------------- 
    l_text := ' LIST ALL GROUPS - '||to_char(sysdate,('dd Mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    for c_groups in (select  /*+ Parallel(tg,8) */ distinct group_no from dwh_cust_performance.temp_cust_db_grp_month_involve tg order by group_no)
    loop
        g_groups.extend;
        g_groups(g_groups.last) := c_groups.group_no;
    end loop;
    ...
    for i in 1..25 
    loop
      if to_char(sysdate,('hh24')) = '21' or
         to_char(sysdate,('hh24')) = '22' 
         then
         g_exit_tru := true;
         exit when g_exit_tru = true ;
         return ;
      end if;
      
      l_text := '   INVOLVEMENT SCORE UPDATE - LOOP #'||i;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

      for j in g_groups.first..g_groups.last
      loop
        if to_char(sysdate,('hh24')) = '21' or
           to_char(sysdate,('hh24')) = '22' 
           then 
           g_exit_tru := true;
           l_text := '     TIME TO EXIT GROUP PROCESSING  # ';
           dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
           exit when g_exit_tru = true ;
           return ;
        end if;
        ...
        g_stmt := g_sqls(i);
        g_stmt := replace(g_stmt,'this_group_no',g_groups(j));
  
        if i = 1 or
           (i = 2 and
           g_groups(j) <= 9004) then
           l_text := '       SKIPPING LOOP  # '||i||'   GROUP # '||g_groups(j);
           dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
           continue;
        end if;
        ...
        l_text := '     PROCESSED GROUP  # '||g_groups(j)||' : '|| SQL%ROWCOUNT;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        commit;
        
      end loop;
      
      if g_exit_tru = true then
        l_text := '     EXITING LOOP PROCESSING ';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        exit when g_exit_tru = true ;
      end if;
    end loop;
    ...
    l_text := 'LOAD DWH_CUST_PERFORMANCE.CUST_DB_GROUP_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    execute immediate 'insert /*+ Append Parallel(ci,8) */ into dwh_cust_performance.cust_db_group_month_involve ci select /*+ Parallel(bi,8) */ * from dwh_cust_performance.temp_cust_db_grp_month_involve bi';
    
    l_text := ' * GROUP INVOLVE RECORDS INSERTED INTO CUST_DB_GROUP_MONTH_INVOLVE '|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    commit;

    ----------------------------------------------------------------------------------------------
    --  Cleanup TEMP_CUST_DB_GRP_MONTH_INVOLVE
    ----------------------------------------------------------------------------------------------
--    execute immediate 'truncate table dwh_cust_performance.temp_cust_db_grp_month_involve';
--    commit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_294U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_294U" (p_forall_limit in integer,p_success out boolean) AS 
...
--    COMMIT;
    ...
    INSERT INTO DWH_CUST_PERFORMANCE.TEMP_CUST_COMP_SALE_RANKING (fin_year_no,fin_month_no,company_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    ...
    INSERT INTO DWH_CUST_PERFORMANCE.TEMP_CUST_COMP_ITEM_RANKING (fin_year_no,fin_month_no,company_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                        a.fin_year_no,
                        a.fin_month_no,
                        company_no,
...
--**************************************************************************************************  
    ...
    select /*+ Parallel(cma,8) Parallel(cmb,8) Parallel(cmc,8) Parallel(cmd,8) Full(cma) Full(cmb) Full(cmc) Full(cmd) */
          ...
          from dwh_cust_performance.cust_db_company_month cma 
               left join
               dwh_cust_performance.temp_cust_comp_item_ranking cmb on cma.num_item_yr1_mn01 + 
               ...
               left join
               dwh_cust_performance.temp_cust_comp_sale_ranking cmc on ROUND(cma.sales_yr1_mn01 + 
                                                                             cma.sales_yr1_mn02 +
                                                                             cma.sales_yr1_mn03 +
                                                                             cma.sales_yr1_mn04 + 
                                                                             cma.sales_yr1_mn05 + 
...
;
    ...
    g_stmt := 'ALTER TABLE dwh_cust_performance.cust_db_company_month_involve TRUNCATE SUBPARTITION FOR ('||g_trunc_year_no||','||g_trunc_month_no||')';
    l_text := g_stmt;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate g_stmt;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_264FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_264FIX" (p_forall_limit in integer,p_success out boolean) AS
...
--**************************************************************************************************
    ...
    from     dim_calendar
    where    fin_year_no  =  2018 and --(select last_yr_fin_year_no from dim_control)  and
             fin_month_no =  3 ; --(select last_mn_fin_month_no from dim_control);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_167U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_167U" (p_forall_limit in integer,p_success out boolean,p_date in date) AS 
...
--               Data for customers who have not shopped in the prev month is rolled into the new month
--
--  Naming conventions:
--  g_  -  Global variable
...
-- 
--**************************************************************************************************
MERGE  INTO dwh_cust_performance.temp_wod_tier_mth_detail TWOD USING
(
   with unpvt as (
                  SELECT  * FROM (
                  SELECT /*+ Parallel(tw,6) full(tw) */
                         FIN_YEAR_NO,
        ...
        ----- Existing Columns ^ ---    New Columns v  ------
                  ...
                  FROM   DWH_CUST_FOUNDATION.FND_WOD_TIER_MTH_DETAIL tw)
...
--************************************************************************************************** 
    ...
    (
       SELECT /*+ Parallel(tw,6) full(tw) */ FIN_YEAR_NO,
              FIN_MONTH_NO,
              CUSTOMER_NO,
              START_TIER,
              YTD_SPEND,
       ...
       FROM TEMP_WOD_TIER_MTH_DETAIL tw

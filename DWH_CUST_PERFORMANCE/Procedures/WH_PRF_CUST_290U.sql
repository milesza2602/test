--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_290U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_290U" (p_forall_limit in integer,p_success out boolean) AS 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_290U';
...
--************************************************************************************************** 
-- Should we delete records. 
-- For now I'm doing a merge
--**************************************************************************************************              
    select count(*)
      into g_recs_read
      from cust_db_dept_month_involve
...
--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_DEPT_SALE_RANKING',estimate_percent=>1, DEGREE => 32);
--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_DEPT_ITEM_RANKING',estimate_percent=>1, DEGREE => 32);
--    COMMIT;

    l_text := 'Populate Sale Ranking for CUST_DB_DEPT_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    INSERT INTO DWH_CUST_PERFORMANCE.TEMP_CUST_DEPT_SALE_RANKING (fin_year_no,fin_month_no,department_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    ...
    INSERT INTO DWH_CUST_PERFORMANCE.TEMP_CUST_DEPT_ITEM_RANKING (fin_year_no,fin_month_no,department_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                        a.fin_year_no,
                        a.fin_month_no,
                        department_no,
...
-- Execute the insert
--**************************************************************************************************  
  insert /*+ Append Parallel(a,8) */ into dwh_cust_performance.cust_db_dept_month_involve a
  select /*+ cardinality(cmd,800000000) Parallel(cma,8) Parallel(cmb,8) Parallel(cmc,8) Parallel(cmd,8) Full(cma) Full(cmb) Full(cmc) Full(cmd) */
               distinct
               cma.fin_year_no,
               cma.fin_month_no,
          ...
          from dwh_cust_performance.cust_db_dept_month cma 
               left join
               dwh_cust_performance.temp_cust_dept_item_ranking cmb on cma.num_item_yr1_mn01 + 
               ...
               left join
               dwh_cust_performance.temp_cust_dept_sale_ranking cmc on ROUND(cma.sales_yr1_mn01 + 
                                                                             cma.sales_yr1_mn02 +
                                                                             cma.sales_yr1_mn03 +
                                                                             cma.sales_yr1_mn04 + 
                                                                             cma.sales_yr1_mn05 + 
               ...
               left join
               dwh_cust_performance.cust_db_dept_month_involve cmd on cma.primary_customer_identifier = cmd.primary_customer_identifier AND
...
--                 cma.sales_yr1_mn07 + cma.sales_yr1_mn08 + cma.sales_yr1_mn09 + cma.sales_yr1_mn10 + cma.sales_yr1_mn11 + cma.sales_yr1_mn12) > 0)
    ...
    g_stmt := 'ALTER TABLE dwh_cust_performance.cust_db_dept_month_involve TRUNCATE SUBPARTITION FOR ('||g_trunc_year_no||','||g_trunc_month_no||')';
    l_text := g_stmt;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate g_stmt;
...
--   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','CUST_DB_DEPT_MONTH_INVOLVE',estimate_percent=>0.1, DEGREE => 16);

  filter_lst.extend(1);
  filter_lst(1).ownname := 'DWH_CUST_PERFORMANCE';
  filter_lst(1).objname := 'CUST_DB_DEPT_MONTH_INVOLVE';

...
--   g_stmt      := 'Alter table  DWH_CUST_PERFORMANCE.CUST_DB_DEPT_MONTH truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_DEPT_MONTH_1';
    execute immediate g_stmt; 
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_DEPT_MONTH_2';
    execute immediate g_stmt; 
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_DEPT_MONTH_3';
    execute immediate g_stmt; 
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_DEPT_MONTH_4';
    execute immediate g_stmt;     

   l_text := 'UPDATE STATS ON ALL TABLES'; 
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
 
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_DEPT_MONTH_1',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_DEPT_MONTH_2',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_DEPT_MONTH_3',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_DEPT_MONTH_4',estimate_percent=>1, DEGREE => 32);

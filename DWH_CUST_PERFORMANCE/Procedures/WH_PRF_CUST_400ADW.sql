--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_400ADW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_400ADW" (p_forall_limit in integer,p_success out boolean) AS 
...
--        dbms_output.put_line('Processing Survey '||c_survey.survey_code);
        ...
        g_stmt :=' insert into temp_basket_item 
                   select /*+ parallel(cbi,6) FULL(cbi)  */ 
                    ...
                    inner join (select /*+ parallel(dm,6) FULL(dm) */ 
                                       dm.*,
                    ...
                    inner join (select /*+ parallel(dcm,6) */ master_subscriber_key,identity_document_no,
                   ...
                   inner join (select   /*+ parallel(piv,6) */  
...
--************************************************************************************************** 
        case when g_recs_updated >= c_survey.quota_minimum  then
            ...
            g_stmt :=' insert  into temp_csats_survey_detail 
                       select /*+ parallel(bi,6) FULL(bi)  */   distinct *
                         from (select /*+ parallel(sm,6) FULL(sm)  */  distinct 
                                ...
                                inner join dwh_cust_performance.temp_basket_item cb on (sm.survey_code = cb.survey_code) 
                                 left join (select 
                                              ...
                                              from (
                                                    select /*+ parallel(csd,6) FULL(csd) */
                        ...
                        left join (select /*+ parallel(ao,6) FULL(ao) 

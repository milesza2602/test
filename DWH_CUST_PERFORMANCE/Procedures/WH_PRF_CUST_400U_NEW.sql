--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_400U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_400U_NEW" (p_forall_limit in integer,p_success out boolean) AS 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_400U_NEW';
...
--************************************************************************************************** 
-- Display all DBMS OUTPUT to facilitate debugging
...
--**************************************************************************************************
    ...
    case
         when to_char(g_rundate,'D') in (2) then      -- Monday (Friday’s transactions;)
              g_start_date  := g_rundate - 3;
              g_end_date    := g_rundate - 3;
         when to_char(g_rundate,'D') in (3) then      -- Tuesday (Saturday and Sunday’s transactions) 
              g_start_date  := g_rundate - 3;
              g_end_date    := g_rundate - 2;
         when to_char(g_rundate,'D') in (4) then      -- Wednesday (Monday and Tuesday’s transactions;)
              g_start_date  := g_rundate - 2;
              g_end_date    := g_rundate - 1;
         when to_char(g_rundate,'D') in (5,6) then    -- Thursday/Friday (Wednesday’s/Thursday’s transactions )
              g_start_date  := g_rundate - 1;
              g_end_date    := g_rundate - 1;
     ...
     g_search_days := ((g_start_date - g_end_date)+1);
...
--**************************************************************************************************
    ...
    select survey_code,
     ...
     from (
           select 
--           survey_code,replace(WM_CONCAT(concat( 'AND ',params0)),',AND','AND') survey_params,replace(WM_CONCAT(concat( 'AND ',params)),',AND','AND') all_survey_params from (
--listagg(race_name) within group (order by race_name)
        survey_code,replace(listagg(concat( 'AND ',params0), ',') within group (order by params0),',AND','AND') survey_params,replace(listagg(concat( 'AND ',params), ',') within group (order by params) ,',AND','AND') all_survey_params from (
           select survey_code,
              ...
              from (
                    select /*+ Full(apd) */ apd.survey_code,apd.product_location_other_desc,
                    ...
                    group by apd.survey_code,apd.product_location_other_desc,case include_exclude_ind when 'INCLUDE' then ' IN ' else ' NOT IN ' end--,
                    --rank() over (partition by apd.survey_code,product_location_other_desc order by include_exclude_ind desc)
...
--    dbms_output.put_line('Inserted into ALL_PARAMS - '||sql%rowcount);
    ...
    g_stmt := ' insert /*+ Append */ into temp_survey_master
                ...
                select aps.survey_code,aps.survey_name,aps.survey_area,aps.minimum_spend_criteria,aps.minimum_units_criteria,aps.priority_order,
                       ...
                       apd.subgroup_no,apd.till_no,survey_params ,all_survey_params,aps.channel 
...
--        dbms_output.put_line('Processing Survey '||c_survey.survey_code);
        ...
        g_stmt :=' insert /*+ Append Parallel(8)*/ into temp_basket_item 
                   select /*+Parallel(cbi,8) Parallel(di,8) Parallel(cm,8) Parallel(dcm,8) Parallel(dr,8) Parallel(csm,8) Parallel(cw,8) Parallel(cs,8) Parallel(sd,8) Parallel(ap,8) Parallel(dl,8)*/ 
                    ...
                    inner join dim_customer_mapping cm on (cbi.primary_customer_identifier=cm.source_key) 
...
--************************************************************************************************** 
        case when g_recs_updated >= c_survey.quota_minimum  then
            ...
            g_stmt :=' insert /*+ Append Parallel(8) */ into temp_csats_survey_detail 
                       select /*+ Parallel(bi,8)*/ distinct *
                         from (select /*+ Parallel(sm,8) Parallel(cb,8) Parallel(cc,8) Parallel(oo,8)*/ distinct 
             ...
             for c_insert in (
                   ...
                   select /*+ Parallel(8) */ 
                     ...
                     select survey_code,
                          ...
                          nonfoods_value_segment_no,
                          extracted_date,
                          last_updated_date from unique_rows order by selling_value desc, item_qty desc )
                     ...
                     loop
                      ...
                      insert into dwh_cust_performance.cust_csats_survey_detail (survey_code,
                                                                                ...
                                                                                nonfoods_value_segment_no,
                                                                                extracted_date,
                                                                                last_updated_date) values (c_insert.survey_code,
                      ...
                      exit when g_recs_loaded >= c_survey.quota_maximum;
                     end loop;

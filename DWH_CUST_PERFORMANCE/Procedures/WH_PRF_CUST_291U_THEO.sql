--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_291U_THEO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_291U_THEO" (p_forall_limit in integer,p_success out boolean) AS
--**************************************************************************************************
--  Date:        Aug 2017
--  Author:      Theo Filander
--  Purpose:     Create Subgroup Involvement Scores History Update for the last 2 years.
--  Tables:      Input  - cust_db_subgroup_month
--                      - dim_calendar
--                      - temp_cust_sgrp_item_ranking
--                      - temp_cust_sgrp_sale_ranking
--                      - temp_item_sales_sgrp_involve
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_291U_THEO';
...
--    dbms_output.put_line('PROCESSING populate_item_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_sgrp_item_ranking b (fin_year_no,fin_month_no,subgroup_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    with customer_cnt as 
                 (select /*+ Parallel(cust,8) Full(cust) */
                    ...
                    from dwh_cust_performance.temp_item_sales_sgrp_involve
         ...
         customer_item_cnt as 
                 (select /*+ Parallel(c,8) Parallel(d,8) Full(c) Full(d) */
                    ...
                    from dwh_cust_performance.temp_item_sales_sgrp_involve c
...
--    dbms_output.put_line('PROCESSING populate_sale_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '    
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_sgrp_sale_ranking (fin_year_no,fin_month_no,subgroup_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH customer_cnt AS 
                 (SELECT /*+ Parallel(cust,8) Full(cust) */
                         fin_year_no,
                         fin_month_no,
    ...
    for i in (     select fin_year_no,
                        ...
                        from (
                              select distinct fin_year_no,fin_month_no, this_mn_start_date,this_mn_end_date
                                from dim_calendar
                               where calendar_date < (select this_mn_start_date -1 from dim_calendar where calendar_date = to_date(trunc(sysdate)-6))
...
----------------------------------------------------------------------------------------------     
        ...
        execute immediate ' create table dwh_cust_performance.temp_item_sales_sgrp_involve as 
                             (select /*+ Parallel(a,8) Full(a) */ '||
        ...
        l_text := '   TEMP_ITEM_SALES_SGRP_INVOLVE Inserted '|| SQL%ROWCOUNT;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        commit;
        ...
        array(g_loop) := '
                          merge /*+ Parallel(di,8) Append */ into dwh_cust_performance.temp_cust_db_sgp_month_involve di
                          using (select /*+ Parallel(ir,8) Parallel(sr,8) */ ir.subgroup_no,
                                        ir.item_sum,
                                        sr.sales_sum,
                                        ir.ranking item_rank ,
                                        sr.ranking sale_rank
                                   from dwh_cust_performance.temp_cust_sgrp_item_ranking ir
                                  inner join dwh_cust_performance.temp_cust_sgrp_sale_ranking sr
                                     on (ir.fin_year_no  = sr.fin_year_no and
                                         ir.fin_month_no = sr.fin_month_no and
                                         ir.subgroup_no  = sr.subgroup_no 
                                        )
                                  where ir.fin_year_no  = '||i.fin_year_no||' and 
                                        ir.fin_month_no = '||i.fin_month_no||' 
                                ) tt
                             on (
                                  di.subgroup_no                = tt.subgroup_no and
        ...
        ----------------------------------------------------------------------------------------------        
            ...
            execute immediate 'DROP TABLE TEMP_ITEM_SALES_SGRP_INVOLVE';
            commit;

     end loop;

    ---------------------------------------------------------------------------------------------- 
    -- Low Impact. No update Statistics required
    ---------------------------------------------------------------------------------------------- 
--    l_text := ' UPDATE STATISTICS ON TEMP_CUST_SGRP_ITEM_RANKING';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SGRP_ITEM_RANKING',estimate_percent=>1, DEGREE => 32);

--    l_text := ' UPDATE STATISTICS ON TEMP_CUST_SGRP_SALE_RANKING';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
----------------------------------------------------------------------------------------------
    execute immediate 'create table dwh_cust_performance.temp_cust_db_sgp_month_involve nologging as ( select * from dwh_cust_performance.cust_db_subgroup_month_involve where 1=0)';
    commit;
     
    l_text := 'LOAD DWH_CUST_PERFORMANCE.TEMP_CUST_DB_SGP_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    g_stmt_main := 'insert /*+ Append Parallel(a,8) */ into dwh_cust_performance.temp_cust_db_sgp_month_involve a 
                   ...
                   to_date(trunc(sysdate)) last_updated_date from dwh_cust_performance.cust_db_subgroup_month cma where cma.fin_year_no = '||g_year_no||' and cma.fin_month_no = '||g_month_no;

--    dbms_output.put_line('MAIN SQL STATEMENT: '||LENGTH(g_stmt_main)||g_stmt_main);

    execute immediate g_stmt_main;

    l_text := ' * SUBGROUP INVOLVE RECORDS INSERTED INTO TEMP '|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    commit;

    execute immediate 'alter table dwh_cust_performance.cust_db_subgroup_month_involve nologging';
    commit;
    ...
    l_text := 'LOAD DWH_CUST_PERFORMANCE.CUST_DB_SUBGROUP_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    execute immediate 'insert /*+ Append Parallel(ci,8) */ into dwh_cust_performance.cust_db_subgroup_month_involve ci select /*+ Parallel(bi,8) */ * from dwh_cust_performance.temp_cust_db_sgp_month_involve bi';
    
    l_text := ' * SUBGROUP INVOLVE RECORDS INSERTED INTO CUST_DB_SUBGROUP_MONTH_INVOLVE '|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    commit;
    ...
    ----------------------------------------------------------------------------------------------
    execute immediate 'drop table dwh_cust_performance.temp_cust_db_sgp_month_involve';
    commit;
    
--    l_text := ' UPDATE STATISTICS ON CUST_DB_SUBGROUP_MONTH_INVOLVE';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_108U2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_108U2" (p_forall_limit in integer,p_success out boolean) AS
...
cursor c_prf_cust_basket is
   select /*+  parallel (cb,4) */ location_no,till_no,tran_no,tran_date,loyalty_ww_swipe_no,loyalty_ext_swipe_no,ww_online_order_no,customer_no
   from   cust_basket cb
...
--   and    cb.tran_type in('R','S','V','Q','P','N')
   and    cb.location_no           between 438 and 99999;
...
begin
       ...
       where   location_no                   = g_location_no
         ...
         and  (primary_customer_identifier <> g_pci or nvl(customer_no,0) <> nvl(g_tabl_customer_no,1)) 
...
-- REMOVE BELOW 4 LINES TO REVERT BACK TO RUNNING THIS CODE
    l_text := 'JUMP OUT OF CODE AND PROCESS NO FURTHER';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    p_success := TRUE;
    return;
...
--    write_date_last_transaction;      **** old code inline for last transaction date - changed to multiple dates in a BULK merge at end ***    write_date_last_transaction;
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := dwh_cust_constants.vc_log_run_completed||'108U2'||sysdate;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := dwh_cust_constants.vc_log_draw_line;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        l_text :=  ' ';

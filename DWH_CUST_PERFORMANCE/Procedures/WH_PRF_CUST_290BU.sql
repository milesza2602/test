--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_290BU
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_290BU" (p_forall_limit in integer,p_success out boolean) AS 
...
--  Tables:      Input  - fnd_cust_db_dept_involve  
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CUST_DB_DEPT_MONTH_INVOLVE EX FND_CUST_DB_DEPT_INVOLVE';
...
begin 
    ...
    l_text := 'LOAD OF CUST_DB_DEPT_MONTH_INVOLVE EX FND_CUST_DB_DEPT_INVOLVE STARTED AT '||
...
--**************************************************************************************************
   ...
   g_stmt      := 'Alter table  DWH_CUST_PERFORMANCE.CUST_DB_DEPT_MONTH_INVOLVE truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
   l_text      := g_stmt;
...
--    execute immediate g_stmt; 
  /* g_run_date := g_this_mn_end_date + 10;
   ...
   DBMS_STATS.gather_table_stats ('DWH_CUST_FOUNDATION','FND_CUST_DB_DEPT_INVOLVE',estimate_percent=>1, DEGREE => 32);

   insert /*+ parallel (prfi,8) */ into cust_db_dept_month_involve prfi
   select  /*+ FULL(prf)  parallel (prf,8) parallel (bskt,8) */
         --   G_YR_00,G_MN_00,
                2014, 12,
            nvl(prf.primary_customer_identifier,bskt.primary_customer_identifier) primary_customer_identifier,
            nvl(prf.department_no,bskt.department_no) department_no,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_400BU
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_400BU" (p_forall_limit in integer,p_success out boolean) AS 
...
--        dbms_output.put_line('Processing Survey '||c_survey.survey_code);
        ...
        g_stmt :=' insert into temp_basket_item 
                   select /*+ materialize cardinality (cbi,300000) */ 

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_274U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_274U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_274U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'TAKEON/RESTRUCTURE cust_db_group_month EX cust_basket_item';
...
begin 
    ...
    l_text := 'TAKEON OF cust_db_group_month EX cust_basket_item STARTED AT '||
...
-- CLEAR TABLES DOWN ---
--   g_stmt      := 'Alter table  DWH_CUST_PERFORMANCE.CUST_DB_GROUP_MONTH truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
--   l_text      := g_stmt;
    l_text      := 'TRUNCATE ALL TABLES';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.CUST_DB_GROUP_MONTH';
    execute immediate g_stmt;  
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_GROUP_MONTH_1';
    execute immediate g_stmt; 
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_GROUP_MONTH_2';
    execute immediate g_stmt; 
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_GROUP_MONTH_3';
    execute immediate g_stmt; 
    g_stmt      := 'TRUNCATE table  DWH_CUST_PERFORMANCE.TEMP_DB_GROUP_MONTH_4';
    execute immediate g_stmt;     

   l_text := 'UPDATE STATS ON ALL TABLES'; 
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
 
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_GROUP_MONTH_1',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_GROUP_MONTH_2',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_GROUP_MONTH_3',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_GROUP_MONTH_4',estimate_percent=>1, DEGREE => 32);
--   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','CUST_DB_GROUP_MONTH',estimate_percent=>1, DEGREE => 32);
   COMMIT;
...
--MAIN INSERT STATEMENT USING UNION ALL ACCROSS ALL DATE RANGES --
   ...
   insert /*+ APPEND parallel (prf,8) */ into temp_db_group_month_1 prf
   with     bskt1 as 
   (
   ...
   group by primary_customer_identifier,di.group_no
   ),
            bskt2 as 
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */ 
            primary_customer_identifier,
            di.group_no   group_no,
            max(customer_no)   customer_no,
   ...
   insert /*+ APPEND parallel (prf,12) */ into temp_db_group_month_2 prf
   with         bskt10 as 
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */ 
            primary_customer_identifier,
   ...
   insert /*+ APPEND parallel (prf,12) */ into temp_db_group_month_3 prf   
   with         bskt19 as 
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */ 
            primary_customer_identifier,
   ...
   insert /*+ APPEND parallel (prf,12) */ into temp_db_group_month_4 prf     
   with         bskt28 as 
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */ 
            primary_customer_identifier,
   ...
   insert /*+ APPEND parallel (prf,12) */ into cust_db_group_month  prf
   with        cust_union_all as
   (
   select  /*+ FULL(t1)  parallel (t1,8)  */  *   from temp_db_group_month_1  t1
   union all
   select  /*+ FULL(t2)  parallel (t2,8)  */  *   from temp_db_group_month_2  t2
   union all
   select  /*+ FULL(t3)  parallel (t3,8)  */  *   from temp_db_group_month_3  t3
   union all
   select  /*+ FULL(t4)  parallel (t4,8)  */  *   from temp_db_group_month_4  t4
   )         
...
--   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','CUST_DB_group_MONTH',estimate_percent=>1, DEGREE => 32);
    ...
    filter_lst(1).objname := 'CUST_DB_GROUP_MONTH';

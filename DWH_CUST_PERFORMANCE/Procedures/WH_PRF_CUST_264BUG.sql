--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_264BUG
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_264BUG" (p_forall_limit in integer,p_success out boolean) AS
...
begin
   ...
   exception
            ...
            l_text := 'AV BASKET ERR:- '||g_average_basket||' '||g_customer_identifier||' '|| g_num_st_dy_visit||' '||g_num_wk_visit||' '||g_last_wk_visit;
   ...
   exception
            when no_data_found then
            l_text := 'VIS PAT ERR:- '||g_max_count||' '||g_customer_identifier||' '|| g_num_st_dy_visit||' '||g_num_wk_visit||' '||g_last_wk_visit;
            dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

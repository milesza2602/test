create or replace PROCEDURE WH_PRF_CUST_276U (p_forall_limit in integer,p_success out boolean) AS 
...
--MAIN INSERT STATEMENT USING UNION ALL ACCROSS ALL DATE RANGES --
   ...
   insert /*+ APPEND parallel (prf,4) */ into cust_db_business_unit_month  prf
   with bskt_det as (
      ...
      bskt_bd as (
         ...
         from     bskt_det
         group by primary_customer_identifier, business_unit_no, mnth),
      pvt as (
         select   primary_customer_identifier,
                  business_unit_no,
                  customer_no,

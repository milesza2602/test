--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_270R
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_270R" (p_forall_limit in integer,p_success out boolean) AS 
...
--   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_DB_DEPT_MONTH_4',estimate_percent=>1, DEGREE => 32);
   ...
   insert /*+ APPEND parallel (prf,16) */ into cust_db_dept_month  prf
   with        cust_union_all as
   (
   select  /*+ FULL(t1)  parallel (t1,10)  */  *   from temp_db_dept_month_1  t1
   union all
   select  /*+ FULL(t2)  parallel (t2,10)  */  *   from temp_db_dept_month_2  t2
   union all
   select  /*+ FULL(t3)  parallel (t3,10)  */  *   from temp_db_dept_month_3  t3
   union all
   select  /*+ FULL(t4)  parallel (t4,10)  */  *   from temp_db_dept_month_4  t4
   )         
   select /*+ FULL(cua)  parallel (cua,10)  */
            g_yr_00,g_mn_00,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_364E
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_364E" (p_forall_limit in integer,p_success out boolean)
as
...
-----------------------------------------------------------------------
  dbms_output.put_line('EXTRACT DIM_ITEM ');
    ...
    G_COUNT := DWH_PERFORMANCE.DWH_GENERIC_FILE_EXTRACT
       (' select * FROM DWH_CUST_PERFORMANCE.VS_EXT_KEYSTONE_DIM_ITEM ' ,
        '|' ,
        'DWH_FILES_OUT',
        'dim_item.det');
     --     'dim_item.csv');
...
-----------------------------------------------------------------------
  ...
  DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
         ...
         G_COUNT := DWH_PERFORMANCE.DWH_GENERIC_FILE_EXTRACT
       (' select * from DWH_CUST_PERFORMANCE.VS_EXT_CUST_SEGM_LIFESTYLE',
         '|',
         'DWH_FILES_OUT',
...
-----------------------------------------------------------------------   
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    G_COUNT := DWH_PERFORMANCE.DWH_GENERIC_FILE_EXTRACT
      ('  select * from DWH_CUST_PERFORMANCE.VS_EXT_CUST_CSM_VALUE_SEGMENT ',
          '|',
          'DWH_FILES_OUT',
...
-----------------------------------------------------------------------   
    ...
    G_COUNT := DWH_PERFORMANCE.DWH_GENERIC_FILE_EXTRACT
       ('  select * from DWH_CUST_PERFORMANCE.VS_EXT_CUST_WOD_TIER_MTH   ',
       '|',
       'DWH_FILES_OUT',
...
-----------------------------------------------------------------------   
  ...
  DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
    ...
    G_COUNT := DWH_PERFORMANCE.DWH_GENERIC_FILE_EXTRACT
       ...
       ('  select  * from DWH_CUST_PERFORMANCE.VS_EXT_KEYSTONE_CUSTOMER  ',
       '|',
       'DWH_FILES_OUT',
       'dim_customer.det');
     --   'dim_customer.csv');
...
dbms_output.put_line('DIM_LOCATION');
   ...
   l_text :=  'EXTRACT DIM_LOCATION STARTED AT '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    ...
    g_count := dwh_performance.dwh_generic_file_extract
       (' select * from DWH_CUST_PERFORMANCE.VS_EXT_KEYSTONE_DIM_LOCATION  ',
       '|',
       'DWH_FILES_OUT', 
       'dim_location.det');
    --   'dim_location.csv');

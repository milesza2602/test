--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_153TO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_153TO" (p_forall_limit in integer,p_success out boolean) as
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      select /*+ FULL(fnd)  parallel (fnd,2) */
       ...
       where  fnd.fin_year_no = 2015 and --fnd.fin_month_no IN (7,8,9,10,11,12)  and
       not exists
...
--    G_YEAR = 2015;
--    G-MONTH = 07;
--    g_stmt      := 'Alter table  DWH_CUST_PERFORMANCE.CUST_SEGMENT_LIFESTYLE truncate  subpartition for ('||g_year||','||g_month||') update global indexes';
--    l_text      := g_stmt;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_112U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_112U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2016
...
--
--  Maintenance:
--  November 2016 - Mariska Matthee - Swop dwh_cust_performance.apex_ap3_param_1 for apex_app_cust_01.apex_ap3_param_1
...
begin
  ...
  update /*+ PARALLEL(dim,4)*/ dwh_cust_performance.dim_customer dim
...
begin
  l_text := 'UPDATE UCOUNT_CUST_TYPE TO UCOUNT CUSTOMER ON DWH_CUST_PERFORMANCE.DIM_CUSTOMER STARTED AT '||
  ...
  l_text := 'UPDATE UCOUNT_CUST_TYPE TO UCOUNT REDEEMED ON DWH_CUST_PERFORMANCE.DIM_CUSTOMER STARTED AT '||
  to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--   and ucount_cust_type = 'UCount Customer';
    ...
    update dwh_cust_performance.dim_customer dc
     set ucount_cust_type = 'UCount Redeemed'

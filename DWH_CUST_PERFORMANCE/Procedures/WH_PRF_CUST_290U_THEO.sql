--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_290U_THEO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_290U_THEO" (p_forall_limit in integer,p_success out boolean) AS
--**************************************************************************************************
--  Date:        Aug 2017
--  Author:      Theo Filander
--  Purpose:     Create Dept Involvement Scores History Update for the last 2 years.
--  Tables:      Input  - cust_db_dept_month
--                      - dim_calendar
--                      - temp_cust_dept_item_ranking
--                      - temp_cust_dept_sale_ranking
--                      - temp_item_sales_db_involve
--               Output - cust_db_dept_month_involve
--  Packages:    constants, dwh_log, dwh_valid
--  Functions:  get_score_dept
...
--**************************************************************************************************
  ...
  type array_dept is table of number(6);
  g_dept array_dept := array_dept(); -- Initialise it

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_290U_THEO';
...
--    dbms_output.put_line('PROCESSING populate_item_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_dept_item_ranking b (fin_year_no,fin_month_no,department_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    with customer_cnt as 
                 (select /*+ Parallel(cust,8) Full(cust) */
                    ...
                    from dwh_cust_performance.temp_item_sales_db_involve
         ...
         customer_item_cnt as 
                 (select /*+ Parallel(c,8) Parallel(d,8) Full(c) Full(d) */
                    ...
                    from dwh_cust_performance.temp_item_sales_db_involve c
...
--    dbms_output.put_line('PROCESSING populate_sale_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '    
    insert /*+ Append Parallel(b,8) */ INTO DWH_CUST_PERFORMANCE.TEMP_CUST_DEPT_SALE_RANKING b (fin_year_no,fin_month_no,department_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH customer_cnt AS 
                 (SELECT /*+ Parallel(cust,8) Full(cust) */
                         fin_year_no,
                         fin_month_no,
...
l_text := 'LOAD OF CUST_DB_DEPT_MONTH_INVOLVE EX CUST_DB_DEPT_MONTH HISTORY STARTED AT '||
...
g_stmt := 'truncate table dwh_cust_performance.temp_cust_dept_sale_ranking';
execute immediate g_stmt; 

g_stmt := 'truncate table dwh_cust_performance.temp_cust_dept_item_ranking';
execute immediate g_stmt;
    ...
    for i in (     select fin_year_no,
                        ...
                        from (
                              select distinct fin_year_no,fin_month_no, this_mn_start_date,this_mn_end_date
                                from dim_calendar
                               where calendar_date < (select this_mn_start_date -1 from dim_calendar where calendar_date = to_date(trunc(sysdate)-7))
...
----------------------------------------------------------------------------------------------     
        case when g_loop = 1 then
                  ...
                  g_stmt := 'alter table dwh_cust_performance.cust_db_dept_month_involve truncate subpartition for ('||g_year_no||','||g_month_no||')';
                  l_text := g_stmt;
                  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
                  execute immediate g_stmt;
                  commit;
        ...
        execute immediate ' create table dwh_cust_performance.temp_item_sales_db_involve nologging as 
                             (select /*+ Parallel(a,8) Full(a) */ '||
                            ...
                            from dwh_cust_performance.cust_db_dept_month a
        ...
        l_text := '   TEMP_ITEM_SALES_DEBT_INVOLVE Inserted '|| SQL%ROWCOUNT;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        ...
        g_sqls(g_loop) := '
                          merge /*+ Parallel(di,8) Append */ into dwh_cust_performance.temp_cust_db_dp_month_involve di
                          using (select /*+ Parallel(ir,8) Parallel(sr,8) */ ir.department_no,
                                        ir.item_sum,
                                        sr.sales_sum,
                                        ir.ranking item_rank ,
                                        sr.ranking sale_rank
                                   from dwh_cust_performance.temp_cust_dept_item_ranking ir
                                  inner join dwh_cust_performance.temp_cust_dept_sale_ranking sr
                                     on (ir.fin_year_no               = sr.fin_year_no and
                                         ir.fin_month_no              = sr.fin_month_no and
                                         ir.department_no             = sr.department_no 
                                        )
                                  where ir.fin_year_no  = '||i.fin_year_no||' and 
                                        ir.fin_month_no = '||i.fin_month_no||' 
                                ) tt
                             on (
                                  di.department_no             = tt.department_no and
        ...
        ----------------------------------------------------------------------------------------------        
            ...
            execute immediate 'DROP TABLE TEMP_ITEM_SALES_DB_INVOLVE';
            COMMIT;
...
----------------------------------------------------------------------------------------------
    execute immediate 'create table dwh_cust_performance.temp_cust_db_dp_month_involve nologging as ( select * from dwh_cust_performance.cust_db_dept_month_involve where 1=0)';
    commit;
     
    l_text := 'LOAD DWH_CUST_PERFORMANCE.TEMP_CUST_DB_DP_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    g_stmt_main := 'insert /*+ Append Parallel(a,8) */ into dwh_cust_performance.temp_cust_db_dp_month_involve a 
                   ...
                   to_date(trunc(sysdate)) last_updated_date from dwh_cust_performance.cust_db_dept_month cma where cma.fin_year_no = '||g_year_no||' and cma.fin_month_no = '||g_month_no;

    execute immediate g_stmt_main;

    l_text := ' * DEPT INVOLVE RECORDS INSERTED INTO TEMP '|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    commit;
    
    execute immediate 'alter table dwh_cust_performance.cust_db_dept_month_involve nologging';
    commit;
    
    g_exit_tru := true;
    if g_exit_tru = true then
        l_text := '     EXITING PROCESSING ';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        p_success := true;
        return ;
    end if;
    ...
    l_text := 'LOAD DWH_CUST_PERFORMANCE.CUST_DB_DEPT_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    execute immediate 'insert /*+ Append Parallel(ci,8) */ into dwh_cust_performance.cust_db_dept_month_involve ci select /*+ Parallel(bi,8) */ * from dwh_cust_performance.temp_cust_db_dp_month_involve bi';
    
    l_text := ' * DEPT INVOLVE RECORDS INSERTED INTO CUST_DB_DEPT_MONTH_INVOLVE '|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    commit;
    ...
    ----------------------------------------------------------------------------------------------
    execute immediate 'drop table dwh_cust_performance.temp_cust_db_dp_month_involve';
    commit;    

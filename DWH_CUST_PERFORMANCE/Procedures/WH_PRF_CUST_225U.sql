--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_225U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_225U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        April 2011
--  Author:      Alastair de Wet
--  Purpose:     Create cust_db_dept_mn rollup ex temp tables
--  Tables:      Input  - temp_cust_dept_mn_1,2,3,c
--               Output - cust_db_dept_mn
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_225U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE cust_db_dept_mn EX temp_cust_dept_mn table';
...
begin
    ...
    l_text := 'LOAD OF cust_db_dept_mn   EX temp_cust_dept_mn tables STARTED AT '||
...
/*
    ...
    g_partition_name := 'CST_DPT_MN'||'_'||g_month_code;
    g_stmt           := 'alter table '||'cust_db_dept_mn'||' truncate SUBPARTITION '||g_partition_name;
   
*/
-- Do this for the table if it is purely Date based
--alter table dwh_wfs_foundation.FND_WFS_CUST_MTH_STMT drop partition
--for (to_date('06 Jul 2014','DD-MON-YYYY'))

    g_stmt      := 'Alter table  DWH_CUST_PERFORMANCE.CUST_DB_DEPT_MN truncate  subpartition for ('||g_yr_00||','||g_mn_00||')';
    l_text      := g_stmt;
...
-------------------------------
--    delete from cust_db_dept_mn
--    where fin_year_no  = g_yr_00 and
--          fin_month_no = g_mn_00;

--   execute immediate 'truncate table dwh_cust_performance.cust_db_dept_mn'; ---For testing only---
--   l_text := 'Delete from table  dwh_cust_performance.cust_db_dept_mn' ;
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--*********************************************************************************************

   insert into cust_db_dept_mn
   with cust_dept_union as
   (
   select * from temp_cust_dept_mn_1
   union all
   select * from temp_cust_dept_mn_2
   union all
   select * from temp_cust_dept_mn_3
   union all
   select * from temp_cust_dept_mn_c
   )
   ...
   group by fin_year_no,fin_month_no,primary_customer_identifier,department_no;

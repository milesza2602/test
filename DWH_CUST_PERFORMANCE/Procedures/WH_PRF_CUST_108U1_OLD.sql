--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_108U1_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_108U1_OLD" (p_forall_limit in integer,p_success out boolean) AS
...
cursor c_prf_cust_basket is
   select /*+ full(cb) parallel (cb,8) */ location_no,till_no,tran_no,tran_date,loyalty_ww_swipe_no,loyalty_ext_swipe_no,ww_online_order_no,customer_no
   from   cust_basket cb
...
------------------------------------------------------------------------
       ...
       update  /*+ INDEX ( CUST_BASKET_ITEM PK_P_CUST_BSKT_ITM) */ cust_basket_item cbi
       set     primary_customer_identifier   = g_pci,
               customer_no                   = g_tabl_customer_no 
       where   location_no                   = g_location_no
         and   till_no                       = g_till_no
...
------------------------------------------------------------------------
       ...
       update  /*+ INDEX ( CUST_BASKET_AUX PK_P_CUST_BSKT_AUX)  */  cust_basket_aux cba
       set     primary_customer_identifier   = g_pci,
               customer_no                   = g_tabl_customer_no
       where   location_no                   = g_location_no
         and   till_no                       = g_till_no

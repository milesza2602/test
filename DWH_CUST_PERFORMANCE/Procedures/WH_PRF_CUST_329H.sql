--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_329H
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_329H" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_home_and_away_sclass fact table in the performance layer
...
--               Output - cust_mart_home_and_away_sclass
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_home_and_away_sclass%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_329U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_BASKET_ITEM to HOME&AWAY SUBCLASS MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_home_and_away_sclass EX cust_basket item LEVEL STARTED AT '||
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    l_text := 'DELETE OLD RECORDS FROM HISTORY:- ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DELETE FROM cust_mart_home_and_away_sclass
    WHERE       fin_year_no      = g_yr_00
    AND         fin_quarter_no   = g_qt_00;
COMMIT;
...
insert /*+ APPEND parallel(csm,4)*/ into cust_mart_home_and_away_sclass csm
     ...
     select   /*+ FULL(bih)  parallel(bih,8) FULL(s2) parallel(s2,8) full(di) */ 
              ...
              max(di.subclass_no) subclass_no,max(di.class_no) class_no,max(di.department_no) department_no,di.sk1_subclass_no,
              sum ( case
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no between 51 and 55  
     group by primary_customer_identifier,sk1_subclass_no) 

select   /*+ FULL(home) FULL(away) parallel(8)  */ 
         g_yr_00,
         g_qt_00,
         home.location_no,
         max(home.subclass_no),max(home.class_no), max(home.department_no),
...
and      home.sk1_subclass_no            = away.sk1_subclass_no 
group by home.location_no,
         home.sk1_subclass_no
...
order by home.location_no
   ...
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','cust_mart_home_and_away_sclass',estimate_percent=>1, DEGREE => 32);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_381E
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_381E" (p_forall_limit in integer,p_success out boolean,p_run_date in date)
as
...
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_PROM_RANKED',cascade=>true, DEGREE => 8);
  ...
  l_text := 'START OF STATS COLLECT ON TEMP_CUST_SVOC_WK_TRANS ';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
begin
  ...
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_wk_trans';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_dept';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_sgrp';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_grp';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_lssitm';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_lssntrn';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_lssntn2';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_ntrnseg';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_nf_prom_ranked';

  l_text := 'BUILD TEMP TABLE STATS AFTER TRUNCATION';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_WK_TRANS',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_TOP_TAG',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_DEPT',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_SGRP',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_GRP',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_LSSITM',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_LSSNTRN',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_LSSNTN2',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_NTRNSEG',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_RANKED',cascade=>true, DEGREE => 8);
  commit;

  l_text := 'LOAD NON FOOD PROMOTIONS';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  insert into dwh_cust_performance.temp_cust_svoc_nf_prom tmp 
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_wk_trans tmp 
  ...
  with tbl as (select /*+ parallel(seg,6) full(seg) */ 
                ...
                where segment_type in ('Non-Foods')
                  ...
                  and fin_month_no = g_lss_month_no),
       prm as (select distinct department_no
                 from dwh_cust_performance.temp_cust_svoc_nf_prom tmp),
       trns as (select /*+ parallel(cbi,6) parallel(dimi,6) full(cbi) full(dimi) */ 
                       cbi.primary_customer_identifier,
                       dimi.department_no,
                       dimi.subgroup_no,
  ...
  insert into dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tmp 
  ...
  select prom_no,
    ...
    from (select distinct prom_no,department_no,department_name,null subgroup_no,null subgroup_name,null group_no,null group_name,null business_unit_no,null business_unit_name
            from dwh_cust_performance.temp_cust_svoc_nf_prom prm
           where prom_no in (select prom_no
                               from (select prom_no,department_no
                                       from dwh_cust_performance.temp_cust_svoc_nf_prom
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_dept tmp 
    (primary_customer_identifier,prom_no,item_qty)
  ...
  select /*+ parallel(trn,6) full(trn)*/
    ...
    from (select /*+ parallel(tt,6) full(tt) */
                 ...
                 department_no,
                 sum(tt.item_qty) item_qty
            from dwh_cust_performance.temp_cust_svoc_nf_wk_trans tt
           group by primary_customer_identifier,department_no) trn
   inner join (select /*+ full(prm) */
                      prom_no,department_no
                 from prm) tp 
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_ranked tmp 
    ...
    from (select /*+ parallel(distprom,6) full(distprom) */
            ...
            from (select /*+ parallel(tagprom,6) full(tagprom) */
                    ...
                    from (select /*+ parallel(allprom,6) full(allprom) */
                            ...
                            from (select /*+ parallel(tmp1,6) full(tmp1) */
                                         primary_customer_identifier,prom_no,item_qty,1 precedence
                                    from dwh_cust_performance.temp_cust_svoc_nf_prom_dept tmp1
                                 ) allprom
                            left join dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tdet
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_sgrp tmp 
    (primary_customer_identifier,prom_no,item_qty)
  with tbl as (select /*+ parallel(rnkd,6) full(rnkd) */
                      primary_customer_identifier
                 from dwh_cust_performance.temp_cust_svoc_nf_prom_ranked rnkd
                where rank_no = 2),
            ...
            from (select /*+ parallel(tagprom,6) full(tagprom) */
                    ...
                    from (select /*+ parallel(allprom,6) full(allprom) */
                            ...
                            from (select /*+ parallel(tmp1,6) full(tmp1) */
                                  ...
                                  select /*+ parallel(tmp2,6) full(tmp2) */
                                         primary_customer_identifier,prom_no,item_qty,2 precedence
                                    from dwh_cust_performance.temp_cust_svoc_nf_prom_sgrp tmp2
                                 ) allprom
                            left join dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tdet
                              on allprom.prom_no = tdet.prom_no
                         ) tagprom
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_grp tmp 
    (primary_customer_identifier,prom_no,item_qty)
  with tbl as (select /*+ parallel(rnkd,6) full(rnkd) */
                      primary_customer_identifier
                 from dwh_cust_performance.temp_cust_svoc_nf_prom_ranked rnkd
...
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_RANKED',cascade=>true, DEGREE => 8);
            ...
            from (select /*+ parallel(tagprom,6) full(tagprom) */
                    ...
                    from (select /*+ parallel(allprom,6) full(allprom) */
                            ...
                            from (select /*+ parallel(tmp1,6) full(tmp1) */
                                  ...
                                  select /*+ parallel(tmp3,6) full(tmp3) */
                                         primary_customer_identifier,prom_no,item_qty,3 precedence
                                    from dwh_cust_performance.temp_cust_svoc_nf_prom_grp tmp3
                                 ) allprom
                            left join dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tdet
                              on allprom.prom_no = tdet.prom_no
                         ) tagprom
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_lssitm tmp 
    (primary_customer_identifier,prom_no,item_qty)
  with tbl as (select /*+ parallel(rnkd,6) full(rnkd) */
                      primary_customer_identifier
                 from dwh_cust_performance.temp_cust_svoc_nf_prom_ranked rnkd
  ...
  select /*+ parallel(cust,6) parallel(seg,6) full(cust) full(seg)*/
    ...
    from (select /*+ parallel(trn,6) full(trn) */
                 distinct primary_customer_identifier,segment_no
            from dwh_cust_performance.temp_cust_svoc_nf_wk_trans trn
           where trn.primary_customer_identifier not in (select /*+ full(tbl) */
                                                               primary_customer_identifier
                                                          from tbl)
         ) cust -- non foods only
            ...
            from (select /*+ parallel(tagprom,6) full(tagprom) */
                    ...
                    from (select /*+ parallel(allprom,6) full(allprom) */
                            ...
                            from (select /*+ parallel(tmp1,6) full(tmp1) */
                                  ...
                                  select /*+ parallel(tmp4,6) full(tmp4) */
                                         primary_customer_identifier,prom_no,item_qty,4 precedence
                                    from dwh_cust_performance.temp_cust_svoc_nf_prom_lssitm tmp4
                                 ) allprom
                            left join dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tdet
                              on allprom.prom_no = tdet.prom_no
                         ) tagprom
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_lssntrn tmp 
    (segment_no,prom_no,item_qty)
  with prm as (select distinct prom_no,department_no
                 from dwh_cust_performance.temp_cust_svoc_nf_prom prm)            
  select segment_no,
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_lssntn2 tmp 
    (primary_customer_identifier,prom_no,item_qty)
  with tbl as (select /*+ parallel(rnkd,6) full(rnkd) */
                      primary_customer_identifier
                 from dwh_cust_performance.temp_cust_svoc_nf_prom_ranked rnkd
       ...
       lssprm as (select segment_no,
                         prom_no,
                         item_qty
                    from dwh_cust_performance.temp_cust_svoc_nf_prom_lssntrn)
  select /*+ parallel(cust,6) parallel(promo,6) full(cust) full(promo)*/
         cust.primary_customer_identifier,
         promo.prom_no,
         promo.item_qty
...
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_NF_PROM_RANKED',cascade=>true, DEGREE => 8);
            ...
            from (select /*+ parallel(tagprom,6) full(tagprom) */
                    ...
                    from (select /*+ parallel(allprom,6) full(allprom) */
                            ...
                            from (select /*+ parallel(tmp1,6) full(tmp1) */
                                  ...
                                  select /*+ parallel(tmp5,6) full(tmp5) */
                                         primary_customer_identifier,prom_no,item_qty,5 precedence
                                    from dwh_cust_performance.temp_cust_svoc_nf_prom_lssntn2 tmp5
                                 ) allprom
                            left join dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tdet
                              on allprom.prom_no = tdet.prom_no
                         ) tagprom
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_nf_prom_ntrnseg tmp 
    (primary_customer_identifier,prom_no,item_qty)
  with tbl as (select /*+ parallel(rnkd,6) full(rnkd) */
                      primary_customer_identifier
                 from dwh_cust_performance.temp_cust_svoc_nf_prom_ranked rnkd
            ...
            from (select /*+ parallel(tagprom,6) full(tagprom) */
                    ...
                    from (select /*+ parallel(allprom,6) full(allprom) */
                            ...
                            from (select /*+ parallel(tmp1,6) full(tmp1) */
                                  ...
                                  select /*+ parallel(tmp6,6) full(tmp6) */
                                         primary_customer_identifier,prom_no,item_qty,6 precedence
                                    from dwh_cust_performance.temp_cust_svoc_nf_prom_ntrnseg tmp6
                                 ) allprom
                            left join dwh_cust_performance.temp_cust_svoc_nf_prom_top_tag tdet
                              on allprom.prom_no = tdet.prom_no
                         ) tagprom
...
begin
  ...
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spf_wk_trans';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spnf_wk_trans';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spf_lsssgrp';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spnf_lsssgrp';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spf_ranked';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spnf_ranked';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spfoc_ranked';
  dbms_lock.sleep(0.25);
  execute immediate 'TRUNCATE TABLE dwh_cust_performance.temp_cust_svoc_spnfoc_ranked';

  l_text := 'BUILD TEMP TABLE STATS AFTER TRUNCATION';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPF_WK_TRANS',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPNF_WK_TRANS',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPF_LSSSGRP',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPNF_LSSSGRP',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPF_RANKED',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPNF_RANKED',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPFOC_RANKED',cascade=>true, DEGREE => 8);
--  dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_SVOC_SPNFOC_RANKED',cascade=>true, DEGREE => 8);
  commit;

  l_text := 'LOAD FOOD TRANSACTIONS FOR SUGGESTED PRODUCTS';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spf_wk_trans tmp 
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spnf_wk_trans tmp 
      (primary_customer_identifier,segment_no,subgroup_no,subgroup_name,item_qty)
  with tbl as (select /*+ parallel(seg,6) full(seg) */ 
                      primary_customer_identifier,
                      segment_no
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spf_lsssgrp tmp 
  ...
  select /*+ parallel(cust,6) parallel(seg,6) full(cust) full(seg)*/
   ...
   inner join (select /*+ parallel(tt,6) full(tt) */
                      ...
                      subgroup_name,
                      sum(tt.item_qty) item_qty
                 from dwh_cust_performance.temp_cust_svoc_spf_wk_trans tt
                where segment_no is not null
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spnf_lsssgrp tmp 
    (primary_customer_identifier,subgroup_no,subgroup_name,item_qty)
  with lss as (select /*+ parallel(seg,6) full(seg) */ 
                      primary_customer_identifier,
                      segment_no
  ...
  select /*+ parallel(cust,6) parallel(seg,6) full(cust) full(seg)*/
   ...
   inner join (select /*+ parallel(tt,6) full(tt) */
                      ...
                      subgroup_name,
                      sum(tt.item_qty) item_qty
                 from dwh_cust_performance.temp_cust_svoc_spnf_wk_trans tt
                where segment_no is not null
                group by segment_no,subgroup_no,subgroup_name) seg
      on cust.segment_no = seg.segment_no;
  commit; 
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spf_ranked tmp 
  ...
  select /*+ parallel(rankprom,6) full(rankprom) */
    ...
    from (select /*+ parallel(allprom,6) full(allprom) */
            ...
            from (select /*+ parallel(tmp1,6) full(tmp1) */
                    ...
                    from dwh_cust_performance.temp_cust_svoc_spf_wk_trans tmp1
                  union all
                  select /*+ parallel(tmp2,6) full(tmp2) */
                         primary_customer_identifier,subgroup_no,subgroup_name,item_qty,2 precedence
                    from dwh_cust_performance.temp_cust_svoc_spf_lsssgrp tmp2
                 ) allprom
         ) rankprom
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spnf_ranked tmp 
    (primary_customer_identifier,subgroup_name)
  select /*+ parallel(rankprom,6) full(rankprom) */
         rankprom.primary_customer_identifier,
         rankprom.subgroup_name
    from (select /*+ parallel(allprom,6) full(allprom) */
            ...
            from (select /*+ parallel(tmp1,6) full(tmp1) */
                         primary_customer_identifier,subgroup_no,subgroup_name,item_qty,1 precedence
                    from dwh_cust_performance.temp_cust_svoc_spnf_wk_trans tmp1
                  union all
                  select /*+ parallel(tmp2,6) full(tmp2) */
                         primary_customer_identifier,subgroup_no,subgroup_name,item_qty,2 precedence
                    from dwh_cust_performance.temp_cust_svoc_spnf_lsssgrp tmp2
                 ) allprom
         ) rankprom
   where rankprom.rank_no = 1;
  commit;
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spfoc_ranked tmp 
    (primary_customer_identifier,subgroup_name)
  select /*+ parallel(rankprom,6) full(rankprom) */
         rankprom.primary_customer_identifier,
         rankprom.subgroup_name
    from (select /*+ parallel(allprom,6) full(allprom) */
            ...
            from ((select /*+ parallel(tmp1,6) full(tmp1) */
                          primary_customer_identifier,subgroup_no,subgroup_name,item_qty,1 precedence
                     from dwh_cust_performance.temp_cust_svoc_spf_lsssgrp tmp1
                    where (primary_customer_identifier,subgroup_no) in (select /*+ parallel(lss,6) full(lss) */
                                                                               primary_customer_identifier,subgroup_no
                                                                          from dwh_cust_performance.temp_cust_svoc_spf_lsssgrp lss
                                                                        minus 
                                                                        select /*+ parallel(trn,6) full(trn) */
                                                                               primary_customer_identifier,subgroup_no
                                                                          from dwh_cust_performance.temp_cust_svoc_spf_wk_trans trn2))
                  union all
                  select /*+ parallel(tmp3,6) full(tmp3) */
                         primary_customer_identifier,subgroup_no,subgroup_name,item_qty,2 precedence
                    from dwh_cust_performance.temp_cust_svoc_spf_wk_trans tmp3
                   where (primary_customer_identifier,subgroup_no) in (select /*+ parallel(tt,6) full(tt) */
                                                                        ...
                                                                        where (primary_customer_identifier,item_qty) in (select /*+ parallel(trn,6) full(trn) */
                                                                                                                           ...
                                                                                                                           from dwh_cust_performance.temp_cust_svoc_spf_wk_trans trn
                                                                                                                          where segment_no is not null 
  ...
  insert /*+ APPEND parallel(tmp,6) */ into dwh_cust_performance.temp_cust_svoc_spnfoc_ranked tmp 
    (primary_customer_identifier,subgroup_name)
  select /*+ parallel(rankprom,6) full(rankprom) */
         rankprom.primary_customer_identifier,
         rankprom.subgroup_name
    from (select /*+ parallel(allprom,6) full(allprom) */
            ...
            from ((select /*+ parallel(tmp1,6) full(tmp1) */
                          primary_customer_identifier,subgroup_no,subgroup_name,item_qty,1 precedence
                     from dwh_cust_performance.temp_cust_svoc_spnf_lsssgrp tmp1
                    where (primary_customer_identifier,subgroup_no) in (select /*+ parallel(lss,6) full(lss) */
                                                                               primary_customer_identifier,subgroup_no
                                                                          from dwh_cust_performance.temp_cust_svoc_spnf_lsssgrp lss
                                                                        minus 
                                                                        select /*+ parallel(trn,6) full(trn) */
                                                                               primary_customer_identifier,subgroup_no
                                                                          from dwh_cust_performance.temp_cust_svoc_spnf_wk_trans trn2))
                  union all
                  select /*+ parallel(tmp3,6) full(tmp3) */
                         primary_customer_identifier,subgroup_no,subgroup_name,item_qty,2 precedence
                    from dwh_cust_performance.temp_cust_svoc_spnf_wk_trans tmp3
                   where (primary_customer_identifier,subgroup_no) in (select /*+ parallel(tt,6) full(tt) */
                                                                              primary_customer_identifier,max(subgroup_no) subgroup_no
                                                                         from dwh_cust_performance.temp_cust_svoc_spnf_wk_trans tt 
                                                                        where (primary_customer_identifier,item_qty) in (select /*+ parallel(trn,6) full(trn) */
                                                                                                                                primary_customer_identifier,min(item_qty) item_qty
                                                                                                                           from dwh_cust_performance.temp_cust_svoc_spnf_wk_trans trn
                                                                                                                          where segment_no is not null 
                                                                                                                          group by primary_customer_identifier)
                                                                        group by primary_customer_identifier)
                 ) allprom
...
begin
    ...
    l_text := 'BUILD OF SVOC DAILY DATA TO OUT_DWH_SVOC_DAILY STARTED AT '||
...
--**************************************************************************************************
    ...
    execute immediate 'TRUNCATE TABLE "DWH_CUST_PERFORMANCE"."OUT_DWH_SVOC_DAILY"'; 
    dbms_stats.gather_table_stats ('DWH_CUST_PERFORMANCE','OUT_DWH_SVOC_DAILY',cascade=>true, DEGREE => 8);
    commit;

    l_text := 'POPULATE TABLE OUT_DWH_SVOC_DAILY.' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    insert /*+ parallel(OUT_DWH_SVOC_DAILY ,6) */ into DWH_CUST_PERFORMANCE.OUT_DWH_SVOC_DAILY
    with promo_details as (select /*+ Parallel(prm,6) full(prm) */
         ...
         nf_promo_details as (select /*+ Parallel(prm,6) full(prm) */
                                     ...
                                     prm.rank_no
                                from dwh_cust_performance.temp_cust_svoc_nf_prom_ranked prm),
         spf_details as (select /*+ Parallel(sps,6) full(sps) */
                                primary_customer_identifier,
                                subgroup_name
                           from dwh_cust_performance.temp_cust_svoc_spf_ranked sps),
         spnf_details as (select /*+ Parallel(sps,6) full(sps) */
                                 primary_customer_identifier,
                                 subgroup_name
                            from dwh_cust_performance.temp_cust_svoc_spnf_ranked sps),
         spfoc_details as (select /*+ Parallel(sps,6) full(sps) */
                                  primary_customer_identifier,
                                  subgroup_name
                             from dwh_cust_performance.temp_cust_svoc_spfoc_ranked sps),
         spnfoc_details as (select /*+ Parallel(sps,6) full(sps) */
                                   primary_customer_identifier,
                                   subgroup_name
                              from dwh_cust_performance.temp_cust_svoc_spnfoc_ranked sps)

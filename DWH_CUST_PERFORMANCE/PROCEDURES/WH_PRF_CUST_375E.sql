--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_375E
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_375E" 
...
--**************************************************************************************************
    ...
    g_count := dwh_performance.dwh_generic_file_extract(
         q'[select /*+ materialize cardinality (b,300000) Full(b) */ 
                   sk1_supplier_no, 
                   supplier_no, 
                   supplier_name,
                   parent_supplier_no,
                   last_active_date
              from dwh_performance.dim_supplier b
             where supplier_no in (select /*+ materialize cardinality (a,300000) Full(a) */ 
                                          primary_supplier_no 
                                     from dwh_performance.dim_item a

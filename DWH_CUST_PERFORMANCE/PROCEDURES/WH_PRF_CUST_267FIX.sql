--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_267FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_267FIX" (p_forall_limit in integer,p_success out boolean) AS
...
--
--   !!!!!!!!!!!!!!!!!!!!!!!THIS IS THE FOODS PROGRAM !!!!!!!!!!!!!!!!!!!!!!!
--
--  Maintenance:
--  HardCode Value 115,184,185,186,339,340 needs to change regularly as inflation happens - address_variables
--
--  Naming conventions:
--  g_  -  Global variable
...
--  c_  -  Prefix to cursor
--**************************************************************************************************
--g_avg_spend_... changes once a year to allow for inflation.
...
--g_avg_spend_1        integer       := 130;  -- Implemented this threshold in July 2015
--g_avg_spend_2        integer       := 210;  -- Implemented this threshold in July 2015
--g_avg_spend_3        integer       := 385;  -- Implemented this threshold in July 2015

--g_avg_spend_1        integer       := 140;  -- Implemented this threshold in July 2016
--g_avg_spend_2        integer       := 225;  -- Implemented this threshold in July 2016
--g_avg_spend_3        integer       := 415;  -- Implemented this threshold in July 2016

g_avg_spend_1        integer       := 150;  -- Implemented this threshold in July 2017
g_avg_spend_2        integer       := 245;  -- Implemented this threshold in July 2017
g_avg_spend_3        integer       := 450;  -- Implemented this threshold in July 2017
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_267U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE CUST_CSM_VALUE_SEGMENT EX BASKET_ITEM - FOODS';
...
begin
   ...
   g_rec_out.avg_value_per_visit             := g_rec_out.total_value /g_rec_out.num_visit ;
      ...
      if g_rec_out.num_visit <= 4 and g_rec_out.avg_value_per_visit < g_avg_spend_1 then
         g_rec_out.current_seg := 6;
      end if;

      if g_rec_out.num_visit <= 4
      and (g_rec_out.avg_value_per_visit >= g_avg_spend_1 and g_rec_out.avg_value_per_visit <  g_avg_spend_2) then
         g_rec_out.current_seg := 5;
      end if;

      if g_rec_out.num_visit <= 2
      and (g_rec_out.avg_value_per_visit >= g_avg_spend_2 and g_rec_out.avg_value_per_visit < g_avg_spend_3 ) then
         g_rec_out.current_seg := 5;
      end if;
      ...
      if g_rec_out.num_visit <= 4 and g_rec_out.avg_value_per_visit >= g_avg_spend_3 then
         g_rec_out.current_seg := 4;
      end if;

      if g_rec_out.num_visit > 4 and g_rec_out.avg_value_per_visit <= g_avg_spend_2 then

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_384E
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_384E" (p_forall_limit in integer,p_success out boolean,p_run_date in date)
as
--**************************************************************************************************
--  Date:        APR 2016
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table "DWH_CUST_PERFORMANCE"."OUT_DWH_SVOC_DAILY_SK"';
    commit;

--    Do not update stats after truncate.
--    l_text := 'UPDATE STATS ON OUT_DWH_SVOC_DAILY_SK TABLES_SK';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','OUT_DWH_SVOC_DAILY_SK',estimate_percent=>1, DEGREE => 32);
    ...
    insert /*+ Parallel(ts,6)  */ into dwh_cust_performance.temp_subscriber_ucount ts
    select  distinct
           subscriber_key,
      ...
      from (
            select /*+ Parallel(ci,6) Full(ci) Parallel(sm,6) Full(sm) */
                   subscriber_key,
                   ucount_cust_type
              from dwh_cust_performance.out_dwh_svoc_daily ci 
        ...
        DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_SUBSCRIBER_UCOUNT',estimate_percent=>1, DEGREE => 32);
        commit;

        l_text := 'Populate OUT_DWH_SVOC_DAILY_SK. ' ;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

insert into DWH_CUST_PERFORMANCE.OUT_DWH_SVOC_DAILY_SK sk
    select 
           uc.subscriber_key,
           uc.ucount_cust_type,
      ...
      from dwh_cust_performance.temp_subscriber_ucount uc
     inner join dwh_cust_foundation.fnd_svoc_mapping sm on sm.subscriber_key = uc.subscriber_key
     inner join dwh_cust_performance.out_dwh_svoc_daily ci on ci.primary_customer_identifier = sm.source_key
     where sm.source = 'C2'

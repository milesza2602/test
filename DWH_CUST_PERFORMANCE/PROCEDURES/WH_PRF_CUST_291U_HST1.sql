--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_291U_HST1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_291U_HST1" (p_forall_limit in integer,p_success out boolean) AS
  g_year_no             NUMBER;
  g_month_no            NUMBER;
  g_prev_mn_date        DATE;
...
insert /*+ APPEND Parallel(a,8) */ into DWH_CUST_PERFORMANCE.TEMP_CUST_SGRP_SALE_RANKING (fin_year_no,fin_month_no,subgroup_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    ...
    insert /*+ APPEND Parallel(a,8) */ into DWH_CUST_PERFORMANCE.TEMP_CUST_SGRP_ITEM_RANKING (fin_year_no,fin_month_no,subgroup_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                        g_year_no fin_year_no,
                        g_month_no fin_month_no,
                        subgroup_no,

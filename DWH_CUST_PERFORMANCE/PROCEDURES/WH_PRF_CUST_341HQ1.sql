--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_341HQ1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_341HQ1" (p_forall_limit in integer,p_success out boolean) as
...
from    TEMP_DB_DEPT_INVOLVE  invl
             ),
...
select  /*+ FULL(a)  FULL(dc)  FULL(dcr)  FULL(prf) FULL(hbt) FULL(ttm) FULL(inv) parallel (8) */
         g_yr_00,
         g_qt_00,
         a.department_no,
...
on       a.primary_customer_identifier = inv.primary_customer_identifier and 
         a.department_no               = inv.department_no
-- END ADD INVOLVEMENT SCORES         
left outer join

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_108LOD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_108LOD" (p_forall_limit in integer,p_success out boolean) AS
...
MERGE  INTO dim_customer cust
   ...
   and    cb.tran_date    BETWEEN '17 SEP 2017' and '20 SEP 2017'            
   and    cb.customer_no  is not null
   group by cb.customer_no
         ) mer_rec
   ON    (  cust.	customer_no	     =	mer_rec.	customer_no )

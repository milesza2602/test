--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_293U_THEO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_293U_THEO" (p_forall_limit in integer,p_success out boolean) AS
--**************************************************************************************************
--  Date:        Aug 2017
--  Author:      Theo Filander
--  Purpose:     Create Business Unit Involvement Scores History Update for the last 2 years.
--  Tables:      Input  - cust_db_business_unit_month;
--                      - dim_calendar
--                      - temp_cust_bu_item_ranking
--                      - temp_cust_bu_sale_ranking
--                      - temp_item_sales_bu_involve
--               Output - cust_db_bu_month_involve
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_293U_THEO';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CUST_DB_BU_MONTH_INVOLVE EX CUST_DB_BUSINESS_UNIT_MONTH';
...
--    dbms_output.put_line('PROCESSING populate_item_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_bu_item_ranking b (fin_year_no,fin_month_no,business_unit_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    with customer_cnt as 
                 (select /*+ Parallel(cust,8) Full(cust) */
                    ...
                    from dwh_cust_performance.temp_item_sales_bu_involve
                   group by fin_year_no, fin_month_no, business_unit_no
                 ),
         customer_item_cnt as 
                 (select /*+ Parallel(c,8) Parallel(d,8) Full(c) Full(d) */
                    ...
                    from dwh_cust_performance.temp_item_sales_bu_involve c
                   inner join customer_cnt d
                      on c.fin_year_no = d.fin_year_no and 
                         c.fin_month_no = d.fin_month_no and 
                         c.business_unit_no = d.business_unit_no
                   group by c.fin_year_no,c.fin_month_no,c.business_unit_no,c.item_sum,d.customer_cnt
                 ),
         customer_item_ranking as 
                 (select /*+ Parallel(a,8) Full(a) */
                         ...
                         a.customer_cnt,
                         a.customer_item_cnt,
                         sum(a.customer_item_cnt) over (partition by a.fin_year_no, a.fin_month_no, a.business_unit_no order by a.fin_year_no, a.fin_month_no, a.business_unit_no, a.item_sum rows between unbounded preceding and current row) - a.customer_item_cnt as diff_customer_item_cnt
...
--    dbms_output.put_line('PROCESSING populate_sale_rank '||f_year_no||' - '||f_month_no);
    f_stmt := '    
    insert /*+ Append Parallel(b,8) */ into dwh_cust_performance.temp_cust_bu_sale_ranking (fin_year_no,fin_month_no,business_unit_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH customer_cnt AS 
                 (select /*+ Parallel(cust,8) Full(cust) */
                         fin_year_no,
                         fin_month_no,
                 ...
                 (select /*+ Parallel(c,8) Parallel(d,8) Full(c) Full(d) */
                   ...
                   inner join customer_cnt d
                      on c.fin_year_no = d.fin_year_no and 
                         c.fin_month_no = d.fin_month_no and 
                         c.business_unit_no = d.business_unit_no
                   group by c.fin_year_no,c.fin_month_no,c.business_unit_no,c.sales_sum,d.customer_cnt
                 ),
         customer_item_ranking as 
                 (select /*+ Parallel(a,8) Full(a) */
                         a.fin_year_no,
                         ...
                         a.customer_cnt,
                         a.customer_item_cnt,
                         sum(a.customer_item_cnt) over (partition by a.fin_year_no, a.fin_month_no, a.business_unit_no order by a.fin_year_no, a.fin_month_no, a.business_unit_no, a.sales_sum rows between unbounded preceding and current row) - a.customer_item_cnt as diff_customer_item_cnt
                    from customer_item_cnt a 
                 )  
    select /*+ Parallel(a,8) Full(a) */
           '||f_year_no||','||f_month_no||',
...
g_stmt := 'alter session enable parallel dml';
execute immediate g_stmt; 

l_text := dwh_cust_constants.vc_log_draw_line;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

l_text := 'LOAD OF CUST_DB_BU_MONTH_INVOLVE EX CUST_DB_BUSINESS_UNIT_MONTH STARTED AT '||
...
--g_stmt := 'truncate table dwh_cust_performance.temp_cust_bu_sale_ranking';
--execute immediate g_stmt; 

--g_stmt := 'truncate table dwh_cust_performance.temp_cust_bu_item_ranking';
--execute immediate g_stmt;
...
--                  g_stmt := 'alter table dwh_cust_performance.cust_db_bu_month_involve truncate subpartition for ('||g_year_no||','||g_month_no||')';
--                  l_text := g_stmt;
--                  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--                  execute immediate g_stmt;
--                  commit;
...
--        execute immediate ' create table dwh_cust_performance.temp_item_sales_bu_involve as 
--                             (select /*+ Parallel(a,8) Full(a) */ '||
...
--                            from dwh_cust_performance.cust_db_business_unit_month a
...
--        l_text := '   TEMP_ITEM_SALES_BU_INVOLVE Inserted '|| SQL%ROWCOUNT;
--        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        ...
        array(g_loop) := '
                          merge /*+ Parallel(di,8) */ into dwh_cust_performance.temp_cust_db_bu_month_involve di
                          using (select /*+ Parallel(ir,8) Parallel(sr,8) */
                                        ir.business_unit_no,
                                        ir.item_sum,
                                        sr.sales_sum,
                                        ir.ranking item_rank ,
                                        sr.ranking sale_rank
                                   from dwh_cust_performance.temp_cust_bu_item_ranking ir
                                  inner join dwh_cust_performance.temp_cust_bu_sale_ranking sr
                                     on (ir.fin_year_no  = sr.fin_year_no and
                                         ir.fin_month_no = sr.fin_month_no and
                                         ir.business_unit_no  = sr.business_unit_no 
                                        )
                                  where ir.fin_year_no  = '||i.fin_year_no||' and 
                                        ir.fin_month_no = '||i.fin_month_no||' 
                                ) tt
                             on (
                                  di.business_unit_no           = tt.business_unit_no and
...
--              dbms_output.put_line(array(g_loop)||';');
...
--            execute immediate 'DROP TABLE TEMP_ITEM_SALES_BU_INVOLVE';
--            COMMIT;

     end loop;


    l_text := ' UPDATE STATISTICS ON TEMP_CUST_BU_ITEM_RANKING';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 

--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_BU_ITEM_RANKING',estimate_percent=>1, DEGREE => 32);

    l_text := ' UPDATE STATISTICS ON TEMP_CUST_BU_SALE_RANKING';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','TEMP_CUST_BU_SALE_RANKING',estimate_percent=>1, DEGREE => 32);
----------------------------------------------------------------------------------------------
--  Create the involve table.
----------------------------------------------------------------------------------------------  
    l_text := 'LOAD DWH_CUST_PERFORMANCE.TEMP_CUST_DB_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    g_stmt_main := 'insert /*+ Append Parallel(a,8) */ into dwh_cust_performance.temp_cust_db_bu_month_involve a 
                   ...
                   to_date(trunc(sysdate)) last_updated_date from dwh_cust_performance.cust_db_business_unit_month cma where cma.fin_year_no = '||g_year_no||' and cma.fin_month_no = '||g_month_no;

--    g_stmt_main := 'create table dwh_cust_performance.temp_cust_db_bu_month_involve as ('||
--                   g_stmt_main||
--                   ',NULL num_item_sum_yr3_mn02,NULL sales_sum_yr3_mn02,NULL involvement_score_yr3_mn02,NULL num_item_sum_yr3_mn03,NULL sales_sum_yr3_mn03,NULL involvement_score_yr3_mn03,'||
--                   ' NULL num_item_sum_yr3_mn04,NULL sales_sum_yr3_mn04,NULL involvement_score_yr3_mn04,NULL num_item_sum_yr3_mn05,NULL sales_sum_yr3_mn05,NULL involvement_score_yr3_mn05,'||
--                   ' NULL num_item_sum_yr3_mn06,NULL sales_sum_yr3_mn06,NULL involvement_score_yr3_mn06,NULL num_item_sum_yr3_mn07,NULL sales_sum_yr3_mn07,NULL involvement_score_yr3_mn07,'||
--                   ' NULL num_item_sum_yr3_mn08,NULL sales_sum_yr3_mn08,NULL involvement_score_yr3_mn08,NULL num_item_sum_yr3_mn09,NULL sales_sum_yr3_mn09,NULL involvement_score_yr3_mn09,'||
--                   ' NULL num_item_sum_yr3_mn10,NULL sales_sum_yr3_mn10,NULL involvement_score_yr3_mn10,NULL num_item_sum_yr3_mn11,NULL sales_sum_yr3_mn11,NULL involvement_score_yr3_mn11,'||
--                   ' NULL num_item_sum_yr3_mn12,NULL sales_sum_yr3_mn12,NULL involvement_score_yr3_mn12,'||
--                   'to_date(trunc(sysdate)) last_updated_date '||
--                   'from dwh_cust_performance.cust_db_business_unit_month cma '||
--                   'where cma.fin_year_no = '||g_year_no||' and cma.fin_month_no = '||g_month_no||')';

--    dbms_output.put_line(g_stmt_main);
--    g_stmt_main := 'merge /*+ Append Parallel(di,8) */ into dwh_cust_performance.cust_db_bu_month_involve di
--                    using ('||g_stmt_main||') ti 
--                       on (di.fin_year_no  = ti.fin_year_no and 
--                           di.fin_month_no = ti.fin_month_no and  
--                           di.business_unit_no  = ti.business_unit_no and 
...
--    dbms_output.put_line('MAIN SQL STATEMENT: '||LENGTH(g_stmt_main)||g_stmt_main);
    ...
    l_text := ' * BUSINESS UNIT INVOLVE RECORDS INSERTED INTO TEMP'|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    COMMIT;

--    l_text := ' UPDATE STATISTICS ON CUST_DB_BU_MONTH_INVOLVE';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','CUST_DB_BU_MONTH_INVOLVE',estimate_percent=>1, DEGREE => 32);
--    COMMIT;
    ...
    l_text := 'LOAD DWH_CUST_PERFORMANCE.CUST_DB_BU_MONTH_INVOLVE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    execute immediate 'insert /*+ Parallel(ci,8) */ cust_performance.cust_db_bu_month_involve ci select /*+ Parallel(bi,8) */ * from dwh_cust_performance.temp_cust_db_bu_month_involve bi)';
    
    l_text := ' * BUSINESS UNIT INVOLVE RECORDS INSERTED INTO CUST_DB_BU_MONTH_INVOLVE'|| SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);        
    COMMIT;
--    l_text := ' UPDATE STATISTICS ON CUST_DB_BU_MONTH_INVOLVE';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
--    DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','CUST_DB_BU_MONTH_INVOLVE',estimate_percent=>1, DEGREE => 32);
    ...
    ----------------------------------------------------------------------------------------------
    --  Cleanup TEMP_CUST_DB_MONTH_INVOLVE
    ----------------------------------------------------------------------------------------------
    execute immediate 'drop table TEMP_CUST_DB_MONTH_INVOLVE';

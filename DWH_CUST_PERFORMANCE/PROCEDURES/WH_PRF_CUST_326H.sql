--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_326H
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_326H" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_home_and_away_sgrp fact table in the performance layer
...
--               Output - cust_mart_home_and_away_sgrp
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_home_and_away_sgrp%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_326U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_BASKET_ITEM to HOME&AWAY SUBGROUP MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_home_and_away_sgrp EX cust_basket item LEVEL STARTED AT '||
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    DELETE FROM cust_mart_home_and_away_sgrp
...
insert /*+ APPEND parallel(csm,4)*/ into cust_mart_home_and_away_sgrp csm
     ...
     group by bih.primary_customer_identifier, di.subgroup_no),          
...
and      home.subgroup_no            = away.subgroup_no 
...
order by home.location_no,
   ...
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','cust_mart_home_and_away_sgrp',estimate_percent=>1, DEGREE => 32);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_610U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_610U" (p_forall_limit in integer,p_success out boolean) AS 
...
--**************************************************************************************************
   ...
   (
    ...
    FROM   DWH_CUST_FOUNDATION.FND_CUSTOMER_MASTER cm
...
--*****************************************************************************************************
MERGE  /*+ parallel (DC,4) */ INTO dwh_cust_PERFORMANCE.DIM_CUSTOMER_MASTER DC USING
(
   SELECT /*+ FULL(CST) parallel(CST,12) */ 
          CST.MASTER_SUBSCRIBER_KEY,
          CST.DERIVED_RACE,
          RR.RACE
   FROM   DWH_CUST_PERFORMANCE.DIM_CUSTOMER_MASTER CST
   JOIN   DWH_CUST_FOUNDATION.FND_CUST_RACE_REF RR 

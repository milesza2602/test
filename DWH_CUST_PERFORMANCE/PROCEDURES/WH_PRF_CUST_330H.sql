--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_330H
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_PERFORMANCE"."WH_PRF_CUST_330H" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_home_and_away_iteml1 fact table in the performance layer
...
--               Output - cust_mart_home_and_away_iteml1
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_home_and_away_iteml1%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_330U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_BASKET_ITEM to HOME&AWAY ITEML1 MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_home_and_away_iteml1 EX cust_basket item LEVEL STARTED AT '||
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    DELETE FROM cust_mart_home_and_away_iteml1
...
insert /*+ APPEND parallel(csm,6)*/ into cust_mart_home_and_away_iteml1 csm

     with         home as  (
     select   /*+ FULL(bih)  parallel(bih,10) FULL(s2) parallel(s2,10) full(di) */ 
     ...
     group by bih.primary_customer_identifier, di.item_level1_no),          

              away as  (
     select   /*+ FULL(bi) parallel (bi,10)  full(di) */ 
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no between 51 and 55  
     group by primary_customer_identifier,item_level1_no) 
...
and      home.item_level1_no            = away.item_level1_no 
...
order by home.location_no,
   ...
   DBMS_STATS.gather_table_stats ('DWH_CUST_PERFORMANCE','cust_mart_home_and_away_iteml1',estimate_percent=>1, DEGREE => 32);

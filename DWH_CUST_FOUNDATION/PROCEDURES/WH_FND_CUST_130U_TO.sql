--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_130U_TO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_130U_TO" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - dwh_cust_foundation.stg_lst_geo_codes_cpy@link_to_dev
--               Output - fnd_customer
...
g_customer_no        dwh_cust_foundation.stg_lst_geo_codes_cpy@link_to_dev.customer_no%type; 
...
cursor stg_dup is
select * from dwh_cust_foundation.stg_lst_geo_codes_cpy@link_to_dev 
where (customer_no)
in
(select customer_no
from dwh_cust_foundation.stg_lst_geo_codes_cpy@link_to_dev 
...
--************************************************************************************************** 
-- Eliminate duplicates on the very rare occasion they may be present@link_to_dev
--**************************************************************************************************
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.customer_no    = g_customer_no  then
        update dwh_cust_foundation.stg_lst_geo_codes_cpy@link_to_dev stg
...
--/*+ first_rows parallel(fnd) parallel(upd_rec) */
   ...
   USING (
      select /*+ FULL(cpy)  parallel (4) */  
              cpy.*
      from    dwh_cust_foundation.stg_lst_geo_codes_cpy@link_to_dev cpy

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_610ID
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_610ID" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        May 2017
--  Author:      Alastair de Wet
--               Create fnd_customer_master_new dimention table in the foundation layer
--               with input ex staging table from SVOC  .
--  Tables:      Input  - stg_svoc_customer_master_cpy
--               Output - fnd_customer_master_new
...
--/*+ first_rows parallel(fnd) parallel(upd_rec) */
   ...
   MERGE  /*+ parallel(fnd,8) */ INTO fnd_customer_master_new fnd 
   USING (

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_022U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_022U_OLD" (p_forall_limit in integer,p_success out boolean) AS
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_c2_customer_card_hsp.sys_process_msg%type;
g_rec_out            fnd_customer_card%rowtype;
g_rec_in             stg_c2_customer_card_cpy%rowtype;
g_found              boolean;
...
type stg_array is table of stg_c2_customer_card_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_customer_card%rowtype index by binary_integer;
type tbl_array_u is table of fnd_customer_card%rowtype index by binary_integer;
...
type staging_array1 is table of stg_c2_customer_card_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_c2_customer_card_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   g_rec_out.vision_customer_no    := g_rec_in.vision_customer_no ;
...
/*
   if not dwh_cust_valid.fnd_customer_product(g_rec_out.product_no ) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_cust_constants.vc_c2_customer_prod_not_found;
     l_text          := dwh_cust_constants.vc_c2_customer_prod_not_found||g_rec_out.product_no   ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     return;
   end if;
*/
   ...
   if not dwh_cust_valid.fnd_customer_product(g_rec_out.product_no ) then
      insert into fnd_customer_product fnd
      values (g_rec_out.product_no,g_rec_out.customer_no,g_rec_out.product_code_no,g_date,1);
...
begin
   ...
   insert into stg_c2_customer_card_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_customer_card values a_tbl_insert(i);
...
BEGIN
   g_found := dwh_cust_valid.fnd_customer_card (g_rec_out.card_no );
...
begin
    ...
    l_text := 'LOAD OF fnd_customer_card EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_c2_customer_card;
    fetch c_stg_c2_customer_card bulk collect into a_stg_input limit g_forall_limit;

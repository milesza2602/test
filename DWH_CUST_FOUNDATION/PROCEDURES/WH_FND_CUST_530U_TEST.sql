--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_530U_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_530U_TEST" (p_forall_limit in integer,p_success out boolean) AS
...
--  Tables:      Input  - stg_cust_cl_inquiry
...
g_rec_in             stg_cust_cl_inquiry%rowtype;
g_found              boolean;
...
type stg_array is table of stg_cust_cl_inquiry%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_cust_cl_inquiry%rowtype index by binary_integer;
type tbl_array_u is table of fnd_cust_cl_inquiry%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cust_cl_inquiry.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cust_cl_inquiry.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   insert into stg_cust_cl_inquiry_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_cust_cl_inquiry values a_tbl_insert(i);
...
begin
   g_found := dwh_cust_valid.fnd_cust_cl_inquiry (g_rec_out.inquiry_no );

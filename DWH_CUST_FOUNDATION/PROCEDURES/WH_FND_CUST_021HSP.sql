--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_021HSP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_021HSP" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_c2_customer_product
...
g_product_no        stg_c2_customer_product.product_no%type;
...
cursor stg_dup is
select * from stg_c2_customer_product
where (product_no)
in
(select product_no
from stg_c2_customer_product
group by product_no
having count(*) > 1)
order by product_no,
...
select /*+ FULL(cpy)  parallel (cpy,2) */
              cpy.*
      from    stg_c2_customer_product cpy,
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.product_no    = g_product_no  then
        update stg_c2_customer_product stg
...
--    update stg_c2_customer_product

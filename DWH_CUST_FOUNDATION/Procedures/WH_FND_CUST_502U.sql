--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_502U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_502U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        August 2010
--  Author:      Alastair de Wet
--  Purpose:     Create CL_FEEDBACK dimention table in the foundation layer
--               with input ex staging table from Customer Liaison.
--  Tables:      Input  - stg_cust_cl_feedback_cpy
--               Output - fnd_cust_cl_feedback
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_cust_cl_feedback_hsp.sys_process_msg%type;
g_rec_out            fnd_cust_cl_feedback%rowtype;
g_rec_in             stg_cust_cl_feedback_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_502U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CL_FEEDBACK DATA EX CUST LIAISON';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_cust_cl_feedback_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_cust_cl_feedback%rowtype index by binary_integer;
type tbl_array_u is table of fnd_cust_cl_feedback%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cust_cl_feedback_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cust_cl_feedback_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cust_cl_feedback is
   select *
   from stg_cust_cl_feedback_cpy
...
begin
   ...
   g_rec_out.cl_inq_feedback_level_ind := g_rec_in.cl_inq_feedback_level_ind ;
   g_rec_out.cl_inq_category_no        := nvl(g_rec_in.cl_inq_category_no,0);
   g_rec_out.cl_inq_feedback_desc      := g_rec_in.cl_inq_feedback_desc ;
...
--   end if;
   ...
   if not dwh_cust_valid.fnd_cust_cl_inq_cat(g_rec_out.cl_inq_category_no) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_cust_constants.vc_cl_inq_cat_not_found;
     l_text          := dwh_cust_constants.vc_cl_inq_cat_not_found||g_rec_out.cl_inq_feedback_no||' '||g_rec_out.cl_inq_category_no  ;
...
begin
   ...
   insert into stg_cust_cl_feedback_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_cust_cl_feedback values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).cl_inq_feedback_no ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_cust_cl_feedback
       set    cl_inq_feedback_level_ind  = a_tbl_update(i).cl_inq_feedback_level_ind,
       ...
       where  cl_inq_feedback_no         = a_tbl_update(i).cl_inq_feedback_no  ;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).cl_inq_feedback_no ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_cust_cl_feedback_cpy
...
begin
   g_found := dwh_cust_valid.fnd_cust_cl_feedback (g_rec_out.cl_inq_feedback_no );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).cl_inq_feedback_no  = g_rec_out.cl_inq_feedback_no  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_cust_cl_feedback EX CUST LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_cust_cl_feedback;
    fetch c_stg_cust_cl_feedback bulk collect into a_stg_input limit g_forall_limit;

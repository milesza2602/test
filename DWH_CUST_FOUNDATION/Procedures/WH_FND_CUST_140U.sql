--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_140U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_140U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        January 2014
--  Author:      Alastair de Wet
--  Purpose:     Create GIFT CARD EXCHANGE fact table in the foundation layer
--               with input ex staging table from POS.
--  Tables:      Input  - stg_pos_gift_card_exchange_cpy
--               Output - fnd_gift_card_exchange
...
g_location_no    stg_pos_gift_card_exchange_cpy.location_no%type;
g_tran_date      stg_pos_gift_card_exchange_cpy.tran_date%type;
g_till_no        stg_pos_gift_card_exchange_cpy.till_no%type;
g_tran_no        stg_pos_gift_card_exchange_cpy.tran_no%type;

g_date               date          := trunc(sysdate);

L_MESSAGE            SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_140U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD GIFT CARD EXCHANGE EX POS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

cursor stg_dup is
select * from stg_pos_gift_card_exchange_cpy
where (location_no,
tran_date,
till_no,tran_no)
in
...
tran_date,
till_no,tran_no
from stg_pos_gift_card_exchange_cpy
...
cursor c_stg_pos_gift_card_exchange is
select /*+ FULL(cpy)  parallel (cpy,2) */
              cpy.*
      from    stg_pos_gift_card_exchange_cpy cpy,
              fnd_gift_card_exchange fnd
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.location_no   = g_location_no and
        ...
        update stg_pos_gift_card_exchange_cpy stg
...
--******************************************************************************
-- On informix none of the numbers on gift card exchnage were ever on the masters
-- so we do not need to create a dummy

--      insert /*+ APPEND parallel (fnd,2) */ into fnd_customer_card fnd
--      select /*+ FULL(cpy)  parallel (cpy,2) */
--             distinct
--             cpy.from_gift_card_no,
--             0,0,0,1,
--             g_date,
--             1
--      from   stg_pos_gift_card_exchange_cpy cpy
...
--       where  card_no         = cpy.from_gift_card_no )
--       and    sys_process_code    = 'N'
--       and    cpy.from_gift_card_no is not null;
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (fnd,2) */ into fnd_gift_card_exchange fnd
      select /*+ FULL(cpy)  parallel (cpy,2) */
            	cpy.	location_no	,
            	cpy.	till_no	,
            	cpy.	tran_no	,
       ...
       from  stg_pos_gift_card_exchange_cpy cpy
       where  not exists
      (select /*+ nl_aj */ * from fnd_gift_card_exchange
...
FOR upd_rec IN c_stg_pos_gift_card_exchange
   loop
     update fnd_gift_card_exchange fnd
     set    fnd.	exchange_amount 	=	upd_rec.	exchange_amount	,
            fnd.	operator_id	      =	upd_rec.	operator_id	,
            fnd.	from_gift_card_no	=	upd_rec.	from_gift_card_no	,
            fnd.	to_gift_card_no 	=	upd_rec.	to_gift_card_no	,
     ...
     where  fnd.	location_no	      =	upd_rec.	location_no and
            ...
            (
            nvl(fnd.exchange_amount	  ,0) <>	upd_rec.	exchange_amount	or
            nvl(fnd.operator_id	      ,0) <>	upd_rec.	operator_id	or
            nvl(fnd.from_gift_card_no	,0) <>	upd_rec.	from_gift_card_no	or
            nvl(fnd.to_gift_card_no 	,0) <>	upd_rec.	to_gift_card_no
            );
...
begin
      ...
      insert /*+ APPEND parallel (hsp,2) */ into stg_pos_gift_card_exchange_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */
...
--    update stg_pos_gift_card_exchange_cpy

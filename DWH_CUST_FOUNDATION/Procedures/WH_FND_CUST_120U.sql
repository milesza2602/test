--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_120U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_120U" (p_forall_limit in integer,p_success out boolean) as
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      select /*+ FULL(cpy)  parallel (cpy,2) */
                ...
                cpy.	reward_off_percentage,
                cpy.	threshold,
                cpy.	voucher_purpose_id,
                cpy.	voucher_purpose_desc,
                cpy.	redeemed_channel_id,
                cpy.	redeemed_channel_desc,
                cpy.	customer_id,
                cpy.	known_customer_ind,
                cpy.	connection_id,
                cpy.	change_user_name,
                cpy.	loyalty_reward_id,
                cpy.	trial_ind,
                cpy.	ext_customer_id,
                cpy.	ext_source_system
       from  stg_fv_voucher_cpy cpy
       where  not exists
...
FOR upd_rec IN c_stg_fv_voucher
   loop
     ...
     set    fnd.	campaign_no	=	upd_rec.	campaign_no	,
            ...
            fnd.	reward_off_percentage   =   upd_rec.reward_off_percentage,
            fnd.	threshold               =   upd_rec.threshold,
            fnd.	voucher_purpose_id      =   upd_rec.voucher_purpose_id,
            fnd.	voucher_purpose_desc    =   upd_rec.voucher_purpose_desc,
            fnd.	redeemed_channel_id     =   upd_rec.redeemed_channel_id,
            fnd.	redeemed_channel_desc   =   upd_rec.redeemed_channel_desc,
            fnd.	customer_id             =   upd_rec.customer_id,
            fnd.	known_customer_ind      =   upd_rec.known_customer_ind,
            fnd.	connection_id           =   upd_rec.connection_id,
            fnd.	change_user_name        =   upd_rec.change_user_name,
            fnd.	loyalty_reward_id       =   upd_rec.loyalty_reward_id,
            fnd.	trial_ind               =   upd_rec.trial_ind,
            fnd.	ext_customer_id         =   upd_rec.ext_customer_id,
            fnd.	ext_source_system       =   upd_rec.ext_source_system
     where  fnd.	voucher_no	      =	upd_rec.	voucher_no and
            ...
            nvl(fnd.voucher_amount	,0) <>	upd_rec.	voucher_amount or
            nvl(fnd.reward_off_percentage,0)   <>   upd_rec.reward_off_percentage or
            nvl(fnd.threshold,0)               <>   upd_rec.threshold or
            nvl(fnd.voucher_purpose_id,0)      <>   upd_rec.voucher_purpose_id or
            nvl(fnd.voucher_purpose_desc,0)    <>   upd_rec.voucher_purpose_desc or
            nvl(fnd.redeemed_channel_id,0)     <>   upd_rec.redeemed_channel_id or
            nvl(fnd.redeemed_channel_desc,0)   <>   upd_rec.redeemed_channel_desc or
            nvl(fnd.customer_id,0)             <>   upd_rec.customer_id or
            nvl(fnd.known_customer_ind,0)      <>   upd_rec.known_customer_ind or
            nvl(fnd.connection_id,0)           <>   upd_rec.connection_id or
            nvl(fnd.change_user_name,0)        <>   upd_rec.change_user_name or
            nvl(fnd.loyalty_reward_id,0)       <>   upd_rec.loyalty_reward_id or
            nvl(fnd.trial_ind,0)               <>   upd_rec.trial_ind or
            nvl(fnd.ext_customer_id,0)         <>   upd_rec.ext_customer_id or
            nvl(fnd.ext_source_system,0)       <>   upd_rec.ext_source_system

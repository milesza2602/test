--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_132U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_132U" (p_forall_limit in integer,p_success out boolean) as
...
--/*+ first_rows parallel(fnd) parallel(upd_rec) */
   ...
   USING (
         select /*+ FULL(cpy)  parallel (4) */  
              cpy.* 
      from    stg_lst_ea_codes_cpy cpy
      where   cpy.sys_process_code      = 'N'
      order by   sys_source_batch_id,sys_source_sequence_no      
         ) mer_rec
   ON    (  fnd.	ea_code	          =	mer_rec.	ea_code and
            fnd.  fin_year_no       = g_fin_year_no)
   WHEN MATCHED THEN 
   UPDATE SET
...
--**************************************************************************************************
    ...
    select today_fin_year_no into g_fin_year_no from dim_control;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_066U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_066U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        January 2013
--  Author:      Alastair de Wet
--  Purpose:     Create WOD_CARD HISTORY dimention table in the foundation layer
--               with input ex staging table from Customer Central.
--  Tables:      Input  - stg_c2_wod_card_history_cpy
--               Output - fnd_wod_card_history
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_c2_wod_card_history_hsp.sys_process_msg%type;
g_rec_out            fnd_wod_card_history%rowtype;
g_rec_in             stg_c2_wod_card_history_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_066U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WOD_CARD HISTORY DIM EX CUSTOMER CENTRAL';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_c2_wod_card_history_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wod_card_history%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wod_card_history%rowtype index by binary_integer;
...
type staging_array1 is table of stg_c2_wod_card_history_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_c2_wod_card_history_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_c2_wod_card_history is
   select *
   from stg_c2_wod_card_history_cpy
...
begin
   ...
   g_hospital                                := 'N';
   g_rec_out.card_history_id                 := g_rec_in.card_history_id;
   g_rec_out.card_id                         := g_rec_in.card_id;
   g_rec_out.card_status_id                  := g_rec_in.card_status_id;
   g_rec_out.change_user_id                  := g_rec_in.change_user_id;
   g_rec_out.correspondence_id               := g_rec_in.correspondence_id;
   g_rec_out.create_date                     := g_rec_in.create_date;
   g_rec_out.customer_no                     := g_rec_in.customer_no;
   g_rec_out.embossed_ind                    := g_rec_in.embossed_ind;
   g_rec_out.product_code_no                 := g_rec_in.product_code_no;
   g_rec_out.replacement_reason              := g_rec_in.replacement_reason;
...
begin
   ...
   insert into stg_c2_wod_card_history_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wod_card_history values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).card_history_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_wod_card_history
       set    card_id                         = a_tbl_update(i).card_id,
              card_status_id                  = a_tbl_update(i).card_status_id,
              change_user_id                  = a_tbl_update(i).change_user_id,
              correspondence_id               = a_tbl_update(i).correspondence_id,
              create_date                     = a_tbl_update(i).create_date,
              customer_no                     = a_tbl_update(i).customer_no,
              embossed_ind                    = a_tbl_update(i).embossed_ind,
              product_code_no                 = a_tbl_update(i).product_code_no,
              replacement_reason              = a_tbl_update(i).replacement_reason,
              last_updated_date               = a_tbl_update(i).last_updated_date
       where  card_history_id                 = a_tbl_update(i).card_history_id  ;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).card_history_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_c2_wod_card_history_cpy
...
-- Check to see if present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_wod_card_history
   where  card_history_id = g_rec_out.card_history_id;
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).card_history_id  = g_rec_out.card_history_id  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_wod_card_history EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_c2_wod_card_history;
    fetch c_stg_c2_wod_card_history bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_c2_wod_card_history;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_034U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_034U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        January 2013
--  Author:      Alastair de Wet
--  Purpose:     Create LS_SEG_CAT dimention table (FACTORS) in the foundation layer
--               with input ex staging table from MARKETING. ONCE A YEAR MANUAL LOAD!!!
--  Tables:      Input  - stg_excel_seg_cat_cpy
--               Output - fnd_ls_seg_cat
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_seg_cat_hsp.sys_process_msg%type;
g_rec_out            fnd_ls_seg_cat%rowtype;
g_rec_in             stg_excel_seg_cat_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_034U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE SEG CAT DIM EX MARKETING';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_seg_cat_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_ls_seg_cat%rowtype index by binary_integer;
type tbl_array_u is table of fnd_ls_seg_cat%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_seg_cat_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_seg_cat_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_seg_cat is
   select *
   from stg_excel_seg_cat_cpy
...
begin
   ...
   g_rec_out.cat_descr                       := g_rec_in.cat_descr;
...
begin
   ...
   insert into stg_excel_seg_cat_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_ls_seg_cat values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).ls_seg_cat_id ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_ls_seg_cat
       set    cat_descr                       = a_tbl_update(i).cat_descr  ,
              food_non_food                   = a_tbl_update(i).food_non_food,
              last_updated_date               = a_tbl_update(i).last_updated_date
       where  ls_seg_cat_id                      = a_tbl_update(i).ls_seg_cat_id  ;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).ls_seg_cat_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
-- Check to see if present on table and update/insert accordingly
   ...
   into   g_count
   from   fnd_ls_seg_cat
   where  ls_seg_cat_id = g_rec_out.ls_seg_cat_id ;
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).ls_seg_cat_id  = g_rec_out.ls_seg_cat_id  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_ls_seg_cat EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_seg_cat;
    fetch c_stg_excel_seg_cat bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_154U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_154U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        January 2014
--  Author:      Alastair de Wet
--  Purpose:     Create segment distance fact table in the foundation layer
--               with input ex staging table from SAS. INSERT ONLY!!
--               FOR RERUN - DROP PARTISION FIRST THEN RERUN
--  Tables:      Input  - stg_sas_cust_seg_distance_cpy
--               Output - fnd_cust_segment_distance
...
g_primary_customer_identifier  stg_sas_cust_seg_distance_cpy.primary_customer_identifier%type;
g_fin_year_no                  stg_sas_cust_seg_distance_cpy.fin_year_no%type;
g_fin_month_no                 stg_sas_cust_seg_distance_cpy.fin_month_no%type;
g_segment_no                   stg_sas_cust_seg_distance_cpy.segment_no%type;
g_segment_type                 stg_sas_cust_seg_distance_cpy.segment_type%type;

g_date               date          := trunc(sysdate);

L_MESSAGE            SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_154U';
...
cursor stg_dup is
SELECT * FROM stg_sas_cust_seg_distance_CPY
...
SEGMENT_NO,
SEGMENT_TYPE
FROM stg_sas_cust_seg_distance_CPY
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.primary_customer_identifier = g_primary_customer_identifier and
        ...
        update stg_sas_cust_seg_distance_cpy stg
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (fnd,4) */ into fnd_cust_segment_distance fnd
      select /*+ FULL(cpy)  parallel (cpy,4) */
       ...
       from  stg_sas_cust_seg_distance_cpy cpy
       where  not exists
      (select /*+ nl_aj */ * from fnd_cust_segment_distance
...
--    g_stmt      := 'Alter table  DWH_CUST_FOUNDATION.FND_CUST_SEGMENT_DISTANCE truncate  subpartition for (2015,7)';
--    l_text      := g_stmt;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
   ...
   if g_recs_read <> g_recs_inserted + g_recs_updated  then
      ...
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_text :=  '(+-Line 225) DELETE THE DAYS RECS OFF THE fnd_cust_segment_distance TABLE &  RERUN -OR- '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
      DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);

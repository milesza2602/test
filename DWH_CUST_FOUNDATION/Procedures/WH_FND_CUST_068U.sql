--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_068U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_068U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        January 2013
--  Author:      Alastair de Wet
--  Purpose:     Create WOD_CHILDREN dimention table in the foundation layer
--               with input ex staging table from Customer Central.
--  Tables:      Input  - stg_c2_wod_children_cpy
--               Output - fnd_wod_children
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_c2_wod_children_hsp.sys_process_msg%type;
g_rec_out            fnd_wod_children%rowtype;
g_rec_in             stg_c2_wod_children_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_068U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WOD_CHILDREN DIM EX CUSTOMER CENTRAL';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_c2_wod_children_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wod_children%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wod_children%rowtype index by binary_integer;
...
type staging_array1 is table of stg_c2_wod_children_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_c2_wod_children_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_c2_wod_children is
   select *
   from stg_c2_wod_children_cpy
...
begin
   ...
   g_rec_out.child_id                        := g_rec_in.child_id;
   ...
   g_rec_out.cust_portfolio_no               := g_rec_in.cust_portfolio_no;
   g_rec_out.child_first_name                := g_rec_in.child_first_name ;
   g_rec_out.child_gender                    := g_rec_in.child_gender ;
   g_rec_out.child_last_name                 := g_rec_in.child_last_name;
   g_rec_out.customer_no                     := g_rec_in.customer_no ;
   g_rec_out.product_code_no                 := g_rec_in.product_code_no ;
   g_rec_out.status_id                       := g_rec_in.status_id ;
   g_rec_out.cust_relationship_id            := g_rec_in.cust_relationship_id;
...
/*
  if not dwh_cust_valid.fnd_customer(g_rec_out.customer_no) then
   ...
   if not dwh_cust_valid.fnd_customer_portfolio (g_rec_out.cust_portfolio_no,g_rec_out.customer_no ) then
     G_HOSPITAL      := 'Y';
     g_hospital_text := dwh_cust_constants.vc_c2_customer_pf_not_found;
     l_text          := dwh_cust_constants.vc_c2_customer_pf_not_found||g_rec_out.customer_no||g_rec_out.cust_portfolio_no ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     RETURN;
   end if;
*/
...
begin
   ...
   insert into stg_c2_wod_children_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wod_children values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).child_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_wod_children
       set    birth_date                      = a_tbl_update(i).birth_date,
              change_date                     = a_tbl_update(i).change_date,
              create_date                     = a_tbl_update(i).create_date,
              cust_portfolio_no               = a_tbl_update(i).cust_portfolio_no,
              child_first_name                = a_tbl_update(i).child_first_name ,
              child_gender                    = a_tbl_update(i).child_gender ,
              child_last_name                 = a_tbl_update(i).child_last_name,
              customer_no                     = a_tbl_update(i).customer_no ,
              product_code_no                 = a_tbl_update(i).product_code_no ,
              status_id                       = a_tbl_update(i).status_id ,
              cust_relationship_id            = a_tbl_update(i).cust_relationship_id,
              last_updated_date               = a_tbl_update(i).last_updated_date
       where  child_id                        = a_tbl_update(i).child_id;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).child_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_c2_wod_children_cpy
...
-- Check to see if present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_wod_children
   where  child_id = g_rec_out.child_id;
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).child_id  = g_rec_out.child_id  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_wod_children EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_c2_wod_children;
    fetch c_stg_c2_wod_children bulk collect into a_stg_input limit g_forall_limit;

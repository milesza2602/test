--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_514U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_514U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        August 2010
--  Author:      Alastair de Wet
--  Purpose:     Create cl_user dimention table in the foundation layer
--               with input ex staging table from Customer Liaison.
--  Tables:      Input  - stg_cust_cl_user_cpy
--               Output - fnd_cust_cl_user
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_cust_cl_user_hsp.sys_process_msg%type;
g_rec_out            fnd_cust_cl_user%rowtype;
g_rec_in             stg_cust_cl_user_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_514U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CL_USER DATA EX CUST LIAISON';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_cust_cl_user_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_cust_cl_user%rowtype index by binary_integer;
type tbl_array_u is table of fnd_cust_cl_user%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cust_cl_user_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cust_cl_user_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cust_cl_user is
   select *
   from stg_cust_cl_user_cpy
...
begin
   ...
   g_hospital                          := 'N';
   g_rec_out.cl_user_no                := nvl(g_rec_in.cl_user_no,0);
   g_rec_out.cl_user_name              := g_rec_in.cl_user_name ;
...
begin
   ...
   insert into stg_cust_cl_user_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_cust_cl_user values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).cl_user_no;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_cust_cl_user
       set    cl_user_name               = a_tbl_update(i).cl_user_name,
              source_data_status_code    = a_tbl_update(i).source_data_status_code,
              last_updated_date          = a_tbl_update(i).last_updated_date
       where  cl_user_no                 = a_tbl_update(i).cl_user_no  ;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).cl_user_no;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
   g_found := dwh_cust_valid.fnd_cust_cl_user (g_rec_out.cl_user_no);
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).cl_user_no = g_rec_out.cl_user_no then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_cust_cl_user EX CUST LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_cust_cl_user;
    fetch c_stg_cust_cl_user bulk collect into a_stg_input limit g_forall_limit;

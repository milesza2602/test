--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_516U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_516U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        August 2010
--  Author:      Alastair de Wet
--  Purpose:     Create CL_status_catgory dimention table in the foundation layer
--               with input ex staging table from Customer Liaison.
--  Tables:      Input  - stg_cust_cl_status_catgory_cpy
--               Output - fnd_cust_cl_status_catgory
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_cust_cl_status_catgory_hsp.sys_process_msg%type;
g_rec_out            fnd_cust_cl_status_catgory%rowtype;
g_rec_in             stg_cust_cl_status_catgory_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_516U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CL_STATUS_CATGORY DATA EX CUST LIAISON';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_cust_cl_status_catgory_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_cust_cl_status_catgory%rowtype index by binary_integer;
type tbl_array_u is table of fnd_cust_cl_status_catgory%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cust_cl_status_catgory_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cust_cl_status_catgory_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cust_cl_status_catgory is
   select *
   from stg_cust_cl_status_catgory_cpy
...
begin
   ...
   insert into stg_cust_cl_status_catgory_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_cust_cl_status_catgory values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_cust_cl_status_catgory
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_cust_cl_status_catgory_cpy
...
begin
   g_found := dwh_cust_valid.fnd_cust_cl_status_catgory (g_rec_out.cl_status_category_no );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).cl_status_category_no  = g_rec_out.cl_status_category_no  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_cust_cl_status_catgory EX CUST LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_cust_cl_status_catgory;
    fetch c_stg_cust_cl_status_catgory bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_cust_cl_status_catgory;
--**************************************************************************************************

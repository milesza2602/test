--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_104U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_104U" (p_forall_limit in integer,p_success out boolean) as
...
g_location_no    stg_pos_cust_basket_item_cpy.location_no%type;
g_tran_date      stg_pos_cust_basket_item_cpy.tran_date%type;
g_till_no        stg_pos_cust_basket_item_cpy.till_no%type;
g_tran_no        stg_pos_cust_basket_item_cpy.tran_no%type;
g_item_seq_no    stg_pos_cust_basket_item_cpy.item_seq_no%type;
g_item_no        stg_pos_cust_basket_item_cpy.item_no%type;
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD BASKET_ITEM EX POS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

cursor stg_dup is
select * from stg_pos_cust_basket_item_cpy
where (location_no,
tran_date,
...
select /*+ FULL(cpy)  parallel (4) */
              cpy.*
      from    stg_pos_cust_basket_item_cpy cpy,
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.location_no   = g_location_no and
        ...
        update stg_pos_cust_basket_item_cpy stg
...
--            	cpy.	dept_no	,
              '', 
            	cpy.	item_input_code	,
            	nvl(cpy.waste_discount_selling,0)	,   
             ...
             g_date as last_updated_date,
              cpy.	EMPLOYEE_ID,
              cpy.	COMPANY_CODE,
              cpy.	ITEM_TRAN_SELLING_LOCAL,
              cpy.	DISCOUNT_SELLING_LOCAL,
              cpy.	WASTE_DISCOUNT_SELLING_LOCAL,
              cpy.	GREEN_VALUE_LOCAL
       from  stg_pos_cust_basket_item_cpy cpy
       where  not exists
...
--            fnd.	dept_no	=	upd_rec.	dept_no	,
            ...
            fnd.	ITEM_TRAN_SELLING_LOCAL =	upd_rec.	ITEM_TRAN_SELLING_LOCAL	,
            fnd.	DISCOUNT_SELLING_LOCAL	=	upd_rec.	DISCOUNT_SELLING_LOCAL	,
            fnd.	WASTE_DISCOUNT_SELLING_LOCAL	  =	upd_rec.	WASTE_DISCOUNT_SELLING_LOCAL	,
            fnd.	GREEN_VALUE_LOCAL	      =	upd_rec.	GREEN_VALUE_LOCAL	,
...
--            nvl(fnd.dept_no	,0) <>	upd_rec.	dept_no	or
            ...
            nvl(fnd.green_factor	,0) <>	upd_rec.	green_factor or
...
begin
      ...
      insert /*+ APPEND parallel (hsp,2) */ into stg_pos_cust_basket_item_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */

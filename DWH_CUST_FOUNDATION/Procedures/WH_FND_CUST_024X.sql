--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_024X
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_024X" (p_forall_limit in integer,p_success out boolean) as
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      from   stg_c2_customer_portfolio_cpy cpy,
             fnd_product pcd,
             fnd_customer@dwhuat cust
       where cpy.product_code_no    = pcd.product_code_no
       and   cpy.customer_no        = cust.customer_no
       and   not exists
...
begin
      ...
      (
      not exists
        (select * from  fnd_customer@dwhuat cust

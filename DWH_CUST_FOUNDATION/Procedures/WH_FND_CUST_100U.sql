--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_100U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_100U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        January 2014
--  Author:      Alastair de Wet
--  Purpose:     Create cust_basket fact table in the foundation layer
--               with input ex staging table from POS.
--  Tables:      Input  - stg_pos_cust_basket_cpy
...
g_location_no    stg_pos_cust_basket_cpy.location_no%type;
g_tran_date      stg_pos_cust_basket_cpy.tran_date%type;
g_till_no        stg_pos_cust_basket_cpy.till_no%type;
g_tran_no        stg_pos_cust_basket_cpy.tran_no%type;

g_date               date          := trunc(sysdate);

L_MESSAGE            SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_100U';
...
cursor stg_dup is
select * from stg_pos_cust_basket_cpy
where (location_no,
tran_date,
till_no,tran_no)
in
...
cursor c_stg_pos_cust_basket is
select /*+ FULL(cpy)  parallel (cpy,2) */
              cpy.*
      from    stg_pos_cust_basket_cpy cpy,
              fnd_cust_basket fnd
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.location_no   = g_location_no and
        ...
        update stg_pos_cust_basket_cpy stg
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      select /*+ FULL(cpy)  parallel (cpy,2) */
              ...
              cpy.	EMPLOYEE_ID,
              cpy.	COMPANY_CODE,
              cpy.	TRAN_SELLING_LOCAL,
              cpy.	DISCOUNT_SELLING_LOCAL,
              cpy.	CHANGE_SELLING_LOCAL,
              cpy.	CHANGE_ROUNDING_SELLING_LOCAL
       from  stg_pos_cust_basket_cpy cpy
       where  not exists
...
FOR upd_rec IN c_stg_pos_cust_basket
   loop
     ...
     set    fnd.	tran_time             	=	upd_rec.	tran_time	,
            ...
            fnd.	change_selling	        =	upd_rec.	change_selling	,
            fnd.	change_rounding_selling	=	upd_rec.	change_rounding_selling	,
            fnd.	waste_ind	              =	upd_rec.	waste_ind	,
            ...
            fnd.	vitality_cust_ind	      =	upd_rec.	vitality_cust_ind	,
            fnd.	EMPLOYEE_ID	            =	upd_rec.	EMPLOYEE_ID	,
            fnd.	COMPANY_CODE            =	upd_rec.	COMPANY_CODE	,
            fnd.	TRAN_SELLING_LOCAL      =	upd_rec.	TRAN_SELLING_LOCAL	,
            fnd.	DISCOUNT_SELLING_LOCAL	=	upd_rec.	DISCOUNT_SELLING_LOCAL	,
            fnd.	CHANGE_SELLING_LOCAL	  =	upd_rec.	CHANGE_SELLING_LOCAL	,
            fnd.	CHANGE_ROUNDING_SELLING_LOCAL	   =	upd_rec.	CHANGE_ROUNDING_SELLING_LOCAL	,
     ...
     where  fnd.	location_no	      =	upd_rec.	location_no and
            ...
            nvl(fnd.COMPANY_CODE     	      ,0) <>	upd_rec.	COMPANY_CODE 
...
begin
      ...
      insert /*+ APPEND parallel (hsp,2) */ into stg_pos_cust_basket_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */
...
--    update stg_pos_cust_basket_cpy

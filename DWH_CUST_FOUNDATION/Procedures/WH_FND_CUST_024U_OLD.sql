--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_024U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_024U_OLD" (p_forall_limit in integer,p_success out boolean) AS
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_c2_customer_portfolio_hsp.sys_process_msg%type;
g_rec_out            fnd_customer_portfolio%rowtype;
g_rec_in             stg_c2_customer_portfolio_cpy%rowtype;
g_found              boolean;
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CUTOMER PORTFOLIO DIM EX CUSTOMER CENTRAL';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_c2_customer_portfolio_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_customer_portfolio%rowtype index by binary_integer;
type tbl_array_u is table of fnd_customer_portfolio%rowtype index by binary_integer;
...
type staging_array1 is table of stg_c2_customer_portfolio_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_c2_customer_portfolio_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   g_rec_out.portfolio_status_desc           := g_rec_in.portfolio_status_desc;
   g_rec_out.portfolio_create_date           := g_rec_in.portfolio_create_date;
   g_rec_out.portfolio_change_date           := g_rec_in.portfolio_change_date;
   g_rec_out.portfolio_close_date            := g_rec_in.portfolio_close_date;
   g_rec_out.product_no                      := g_rec_in.product_no;
   g_rec_out.portfolio_role_type_desc        := g_rec_in.portfolio_role_type_desc;
   g_rec_out.cust_prod_close_reason_code     := g_rec_in.cust_prod_close_reason_code;
   g_rec_out.atg_customer_no                 := g_rec_in.atg_customer_no;
...
begin
   ...
   insert into stg_c2_customer_portfolio_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_customer_portfolio values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_customer_portfolio
       set    product_code_no                 = a_tbl_update(i).product_code_no,
              portfolio_status_desc           = a_tbl_update(i).portfolio_status_desc,
              portfolio_create_date           = a_tbl_update(i).portfolio_create_date,
              portfolio_change_date           = a_tbl_update(i).portfolio_change_date,
              portfolio_close_date            = a_tbl_update(i).portfolio_close_date,
              product_no                      = a_tbl_update(i).product_no,
              portfolio_role_type_desc        = a_tbl_update(i).portfolio_role_type_desc,
              cust_prod_close_reason_code     = a_tbl_update(i).cust_prod_close_reason_code,
              atg_customer_no                 = a_tbl_update(i).atg_customer_no,
              last_updated_date               = a_tbl_update(i).last_updated_date
       where cust_portfolio_no                = a_tbl_update(i).cust_portfolio_no and
...
begin
   g_found := dwh_cust_valid.fnd_customer_portfolio (g_rec_out.cust_portfolio_no,g_rec_out.customer_no );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).cust_portfolio_no  = g_rec_out.cust_portfolio_no  and
            a_tbl_insert(i).customer_no  = g_rec_out.customer_no      then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_customer_portfolio EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_c2_customer_portfolio;
    fetch c_stg_c2_customer_portfolio bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_c2_customer_portfolio;
--**************************************************************************************************

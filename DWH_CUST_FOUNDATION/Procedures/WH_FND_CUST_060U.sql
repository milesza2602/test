--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_060U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_060U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        January 2013
--  Author:      Alastair de Wet
--  Purpose:     Create WOD_LOYALTY_SEGMENT dimention table in the foundation layer
--               with input ex staging table from Customer Central.
--  Tables:      Input  - stg_c2_wod_loyalty_segment_cpy
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_c2_wod_loyalty_segment_hsp.sys_process_msg%type;
g_rec_out            fnd_wod_loyalty_segment%rowtype;
g_rec_in             stg_c2_wod_loyalty_segment_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CUST_060U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WOD_LOYALTY_SEGMENT DIM EX CUSTOMER CENTRAL';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_c2_wod_loyalty_segment_cpy%rowtype;
...
type staging_array1 is table of stg_c2_wod_loyalty_segment_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_c2_wod_loyalty_segment_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_c2_wod_loyalty_segment is
   select *
   from stg_c2_wod_loyalty_segment_cpy
...
begin
   ...
   g_rec_out.segment_code                    := g_rec_in.segment_code;
   g_rec_out.segment_type                    := g_rec_in.segment_type;
   g_rec_out.segment_name                    := g_rec_in.segment_name;
   g_rec_out.food_non_food                   := g_rec_in.food_non_food;

   g_rec_out.last_updated_date               := g_date;

 if not dwh_cust_valid.fnd_segment_type(g_rec_out.segment_type_no) then
     g_hospital      := 'Y';
     g_hospital_text := 'REFERENTIAL ERROR - SEGMENT_TYPE DOES NOT EXIST ';
     l_text          := 'REFERENTIAL ERROR - SEGMENT_TYPE DOES NOT EXIST '||g_rec_out.segment_type_no  ;
...
begin
   ...
   insert into stg_c2_wod_loyalty_segment_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    segment_code                    = a_tbl_update(i).segment_code,
              segment_type                    = a_tbl_update(i).segment_type,
              segment_name                    = a_tbl_update(i).segment_name,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_c2_wod_loyalty_segment_cpy
...
--**************************************************************************************************
    open c_stg_c2_wod_loyalty_segment;
    fetch c_stg_c2_wod_loyalty_segment bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_c2_wod_loyalty_segment;
--**************************************************************************************************

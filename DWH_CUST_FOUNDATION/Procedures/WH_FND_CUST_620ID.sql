--------------------------------------------------------
--  DDL for Procedure WH_FND_CUST_620ID
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_CUST_FOUNDATION"."WH_FND_CUST_620ID" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        May 2017
--  Author:      Alastair de Wet
--               Create fnd_ALL_BEAN_master_new dimention table in the foundation layer
--               with input ex staging table from SVOC  .
--  Tables:      Input  - stg_svoc_ALL_BEAN_master_cpy
--               Output - fnd_ALL_BEAN_master_new
...
g_BEAN_ID        stg_svoc_all_bean_master_cpy.BEAN_ID%type; 
...
--/*+ first_rows parallel(fnd) parallel(upd_rec) */
   ...
   merge  /*+ PARALLEL(fnd,8) */ into fnd_ALL_BEAN_master_new fnd 
   using (
   
    SELECT /*+ PARALLEL(CPY1,8) */ CPY1.*
    FROM STG_SVOC_ALL_BEAN_MASTER_CPY CPY1
          ...
          (SELECT /*+ PARALLEL(8) */ SYS_SOURCE_BATCH_ID, SYS_SOURCE_SEQUENCE_NO,BEAN_ID
           FROM (SELECT /*+ PARALLEL(tmp,8) */ tmp.*,
                 RANK ()
                    ...
                    AS RANK
                 FROM STG_SVOC_ALL_BEAN_MASTER_CPY tmp
           )
           WHERE RANK = 1)
   ...
   WHERE    NVL(fnd.	SUBSCRIBER_KEY                  ,'0')  <>	MER_REC.	SUBSCRIBER_KEY	 OR
            ...
            NVL(fnd.	PREFERENCE_1_IND	              ,9)  <>	MER_REC.	PREFERENCE_1_IND	 OR
            NVL(fnd.	PREFERENCE_2_IND              	,9)  <>	MER_REC.	PREFERENCE_2_IND	 OR
            NVL(fnd.	PREFERENCE_3_IND	              ,9)  <>	MER_REC.	PREFERENCE_3_IND	 OR
            NVL(fnd.	PREFERENCE_4_IND              	,9)  <>	MER_REC.	PREFERENCE_4_IND	 OR
            NVL(fnd.	PREFERENCE_5_IND              	,9)  <>	MER_REC.	PREFERENCE_5_IND	 OR
            NVL(fnd.	PREFERENCE_6_IND              	,9)  <>	MER_REC.	PREFERENCE_6_IND	 OR
            NVL(fnd.	PREFERENCE_7_IND	              ,9)  <>	MER_REC.	PREFERENCE_7_IND	 OR
            NVL(fnd.	SWIPE_DEVICE_IND	              ,9)  <>	MER_REC.	SWIPE_DEVICE_IND	 OR
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   stg_svoc_all_bean_master_cpy

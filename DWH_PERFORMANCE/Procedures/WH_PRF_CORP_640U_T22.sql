--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_640U_T22
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_640U_T22" 
  (
...
as
  ...
  --               OUTPUT -    DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22
  ...
  --                             populating DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22
  --
  ...
  --
  --  PROCESSING FLOW
  --   1. Clear DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22
  ...
  --                which have in DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22
  --**************************************************************************************************
...
g_rec_out            DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22%rowtype;
g_found              boolean;
g_count              number       := 0;
...
P_start_date         date          := trunc(sysdate);
P_end_date           date          := trunc(sysdate);
...
l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_640U_T22';
...
type tbl_array_i is table of DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22%rowtype index by binary_integer;
type tbl_array_u is table of DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22%rowtype index by binary_integer;
...
begin
  ...
  save exceptions
   insert into DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 values a_tbl_insert(i);
...
begin
  ...
  for v_cur    in
      ( WITH selpo AS
                  (SELECT a.sk1_contract_no,    a.sk1_item_no,    a.tran_date
                     FROM DWH_PERFORMANCE.rtl_po_supchain_loc_item_dy_DJ a,    DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 b
                    WHERE po_ind          = 1
          ...
          --            AND B.contract_no in(252722	,252744	,253034	,253028	,242026,228740,209022	)
              ...
              selcon AS
                  (SELECT sk1_contract_no,    sk1_item_no, tran_date
                     FROM DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22
  ...
  loop
     insert
       into DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 values
     ( v_cur.contract_no          ,
...
--  end if;
 ...
 --**************************************************************************************************
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  --**************************************************************************************************
  -- TRUNCATE DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22
  -- DWH_LOG.WRITE_LOG   : DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 is truncated before running
  --**************************************************************************************************
   execute immediate
   ('truncate table DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22');
  commit;
  l_text :='TRUNCATED DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 AT '||sysdate;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
dbms_output.put_line('TRUNCATED DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 AT '||sysdate);
  ...
  l_text    := dwh_constants.vc_log_draw_line;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  l_text := 'LOAD OF DWH_PERFORMANCE.temp_fnd_boc_contract_list_T22 EX fnd_contract and DWH_PERFORMANCE.rtl_po_supchain_loc_item_dy_DJ STARTED '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  dwh_log.insert_log_summary(l_name,l_system_name,l_script_name,l_procedure_name,l_description, l_process_type,dwh_constants.vc_log_started,'','','','','');

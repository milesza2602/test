--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_CGMR_BCK2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_CGMR_BCK2" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
-- RESET META-TABLE
--**************************************************************************************************
--  Date:        May 2017
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_CGMR_BCK2';
...
/*    l_text := 'META TABLE DATA UPDATES STARTED AT '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    ...
    update meta_table
       set skip_alert_ind = 'Y',
           release_time = to_date(to_char(trunc(sysdate), 'yyyy/mm/dd') || ' 22:30:00','yyyy/mm/dd hh24:mi:ss')
 ...
 */        
      ...
      update meta_table mt
         set mt.skip_alert_ind = (select mtb.skip_alert_ind
                                    from DWH_META_DATA.META_TABLE_BCK20170508 mtb

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_263U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_263U_FIX" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_263U_FIX';
...
with cal as (
    ...
    --  where calendar_date between to_date('2015-09-28','yyyy-mm-dd') and to_date('2015-10-04','yyyy-mm-dd')),
     ...
     loc_list as ( select unique fnd_supp.zone_no,dz.sk1_zone_group_zone_no,dl.sk1_location_no,
...
--                    where dl.sk1_fd_zone_group_zone_no = dz.SK1_ZONE_GROUP_ZONE_NO) ,
     ...
     st_ord_first_date as (select /*+ PARALLEL(a,4) */ a.sk1_item_no, a.sk1_location_no,
...
,
    ...
    loc_item as (select /*+ PARALLEL(rli,4) */ rli.sk1_location_no,
       ...
       ,
   zone_item_om as (select /*+ PARALLEL(rzom,4)  */  rzom.sk1_zone_group_zone_no, rzom.sk1_item_no,  rzom.from_loc_no, supp_list.supplier_no,

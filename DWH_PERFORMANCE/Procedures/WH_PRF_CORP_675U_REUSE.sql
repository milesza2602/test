--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_675U_REUSE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_675U_REUSE" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        March 2009
--  Author:      M Munnik
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_675U';
...
cursor c_prom_loc_sc_dy is
   select   /*+ no_index (dn) full (ia) PARALLEL(dn,6) */
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       and
            (
            nvl(sales_qty	              ,0) <>	a_tbl_update(i).sales_qty or
            nvl(reg_sales_qty           ,0) <>	a_tbl_update(i).reg_sales_qty	or
            nvl(actl_store_rcpt_qty	    ,0) <>	a_tbl_update(i).actl_store_rcpt_qty	or
            nvl(sales                   ,0) <>	a_tbl_update(i).sales  or
            nvl(reg_sales               ,0) <>	a_tbl_update(i).reg_sales 	or
            nvl(actl_store_rcpt_selling ,0) <>	a_tbl_update(i).actl_store_rcpt_selling	or
...
--**************************************************************************************************
 FOR G_SUB IN 0..15
  LOOP
   ...
   IF G_SUB = 0 THEN
      G_START_DATE := G_DATE - 108;
      G_END_DATE   := G_DATE - 102;
   END IF;   
   IF G_SUB = 1 THEN
      G_START_DATE := G_DATE - 101;
      G_END_DATE   := G_DATE - 95;
   END IF;
   IF G_SUB = 2 THEN
      G_START_DATE := G_DATE - 94;
      G_END_DATE   := G_DATE - 88;
   END IF;
   IF G_SUB = 3 THEN
      G_START_DATE := G_DATE - 87;
      G_END_DATE   := G_DATE - 81;
   END IF;
   IF G_SUB = 4 THEN
      G_START_DATE := G_DATE - 80;
      G_END_DATE   := G_DATE - 74;
   END IF;
   IF G_SUB = 5 THEN
      G_START_DATE := G_DATE - 73;
      G_END_DATE   := G_DATE - 67;
   END IF;
   IF G_SUB = 6 THEN
      G_START_DATE := G_DATE - 66;
      G_END_DATE   := G_DATE - 60;
   END IF;
   IF G_SUB = 7 THEN
      G_START_DATE := G_DATE - 59;
      G_END_DATE   := G_DATE - 53;
   END IF;
   IF G_SUB = 8 THEN
      G_START_DATE := G_DATE - 52;
      G_END_DATE   := G_DATE - 46;
   END IF;
   IF G_SUB = 9 THEN
      G_START_DATE := G_DATE - 45;
      G_END_DATE   := G_DATE - 39;
   END IF;
   IF G_SUB = 10 THEN
      G_START_DATE := G_DATE - 38;
      G_END_DATE   := G_DATE - 32;
   END IF;
   IF G_SUB = 11 THEN
      G_START_DATE := G_DATE - 31;
      G_END_DATE   := G_DATE - 25;
   END IF;
   IF G_SUB = 12 THEN
      G_START_DATE := G_DATE - 24;
      G_END_DATE   := G_DATE - 18;
   END IF;
   IF G_SUB = 13 THEN
      G_START_DATE := G_DATE - 17;

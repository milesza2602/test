--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_681U_FIX201408
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_681U_FIX201408" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Feb 2009
--  Author:      M Munnik
--  Purpose:     Load Foods Waste and Availability Toolbox fact table from RMS Stock.
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_681U_FIX201408';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD FOODS WASTE and AVAILABILITY TOOLBOX from STOCK';
...
cursor c_rtl_loc_item_dy_waste_avail is
  ...
  (select sk1_item_no
   ...
   select r.sk1_location_no, r.sk1_item_no, r.post_date, r.sk2_location_no, r.sk2_item_no,
          (case when r.soh_qty < 0 then 1 else 0 end) a01_negative_soh_ind
   from   rtl_loc_item_dy_rms_stock r
   join   fd_items i on r.sk1_item_no = i.sk1_item_no
   where  r.post_date = g_date;
...
begin
   ...
   g_rec_out.a01_negative_soh_ind           := g_rec_in.a01_negative_soh_ind;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    sk2_location_no                  = a_tbl_update(i).sk2_location_no,
              ...
              a01_negative_soh_ind             = a_tbl_update(i).a01_negative_soh_ind, 
...
begin
    ...
    l_text := 'LOAD RTL_LOC_ITEM_DY_WASTE_AVL_TBOX(STOCK) STARTED '||
...
END WH_PRF_CORP_681U_FIX201408;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_269U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_269U_LOAD" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_269U_LOAD';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE FOODS SUPPLIER SHIPMENT MART - EXTRACT DATA FOR UAT';
...
begin
    ...
    l_text := 'LOAD OF MART_FOODS_SUPPLIER_SH - EXTRACT DATA FOR UAT - STARTED AT '||
...
MERGE  INTO W6005682.MART_FOODS_SUPPLIER_SH rtl_mart USING
(
with ITEM_LIST AS
  (
  SELECT DI.ITEM_NO,
...
from ITEM_LIST a, 
 ...
 where  G.RECEIVE_DATE between G_FROM_DATE and G_TO_DATE   --= G_DATE

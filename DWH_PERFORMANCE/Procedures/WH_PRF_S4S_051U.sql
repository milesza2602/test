--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_051U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_051U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
...
--                               is because someone can be absent for a week 
...
l_procedure_name     sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_051U';
l_table_name         all_tables.table_name%type                := 'RTL_SCH_LOC_EMP_JB_WK';
...
WITH selext1a AS (
        SELECT /*+ full(flr) parallel(flr,g_degrees) */
               FLR.SK1_JOB_ID ,
               FLR.SK1_LOCATION_NO ,
               FLR.sk1_employee_ID ,
               flr.fin_year_no ,
          ...
          JOIN DWH_PERFORMANCE.RTL_EMP_JOB_WK RE
           ...
           AND RE.FIN_YEAR_NO     = flr.FIN_YEAR_NO --flr.FIN_YEAR_NO
           AND RE.FIN_WEEK_NO     = flr.FIN_WEEK_NO --flr.FIN_WEEK_NO
         where flr.FIN_YEAR_NO    = g_loop_fin_year_no
           AND flr.FIN_WEEK_NO    = g_loop_fin_week_no
     ...
     selext1b AS (
        SELECT 
         ...
         WHERE AE.FIN_YEAR_NO          = g_loop_fin_year_no
           AND AE.FIN_WEEK_NO          = g_loop_fin_week_no 
          ),
     selext1 AS (
     ...
     selext4 AS (
        ...
        SELECT
              /*+ full(rtl) parallel(rtl,g_degrees) */
          ...
          FROM selext4 se4
     Left join DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK rtl
            ON se4.sk1_LOCATION_NO   = rtl.sk1_LOCATION_NO
         ...
         WHERE se4.FIN_YEAR_NO       = g_loop_fin_year_no
           AND se4.FIN_WEEK_NO       = g_loop_fin_week_no
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
         ...
         -- dbms_output.put_line('Insert!');
          l_message := 'I'||dwh_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
...
begin
   ...
   exception
      when others then
       ...
       for i in 1 .. g_error_count loop
          g_error_index := sql%bulk_exceptions(i).error_index;
          l_message := 'U'||dwh_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
...
--**************************************************************************************************
  ...
  --g_date := trunc(sysdate); -- hardcoding batch_date for testing
    l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
   -- Approx 7 weeks in the future
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--     DBMS_STATS.gather_table_stats (l_table_owner,'RTL_SCH_LOC_EMP_JB_DY', DEGREE => 8);
  --**************************************************************************************************
  ...
  --**************************************************************************************************       
    begin
--       for g_sub in 1 .. g_loop_cnt +3  
       for g_sub in 0 .. g_loop_cnt-1
         loop 
           select distinct fin_year_no, fin_week_no
           into   g_loop_fin_year_no, g_loop_fin_week_no
           from   dwh_performance.dim_calendar
           where  calendar_date = (g_last_shift_date) - (g_sub * 7); 
         
--           if g_sub = 1 then
--              DWH_PERFORMANCE.DWH_S4S.get_partition( l_table_name, l_table_owner, g_last_shift_date, g_subpart_type, 
...
--           end if;
             ...
             DWH_PERFORMANCE.DWH_S4S.remove_subpartition_of_year (l_procedure_name,
            ...
            --    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            --     DBMS_STATS.gather_table_stats (l_table_owner,l_table_name, DEGREE => g_degrees);
                  a_tbl_insert  := a_empty_set_i;
      a_tbl_update  := a_empty_set_u;
...
--**************************************************************************************************
    DWH_PERFORMANCE.DWH_S4S.write_final_log_data(l_procedure_name,l_description,
                                                 g_recs_read,g_recs_inserted,g_recs_updated);
    commit;
    p_success := true;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_034A_WLFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_034A_WLFIX" (
    ...
    --
     --**************************************************************************************************
      ...
      l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_AST_034A_WLFIX';
   ...
   --   EXECUTE immediate 'ALTER SESSION ENABLE PARALLEL DML';
    ...
    WHERE calendar_date = G_LOOP_DATE + (g_sub * 7);
     ...
     l_text := 'BATCH DATE BEING PROCESSED - G_LOOP_DATE='||G_LOOP_DATE||' G_DATE='||G_DATE;
      ...
      execute immediate 'Truncate Table dwh_performance.temp_rtl_ast_sc_cont_wk_034a';
     l_text := 'Truncate Table dwh_performance.temp_rtl_ast_sc_cont_wk_034a';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            
      insert 
      into dwh_performance.temp_rtl_ast_sc_cont_wk_034a a
   with selext as
      (select 
               sk1_style_colour_no,
              continuity_ind,
 ...
 --     AND   sk1_style_colour_no in (5502589,16420002)
    ...
    l_text := 'Recs Inserted Into TEMP_RTL_SC_CONTINUITY_WK_034A = '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--
    ...
    insert /*+append */ into  dwh_performance.mart_ch_ast_chn_grd_sc_wkWLFx2 m
      with
    --------------------------------------------------------------------------------
    -- last completed week
    --------------------------------------------------------------------------------
...
--                and wkcat.fin_week_no = dcwkcat.fin_week_no
   ...
   --         and sk1_chain_no = 243 and   sk1_style_colour_no in (5502589,16420002)
    ...
    L_TEXT := 'Recs inserted into MART_CH_AST_CHN_GRD_SC_wkWLFx2 = '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

     l_text := '************************************* C COMPLETED****************************************************';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','MART_CH_AST_CHN_GRD_SC_WKWLFX2', DEGREE => 8);
   l_text := '**********Completed GATHER_TABLE_STATS ON MART_CH_AST_CHN_GRD_SC_wkWLFx2********************************';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

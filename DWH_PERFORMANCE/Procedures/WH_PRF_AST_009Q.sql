--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_009Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_009Q" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        June 2012
...
--Below Cursor is used on the style_descr_update procedure.
  ...
  cursor style_cur is
      select sk1_style_no, item_level1_desc
        from dim_lev1;
...
--Below Cursor is used on item_level1_descr_update procedure
  cursor lev1_diff1_cur is
      select sk1_style_no, item_level1_no, item_level1_long_desc
        from dim_lev1_diff1;
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    v_count := 0;
     ...
     for style_rec in style_cur
        loop
         update dim_ast_lev1_diff1
            set item_level1_desc      = style_rec.item_level1_desc,
                item_level1_long_desc = item_level1_no||' - '||style_rec.item_level1_desc
          where sk1_style_no          = style_rec.sk1_style_no
           and  item_level1_desc     <> style_rec.item_level1_desc;
...
-- To overwrite all the Group descriptions in the table with the latest description
--*************************************************************************************************
procedure lev1_diff1_update as
 begin

   l_text := 'ITEM LEVEL1 NO DESCRIPTIONS TO BE PROCESSED :- '||g_structure_count;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    v_count := 0;

     for item_lev1_rec in lev1_diff1_cur
        loop
         update dim_ast_lev1_diff1
            set item_level1_no        = item_lev1_rec.item_level1_no,
                ITEM_LEVEL1_LONG_DESC = item_lev1_rec.ITEM_LEVEL1_LONG_DESC
         where  sk1_style_no          = item_lev1_rec.sk1_style_no
          and   ITEM_LEVEL1_LONG_DESC <> item_lev1_rec.ITEM_LEVEL1_LONG_DESC;

          v_count := v_count + sql%rowcount;
        end loop;

   l_text := 'ITEM LEVEL1 NO D DESCRIPTIONS UPDATED :- '||v_count;
   dwh_log.Write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  commit;
 end lev1_diff1_update;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_261A
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_261A" (p_forall_limit in integer, p_success out boolean) as 
...
g_rec_out            W6005682.FOODS_RENEW_EXTRACT2_2WKSDSO_A%rowtype;
g_count              number        :=  0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_261A';
...
type tbl_array_i is table of W6005682.FOODS_RENEW_EXTRACT2_2WKSDSO_A%rowtype index by binary_integer;
type tbl_array_u is table of W6005682.FOODS_RENEW_EXTRACT2_2WKSDSO_A%rowtype index by binary_integer;
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table W6005682.FOODS_RENEW_EXTRACT2_2WKSDSO_A');
...
INSERT /*+ APPEND PARALLEL (mart,4) */ INTO W6005682.FOODS_RENEW_EXTRACT2_2WKSDSO_A mart
with 
 item_list as (select di.item_no, di.sk1_item_no, di.item_desc, di.sk1_supplier_no, dd.department_no, dd.department_name, di.subclass_no, di.subclass_name
               ...
               where di.department_no in (13,41,44,47,55,58,68,73,75,81,85,88,96)  --(13,42,46,59,66,83,86,89)   --(41,58,73,81,88,96)   --,44,45,47,50,55,58,72,73,75,76,77,81,87,88,90,93,95,96,97,98,99)

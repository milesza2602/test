--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_536U_LOAD1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_536U_LOAD1" (
...
as
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_536U_LOAD1';
...
--  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--  dbms_stats.gather_table_stats ('DWH_PERFORMANCE','RTL_LOC_ITEM_DY_CATALOG',g_part_name,granularity => 'PARTITION',DEGREE => 4);
--  commit;
...
--  LOOP
  ...
  --**************************************************************************************************
  -- Truncate TEMP_LOC_ITEM_WK_CATALOG1
  --**************************************************************************************************
  l_text := 'Truncate table DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG1';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  execute immediate 'TRUNCATE TABLE DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG1';
  commit;

  --**************************************************************************************************
  --Gather Stats On TEMP_LOC_ITEM_WK_CATALOG1
  --**************************************************************************************************
--  l_text := 'Gather stats on DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG1';
--  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--  dbms_stats.gather_table_stats ('DWH_PERFORMANCE', 'TEMP_LOC_ITEM_WK_CATALOG1', degree => 8);
--  commit;

 l_text := 'START INSERT INTO TEMP_LOC_ITEM_WK_CATALOG1';
 dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  --**************************************************************************************************
  -- Insert Into TEMP_LOC_ITEM_WK_CATALOG1
  --**************************************************************************************************
  insert /*+ PARALLEL(CAT,6) */ into dwh_performance.TEMP_LOC_ITEM_WK_CATALOG1 CAT
  ...
  --Gather Stats On TEMP_LOC_ITEM_WK_CATALOG1
  --**************************************************************************************************
  l_text := 'Gather stats START on  DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG1';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  
  dbms_stats.gather_table_stats ('DWH_PERFORMANCE', 'TEMP_LOC_ITEM_WK_CATALOG1', degree => 32, cascade => true );
  commit;
  
  l_text := 'Gather stats END on  DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG1';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  into dwh_performance.RTL_LOC_ITEM_WK_CATALOG CAT
  ...
  from  dwh_performance.rtl_loc_item_dy_catalog lid,
        ...
        dwh_performance.TEMP_LOC_ITEM_WK_CATALOG1 z
  ...
  --      and z.maxdate           = lid.calendar_date

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_048U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_048U_LOAD" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_048U_LOAD';
...
cursor c_fnd_item_uda is
   ...
   from (select di.sk1_item_no,
         ...
         --join   w6005682.stg_rms_item_uda_q qq on fiu.item_no = qq.item_no and fiu.uda_no = qq.uda_no                           
         ...
         and    fiu.uda_no in (100,102,104,106,300,303,304,306,307,309,310,313,314,315,316,317,318,
                               ...
                               3202,3502,4901,
                               5102, 5104, 5105, 5106,
                               5402,5403,5404,5406,5407,5408,5409,5301,5103)
         AND di.item_no in ( 503232698, 737052993546)
         union

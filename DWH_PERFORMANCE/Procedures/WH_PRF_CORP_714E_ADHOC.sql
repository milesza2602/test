--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_714E_ADHOC
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_714E_ADHOC" (p_forall_limit in integer,
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_714E_ADHOC';
...
--**************************************************************************************************
    ...
    DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
     ...
     g_last_mn_fin_year_no := 2018;
     g_last_mn_fin_month_no := 4;
    ...
    dwh_lookup.dim_control(g_date);
    l_text := 'DATES BEING EXTRACTED:- '||g_this_mn_start_date||' '||g_this_mn_end_date;
    DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
...
DBMS_OUTPUT.PUT_LINE('Department extract');
    ...
    'SELECT /*+ PARALLEL(DNS,8) FULL(DNS) */
     ...
     AND THIS_WEEK_START_DATE  BETWEEN '''||G_THIS_MN_START_DATE||''' AND '''||G_THIS_MN_END_DATE||'''
     AND LOC_TYPE = ''S''
     AND DEPARTMENT_NO IN (75, 6110, 6111, 6112, 6113, 6119)
     GROUP BY FIN_YEAR_NO, FIN_WEEK_NO, LOCATION_NO, LOCATION_NAME, DEPARTMENT_NO, DEPARTMENT_NAME, REGION_NO, REGION_NAME',
     '|','DWH_FILES_OUT','alex_dept75.txt');
...
DBMS_OUTPUT.PUT_LINE('Sub Class extract');
    g_count := dwh_generic_file_extract(
     ...
     AND THIS_WEEK_START_DATE  BETWEEN '''||G_THIS_MN_START_DATE||''' AND '''||G_THIS_MN_END_DATE||'''  
     AND LOC_TYPE = ''S''
     AND DEPARTMENT_NO = 48 AND SUBCLASS_NO IN (2,3,4,5,6,7)
     GROUP BY FIN_YEAR_NO, FIN_WEEK_NO, LOCATION_NO, LOCATION_NAME, SUBCLASS_NO, SUBCLASS_NAME, REGION_NO, REGION_NAME',
     '|','DWH_FILES_OUT','aztec_subclass48.txt');
    l_text :=  'Records extracted to aztec_subclass.txt '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
DBMS_OUTPUT.PUT_LINE('Item extract 48');
    G_COUNT := DWH_GENERIC_FILE_EXTRACT(
    'WITH ITEM_LIST AS (SELECT ITEM_NO FROM DWH_FOUNDATION.FND_LINDT_ITEMS  WHERE DEPARTMENT_NO = 48)
     SELECT  /*+ PARALLEL(DENSE,8) FULL(DENSE  ) */
     LOC.LOCATION_NO, LOC.LOCATION_NAME, REGION_NO, REGION_NAME, ITM.ITEM_NO, ITM.ITEM_DESC, DENSE.FIN_YEAR_NO, DENSE.FIN_WEEK_NO,
     THIS_WEEK_START_DATE,DENSE.SALES_QTY, DENSE.SALES
     FROM DIM_ITEM ITM, DIM_LOCATION LOC, RTL_LOC_ITEM_WK_RMS_DENSE DENSE, ITEM_LIST IL
     WHERE LOC.SK1_LOCATION_NO = DENSE.SK1_LOCATION_NO
     AND ITM.SK1_ITEM_NO = DENSE.SK1_ITEM_NO
     AND ITM.ITEM_NO = IL.ITEM_NO
     AND THIS_WEEK_START_DATE  BETWEEN '''||G_THIS_MN_START_DATE||''' AND '''||G_THIS_MN_END_DATE||''' 
     ',
...
--    '|','DWH_FILES_OUT','aztec_item48.txt');    
    l_text :=  'Records extracted to aztec_item48.txt '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
DBMS_OUTPUT.PUT_LINE('Item extract 75');
    G_COUNT := DWH_GENERIC_FILE_EXTRACT(
    'WITH ITEM_LIST AS (SELECT ITEM_NO FROM DWH_FOUNDATION.FND_LINDT_ITEMS  WHERE DEPARTMENT_NO in (75, 6110, 6111, 6112, 6113, 6119))
     SELECT  /*+ PARALLEL(DENSE  ,8) FULL(DENSE  ) */
     LOC.LOCATION_NO, LOC.LOCATION_NAME, REGION_NO, REGION_NAME, ITM.ITEM_NO, ITM.ITEM_DESC, DENSE.FIN_YEAR_NO, DENSE.FIN_WEEK_NO,
     THIS_WEEK_START_DATE,DENSE.SALES_QTY, DENSE.SALES
     FROM DIM_ITEM ITM, DIM_LOCATION LOC, RTL_LOC_ITEM_WK_RMS_DENSE DENSE,ITEM_LIST IL
...
--    '|','DWH_FILES_OUT','aztec_item75.txt'); 
    l_text :=  'Records extracted to aztec_item75.txt '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);    

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_259U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_259U" (p_forall_limit in integer, p_success out boolean) as
...
g_rec_out            dwh_performance.foods_renewal_extract2_2wk_roq%rowtype;
g_count              number        :=  0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_259U';
...
type tbl_array_i is table of dwh_performance.foods_renewal_extract2_2wk_roq%rowtype index by binary_integer;
type tbl_array_u is table of dwh_performance.foods_renewal_extract2_2wk_roq%rowtype index by binary_integer;
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table dwh_performance.foods_renewal_extract2_2wk_roq');
...
INSERT /*+ APPEND PARALLEL (mart,4) */ INTO dwh_performance.foods_renewal_extract2_2wk_roq mart
with

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110U_WLTST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110U_WLTST" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as
--**************************************************************************************************
--  Date:        Sept 2008
...
--  Tables:      Input  - dwh_datafix.fnd_rtl_locitemdyrmssale_tst
--               Output - dwh_datafix.RTL_LOCITEMDYRMSDENSE_tst
...
g_rec_out            dwh_datafix.RTL_LOCITEMDYRMSDENSE_tst%rowtype;
...
type tbl_array_m is table of dwh_datafix.RTL_LOCITEMDYRMSDENSE_tst%rowtype index by binary_integer;
...
cursor c_fnd_rtl_locitemdyrmssale_tst is
   select /*+ full(di) full(dl) full(dlh) full(dih) full(fnd_li) parallel (fnd_li,6)  */  
          ...
          dih.sk2_item_no,
          dlh.sk2_location_no
   from   dwh_datafix.fnd_rtl_locitemdyrmssale_tst fnd_lid,
          dim_item di,
          dim_location dl,
...
g_rec_in                   c_fnd_rtl_locitemdyrmssale_tst%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_rtl_locitemdyrmssale_tst%rowtype;
...
begin
    forall i in a_tbl_merge.first .. a_tbl_merge.last
       save exceptions
MERGE /*+ USE_HASH(rtl_lidrd ,mer_rlidrd)*/ INTO dwh_datafix.RTL_LOCITEMDYRMSDENSE_tst rtl_lidrd
USING
(
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD OF dwh_datafix.RTL_LOCITEMDYRMSDENSE_tst EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    open c_fnd_rtl_locitemdyrmssale_tst;
    fetch c_fnd_rtl_locitemdyrmssale_tst bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_rtl_locitemdyrmssale_tst;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_120U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_120U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        SEPT 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_120U';
...
cursor c_fnd_rtl_shipment is
 ...
 -- and item_no in (6009204429444,6009207104065,6008000953412)
-- and to_loc_no = 9537
...
--          sum(case when rli.tax_perc is null then                                                   -- REF03Mar2018
          sum(case when   vr.VAT_RATE_PERC is null then                                               -- REF03Mar2018
                ...
                nvl(shp.received_qty,0) * (shp.reg_rsp * 100 / (100 +  vr.VAT_RATE_PERC  ))           -- REF03Mar2018  
--                 nvl(shp.received_qty,0) * (shp.reg_rsp * 100 / (100 + rli.tax_perc  ))             -- REF03Mar2018

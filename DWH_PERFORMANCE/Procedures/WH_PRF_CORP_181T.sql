--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_181T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_181T" (
...
--**************************************************************************************************
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_181T';
...
--      or (fnd.last_updated_date between G_DATE - 391 and  g_date) 
-- --
--  )
--    --    and cycle_no in(24,25)
...
WHEN matched THEN
  UPDATE
  SET CYCLE_FROM_DATE = f.cycle_from_date,

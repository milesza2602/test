--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_663U_FIXWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_663U_FIXWL"                                                                                                                                                                                                                 (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        March 2009
--  Author:      M Munnik
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_663U_FIXWL';
...
begin
   ...
   insert   /*+ APPEND */     into dwh_performance.temp_po_supchain_sum
WITH SELTST
...
OR (SK1_PO_NO = 23981287 AND SK1_ITEM_NO = 22271763 ) */
 ...
 selext as      (select    /*+   FULL(R) PARALLEL(R,4)  FULL(I) */ tl.sk1_po_no,
       ...
       join      seltst tl          on  r.sk1_po_no                = tl.sk1_po_no
       ...
       group by  tl.sk1_po_no, (case when l.loc_type = 'W' then lv.sk1_location_no else r.sk1_location_no end), r.sk1_item_no)
   select    /*+ FULL(PS) PARALLEL(PS,4) */   
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    
--              fillrate_actl_grn_qty          = a_tbl_update(i).fillrate_actl_grn_qty,

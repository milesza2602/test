--------------------------------------------------------
--  DDL for Procedure WH_PRF1_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF1_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF1_PO_DELETE_3JUN2015';
...
FOR V_CUR IN
            ...
            FROM DWH_PERFORMANCE.RTL_STOCKOWN_DCR A,
                  selext B
...
LOOP
 ...
 --                 l_text := 'RTL_STOCKOWN_DCR';
  --                dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
      begin
            DELETE /*+ FULL(A)  */ FROM DWH_PERFORMANCE.RTL_STOCKOWN_DCR  a
              where a.ORDER_NO = V_CUR.PO_NO;
              
              g_recs_DELETED_RSD := g_recs_DELETED_RSD + SQL%ROWCOUNT;
              commit;
              
                    if g_recs_DELETED_RSD >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_RSD;
        ...
        when no_data_found then
          g_recs_DELETED_RSD := g_recs_DELETED_RSD ;
      end;      
...
l_text := 'RTL_STOCKOWN_DCR recs deleted = '||g_recS_deleted_RSD;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
END WH_PRF1_PO_DELETE_3JUN2015;

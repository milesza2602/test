--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_769E_test
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_769E_test" (p_forall_limit in integer, p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2016
--  Author:      A Joshua
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_769E';
...
g_control_no         dwh_performance.rtl_sms_publish_supp.control_no%type;
...
/*
cursor c_stock_dly (p_supplier_no number) is
   with stock as
...
from
  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) tot_out_of_stock
    ...
    group by b.primary_supplier_no, b.business_unit_no ) tots
...
--MONTAGUE GARDENS
  ...
  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) mont_out_of_stock
   ...
   group by b.primary_supplier_no, b.business_unit_no) mont
  on     (tots.business_unit_no    = mont.business_unit_no
    and   tots.primary_supplier_no = mont.primary_supplier_no)

  left join -- row 5a

  (select  b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) monta_out_of_stock
   from    rtl_depot_item_dy a
    join   dim_item          b on a.sk1_item_no     = b.sk1_item_no
    join   dim_item_uda      c on b.sk1_item_no     = c.sk1_item_no
    join   dim_location      d on a.sk1_location_no = d.sk1_location_no
   ...
   group by b.primary_supplier_no, b.business_unit_no) monta
  on      (tots.business_unit_no    = monta.business_unit_no
    and    tots.primary_supplier_no = monta.primary_supplier_no)

  left join -- row 5b

  (select  b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) montb_out_of_stock
   from  rtl_depot_item_dy a
    join dim_item          b on a.sk1_item_no     = b.sk1_item_no
    join dim_item_uda      c on b.sk1_item_no     = c.sk1_item_no
    join dim_location      d on a.sk1_location_no = d.sk1_location_no
   ...
   group by b.primary_supplier_no, b.business_unit_no) montb
  on   (tots.business_unit_no    = montb.business_unit_no
    and tots.primary_supplier_no = montb.primary_supplier_no)

  left join -- -- row 5c

  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) montc_out_of_stock
   from   rtl_depot_item_dy a
    join  dim_item          b on a.sk1_item_no     = b.sk1_item_no
    join  dim_item_uda      c on b.sk1_item_no     = c.sk1_item_no
    join  dim_location      d on a.sk1_location_no = d.sk1_location_no
   ...
   group by b.primary_supplier_no, b.business_unit_no) montc
  on     (tots.business_unit_no    = montc.business_unit_no
    and   tots.primary_supplier_no = montc.primary_supplier_no)

--MIDRAND
  left join -- row 6 -- why not group on supplier as per Montague??
  ...
  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) mid_out_of_stock
   ...
   group by b.primary_supplier_no, b.business_unit_no) mid
  on      (tots.business_unit_no    = mid.business_unit_no
    and    tots.primary_supplier_no = mid.primary_supplier_no)

  left join -- row 6a

  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) mida_out_of_stock
   from   rtl_depot_item_dy a
    join  dim_item          b on a.sk1_item_no     = b.sk1_item_no
    join  dim_item_uda      c on b.sk1_item_no     = c.sk1_item_no
    join  dim_location      d on a.sk1_location_no = d.sk1_location_no
   ...
   where  a.post_date                     = l_dc_date  --g_date
    ...
    and   d.wh_fd_zone_no                 = 30
    and   c.product_class_desc_507        = 'A'
   group by b.primary_supplier_no, b.business_unit_no) mida
  on    (tots.business_unit_no    = mida.business_unit_no
   and   tots.primary_supplier_no = mida.primary_supplier_no)

  left join -- row 6b

  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) midb_out_of_stock
   from   rtl_depot_item_dy a
    join  dim_item          b on a.sk1_item_no     = b.sk1_item_no
    join  dim_item_uda      c on b.sk1_item_no     = c.sk1_item_no
    join  dim_location      d on a.sk1_location_no = d.sk1_location_no
   ...
   where  a.post_date                     = l_dc_date  --g_date
    ...
    and   d.wh_fd_zone_no                 = 30
    and   c.product_class_desc_507        = 'B'
   group by b.primary_supplier_no, b.business_unit_no) midb
  on     (tots.business_unit_no    = midb.business_unit_no
    and   tots.primary_supplier_no = midb.primary_supplier_no)

  left join -- row 6c

  (select b.primary_supplier_no, b.business_unit_no, sum(a.ll_dc_out_of_stock) midc_out_of_stock
   from   rtl_depot_item_dy a
    join  dim_item          b on a.sk1_item_no     = b.sk1_item_no
    join  dim_item_uda      c on b.sk1_item_no     = c.sk1_item_no
    join  dim_location      d on a.sk1_location_no = d.sk1_location_no
   ...
   where  a.post_date                     = l_dc_date  --g_date
    ...
    and   d.wh_fd_zone_no                 = 30
    and   c.product_class_desc_507        = 'C'
   group by b.primary_supplier_no, b.business_unit_no) midc
  on     (tots.business_unit_no    = midc.business_unit_no
    and   tots.primary_supplier_no = midc.primary_supplier_no)

--SUPPLIER ORDERFILL % FOR TOTAL FOODS (Montague)
  left join -- row 9
  ...
  (select c.supplier_no, b.business_unit_no,
   ...
   from rtl_po_supchain_loc_item_dy a,
        ...
        dim_item_uda d,
        dim_location e
   where tran_date                      = l_dc_date --g_date
   ...
   group by c.supplier_no, b.business_unit_no) mont_odf
  on (tots.business_unit_no      = mont_odf.business_unit_no
    and tots.primary_supplier_no = mont_odf.supplier_no)

--SUPPLIER ORDERFILL % FOR TOTAL FOODS (Midrand)
  left join -- row 10 new
  ...
  (select c.supplier_no, b.business_unit_no,
   ...
   group by c.supplier_no, b.business_unit_no) md_odf
...
/*
  ...
  (select c.supplier_no, b.business_unit_no,
   ...
   where tran_date                      = l_dc_date
    ...
    and a.sk1_location_no               = e.sk1_location_no
    and e.wh_fd_zone_no                 = 10
   group by c.supplier_no, b.business_unit_no, 0) mont_odf
  on (tots.business_unit_no      = mont_odf.business_unit_no
    and tots.primary_supplier_no = mont_odf.supplier_no)
*/
-- OUT OF STOCK
...
*/
-- OUT OF STOCK
  left join -- row 8 -- why not group on supplier as per Montague??
...
--   g_l_dc_date :=  to_char(trunc(l_dc_date),'dd/mm/yyyy');
   ...
   select nvl(max(control_no),0) into g_control_no from rtl_sms_publish_supp;

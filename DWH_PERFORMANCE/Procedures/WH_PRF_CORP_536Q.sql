--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_536Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_536Q" (
...
as
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_536Q';
...
begin
 ...
 FOR g_sub IN 0..1
  ...
  l_text := 'INSERT INTO TEMP_LOC_ITEM_WK_CATALOG STARTING ';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  --**************************************************************************************************
  -- Insert Into TEMP_LOC_ITEM_WK_CATALOG
  --**************************************************************************************************
  insert /*+ PARALLEL(CAT,4) */ into dwh_performance.TEMP_LOC_ITEM_WK_CATALOG CAT
  ...
  insert /*+ PARALLEL(CAT,6) */
  into W6005682.RTL_LOC_ITEM_WK_CATALOG_11
  select  /*+ PARALLEL(lid,6) FULL(lid) PARALLEL(z,6) FULL(z) */

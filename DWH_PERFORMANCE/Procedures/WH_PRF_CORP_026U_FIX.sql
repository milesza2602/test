--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_026U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_026U_FIX" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Sept 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_026U_FIX';
...
from fnd_location fl,
   ...
   where fl.district_no  = dd.district_no and
         ...
         fl.wh_org_hrchy_value = dc.chain_no(+) and
         fl.last_updated_date = G_DATE
         and wh_zone_no is null
         and fl.location_no = 196;

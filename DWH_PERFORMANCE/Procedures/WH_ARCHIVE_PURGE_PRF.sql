--------------------------------------------------------
--  DDL for Procedure WH_ARCHIVE_PURGE_PRF
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_ARCHIVE_PURGE_PRF" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2017
--  Author:      Quentin Smit
...
L_Process_Type       Sys_Dwh_Log_Summary.Log_Process_Type%Type := Dwh_Constants.Vc_Log_Process_Type_N;
   ...
   SELECT *  
    ...
    WHERE TABLE_NAME  NOT IN ('FND_DEAL_LOC_ITEM_DY','FND_RTL_ALLOCATION','FND_RTL_PURCHASE_ORDER','FND_RTL_SHIPMENT_ST_ORD','FND_RTL_CLEARANCE','FND_RTL_LOC_ITEM_DY_RMS_INBND',
                              'FND_ITEM_VAT_RATE','FND_RTL_RTV','FND_RTL_RTV_TRANSACTION','FND_RTL_SNGL_RTV_WH_FAULT_DTL','FND_RTL_PRICE_HISTORY','FND_RTL_INVOICE_MATCHING',
                              'FND_RTL_SHIPMENT','FND_RTL_LOC_ITEM_DY_CH_AVAIL')
     
 ORDER BY TABLE_NAME  ;
 
   -- NON-PARTITIONED PRF TABLES
   --===========================
   CURSOR C3 IS
     SELECT *  
       FROM W6005682.PRF_NON_PART_ARCH_TABS1
      WHERE TABLE_NAME NOT IN ('RTL_DEPOT_ITEM_WK_BRIDGETHORN','RTL_CONTRCT_CHN_ITEM_WK_BOC_DJ','RTL_CONTRACT_CHAIN_ITEM_WK_DJ','RTL_CONTRACT_CHAIN_ITEM_WK_BOC',
                                'RTL_CONTRACT_CHAIN_ITEM_WK','MART_SUPPLIER_MASTER_DATA_TEMP','MART_FOODS_SUPPLIER','MART_FD_AREA_ITEM_DY')
      ORDER BY TABLE_NAME;
...
--            END IF;
            ...
            IF L_TABLE_SIZE > 400 THEN
               L_STATE := 'MANUAL_COPY_OUT_NEEDED';
               L_TEXT := ' TABLE ' || L_TABLE_NAME || ' FOR YEAR ' || L_YEAR_NO || ' - SIZE > 400MB - MANUAL COPY OUT NEEDED ';
            ELSE
               L_STATE := 'AUTO_DELETE_OK';
               L_TEXT := ' TABLE ' || L_TABLE_NAME || ' FOR YEAR ' || L_YEAR_NO || ' - SIZE < 400MB - DELETE WILL BE OK';
            END IF;
            DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
            
            INSERT INTO W6005682.WW_ARCH_METADATAQ (WHEN_LOADED, TABLE_SEQUENCE,  TABLE_OWNER, TABLE_NAME,  TABLE_ROW_COUNT, WHERE_CLAUSE, EXT_TABLE_NAME, EXT_TABLE_DDL, EXT_TABLE_FILE_NAMES, STATE, AVG_ROW_LEN, TABLE_SIZE, EXT_YEAR_NO)  --, INTERVAL_VAL) 
            VALUES (SYSDATE, L_SEQ_NO, L_TABLE_OWNER, L_TABLE_NAME, L_REC_CNT, L_WHERE_CLAUSE, L_EXT_TABLE_NAME, L_EXT_TABLE_DDL_SQL, L_EXT_TABLE_FILE_NAMES, L_STATE, L_AVG_ROW_LEN, L_TABLE_SIZE, L_YEAR_NO);   --, L_INTERVAL);
            COMMIT;
...
--==========================================================================================================
procedure load_non_part_prf_table_data as
begin
 ...
 FOR r IN C3
   LOOP
     ...
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     dbms_output.put_line('--** TABLE BEING PROCESSED - ' || L_TABLE_NAME); 
     l_text := '--** TABLE BEING PROCESSED - ' || L_TABLE_NAME || ' **--';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    END IF;
     ...
     --L_CURSOR_STRING := 'SELECT B.FIN_YEAR_NO, COUNT(*) FROM ' || L_TABLE_NAME || ' A, DIM_CALENDAR B WHERE A.' || L_WHERE_FIELD || ' = B.CALENDAR_DATE AND A.' || L_WHERE_FIELD || ' < ''29/JUN/09'' GROUP BY B.FIN_YEAR_NO ORDER BY B.FIN_YEAR_NO;';
     
     -- DETERMINE SQL BASED ON WHETHER THE TABLE IS TO BE ACCESSED BY YEAR/WEEK OR DATE 
     IF L_WHERE_FIELD = 'YEAR_WEEK' THEN
        L_CURSOR_STRING := 'SELECT /*+ FULL(A) PARALLEL(A,4) */ A.FIN_YEAR_NO,  COUNT(*) FROM ' || L_TABLE_NAME || ' A WHERE A.FIN_YEAR_NO < 2013 GROUP BY A.FIN_YEAR_NO ORDER BY A.FIN_YEAR_NO' ;
     ELSE
        L_CURSOR_STRING := 'SELECT /*+ FULL(A) PARALLEL(A,4) */ B.FIN_YEAR_NO,  COUNT(*) FROM ' || L_TABLE_NAME || ' A, DIM_CALENDAR B WHERE A.' || L_WHERE_FIELD || ' = B.CALENDAR_DATE AND A.' || L_WHERE_FIELD || ' < ''25/JUN/12'' GROUP BY B.FIN_YEAR_NO ORDER BY B.FIN_YEAR_NO';
     END IF;
     
     --' < :L_DATE GROUP BY B.FIN_YEAR_NO ORDER BY B.FIN_YEAR_NO;';
     
     
                  
     --dbms_output.put_line(L_CURSOR_STRING);
     --dbms_output.put_line('GOING TO OPEN CURSOR');
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,L_CURSOR_STRING);
     ...
     OPEN V_DATE_CURSOR FOR L_CURSOR_STRING;  -- USING '29/JUN/09';
        ...
        LOOP
            ...
            --L_EXT_TABLE_NAME := L_TABLE_NAME || '_' || l_year_no;
            --L_DIRECTORY       := 'DWHPRD_ARCHIVE/' || L_YEAR_NO;
            L_DIRECTORY := 'QS_ARCH_' || L_YEAR_NO;
            ...
            IF L_WHERE_FIELD = 'YEAR_WEEK' THEN
                ...
                -- SET UP THE START OF THE EXTERNAL TABLE DDL SQL - YEAR/WEEK WHERE CLAUSE
                ---------------------------------------------------------------------
                --L_EXT_TABLE_DDL_SQL := 'CREATE TABLE ' || L_TABLE_NAME || '_' || l_year_no || '_EXT organization external (type oracle_datapump default directory ' || l_directory
                L_EXT_TABLE_DDL_SQL := 'CREATE TABLE ' || L_EXT_TABLE_NAME || ' organization external (type oracle_datapump default directory ' || l_directory
                 || ' location ( '
                 ...
                 || ''''
                 || ') '
                 || ') parallel (degree 4) as select /*+ parallel (t,4)  full(t) */ t.* from ' || L_TABLE_NAME || ' t  WHERE t.fIN_YEAR_NO = ' || L_YEAR_NO  ;

                L_WHERE_CLAUSE := 'WHERE FIN_YEAR_NO = ' || L_YEAR_NO;
            
            ELSE
            
                -- SET UP THE START OF THE EXTERNAL TABLE DDL SQL - DATE WHERE CLAUSE
                ---------------------------------------------------------------------
                --L_EXT_TABLE_DDL_SQL := 'CREATE TABLE ' || L_TABLE_NAME || '_' || l_year_no || '_EXT organization external (type oracle_datapump default directory ' || l_directory
                L_EXT_TABLE_DDL_SQL := 'CREATE TABLE ' || L_EXT_TABLE_NAME || ' organization external (type oracle_datapump default directory ' || l_directory
                 || ' location ( '
            ...
            END IF;
            --dbms_output.put_line (L_EXT_TABLE_DDL_SQL); 
            --dbms_output.put_line(' ');  
            --dbms_output.put_line (L_WHERE_CLAUSE); 
...
end load_non_part_prf_table_data;
--==========================================================================================================

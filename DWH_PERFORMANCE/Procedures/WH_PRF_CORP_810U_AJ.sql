--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_810U_AJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_810U_AJ" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_810U_AJ';                              
...
begin
 ...
 FOR g_sub IN 0..0
  LOOP
   ...
   merge /* parallel (rtl,8) append */ into dwh_datafix.aj_rtl_loc_item_wk_plan_facts rtl 
   using (
    with 
        ans_list  as 
...
--                and  ans.sk1_item_no = 17533076
        ...
        distinct_key as (
            select /*+ full (ans) parallel (grp,6) */
                   distinct sk1_location_no, sk1_planogram_id, sk1_fixture_seq_no,
...
--              and  sk1_item_no = 17533076
        ...
        distinct_cnt as ( 
            select distinct count(*) as cnt, sk1_location_no, sk1_item_no, fin_year_no, fin_week_no, 
                                             this_week_start_date
            from distinct_key 
...
--            having count(*) > 1
        ...
        sales_measures as 
            ...
            select distinct
                ...
                nvl(nvl(nvl(nvl(f0.sk1_fixture_seq_no, f1.sk1_fixture_seq_no), f2.sk1_fixture_seq_no), f3.sk1_fixture_seq_no), f4.sk1_fixture_seq_no) sk1_fixture_seq_no,
            ...
            from catalog_list f0
               ...
               full outer join
                 multi_dsp f4
                   ...
                   and nvl(nvl(nvl(f0.sk1_fixture_seq_no, f1.sk1_fixture_seq_no), f2.sk1_fixture_seq_no), f3.sk1_fixture_seq_no)  = f4.sk1_fixture_seq_no
            group by
                 ...
                 nvl(nvl(nvl(nvl(f0.sk1_fixture_seq_no, f1.sk1_fixture_seq_no), f2.sk1_fixture_seq_no), f3.sk1_fixture_seq_no), f4.sk1_fixture_seq_no),
   ...
   on    (rtl.sk1_item_no	        =	mer_rec.sk1_item_no      and
          ...
          rtl.sk1_planogram_id    =	mer_rec.sk1_planogram_id and
...
--    l_text :=  dwh_constants.vc_log_records_read||g_recs_read;
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       raise;
END wh_prf_corp_810u_aj                                                                                             -- STORE PROC CHANGE 
;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_030C
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_030C" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - fnd_group, fnd_business_unit
--               Output - dim_group
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  23 April 2009 - defect 1356 - Change total_descr from plural to singular
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            dim_group%rowtype;
g_rec_in             fnd_group%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_030C';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE DIM_GROUP EX FND_GROUP';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_group%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_group%rowtype index by binary_integer;
type tbl_array_u is table of dim_group%rowtype index by binary_integer;
...
---------------------------------------------------------
   g_rec_out.group_long_desc            := g_rec_in.group_no||' - '||g_rec_out.group_name;
   g_rec_out.business_unit_long_desc    := g_rec_in.business_unit_no||' - '||g_rec_out.business_unit_name;
   g_rec_out.company_long_desc          := g_rec_out.company_no||' - '||g_rec_out.company_name;
   g_rec_out.total                      := 'TOTAL';
   g_rec_out.total_desc                 := 'ALL GROUP';
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_group values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_group
      set    group_name               = a_tbl_update(i).group_name,
...
begin
   g_found := dwh_valid.dim_group(g_rec_out.group_no);

-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_group_no  := merch_hierachy_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF DIM_group EX FND_group STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_group;
    fetch c_fnd_group bulk collect into a_stg_input limit g_forall_limit;

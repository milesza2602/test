--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_661U_BCK11092015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_661U_BCK11092015" 
...
--                The table temp_po_list_dj contains the POs updated during batch or between selected from and to not_before_dates.
--                The not_before_date/not_after_date for a PO may change,
--                but this can only happen if there are no shipments yet.
--                This program will delete the records which have not been updated today, but exist in temp_po_list_dj.
--                These will be the records where the not_before_date/not_after_date is the tran_date for the record,
--                but the not_before_date/not_after_date has changed after the record was inserted to rtl_po_supchain_loc_item_dy_dj.
...
cursor c_fnd_po_ship is
   ...
   (select    s.po_no, s.to_loc_no, s.item_no, sum(s.asn_qty) bc_shipment_qty,
    ...
    join      temp_po_list_dj pl    on  s.po_no = pl.po_no and s.item_no = pl.item_no
   ...
   (select    p.po_no, pl.supply_chain_type, p.location_no, p.item_no, di.business_unit_no, pl.param_ind,
    ...
    join      temp_po_list_dj pl            on  p.po_no              = pl.po_no
    ...
    left join dim_dj_contract dc         on  p.contract_no        = dc.contract_no
...
Bc_Shipment_Qty,Bc_Shipment_Selling,Bc_Shipment_Cost,p.sk1_chain_code_ind,
 -- QC4604 START
(Case
...
p.param_ind,p.sk1_po_no,p.sk1_supply_chain_no,p.sk1_location_no,p.sk1_item_no,
p.sk1_contract_no,p.sk1_supplier_no,p.sk1_chain_code_ind,
p.SK2_LOCATION_NO,p.SK2_ITEM_NO,p.FD_DISCIPLINE_TYPE,
p.STANDARD_UOM_CODE,p.RANDOM_MASS_IND,p.DEPT_PLACEHOLDER_02_IND,p.DEPT_PLACEHOLDER_03_IND,
p.NOT_BEFORE_DATE,p.NOT_AFTER_DATE,p.PO_STATUS_CODE,p.CANCEL_CODE,p.REG_RSP,
p.FD_FROM_LOC_REG_RSP,p.COST_PRICE,p.VAT_RATE_PERC,p.STATIC_MASS,p.DEBTORS_COMMISSION_PERC,
p.CHAIN_NO,p.PACK_ITEM_SIMPLE_IND,p.NUM_UNITS_PER_TRAY,p.UDA_ITEM501,p.UDA_ITEM1601,
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_po_supchain_loc_item_dy_dj values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_po_supchain_loc_item_dy_dj
       set    sk1_contract_no		              = a_tbl_update(i).sk1_contract_no,
...
begin
    ...
    from   rtl_po_supchain_loc_item_dy_dj r
...
--**************************************************************************************************
    ...
    if a_stg_input.count > 0 then
       ...
       if g_param_ind = 1 then
          ...
          l_text := 'TABLE TEMP_PO_DELETES_DJ TRUNCATED.';
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       end if;
...
end wh_prf_dj_661u_BCK11092015;

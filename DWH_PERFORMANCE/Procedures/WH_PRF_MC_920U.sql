--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_920U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_920U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2018
...
--               Output - RTL_MC_LOC_ITEM_DY_SIMANTIC
...
g_rec_out            RTL_MC_LOC_ITEM_DY_SIMANTIC%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_920U';
...
begin
    ...
    l_text := 'LOAD OF RTL_MC_LOC_ITEM_DY_SIMANTIC EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    MERGE /*+ parallel(rtl_sim,4) */ INTO RTL_MC_LOC_ITEM_DY_SIMANTIC rtl_sim
    USING
    (
...
--                       AND   SIMR.POST_DATE       = STK.POST_DATE)
    ...
    and mer_rec.POST_DATE       = rtl_sim.POST_DATE)
    WHEN MATCHED
    THEN
    UPDATE
    SET           COM_FLAG_IND	=	MER_REC.	COM_FLAG_IND	,
...
/*    
    select sum(sales) 
    into g_prf_sale
    from RTL_MC_LOC_ITEM_DY_SIMANTIC

--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_030U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_030U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_BRTH_030U';
...
--**************************************************************************************************
   ...
   where this_week_start_date = (select this_week_start_date from dim_calendar where calendar_date = g_end_date);
    ...
    execute immediate ('truncate table dwh_performance.rtl_supp_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_performance.rtl_supp_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'rtl_supp_item_wk_bridgethorn', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_performance.rtl_supp_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
WITH selsupp AS
  ...
  FROM dwh_performance.rtl_zone_item_supp_dy rzia,
    ...
    WHERE rzi.calendar_date BETWEEN g_last_week_start_date AND g_last_week_end_date
  ...
  and (RAI.cancel_code <> 'B'
  or RAI.cancel_code is null)
...
AND ss.sk1_supplier_no = ssc.sk1_supplier_no
  ...
  loop
   INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.RTL_SUPP_ITEM_WK_BRIDGETHORN  values
    (v_cur.fin_year_no ,
     v_cur.fin_week_no ,
     v_cur.this_week_start_date ,
     v_cur.item_no ,
     v_cur.supplier,
     v_cur.latest_po_cases ,
     v_cur.po_grn_cases ,
     v_cur.supplier_service_level ,
     v_cur.current_supplier_ind  );

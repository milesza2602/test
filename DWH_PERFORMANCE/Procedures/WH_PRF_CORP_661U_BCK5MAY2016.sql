--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_661U_BCK5MAY2016
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_661U_BCK5MAY2016" 
...
--
--     4 may 2016  wl  -- added code to exclude certain beverages due to pack-item issue with data from source
...
-----------------------
-- Temporary section start - 4 may 2016
------------------------------------------------------------------------------------------------------
-- 4 May 2016 - wendy lyttle
--   due to data issues from source regarding pack-item information setup incorrectly,
--   we have to bypass the following items until a resolution is in place
--
--    6001134087305
--    5449000050229
--    5449000010117
...
--    5449000131843
   ...
   begin
     select count(*) into g_cnt from dwh_performance.temp_po_list
     where item_no in (6001134087305
                        ,5449000050229
                        ,5449000010117
                        ,5449000010049
                        ,5449000120984
     ...
     if g_cnt > 0
        then 
             delete from dwh_performance.temp_po_list
             where item_no in (6001134087305
                        ,5449000050229
                        ,5449000010117
                        ,5449000010049
        ...
        L_TEXT := 'Records deleted from temp_po_list = '||g_cnt;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
    
    EXCEPTION
  WHEN no_data_found THEN
    l_text := 'no data in dwh_performance.temp_po_list';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
--
--
-----------------------
-- Temporary section end - 4 may 2016
------------------------------------------------------------------------------------------------------
...
end wh_prf_corp_661U_bck5may2016;

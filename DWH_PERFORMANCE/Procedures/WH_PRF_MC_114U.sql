--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_114U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_114U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_114U';
...
type tbl_array_m is table of rtl_mc_loc_item_dy_rms_sparse%rowtype index by binary_integer;
...
-- and shp.to_loc_no           = 207
-- and shp.item_no             in (6009207469676,6009207598017,6009207480527)
         ...
         sum(nvl(shp.sdn_qty,0)) as trunked_qty,
          ...
          sum(case when vr.VAT_RATE_PERC is null then                                                         -- REF03Mar2018
                (case when dl.vat_region_no = 1000 then
                     nvl(shp.sdn_qty,0) * (shp.reg_rsp_opr * 100 / (100 + di.VAT_RATE_PERC)) 
                 else
                     nvl(shp.sdn_qty,0) * (shp.reg_rsp_opr * 100 / (100 + dl.default_tax_region_no_perc)) 
                 end)
              else 
                 nvl(shp.sdn_qty,0) * (shp.reg_rsp_opr * 100 / (100 + vr.VAT_RATE_PERC))                          -- REF03Mar2018 
           end) as trunked_selling,       

          sum(case when vr.VAT_RATE_PERC is null then                                                         -- REF03Mar2018
                (case when dl.vat_region_no = 1000 then
                     nvl(shp.sdn_qty,0) * (shp.reg_rsp_local * 100 / (100 + di.VAT_RATE_PERC)) 
                 else
                     nvl(shp.sdn_qty,0) * (shp.reg_rsp_local * 100 / (100 + dl.default_tax_region_no_perc)) 
                 end)
              else 
                 nvl(shp.sdn_qty,0) * (shp.reg_rsp_local * 100 / (100 + vr.VAT_RATE_PERC))                          -- REF03Mar2018 
           end) as trunked_selling_local,   
          
          sum(nvl(shp.sdn_qty,0) * shp.cost_price_opr) as trunked_cost,
          sum(nvl(shp.sdn_qty,0) * shp.cost_price_local) as trunked_cost_local,          
...
begin
   ...
   g_rec_out.trunked_cost_local             := g_rec_in.trunked_cost_local;
   g_rec_out.trunked_selling_local          := g_rec_in.trunked_selling_local;
   ...
   if g_rec_in.business_unit_no = 50 and  g_rec_in.standard_uom_code = 'EA' and g_rec_in.random_mass_ind = 1 then
      ...
      g_rec_out.trunked_cost_local           := g_rec_out.trunked_cost_local * g_rec_in.static_mass;
      g_rec_out.trunked_selling_local        := g_rec_out.trunked_selling_local * g_rec_in.static_mass;
   end if;
--   g_rec_out.grn_qty     := 0;
...
/* Removed and placed into 116u
   if g_rec_in.gifting_department_ind = 0 and g_rec_in.book_magazine_dept_ind = 0 then
      begin
         ...
         from   rtl_mc_loc_item_dy_rms_sparse
...
begin
    forall i in a_tbl_merge.first .. a_tbl_merge.last
       save exceptions
MERGE INTO rtl_mc_loc_item_dy_rms_sparse rtl_lidrs
USING
...
UPDATE SET   trunked_qty             = mer_lidrs.trunked_qty,
             ...
             trunked_selling_local   = mer_lidrs.trunked_selling_local,
             trunked_cost_local      = mer_lidrs.trunked_cost_local,             
...
(
        ...
        rtl_lidrs.TRUNKED_COST_LOCAL,
        rtl_lidrs.TRUNKED_SELLING_LOCAL,
...
(
         ...
         mer_lidrs.TRUNKED_COST_LOCAL,
         mer_lidrs.TRUNKED_SELLING_LOCAL,
...
begin
    ...
    l_text := 'LOAD OF rtl_mc_loc_item_dy_rms_sparse EX FOUNDATION STARTED AT '||
...
--g_date := '28 dec 2017'; -- testing only

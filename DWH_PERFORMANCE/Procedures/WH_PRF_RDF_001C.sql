--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_001C
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_001C" 
...
g_rec_out            rtl_loc_item_dy_rdf_fcst%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_001C';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD RDF DAILY FCST FACTS EX RMS TABLE';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of rtl_loc_item_dy_rdf_fcst%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_item_dy_rdf_fcst%rowtype index by binary_integer;
...
cursor c_fnd_rtl_loc_item_wk_dyfcst is
  ...
  select  fcst.sk1_item_no,
          ...
          fcst.post_date,
          fcst.sales_dly_sys_fcst_qty,
          fcst.sales_dly_sys_fcst,
          fcst.sales_dly_app_fcst_qty,
          fcst.sales_dly_app_fcst,
          dl.forecastable_ind,
          dl.active_store_ind,
          dl.chain_no,
          nvl(lic.fd_num_catlg_days,0) fd_num_catlg_days
  from    rtl_loc_item_dy_catalog lic
  join    dim_item di on
          di.sk1_item_no          = lic.sk1_item_no
  join    dim_location dl on
          dl.sk1_location_no      = lic.sk1_location_no
  join    rtl_loc_item_dy_rdf_fcst fcst on
          lic.sk1_item_no         = fcst.sk1_item_no and
          lic.sk1_location_no     = fcst.sk1_location_no and
          lic.calendar_date       = fcst.post_date
  where   lic.calendar_date       = g_date;
...
begin
   ...
   if g_rec_in.sales_dly_app_fcst_qty is null then
      ...
      if (g_rec_in.forecastable_ind  = 1  and
          ...
          g_rec_out.fcst_err_sls_dly_app_fcst_qty := g_rec_in.sales_dly_app_fcst_qty;
          g_rec_out.fcst_err_sales_dly_app_fcst   := g_rec_in.sales_dly_app_fcst;
      else
          g_rec_out.fcst_err_sls_dly_app_fcst_qty := null;
          g_rec_out.fcst_err_sales_dly_app_fcst   := null;
      end if;
   ...
   if g_rec_in.sales_dly_sys_fcst_qty is null then
      ...
      if (g_rec_in.forecastable_ind  = 1  and
          ...
          g_rec_in.chain_no          = 10 and
          g_rec_in.fd_num_catlg_days > 0)  then 
          g_rec_out.fcst_err_sls_dly_sys_fcst_qty := g_rec_in.sales_dly_sys_fcst_qty;
          g_rec_out.fcst_err_sales_dly_sys_fcst   := g_rec_in.sales_dly_sys_fcst;
      else
          g_rec_out.fcst_err_sls_dly_sys_fcst_qty  := null;
          g_rec_out.fcst_err_sales_dly_sys_fcst    := null;
      end if;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_item_dy_rdf_fcst values a_tbl_insert(i);

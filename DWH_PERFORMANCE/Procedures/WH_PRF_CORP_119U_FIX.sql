--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_119U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_119U_FIX" (p_forall_limit in integer,p_success out boolean) as
...
cursor c_fnd_rtl_allocation is
   select sum(nvl(aloc.alloc_qty,0)) alloc_qty,
          ...
          max(aloc.reg_rsp_excl_vat) reg_rsp_excl_vat,
  ...
  -- qc4779         nvl(aloc.alloc_qty,0)      <> 0 AND
   ...
   group by di.sk1_item_no, dl.sk1_location_no, aloc.release_date;
...
-- Place data into and array for later writing to table in bulk
   if not g_found then
      ...
      if g_ch_alloc_selling <> g_rec_out.ch_alloc_selling or
         g_ch_alloc_qty     <> g_rec_out.ch_alloc_qty then
...
--**************************************************************************************************
WHILE G_DATE < '30/JUN/15' LOOP

    G_START_DATE := G_DATE - 91;

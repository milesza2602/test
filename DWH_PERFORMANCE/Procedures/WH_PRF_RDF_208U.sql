--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_208U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_208U" 
                                                                                                                                                                                                                                                                                (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        September 2008
--  Author:      Alastair de Wet
--  Purpose:     Update RDF Corrected sales/Lost sales fact table in performance layer
--               with input ex RMS catalog table from performance layer.
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_208U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rdf;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'UPDATE RDF LOST/CORR SALES FACTS EX FOUNDATION';
...
cursor c_rtl_loc_item_dy_rdf_sale is
   ...
   select rdf.sk1_item_no,
          ...
          dl.active_store_ind,
          dl.chain_no,
          lic.fd_num_catlg_days
   from   rtl_loc_item_dy_rdf_sale rdf,
          dim_item di,
          dim_location dl,
          rtl_loc_item_dy_catalog lic
   where  rdf.last_updated_date      = g_date and
          rdf.sk1_item_no            = di.sk1_item_no  and
          rdf.sk1_location_no        = dl.sk1_location_no and
          rdf.sk1_item_no            = lic.sk1_item_no and
          rdf.sk1_location_no        = lic.sk1_location_no and
          rdf.post_date              = lic.calendar_date ;
...
begin
   ...
   if g_rec_in.corr_sales_qty is null then
      g_rec_out.fcst_err_corr_sales_qty      := null;
      g_rec_out.fcst_err_corr_sales          := null;
   else
      if (g_rec_in.forecastable_ind  = 1  and
          ...
          g_rec_out.fcst_err_corr_sales_qty  := g_rec_in.corr_sales_qty;
          g_rec_out.fcst_err_corr_sales      := g_rec_in.corr_sales;
      else
          g_rec_out.fcst_err_corr_sales_qty  := null;
          g_rec_out.fcst_err_corr_sales      := null;
      end if;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    last_updated_date               = a_tbl_update(i).last_updated_date,
              fcst_err_corr_sales_qty         = a_tbl_update(i).fcst_err_corr_sales_qty,
              fcst_err_corr_sales             = a_tbl_update(i).fcst_err_corr_sales

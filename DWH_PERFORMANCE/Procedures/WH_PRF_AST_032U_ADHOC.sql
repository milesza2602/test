--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_032U_ADHOC
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_032U_ADHOC" 
...
l_module_name           sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_032U_ADHOC';
l_name                  sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name           sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name           sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name        sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text                  sys_dwh_log.log_text%type ;
l_description           sys_dwh_log_summary.log_description%type  := 'LOAD THE ATRRIBUTE/PRODUCT VIEW DATAMART - 2013 DATA';
...
/*
   ...
   where  fin_year_no = g_fin_year_no_ly;
*/
   g_6wk_bck_start_date_ly := '26 dec 11';
   g_calendar_end_date_ly  := '24 jun 12';
   ...
   execute immediate 'alter session enable parallel dml';
      ...
      insert /*+ APPEND parallel (mart,4) */ into dwh_datafix.mart_ch_ast_chn_sc_wk_to mart
...
--             dim_calendar_wk cal
      where  apln.sk1_plan_type_no = 60
       and   apln.fin_year_no      = 2013
       and   apln.fin_week_no      between 27 and 52
      group by apln.sk1_chain_no, apln.sk1_style_colour_no, apln.fin_year_no, apln.fin_week_no, 'W'||apln.fin_year_no||apln.fin_week_no),
-- Original Plan - extract2
      orig_data as (
      select /*+ parallel (orig,6) */
             orig.sk1_chain_no,
             orig.sk1_style_colour_no,
             orig.fin_year_no,
             orig.fin_week_no,
             'W'||orig.fin_year_no||orig.fin_week_no as fin_week_code,
...
--             dim_calendar cal
      where  orig.sk1_plan_type_no = 62
       and   orig.fin_year_no      = 2013
       and   orig.fin_week_no      between 27 and 52
      group by orig.sk1_chain_no, orig.sk1_style_colour_no, orig.fin_year_no, orig.fin_week_no, 'W'||orig.fin_year_no||orig.fin_week_no),
-- Actualised Plan - extract3  -- must change to include prom_reg (qty and sell)
      act_data as (
      select /*+ parallel (act,6) */
             act.sk1_chain_no,
             act.sk1_style_colour_no,
             act.fin_year_no,
             act.fin_week_no,
             'W'||act.fin_year_no||act.fin_week_no as fin_week_code,
...
--             sum(act.selling_price) * sum(sit_qty) sit_selling,
 ...
 --            dim_calendar cal
      where  act.sk1_plan_type_no = 63
       and   act.fin_year_no      = 2013
       and   act.fin_week_no      between 27 and 52
      group by act.sk1_chain_no, act.sk1_style_colour_no, act.fin_year_no, act.fin_week_no, 'W'||act.fin_year_no||act.fin_week_no),
-- Pre-Actualised Plan - extract4
      preact_data as (
      select /*+ parallel (preact,6) */
             preact.sk1_chain_no,
             preact.sk1_style_colour_no,
             preact.fin_year_no,
             preact.fin_week_no,
             'W'||preact.fin_year_no||preact.fin_week_no as fin_week_code,
...
--             dim_calendar cal
      where  preact.sk1_plan_type_no  = 64
       and   preact.fin_year_no       = 2013
       and   preact.fin_week_no       between 27 and 52
      group by preact.sk1_chain_no, preact.sk1_style_colour_no, preact.fin_year_no, preact.fin_week_no, 'W'||preact.fin_year_no||preact.fin_week_no),
-- Approved Chain StyleColour - extract5
      aplnchain_data as (
      select /*+ parallel (aplnch,6) */
             aplnch.sk1_chain_no,
...
--             dim_calendar cal
      where  aplnch.sk1_plan_type_no = 60
       and   aplnch.fin_year_no      = 2013
       and   aplnch.fin_week_no      between 27 and 52),
-- Original Chain StyleColour- - extract6
      origchain_data as (
      select /*+ parallel (oplnch,6) */
             oplnch.sk1_chain_no,
...
--             dim_calendar cal
      where  oplnch.sk1_plan_type_no  = 62
       and   oplnch.fin_year_no       = 2013
       and   oplnch.fin_week_no     between 27 and 52),
-- Actualised Chain StyleColour - extract7
      actchain_data as (
      select /*+ parallel (actch,6) */
             actch.sk1_chain_no,
...
--             dim_calendar cal
      where  actch.sk1_plan_type_no   = 63
       and   actch.fin_year_no        = 2013
       and   actch.fin_week_no       between 27 and 52),
-- Pre-Actualised Chain StyleColour - extract8
      preactchain_data as (
      select /*+ parallel (preactch,6) */
...
--             dim_calendar cal
      where  preactch.sk1_plan_type_no   = 64
       and   preactch.fin_year_no        = 2013
       and   preactch.fin_week_no      between 27 and 52),
-- LY Original Plan - extract9
      ...
      where  lyorig.sk1_plan_type_no  = 62
       and   lyorig.fin_year_no    = 2012
       and   lyorig.fin_week_no    between 27 and 52
...
-- LY Actualised Plan - extract10
      ...
      where  lyact.sk1_plan_type_no  = 63
       and   lyact.fin_year_no  = 2012
       and   lyact.fin_week_no  between 27 and 52
...
-- Actual RMS Bought - extract11
      rms_act_data as (
         select /*+ parallel (rmsact,6) index (rmsact I16_N_RTL_CNTRCT_CHN_SC_WK) */
             rmsact.sk1_chain_no,
...
--             dim_calendar cal
      where  rmsact.sk1_style_colour_no = sc.sk1_style_colour_no
       and   rmsact.fin_year_no         = 2013
       and   rmsact.fin_week_no         between 27 and 52
...
--      , dim_calendar b
      where  a.fin_year_no = 2013
       and   a.fin_week_no between 27 and 52),

      rms_stock_data as (
      select /*+ parallel (stk,6) index (stk PK_P_RTL_LC_SC_WK_RMS_STCK) */
             act.sk1_chain_no,
      ...
      from   rtl_loc_sc_wk_rms_stock stk,
             dim_location loc
      where  stk.fin_year_no = 2013 and
             stk.fin_week_no between 27 and 52 and
             stk.sk1_location_no     = loc.sk1_location_no     and
             loc.chain_no            = 30

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_009U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_009U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
g_sub                PLS_INTEGER   :=  0;
g_loop_cnt           PLS_INTEGER   :=  36; -- Number of partitions to be truncated/replaced include future weeks
...
--  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  INSERT /*+ APPEND  parallel (X,g_degrees) */ INTO DWH_PERFORMANCE.RTL_EMP_JOB_WK X
  WITH seldat as (
      select /*+ FULL(RTL) parallel(RTL,g_degrees) */ 
      ...
      FROM dwh_performance.rtl_emp_job_dy jd,
       ...
       where jd.sk1_employee_id = sd.sk1_employee_id
        ...
        l_TEXT := l_table_name||' Inserted recs = '||g_recs ||' for Fin '||g_loop_fin_year_no||'w'||g_loop_fin_week_no;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

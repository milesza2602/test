--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_050U_TST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_050U_TST" 
...
--  
--  Maintenance:
--  26 Jun 18 - include missing items only       
--  
--
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_BRTH_050U_TST';                              -- STORE PROC CHANGE
   ...
   using (
    with 
        ...
        item_list as 
         ...
         curr_tab_item AS
               (SELECT DISTINCT sk1_item_no, sk1_supplier_no
                FROM dwh_datafix.aj_rtl_item_sup_wk_brth_list
                where fin_year_no = g_fin_year_no and fin_week_no = g_fin_week_no),
               
        item_miss as (select sk1_item_no, sk1_supplier_no
                      from item_list il
                      WHERE NOT EXISTS (SELECT 1 FROM curr_tab_item cti
                                      WHERE il.sk1_item_no     = cti.sk1_item_no
                                       and  il.sk1_supplier_no = cti.sk1_supplier_no)),
--                  ),
        calendar_list as
...
--                   item_list di
   ...
   update set                                                                                                      -- COLUNM NAME CHANGE 
          ...
          rtl.last_updated_date          = g_date --g_end_date
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       raise;
END WH_PRF_BRTH_050U_TST                                                                                             -- STORE PROC CHANGE 
;

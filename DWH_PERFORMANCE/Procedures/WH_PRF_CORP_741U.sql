--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_741U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_741U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2013
--  Author:      Q. Smit
...
--**************************************************************************************************
    if l_day_no = 1 then
      MERGE /*+ full(di_mart) PARALLEL(di_mart,4) */ INTO rtl_loc_item_dc_wh_plan  di_mart  USING (   
       select /*+ full(jdaff) parallel(jdaff,4) full(di) full(dl) */
  ...
  and di_mart.calendar_date     = g_start_date)
  ...
  if l_day_no = 2 then
    ...
    ---------------------------------------------------------------------------------------
      MERGE /*+ full(di_mart) PARALLEL(di_mart,4) */ INTO rtl_loc_item_dc_wh_plan  di_mart  USING (   
       select /*+ full(jdaff) parallel(jdaff,4) full(rtl) parallel(rtl,4) full(di) full(dl) */
            dl.sk1_location_no, di.sk1_item_no, jdaff.calendar_date,
            nvl(rtl.week_1_day_1_cases,0)  week_1_day_1_cases,    --Monday's value
            jdaff.week_1_day_1_cases  week_1_day_2_cases,
            jdaff.week_1_day_2_cases  week_1_day_3_cases,
            jdaff.week_1_day_3_cases  week_1_day_4_cases,
            jdaff.week_1_day_4_cases  week_1_day_5_cases,
            jdaff.week_1_day_5_cases  week_1_day_6_cases,
            jdaff.week_1_day_6_cases  week_1_day_7_cases,
            
            jdaff.week_1_day_7_cases  week_2_day_1_cases,
            jdaff.week_2_day_1_cases  week_2_day_2_cases,
            jdaff.week_2_day_2_cases  week_2_day_3_cases,
            jdaff.week_2_day_3_cases  week_2_day_4_cases,
            jdaff.week_2_day_4_cases  week_2_day_5_cases,
            jdaff.week_2_day_5_cases  week_2_day_6_cases,
            jdaff.week_2_day_6_cases  week_2_day_7_cases,
            
            jdaff.week_2_day_7_cases  week_3_day_1_cases,
            jdaff.week_3_day_1_cases  week_3_day_2_cases,
            jdaff.week_3_day_2_cases  week_3_day_3_cases,
            jdaff.week_3_day_3_cases  week_3_day_4_cases,
            jdaff.week_3_day_4_cases  week_3_day_5_cases,
            jdaff.week_3_day_5_cases  week_3_day_6_cases,
            jdaff.week_3_day_6_cases  week_3_day_7_cases

   from     dwh_foundation.fnd_loc_item_jdaff_wh_plan jdaff
   join     dim_location dl               on jdaff.location_no    = dl.location_no
   join     dim_item di                   on jdaff.item_no        = di.item_no
   
   left join rtl_loc_item_dc_wh_plan rtl  on rtl.sk1_location_no  = dl.sk1_location_no
  ...
  if l_day_no = 3 then
    ...
    MERGE /*+ full(di_mart) PARALLEL(di_mart,4) */ INTO rtl_loc_item_dc_wh_plan  di_mart  USING (   
       select /*+ full(jdaff) parallel(jdaff,4) 
                  full(rtl_1) parallel(rtl_1,4) 
                  full(rtl_2) parallel(rtl_2,4) 
                  full(di) full(dl) */
            dl.sk1_location_no, di.sk1_item_no, jdaff.calendar_date,
            
            nvl(rtl_2.week_1_day_1_cases,0) week_1_day_1_cases,     --Tuesday's value
            nvl(rtl_1.week_1_day_2_cases,0) week_1_day_2_cases,     --Monday's value  
   ...
   left join rtl_loc_item_dc_wh_plan rtl_1 on rtl_1.sk1_location_no  = dl.sk1_location_no
                                          and rtl_1.sk1_item_no     = di.sk1_item_no
                                          and rtl_1.calendar_date   = jdaff.calendar_date -1
                                          
   left join rtl_loc_item_dc_wh_plan rtl_2 on rtl_2.sk1_location_no  = dl.sk1_location_no
...
--   and jdaff.item_no        = di.item_no
  ...
  if l_day_no = 4 then
   ...
   left join rtl_loc_item_dc_wh_plan rtl_3 on rtl_3.sk1_location_no  = dl.sk1_location_no
  ...
  if l_day_no = 5 then
    ...
    MERGE /*+ full(di_mart) PARALLEL(di_mart,4) */ INTO rtl_loc_item_dc_wh_plan  di_mart  USING (   
     select   /*+ full(jdaff) parallel(jdaff,4) 
                  ...
                  full(rtl_2) parallel(rtl_2,4) 
                  full(rtl_3) parallel(rtl_3,4)
                  full(rtl_4) parallel(rtl_4,4) 
                  full(di) full(dl) */
            dl.sk1_location_no, di.sk1_item_no, jdaff.calendar_date,
            nvl(rtl_4.week_1_day_1_cases,0) week_1_day_1_cases,
            nvl(rtl_3.week_1_day_2_cases,0) week_1_day_2_cases,
            nvl(rtl_2.week_1_day_3_cases,0) week_1_day_3_cases,
            nvl(rtl_1.week_1_day_4_cases,0) week_1_day_4_cases,
   ...
   left join rtl_loc_item_dc_wh_plan rtl_4 on rtl_4.sk1_location_no  = dl.sk1_location_no
...
--   and jdaff.item_no        = di.item_no
  ...
  if l_day_no = 6 then
    ...
    MERGE /*+ full(di_mart) PARALLEL(di_mart,4) */ INTO rtl_loc_item_dc_wh_plan  di_mart  USING (   
      select /*+ full(jdaff) parallel(jdaff,4) 
                  ...
                  full(rtl_3) parallel(rtl_3,4)
                  full(rtl_4) parallel(rtl_4,4) 
                  full(rtl_5) parallel(rtl_5,4) 
                  full(di) full(dl) */
            dl.sk1_location_no, di.sk1_item_no, jdaff.calendar_date,
            
            nvl(rtl_5.week_1_day_1_cases,0) week_1_day_1_cases,
            nvl(rtl_4.week_1_day_2_cases,0) week_1_day_2_cases,
            nvl(rtl_3.week_1_day_3_cases,0) week_1_day_3_cases,
            nvl(rtl_2.week_1_day_4_cases,0) week_1_day_4_cases,
            nvl(rtl_1.week_1_day_5_cases,0) week_1_day_5_cases,
            ...
            jdaff.week_1_day_7_cases week_2_day_5_cases,
            jdaff.week_2_day_1_cases week_2_day_6_cases,
            jdaff.week_2_day_2_cases week_2_day_7_cases,
   ...
   left join rtl_loc_item_dc_wh_plan rtl_5 on rtl_5.sk1_location_no   = dl.sk1_location_no
  ...
  if l_day_no = 7 then
    ...
    MERGE /*+ full(di_mart) PARALLEL(di_mart,4) */ INTO rtl_loc_item_dc_wh_plan  di_mart  USING (   
       select /*+ full(jdaff) parallel(jdaff,4) 
                  ...
                  full(rtl_4) parallel(rtl_4,4) 
                  full(rtl_5) parallel(rtl_5,4)
                  full(rtl_6) parallel(rtl_6,4)
                  full(di) full(dl) */
            dl.sk1_location_no, di.sk1_item_no, jdaff.calendar_date,
            
            nvl(rtl_6.week_1_day_1_cases,0) week_1_day_1_cases,   --Saturday's value
            nvl(rtl_5.week_1_day_2_cases,0) week_1_day_2_cases,   --Firday's value
            nvl(rtl_4.week_1_day_3_cases,0) week_1_day_3_cases,   --Thursday's value
            nvl(rtl_3.week_1_day_4_cases,0) week_1_day_4_cases,   --Wednesday's value
            nvl(rtl_2.week_1_day_5_cases,0) week_1_day_5_cases,   --Tuesday's value
            nvl(rtl_1.week_1_day_6_cases,0) week_1_day_6_cases,   --Monday's value
   ...
   left join rtl_loc_item_dc_wh_plan rtl_6 on rtl_6.sk1_location_no             = dl.sk1_location_no
  ...
  when matched then 
    ...
    l_text := 'DONE rtl_loc_item_dc_wh_plan UPDATE ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);      

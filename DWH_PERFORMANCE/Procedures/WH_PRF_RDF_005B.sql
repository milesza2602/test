--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_005B
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_005B" 
...
--               Output - temp_loc_item_wk_rdf_appfcst
...
g_rec_out            temp_loc_item_wk_rdf_appfcst%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
g_calendar_date      date          := trunc(sysdate);
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_005B';
...
l_description        sys_dwh_log_summary.log_description%type  := 'TEMP LOAD RDF WEEKLY APP FCST FACTS EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of temp_loc_item_wk_rdf_appfcst%rowtype index by binary_integer;
type tbl_array_u is table of temp_loc_item_wk_rdf_appfcst%rowtype index by binary_integer;
...
cursor c_fnd_rtl_loc_item_wk_dyfcst is
        ...
        select location_no,
               ...
               fin_year_no,
               fin_week_no,
               to_number(substr(appcol,4,2)) fin_week_no_calc,
               appcol,
               appdata
        from   fnd_rtl_loc_item_wk_rdf_wkfcst
               unpivot include nulls (appdata for appcol in (wk_01_app_fcst_qty,
                                                             wk_02_app_fcst_qty,
                                                             wk_03_app_fcst_qty,
                                                             wk_04_app_fcst_qty,
                                                             wk_05_app_fcst_qty,
        ...
        where  last_updated_date = g_date and
               to_number(substr(appcol,4,2)) >= fin_week_no and
               to_number(substr(appcol,4,2)) <= fin_week_no + 12
...
begin
   ...
   g_rec_out.sales_wkly_app_fcst_qty         := g_rec_in.appdata;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into temp_loc_item_wk_rdf_appfcst values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update temp_loc_item_wk_rdf_appfcst
       set    sales_dly_app_fcst_qty        = a_tbl_update(i).sales_dly_app_fcst_qty,
...
-- Check to see if item is present on table and update/insert accordingly
   if g_fnd_count > 0 then  /* check done to determine multiple staging loads into foundation - TD-1469 */

      select count(1)
      into   g_count
      from   temp_loc_item_wk_rdf_appfcst
...
begin
    ...
    l_text := 'LOAD TEMP_LOC_ITEM_WK_RDF_APPFCST EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    l_text := 'CURRENT DATE BEING PROCESSED - '||g_calendar_date||' '||g_fin_week_no;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    execute immediate 'truncate table dwh_performance.temp_loc_item_wk_rdf_appfcst';
    l_text := 'TABLE temp_loc_item_wk_rdf_appfcst TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    prior to data population (temp_loc_item_wk_rdf_appfcst)
*/
    select count(*)
    into g_fnd_count
    from

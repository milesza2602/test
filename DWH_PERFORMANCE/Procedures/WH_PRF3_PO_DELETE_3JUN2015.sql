--------------------------------------------------------
--  DDL for Procedure WH_PRF3_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF3_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF3_PO_DELETE_3JUN2015';
...
FOR V_CUR IN
            ...
            FROM DWH_PERFORMANCE.RTL_PO_CHAIN_SC_WK A,
                  selext B
            WHERE a.SK1_PO_NO = b.SK1_PO_NO
                AND FIN_YEAR_NO <= 2009
)
LOOP


      begin
            DELETE /*+ FULL(A)  */ FROM DWH_PERFORMANCE.RTL_PO_CHAIN_SC_WK a
                where a.SK1_PO_NO = V_CUR.SK1_PO_NO
                ;
                
                g_recs_DELETED_RPCSW := g_recs_DELETED_RPCSW + SQL%ROWCOUNT;
                commit;
                
                                                  if g_recs_DELETED_RPCSW >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_RPCSW;
        ...
        when no_data_found then
          g_recs_DELETED_RPCSW := g_recs_DELETED_RPCSW ;
      end;
...
l_text := 'RTL_PO_CHAIN_SC_WK recs deleted = '||g_recS_deleted_RPCSW;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
END WH_PRF3_PO_DELETE_3JUN2015;

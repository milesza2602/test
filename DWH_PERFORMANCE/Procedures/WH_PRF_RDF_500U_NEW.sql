--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_500U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_500U_NEW" 
...
--  23 Jan 2019  - B Kirschner - adjust stats collection on very large partitioned tables for 12c upgrade (old way is very slow)   Ref:23Jan2019
--
--------------------------------NEW VERSION--------------------------------------------------------------------
--  Maintenance:
...
--
--  March 2018 - Q. Smit  Catered for VAT increase from 14% to 15%.  This is done by doing an outer join to 
--                        the item vat rate table to get the vat rate for the post date.  This is needed for
--                        the potential for data being processed before and after 1 April where the vat rate changed.
--
--  Naming conventions
--  g_  -  Global variable
...
--**************************************************************************************************
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_RDF_500U_NEW';
...
--  g_min_date := '27/JAN/18' ;
--  g_max_date := '28/JAN/18' ;
  ...
  --MERGE /*+ parallel(rdf_mart,8) full(rdf_mart) */ into DWH_PERFORMANCE.RTL_LOC_ITEM_RDF_DYFCST_L1 rdf_mart USING      --rtl_loc_item_dy_rdf_fcst_L1
  MERGE /*+ parallel(rdf_mart,6) */ into DWH_PERFORMANCE.RTL_LOC_ITEM_RDF_DYFCST_L1 rdf_mart USING
 ( 
  WITH SELRDF
                  AS(
                --  SELECT /*+ materialize PARALLEL(rtl,8) FULL(rtl) PARALLEL(sys_ilv,8) FULL(sys_ilv) FULL(DI) full(zi) full(dl) */
                select /*+ full(rtl) parallel(rtl,6) PARALLEL(sys_ilv,6) FULL(sys_ilv)) full(fi_vr) parallel(fi_vr,6)  */
                    sys_ilv.location_no,
                    sys_ilv.item_no,
                    sys_ilv.post_date,
                  ...
                  FROM dwh_performance.TEMP_LOC_ITM_DY_RDF_SYSFCST_L1  sys_ilv
                     ...
                     LEFT OUTER JOIN FND_ITEM_VAT_RATE  fi_vr  on (sys_ilv.item_no   = fi_vr.item_no                                            -- VAT rate change
                                                               and  dl.vat_region_no  = fi_vr.vat_region_no                                     -- VAT rate change
                                                               and  sys_ilv.post_date between fi_vr.active_from_date and fi_vr.active_to_date)  -- VAT rate change

      
          where sys_ilv.post_date between g_min_date and g_max_date   
            and rtl.post_date between g_min_date and g_max_date   
-- for testing ...              
--where sys_ilv.post_date = '28 jaN 2018'
--and dl.LOCATION_NO =  269

          --        WHERE sys_ilv.location_no BETWEEN P_FROM_LOC_NO AND P_TO_LOC_NO
          ----        order by sys_ilv.post_date
                  )
SELECT /*+ parallel(SR,8) parallel(dih,8) parallel(dlh,8) FULL(dih) FULL(dil) */
SR.SK1_LOCATION_NO
,SR.SK1_ITEM_NO
,SR.POST_DATE
,DLH.SK2_LOCATION_NO
...
--        DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','RTL_LOC_ITEM_RDF_DYFCST_L1', DEGREE => 32);
    ...
    filter_lst(1).objname := 'RTL_LOC_ITEM_RDF_DYFCST_L1';

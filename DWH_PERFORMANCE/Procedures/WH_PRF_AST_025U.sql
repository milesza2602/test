--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_025U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_025U" 
                                                                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        May 2012
--  Author:      A Joshua
--  Purpose:     Load Pre-Actual Plan to Actual Plan from performance level.
--  Tables:      Input  - rtl_chn_geo_grd_sc_wk_ast_pact
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_025U';
...
--**************************************************************************************************
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      ...
      select /*+ full (apln) parallel (apln,2) */ apln.*
      from   rtl_chn_geo_grd_sc_wk_ast_pact apln,
             dim_calendar dim
      where  apln.fin_week_no  = dim.fin_week_no
       and   apln.fin_year_no  = dim.fin_year_no

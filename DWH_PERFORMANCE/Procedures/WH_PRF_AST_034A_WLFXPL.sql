--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_034A_WLFXPL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_034A_WLFXPL" (
...
G_RUN_DATE DATE;
      ...
      l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_AST_034A_WLFXPL';
...
begin
    ...
    g_recs_inserted := 0;
      ...
      insert 
      into dwh_datafix.trtl_ast_sccontwk_034a_wlFIX2 a
   with selext as
      (select 
               sk1_style_colour_no,
              continuity_ind,
              ...
              min(case when c1.calendar_date < season_first_trade_date
                       ...
                       over (partition by T.sk1_style_colour_no)                                                                fash_start_week_date,
      ...
      from rtl_sc_trading t
            ...
            on few.calendar_date = (case when c1.calendar_date <= season_first_trade_date then season_first_trade_date
                 ...
                 ON WS.SK1_STYLE_COLOUR_NO = T.SK1_STYLE_COLOUR_NO
                 --- TESTING
...
--  FOR RERUN NOV 2016
 --     AND   sk1_style_colour_no in (5502589,16420002)
    ...
    l_text := 'Recs Inserted Into trtl_ast_sccontwk_034a_wlFIX2 = '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   exception
...
insert /*+ append */ into  dwh_datafix.trtl_ast_sc_dates_wlFIX2
                   select
                            sk1_style_colour_no 
                            ,continuity_ind 
                            ,season_first_trade_date 
                          ...
                          from dwh_datafix.trtl_ast_sccontwk_034a_wlFIX2 a,
                                dim_calendar dc1
...
--**************************************************************************************************
    ...
    insert /*+ append */ into  dwh_datafix.trtl_ast_sc_6wk_034b_wlFIX2
              with selcal as
              ...
              from dwh_performance.rtl_chn_geo_grd_sc_wk_ast_act a
                                 ...
                                 join dwh_datafix.trtl_ast_sc_dates_wlFIX2 ic
...
--and a.sk1_grade_no = 6
    ...
    L_TEXT := 'Recs inserted into trtl_ast_sc_6wk_034b_wlFIX2 = '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    ...
    insert /*+append */ into  dwh_datafix.mart_ch_ast_chngrdscwk_wlFIX2 m
      with
    --------------------------------------------------------------------------------
    -- last completed week
    --------------------------------------------------------------------------------
   ...
   --                and sk1_chain_no = 243
    ...
    --------------------------------------------------------------------------------
     sixwk as (
              ...
              from dwh_datafix.trtl_ast_sc_6wk_034b_wlFIX2  w
...
--
    --------------------------------------------------------------------------------
     ...
     selsdt as
             ...
             where dc3.this_week_start_date >=     g_curr_season_start_date
         --   '24/DEC/12'
      ...
      seldays as
              ...
              WKCAT.SK1_STYLE_COLOUR_NO  ,      --     sk1_item_no,
                           wkcat.fin_year_no,
                           wkcat.fin_week_no,
                           SK1_AVAIL_UDA_VALUE_NO,
                           dcwkcat.this_week_start_date,
                           dcwkcat.fin_half_end_date,
                                   case when dcwkcat.this_week_start_date < '17 oct 2016'   then ch_num_avail_days  
                                   else avail_ch_num_avail_days end  
                           NUM_AVAIL_DAYS_STD_ACT   ,
                                   case when dcwkcat.this_week_start_date < '17 oct 2016'  then ch_num_catlg_days  
                                   else avail_ch_num_catlg_days end  
                           NUM_CATLG_DAYS_STD_ACT , 
                           ch_num_avail_days    PRE_PL_NUM_AVAIL_DAYS_STD_ACT   ,
                           ch_num_catlg_days   PRE_PL_NUM_CATLG_DAYS_STD_ACT  
...
--                and wkcat.fin_week_no = dcwkcat.fin_week_no
   ...
   --         and sk1_chain_no = 243 and   sk1_style_colour_no in (5502589,16420002)
      ...
      ssna as
                (select /*+ full (di) */ sk1_chain_no,
                        ...
                        sum(nvl(PRE_PL_NUM_AVAIL_DAYS_STD_ACT,0))          PRE_PL_NUM_AVAIL_DAYS_STD_ACT,
                        sum(nvl(PRE_PL_NUM_CATLG_DAYS_STD_ACT ,0))         PRE_PL_NUM_CATLG_DAYS_STD_ACT
                from selsdt dt ,
                     seldays wkc
                     --,
                    -- dim_item si
                ...
                and dt.this_week_start_date between  g_curr_season_start_date and g_curr_season_end_date
             --   and si.sk1_item_no = wkc.sk1_item_no
                group by  sk1_chain_no,
                          sk1_style_colour_no,
                          dt.fin_year_no ,
                          dt.fin_week_no, dt.this_week_start_date
       ...
       ssnaval as
                (select sa.sk1_chain_no                                   sk1_chain_no,
                        ...
                        sa.PRE_PL_NUM_AVAIL_DAYS_STD_ACT                  PRE_PL_NUM_AVAIL_DAYS_STD_ACT,
                        sa.PRE_PL_NUM_CATLG_DAYS_STD_ACT                  PRE_PL_NUM_CATLG_DAYS_STD_ACT
                from ssna sa
    ...
    ---         MAIN JOIN OF ALL DATA EXTRACTS ABOVE
     ...
     select
               ...
               round(sum(PRE_PL_NUM_AVAIL_DAYS_STD_ACT),2)                                        PRE_PL_NUM_AVAIL_DAYS_STD_ACT,
               round(sum(PRE_PL_NUM_CATLG_DAYS_STD_ACT),2)                                        PRE_PL_NUM_CATLG_DAYS_STD_ACT
     from act ac
    ...
    L_TEXT := 'Recs inserted into dwh_datafix.MART_CH_AST_CHNGRDSCWK_WLFIX2 = '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   exception
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     l_text := '************************************* A STARTED****************************************************';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'Truncate Table dwh_datafix.trtl_ast_sccontwk_034a_wlFIX2';
     l_text := 'Truncate Table dwh_datafix.trtl_ast_sccontwk_034a_wlFIX2';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_date := G_RUN_DATE;
    g_date := g_date + 1;
    ...
    execute immediate 'truncate table dwh_datafix.trtl_ast_sc_dates_wlFIX2';
    L_TEXT := 'truncate table dwh_datafix.trtl_ast_sc_dates_wlFIX2';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate 'truncate table dwh_datafix.trtl_ast_sc_6wk_034b_wlFIX2';
    L_TEXT := 'truncate table dwh_datafix.trtl_ast_sc_6wk_034b_wlFIX2';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_date := G_RUN_DATE;
    g_date := g_date + 1;
    l_text := 'Test BATCH DATE BEING PROCESSED IS:- '||g_date;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     l_text := '************************************* C STARTED****************************************************';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate 'truncate table DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX2';
    L_TEXT := 'truncate table MART_CH_AST_CHNGRDSCWK_WLFIX2';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_date := G_RUN_DATE;
    g_date := g_date + 1;
    l_text := 'Test BATCH DATE BEING PROCESSED IS:- '||g_date;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     DBMS_STATS.gather_table_stats ('DWH_DATAFIX','MART_CH_AST_CHNGRDSCWK_WLFIX2', DEGREE => 8);
   l_text := '**********Completed GATHER_TABLE_STATS ON MART_CH_AST_CHNGRDSCWK_WLFIX2********************************';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    G_RUN_DATE := '3 JUL 2016';
    l_text := 'Run date = '||g_run_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
 ...
 --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   l_text := '**********Completed GATHER_TABLE_STATS ON MART_CH_AST_CHNGRDSCWK_WLFIX2********************************';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
------- SQL TO DO FINAL UPDATE TO TARGET LIVE TABLE

    execute immediate 'truncate table DWH_PERFORMANCE.MART_CHASTCHNGRDSCWK_SEASON';
    L_TEXT := 'truncate table MART_CHASTCHNGRDSCWK_SEASON';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  COMMIT;
  
   l_text := 'Starting insert...';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
INSERT /*+ APPEND */ 
INTO DWH_PERFORMANCE.MART_CHASTCHNGRDSCWK_SEASON

with SELCAL AS  (SELECT DISTINCT FIN_YEAR_NO, FIN_WEEK_NO, THIS_WEEK_START_DATE + 42 LAST_UPDATED_DATE 
            FROM DIM_CALENDAR WHERE FIN_YEAR_NO = 2017 AND FIN_WEEK_NO BETWEEN 1 AND 13)
SELECT /*+ FULL(mart) PARALLEL(mart,4) */ 
            DISTINCT MART.* 
            FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX2 MART, SELCAL SC
            WHERE MART.FIN_YEAR_NO = SC.FIN_YEAR_NO
            AND MART.FIN_WEEK_NO = SC.FIN_WEEK_NO
            AND MART.LAST_UPDATED_DATE  = SC.LAST_UPDATED_DATE ;
            
   g_recs_inserted := sql%rowcount;
   
    commit;
    
    L_TEXT := 'Recs inserted form DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX2 into MART_CHASTCHNGRDSCWK_SEASON= '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);


INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.MART_CHASTCHNGRDSCWK_SEASON
SELECT /*+ FULL(mart) PARALLEL(mart,4) */ 
            DISTINCT MART.* 
            FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX2 MART
            WHERE MART.FIN_YEAR_NO = 2017
            AND MART.FIN_WEEK_NO = 14
            AND MART.LAST_UPDATED_DATE  = '31 oct 2016';
            
   g_recs_inserted := sql%rowcount;
   
    commit;
    
    L_TEXT := 'Recs inserted into DWH_PERFORMANCE.MART_CHASTCHNGRDSCWK_SEASON= '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--
--- THIS IS TO LOAD LAST 5 WEEKS WHERE LAST_UPDATED_DATE FOR THESE WEEKS = LATEST_WEEK RUN
--
--INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.MART_CHASTCHNGRDSCWK_SEASON
--with   SELCALMAX AS  (SELECT DISTINCT  THIS_WEEK_end_DATE+1 MATCH_LUD
--                      FROM DIM_CALENDAR WHERE FIN_YEAR_NO = 2017 AND FIN_WEEK_NO = 18),
--      SELCAL AS  (SELECT DISTINCT FIN_YEAR_NO, FIN_WEEK_NO
--                  FROM DIM_CALENDAR WHERE FIN_YEAR_NO = 2017 AND FIN_WEEK_NO BETWEEN 14 AND 18)
--SELECT /*+ FULL(mart) PARALLEL(mart,4) */ 
--            DISTINCT MART.* 
--            FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX2 MART, SELCAL SC, SELCALMAX SCM
--            WHERE MART.FIN_YEAR_NO = SC.FIN_YEAR_NO
--            AND MART.FIN_WEEK_NO = SC.FIN_WEEK_NO
--            AND MART.LAST_UPDATED_DATE  = SCM.MATCH_LUD ;
--            
--   g_recs_inserted := sql%rowcount;
--   
--    commit;

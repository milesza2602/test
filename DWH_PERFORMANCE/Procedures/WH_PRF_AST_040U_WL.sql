--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_040U_WL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_040U_WL" 
...
l_module_name            sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_040U_WL';
...
begin
   ...
   using (
        WITH 
           SELEXT AS 
                           ( 
                            select /*+ materialize parallel (FND_AST_LOC_ITEM_DY_CATLG,8)  full (FND_AST_LOC_ITEM_DY_CATLG)*/
                                      FND.*
          ...
          SELSTK AS (SELECT /*+ materialize parallel (RTL_LOC_ITEM_DY_RMS_STOCK,8)  full (RTL_LOC_ITEM_DY_RMS_STOCK)*/
                       *
                      FROM RTL_LOC_ITEM_DY_RMS_STOCK 
                      WHERE POST_DATE = G_DATE)
          select /*+  parallel (DIU,8)  full (DIU) parallel (DCAP,8)  full (DCAP) */ 
                             sf.sk1_location_no, 
                             sf.sk1_item_no, 
                             sf.post_date, 

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_662U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_662U" 
...
--                           The assumption was made that the ship date on the shipments table is to be used to determine the vat rate from the vat rate table
--                           to cater for possible senario of records being process after 1 April that must use the old vat rate.
--                           This meant that the ship_date had to be included in the shipment data.
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_662U';
...
(
   ...
   from      fnd_rtl_purchase_order p
    join      dwh_performance.temp_po_list tp           on  p.po_no              = tp.po_no
...
select         /*+ materialize parallel(li,4) parallel(ldd,4) full(di) full(dih)  full(dl) full(dlh) */
                ...
                NVL(fi_vr.VAT_RATE_PERC, (case when ps.vat_region_no = 1000 then di.vat_rate_perc else ps.DEFAULT_TAX_REGION_NO_PERC end ) )  TAX_PERC    -- VAT rate change
    from selpoship ps 
    ...
    LEFT OUTER JOIN FND_ITEM_VAT_RATE  fi_vr  on (ps.item_no    = fi_vr.item_no                                      -- VAT rate change
                                         and  ps.vat_region_no  = fi_vr.vat_region_no                                -- VAT rate change
                                         and  ps.ship_date between fi_vr.active_from_date and fi_vr.active_to_date)  -- VAT rate change                                        
                                        )
...
SELECT /*+ materialize */ DISTINCT 
    ...
    from     selcalc1 sc1 where sc1.sk1_location_no is not null -- AJ
     ),
selcalc2 as ( 
SELECT /*+ materialize */ DISTINCT 
                sc1.actl_rcpt_date,  sc1.reg_rsp, sc1.cost_price,
...
MERGE INTO  DWH_PERFORMANCE.RTL_PO_SUPCHAIN_LOC_ITEM_DY RTL
 using ( SELECT A.* FROM     DWH_PERFORMANCE.TEMP_POSHIP_CALC A where a.sk1_location_no is not null ) MER_MART

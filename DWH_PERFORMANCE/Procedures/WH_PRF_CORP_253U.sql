--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_253U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_253U" (p_forall_limit in integer, p_success out boolean) as
...
g_rec_out            DWH_PERFORMANCE.FOODS_RENEWAL_EXTRACT1_3%rowtype;
g_count              number        :=  0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_253U';
...
type tbl_array_i is table of DWH_PERFORMANCE.FOODS_RENEWAL_EXTRACT1_3%rowtype index by binary_integer;
type tbl_array_u is table of DWH_PERFORMANCE.FOODS_RENEWAL_EXTRACT1_3%rowtype index by binary_integer;
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table DWH_PERFORMANCE.FOODS_RENEWAL_EXTRACT1_3');
...
INSERT /*+ APPEND PARALLEL (mart,4) */ INTO DWH_PERFORMANCE.FOODS_RENEWAL_EXTRACT1_3 mart
WITH item_list AS
  (  select di.item_no, di.sk1_item_no, di.item_desc, di.sk1_supplier_no, dd.department_no, dd.department_name, di.subclass_no, di.subclass_name
            from dim_item di, dim_department dd
              -- where dd.department_no in (44, 88, 41, 47, 68, 75, 85, 46, 51)  --(41,58,73,81,88,96)   --,44,45,47,50,55,58,72,73,75,76,77,81,87,88,90,93,95,96,97,98,99)

--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_034A_WLFX9
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_034A_WLFX9" (
...
G_RUN_DATE DATE;
      ...
      l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_AST_034A_WLFX9';
...
------- SQL TO DO FINAL UPDATE TO TARGET LIVE TABLE
   l_text := 'Truncating suBpartitions...';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20171_1');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20171_2');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20171_3');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20171_4');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20172_5');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20172_6');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20172_7');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20172_8');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20172_9');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20173_10');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20173_11');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20173_12');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20173_13');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20174_14');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20174_15');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20174_16');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20174_17');
 execute immediate ('alter table dwh_performance.mart_ch_ast_chn_grd_sc_wk truncate SUBPARTITION MRT_CACGSW_M20175_18');
  COMMIT;
   l_text := 'Starting insert...';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
INSERT /*+ APPEND */ INTO dwh_performance.mart_ch_ast_chn_grd_sc_wk
with SELCAL AS  (SELECT DISTINCT FIN_YEAR_NO, FIN_WEEK_NO, THIS_WEEK_START_DATE + 42 LAST_UPDATED_DATE 
                  FROM DIM_CALENDAR WHERE FIN_YEAR_NO = 2017 AND FIN_WEEK_NO between 1 and 18),
selall as (SELECT /*+ FULL(mart) PARALLEL(mart,4) */ 
                      MART.* 
                      FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX1 MART, SELCAL SC
...
union
            SELECT /*+ FULL(mart) PARALLEL(mart,4) */
                        MART.* 
                        FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX3 MART, SELCAL SC
                        WHERE MART.FIN_YEAR_NO = SC.FIN_YEAR_NO
                        AND MART.FIN_WEEK_NO = SC.FIN_WEEK_NO
                        AND MART.LAST_UPDATED_DATE  = SC.LAST_UPDATED_DATE 
union
            SELECT /*+ FULL(mart) PARALLEL(mart,4) */
                        MART.* 
                        FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX4 MART, SELCAL SC
                        WHERE MART.FIN_YEAR_NO = SC.FIN_YEAR_NO
                        AND MART.FIN_WEEK_NO = SC.FIN_WEEK_NO
                        AND MART.LAST_UPDATED_DATE  = SC.LAST_UPDATED_DATE )
select distinct * from selall;
    g_recs_inserted := sql%rowcount;
    commit;
    L_TEXT := 'Recs inserted FROM MART_CH_AST_CHNGRDSCWK_WLFIX1,3,4 into mart_ch_ast_chn_grd_sc_wk = '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
SELECT /*+ FULL(mart) PARALLEL(mart,4) */ 
            MART.* 
            FROM DWH_DATAFIX.MART_CH_AST_CHNGRDSCWK_WLFIX3  MART, SELCAL SC, SELCALMAX SCM
            WHERE MART.FIN_YEAR_NO = SC.FIN_YEAR_NO
            AND MART.FIN_WEEK_NO = SC.FIN_WEEK_NO
   ...
   g_recs_inserted := sql%rowcount;
    ...
    L_TEXT := 'Recs inserted FROM MART_CH_AST_CHNGRDSCWK_WLFIX3 into MART_CHASTCHNGRDSCWK_SEASON= '||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

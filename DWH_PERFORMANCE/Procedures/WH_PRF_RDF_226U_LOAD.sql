--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_226U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_226U_LOAD" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2015
--  Author:      Quentin Smit
...
l_module_name           sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_226U_LOAD';
...
cursor c_rtl_loc_item_dy_rdf_fcst is
  ...
  select    /*+ parallel(fcst,4) */ fcst.sk1_item_no,
...
begin
    ...
    l_text := 'LOAD RTL_LOC_ITEM_RDF_WKFCST_L2 EX RTL_LOC_ITEM_RDF_DYFCST_L2 TABLES STARTED '||
...
--**************************************************************************************************
    ...
    select unique fin_year_no, fin_week_no, this_week_start_date, this_week_end_date
    into   g_fin_year_no, g_fin_week_no, g_this_week_start_date, g_this_week_end_date
    from   dim_calendar
    where  fin_year_no = 2015 and fin_week_no = 42;

    l_text := 'START WEEK/YEAR OF ROLLUP IS:- '||g_fin_year_no||' '||g_fin_week_no;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    l_text := 'START / END DATE OF ROLLUP IS:- '||g_this_week_start_date||' '||g_this_week_end_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

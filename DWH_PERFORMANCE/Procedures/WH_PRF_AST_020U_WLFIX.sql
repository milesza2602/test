--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_020U_WLFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_020U_WLFIX" 
                                                                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        May 2012
--  Author:      A Joshua
--  Purpose:     Load Assort Approved and Original Plans from foundation level.
--  Tables:      Input  - fnd_ast_geo_grd_sc_wk_apln, fnd_ast_geo_grd_sc_wk_opln and fnd_ast_geo_grd_sc_wk_fpln
--               Output - rtl_chn_geo_grd_sc_wk_ast_pln
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_020U_WLFIX';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ASSORT FACT PLANS';
...
begin
   ...
   l_text := 'LOAD OF RTL_CHN_GEO_GRD_SC_WK_AST_PLN STARTED AT '||
...
--RTL_CGGSWAA_M20133_10
          g_partition_name :=   'RTL_CGGSWAP_M'||g_START_YEAR||g_START_month||'_'||g_START_week;    
          g_sql_trunc_partition := 'alter table dwh_performance.RTL_CHN_GEO_GRD_SC_WK_AST_PLN truncate SUBPARTITION '||g_partition_name;
          ...
          commit; 
--RTL_CGGSWAP_M201312_49

      insert /*+ APPEND parallel (ast,2) */ into RTL_CHN_GEO_GRD_SC_WK_AST_PLN ast
...
--             cal.this_week_start_date,
      ...
      from   fnd_ast_geo_grd_sc_wk_apln apln,
   ...
   --    and  apln.last_updated_date = g_date
      ...
      UNION
      select /*+ full (opln) parallel (opln,2) */
--             (CASE WHEN opln.grade_no in (14)  THEN g_chain_franchise
--                 ELSE g_chain_corporate
--              END) sk1_chain_no,
             g_chain_corporate as sk1_chain_no,
             geo.sk1_geography_no,
             ...
             opln.fin_week_no,
             pln.sk1_plan_type_no,
             'W'||opln.fin_year_no||opln.fin_week_no as fin_week_code,
             opln.sales_qty,
             opln.sales_cost,
             opln.sales,
             0, -- as opln.clearance_cost,
             0, -- as opln.prom_cost,
             0, -- as opln.store_intake_qty,
             0, -- as opln.store_intake_cost,
             0, -- as opln.store_intake_selling,
             0, -- as opln.target_stock_qty,
             0, -- as opln.target_stock_cost,
             0, -- as opln.target_stock_selling,
             0, -- as opln.pres_min,
             0, -- as opln.store_count,
             0, -- as opln.time_on_offer_ind,
             0, -- as opln.facings,
             0, -- as opln.sit_qty,
             opln.sales_qty as aps_sales_qty,
             0, -- as sales_margin,
             0, -- as store_intake_margin,
             0, -- as target_stock_margin,
--             cal.this_week_start_date,
             g_date as last_updated_date
      from   fnd_ast_geo_grd_sc_wk_opln opln,
             dim_ast_lev1_diff1 sc,
             dim_ast_geography geo,
             dim_ast_grade grd,
             dim_plan_type pln
      ...
      where opln.style_colour_no   = sc.style_colour_no
       ...
       and  opln.fin_year_no       = g_start_year
       and  opln.fin_week_no       = g_start_WEEK
   --    and  cal.fin_day_no         = 3
    --   and  opln.last_updated_date = g_date

      UNION
      select /*+ full (fpln) parallel (fpln,2) */
--             (CASE WHEN fpln.grade_no in (14)  THEN g_chain_franchise
--                 ELSE g_chain_corporate
--              END) sk1_chain_no,
             g_chain_corporate as sk1_chain_no,
             geo.sk1_geography_no,
             ...
             fpln.fin_week_no,
             pln.sk1_plan_type_no,
             'W'||fpln.fin_year_no||fpln.fin_week_no as fin_week_code,
             fpln.sales_qty,
             fpln.sales_cost,
             fpln.sales,
             0, -- as fpln.clearance_cost,
             0, -- as fpln.prom_cost,
             ...
             fpln.pres_min,
             0, -- as fpln.store_count,
             0, -- as fpln.time_on_offer_ind,
             0, -- as fpln.facings,
             0, -- as fpln.sit_qty,
             0, -- as fpln.sales_qty,
             0, -- as sales_margin,
...
--             cal.this_week_start_date,
             g_date as last_updated_date
      from   fnd_ast_geo_grd_sc_wk_fpln fpln,
             dim_ast_lev1_diff1 sc,
             dim_ast_geography geo,
             dim_ast_grade grd,
             dim_plan_type pln
      ...
      where fpln.style_colour_no   = sc.style_colour_no
       ...
       and  fpln.fin_year_no       = g_start_year
       and  fpln.fin_week_no       = g_start_WEEK
 --      and  cal.fin_day_no         = 3
   --    and  fpln.last_updated_date = g_date ;

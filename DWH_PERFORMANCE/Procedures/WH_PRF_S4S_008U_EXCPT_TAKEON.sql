--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_008U_EXCPT_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_008U_EXCPT_TAKEON" (
...
AS
  ...
  --               Output   - DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_X
  ...
  g_count         NUMBER  := 0;
  g_rec_out RTL_EMP_AVAIL_LOC_JOB_WK_X%rowtype;
  g_found BOOLEAN;
  g_date  DATE;
...
g_recs_deleted      integer       :=  0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_S4S_008U_EXCPT_TAKEON';
  l_name sys_dwh_log.log_name%type                     := dwh_constants.vc_log_name_rtl_md;
  l_system_name sys_dwh_log.log_system_name%type       := dwh_constants.vc_log_system_name_rtl_prf;
  l_script_name sys_dwh_log.log_script_name%type       := dwh_constants.vc_log_script_rtl_prf_md;
  l_procedure_name sys_dwh_log.log_procedure_name%type := l_module_name;
  l_text sys_dwh_log.log_text%type ;
  l_description sys_dwh_log_summary.log_description%type   := 'LOAD THE RTL_EMP_AVAIL_LOC_JOB_WK_X data  EX FOUNDATION';
...
BEGIN
  ...
  l_text := 'LOAD OF RTL_EMP_AVAIL_LOC_JOB_WK_X  EX FOUNDATION STARTED '|| TO_CHAR(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--g_date := '7 dec 2014';
    ...
    l_text := 'TRUNCATE TABLE DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_X';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
EXECUTE IMMEDIATE('TRUNCATE TABLE DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_X');
  ...
  INSERT
    /*+ APPEND */
  INTO DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_X
        WITH seldat AS
                      (SELECT 
...
COMMIT;
    ...
    l_text := 'Running GATHER_TABLE_STATS ON RTL_EMP_AVAIL_LOC_JOB_WK_X';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_EMP_AVAIL_LOC_JOB_WK_X', DEGREE => 8);
...
WHEN OTHERS THEN
  ...
  p_success := false;
  raise;
END WH_PRF_S4S_008U_EXCPT_TAKEON;

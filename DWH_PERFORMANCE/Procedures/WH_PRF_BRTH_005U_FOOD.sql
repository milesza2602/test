--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_005U_FOOD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_005U_FOOD" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_BRTH_005U_FOOD';
...
--**************************************************************************************************
    ...
    execute immediate ('truncate table dwh_DATAFIX.T14_rtl_area_item_wk_stock');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.T14_rtl_area_item_wk_stock';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   'T14_rtl_area_item_wk_stock', DEGREE => 8);
--    commit;
--    l_text := 'GATHER STATS on dwh_DATAFIX.T14_rtl_area_item_wk_stock';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';

    INSERT /*+ APPEND */ INTO dwh_DATAFIX.T14_rtl_area_item_wk_stock
...
commit;
    ...
    execute immediate ('truncate table dwh_DATAFIX.T13_rtl_area_item_wk_stock');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.T13_rtl_area_item_wk_stock';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   'T13_rtl_area_item_wk_stock', DEGREE => 8);
--    commit;
--    l_text := 'GATHER STATS on dwh_DATAFIX.T13_rtl_area_item_wk_stock';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';

    INSERT /*+ APPEND */ INTO dwh_DATAFIX.T13_rtl_area_item_wk_stock
       select   /*+ full(rai) parallel (rai	4) */
       dl.area_no
                ,di.item_no
                ,rai.fin_year_no
...
commit;
    ...
    execute immediate ('truncate table dwh_DATAFIX.t18_rtl_area_item_wk_stock');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.t18_rtl_area_item_wk_stock';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--
--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   't18_rtl_area_item_wk_stock', DEGREE => 8);
--    commit;
--    l_text := 'GATHER STATS on dwh_DATAFIX.t18_rtl_area_item_wk_stock';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';

    INSERT /*+ APPEND */ INTO dwh_DATAFIX.t18_rtl_area_item_wk_stock
       select   /*+ full(rai) parallel (rai	4) */
       dl.area_no
                ,di.item_no
                ,rai.fin_year_no

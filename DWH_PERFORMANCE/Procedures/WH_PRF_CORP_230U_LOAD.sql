--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_230U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_230U_LOAD" (p_forall_limit in integer, p_success out boolean) as
--**************************************************************************************************
--  Date:        June 2012
--  Author:      Quentin Smit
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_230U_LOAD';
...
l_to_date           date;   -- := g_date;    --'11/SEP/14' +13;    --13
--in_batch            integer  :=1;
...
--**************************************************************************************************
    ...
    l_text := 'Date range being processed : ' || '11/SEP/14' || ' - ' || '27/OCT/14';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
uda_3502_item_list as (
  ...
  from rtl_item_uda_dy uda_3502, item_list il
   where uda_3502.calendar_date between '11/SEP/14' and '26/OCT/14'
...
from rtl_loc_item_dy_catalog cat,
     ...
     uda_item_list uda
where cat.calendar_date between '11/SEP/14' and '26/OCT/14'
...
from rtl_loc_item_dy_rms_dense dense,
     ...
     uda_3502_item_list uda_3502,
     uda_item_list uda
where dense.post_date between '11/SEP/14' and '26/OCT/14'

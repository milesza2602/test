--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_046U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_046U" (p_forall_limit in integer,p_success out boolean, p_from_loc_no in integer,p_to_loc_no in integer) as
-- *************************************************************************************************
-- * Notes from 12.2 upgrade performance tuning
-- *************************************************************************************************
-- Date:   2019-01-23 
-- Author: Paul Wakefield
-- Removed FTS hints from MERGE
-- **************************************************************************************************
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_046U_'|| p_from_loc_no;
...
begin
   ...
   merge /*+ parallel(6) */ into rtl_location_item   mer_lid
    using  (
      select /*+ parallel(6) */
          fli.*,
   ...
   where  fli.item_no                = di.item_no  and
       ...
       update
          set  supply_chain_type               = mer_mart.supply_chain_type,
            ...
            --   primary_country_code            := mer_mart.primary_country_code;    -- TD-491
               ...
               num_facings                     = mer_mart.num_facings,       --  Chg4256
               
               --MULTI CURRENCY FIELDS
               REG_RSP_LOCAL                   = mer_mart.REG_RSP_LOCAL, 
               REG_RSP_OPR                     = mer_mart.REG_RSP_OPR,
               SELLING_RSP_LOCAL               = mer_mart.SELLING_RSP_LOCAL, 
               SELLING_RSP_OPR                 = mer_mart.SELLING_RSP_OPR,
               PROM_RSP_LOCAL                  = mer_mart.PROM_RSP_LOCAL,
               PROM_RSP_OPR                    = mer_mart.PROM_RSP_OPR,
               PROM_SELLING_RSP_LOCAL          = mer_mart.PROM_SELLING_RSP_LOCAL,
               PROM_SELLING_RSP_OPR            = mer_mart.PROM_SELLING_RSP_OPR,
    ...
    when not matched then
       insert (
                ...
                PROM_RSP_LOCAL,
                PROM_RSP_OPR,
                PROM_SELLING_RSP_LOCAL,
                PROM_SELLING_RSP_OPR,
                CASE_MASS,
         ...
         values (
                ...
                0,
                0,
                mer_mart.num_facings, -- Chg4256
                
                --MULTI CURRENCY FIELDS
                mer_mart.REG_RSP_LOCAL,
                mer_mart.REG_RSP_OPR,
                mer_mart.SELLING_RSP_LOCAL,
                mer_mart.SELLING_RSP_OPR,
                mer_mart.PROM_RSP_LOCAL,
                mer_mart.PROM_RSP_OPR,
                mer_mart.PROM_SELLING_RSP_LOCAL,
                mer_mart.PROM_SELLING_RSP_OPR,
                mer_mart.CASE_MASS,
                mer_mart.CASE_SELLING_INCL_VAT,
                mer_mart.REG_RSP_INCL_VAT,
                mer_mart.CASE_COST_PRICE,
                mer_mart.REG_RSP_EXCL_VAT,
                mer_mart.CASE_SELLING_EXCL_VAT,
                mer_mart.COST_PRICE,
                mer_mart.CASE_COST_PRICE_LOCAL,
                mer_mart.CASE_COST_PRICE_OPR,
                mer_mart.CASE_SELLING_EXCL_VAT_LOCAL,
                mer_mart.CASE_SELLING_EXCL_VAT_OPR,
                mer_mart.CASE_SELLING_INCL_VAT_LOCAL,
                mer_mart.CASE_SELLING_INCL_VAT_OPR,
                mer_mart.COST_PRICE_LOCAL,
                mer_mart.COST_PRICE_OPR,
                mer_mart.REG_RSP_EXCL_VAT_LOCAL,
                mer_mart.REG_RSP_EXCL_VAT_OPR,
                mer_mart.REG_RSP_INCL_VAT_LOCAL,
                mer_mart.REG_RSP_INCL_VAT_OPR
...
merge /*+ parallel(6) */ into rtl_location_item rlid
  using (
     select /*+ parallel(6) */
              li.sk1_item_no,
              li.sk1_location_no,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_715EQ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_715EQ" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_715EQ';
...
--**************************************************************************************************
 ...
 --   while g_last_week_no < 17 loop
    ...
    SELECT
      ...
      prom_sales_qty
    FROM Seldns Sd
    FULL OUTER JOIN Selsprs Ss
    ON Ss.Fin_week_No  = Sd.Fin_week_No
    AND Ss.fin_year_no = Sd.fin_year_no
    AND Ss.location_no = Sd.location_no
    AND Ss.item_no     = Sd.item_no
    FULL OUTER JOIN wkcatalg wc
    ON NVL(Ss.Fin_week_No, Sd.Fin_week_No)  = wc.Fin_week_No
    ...
    FULL OUTER JOIN rtl_stock rs
    ON NVL(NVL(Ss.Fin_week_No, Sd.Fin_week_No),wc.Fin_week_No)   = rs.Fin_week_No

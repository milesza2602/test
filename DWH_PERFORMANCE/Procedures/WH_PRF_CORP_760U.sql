--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_760U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_760U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        January 2013
--  Author:      Q Smit
...
begin
 ...
 --select count(*) into g_count from rtl_zone_item_dy_supp_po_plan r
   where exists (
     with aa as (
       select b.sk1_zone_group_zone_no, c.sk1_item_no, d.sk1_supplier_no, a.calendar_date--, a.last_updated_date
         from fnd_zone_item_supp_ff_po_plan a, dim_zone b, dim_item c, dim_supplier d
         --, fnd_jdaff_dept_rollout dept_rollout
       where --dept_rollout.department_no = c.department_no
          --and dept_rollout.department_live_ind = 'Y'
              a.zone_group_no = b.zone_group_no
          and a.item_no = c.item_no
          and a.supplier_no = d.supplier_no
          and a.zone_no = b.zone_no
  ...
  COMMIT;
   ...
   exception
      when dwh_errors.e_insert_error then
       l_message := 'delete rtl_JDA error '||dwh_constants.vc_err_lw_insert||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
       raise;

      when others then
       l_message := 'delete rtl_JDA error '||dwh_constants.vc_err_lw_other||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
       raise;
...
begin
   ...
   merge /*+ parallel(mer_lid,4) */ into RTL_ZONE_ITEM_DY_SUPP_PO_PLAN mer_lid      --rtl_zone_item_dy_supp_po_plan
     using (
       select  /*+ parallel(dy,4) parallel(rs,4) */ dy.tran_date, dy.sk1_item_no, dy.sk1_supplier_no,
        ...
        from rtl_supchain_loc_item_dy dy, 
              ...
              dim_supplier ds,
              dim_location dl,
              RTL_ZONE_ITEM_DY_SUPP_PO_PLAN rs    --> join to this table to get records that exist
   ...
   exception
      when dwh_errors.e_insert_error then
       l_message := 'MERGE POJDA error '||dwh_constants.vc_err_lw_insert||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
       raise;

      when others then
       l_message := 'MERGE POJDA error '||dwh_constants.vc_err_lw_other||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
       raise;
end get_po_data;
...
--**************************************************************************************************
    ...
    select min(calendar_date) into g_stg_date from dwh_foundation.stg_jdaff_po_plan_cpy;

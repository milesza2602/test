--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_131U_AJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_131U_AJ" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_131U_AJ';
...
--**************************************************************************************************
    ...
    l_text := 'TRUNCATING TABLE AJ_RTL_LID_RMS_PRICE_FF ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);    
    
    execute immediate 'truncate table DWH_DATAFIX.AJ_RTL_LID_RMS_PRICE_FF';
/*
...
INSERT /*+ parallel(ff,4) */ INTO DWH_DATAFIX.AJ_RTL_LID_RMS_PRICE_FF ff
...
from    (
          select /*+ parallel(prc,4) parallel(ord,4) full (di), full (dl), full (d) */  prc.*
...
/*
    ...
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','RTL_LID_RMS_PRICE_FF', DEGREE => 32);

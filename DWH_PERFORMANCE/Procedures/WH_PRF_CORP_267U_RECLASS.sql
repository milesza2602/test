--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_267U_RECLASS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_267U_RECLASS" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_267U_RECLASS';
...
--MERGE  INTO DWH_DATAFIX.MART_FOODS_SUPPLIER_PO_AJ rtl_mart USING
(
with ITEM_LIST AS
  (
  SELECT DI.ITEM_NO,
...
from ITEM_LIST a, LOC_LIST B, RTL_PO_SUPCHAIN_LOC_ITEM_DY C, DIM_CALENDAR D, SUPP_LIST E, PURCH_LIST F ,UDA_LIST H
   ...
   where c.po_ind = 1
        and c.tran_date between g_from_date and g_to_date

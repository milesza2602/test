--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_527HST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_527HST" (p_forall_limit in integer,p_success out boolean) as
...
cursor c_lid_sparse is
     SELECT /*+ FULL(lid)  parallel (lid,8) */
              lid.sk1_location_no,
              di.sk1_business_unit_no,
              lid.post_date,
              sum(nvl(lid.shrinkage_qty,0)) shrinkage_qty,
              sum(nvl(lid.shrinkage_selling,0)) shrinkage_selling,
              sum(nvl(lid.shrinkage_cost,0)) shrinkage_cost
     FROM     dwh_performance.rtl_loc_item_dy_rms_sparse lid,
              rtl_loc_bu_dy_shrinkage shr,
              dim_calendar dc,
              dim_item di
     WHERE    lid.post_date           = shr.post_date
     and      lid.sk1_location_no     = shr.sk1_location_no
     AND      di.sk1_business_unit_no = shr.sk1_business_unit_no
     AND      lid.post_date           = dc.calendar_date
     AND      lid.sk1_item_no         = di.sk1_item_no
     and      lid.shrinkage_qty          is not null
     AND      lid.post_date  BETWEEN g_start_date AND g_end_date
     GROUP BY lid.post_date,lid.sk1_location_no, di.sk1_business_unit_no
     ORDER BY lid.post_date,lid.sk1_location_no, di.sk1_business_unit_no
;
...
BEGIN
   ...
   INSERT /*+ APPEND */ INTO dwh_performance.rtl_loc_bu_dy_shrinkage rtl_liwrd
   SELECT /*+ FULL(lid)  parallel (lid,2) */
          lid.sk1_location_no sk1_location_no,
          di.sk1_business_unit_no sk1_business_unit_no,
          lid.post_date,
          0,0,0,
          sum(nvl(lid.shrinkage_qty,0)) shrinkage_qty,
          sum(nvl(lid.shrinkage_selling,0)) shrinkage_selling,
   ...
   AND    lid.post_date          BETWEEN g_start_date AND g_end_date
   and    shrinkage_qty             is not null
   and    not exists
              (SELECT /*+ nl_aj */  * FROM rtl_loc_bu_dy_shrinkage
               WHERE   sk1_location_no       = lid.sk1_location_no AND
                       sk1_business_unit_no  = di.sk1_business_unit_no  AND
                       post_date             = lid.post_date )
   GROUP BY lid.post_date,
            lid.sk1_location_no,di.sk1_business_unit_no;
...
--**************************************************************************************************
    ...
    into   g_start_date
    from   dim_calendar
    WHERE  calendar_date = g_date - 603;   

    select this_week_end_date
    into   g_end_date
    from   dim_calendar
    where  calendar_date = g_date - 36;

--    select count(*)
--    into   g_recs_read
--    from   DWH_FOUNDATION.rtl_loc_item_dy_rms_sparse

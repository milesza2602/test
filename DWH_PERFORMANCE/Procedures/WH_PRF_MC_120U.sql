--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_120U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_120U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        SEPT 2008
...
--  Tables:      Input  - fnd_mc_rtl_shipment
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_120U';
...
type tbl_array_i is table of rtl_mc_loc_item_dy_rms_dense%rowtype index by binary_integer;
type tbl_array_u is table of rtl_mc_loc_item_dy_rms_dense%rowtype index by binary_integer;
...
--+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
cursor c_fnd_mc_rtl_shipment is
   with lid_list as
   (
    select item_no,to_loc_no,shp.actl_rcpt_date
   from   fnd_mc_rtl_shipment shp,
   ...
   select sum(nvl(shp.received_qty,0)) as received_qty,
          sum(case when   vr.VAT_RATE_PERC is null then                                               -- REF03Mar2018
                case when dl.vat_region_no = 1000 then
                    nvl(shp.received_qty,0) * (shp.reg_rsp_opr * 100 / (100 + di.VAT_RATE_PERC))
                else
                    nvl(shp.received_qty,0) * (shp.reg_rsp_opr * 100 / (100 + dl.default_tax_region_no_perc))
                end
              else 
                nvl(shp.received_qty,0) * (shp.reg_rsp_opr * 100 / (100 +  vr.VAT_RATE_PERC  ))           -- REF03Mar2018  
              end) as received_selling,
              
          sum(case when   vr.VAT_RATE_PERC is null then                                               -- REF03Mar2018
                case when dl.vat_region_no = 1000 then
                    nvl(shp.received_qty,0) * (shp.reg_rsp_local * 100 / (100 + di.VAT_RATE_PERC))
                else
                    nvl(shp.received_qty,0) * (shp.reg_rsp_local * 100 / (100 + dl.default_tax_region_no_perc))
                end
              else 
                nvl(shp.received_qty,0) * (shp.reg_rsp_local * 100 / (100 +  vr.VAT_RATE_PERC  ))           -- REF03Mar2018  
              end) as received_selling_local,    
           
          sum(nvl(shp.received_qty,0) * shp.cost_price_opr) as received_cost,
          sum(nvl(shp.received_qty,0) * shp.cost_price_local) as received_cost_local,
   ...
   from   fnd_mc_rtl_shipment shp
          ...
          left join fnd_item_vat_rate vr                                                                                -- REF03Mar2018
                                        on (vr.item_no = shp.item_no and vr.vat_region_no = dl.VAT_REGION_NO and        -- REF03Mar2018
                                            shp.actl_rcpt_date between active_from_date and active_to_date)             -- REF03Mar2018
...
--     and dl.sk1_location_no <> 11943
--     and di.sk1_item_no <> 28711524
--     and trunc(shp.actl_rcpt_date) <> '09-FEB-19' 
   group by trunc(shp.actl_rcpt_date), di.sk1_item_no, dl.sk1_location_no ;



g_rec_in             c_fnd_mc_rtl_shipment%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_mc_rtl_shipment%rowtype;
...
begin
   ...
   g_rec_out.actl_store_rcpt_selling_local   := g_rec_in.received_selling_local;
   g_rec_out.actl_store_rcpt_cost_local      := g_rec_in.received_cost_local;
   ...
   if g_rec_in.chain_no = 20 then
     ...
     if g_debtors_commission_perc is null then
      ...
      g_rec_out.actl_store_rcpt_fr_cost  := 
      nvl(g_rec_out.actl_store_rcpt_cost,0) + round((nvl(g_rec_out.actl_store_rcpt_cost,0) * g_debtors_commission_perc / 100),2);
      g_rec_out.actl_store_rcpt_fr_cost_local  := 
      nvl(g_rec_out.actl_store_rcpt_cost_local,0) + round((nvl(g_rec_out.actl_store_rcpt_cost_local,0) * g_debtors_commission_perc / 100),2);
   end if;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_mc_loc_item_dy_rms_dense values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_mc_loc_item_dy_rms_dense
       set    actl_store_rcpt_qty             = a_tbl_update(i).actl_store_rcpt_qty,
              ...
              actl_store_rcpt_selling_local   = a_tbl_update(i).actl_store_rcpt_selling_local,
              actl_store_rcpt_cost_local      = a_tbl_update(i).actl_store_rcpt_cost_local,
              actl_store_rcpt_fr_cost_local   = a_tbl_update(i).actl_store_rcpt_fr_cost_local,
...
begin
    ...
    l_text := 'LOAD OF rtl_mc_loc_item_dy_rms_dense EX FOUNDATION STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_mc_rtl_shipment;
    fetch c_fnd_mc_rtl_shipment bulk collect into a_stg_input limit g_forall_limit;

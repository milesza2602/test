--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_030U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_030U_FIX" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_030U_FIX';
...
--   begin
      insert /*+ APPEND parallel (ast,4) */ into mart_ch_ast_chn_geo_grd_sc_wk ast
      with aplan_data as (
-- Approved Plan
...
--       and   apln.fin_week_no      between 1 and 28),
      actual_data as (
-- Actualised Plan
...
--       and   act.fin_year_no      = 2014
--       and   act.fin_week_no      between 1 and 28),
      preact_data as (
-- Pre-Actualised Plan
...
--       and   preact.fin_year_no      = 2014
--       and   preact.fin_week_no      between 1 and 28)
-- collapsing of data from respective tables (aplan_data, actual_data, preact_data)
      select sk1_chain_no,
             ...
             '25 jun 12'
      from (
      select /*+ paralle (extr1,6) parallel (extr2,6) */
...
--
      ...
      group by sk1_chain_no, sk1_geography_no, sk1_grade_no, sk1_style_colour_no, fin_year_no, fin_week_no, fin_week_code, '25 jun 12';
/*
      EXCEPTION
        WHEN OTHERS
          THEN

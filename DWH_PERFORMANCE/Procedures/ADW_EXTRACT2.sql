--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACT2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACT2" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACT2';
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_count := dwh_generic_file_extract('with datex as
(select distinct calendar_date from dim_calendar where fin_year_no = 2017)

select /*+ parallel (a,8) parallel (b,8) parallel (c,8) full (b) full (c)*/
a.calendar_date, b.item_no, c.location_no, product_status_1_code,  product_status_code
from rtl_loc_item_dy_catalog a, dim_item b, dim_location c 
where a.calendar_date in (select /*+ full (d) */ calendar_date from datex d  ) 
and a.sk1_item_no = b.sk1_item_no
and a.sk1_location_no = c.sk1_location_no
and this_wk_catalog_ind = 1
and product_status_1_code not in (15,4,26)

order by a.calendar_date asc','|','DWH_FILES_OUT','aj_catalog');

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_254U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_254U" (p_forall_limit in integer, p_success out boolean) as
...
--               Output -   foods_renewal_extract2_1
...
g_rec_out            DWH_PERFORMANCE.foods_renewal_extract2_1%rowtype;
g_count              number        :=  0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_254U';
...
type tbl_array_i is table of DWH_PERFORMANCE.foods_renewal_extract2_1%rowtype index by binary_integer;
type tbl_array_u is table of DWH_PERFORMANCE.foods_renewal_extract2_1%rowtype index by binary_integer;
...
begin
    ...
    l_text := 'LOAD OF foods_renewal_extract2_1 started AT '||
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table DWH_PERFORMANCE.foods_renewal_extract2_1');
...
INSERT /*+ APPEND PARALLEL (mart,4) */ INTO DWH_PERFORMANCE.foods_renewal_extract2_1 mart
WITH item_list AS

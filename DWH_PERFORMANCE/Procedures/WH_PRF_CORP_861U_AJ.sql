--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_861U_AJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_861U_AJ" (p_forall_limit in integer,p_success out boolean) as
...
g_start_date         date          := '27 jun 16';
g_end_date           date          := '25 jun 17';
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_861U_AJ';
...
/*
    begin
       for g_sub in 1 .. g_loop_cnt                                                      -- BK30Sep2016                                                
         ...
         execute immediate 'alter table '|| 'dwh_performance' || '.'|| 'MART_FDS_LI_WK_TAB_REBUILD' ||' truncate subpartition for ('||G_LOOP_FIN_YEAR_NO||','||G_LOOP_FIN_WEEK_NO||')';
...
--           where  calendar_date = trunc(sysdate) - (g_sub * 7);
           where  calendar_date = g_end_date - (g_sub * 7);

        l_text := 'Rollup = '||g_fin_year_no||' - '||g_fin_week_no;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    insert /*+ APPEND parallel (t,6) */ into DWH_PERFORMANCE.MART_FDS_LI_WK_TAB_REBUILD t
    insert /*+ APPEND parallel (t,6) */ into DWH_DATAFIX.AJ_FDS_LI_WK_TAB_REBUILD t

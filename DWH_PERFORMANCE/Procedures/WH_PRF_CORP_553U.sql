--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_553U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_553U" (p_forall_limit in integer, p_success out boolean) as
  --**************************************************************************************************
  --  Date:        February 2009
  --  Author:      Wendy Lyttle
  --  Purpose:     Create location business unit day fact table in the performance layer with
  --               added value ex foundation layer.
  --  Tables:      Input  -   fnd_rtl_till_hist
  --               Output -   rtl_loc_bus_unit_dy
  --  Packages:    constants, dwh_log, dwh_valid
  --
  --  Maintenance:
  --  1 April 2009 - defect 1224 - - no filter on driving cursor
  --
  --  Naming conventions:
  --  g_  -  Global variable
...
l_module_name sys_dwh_errlog.log_procedure_name%type     := 'WH_PRF_CORP_553U';
l_name sys_dwh_log.log_name%type                         := dwh_constants.vc_log_name_rtl_md;
l_system_name sys_dwh_log.log_system_name%type           := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name sys_dwh_log.log_script_name%type           := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name sys_dwh_log.log_procedure_name%type     := l_module_name;
l_text sys_dwh_log.log_text%type ;
l_description sys_dwh_log_summary.log_description%type   := 'CREATE rtl_loc_bus_unit_dy EX fnd_rtl_till_hist';
...
cursor c_fnd_loc_bus_unit_wk_fin_fcst is
with chgrecs as
  (select    location_no, business_unit_no, tran_date
   from      fnd_rtl_till_hist
   where     last_updated_date = g_date
   group  by location_no, business_unit_no, tran_date)

   select dl.sk1_location_no,
          dbu.sk1_business_unit_no,
          frth.tran_date,
          max(dlh.sk2_location_no) sk2_location_no,
          sum(frth.num_tills) num_tills,
          sum(frth.sales_incl_vat) sales_incl_vat,
          sum(frth.sales_vat) sales_vat,
          sum(frth.waste_recov_revenue) waste_recov_revenue,
          sum(frth.waste_recov_revenue_vat) waste_recov_revenue_vat,
          sum(frth.sales_returns) sales_returns,
          sum(frth.sales_returns_vat) sales_returns_vat,
          sum(frth.num_items) num_items,
          sum(frth.num_trans) num_trans,
          sum(frth.num_void_trans) num_void_trans,
          trunc(sysdate) last_updated_date,
          0 sales_hl_ho_fcst,
          0 num_tran_budget
   from   chgrecs cr
   join   fnd_rtl_till_hist frth            on  frth.location_no      = cr.location_no
                                            and frth.business_unit_no = cr.business_unit_no
                                            and frth.tran_date        = cr.tran_date
   join   dim_business_unit dbu             on  frth.business_unit_no = dbu.business_unit_no
   join   dim_location dl                   on  frth.location_no      = dl.location_no
   join   dim_location_hist dlh             on  dl.location_no        = dlh.location_no
                                            and frth.tran_date between dlh.sk2_active_from_date and dlh.sk2_active_to_date
   where  frth.business_unit_no not in (999, 998)
   group  by dl.sk1_location_no, dbu.sk1_business_unit_no, frth.tran_date;

-- Input record declared as cursor%rowtype
g_rec_in c_fnd_loc_bus_unit_wk_fin_fcst%rowtype;
-- Input bulk collect table declared
type stg_array is table of c_fnd_loc_bus_unit_wk_fin_fcst%rowtype;
...
begin
   ...
   g_rec_out.sales_hl_ho_fcst        := null;
   g_rec_out.num_tran_budget         := null;
...
begin
  ...
  save exceptions
   ...
   set    sk2_location_no         = a_tbl_update(i).sk2_location_no,
          num_tills               = a_tbl_update(i).num_tills,
  ...
  when others then
     ...
     loop
        g_error_index := sql%bulk_exceptions(i).error_index;
        l_message     := dwh_constants.vc_err_lb_loop||i|| ' '||g_error_index|| ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)|| ' '
...
begin
  ...
  l_text := 'LOAD OF rtl_loc_bus_unit_dy EX fnd_rtl_till_hist STARTED '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  --**************************************************************************************************
  open c_fnd_loc_bus_unit_wk_fin_fcst;
  fetch c_fnd_loc_bus_unit_wk_fin_fcst bulk collect
  ...
  close c_fnd_loc_bus_unit_wk_fin_fcst;
  --**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_660U_WLTST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_660U_WLTST" (p_forall_limit in integer,p_success out boolean
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_660U_WLTST';
...
--  NB. change made to this proc to not only pick-up chain_code = 'DJ'
--         but also to pickup DJ records for DJ locations based on chain
--      
--**************************************************************************************************
    g_param_ind := 1;
     ...
     insert /*+ append */ into dwh_performance.TEMP_PO_LIST_DJ
           with selpo as
                                      (select po_no, max(supply_chain_type) supply_chain_type
                                       from   fnd_rtl_purchase_order FND, dim_location dl
                                       where  not_before_date > '24 Jun 2007'
                                           and   (supply_chain_type is not null)
                                           and    (FND.chain_code = 'DJ' OR DL.CHAIN_NO = 40)
           ...
           select po.po_no, dp.sk1_po_no, po.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
               ...
               join   selpo ps                    on po.po_no   = ps.po_no
               join   dim_location dl             on po.LOCATION_NO = DL.LOCATION_NO
                   where  (po.chain_code = 'DJ' OR DL.CHAIN_NO = 40)
 ...
 union
           select s.po_no, dp.sk1_po_no, s.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
                   ...
                   join   selpo ps                    on s.po_no    = ps.po_no

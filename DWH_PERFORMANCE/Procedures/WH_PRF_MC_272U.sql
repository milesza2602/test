--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_272U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_272U" (p_forall_limit in integer,p_success out boolean) as
...
--
--
--  Tables:      Input  - fnd_mc_price_change
--               Output - rtl_mc_zone_item_price_change
...
g_count              number        :=  0;
g_rec_out            rtl_mc_zone_item_price_change%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_272U';
...
type tbl_array_i is table of rtl_mc_zone_item_price_change%rowtype index by binary_integer;
type tbl_array_u is table of rtl_mc_zone_item_price_change%rowtype index by binary_integer;
...
cursor c_fnd_mc_price_change is
   select fpr.*,
          dz.sk1_zone_group_zone_no,
          di.sk1_item_no
   from   fnd_mc_price_change fpr,
          dim_zone dz,
          dim_item di
   where  fpr.item_no                = di.item_no  and
          fpr.zone_group_no          = dz.zone_group_no and
          fpr.zone_no                = dz.zone_no and
          ((FPR.STATUS IN ('A','E'))    OR
           (FPR.STATUS = 'D' AND APPROVAL_DATE IS NOT NULL and EXTRACT_DATE IS NOT NULL));
...
type stg_array is table of c_fnd_mc_price_change%rowtype;
a_stg_input      stg_array;

g_rec_in             c_fnd_mc_price_change%rowtype;
...
begin
    ...
    --  g_rec_out.zone_no                := g_rec_in.zone_no;
      ...
      g_rec_out.unit_retail            := g_rec_in.unit_retail_opr;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_mc_zone_item_price_change values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_mc_zone_item_price_change
       set
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_mc_zone_item_price_change
...
begin
    ...
    l_text := 'LOAD OF rtl_mc_zone_item_price_change EX FOUNDATION STARTED AT '||
...
--**************************************************************************************************
    ...
    open c_fnd_mc_price_change;

    fetch c_fnd_mc_price_change bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_263U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_263U_LOAD" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_263U_LOAD';
...
--**************************************************************************************************
    ...
    l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date || ' - NO, ITS ACTUALLY 03/JUL/14 THATS BEING PROCESSED..';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
(
with cal as (
      select fin_year_no year_no, fin_week_no week_no, this_week_start_date, this_week_end_date, calendar_date today_date from dim_calendar where calendar_date = '03/JUL/14'),

     item_list as (select * from dim_item where business_unit_no = 50),
     ...
     zone_item_supp_hist as (select /*+ PARALLEL(rz,4) full(rz) */ unique rz.sk1_item_no, rz.sk1_supplier_no, rz.sk1_zone_group_zone_no,
                             ...
                             from rtl_zone_item_supp_hist rz, loc_list, item_list, supp_list, dim_zone dz
                              where rz.sk1_zone_group_zone_no   = loc_list.sk1_fd_zone_group_zone_no
                                ...
                                and rz.calendar_date = '03/JUL/14'
                                )
     ,
...
,
    ...
    loc_item as (select /*+ PARALLEL(rli,4) */ rli.sk1_location_no,
                                               rli.sk1_item_no, nvl(rli.min_order_qty,0) moq, item_list.item_no, item_list.item_desc,
   ...
   zone_item_om as (select /*+ PARALLEL(rzom,4)  */  unique rzom.sk1_zone_group_zone_no, rzom.sk1_item_no,  rzom.from_loc_no, supp_list.supplier_no,
                            rzom.num_shelf_life_days, rzom.reg_delivery_pattern_code,
                      ...
                      where rzom.sk1_zone_group_zone_no = loc_list.sk1_fd_zone_group_zone_no
                        ...
                        and dps.product_status_code in ('15', '14', '4', '26')
                        and cast(rzom.product_status_code as varchar2(2)) = dps.product_status_code
                        and dps.sk1_product_status_no <> 0
                        )
                ,
    depot_item_wk as (select /*+ PARALLEL(depot,4) FULL(depot) */ depot.fin_year_no, depot.fin_week_no, depot.sk1_location_no,
                                                  depot.sk1_item_no, ship_ti, ship_hi
                        from rtl_depot_item_wk depot, loc_list, item_list, supp_list, cal
                       where depot.sk1_item_no = item_list.sk1_item_no
                         and depot.sk1_location_no = loc_list.sk1_location_no
                         and depot.supplier_no     = supp_list.supplier_no
                         and depot.fin_year_no = cal.year_no
                         and depot.fin_week_no = cal.week_no)
...
select  zone_item_supp_hist.sk1_zone_group_zone_no,
        ...
        nvl(depot_item_wk.ship_ti, 0) ship_ti,
        nvl(depot_item_wk.ship_hi, 0) ship_hi,
        ...
        nvl(loc_item.moq,0) min_order_qty,
        st_ord.post_date ,
        st_ord.vendor_pack,
        loc_item.moq,
        0 as subcase,
...
from loc_item
     ...
     left outer join depot_item_wk on loc_item.sk1_item_no      = depot_item_wk.sk1_item_no
                                 and loc_item.sk1_location_no  = depot_item_wk.sk1_location_no

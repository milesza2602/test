--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_003U_FOOD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_003U_FOOD" 
...
--**************************************************************************************************
    ...
    execute immediate ('truncate table dwh_DATAFIX.T14_rtl_area_item_wk_price');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.T14_rtl_area_item_wk_price';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
                                   'T14_rtl_area_item_wk_price', DEGREE => 8);
    commit;
--    l_text := 'GATHER STATS on dwh_DATAFIX.T14_rtl_area_item_wk_price';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';
    
    INSERT /*+ APPEND */ INTO dwh_DATAFIX.T14_rtl_area_item_wk_price
...
commit;
 ...
 SELECT MIN(THIS_WEEK_START_DATE), MAX(THIS_WEEK_END_DATE) 
    ...
    execute immediate ('truncate table dwh_DATAFIX.T13_rtl_area_item_wk_price');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.T13_rtl_area_item_wk_price';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
                                   'T13_rtl_area_item_wk_price', DEGREE => 8);
    commit;
--    l_text := 'GATHER STATS on dwh_DATAFIX.T13_rtl_area_item_wk_price';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';
    
    INSERT /*+ APPEND */ INTO dwh_DATAFIX.T13_rtl_area_item_wk_price
       select   /*+ full(rai) parallel (rai	4) */
       dl.area_no
                ,di.item_no
                ,dc.fin_year_no

--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_500C
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_500C" 
(p_forall_limit in integer,p_success out boolean) 
as
--**************************************************************************************************
--  Date:        March 2015
--  Author:      Quentin Smit
--  Purpose:     Load CATALOGUE data into performance layer daily forecast LEVEL 1(ITEM LEVEL)
...
--------------------------------NEW VERSION--------------------------------------------------------------------
--  Maintenance:
--  qc4340 - Q SMIT: RDF Rollup of LEVEL 1(ITEM LEVEL) data
      --                        - This procedure IS A NEW PROCEDURE based on WH_PRF_RDF_001C
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_500C';
...
cursor c_fnd_rtl_loc_item_wk_dyfcst is
  ...
  join    RTL_LOC_ITEM_RDF_DYFCST_L1 fcst on
 ...
 --   and   fcst.sk1_item_no = 12787 and fcst.sk1_location_no = 13322
 ;
...
begin
    ...
    l_text := 'LOAD RTL_LOC_ITEM_RDF_DYFCST_L1 EX TEMP TABLES STARTED '||

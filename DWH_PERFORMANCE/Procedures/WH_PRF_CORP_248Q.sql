--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_248Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_248Q" (p_forall_limit in integer, p_success out boolean) as 

--**************************************************************************************************
--  Date:        December l_fin_year_no - 1
--  Author:      Quentin Smit
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_248Q';
...
INSERT /*+ APPEND PARALLEL (mart,4) */ INTO dwh_performance.rtl_loc_item_wk_sales_index mart
...
where f.sk1_item_no       = il.sk1_item_no
and f.sk1_location_no     = sl.sk1_location_no
and f.fin_year_no         = l_fin_year_no - 1  -- current week fin year
and f.fin_week_no         = l_fin_week_no    -- current week to date

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_680U_FIX201408
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_680U_FIX201408" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Feb 2009
--  Author:      M Munnik
...
cursor c_rtl_loc_item_dy_waste_avail is
   ...
   left join (select r.sk1_location_no, r.sk1_item_no, c.this_week_end_date, sum(r.sales_dly_app_fcst_qty) sales_dly_app_fcst_qty
              from rtl_loc_item_dy_rdf_fcst r join dim_calendar c on r.post_date = c.calendar_date
...
END WH_PRF_CORP_680U_FIX201408;

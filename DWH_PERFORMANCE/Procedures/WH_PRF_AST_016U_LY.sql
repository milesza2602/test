--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_016U_LY
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_016U_LY" 
...
g_min_fin_week_no    dim_calendar.fin_week_no%type;
g_max_fin_week_no    dim_calendar.fin_week_no%type;
g_fin_year_no        dim_calendar.fin_year_no%type;
g_min_fin_year_no    dim_calendar.fin_year_no%type;
g_max_fin_year_no    dim_calendar.fin_year_no%type;
g_ly_fin_year_no     dim_calendar.fin_year_no%type;
g_ly_fin_week_no     dim_calendar.fin_week_no%type;
g_date               date;
g_ly_this_week_start_date   date;
g_ly_this_week_end_date     date;
...
--**************************************************************************************************
   ...
   select min(fin_year_no), min(fin_week_no), max(fin_year_no), max(fin_week_no)
   into g_min_fin_year_no, g_min_fin_week_no, g_max_fin_year_no, g_max_fin_week_no
   from fnd_ast_loc_sc_wk_act
   where last_updated_date = g_date;

   select ly_fin_year_no, ly_fin_week_no
   into   g_ly_fin_year_no, g_ly_fin_week_no
   from   dim_calendar
   where  fin_year_no = g_min_fin_year_no
   and    fin_week_no = g_min_fin_week_no
   and    fin_day_no  = 3;

   select this_week_start_date
   into g_ly_this_week_start_date
   from dim_calendar
   where fin_year_no = ly_fin_year_no
   and   fin_week_no = ly_fin_week_no
   and   fin_day_no  = 3;

   select ly_fin_year_no, ly_fin_week_no
   into   g_ly_fin_year_no, g_ly_fin_week_no
   from dim_calendar
   where fin_year_no = g_max_fin_year_no
   and   fin_week_no = g_max_fin_week_no
   and   fin_day_no  = 3;

   select this_week_end_date
   into   g_ly_this_week_end_date
   from   dim_calendar
   where  fin_year_no = g_max_fin_year_no
   and    fin_week_no = g_max_fin_week_no
   and    fin_day_no  = 3;
   ...
   execute immediate 'alter session enable parallel dml';
      ...
      insert /*+ APPEND parallel (ast,2) */ into rtl_loc_geo_grd_sc_wk_ast_act ast
      ...
      select /*+ full (apln) parallel (apln,2) */
             ...
             nvl(lsc.grade_no,999) as sk1_grade_no,
             sc.sk1_style_colour_no,
             apln.fin_year_no + 1 as fin_year_no,
             apln.fin_week_no,
             pln.sk1_plan_type_no,
             'W'||apln.fin_year_no||apln.fin_week_no as fin_week_code,
             apln.store_count,
      ...
      where apln.style_colour_no   = sc.style_colour_no
       ...
       and  apln.fin_week_no       = cal.fin_week_no
       and  cal.fin_day_no         = 3
       and  cal.this_week_start_date between g_ly_this_week_start_date and g_ly_this_week_end_date
      order by apln.fin_year_no, apln.fin_week_no)

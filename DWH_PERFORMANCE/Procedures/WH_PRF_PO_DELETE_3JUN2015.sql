--------------------------------------------------------
--  DDL for Procedure WH_PRF_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_PO_DELETE_3JUN2015';
...
begin
  ...
  execute immediate 'alter session enable parallel dml';
            ...
            DELETE /*+  parallel(A,4)  */ FROM dwh_performance.RTL_PO_SUPCHAIN_LOC_ITEM_DY  a
              where a.sk1_po_no in (SELECT SK1_PO_NO
                          FROM DIM_PURCHASE_ORDER
                          WHERE 
                          PO_NO BETWEEN 1000000 AND 2999999
...
l_text := 'RTL_PO_SUPCHAIN_LOC_ITEM_DY recs deleted = '||g_recs_deleted;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 


            DELETE /*+  full(a)  */ FROM dwh_performance.RTL_STOCKOWN_DCR  a
...
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
 DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','RTL_STOCKOWN_DCR', DEGREE => 8);
...
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
 ...
 DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','RTL_STOCKOWN_DCR', DEGREE => 8);
 DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','RTL_PO_SUPCHAIN_LOC_ITEM_DY', DEGREE => 8);
--**************************************************************************************************

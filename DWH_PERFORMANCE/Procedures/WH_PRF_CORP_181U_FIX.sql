--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_181U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_181U_FIX" (
...
--**** setup rtl table with parttition on cycle_no.
  ...
  --**************************************************************************************************
      ...
      WHERE area_no = 9951
  --    and ((fnd.last_updated_date = '1 dec 2013')
  --    or (fnd.calendar_date between '1 oct 2013' and  '1 dec 2013')  )
      and ((fnd.last_updated_date = g_date)
 --
 -- REMOVED WL 14 MAR 2014 - filter to limiting, must redo whole cycle if there is a change
       or (fnd.last_updated_date between G_DATE - 502 and  g_date) 
 --or (fnd.calendar_date between g_date_6wks and  g_date) 
 --
  )
    --    and cycle_no in(24,25)
...
( WITH seldat AS
      ...
      seldns AS
              (SELECT
                ...
                dsd.sk1_location_no,
                cycle_no,
                max(cycle_from_date) cycle_from_date,
              ...
              AND DNS.POST_DATE = '02/OCT/17'
              AND DNS.LAST_UPDATED_DATE = '14/FEB/18'

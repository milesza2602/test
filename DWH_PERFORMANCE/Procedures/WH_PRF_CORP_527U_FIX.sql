--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_527U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_527U_FIX" (p_forall_limit in integer,p_success out boolean) as
...
--**************************************************************************************************
    ...
    into   g_start_date
    FROM   dim_calendar
    WHERE  calendar_date = '02/OCT/17' ;  --g_date - 476;

    select this_week_end_date
    into   g_end_date
    FROM   dim_calendar
    where  calendar_date = '02/OCT/17' ;  --g_date ;

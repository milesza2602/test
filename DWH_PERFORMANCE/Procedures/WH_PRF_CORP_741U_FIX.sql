--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_741U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_741U_FIX" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2013
--  Author:      Q. Smit
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_741U_FIX';
...
--    where calendar_date = g_rec_in.calendar_date;
   ...
   case l_day_no
    ...
    when 7 then
      ...
      if l_cnt > 0 then
         select nvl(week_1_day_6_cases,0) into l_week_1_day_6_cases
           from rtl_loc_item_dc_wh_plan
          where sk1_item_no        = g_rec_out.sk1_item_no 
            and sk1_location_no    = g_rec_out.sk1_location_no
            and calendar_date      = g_rec_out.calendar_date-1;

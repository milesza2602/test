--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACT5
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACT5" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACT5';
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_count := dwh_generic_file_extract('select distinct SUPPLIER_NO,SUPPLIER_NAME,SUBCLASS_NO,SUBCLASS_NAME,ITEM_NO,ITEM_DESC                
from RTL_LOC_ITEM_DY_RMS_DENSE dns, dim_supplier a,        
dim_item b         
where dns.SK1_item_no = b.SK1_item_no
and a.SK1_SUPPLIER_NO = b.SK1_SUPPLIER_NO              
and department_no = 48             
and dns.Post_date between ''1-Jun-2018'' and ''30-Jun-2018''
and SUPPLIER_No = 11933
order by SUBCLASS_NO desc ','|','DWH_FILES_OUT','aztec_SKU_201807');

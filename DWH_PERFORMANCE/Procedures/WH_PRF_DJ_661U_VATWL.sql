--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_661U_VATWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_661U_VATWL" 
...
--                but the not_before_date/not_after_date has changed after the record was inserted to rtl_po_supchain_loc_item_dy_vatwl.
--  Tables:       Input  - fnd_rtl_purchase_order, fnd_rtl_shipment
--                Output - rtl_posupchnlocitemdy_DJ_vatwl  rtl_posupchnlocitemdy_DJ_vatwl
...
g_date100               date          :=  trunc(sysdate);
g_rec_out            rtl_posupchnlocitemdy_DJ_vatwl%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_661U_VATWL';
...
type tbl_array_i is table of rtl_posupchnlocitemdy_DJ_vatwl%rowtype index by binary_integer;
type tbl_array_u is table of rtl_posupchnlocitemdy_DJ_vatwl%rowtype index by binary_integer;
...
--**************************************************************************************************
--
-- Do calculations and load into temp_po_DATA_DJ
--
--**************************************************************************************************

procedure INSERT_INTO_temp_po_DATA_DJ  as
begin

INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.temp_po_DATA_DJ
with
---------------------------
------- pack items --------
---------------------------
      ...
      as
         (select    /*+ materialize full(pi) full(tp) */
                            distinct
                              tp.po_no
                            , tp.item_no
                   from DWH_PERFORMANCE.temp_po_list_DJ_vatwl tp 
...
----------------------
   ...
   (
      select    /*+ materialize parallel(di,4) full(di) 
                            ...
                            , vat_rate_perc
                   from          dim_item di
                   join          DWH_PERFORMANCE.temp_po_list_DJ_vatwl tp            on  tp.item_no            = di.item_no
     ...
     selship as
       (select   
                 ...
                 from      fnd_rtl_shipment s
                  join      DWH_PERFORMANCE.temp_po_list_DJ_vatwl tp    
...
(
   ...
   /*+  MATERIALIZE parallel(P,4) full(TP) */
          ...
          ,
            (case when p.chain_code is null
                  then 0
                  else 1 end) sk1_chain_code_ind
    from      fnd_rtl_purchase_order p
    join      DWH_PERFORMANCE.temp_po_list_DJ_vatwl tp       on  Tp.item_no                   = p.item_no
...
---------------------------------------------------
    SELECT /*+  parallel(diT,2) full(diT) parallel(diH,2) full(diH) 
          ...
          , SP.sk1_chain_code_ind
from selpo sp
join selitpo ip
        on ip.item_no = sp.item_no
        and ip.po_no = sp.po_no
join  dim_location_vatwl dl            
        on  dl.location_no = sp.location_no
...
left join rtl_location_item li  on  LI.sk1_item_no       = SP.sk1_item_no  and LI.sk1_location_no   = DL.sk1_location_no
GROUP BY Sp.sk1_po_no
          ...
          , nvl(ds.sk1_supplier_no, 0) 
           ...
           , NVL(LI.TAX_PERC, (case when vat_region_no = 1000 then ip.vat_rate_perc else dl.DEFAULT_TAX_REGION_NO_PERC end ) ) ,
           SP.sk1_chain_code_ind
;
    ...
    commit; 
    l_text := 'Recs inserted into temp_po_DATA_DJ='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   exception
     when others then
       l_message := 'error in INSERT_temp_po_DATA_DJ '||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
       raise;

end INSERT_INTO_temp_po_DATA_DJ ;

--**************************************************************************************************
--
-- Do calculations and load into temp_po_CALC_DJ
--
--**************************************************************************************************
procedure INSERT_INTO_temp_po_CALC_DJ as
begin

INSERT /*+ APPEND */ INTO  DWH_PERFORMANCE.temp_po_CALC_DJ
...
--                           ---------- 
           ...
           ,CASE WHEN P.business_unit_no not in(50, 70) then
              ...
              , CASE WHEN  P.business_unit_no = 50 then
                    CASE WHEN (P.amended_po_qty is null) then  P.original_po_qty   else  P.amended_po_qty + nvl(P.cancel_po_qty,0)  END
                 else
                    CASE WHEN p.po_status_code = 'C' then   P.grn_qty      
                    else
                       CASE WHEN p.po_status_code = 'A' and (P.bc_shipment_qty is not null) then   P.bc_shipment_qty  else   P.amended_po_qty  end 
                    end 
                 end                                                                                                                                        LATEST_PO_QTY
              ,    CASE WHEN  P.business_unit_no = 50 then
                        CASE WHEN (P.amended_po_qty is null) then
                         ...
                         END
                  END                                                                                                                                           LATEST_PO_CASES 
          FROM DWH_PERFORMANCE.TEMP_PO_DATA_DJ P
...
selext2 as ( SELECT 
           ...
           , CASE WHEN se1.business_unit_no not in(50, 70) then
              ...
              , CASE WHEN  se1.business_unit_no = 50 then      
                   CASE WHEN (se1.amended_po_qty is null) then   se1.original_po_selling  else    se1.amended_po_selling + nvl(se1.cancel_po_selling,0)  END
                 else
                    CASE WHEN se1.po_status_code = 'C' then  se1.po_grn_selling
                    else
                       CASE WHEN se1.po_status_code = 'A' and (se1.bc_shipment_qty is not null) then  se1.bc_shipment_selling  else    se1.amended_po_selling  end 
                    end 
                 end                                                                                                                                           LATEST_PO_SELLING
               , CASE WHEN  se1.business_unit_no = 50 then
                    CASE WHEN (se1.amended_po_qty is null) then  se1.original_po_cost else   se1.amended_po_cost + nvl(se1.cancel_po_cost,0)  END
                 else
                    CASE WHEN se1.po_status_code = 'C' then  se1.po_grn_cost
                    else
                       CASE WHEN se1.po_status_code = 'A' and (se1.bc_shipment_qty is not null) then  se1.bc_shipment_cost else   se1.amended_po_cost   end 
                    end 
                 end                                                                                                                                            LATEST_PO_COST
    ...
    g_recs_inserted :=  SQL%ROWCOUNT;
    commit; 
    l_text := 'Recs inserted into temp_po_CALC_DJ='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   exception
     when others then
       l_message := 'error in INSERT_temp_po_CALC_DJ '||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
       raise;


end INSERT_INTO_temp_po_CALC_DJ;

--**************************************************************************************************
--
-- Do calculations and load into rtl_posupchnlocitemdy_DJ_vatwl
--
--**************************************************************************************************
procedure MERGE_INTO_RTL as
begin

MERGE INTO  DWH_PERFORMANCE.rtl_posupchnlocitemdy_DJ_vatwl RTL
 using ( SELECT a.*, g_date+100 gdate100 FROM     DWH_PERFORMANCE.temp_po_CALC_DJ a ) MER_MART
on (mer_mart.sk1_po_no                   = RTL.sk1_po_no
 ...
 when matched then
  update
     set      sk1_chain_code_ind		          = MER_MART.sk1_chain_code_ind,
              sk1_contract_no		              = MER_MART.sk1_contract_no,
              ...
              last_updated_date               = MER_MART.gdate100,
  ...
  when not matched then
   ...
   values 
      (MER_MART.SK1_PO_NO
                  ...
                  ,MER_MART.gdate100 --LAST_UPDATED_DATE
    ...
    g_recs_inserted :=  SQL%ROWCOUNT;
    commit; 
    l_text := 'Recs merged into rtl_posupchnlocitemdy_DJ_vatwl='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   insert into temp_po_deletes_dj_vatwl
    select /*+ no_index (r) full(r) parallel (r,8) */ r.sk1_po_no, r.sk1_supply_chain_no, r.sk1_location_no, r.sk1_item_no, r.tran_date,
	         r.sk1_chain_code_ind, r.sk1_supplier_no, di.sk1_style_colour_no, dc.fin_year_no, dc.fin_week_no
    from   DWH_PERFORMANCE.rtl_posupchnlocitemdy_DJ_vatwl r
    join   temp_po_list_dj_vatwl t     on  r.sk1_po_no        = t.sk1_po_no
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    delete from DWH_PERFORMANCE.rtl_posupchnlocitemdy_DJ_vatwl r
    where  exists(select t.sk1_po_no
                  from   temp_po_deletes_dj_vatwl t
                  ...
                  and    t.sk1_chain_code_ind = r.sk1_chain_code_ind);
--    delete from rtl_po_supchain_loc_item_dy_dj r
--    where  exists(select t.po_no
--                  from   temp_po_list_dj t
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD OF rtl_posupchnlocitemdy_DJ_vatwl EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table DWH_PERFORMANCE.temp_po_DATA_DJ';
    l_text := 'DWH_PERFORMANCE.temp_po_DATA_DJ TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    INSERT_INTO_temp_po_DATA_DJ;

    execute immediate 'truncate table DWH_PERFORMANCE.temp_po_CALC_DJ';
    l_text := 'DWH_PERFORMANCE.temp_po_CALC_DJ TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    INSERT_INTO_temp_po_CALC_DJ;

    MERGE_INTO_RTL;
 
   /* execute immediate 'truncate table DWH_PERFORMANCE.temp_po_deletes_VATWL';
    l_text := 'TABLE DWH_PERFORMANCE.temp_po_deletes_VATWL TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DELETE_INVALID_RECS;
*/
--**************************************************************************************************

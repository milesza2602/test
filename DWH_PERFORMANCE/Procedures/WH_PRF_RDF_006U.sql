--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_006U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_006U" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as

--**************************************************************************************************
--  Date:        August 2010
--  Author:      Alfonso Joshua
--  Purpose:     Create the RDF weekly forecast level 1 temp table in the performance layer
--               with input ex temp RDF forecast level 1
--  Tables:      Input  - temp_loc_item_wk_rdf_sysfcstl1
--               Output - rtl_loc_item_wk_rdf_fcst_l2
...
g_count              number        :=  0;
g_rec_out            rtl_loc_item_wk_rdf_fcst_l2%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_006U_'|| p_from_loc_no;
...
type tbl_array_i is table of rtl_loc_item_wk_rdf_fcst_l2%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_item_wk_rdf_fcst_l2%rowtype index by binary_integer;
...
cursor c_tmp_rtl_loc_item_wk_wkfcst is
 select /*+ FULL(sys_ilv) */
          ...
          di.random_mass_ind,
          di.static_mass
  from    temp_loc_item_wk_rdf_sysfcstl1 sys_ilv
  join    dim_item di on
...
--   g_rec_out.last_updated_date               := g_rec_in.this_week_start_date; - data take-on
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_item_wk_rdf_fcst_l2 values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_loc_item_wk_rdf_fcst_l2
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_loc_item_wk_rdf_fcst_l2
...
begin
    ...
    l_text := 'LOAD OF rtl_loc_item_wk_rdf_fcst_l2 EX FOUNDATION STARTED AT '||

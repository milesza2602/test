--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_006A
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_006A" 
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        August 2010
--  Author:      Alfonso Joshua
--  Purpose:     Create the weekly temp swing system forecast level 1 table in the performance layer
--               with input ex RDF forecast table from foundation layer.
--  Tables:      Input  - fnd_rtl_li_wk_rdf_wkfcst_l2
--               Output - temp_loc_item_wk_rdf_sysfcstl1
...
g_max_week           number        :=  0;
g_rec_out            temp_loc_item_wk_rdf_sysfcstl1%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
g_calendar_date      date          := trunc(sysdate);
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_006A';
...
type tbl_array_i is table of temp_loc_item_wk_rdf_sysfcstl1%rowtype index by binary_integer;
type tbl_array_u is table of temp_loc_item_wk_rdf_sysfcstl1%rowtype index by binary_integer;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into temp_loc_item_wk_rdf_sysfcstl1 values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update temp_loc_item_wk_rdf_sysfcstl1
...
/*
   if g_fnd_count > 0 then  /* check done to determine multiple staging loads into foundation - TD-1169
      select count(1)
      into   g_count
      from   temp_loc_item_wk_rdf_sysfcstl1
...
begin
    ...
    l_text := 'LOAD TEMP_LOC_ITEM_WK_RDF_SYSFCSTL1 EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table dwh_performance.temp_loc_item_wk_rdf_sysfcstl1';
    l_text := 'TABLE temp_loc_item_wk_rdf_sysfcstl1 TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

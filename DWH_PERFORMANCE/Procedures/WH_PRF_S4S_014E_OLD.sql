--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_014E_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_014E_OLD" (p_forall_limit in integer,p_success out boolean) as
...
--
--  DIM Tables:  Input  - FND_S4S_LABOUR_ROLE
--               Output - DIM_LABOUR_ROLE
...
g_rec_out            DIM_LABOUR_ROLE%rowtype;
--g_rec_out            RTL_S4S_LABOUR_ROLE%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_014E_OLD';
...
type tbl_array_i is table of DIM_LABOUR_ROLE%rowtype index by binary_integer;
type tbl_array_u is table of DIM_LABOUR_ROLE%rowtype index by binary_integer;
...
cursor c_JOBGROUP is
    ...
    select sk1_workgroup_id,workgroup_id,sk1_s4s_business_unit_no, count(*)
     from (Select Distinct
             ...
             from fnd_s4s_labour_role fnd,
                  dim_job dmj
--                  rtl_s4s_job dmj
            where fnd.workgroup_id = dmj.workgroup_id
              and fnd.s4s_business_unit_no = dmj.s4s_business_unit_no 
            order by workgroup_id,s4s_business_unit_no,sk1_effective_from_date
          )
...
--                  rtl_s4s_job dmj
    ...
    jobid as (
    select sk1_job_id,job_id, count(*)
     from (Select Distinct
                  dmj.sk1_job_id,
                  fnd.job_id,
                  dmj.sk1_jobgroup_id,
...
--                  rtl_s4s_job dmj
    ...
    --    ,case when fnd.effective_to_date < '30/DEC/3999' then 'INACTIVE' ELSE 'ACTIVE' end as Ind
    FROM fnd_s4s_labour_role fnd
         ,workgroup wg
         ,jobgroup jg
         ,jobid jd
...
--      and fnd.labour_role_id in (1000192,1000217,1000243,1000271,1000316,1000390,1000410,1000413,1000414)
    order by fnd.labour_role_id,fnd.job_id,fnd.jobgroup_id,fnd.workgroup_id,fnd.s4s_business_unit_no,fnd.effective_from_date
    ;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into DIM_LABOUR_ROLE values a_tbl_insert(i);
--      insert into RTL_S4S_LABOUR_ROLE values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       UPDATE DIM_LABOUR_ROLE
--       UPDATE RTL_S4S_LABOUR_ROLE
...
begin
   ...
   select count(1)
     into g_count
     from DIM_LABOUR_ROLE
--     from RTL_S4S_LABOUR_ROLE

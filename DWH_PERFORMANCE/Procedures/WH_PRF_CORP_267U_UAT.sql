--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_267U_UAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_267U_UAT" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_267U_UAT';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE FOODS SUPPLIER PO MART';
...
begin
  ...
  --   from dim_calendar
  --  where fin_year_no = 2015
  --    and fin_week_no in (1,2);
    
    G_FROM_DATE := '30/MAR/15';
    G_TO_DATE   := '26/APR/15';
...
MERGE  INTO W6005682.MART_FOODS_SUPPLIER_POQ rtl_mart USING
(
with ITEM_LIST AS
  (
  SELECT DI.ITEM_NO,

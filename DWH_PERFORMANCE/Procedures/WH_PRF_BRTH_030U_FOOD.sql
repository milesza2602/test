--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_030U_FOOD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_030U_FOOD" 
...
--**************************************************************************************************
   ...
   where this_week_start_date = (select this_week_start_date from dim_calendar where calendar_date = g_end_date);
    ...
    --l_text := '2016 PERIOD - '||g_last_week_start_date||' to '||g_last_week_end_date;
    --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'START DATE OF ROLLUP - '||g_start_date||' to '||g_end_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    execute immediate ('truncate table dwh_DATAFIX.rtl14_supp_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.rtl14_supp_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
WITH selsupp AS
                      ...
                      FROM dwh_performance.rtl_zone_item_supp_dy rzia,
                                        ...
                                        --AND dc.this_week_start_date BETWEEN '1 JULY 2013' AND '29 JUNE 2014'
                                        --and dc.fin_year_no = 2016
                                        ...
                                        and rzi.sk1_supplier_no = di.sk1_supplier_no
  ...
  selsuppch AS
              ...
              LEFT OUTER JOIN dwh_performance.dim_item DI
                    ...
                    and di.sk1_supplier_no = rai.sk1_supplier_no                   --XX
              ...
              WHERE dc.this_week_start_date BETWEEN g_start_date AND g_end_date
                  ...
                  and di.primary_supplier_no  in (9548)  --(24785, 25357)  --(59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110) --(50885)  --, 14314)
                  and ds.supplier_no in (9548)  --(24785, 25357)  --(59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110) --= 50885                                        --XX
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    execute immediate ('truncate table dwh_DATAFIX.rtl13_supp_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.rtl13_supp_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   'rtl13_supp_item_wk_bridgethorn', DEGREE => 8);
--    commit;
--    l_text := 'GATHER STATS on dwh_DATAFIX.rtl13_supp_item_wk_bridgethorn';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';

  g_recs_count :=0;
  INSERT /*+ APPEND */ INTO dwh_DATAFIX.rtl13_supp_item_wk_bridgethorn    
 WITH selsupp AS
                      (SELECT
                                /*+ no_merge full(rzia) parallel (rzia 4) */
                                rzia.sk1_item_no,
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    execute immediate ('truncate table dwh_DATAFIX.rtl18_supp_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.rtl18_supp_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--
--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   'rtl13_supp_item_wk_bridgethorn', DEGREE => 8);
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  g_recs_count :=0;
  INSERT /*+ APPEND */ INTO dwh_DATAFIX.rtl18_supp_item_wk_bridgethorn    
 WITH selsupp AS
                      (SELECT
                                /*+ no_merge full(rzia) parallel (rzia 4) */
                                rzia.sk1_item_no,
                      ...
                      FROM dwh_performance.rtl_zone_item_supp_dy rzia,
                                        ...
                                        and di.primary_supplier_no in (9548)  --(24785, 25357)  --(50885, 59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110, 14314, 24785, 25357)
                                        GROUP BY rzi.sk1_item_no,
                                          rzi.sk1_supplier_no, dc.this_week_start_date ,dc.this_week_end_date
                                        ) b
                      WHERE rzia.sk1_item_no   = b.sk1_item_no
  ...
  selsuppch AS
              ...
              WHERE dc.this_week_start_date BETWEEN g_start_date AND g_end_date
                  ...
                  and (RAI.cancel_code <> 'B'or RAI.cancel_code is null)
                  --and di.primary_supplier_no = 50885  -- in (59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110) --(50885)  --, 14314)
                  --and ds.supplier_no = 50885  --in (59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110) --= 50885            
                  and di.primary_supplier_no in (9548)  --(24785, 25357)  --(50885, 59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110, 14314, 24785, 25357)--XX
                  and ds.supplier_no in (9548)  --(24785, 25357)  --(50885, 59816,11821,10287,59815,  22111,22101,22106,22112,22102,22103,22104,22105,22107,22108,22109,22110, 14314, 24785, 25357)
              GROUP BY dc.fin_year_no ,
                        dc.fin_week_no ,
                        dc.this_week_start_date ,
                        dc.this_week_END_date ,
...
commit;
    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
                                   'rtl18_supp_item_wk_bridgethorn', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_DATAFIX.rtl18_supp_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

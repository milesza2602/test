--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_733U_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_733U_TEST" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_733U_TEST';
 ...
 (select dl.sk1_location_no,
  ...
  (select sk1_item_no,
   ...
   where  cat.post_date         = dep.post_date and
     ...
     --     and cat.sk1_item_no = 14315406
    --      and cat.sk1_location_no = 579;
...
--   l_text := 'FD_NUM_DC_CATLG_DAYS SET = 1';
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--   l_text := 'STOCK_CASES = '|| g_rec_in.stock_cases;
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   if g_rec_out.fd_num_dc_catlg_days = 1 and
      g_rec_in.stock_cases > 0 then
      g_rec_out.fd_num_dc_avail_days         := 1;
   --   l_text := 'FD_NUM_DC_AVAIL_DAYS SET = 1';
   --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   else
      g_rec_out.fd_num_dc_avail_days         := 0;
   --   l_text := 'FD_NUM_DC_AVAIL_DAYS SET = 0';
   --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;

   begin
      select uda_value_no_or_text_or_date
      into   g_uda_value_no_or_text_or_date
      from   fnd_item_uda
      where  item_no = g_rec_in.inline_item_no and
             uda_no  = 542;

      exception
         when no_data_found then
            g_uda_value_no_or_text_or_date := 0;
   end;

   --l_text := 'FD_NUM_DC_CATLG_DAYS  = ' || g_rec_out.fd_num_dc_catlg_days ;
   --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   --l_text := 'ITEM_NO = '|| g_rec_in.item_no;
   --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   --l_text := 'G_UDA_VALUE_NO_OR_TEXT_OR_DATE = '|| g_uda_value_no_or_text_or_date;
   --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   
   if g_rec_out.fd_num_dc_catlg_days = 1 and
      g_rec_in.item_no               = 0 and
      g_uda_value_no_or_text_or_date = 1 then
      g_rec_out.fd_num_dc_catlg_adj_days     := 1;
      --l_text := 'FD_NUM_DC_CATLG_ADJ_DAYS SET = 1';
      --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   else
      g_rec_out.fd_num_dc_catlg_adj_days     := 0;
      --l_text := 'FD_NUM_DC_CATLG_ADJ_DAYS SET = 0';
      --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;
   
   --l_text := 'FD_NUM_DC_DC_AVAIL_DAY = '|| g_rec_out.fd_num_dc_catlg_days;
   --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

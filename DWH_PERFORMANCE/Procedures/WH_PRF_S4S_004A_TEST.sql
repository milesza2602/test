--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_004A_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_004A_TEST" (
...
--COMMIT;
  ...
  BEGIN
    ...
    WHERE CONSTRAINT_NAME = 'PK_TMP_S4S_LC_EMP_DY_PRT1'
    AND TABLE_NAME        = 'TEMP_S4S_LOC_EMP_DY_PART1';
    
    l_text               := 'alter table dwh_performance.TEMP_S4S_LOC_EMP_DY_PART1 drop constraint PK_TMP_S4S_LC_EMP_DY_PRT1';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_performance.TEMP_S4S_LOC_EMP_DY_PART1 drop constraint PK_TMP_S4S_LC_EMP_DY_PRT1');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'constraint PK_TMP_S4S_LC_EMP_DY_PRT1 does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'TEMP_S4S_LOC_EMP_DY_PART1', DEGREE => 4);
...
---------------------------------------------------------------
    ...
    l_text := 'ALTER TABLE dwh_performance.temp_S4S_LOC_EMP_DY_part1 ADD CONSTRAINT PK_TMP_S4S_LC_EMP_DY_PRT1';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
          EXECUTE IMMEDIATE('ALTER TABLE dwh_performance.temp_S4S_LOC_EMP_DY_part1
ADD CONSTRAINT PK_TMP_S4S_LC_EMP_DY_PRT1
PRIMARY KEY (EMPLOYEE_ID,
AVAILABILITY_START_DATE,
CYCLE_START_DATE) USING INDEX TABLESPACE PRF_MASTER ENABLE') ;
 COMMIT;   
...
---------------------------------------------------------------
  ...
  BEGIN
    ...
    INTO G_name
    FROM DBA_CONSTRAINTS
    WHERE CONSTRAINT_NAME = 'PK_F_TEMP_S4S_LC_MP_DY_PRT2'
    AND TABLE_NAME        = 'TEMP_S4S_LOC_EMP_DY_PART2';
    
    l_text               := 'alter table dwh_performance.TEMP_S4S_LOC_EMP_DY_PART2 drop constraint PK_F_TEMP_S4S_LC_MP_DY_PRT2';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_performance.TEMP_S4S_LOC_EMP_DY_PART2 drop constraint PK_F_TEMP_S4S_LC_MP_DY_PRT2');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'constraint PK_F_TEMP_S4S_LC_MP_DY_PRT2 does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
...
---------------------------------------------------------------
 ...
 INSERT /*+ APPEND */
         ...
         WITH selext1
                          AS (
                          SELECT /*+ FULL(SC) FULL(FS) PARALLEL(SC,4) PARALLEL(FS,86)  */
    ...
    l_text := 'ALTER TABLE dwh_performance.temp_S4S_LOC_EMP_DY_part2 ADD CONSTRAINT PK_F_TEMP_S4S_LC_MP_DY_PRT2';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
          EXECUTE IMMEDIATE('ALTER TABLE dwh_performance.temp_S4S_LOC_EMP_DY_part2
ADD CONSTRAINT PK_F_TEMP_S4S_LC_MP_DY_PRT2
PRIMARY KEY (EMPLOYEE_ID, THIS_WEEK_START_DATE, CYCLE_START_DATE, AVAILABILITY_START_DATE) USING INDEX TABLESPACE PRF_MASTER ENABLE') ;
 ...
 l_text := 'create INDEX DWH_PERFORMANCE.I10_TMP_S4S_LC_EMP_DY_PRT2';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I10_TMP_S4S_LC_EMP_DY_PRT2 ON DWH_PERFORMANCE.temp_S4S_LOC_EMP_DY_part2 (EMPLOYEE_ID, CYCLE_START_DATE, WEEK_NUMBER)     
...
WITH selext1 AS
  (SELECT /*+ FULL(a)  PARALLEL(a,4)  */ DISTINCT EMPLOYEE_ID,
    ORIG_CYCLE_START_DATE,
    AVAILABILITY_START_DATE,
    AVAILABILITY_END_DATE ,
    CYCLE_START_DATE,
  ...
  selext2 AS
  (SELECT /*+ FULL(tmp)  PARALLEL(tmp,4)  FULL(se1)  PARALLEL(se1,4)*/ DISTINCT tmp.EMPLOYEE_ID ,

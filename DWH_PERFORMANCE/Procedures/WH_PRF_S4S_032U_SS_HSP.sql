--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_032U_SS_HSP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_032U_SS_HSP" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
...
--
--  Tables:      Input    - dwh_foundation.FND_S4S_EMP_CONSTR_WK_SS_2019
--               Output   - RTL_EMP_CONSTR_LOC_JOB_WK_SS
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_032U_SS';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_EMP_CONSTR_LOC_JOB_WK_SS data  EX FOUNDATION';
...
BEGIN
  ...
  BEGIN
    ...
    WHERE CONSTRAINT_NAME = 'PK_P_RTL_EMPCONSTLCJB_WK_SS'
    AND TABLE_NAME        = 'RTL_EMP_CONSTR_LOC_JOB_WK_SS';

    l_text               := 'alter table dwh_performance.RTL_EMP_CONSTR_LOC_JOB_WK_SS drop constraint PK_P_RTL_EMPCONSTLCJB_WK_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_performance.RTL_EMP_CONSTR_LOC_JOB_WK_SS drop constraint PK_P_RTL_EMPCONSTLCJB_WK_SS');
    COMMIT;

  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'constraint PK_P_RTL_EMPCONSTLCJB_WK_SS does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
     l_text               := 'done';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
  ...
  BEGIN
    ...
    INTO G_name
    FROM DBA_indexes
    WHERE index_NAME = 'I30_RTL_EMPCONSTLCJB_WK_SS'
    AND TABLE_NAME   = 'RTL_EMP_CONSTR_LOC_JOB_WK_SS';

    l_text               := 'drop INDEX DWH_PERFORMANCE.I30_RTL_EMPCONSTLCJB_WK_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('drop INDEX DWH_PERFORMANCE.I30_RTL_EMPCONSTLCJB_WK_SS');
    COMMIT;

  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'index I30_RTL_EMPCONSTLCJB_WK_SS does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
      g_name := null;
  BEGIN
    ...
    INTO G_name
    FROM DBA_indexes
    WHERE index_NAME = 'I40_RTL_EMPCONSTLCJB_WK_SS'
    AND TABLE_NAME   = 'RTL_EMP_CONSTR_LOC_JOB_WK_SS';

    l_text               := 'drop INDEX DWH_PERFORMANCE.I40_RTL_EMPCONSTLCJB_WK_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('drop INDEX DWH_PERFORMANCE.I40_RTL_EMPCONSTLCJB_WK_SS');
    COMMIT;

  EXCEPTION
  WHEN no_data_found THEN
        l_text := 'index I40_RTL_EMPCONSTLCJB_WK_SS does not exist';
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
...
BEGIN
   ...
   l_text := 'Insert into RTL_EMP_CONSTR_LOC_JOB_WK_SS';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

            INSERT
            /*+ append */
          INTO dwh_performance.RTL_EMP_CONSTR_LOC_JOB_WK_SS
                   WITH SELEXT1 AS
                          (SELECT
                          ...
                          FROM FND_S4S_EMP_CONSTR_WK_SS_2019 flr
                          ...
                          JOIN DIM_CALENDAR DC
                              ...
                              OR DC.CALENDAR_DATE BETWEEN FLR.CONSTRAINT_START_DATE AND NVL(FLR.CONSTRAINT_END_DATE- 1, g_constr_end_date)
                                --                ON DC.THIS_WEEK_START_DATE BETWEEN FLR.CONSTRAINT_START_DATE  AND  NVL(FLR.CONSTRAINT_END_DATE- 1, '28/JUL/2014')
                          JOIN RTL_EMP_JOB_WK_SS EJ
                ...
                SELECT DISTINCT SE2.SK1_EMPLOYEE_ID ,SE2.SK1_JOB_ID ,SE2.SK1_LOCATION_NO ,
                 ...
                 where SE2.THIS_WEEK_START_DATE between DERIVE_START_DATE and DERIVE_END_DATE
                   ...
                   and se2.sk1_employee_id <> 1104377
        ...
        L_TEXT := 'RTL_EMP_CONSTR_LOC_JOB_WK_SS : recs = '||g_recs;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
BEGIN
      l_text          := 'Running GATHER_TABLE_STATS ON RTL_EMP_CONSTR_LOC_JOB_WK_SS';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_CONSTR_LOC_JOB_WK_SS', DEGREE => 8);


      l_text := 'create INDEX DWH_PERFORMANCE.I30_RTL_EMPCONSTLCJB_WK_SS';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I30_RTL_EMPCONSTLCJB_WK_SS ON DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_SS (LAST_UPDATED_DATE)     
      TABLESPACE PRF_MASTER NOLOGGING  PARALLEL');
      Execute Immediate('ALTER INDEX DWH_PERFORMANCE.I30_RTL_EMPCONSTLCJB_WK_SS LOGGING NOPARALLEL') ;

      l_text := 'create INDEX DWH_PERFORMANCE.I40_RTL_EMPCONSTLCJB_WK_SS';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I40_RTL_EMPCONSTLCJB_WK_SS ON DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_SS (SK1_EMPLOYEE_ID, CONSTRAINT_START_DATE)     
      TABLESPACE PRF_MASTER NOLOGGING  PARALLEL');
      Execute Immediate('ALTER INDEX DWH_PERFORMANCE.I40_RTL_EMPCONSTLCJB_WK_SS LOGGING NOPARALLEL') ;
--** nb. check logging **
...
BEGIN
  ...
  SELECT MAX(run_seq_no) INTO g_run_seq_no
      FROM dwh_performance.PRF_S4S_CONSTREMPLCJB_DLLST_SS
  ...
  BEGIN
    l_text := 'Insert into  DWH_PERFORMANCE.PRF_S4S_CONSTREMPLCJB_DLLST_SS recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

    INSERT
    INTO dwh_performance.PRF_S4S_CONSTREMPLCJB_DLLST_SS
    -- overlapping tran_date
              ...
              SELECT DISTINCT sk1_employee_id
                  FROM dwh_hr_performance.dim_employee de,
                    (SELECT employee_id, COUNT(*)
                            FROM dwh_foundation.FND_S4S_EMP_CONSTR_WK_SS_2019
        ...
        g_recs_inserted := g_recs;
        l_text          := 'Insert into PRF_S4S_CONSTREMPLCJB_DLLST_SS recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

      EXCEPTION
      WHEN no_data_found THEN
        l_text := 'No deletions done for DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_SS ';
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      END;
    ...
    --**************************************************************************************************
    BEGIN
      l_text := 'Starting delete from RTL_EMP_CONSTR_LOC_JOB_WK_SS recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      DELETE
      FROM dwh_performance.RTL_EMP_CONSTR_LOC_JOB_WK_SS B
      WHERE EXISTS
              (SELECT *
              FROM dwh_performance.PRF_S4S_CONSTREMPLCJB_DLLST_SS a
      ...
      l_text         := 'Deleted from DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_SS recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
...
BEGIN
      ...
      l_text := 'alter table dwh_performance.RTL_EMP_CONSTR_LOC_JOB_WK_SS add constraint PK_P_RTL_EMPCONSTLCJB_WK_SS';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('alter table dwh_performance.RTL_EMP_CONSTR_LOC_JOB_WK_SS add CONSTRAINT PK_P_RTL_EMPCONSTLCJB_WK_SS PRIMARY KEY (SK1_EMPLOYEE_ID,SK1_JOB_ID,SK1_LOCATION_NO,FIN_YEAR_NO,FIN_WEEK_NO)                    
...
BEGIN
  ...
  l_text := 'Running GATHER_TABLE_STATS ON FND_S4S_EMP_CONSTR_WK_SS_2019';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'FND_S4S_EMP_CONSTR_WK_SS_2019', DEGREE => 8);

  l_text := 'truncate table DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_SS';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--  d_delete_prf; not running as doing full load each day

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_168U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_168U" (p_forall_limit in integer,p_success out boolean) as
...
--   g_date := '18/JAN/19';
   ...
   MERGE /*+ PARALLEL(8) */ INTO DWH_PERFORMANCE.rtl_jdaff_st_plan_analysis_dy a
   USING (
      SELECT /*+ parallel(8) full(di) full(dl) */
               di.sk1_item_no,
               dl.sk1_location_no,
               jdaff.trading_date,
               jdaff.POST_DATE,
   ...
   -- QC 37776 add additional column to convert constr_store_cover_day into DDHHMM format
      ...
      where jdaff.item_no            = di.item_no
        ...
        and jdaff.post_date          = g_start_date) jff
      ON (jff.sk1_item_no = a.sk1_item_no 
         AND jff.sk1_location_no = a.sk1_location_no 
         AND jff.trading_date = a.trading_date 
         AND jff.POST_DATE = a.POST_DATE )
   WHEN NOT MATCHED THEN
       INSERT (sk1_item_no,
               ...
               constr_store_cover)
       VALUES (jff.sk1_item_no,
               jff.sk1_location_no,
               jff.trading_date,
               jff.POST_DATE,
               jff.TOTAL_DEMAND_UNIT,
               jff.INVENTORY_UNIT,
               jff.PLANNED_ARRIVALS_UNIT,
               jff.PLANNED_ARRIVALS_CASE,
               jff.REC_ARRIVAL_UNIT,
               jff.REC_ARRIVAL_CASE,
               jff.IN_TRANSIT_UNIT,
               jff.IN_TRANSIT_CASE,
               jff.CONSTRAINT_POH_UNIT,
               jff.SAFETY_STOCK_UNIT,
               jff.constrained_expired_stock,
               jff.constr_store_cover_day ,
               jff.last_updated_date,
               jff.alt_constraint_unused_soh_unit, 
               jff.alt_constraint_poh_unit,	     
               jff.constraint_unmet_demand_unit,   
               jff.constraint_unused_soh_unit,    
               jff.expired_soh_unit,               
               jff.ignored_demand_unit,            
               jff.projected_stock_available_unit,
               jff.constr_store_cover)
   ;
...
/*    Stats still running after 2 hours ..code commented out as discussed (SS)  
-- Stats removes as its was taking to long (SS,BK)

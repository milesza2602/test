--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACT4
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACT4" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACT4';
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_count := dwh_generic_file_extract('select /*+ (parallel a,2) */ dl.location_no, di.item_no, a.fin_year_no, a.fin_week_no, a.reg_sales_qty
from rtl_loc_item_wk_rms_dense a, dim_item di, dim_location dl
where a.fin_year_no = 2014
and   a.fin_week_no between 14 and 26
and   a.sk1_location_no = dl.sk1_location_no
and   a.sk1_item_no     = di.sk1_item_no
and   di.business_unit_no not in (50,70)','|','DWH_FILES_OUT','aj_dense14_26');

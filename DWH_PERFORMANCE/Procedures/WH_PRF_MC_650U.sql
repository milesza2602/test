--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_650U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_650U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        September 2008
...
g_count              number        :=  0;
g_rec_out            rtl_mc_inv_adj_summary%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_650U';
...
type tbl_array_i is table of rtl_mc_inv_adj_summary%rowtype index by binary_integer;
type tbl_array_u is table of rtl_mc_inv_adj_summary%rowtype index by binary_integer;
...
begin
   ...
   if g_rec_in.chain_no = 20 then
      ...
      g_rec_out.inv_adj_fr_cost                := 
      nvl(g_rec_out.inv_adj_cost,0) + round((nvl(g_rec_out.inv_adj_cost,0) * g_debtors_commission_perc / 100),2);

      g_rec_out.inv_adj_fr_cost_local                := 
      nvl(g_rec_out.inv_adj_cost_local,0) + round((nvl(g_rec_out.inv_adj_cost_local,0) * g_debtors_commission_perc / 100),2);
   end if;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_mc_inv_adj_summary values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    inv_adj_qty                     = a_tbl_update(i).inv_adj_qty,
              ...
              inv_adj_fr_cost_local           = a_tbl_update(i).inv_adj_fr_cost_local ,
...
begin
    ...
    l_text := 'LOAD OF rtl_mc_inv_adj_summary EX FOUNDATION STARTED '||

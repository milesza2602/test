--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_008U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_008U_OLD" (
...
AS
  ...
  --
  -- DELETED
  /*  DELETE FROM dwh_performance.RTL_EMP_AVAIL_LOC_JOB_DY
  ...
  */
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_S4S_008U';
  ...
  'LOAD THE RTL_EMP_AVAIL_LOC_JOB_WK data  EX FOUNDATION';
...
BEGIN
  ...
  ;
  l_text := 'LOAD OF RTL_EMP_AVAIL_LOC_JOB_WK  EX FOUNDATION STARTED '||
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text)
  ;
  DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_AVAIL_LOC_JOB_WK',
  DEGREE => 8);
  --***************************************************************************
  -- ***********************
  ...
  INSERT
    /*+ APPEND */
  INTO
    DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK
WITH
  seldat AS
  (
    SELECT

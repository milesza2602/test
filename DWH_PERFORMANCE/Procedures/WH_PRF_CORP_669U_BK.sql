--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_669U_BK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_669U_BK" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        May 2009
--  Author:      M Munnik
--  Purpose:     Rollup from rtl_po_supchain_loc_item_dy to rtl_supchain_loc_item_dy.
--               Rollup excluding PO and PO-specific fields.
--  Tables:      Input  - rtl_po_supchain_loc_item_dy
--               Output - rtl_supchain_loc_item_dy
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  13 June 2009 - defect 1766 - Add fields ..._LOCAL/IMPORT... to tables
--                     RTL_SUPCHAIN_LOC_ITEM_DY from RTL_PO_SUPCHAIN_LOC_ITEM_DY
--
...
g_rec_out            rtl_supchain_loc_item_dy%rowtype;
g_found              boolean;
g_date               date;
g_count              number        :=  0;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_669U_BK';
...
type tbl_array_i is table of rtl_supchain_loc_item_dy%rowtype index by binary_integer;
type tbl_array_u is table of rtl_supchain_loc_item_dy%rowtype index by binary_integer;
...
cursor c_rtl_supchain_loc_item_dy is
  with chgrecs as (select   tran_date, sk1_supply_chain_no, sk1_location_no, sk1_item_no, sk1_supplier_no
                    from     w7104429.rtl_po_supchain_loc_item_dy
                    where    last_updated_date >= g_date
                    group by tran_date, sk1_supply_chain_no, sk1_location_no, sk1_item_no, sk1_supplier_no
                    union
                    select   tran_date, sk1_supply_chain_no, sk1_location_no, sk1_item_no, sk1_supplier_no
...
--qc4148            sum(lid.case_cost) case_cost
   ...
   lid.sk1_supplier_no, (case when 1 = 2 then 0 end), g_date;

-- Input record declared as cursor%rowtype
g_rec_in             c_rtl_supchain_loc_item_dy%rowtype;

-- Input bulk collect table declared
type stg_array is table of c_rtl_supchain_loc_item_dy%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into w7104429.rtl_supchain_loc_item_dy values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update w7104429.rtl_supchain_loc_item_dy
...
-- Check to see if record is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   w7104429.rtl_supchain_loc_item_dy
...
-- Records on rtl_supchain_loc_item_dy are a summary of rtl_po_supchain_loc_item_dy per
...
-- Records on rtl_supchain_loc_item_dy which belongs to the
-- tran_date, sk1_supply_chain_no, sk1_location_no, sk1_item_no, sk1_supplier_no
-- combination of any of the records which have been deleted from rtl_po_supchain_loc_item_dy, have to be re-summarised,
...
-- Therefore, the summarised records are first deleted from rtl_supchain_loc_item_dy.
--**************************************************************************************************
procedure delete_recs as
begin

    delete from w7104429.rtl_supchain_loc_item_dy r
    where  exists(select d.tran_date, d.sk1_supply_chain_no, d.sk1_location_no, d.sk1_item_no, d.sk1_supplier_no
...
begin
    ...
    l_text := 'ROLLUP OF rtl_supchain_loc_item_dy EX PO SUPCHAIN LEVEL STARTED '||
...
--**************************************************************************************************
    ...
    open c_rtl_supchain_loc_item_dy;
    fetch c_rtl_supchain_loc_item_dy bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_rtl_supchain_loc_item_dy;
--**************************************************************************************************

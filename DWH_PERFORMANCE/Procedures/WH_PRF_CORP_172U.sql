--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_172U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_172U" (p_forall_limit in integer,p_success out boolean,p_run_no in integer) as
...
cursor c_run_2 is
   ...
   ( select distinct lid.location_no,lid.item_no,post_date
     ...
     where  lid.last_updated_date = g_date  and
            ...
            business_unit_no      = 50)
   union
   ( select /*+ parallel (lid,4) */ distinct lid.location_no,lid.item_no,post_date
   ...
   select /*+ cardinality (LIL,2000000)  parallel (lil,4) parallel (FLI,4) parallel (di,4) full(di) parallel (fi_vr,4) full(DL) full(DLH) full(FI_VR) full(DIH)*/
          dl.sk1_location_no,
          di.sk1_item_no,
          ...
          dih.sk2_item_no,
          fli.reg_rsp,
          (fli.reg_rsp * 100 / (100 + nvl(fi_vr.vat_rate_perc, di.vat_rate_perc))) reg_rsp_excl_vat,      --VAT rate change
          fli.selling_uom_code,
          fli.prom_rsp,
          fli.clearance_ind,
          nvl(fli.num_units_per_tray,1) as num_units_per_tray,
          nvl(fi_vr.vat_rate_perc, di.vat_rate_perc) as vat_rate_perc,
          di.standard_uom_code,
          nvl(di.static_mass,1) as static_mass,
          nvl(di.random_mass_ind,0) as random_mass_ind

   from   loc_item_list lil
   join   fnd_location_item fli on lil.location_no = fli.location_no
                               and lil.item_no     = fli.item_no
   join   dim_location dl       on lil.location_no = dl.location_no
   join   dim_item di           on lil.item_no     = di.item_no
   join   dim_location_hist dlh on lil.location_no = dlh.location_no
                               and lil.post_date between dlh.sk2_active_from_date and dlh.sk2_active_to_date
   join   dim_item_hist dih on lil.item_no         = dih.item_no
                           and lil.post_date between dih.sk2_active_from_date and dih.sk2_active_to_date 
                           
   left outer join FND_ITEM_VAT_RATE fi_vr on lil.item_no       = fi_vr.item_no                                        -- VAT rate change
                                          and dl.vat_region_no  = fi_vr.vat_region_no                                  -- VAT rate change
                                          and lil.post_date between fi_vr.active_from_date and fi_vr.active_to_date;   -- VAT rate change                       
...
cursor c_fnd_zone_item_store is
   ...
   select dl.sk1_location_no,
          ...
          dih.sk2_item_no,
          fli.reg_rsp,
          (fli.reg_rsp * 100 / (100 + di.vat_rate_perc)) reg_rsp_excl_vat,                  --VAT rate change
          fli.selling_uom_code,
          fli.prom_rsp,
          fli.clearance_ind,
          nvl(fli.num_units_per_tray,1) as num_units_per_tray,

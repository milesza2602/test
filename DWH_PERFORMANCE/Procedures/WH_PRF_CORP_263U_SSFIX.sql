--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_263U_SSFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_263U_SSFIX" (p_forall_limit in integer,p_success out boolean) as
...
MERGE  INTO DWH_PERFORMANCE.MART_SUPPLIER_MASTER_DATA_TEMP rtl_mart USING
(
with cal as (
    ...
    --  where calendar_date in to_date('2015-08-17','yyyy-mm-dd')),
    --  where calendar_date between to_date('2015-09-28','yyyy-mm-dd') and to_date('2015-10-04','yyyy-mm-dd')),
     ...
     item_list as (select a.sk1_item_no,a.item_no,a.item_desc,a.department_no, a.subclass_no,
                   ...
                   from dim_item a, dim_department b, fnd_jdaff_dept_rollout c 
                    where a.business_unit_no = 50 
                      ...
                      and a.sk1_item_no in (select distinct sk1_item_no
                                             from rtl_zone_item_supp_hist  
                                             where calendar_date = g_date)
                   ),
     
     zone_list as (select SK1_ZONE_GROUP_ZONE_NO,zone_no
                    from dim_zone 
                     where zone_no in(select distinct zone_no from fnd_zone_item_supp)
                   ),

     loc_list as ( select distinct fnd_supp.zone_no,dz.sk1_zone_group_zone_no,dl.sk1_location_no,
     ...
     supp_list as (select ds.sk1_supplier_no, ds.supplier_no, ds.supplier_long_desc 
                   from dim_supplier ds 
                   where sk1_supplier_no in (select distinct sk1_supplier_no
                                             from rtl_zone_item_supp_hist  
                                             where calendar_date = g_date)
                   ), 
     ...
     zone_item_supp_hist as (select /*+ PARALLEL(rz,4) full(rz) */ unique rz.sk1_item_no, rz.sk1_supplier_no, rz.sk1_zone_group_zone_no,
                             ...
                             from rtl_zone_item_supp_hist rz, --loc_list, 
                                  item_list, supp_list, dim_zone dz
                            ...
                            --    and rz.calendar_date in to_date('2015-08-17','yyyy-mm-dd') 
                            --    and rz.calendar_date between to_date('2015-09-28','yyyy-mm-dd') and to_date('2015-10-04','yyyy-mm-dd')
                                )
     ,
 
     st_ord_first_date as (select /*+ PARALLEL(a,4) FULL(a) */ a.sk1_item_no, a.sk1_location_no,
                             max(a.post_date) post_date
                           from RTL_LOC_ITEM_DY_ST_ORD a, item_list,  cal, loc_list
                            where a.sk1_item_no     = item_list.sk1_item_no
                              and a.sk1_location_no = loc_list.sk1_location_no
                              and a.post_date = g_date
                            --  and a.post_date in to_date('2015-08-17','yyyy-mm-dd') 
                            --  and a.post_date between to_date('2015-09-28','yyyy-mm-dd') and to_date('2015-10-04','yyyy-mm-dd')
...
,
    ...
    loc_item as (select /*+ PARALLEL(rli,4) */ unique rli.sk1_location_no,
                   ...
                   from rtl_location_item rli,  zone_item_supp_hist ,loc_list, item_list
  ...
  ,                      
    deliv_pat as (select /*+ PARALLEL(fzis,4)  */ unique 
...
--     join loc_list ll3 on  ll3.location_no                 =  zone_item_supp_hist.to_loc_no 
--                       and ll3.sk1_zone_group_zone_no     = zone_item_supp_hist.sk1_zone_group_zone_no
     ...
     join loc_list ll_2 on ll_2.location_no                 = zone_item_om.from_loc_no
...
SET       item_no                     = mer_mart.item_no,
      ...
      --    last_updated_date           = '18/AUG/15'
when not matched then
INSERT    /*+ APPEND parallel (rtl_mart,4) full(rtl_mart)  */

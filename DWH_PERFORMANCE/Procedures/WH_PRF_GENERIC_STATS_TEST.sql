--------------------------------------------------------
--  DDL for Procedure WH_PRF_GENERIC_STATS_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_GENERIC_STATS_TEST" (p_success out boolean, p_schema_name in varchar2, p_table_name in varchar2, 
...
cursor dy_cur is
  ...
  (select   s.subpartition_name, p.partition_position, s.subpartition_position, 
          --  to_char(to_date((substr(s.subpartition_name, length(s.subpartition_name) - 5, 6)),'ddmmyy'),'DD-MON-YY') pdate,
to_date((substr(s.subpartition_name, length(s.subpartition_name) - 5, 6)),'fmddmmyy') pdate
   from     dba_tab_partitions p join dba_tab_subpartitions s
   on       p.table_owner        = s.table_owner
   and      p.table_name         = s.table_name
   and      p.partition_name     = s.partition_name
   where    p.table_owner        = g_schema_name
   and      p.table_name         = g_table_name)
   where    pdate between to_date(g_start_date,'ddmmyy') and to_date(g_end_date,'ddmmyy')

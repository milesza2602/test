--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_105U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_105U_NEW" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_105U_NEW'|| p_from_loc_no;
...
cursor c_fnd_rtl_loc_item_dy_rms_stk is
   select 
          ...
          dih.sk2_item_no,
          dlh.sk2_location_no
   from   fnd_rtl_loc_item_dy_rms_stk_ch fnd_lid,
          dim_item di,
          dim_location dl,
          dim_item_hist dih,
...
begin
    ...
    forall i in a_tbl_merge.first .. a_tbl_merge.last
       save exceptions
       merge /*+ USE_NL(rtl_lid ,mer_lid)*/ into rtl_loc_item_dy_rms_stock rtl_lid
       using

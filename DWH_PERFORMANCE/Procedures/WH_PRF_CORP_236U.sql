--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_236U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_236U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_236U';
...
if l_day_no = 1 then
   if l_fin_week_no = 1 then
      ...
      if l_max_fin_week_last_year = 53 then                  ---- ### STARTED HERE ####
         ...
         --l_text := '## SETTING l_ly_fin_week_no = 1 ##';   --QST
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

      else
         --l_last_wk_fin_year_no := l_last_wk_fin_year_no - 1;   -- First day of new year, get fin year - 1
         --l_last_fin_year := l_last_fin_year - 2;               -- first day of new year, previous fin year will be - 2
         l_last_fin_year := l_last_fin_year - 1;               -- first day of new year, previous fin year will be - 2
         ...
         l_ly_fin_week_no :=  l_last_week_no;                    --QST wk53   ---this was omitted..
      ...
      l_cra_start_date := l_last_week_start_date;
      l_cra_end_date   := l_last_week_end_date;
--l_text := 'l_last_week_start_date = '||l_last_week_start_date ;
--dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--l_text := 'l_last_week_end_date = '||l_last_week_end_date ;
--dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
else

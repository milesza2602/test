--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_114U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_114U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_114U';
...
--          sum(case when fnd_li.tax_perc is null then                                                        -- REF03Mar2018
...
--                nvl(shp.sdn_qty,0) * (shp.reg_rsp * 100 / (100 + fnd_li.tax_perc))                          -- REF03Mar2018
                 nvl(shp.sdn_qty,0) * (shp.reg_rsp * 100 / (100 + vr.VAT_RATE_PERC))                          -- REF03Mar2018 
...
begin
    ...
    l_text := 'LOAD OF rtl_loc_item_dy_rms_sparse EX FOUNDATION STARTED AT '||

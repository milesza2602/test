--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_118U_FIX201508
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_118U_FIX201508" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - rtl_loc_item_dy_rms_alloc_fix
...
g_count              number        :=  0;
g_rec_out            rtl_loc_item_dy_rms_alloc_fix%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_118U_FIX201508';
...
type tbl_array_i is table of rtl_loc_item_dy_rms_alloc_fix%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_item_dy_rms_alloc_fix%rowtype index by binary_integer;
...
cursor c_fnd_rtl_allocation is
   select sum(nvl(aloc.alloc_qty,0)) alloc_qty,
...
--          max(nvl(fnd_li.reg_rsp,0) * 100 / (100 + di.vat_rate_perc)) reg_rsp_excl_vat,
   ...
   where  aloc.item_no                = di.item_no          and
          ...
          aloc.into_loc_date         between '25 JULY 2014' AND '12 AUGUST 2014' and
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_item_dy_rms_alloc_fix values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_loc_item_dy_rms_alloc_fix
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_loc_item_dy_rms_alloc_fix
...
begin
    ...
    l_text := 'LOAD rtl_loc_item_dy_rms_alloc_fix EX FOUNDATION STARTED '||
...
END WH_PRF_CORP_118U_FIX201508;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_015U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_015U" (
...
--               into an table which will be used to report on data in RTL_EMP_JOB_FY
--                that does not exist in RTL_EMP_LOC_STATUS_DY
--               A view will be used to create a summary report
--               viz. VX_S4S_AUDIT_ELS_VS_EJ
--
--  Tables:      Input    - dwh_foundation.FND_S4S_EMP_JOB
--               Output   - DWH_PERFORMANCE.RTL_EMP_JOB_DY_EXCPT_XRPT
...
--  c_  -  Prefix to cursor
  ...
  g_count         NUMBER  := 0;
  g_rec_out RTL_EMP_JOB_DY_EXCPT_XRPT%rowtype;
  g_found                BOOLEAN;
  g_date                 DATE;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_S4S_015U';
  l_name sys_dwh_log.log_name%type                     := dwh_constants.vc_log_name_rtl_md;
  l_system_name sys_dwh_log.log_system_name%type       := dwh_constants.vc_log_system_name_rtl_prf;
  l_script_name sys_dwh_log.log_script_name%type       := dwh_constants.vc_log_script_rtl_prf_md;
  l_procedure_name sys_dwh_log.log_procedure_name%type := l_module_name;
  l_text sys_dwh_log.log_text%type ;
  l_description sys_dwh_log_summary.log_description%type   := 'LOAD THE RTL_EMP_JOB_DY_EXCPT_XRPT data  EX FOUNDATION';
...
BEGIN
  ...
  l_text := 'LOAD OF RTL_EMP_JOB_DY_EXCPT_XRPT  EX FOUNDATION STARTED '|| TO_CHAR(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
INSERT /*+ APPEND */
INTO DWH_PERFORMANCE.RTL_EMP_JOB_DY_EXCPT_XRPT
WITH
SELEXT1 AS (
...
l_text := 'Running GATHER_TABLE_STATS ON RTL_EMP_JOB_DY_EXCPT_XRPT';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_JOB_DY_EXCPT_XRPT', DEGREE => 8);

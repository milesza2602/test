--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110TEST" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_110TESTU_'|| p_from_loc_no;
...
cursor c_fnd_rtl_loc_item_dy_rms_sale is
 select /*+ full(di) full(dl) full(dlh) full(dih) parallel(fnd_li,6)  */  
          post_date,
          sales_qty,
          sales,
          sales_cost,
   ...
   where  fnd_lid.post_date = g_date 
          ...
          and dl.location_no = 105
          and di.item_no = 6009204998384;
...
begin
   ...
   l_text := '- POST_DATE = ' || g_rec_out.POST_DATE;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   l_text := '- SK1_LOCATION_NO = ' || g_rec_out.SK1_LOCATION_NO;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   l_text := '- SK1_ITEM_NO = ' || g_rec_out.SK1_ITEM_NO;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
-- Case quantities can not contain fractions, the case quantity has to be an integer value (ie. 976.0).
   if g_rec_in.business_unit_no = 50 then
      l_text := '- SALES_CASES CALC = round((nvl(g_rec_out.sales_qty,0)/g_rec_in.num_units_per_tray),0)';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_text := '- G_REC_OUT.SALES_QTY = ' || nvl(g_rec_out.sales_qty,0);
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_text := '- G_REC_IN.UNITS_PER_TRAY = ' || g_rec_in.num_units_per_tray;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      
      g_rec_out.sales_cases  := round((nvl(g_rec_out.sales_qty,0)/g_rec_in.num_units_per_tray),0);
      
      l_text := '- SALES_CASES = ' || g_rec_out.sales_cases;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_527U_BK10122014
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_527U_BK10122014" (p_forall_limit in integer,p_success out boolean) as
...
select  /*+ PARALLEL(4) */         
  ...
  FROM  
    dwh_foundation.fnd_rtl_inventory_adj lid, intentory inv --  GROUP BY lid.post_date,lid.location_no
    where lid.reason_code in (0,90,111)
    AND inv.location_no=lid.location_no 
    and lid.item_no=inv.item_no 
    and  lid.post_date =  inv.post_date
    AND      lid.post_date  BETWEEN g_start_date AND g_end_date
GROUP BY lid.post_date, inv.sk1_location_no, inv.sk1_business_unit_no
;
...
select  /*+ PARALLEL(4) */         
  ...
  FROM  
       ...
       AND lid.post_date =  inv.post_date
       AND lid.post_date  BETWEEN g_start_date AND g_end_date
       AND lid.inv_adj_qty    is not null --rtl_liwrd.SHRINKAGE_QTY             is not null --
       AND not exists
              (SELECT /*+ nl_aj */  * FROM dwh_performance.rtl_loc_bu_dy_shrinkage
               WHERE   sk1_location_no       = inv.sk1_location_no       AND
                       sk1_business_unit_no  = inv.sk1_business_unit_no  AND
...
END WH_PRF_CORP_527U_BK10122014;

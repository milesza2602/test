--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_536U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_536U_LOAD" (
...
as
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_536U_LOAD';
...
--  commit;
  ...
  --Gather Stats On TEMP_LOC_ITEM_WK_CATALOG
  --**************************************************************************************************
  l_text := 'Gather stats START on  DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  
  dbms_stats.gather_table_stats ('DWH_PERFORMANCE', 'TEMP_LOC_ITEM_WK_CATALOG', degree => 32, cascade => true );
  commit;
  
  l_text := 'Gather stats END on  DWH_PERFORMANCE.TEMP_LOC_ITEM_WK_CATALOG';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

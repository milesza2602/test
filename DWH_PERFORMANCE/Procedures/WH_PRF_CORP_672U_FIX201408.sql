--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_672U_FIX201408
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_672U_FIX201408" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_672U_FIX201408';
...
l_description        sys_dwh_log_summary.log_description%type  := 'ROLLS STOCK TO PROM/LOC/ITEM/DY';
...
cursor c_prom_loc_item_dy is
   select   st.post_date, st.sk1_location_no, st.sk1_item_no, ia.sk1_prom_no,
            ...
            sum(st.sit_qty) sit_qty, sum(st.sit_selling) sit_selling, sum(st.sit_cost) sit_cost,
            sum(cat.soh_adj_qty) soh_adj_qty,sum(cat.soh_adj_selling) soh_adj_selling,sum(cat.soh_adj_cost) soh_adj_cost
   from     rtl_loc_item_dy_rms_stock st
  left join rtl_loc_item_dy_catalog cat   on  st.sk1_location_no  = cat.sk1_location_no
                                          and st.sk1_item_no      = cat.sk1_item_no
                                          and st.post_date        = cat.calendar_date
...
--   and      pl.prom_no <>  313801
   group by st.post_date, st.sk1_location_no, st.sk1_item_no, ia.sk1_prom_no
...
begin
    ...
    l_text := 'LOAD RTL_PROM_LOC_ITEM_DY from STOCK STARTED '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
END WH_PRF_CORP_672U_FIX201408;

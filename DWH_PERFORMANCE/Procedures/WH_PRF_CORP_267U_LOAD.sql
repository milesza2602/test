--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_267U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_267U_LOAD" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_267U_LOAD';
...
begin
    ...
    l_text := 'RELOAD OF MART_FOODS_SUPPLIER_PO (PO ONLY) FOR JULY 2017 RECLASS STARTED AT '||
    ...
    --30/JUN/14 00:00:00	25/FEB/18 00:00:00
    
    --DATES BEING LOADED : FROM 30-JUN-14 TO : 27-JUL-14
    --DATES BEING LOADED : FROM 28/JUL/14 TO : 28/DEC/14
    --DATES BEING LOADED : FROM 29/DEC/14 TO : 26/JUN/16
    
    --DATES BEING LOADED : FROM 27/JUN/16 TO : 25/FEB/18

    g_from_date := '27/JUN/16';
    g_to_date   := '25/FEB/18';
...
MERGE /*+ PARALELL(rtl_mart,8) */ INTO DWH_PERFORMANCE.MART_FOODS_SUPPLIER_PO rtl_mart USING
(
with ITEM_LIST AS
  (
  SELECT DI.ITEM_NO,
...
from ITEM_LIST a, LOC_LIST B, RTL_PO_SUPCHAIN_LOC_ITEM_DY C, DIM_CALENDAR D, SUPP_LIST E, PURCH_LIST F ,UDA_LIST H
   ...
   where c.po_ind = 1
      and c.tran_date between g_from_date and g_to_date   --= G_DATE 
...
SET       SK1_ZONE_GROUP_ZONE_NO      = mer_mart.SK1_ZONE_GROUP_ZONE_NO,
          ...
          last_updated_date           = mer_mart.TRAN_DATE, --g_date,

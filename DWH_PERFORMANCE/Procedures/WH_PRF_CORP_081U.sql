--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_081U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_081U" (p_forall_limit in integer,p_success out boolean) as
...
--               with input ex RMS fnd_zone_item_supp table from foundation layer.
--               This is the new feed for Foods Renwal, replaces fnd_zone_item_supp
--  Tables:      Input  - fnd_zone_item_supp  (new master load for this interface as it now as two feeds)
--               Output - rtl_zone_item_supp
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  30 Jun 2016  BK: Add additional col NUM_EXTRA_LEAD_TIME_DAYS from source FND table to output FACT         Ref: BK30JUN16
--  10 Aug 2016  BK: Include TO_LOC_NO as part of key in check for table insert or update                     Ref: BK10Aug16
--               Required for alter  of table rtl_zone_item_supp to add FROM_LOC_NO as part of key (see DML) 
--  22 Aug 2016  Add code to populate 2 new fields added in output table (rtl_zone_item_supp)from source table (fnd_zone_item_supp) -
--               FROM_LOC_NO & SUPPLIER_SHELF_LIFE                                                            Ref: BK22Aug16          
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_081U';
...
cursor c_fnd_zone_item_supp is
   select fis.*,
...
type stg_array is table of c_fnd_zone_item_supp%rowtype;
a_stg_input      stg_array;

g_rec_in         c_fnd_zone_item_supp%rowtype;
...
begin
   ...
   g_rec_out.NUM_EXTRA_LEAD_TIME_DAYS        := g_rec_in.NUM_EXTRA_LEAD_TIME_DAYS;            -- BK30JUN16
   g_rec_out.SUPPLIER_SHELF_LIFE             := g_rec_in.SUPPLIER_SHELF_LIFE;                 -- BK22Aug16
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    --to_loc_no                       = a_tbl_update(i).to_loc_no,                        -- BK30JUN16
              ...
              NUM_EXTRA_LEAD_TIME_DAYS        = a_tbl_update(i).NUM_EXTRA_LEAD_TIME_DAYS,           -- BK30JUN16
              SUPPLIER_SHELF_LIFE             = a_tbl_update(i).SUPPLIER_SHELF_LIFE,                -- BK22Aug16
...
-- BK22Aug16
       ...
       where  sk1_item_no                     = a_tbl_update(i).sk1_item_no  and
              ...
              TO_LOC_NO                       = a_tbl_update(i).TO_LOC_NO;                          -- BK30JUN16
...
-- Check to see if item is present on table and update/insert accordingly
   ...
   where  sk1_item_no            = g_rec_out.sk1_item_no  and
          ...
          TO_LOC_NO              = g_rec_out.TO_LOC_NO;                                           -- Ref: BK10Aug16
   if g_count = 1 then
      g_found := TRUE;
   end if;
...
--**************************************************************************************************
    ...
    open c_fnd_zone_item_supp;

    fetch c_fnd_zone_item_supp bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_008U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_008U_SS" (
...
AS
  ...
  --
  --
  --  Tables:      Input    - RTL_EMP_AVAIL_LOC_JOB_DY_SS
  --               Output   - DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_SS
  --  Packages:    dwh_constants, dwh_log, dwh_valid
  --
  --  Maintenance:
  --
  -- DELETED
  /*  DELETE FROM dwh_performance.RTL_EMP_AVAIL_LOC_JOB_DY_SS
  ...
  */
  ...
  g_count         NUMBER  := 0;
  g_rec_out RTL_EMP_AVAIL_LOC_JOB_WK_SS%rowtype;
  g_found        BOOLEAN;
  g_date         DATE;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_S4S_008U_SS';
  ...
  'LOAD THE RTL_EMP_AVAIL_LOC_JOB_WK_SS data  EX FOUNDATION';
...
BEGIN
  ...
  ;
  l_text := 'LOAD OF RTL_EMP_AVAIL_LOC_JOB_WK_SS  EX FOUNDATION STARTED '||
  ...
  g_recs_inserted :=0;
  g_recs_deleted  := 0;
  l_text          := 'TRUNCATE TABLE DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_SS';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text)
  ;
  EXECUTE IMMEDIATE('TRUNCATE TABLE DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_SS');
  l_text := 'Running GATHER_TABLE_STATS ON RTL_EMP_AVAIL_LOC_JOB_WK_SS';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text)
  ;
  DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_AVAIL_LOC_JOB_WK_SS',
  DEGREE => 8);
  --***************************************************************************  
  ...
  INSERT
    /*+ APPEND */
  INTO
    DWH_PERFORMANCE.RTL_EMP_AVAIL_LOC_JOB_WK_SS
WITH
  seldat AS
  (
    SELECT
      ...
      sk1_location_no
    FROM
      dwh_performance.RTL_EMP_AVAIL_LOC_JOB_DY_SS rtl,
      dim_calendar dc
    WHERE
...
SELEXT2 AS (    SELECT
    ...
    FROM
      SelEXT rtl,
      DWH_PERFORMANCE.RTL_EMP_JOB_WK_SS jd
    WHERE
...
SELCNT AS ( SELECT SK1_EMPLOYEE_ID,SK1_JOB_ID,FIN_YEAR_NO,FIN_WEEK_NO , COUNT(*)
            FROM SELEXT2 
            GROUP BY SK1_EMPLOYEE_ID,SK1_JOB_ID,FIN_YEAR_NO,FIN_WEEK_NO  
            HAVING COUNT(*) > 1),
SELDUP AS (SELECT SK1_EMPLOYEE_ID,SK1_JOB_ID,FIN_YEAR_NO,FIN_WEEK_NO 
            FROM SELEXT2 
            GROUP BY SK1_EMPLOYEE_ID,SK1_JOB_ID,FIN_YEAR_NO,FIN_WEEK_NO 
            MINUS
            SELECT DISTINCT SK1_EMPLOYEE_ID,SK1_JOB_ID,FIN_YEAR_NO,FIN_WEEK_NO 
            FROM SELCNT ) 
SELECT SE.*
FROM SELEXT2 SE,
     SELDUP SD
WHERE SE.SK1_EMPLOYEE_ID = SD.SK1_EMPLOYEE_ID
    AND SE.SK1_JOB_ID    = Sd.SK1_JOB_ID
    AND SE.FIN_YEAR_NO   = SD.FIN_YEAR_NO
    AND SE.FIN_WEEK_NO   = SD.FIN_WEEK_NO
;
...
l_text := 'Running GATHER_TABLE_STATS ON RTL_EMP_AVAIL_LOC_JOB_WK_SS';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_AVAIL_LOC_JOB_WK_SS',DEGREE => 8);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_263U_RECLASS1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_263U_RECLASS1" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_263U_RECLASS1';
...
begin
    ...
    dwh_lookup.dim_control(g_date1);
    l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date1;
...
--**************************************************************************************************
 ...
 FOR g_sub IN 0..1
  LOOP
   ...
   l_text       := 'Data processed is:- '||g_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--    MERGE /*+ parallel(rtl_mart,4) */ INTO DWH_PERFORMANCE.MART_SUPPLIER_MASTER_DATA_HIST rtl_mart USING
    MERGE /*+ parallel(rtl_mart,6) */ INTO DWH_DATAFIX.MART_SUPPLIER_MASTER_DATA_HISX rtl_mart USING
    (
    with cal as (
    ...
    ,
        ...
        loc_item as (select /*+ full(rli) parallel(rli,4) */ rli.sk1_location_no,

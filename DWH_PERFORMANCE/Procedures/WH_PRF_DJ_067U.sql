--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_067U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_067U" (p_forall_limit in integer,p_success out boolean) as
...
--  Purpose:     Create contract dimension table in the performance layer
--               with dim_dj_contract data ex foundation table.
--  Tables:      Input  - dim_contract,
--               Output - dim_dj_contract
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
-- wendy lyttle 19 may 2016
--- contracts are excluded due to the fact they they were changed from ww to dj
--  they should have been cancelled in DJ and created as new contracts in WW
 -- will stay so until manual datafix is done
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            dim_dj_contract%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_067U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE dim_dj_contract EX dim_dj_contract';
...
type tbl_array_i is table of dim_dj_contract%rowtype index by binary_integer;
type tbl_array_u is table of dim_dj_contract%rowtype index by binary_integer;
...
cursor c_dim_dj_contract is
   ...
   and     contract_no not in (494539,494553,494554,499033,499034,499036,499037,502543,502632
,502633,502636,502647,502648,503735,503752,503753,503755,503756,503757,506019,520501,520503)
--- these contract above are excluded due to the fact they they were changed from ww to dj
--  they should have been cancelled in DJ and created as new contracts in WW
-- waiting for manual fix before allowing processing again
 --  and contract_no != 495112
-- LOCAL BULK ERROR - POSITION, CODE and RECORD KEY:- 2 4 ORA-00001: unique constraint (.) violated 520501
--LOCAL BULK ERROR - POSITION, CODE and RECORD KEY:- 1 1 ORA-00001: unique constraint (.) violated 520503
 ;

g_rec_in             c_dim_dj_contract%rowtype;
-- For input bulk collect --
type stg_array is table of c_dim_dj_contract%rowtype;
...
begin
   ...
   g_rec_out.sk1_chain_code_ind              := 0;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_dj_contract values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_dj_contract
      set    contract_no                     = a_tbl_update(i).contract_no,
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).sk1_contract_no = g_rec_out.sk1_contract_no then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
-- Check to see if item is present on table and update/insert accordingly
   ...
   if not g_found then
      insert into dim_dj_contract values g_rec_out;
...
begin
    ...
    l_text := 'LOAD OF dim_dj_contract EX dim_dj_contract STARTED AT '||
...
--**************************************************************************************************
    open c_dim_dj_contract;
    fetch c_dim_dj_contract bulk collect into a_stg_input limit g_forall_limit;

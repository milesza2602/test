--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_021U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_021U_TAKEON" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
...
--
--  Tables:      Input    - dwh_foundation.FND_S4S_HYP_LOC_BU_WK
--               Output   - DWH_PERFORMANCE.RTL_LOC_BU_WK
...
g_count              number        :=  0;
g_rec_out            dwh_performance.RTL_LOC_BU_WK%rowtype;
g_found              boolean;
g_date               date;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_021U_TAKEON';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_LOC_BU_WK data  EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dwh_performance.RTL_LOC_BU_WK%rowtype index by binary_integer;
type tbl_array_u is table of dwh_performance.RTL_LOC_BU_WK%rowtype index by binary_integer;
...
-- SELECT *FROM DIM_HR_EMPLOYEE WHERE SK1_S4S_BUSINESS_UNIT_NO IN (396371,
--404245)
--  S4S_BUSINESS_UNIT_NO IN (1002194, 1002198, 1002609)
 --179463    1002194
--179464    1002198
--179630    1002609
--396371    7072436
...
with selext as (SELECT  /*+ full(flr) parallel(flr,6) */              FLR.LOCATION_NO
                  ...
                  , SK1_LOCATION_NO
           FROM dwh_foundation.FND_S4S_HYP_LOC_BU_WK flr,
                dwh_performance.DIM_LOCATION DL,
                dwh_performance.DIM_S4S_BUSINESS_UNIT DE
          WHERE FLR.LOCATION_NO = DL.LOCATION_NO
          AND FLR.s4s_BUSINESS_UNIT_NO = DE.s4s_BUSINESS_UNIT_NO
          and flr.last_updated_date = g_date
      --    AND FIN_YEAR_NO = 2014
          --AND DL.SK1_LOCATION_NO = 	6922
        --  AND DE.SK1_S4S_BUSINESS_UNIT_NO = 1001330
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into dwh_performance.RTL_LOC_BU_WK  values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_INSERT(g_error_index).SK1_S4S_BUSINESS_UNIT_NO;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
             UPDATE dwh_performance.RTL_LOC_BU_WK
                    SET
                      budget_FTE              = a_tbl_update(i).budget_FTE  ,
                      budget_COSt            = a_tbl_update(i).budget_COSt    ,
                      LAST_UPDATED_DATE       = a_tbl_update(i).LAST_UPDATED_DATE
                    WHERE SK1_S4S_BUSINESS_UNIT_NO     = a_tbl_update(i).SK1_S4S_BUSINESS_UNIT_NO
...
/*   l_text := 'G_COUNT='||g_COUNT||' -G_REC_IN.RTL_EXISTS='||G_REC_IN.RTL_EXISTS
   ||'-'||g_rec_out.SK1_S4S_BUSINESS_UNIT_NO
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD OF RTL_LOC_BU_WK  EX FOUNDATION STARTED '||
...
--**************************************************************************************************
   ...
   local_bulk_insert;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   local_bulk_update;
    l_text := 'Running GATHER_TABLE_STATS ON RTL_LOC_BU_WK';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_LOC_BU_WK', DEGREE => 8);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_738U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_738U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_738U';
...
--          fnd_lid.day2_estimate + fnd_lid.safety_qty     as scanned_model_stock_qty,
   ...
   from   fnd_rtl_loc_item_dy_om_st_ord fnd_lid,
          dim_item di,
          dim_location dl,
          dim_item_hist dih,

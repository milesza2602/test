--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_841U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_FND_CORP_841U_NEW" (p_forall_limit in integer,p_success out boolean)
as
--**************************************************************************************************
--  Date:        April 2010
--  Author:      W Lyttle
...
cursor c_fnd_alloc is
   select   
--/*+ index (b, I13_F_FND_RTL_SHIPMENT) */ 
-- /*+ PARALLEL(a,4)full(a) index (b, I13_F_FND_RTL_SHIPMENT) */
 /*+ PARALLEL(a,4) index (b, I13_F_FND_RTL_SHIPMENT) */
   a.release_date, a.alloc_no,
   a.first_dc_no first_whse_supplier_no,
   a.to_loc_no,
   b.to_loc_no first_dc_no
   from   dwh_foundation.temp_alloc_tracker_dc a, fnd_rtl_shipment b
   where a.alloc_no = b.tsf_alloc_no
   and a.first_dc_no = b.from_loc_no
   group by a.release_date, a.alloc_no,
   a.first_dc_no ,
   a.to_loc_no,
   b.to_loc_no ;
...
begin
   ...
   insert /*+ append */ into dwh_foundation.temp_alloc_tracker_dc
 select 
 ----/*+ full(l)  use_hash (a) */
/*+ parallel(s, 4) */
          trunc(sysdate) release_date, a.alloc_no alloc_no, 0 to_loc_no, s.from_loc_no first_dc_no
   from   fnd_alloc_tracker_alloc a
   join   fnd_rtl_shipment s      on  A.alloc_no    = s.tsf_alloc_no
                                  and A.item_no     = s.item_no
    join dim_location l on l.location_no = s.from_loc_no
   where a.supply_chain_code = 'WH'
   and l.loc_type = 'W'
   and a.release_date between g_start_date and g_date
 --  and a.alloc_no = 108114752
 --  and a.release_date between '1/oct/09' and '4/feb/10'
 group by trunc(sysdate),a.alloc_no, s.from_loc_no
   minus
   select 
  -- --/*+ full(l)  use_hash (a) */
  /*+ parallel(s, 4) */
          trunc(sysdate) release_date, a.alloc_no alloc_no, 0 to_loc_no, s.to_loc_no first_dc_no
   from   fnd_alloc_tracker_alloc a
   join   fnd_rtl_shipment s      on  A.alloc_no    = s.tsf_alloc_no
                                  and A.item_no     = s.item_no
...
--   and a.alloc_no = 108114752
--      and a.release_date between '1/oct/09' and '4/feb/10'
   group by trunc(sysdate),a.alloc_no, s.to_loc_no ;

 --  select /*+ parallel(s, 8) */
 --         a.release_date, a.alloc_no, min(s.to_loc_no) first_dc_no
 --  from   fnd_alloc_tracker_alloc a
 --  join   fnd_rtl_shipment s      on  A.alloc_no    = s.tsf_alloc_no
 --                                 and A.item_no     = s.item_no
...
--   group by a.release_date, a.alloc_no;
   ...
   l_text := 'Records inserted to temp_alloc_tracker_dc = '||g_recs_inserted;
...
--      where  release_date                    = a_tbl_update(i).release_date
--       and    alloc_no                        = a_tbl_update(i).alloc_no
--       and    to_loc_no                       = a_tbl_update(i).to_loc_no
       ;
...
--**************************************************************************************************
   ...
   g_start_date := g_date - 90;
    ...
    execute immediate 'truncate table dwh_foundation.temp_alloc_tracker_dc';
    l_text := 'TABLE dwh_foundation.temp_alloc_tracker_dc TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

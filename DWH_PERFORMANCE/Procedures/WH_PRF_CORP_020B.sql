--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_020B
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_020B" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  -   fnd_area fnd_chain
--               Output -   dim_area
...
g_rec_out            dim_area%rowtype;
g_rec_in             fnd_area%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_020B';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE DIM_AREA EX FND_AREA';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_area%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_area%rowtype index by binary_integer;
type tbl_array_u is table of dim_area%rowtype index by binary_integer;
...
-- g_rec_out.source_data_status_code       := g_rec_in.source_data_status_code;
   ...
   dwh_lookup.dim_chain_hierachy(g_rec_out.chain_no,g_rec_out.chain_name,g_rec_out.sk1_chain_no,
...
---------------------------------------------------------
   ...
   g_rec_out.total_desc                 := 'ALL AREAS';
   exception
      when others then
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_area values a_tbl_insert(i);
...
begin
   ...
   g_found := dwh_valid.dim_area(g_rec_out.area_no);


-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_area_no  := location_hierachy_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF DIM_AREA EX FND_AREA STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_area;
    fetch c_fnd_area bulk collect into a_stg_input limit g_forall_limit;

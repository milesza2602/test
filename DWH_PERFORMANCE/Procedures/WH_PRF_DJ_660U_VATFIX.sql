--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_660U_VATFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_660U_VATFIX" (p_forall_limit in integer,p_success out boolean
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_660U_VATFIX';
...
*/
    g_date := '1 january 2015';  -- vatfix is for all records starting last_updated_date >= '26 april 2015'
...
--**************************************************************************************************
 ...
 with po_suppl_chain as
  (select /*+ parallel(a,4) */ po_no, max(supply_chain_type) supply_chain_type
   ...
   group  by po_no
   union all
   select  /*+ parallel(b,4) */  po_no, null supply_chain_type
   ...
   select  /*+ parallel(po,4) parallel(di,4) full(di) */ po.po_no, dp.sk1_po_no, po.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
   ...
   where  
  -- po.last_updated_date >=  g_date
--   and   
   po.not_before_date between '25 Jun 2007' and g_date + 800   --540+257days  -- taking into account where last_updated_date starts
   -- partitions for rtl_po_supchain_loc_item_dy, up till 18 months into future
   and    
   chain_code = 'DJ'
   group  by po.po_no, dp.sk1_po_no, po.item_no, di.sk1_item_no, ps.supply_chain_type
   union
   select /*+ parallel(s,4) parallel(di,4) full(di) */ s.po_no, dp.sk1_po_no, s.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
 ...
 --  where  last_updated_date >=  g_date
  ...
  l_text := 'end insert';
    ...
    g_date_end := g_date + 800;  --540+257days   -- taking into account where last_updated_date starts;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_630U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_630U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - fnd_mc_rtl_stck_ledger_wk
--               Output - rtl_mc_stock_ledger_wk
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  25 March 2009 - QC 1249 -  load data into rtl_mc_stock_ledger_wk from fnd_mc_rtl_stck_ledger_wk
--                             where the department on fnd_mc_rtl_stck_ledger_wk exists on DIM_ITEM
--
--
--  Naming conventions
--  g_  -  Global variable
...
g_count              number        :=  0;
g_rec_out            rtl_mc_stock_ledger_wk%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_630U';
...
type tbl_array_i is table of rtl_mc_stock_ledger_wk%rowtype index by binary_integer;
type tbl_array_u is table of rtl_mc_stock_ledger_wk%rowtype index by binary_integer;
...
cursor c_fnd_mc_rtl_stck_ledger_wk is
   with dept_list as
   (
   select distinct department_no
   from   dim_item
   )
   select sum(nvl(slw.OPN_STK_RETAIL_OPR,0)) as	OPN_STK_RETAIL	,
          sum(nvl(slw.OPN_STK_COST_OPR,0)) as	OPN_STK_COST	,
          sum(nvl(slw.STOCK_ADJ_RETAIL_OPR,0)) as	STOCK_ADJ_RETAIL	,
          sum(nvl(slw.STOCK_ADJ_COST_OPR,0)) as	STOCK_ADJ_COST	,
          sum(nvl(slw.PURCH_RETAIL_OPR,0)) as	PURCH_RETAIL	,
          sum(nvl(slw.PURCH_COST_OPR,0)) as	PURCH_COST	,
          sum(nvl(slw.RTV_RETAIL_OPR,0)) as	RTV_RETAIL	,
          sum(nvl(slw.RTV_COST_OPR,0)) as	RTV_COST	,
          sum(nvl(slw.TSF_IN_RETAIL_OPR,0)) as	TSF_IN_RETAIL	,
          sum(nvl(slw.TSF_IN_COST_OPR,0)) as	TSF_IN_COST	,
          sum(nvl(slw.TSF_OUT_RETAIL_OPR,0)) as	TSF_OUT_RETAIL	,
          sum(nvl(slw.TSF_OUT_COST_OPR,0)) as	TSF_OUT_COST	,
          sum(nvl(slw.NET_SALES_RETAIL_OPR,0)) as	NET_SALES_RETAIL	,
          sum(nvl(slw.NET_SALES_RETAIL_EXCL_VAT_OPR,0)) as	NET_SALES_RETAIL_EXCL_VAT	,
          sum(nvl(slw.NET_SALES_COST_OPR,0)) as	NET_SALES_COST	,
          sum(nvl(slw.RETURNS_RETAIL_OPR,0)) as	RETURNS_RETAIL	,
          sum(nvl(slw.RETURNS_COST_OPR,0)) as	RETURNS_COST	,
          sum(nvl(slw.MARKUP_RETAIL_OPR,0)) as	MARKUP_RETAIL	,
          sum(nvl(slw.MARKUP_CAN_RETAIL_OPR,0)) as	MARKUP_CAN_RETAIL	,
          sum(nvl(slw.CLEAR_MARKDOWN_RETAIL_OPR,0)) as	CLEAR_MARKDOWN_RETAIL	,
          sum(nvl(slw.PERM_MARKDOWN_RETAIL_OPR,0)) as	PERM_MARKDOWN_RETAIL	,
          sum(nvl(slw.PROM_MARKDOWN_RETAIL_OPR,0)) as	PROM_MARKDOWN_RETAIL	,
          sum(nvl(slw.MARKDOWN_CAN_RETAIL_OPR,0)) as	MARKDOWN_CAN_RETAIL	,
          sum(nvl(slw.SHRINKAGE_RETAIL_OPR,0)) as	SHRINKAGE_RETAIL	,
          sum(nvl(slw.SHRINKAGE_COST_OPR,0)) as	SHRINKAGE_COST	,
          sum(nvl(slw.CLS_STK_RETAIL_OPR,0)) as	CLS_STK_RETAIL	,
          sum(nvl(slw.CLS_STK_COST_OPR,0)) as	CLS_STK_COST	,
          sum(nvl(slw.COST_VARIANCE_AMT_OPR,0)) as	COST_VARIANCE_AMT	,
          sum(nvl(slw.HTD_GAFS_RETAIL_OPR,0)) as	HTD_GAFS_RETAIL	,
          sum(nvl(slw.HTD_GAFS_COST_OPR,0)) as	HTD_GAFS_COST	,
          sum(nvl(slw.STOCKTAKE_ADJ_RETAIL_OPR,0)) as	STOCKTAKE_ADJ_RETAIL	,
          sum(nvl(slw.STOCKTAKE_ADJ_COST_OPR,0)) as	STOCKTAKE_ADJ_COST	,
          sum(nvl(slw.RECLASS_IN_COST_OPR,0)) as	RECLASS_IN_COST	,
          sum(nvl(slw.RECLASS_IN_RETAIL_OPR,0)) as	RECLASS_IN_RETAIL	,
          sum(nvl(slw.RECLASS_OUT_COST_OPR,0)) as	RECLASS_OUT_COST	,
          sum(nvl(slw.RECLASS_OUT_RETAIL_OPR,0)) as	RECLASS_OUT_RETAIL	,
          
          sum(nvl(slw.OPN_STK_RETAIL_LOCAL,0)) as	OPN_STK_RETAIL_LOCAL	,
          sum(nvl(slw.OPN_STK_COST_LOCAL,0)) as	OPN_STK_COST_LOCAL	,
          sum(nvl(slw.STOCK_ADJ_RETAIL_LOCAL,0)) as	STOCK_ADJ_RETAIL_LOCAL	,
          sum(nvl(slw.STOCK_ADJ_COST_LOCAL,0)) as	STOCK_ADJ_COST_LOCAL	,
          sum(nvl(slw.PURCH_RETAIL_LOCAL,0)) as	PURCH_RETAIL_LOCAL	,
          sum(nvl(slw.PURCH_COST_LOCAL,0)) as	PURCH_COST_LOCAL	,
          sum(nvl(slw.RTV_RETAIL_LOCAL,0)) as	RTV_RETAIL_LOCAL	,
          sum(nvl(slw.RTV_COST_LOCAL,0)) as	RTV_COST_LOCAL	,
          sum(nvl(slw.TSF_IN_RETAIL_LOCAL,0)) as	TSF_IN_RETAIL_LOCAL,
          sum(nvl(slw.TSF_IN_COST_LOCAL,0)) as	TSF_IN_COST_LOCAL	,
          sum(nvl(slw.TSF_OUT_RETAIL_LOCAL,0)) as	TSF_OUT_RETAIL_LOCAL	,
          sum(nvl(slw.TSF_OUT_COST_LOCAL,0)) as	TSF_OUT_COST_LOCAL	,
          sum(nvl(slw.NET_SALES_RETAIL_LOCAL,0)) as	NET_SALES_RETAIL_LOCAL	,
          sum(nvl(slw.NET_SALES_RETAIL_EXCL_VAT_LOCL,0)) as	NET_SALES_RETAIL_EXCL_VAT_LOCL	,
          sum(nvl(slw.NET_SALES_COST_LOCAL,0)) as	NET_SALES_COST_LOCAL	,
          sum(nvl(slw.RETURNS_RETAIL_LOCAL,0)) as	RETURNS_RETAIL_LOCAL	,
          sum(nvl(slw.RETURNS_COST_LOCAL,0)) as	RETURNS_COST_LOCAL	,
          sum(nvl(slw.MARKUP_RETAIL_LOCAL,0)) as	MARKUP_RETAIL_LOCAL	,
          sum(nvl(slw.MARKUP_CAN_RETAIL_LOCAL,0)) as	MARKUP_CAN_RETAIL_LOCAL	,
          sum(nvl(slw.CLEAR_MARKDOWN_RETAIL_LOCAL,0)) as	CLEAR_MARKDOWN_RETAIL_LOCAL	,
          sum(nvl(slw.PERM_MARKDOWN_RETAIL_LOCAL,0)) as	PERM_MARKDOWN_RETAIL_LOCAL	,
          sum(nvl(slw.PROM_MARKDOWN_RETAIL_LOCAL,0)) as	PROM_MARKDOWN_RETAIL_LOCAL	,
          sum(nvl(slw.MARKDOWN_CAN_RETAIL_LOCAL,0)) as	MARKDOWN_CAN_RETAIL_LOCAL	,
          sum(nvl(slw.SHRINKAGE_RETAIL_LOCAL,0)) as	SHRINKAGE_RETAIL_LOCAL	,
          sum(nvl(slw.SHRINKAGE_COST_LOCAL,0)) as	SHRINKAGE_COST_LOCAL	,
          sum(nvl(slw.CLS_STK_RETAIL_LOCAL,0)) as	CLS_STK_RETAIL_LOCAL	,
          sum(nvl(slw.CLS_STK_COST_LOCAL,0)) as	CLS_STK_COST_LOCAL	,
          sum(nvl(slw.COST_VARIANCE_AMT_LOCAL,0)) as	COST_VARIANCE_AMT_LOCAL	,
          sum(nvl(slw.HTD_GAFS_RETAIL_LOCAL,0)) as	HTD_GAFS_RETAIL_LOCAL	,
          sum(nvl(slw.HTD_GAFS_COST_LOCAL,0)) as	HTD_GAFS_COST_LOCAL	,
          sum(nvl(slw.STOCKTAKE_ADJ_RETAIL_LOCAL,0)) as	STOCKTAKE_ADJ_RETAIL_LOCAL	,
          sum(nvl(slw.STOCKTAKE_ADJ_COST_LOCAL,0)) as	STOCKTAKE_ADJ_COST_LOCAL	,
          sum(nvl(slw.RECLASS_IN_COST_LOCAL,0)) as	RECLASS_IN_COST_LOCAL	,
          sum(nvl(slw.RECLASS_IN_RETAIL_LOCAL,0)) as	RECLASS_IN_RETAIL_LOCAL	,
          sum(nvl(slw.RECLASS_OUT_COST_LOCAL,0)) as	RECLASS_OUT_COST_LOCAL	,
          sum(nvl(slw.RECLASS_OUT_RETAIL_LOCAL,0)) as	RECLASS_OUT_RETAIL_LOCAL	,
   ...
   from   fnd_mc_rtl_stck_ledger_wk slw,
          dim_department dd,
          dim_location dl,
...
g_rec_in             c_fnd_mc_rtl_stck_ledger_wk%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_mc_rtl_stck_ledger_wk%rowtype;
...
begin
   ...
   g_rec_out.last_updated_date               := g_date;
    ...
    g_rec_out.	NET_SALES_RETAIL_EXCL_VAT_LOCL	:= g_rec_in.	NET_SALES_RETAIL_EXCL_VAT_LOCL	;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_mc_stock_ledger_wk values a_tbl_insert(i);
...
begin
    ...
    l_text := 'LOAD OF rtl_mc_stock_ledger_wk EX FOUNDATION STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_mc_rtl_stck_ledger_wk;
    fetch c_fnd_mc_rtl_stck_ledger_wk bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_mc_rtl_stck_ledger_wk;
--**************************************************************************************************

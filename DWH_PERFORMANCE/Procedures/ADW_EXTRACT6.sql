--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACT6
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACT6" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACT6';
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    --g_count := dwh_generic_file_extract('select * from dim_item where department_no = 93','|','DWH_FILES_OUT','dim_item_bu');
    g_count := dwh_generic_file_extract('select * from dwh_performance.dim_item 
    where DEPARTMENT_NO IN (102,105,106,107,109,665,667,678,682)','|','DWH_FILES_OUT','assenture_prod');
--'select * from vw_extr_nielsens_location',',','DWH_FILES_OUT','nielsen_loc'

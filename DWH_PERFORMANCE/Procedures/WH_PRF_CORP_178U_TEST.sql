--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_178U_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_178U_TEST" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_178U_TEST';
...
type tbl_array_i is table of w6005682.rtl_sms_publish_supp_q%rowtype index by binary_integer;
type tbl_array_u is table of w6005682.rtl_sms_publish_supp_q%rowtype index by binary_integer;
...
begin
    ...
    execute immediate 'truncate table dwh_foundation.stg_ait_sms_response_cpy';
    commit;
    l_text := 'TRUNCATE CPY TABLE DONE AT : '||
...
begin
   ...
   select count(1)
   into   v_count
   from   w6005682.rtl_sms_publish_supp_q

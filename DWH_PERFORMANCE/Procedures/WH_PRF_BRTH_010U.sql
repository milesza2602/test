--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_010U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_010U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_BRTH_010U_PRE';
...
--**************************************************************************************************
    ...
    execute immediate ('truncate table dwh_performance.rtl_loc_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_performance.rtl_loc_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'rtl_loc_item_wk_bridgethorn', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_performance.rtl_loc_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    INSERT /*+ APPEND */ INTO dwh_performance.rtl_loc_item_wk_bridgethorn
     select
                 st_store_type
...
--                ,sum((case when pack_item_no is null then 0 else nvl(this_wk_catalog_ind,0) end)  )
    ...
    from
      ( select    (case when raistk.st_store_type IN ('WW','NN')
...
--                ,dl.area_no
                ...
                ,0 stores_with_stock
                ,0 stores_with_sales
     from     dwh_performance.temp_rtl_area_item_wk_catalog raistk  ,
...
group by (case when raistk.st_store_type IN ('WW','NN') THEN 'corporate stores' else 'franchise stores' end),
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
     from     dwh_performance.temp_rtl_area_item_wk_stock raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
      group by (case when raistk.st_store_type IN ('WW','NN') THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_stock
                ,0 stores_with_sales
     from     dwh_performance.temp_rtl_area_item_wk_sparse raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type IN ('WW','NN') THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_stock
                ,0 stores_with_sales
     from     (select    (case when raistk.st_store_type IN ('WW','NN')
                      THEN 'corporate stores'
                      else 'franchise stores'
                      end) st_store_type
                ,raistk.item_no
      ...
      from     dwh_performance.temp_rtl_area_item_wk_price raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type IN ('WW','NN') THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date )
...
--                ,dl.area_no
                ...
                ,0 stores_with_stock
                ,sum(nvl(stores_with_sales,0)   )
     from     dwh_performance.temp_rtl_area_item_wk_dense raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type IN ('WW','NN') THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_124J_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_124J_LOAD" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        September 2008
--  Author:      Alastair de Wet
--  Purpose:     Create Customer orders and ROQ fact table in the performance layer
--               with input ex OM table from foundation layer.
--  Tables:      Input  - fnd_rtl_loc_item_dy_om_ord
...
--
--  March 2014  - adapted to cater for Foods Renewal changes to cater for the fact that one
--                of the existing foundation tables (FND_RTL_LOC_ITEM_DY_OM_ST_ORD) is being
--                replaced by other tables which means that there are changes to the cursor.
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_124J_LOAD';
...
cursor c_fnd_rtl_loc_item_dy_om_ord is
   select ord.*,
          di.sk1_item_no,
          ...
          dih.sk2_item_no,
          dlh.sk2_location_no,
          nvl(so.boh_qty1,0) + nvl(sto.sdn_1_qty,0)  as sod_boh_qty,   -- nvl(so.boh_1_qty,0) + nvl(so.sdn_1_qty,0)  as sod_boh_qty,
   ...
   from   fnd_rtl_loc_item_dy_om_ord ord
          ...
          left outer join fnd_rtl_loc_item_dy_ff_st_ord so    -- WAS fnd_rtl_loc_item_dy_om_st_ord   --Foods Renewal change
                                      on ord.item_no                = so.item_no  and
                                         ord.location_no            = so.location_no and
                                         ord.post_date              = so.post_date
--                                        (ord.post_date + 1)         = so.post_date
          left outer join rtl_loc_item_dy_st_ord_ff sto       -- NEW JOIN FOR SDN QTY - Foods Renewal change
                                      on di.sk1_item_no             = sto.sk1_item_no  and
                                         dl.sk1_location_no         = sto.sk1_location_no and
                                         ord.post_date              = sto.post_date
...
g_rec_in             c_fnd_rtl_loc_item_dy_om_ord%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_rtl_loc_item_dy_om_ord%rowtype;
...
/*
   begin
      ...
      into   g_case_selling_excl_vat,
             g_num_units_per_tray,
             g_reg_rsp_excl_vat,
             g_case_cost,
             g_cost_price
      from   rtl_loc_item_dy_rms_price
      where  sk1_location_no        = g_rec_in.sk1_location_no and
             sk1_item_no            = g_rec_in.sk1_item_no and
             calendar_date          = g_rec_in.post_date;

      if g_num_units_per_tray = 0 or g_num_units_per_tray is null  then
      ...
      exception
         when no_data_found then
           ...
           g_case_cost              := 0;
           g_cost_price             := 0;
           l_message := 'No prices found on price table '||g_rec_in.item_no||' '||g_rec_in.location_no;
...
--**************************************************************************************************
    open c_fnd_rtl_loc_item_dy_om_ord;
    fetch c_fnd_rtl_loc_item_dy_om_ord bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_rtl_loc_item_dy_om_ord;
--**************************************************************************************************

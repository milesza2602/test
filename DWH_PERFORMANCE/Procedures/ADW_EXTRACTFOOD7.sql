--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACTFOOD7
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACTFOOD7" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACTFOOD7';
...
'select dc.fin_year_no,dc.fin_week_no,dc.this_week_start_date,location_no,location_name,item_no,wac,reg_rsp
from rtl_loc_item_dy_rms_price lid,
     dim_item di,
     dim_location dl,
     dim_calendar dc
where lid.sk1_location_no = dl.sk1_location_no and
      ...
      dc.fin_day_no       = 7 and
      lid.reg_rsp        <> 0  and lid.reg_rsp is not null and        
      di.business_unit_no  = 50 and
      dc.fin_year_no = 2014 and dc.fin_week_no between 10 and 13','|','DWH_FILES_OUT','extr_food_prices_2014_1013');    

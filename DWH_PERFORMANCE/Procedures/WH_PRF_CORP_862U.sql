--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_862U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_862U" 
...
--  06 Oct 16 - Combine dense + sparse for 6 week summaries by each week period.  Ref: BK06Oct1016
--  14 Nov 17 - Change log name from WH_PRF_CORP_862U_BK to WH_PRF_CORP_862U
--
--  Naming conventions
--  g_  -  Global variable
...
-- BK06Oct2016 (start)  
  ...
  execute immediate 'truncate table dwh_performance.temp_foods_tab_dense_sparse';
  commit;
--   in error
...
--  from    temp_foods_tab_2yrs_dense a;
  ...
  into    g_maxyr
  from    temp_foods_tab_2yrs_dense a;
  select /*+ parallel (a,4) full (a) */ max(FIN_WEEK_NO)
  into    g_maxwk
  from    temp_foods_tab_2yrs_dense a
  where   fin_year_no = g_maxyr;
  
  l_text := 'Max Year Extract = '||g_maxyr||' Max Week Extract = '||g_maxwk;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  
  select  max(THIS_WEEK_START_DATE), max(THIS_WEEK_end_DATE) 
  into    g_maxsprd, g_maxeprd 
  from    dim_calendar 
  where   FIN_YEAR_NO = g_maxyr and FIN_WEEK_NO = g_maxwk;
       
  -- build combined table key set of last 6 weeks - current week plus 5 weeks pervious weeks (there are 12 weeks in table)...
  insert /*+ parallel (c,4) */ into temp_foods_tab_dense_sparse c  
  with 
      item_list as 
        ( select   /*+ parallel (a,4) full(a)  */ 
                   ...
                   a.SK1_LOCATION_NO                 
          from     temp_foods_tab_2yrs_dense  a
--       where SK1_LOCATION_NO = 509           
          group by a.SK1_ITEM_NO,
                   a.SK1_LOCATION_NO
          union
          select   /*+ parallel (a,4) full(a)  */ 
                   a.SK1_ITEM_NO,
...
--       where SK1_LOCATION_NO = 509         
    ...
    -- load all other 5 week periods with exact same item/locno keys ...
    for i in 1..5  loop
        insert into  temp_foods_tab_dense_sparse c
        select b.THIS_WEEK_START_DATE, 
               ...
               a.SK1_LOCATION_NO,
               0,0,0,0
        from   temp_foods_tab_dense_sparse a
        join   dim_calendar                                b on (a.THIS_WEEK_START_DATE - (i*7) = b.THIS_WEEK_START_DATE and b.FIN_DAY_NO = 1)
        where  a.THIS_WEEK_START_DATE = g_maxsprd;
    end loop;
...
for g_sub in 1..6 loop
    ...
    merge /*+ parallel(tgt,4) */ into temp_foods_tab_dense_sparse tgt using
    (
      select /*+ parallel (a,4) full (a) */ 
            a.sk1_item_no, 
            a.sk1_location_no,
...
-- Part 2
merge /*+ parallel(sls,6) */ into rtl_loc_item_wk_sales_6wkavg sls using
   (
      with 
        sales_xtr as 
          (select * from temp_foods_tab_dense_sparse),

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_119VAT1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_119VAT1" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_119VAT';
...
begin
   ...
   using (
         select /*+ parallel(dnsr,4) full(dnsr) parallel(aloc,4) full(aloc) */  
          dnsr.*,
          nvl(aloc.alloc_qty,0) * (aloc.reg_rsp_excl_vat) vat_fix
   ...
   where  aloc.item_no                = di.item_no          
     ...
     and  aloc.release_date between '30/MAR/15' and '06/JUL/15'
   ...
   --  and  dnsr.last_updated_date   between '29 JUN 2015' and '29 JUN 2015'  
     ...
     and  dnsr.post_date between '29 jun 2015' and '06 jul 2015'  

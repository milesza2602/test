--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_677U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_677U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        March 2009
--  Author:      M Munnik
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_677U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'ROLLS SALES SPARSE TO PROM/LOC/ITEM/DY';
...
cursor c_prom_loc_item_dy is
   select   /*+ no_index (sp) */
   sp.post_date, sp.sk1_location_no, sp.sk1_item_no, ia.sk1_prom_no,
            (case when sp.post_date between dp.prom_start_date and dp.prom_end_date then g_sk1_prom_period_2
                  else g_sk1_prom_period_1 end) sk1_prom_period_no,
            sp.prom_sales_qty, sp.prom_sales, sp.prom_sales_cost,
            sp.ho_prom_discount_qty, sp.ho_prom_discount_amt,
            sp.st_prom_discount_qty, sp.st_prom_discount_amt
   from     rtl_loc_item_dy_rms_sparse sp
   join     dim_location dl               on  sp.sk1_location_no     = dl.sk1_location_no
   join     rtl_prom_item_all ia          on  sp.sk1_item_no         = ia.sk1_item_no
   ...
   And      Sp.Last_Updated_Date          = G_Date
...
begin
    ...
    l_text := 'LOAD RTL_PROM_LOC_ITEM_DY from SALES SPARSE STARTED '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

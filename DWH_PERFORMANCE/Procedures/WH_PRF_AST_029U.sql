--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_029U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_029U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        December 2013
--  Author:      Alfonso Joshua
--  Purpose:     Load Pre-Actual Plan (current week APlan) from performance level.
--  Tables:      Input  - rtl_chain_sc_wk_ast_pln_preact
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_029U';
...
cursor c_chn_geo_grd_sc_wk_ast_pln is
   select pln.sk1_chain_no,
    ...
    from  rtl_chain_sc_wk_ast_pln_preact pln,
          dim_calendar_wk dim
    where pln.fin_week_no  = dim.fin_week_no
     and  pln.fin_year_no  = dim.fin_year_no
...
--    select this_week_end_date - 7, (this_week_end_date + 1 ) - (7*7) 
    select this_week_end_date - 7, this_week_start_date - 7

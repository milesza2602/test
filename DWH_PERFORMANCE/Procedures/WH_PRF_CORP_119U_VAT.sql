--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_119U_VAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_119U_VAT" (p_forall_limit in integer,p_success out boolean) as
...
g_count              number        :=  0;
g_ch_alloc_selling   w6005682.rtl_loc_item_dy_rms_sparseq.ch_alloc_selling%type;
g_ch_alloc_qty       w6005682.rtl_loc_item_dy_rms_sparseq.ch_alloc_qty%type;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_119U_VAT';
...
type tbl_array_i is table of w6005682.rtl_loc_item_dy_rms_sparseq%rowtype index by binary_integer;
...
select /*+ MATERIALIZE  PARALLEL(ALOC,8) PARALLEL(LI) FULL(LI)  FULL(DIH) FULL(DL) FULL(DLH) */
...
--          max(round(nvl(li.reg_rsp,0) * 100 / (100 + nvl(di.vat_rate_perc,0)),2)) reg_rsp_excl_vat,
          max(case when li.tax_perc is null then
                ...
                round(NVL(LI.reg_rsp,0) * 100 / (100 + LI.tax_perc),2)
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into w6005682.rtl_loc_item_dy_rms_sparseq values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update w6005682.rtl_loc_item_dy_rms_sparseq
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD w6005682.rtl_loc_item_dy_rms_sparseq EX FOUNDATION STARTED '||

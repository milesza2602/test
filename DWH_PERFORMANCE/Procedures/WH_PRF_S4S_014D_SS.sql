--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_014D_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_014D_SS" (p_forall_limit in integer,p_success out boolean) as
...
--
--  DIM Tables:  Input  - FND_S4S_JOB_SS
--               Output - RTL_JOB_SS
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_014D_SS';
...
type tbl_array_i is table of RTL_JOB_SS%rowtype index by binary_integer;
type tbl_array_u is table of RTL_JOB_SS%rowtype index by binary_integer;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into RTL_JOB_SS values a_tbl_insert(i);
...
dbms_output.put_line(g_count||':'||g_rec_out.JOB_NAME);
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if 
            ...
            a_tbl_insert(i).S4S_BUSINESS_UNIT_NO    = g_rec_out.S4S_BUSINESS_UNIT_NO and
            a_tbl_insert(i).sk1_effective_from_date = g_rec_out.sk1_effective_from_date then
            g_found := TRUE;
         end if;
      end loop;
   end if;

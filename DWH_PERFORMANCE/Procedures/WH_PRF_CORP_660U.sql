--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_660U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_660U" 
...
g_rec_out            TEMP_PO_LIST_2%rowtype;
g_cnt number;
...
type tbl_array_i is table of TEMP_PO_LIST_2%rowtype index by binary_integer;
...
begin
    ...
    l_text := 'LOAD OF TEMP_PO_LIST_2 STARTED '||
...
*/
    ...
    execute immediate 'truncate table DWH_PERFORMANCE.TEMP_PO_LIST_1';
    l_text := 'TABLE DWH_PERFORMANCE.TEMP_PO_LIST_1 TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    ...
    g_param_ind := 1;
     insert /*+ append */ into DWH_PERFORMANCE.TEMP_PO_LIST_1
                       with selext1 as
                                ( select /*+ parallel(a,4) */ po_no, max(supply_chain_type) supply_chain_type
                                               from   fnd_rtl_purchase_order a
                                               where  not_before_date > '24 Jun 2007'
                                               and   (supply_chain_type is not null)
                                               and   (chain_code <> 'DJ' or chain_code is null)
                                               and    not_after_date < '1 Jan 2026'    -- 31 Mar 2017 remove when PO data fixed. Changed on SB to correct crash in 661
                                               group  by po_no),
                              selext2 as (select /*+ parallel(b,4) */ po_no, null supply_chain_type
                                               from   fnd_rtl_purchase_order b
                                               where  not_before_date > '24 Jun 2007'
                                               and   (chain_code <> 'DJ' or chain_code is null)
                                               group  by po_no
...
--**************************************************************************************************    
  g_recs_inserted := 0;
    ...
    execute immediate 'truncate table dwh_performance.TEMP_PO_LIST_2';
    l_text := 'TABLE TEMP_PO_LIST_2 TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);


    g_param_ind := 1;
     insert /*+ append */ into dwh_performance.TEMP_PO_LIST_2
   select /*+ parallel(po,4) full(ps) parallel(ps,4) full(di) */
                                       po.po_no, dp.sk1_po_no, po.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
                             from   fnd_rtl_purchase_order po
                                     join   dim_purchase_order dp       on po.po_no   = dp.po_no
                                     join   dim_item di                 on po.item_no = di.item_no
                                     join   DWH_PERFORMANCE.TEMP_PO_LIST_1 ps           on po.po_no   = ps.po_no
                             where  po.last_updated_date = G_DATE -- CHANGE 10 SEP 2016 WL TO PICKUP MORE DATA DUE TO DATAFIX
                     ...
                     union
                             select /*+ full(ps) parallel(ps,4) parallel(s,4) full(di) */
                                    s.po_no, dp.sk1_po_no, s.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
                             from   fnd_rtl_shipment s
                                     join   dim_purchase_order dp       on s.po_no    = dp.po_no
                                     join   dim_item di                 on s.item_no  = di.item_no
                                     join   DWH_PERFORMANCE.TEMP_PO_LIST_1 ps           on s.po_no    = ps.po_no
                             where  s.last_updated_date = G_DATE  -- CHANGE 10 SEP 2016 WL TO PICKUP MORE DATA DUE TO DATAFIX
                             group  by s.po_no, dp.sk1_po_no, s.item_no, di.sk1_item_no, ps.supply_chain_type;

  g_recs_read := 0;
  g_recs_inserted :=  0;
    l_text := 'insert1='||SQL%ROWCOUNT;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
---------------------------------------------------------------------------
--- added in for reclass  or to force re-process of po/items - 10 oct 2016
---------------
 --    insert /*+ append */ into dwh_performance.TEMP_PO_LIST_2
  --     with selitem as (select * from dim_item where style_no in (501249266	,501257338	,503491742	,
  --                                                          503482080	,101817050	,503491718	,503484316	,501248932	,101783889	,500902128	,503589986	,
 --                                                           503490926	,501179244	,101383847	,503490790	,503490942	,503484332	,503491558	,503490804	,
 --                                                           501038280	,503490870	,304128	,100579122	,503491532	,105535	,100526924	,503484294	,
 --                                                           310420	,503362882	,501375074	,503490838	,503490774	,503590012	,500358026	,100655010	,
 --                                                           501126298	,500862796	,500429928	,500886718	,503493002	,500277792	,501027700	,101783854	,
 --                                                           501334710	,101783848	,500886696	,503475428	,501286396	,500886700	,503588870	,503491566	,
 --                                                           503385264	,501261146	,501103286	,501333776	,503491540	,503469594	,503490740	,501050820	,
 --                                                           503484324	,501128726	,503493456	,500201250	,500269814	,503491524	,101611163	,503490820	,
  --                                                          503491734	,503490782	,501327658	) )
  --   select /*+ parallel(po,4) full(ps) parallel(ps,4) full(di) */
  --                                       po.po_no, dp.sk1_po_no, po.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
  --                             from   fnd_rtl_purchase_order po
  --                                     join   dim_purchase_order dp       on po.po_no   = dp.po_no
  --                                     join   selitem di                 on po.item_no = di.item_no
  --                                     join   DWH_PERFORMANCE.TEMP_PO_LIST_1 ps           on po.po_no   = ps.po_no
 --                              where  (po.NOT_AFTER_DATE >= '27 june 2016' or po.NOT_before_DATE >= '27 june 2016')
   --                                and    po.not_before_date between '25 Jun 2007' and g_date + 540   -- partitions for rtl_po_supchain_loc_item_dy, up till 18 months into future
  --                                 and   (chain_code <> 'DJ' or chain_code is null)
  --                             group  by po.po_no, dp.sk1_po_no, po.item_no, di.sk1_item_no, ps.supply_chain_type
   --                    union
  ...
  --                                    s.po_no, dp.sk1_po_no, s.item_no, di.sk1_item_no, ps.supply_chain_type, g_param_ind param_ind
   --                            from   fnd_rtl_shipment s
  --                                     join   dim_purchase_order dp       on s.po_no    = dp.po_no
  --                                     join   selitem di                 on s.item_no  = di.item_no
   --                                    join   DWH_PERFORMANCE.TEMP_PO_LIST_1 ps           on s.po_no    = ps.po_no
    --                           where  dp.NOT_AFTER_DATE >= '27 june 2016' or dp.NOT_before_DATE >= '27 june 2016'
    --                           group  by s.po_no, dp.sk1_po_no, s.item_no, di.sk1_item_no, ps.supply_chain_type;

   -- g_recs_read := 0;
  --  g_recs_inserted :=  0;
   --   l_text := 'insert1 extra recs='||SQL%ROWCOUNT;
   --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
---------------------------------------------------------------------------
 ...
 INSERT /*+ APPEND */ INTO dwh_performance.TEMP_PO_LIST
 SELECT DISTINCT * FROM  dwh_performance.TEMP_PO_LIST_2;
  ...
  g_recs_inserted :=  SQL%ROWCOUNT;
    ...
    commit;
    l_text := 'insert4='||g_recs_read;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
   l_text := 'excluding duplicate POs';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_recs_deleted := 0;
  g_cnt := 0;
  select count(*) into g_cnt from dwh_performance.rtl_exclude_po b
  where b.last_updated_date = g_date  
  ;
  if g_cnt > 0
  then 
  delete from  dwh_performance.TEMP_PO_LIST a
  where a.po_no in (select b.po_no from dwh_performance.rtl_exclude_po b
  where b.last_updated_date = g_date  
  );
  ...
  l_text := 'end delete';
    ...
    g_date_end := g_date + 540;
    l_text := 'po.not_before_date between 25 Jun 2007 and '||G_DATE_END;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    l_text := 'po.last_updated_date and ship.last_updated_date =  '||g_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    l_text := '--------------------';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    l_text := 'insert completed recs='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
cursor c_TEMP_PO_LIST_2_hist is
   with po_suppl_chain as
  (select po_no, max(supply_chain_type) supply_chain_type
   from   fnd_rtl_purchase_order
   where  not_before_date > '24 Jun 2007'
...
cursor c_TEMP_PO_LIST_2_old_hist is
   with non_dupl_po as
...
cursor c_TEMP_PO_LIST_2_ship is
   with non_dupl_po as
  (select   po_no, max(reccnt) from
  (select   po_no, item_no, count(*) reccnt
   from     fnd_rtl_purchase_order
...
g_rec_in             c_TEMP_PO_LIST_2%rowtype;
-- For input bulk collect --
type stg_array is table of c_TEMP_PO_LIST_2%rowtype;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into TEMP_PO_LIST_2 values a_tbl_insert(i);
...
--**************************************************************************************************
    case g_param_ind
       when 1 then
          open c_TEMP_PO_LIST_2;
          fetch c_TEMP_PO_LIST_2 bulk collect into a_stg_input limit g_forall_limit;
       when 2 then
          open c_TEMP_PO_LIST_2_hist;
          fetch c_TEMP_PO_LIST_2_hist bulk collect into a_stg_input limit g_forall_limit;
       when 3 then
          open c_TEMP_PO_LIST_2_ship;
          fetch c_TEMP_PO_LIST_2_ship bulk collect into a_stg_input limit g_forall_limit;
       when 4 then
          open c_TEMP_PO_LIST_2_old_hist;
          fetch c_TEMP_PO_LIST_2_old_hist bulk collect into a_stg_input limit g_forall_limit;
    end case;
    ...
    case g_param_ind
       when 1 then
          close c_TEMP_PO_LIST_2;
       when 2 then
          close c_TEMP_PO_LIST_2_hist;
       when 3 then
          close c_TEMP_PO_LIST_2_ship;
       when 4 then
          close c_TEMP_PO_LIST_2_old_hist;
    end case;
...
--**************************************************************************************************
    ...
    ww_dbms_stats.gather_table_stats('dwh_performance','TEMP_PO_LIST_2');
    l_text := 'TABLE TEMP_PO_LIST_2 STATISTICS UPDATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
*/
--**************************************************************************************************

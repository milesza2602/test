--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_570U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_570U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2009
--  Author:      M Munnik
--  Purpose:     Rolls Quality RTV facts in performance layer to Style-Colour/Week level
--  Tables:      Input  -   rtl_loc_item_dy_supp_rsn_rtv
--               Output -   rtl_loc_sc_wk_supp_rsn_rtv
...
g_rec_out            rtl_loc_sc_wk_supp_rsn_rtv%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_570U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLLS RTL_LOC_SC_WK_SUPP_RSN_RTV FROM ITEM/DY LEVEL';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of rtl_loc_sc_wk_supp_rsn_rtv%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_sc_wk_supp_rsn_rtv%rowtype index by binary_integer;
...
cursor c_rtl_loc_sc_wk_supp_rsn_rtv is
   select   lid.sk1_location_no,
            di.sk1_style_colour_no,
            dc.fin_year_no,
            dc.fin_week_no,
            lid.sk1_supplier_no,
            lid.sk1_rtv_reason_code,
            ...
            sum(lid.quality_single_rtv_qty) quality_single_rtv_qty,
            sum(lid.quality_single_rtv_selling) quality_single_rtv_selling,
            sum(lid.quality_single_rtv_cost) quality_single_rtv_cost,
            sum(lid.quality_bulk_rtv_qty) quality_bulk_rtv_qty,
            sum(lid.quality_bulk_rtv_selling) quality_bulk_rtv_selling,
            sum(lid.quality_bulk_rtv_cost) quality_bulk_rtv_cost,
            trunc(sysdate) last_updated_date
   from     rtl_loc_item_dy_supp_rsn_rtv lid
   ...
   group by lid.sk1_location_no, di.sk1_style_colour_no, dc.fin_year_no, dc.fin_week_no, lid.sk1_supplier_no, lid.sk1_rtv_reason_code;

-- Input record declared as cursor%rowtype
g_rec_in             c_rtl_loc_sc_wk_supp_rsn_rtv%rowtype;

-- Input bulk collect table declared
type stg_array is table of c_rtl_loc_sc_wk_supp_rsn_rtv%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into rtl_loc_sc_wk_supp_rsn_rtv values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update rtl_loc_sc_wk_supp_rsn_rtv
...
-- Check to see if record is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_loc_sc_wk_supp_rsn_rtv
...
begin
    ...
    l_text := 'ROLL OF RTL_LOC_SC_WK_SUPP_RSN_RTV FROM ITEM/DY LEVEL STARTED '||
...
--**************************************************************************************************
    open c_rtl_loc_sc_wk_supp_rsn_rtv;
    fetch c_rtl_loc_sc_wk_supp_rsn_rtv bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_rtl_loc_sc_wk_supp_rsn_rtv;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_660U_VATWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_660U_VATWL" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:         February 2009
--  Author:       M Munnik
...
g_start_date         date;
g_end_date           date;
g_rec_out            temp_po_list_VATWL%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_660U_VATWL';
...
type tbl_array_i is table of temp_po_list_VATWL%rowtype index by binary_integer;
...
begin
    ...
    l_text := 'LOAD OF temp_po_list_VATWL STARTED '||
...
*/
    ...
    execute immediate 'truncate table dwh_performance.temp_po_list_VATWL';
    l_text := 'TABLE temp_po_list_VATWL TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    g_param_ind := 1;
     insert /*+ append */ into dwh_performance.temp_po_list_VATWL
  ...
  l_text := 'end insert';
    ...
    l_text := 'TABLE temp_po_list_VATWL STATISTICS UPDATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    ww_dbms_stats.gather_table_stats('dwh_performance','temp_po_list_VATWL');
...
cursor c_temp_po_list_VATWL is
   with po_suppl_chain as
...
cursor c_temp_po_list_VATWL_hist is
   with po_suppl_chain as
  (select po_no, max(supply_chain_type) supply_chain_type
   from   fnd_rtl_purchase_order
   where  not_before_date > '24 Jun 2007'
...
cursor c_temp_po_list_VATWL_old_hist is
   with non_dupl_po as
...
cursor c_temp_po_list_VATWL_ship is
   with non_dupl_po as
  (select   po_no, max(reccnt) from
  (select   po_no, item_no, count(*) reccnt
   from     fnd_rtl_purchase_order
...
g_rec_in             c_temp_po_list_VATWL%rowtype;
-- For input bulk collect --
type stg_array is table of c_temp_po_list_VATWL%rowtype;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into temp_po_list_VATWL values a_tbl_insert(i);
...
--**************************************************************************************************
    case g_param_ind
       when 1 then
          open c_temp_po_list_VATWL;
          fetch c_temp_po_list_VATWL bulk collect into a_stg_input limit g_forall_limit;
       when 2 then
          open c_temp_po_list_VATWL_hist;
          fetch c_temp_po_list_VATWL_hist bulk collect into a_stg_input limit g_forall_limit;
       when 3 then
          open c_temp_po_list_VATWL_ship;
          fetch c_temp_po_list_VATWL_ship bulk collect into a_stg_input limit g_forall_limit;
       when 4 then
          open c_temp_po_list_VATWL_old_hist;
          fetch c_temp_po_list_VATWL_old_hist bulk collect into a_stg_input limit g_forall_limit;
    end case;
    ...
    case g_param_ind
       when 1 then
          close c_temp_po_list_VATWL;
       when 2 then
          close c_temp_po_list_VATWL_hist;
       when 3 then
          close c_temp_po_list_VATWL_ship;
       when 4 then
          close c_temp_po_list_VATWL_old_hist;
    end case;

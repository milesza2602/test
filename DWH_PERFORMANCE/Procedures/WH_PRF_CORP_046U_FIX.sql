--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_046U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_046U_FIX" (p_forall_limit in integer,p_success out boolean, p_from_loc_no in integer,p_to_loc_no in integer) as
--**************************************************************************************************
--  Date:        Sept 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_046U_FIX_'|| p_from_loc_no;
...
begin
   ...
   merge /*+ parallel(mer_lid,6) */ into rtl_location_item   mer_lid
    using  (
   ...
   from   dwh_foundation.fnd_location_item fli,
          dwh_performance.dim_item di,
          dwh_performance.dim_location dl,
          dwh_performance.dim_country dc,
          dwh_performance.dim_supplier ds,
          w6005682.wrong_cataloged_items_31 fix    -- this table name will change each time the program needs to run
   where  fli.item_no                = di.item_no  and
          ...
          and fli.item_no            = fix.item_no
          and fli.location_no        = fix.location_no
...
merge /*+ parallel(rlid,6) */ into rtl_location_item rlid 
  using (
     select /*+ parallel(li,6) full(li) */ 
       ...
       from   rtl_location_item li,
              dim_item di,
              w6005682.wrong_cataloged_items_31 fix
       where  li.sk1_item_no             = di.sk1_item_no  and
              di.business_unit_no        = 50 
              and li.sk1_item_no            = fix.sk1_item_no
              and li.sk1_location_no        = fix.sk1_location_no and

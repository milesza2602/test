--------------------------------------------------------
--  DDL for Procedure WH_PRF_RP_006D
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RP_006D" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RP_006D';
...
cursor c_fnd_rtl_shipment is
 ...
 with stock as(
  SELECT shipv.sk1_item_no
  FROM   rtl_loc_item_dy_ch_avail shipv, rtl_loc_item_dy_rms_stock stk
  WHERE  stk.post_date              = g_date
     AND stk.post_date        BETWEEN shipv.active_from_date AND shipv.active_to_date
     AND shipv.sk1_item_no          = stk.sk1_item_no
     AND shipv.sk1_location_no      = stk.sk1_location_no
     and shipv.valid_record_ind     = 1
     and shipv.active_to_date       > stk.post_date
     AND NVL (stk.clear_soh_qty, 0) <> 0)

  SELECT distinct sk1_item_no from stock;
...
begin
    ...
    l_text := 'UPDATE OF RTL_LOC_ITEM_DY_CH_AVAIL_avail EX FOUNDATION STARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_740U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_740U" 
                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        May 2009
--  Author:      M Munnik
--  Purpose:     Update DC PLANNING data to OM fact table in the performance layer
--               with input ex OM fnd_loc_item_om_wh_plan table from foundation layer.
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_740U';
...
-- This procedure does not select only where last_updated_date = g_date, because fnd_loc_item_om_wh_plan
-- gets fully refreshed every day.

-- For input bulk collect --
type stg_array is table of c_om_wh_plan%rowtype;
a_stg_input          stg_array;
g_rec_in             c_om_wh_plan%rowtype;
...
--**************************************************************************************************
    open c_om_wh_plan;
    fetch c_om_wh_plan bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_661U_WLTST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_661U_WLTST" 
...
--                Output - RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ  RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ
...
--                The distinct supply_chain_type is then used for all the records of the PO, regardless of the item and location (in wh_prf_corp_661U_WLTSTD and _662u).
--  2 nov 2011 - qc 4419 - wendy lyttle
--               Procedure Change:WH_PRF_CORP_661U_WLTST - Use DIM_ITEM.PACK_ITEM_IND to update
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_661U_WLTST';
...
--**************************************************************************************************
--
-- STEP 1 : Do calculations and load into temp_po_DATA_DJ
--
--**************************************************************************************************
      ...
      as
         (select    /*+ materialize full(pi) full(tp) */
                            distinct
                              tp.po_no
                            , tp.item_no
                   from DWH_PERFORMANCE.temp_po_list_DJ tp 
...
----------------------
   ...
   (
      select    /*+ materialize parallel(di,4) full(di) 
                            ...
                            , vat_rate_perc
                   from          dim_item di
                   join          DWH_PERFORMANCE.temp_po_list_DJ tp            on  tp.item_no            = di.item_no
...
(
   ...
   /*+  MATERIALIZE parallel(P,4) full(TP) */
          ...
          ,
            (case when p.chain_code is null OR DL.CHAIN_NO <> 40
                  then 0
                  else  1 end) sk1_chain_code_ind
    from      fnd_rtl_purchase_order p
    join      DWH_PERFORMANCE.temp_po_list_DJ tp       on  Tp.item_no                   = p.item_no
                                       and Tp.po_no                     = p.po_no
    join      DWH_PERFORMANCE.DIM_LOCATION DL       on  DL.LOCATION_NO = P.LOCATION_NO
      where  p.chain_code = 'DJ' OR CHAIN_NO = 40
                                    --   WHERE P.PO_NO = 6031382
/*   CHANGED CODE ABOVE TO PULL ALL dj LOCATIONS REGARDLESS OF CHAIN_CODE
...
left join dim_DJ_contract dc            on  DC.contract_no        = SP.contract_no
...
--**************************************************************************************************
--
-- STEP 2 : Do calculations and load into temp_po_CALC_DJ
--
--**************************************************************************************************
...
--**************************************************************************************************
--
-- STEP 3 : Do calculations and load into RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ
--
--**************************************************************************************************
...
MERGE INTO  DWH_PERFORMANCE.RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ RTL
 using ( SELECT a.* FROM     DWH_PERFORMANCE.temp_po_CALC_DJ a ) MER_MART
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   insert into temp_po_deletes_dj
    ...
    from   DWH_PERFORMANCE.RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ r
    join   temp_po_list_dj t     on  r.sk1_po_no        = t.sk1_po_no
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    delete from DWH_PERFORMANCE.RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ r
    where  exists(select t.sk1_po_no
                  from   temp_po_deletes_dj t

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_199U_BCK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_199U_BCK" (p_forall_limit in integer,p_success out boolean) as
...
/*
    ...
    into g_fin_year_cur
    from dim_calendar 
    where calendar_date = g_date; --'26/JUN/17';
    
    select fin_year_no 
    into g_fin_year_prev
    from dim_calendar 
    where calendar_date = g_date - 1; --'25/JUN/17';
*/
...
Insert  /*+ APPEND parallel (a,4) */ into mart_fd_area_item_dy a

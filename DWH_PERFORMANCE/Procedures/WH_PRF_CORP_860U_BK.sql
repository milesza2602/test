--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_860U_BK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_860U_BK" 
...
--    14 Nov 2017 - Add 3 additional procedures (extract_rate_of_sale, extract_like_4_like, extract_price). 
--                  A Joshua Chg-10153
--  Naming conventions
--  g_  -  Global variable
...
g_date               date;
g_start_date         date          := '29 jun 15';
g_end_date           date          := '30 aug 15';
g_ly_start_date      date;
g_ly_end_date        date;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_860U_BK';
...
BEGIN 
   ...
   DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
         ...
         with selitm as (select /*+ parallel (a,8) full (a) */
          distinct a.fin_year_no, a.fin_week_no, a.item_no, a.location_no, b.sk1_item_no,
          c.sk1_location_no
          from DWH_PERFORMANCE.TEMP_FOODS_TAB_DENSE_BUILD_I a, dim_item b, dim_location c
          where a.fin_year_no = 2017
          and a.item_no = b.item_no
          and a.location_no = c.location_no
          )
                    select /*+ parallel(SRC,8) */ SRC.fin_year_no, SRC.fin_week_no, 
                    SRC.sk1_item_no, SRC.sk1_location_no, 
                    sum(nvl(sales,0)) sales, sum(nvl(sales_qty,0)) sales_qty, 
                    sum(nvl(sales_cost,0)) sales_cost ,sum(nvl(sales_margin,0)) sales_margin
                    from RTL_LOC_ITEM_WK_RMS_DENSE src, selitm b
          where src.fin_year_no = b.fin_year_no
          and   src.fin_week_no = b.fin_week_no
          and   src.sk1_item_no = b.sk1_item_no
          and   src.sk1_location_no = b.sk1_location_no
...
BEGIN
   ...
   DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
    ...
    from RTL_LOC_ITEM_WK_RMS_SPARSE src, selitm b
    where src.fin_year_no = b.fin_year_no
    and   src.fin_week_no = b.fin_week_no
    and   src.sk1_item_no = b.sk1_item_no
    and   src.sk1_location_no = b.sk1_location_no
...
BEGIN
   ...
   DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
    ...
    SELCAL as (select distinct THIS_WEEK_START_DATE,THIS_WEEK_END_DATE,FIN_YEAR_NO, FIN_QUARTER_NO, FIN_MONTH_NO, FIN_WEEK_NO, (FIN_YEAR_NO*100+FIN_WEEK_NO) FIN_YRWK_NO
               from dim_calendar where calendar_date between g_start_date and g_end_date),
...
BEGIN
   ...
   insert /*+ append */ into dwh_performance.TEMP_FOODS_TAB_LIKE_FOR_LIKE
    ...
    with selitm as (select /*+ parallel (a,8) full (a) */
    distinct a.fin_year_no, a.fin_week_no, a.item_no, a.location_no, b.sk1_item_no,
    c.sk1_location_no, d.THIS_WEEK_START_DATE, d.THIS_WEEK_END_DATE, d.calendar_date
    from DWH_PERFORMANCE.TEMP_FOODS_TAB_DENSE_BUILD_I a, dim_item b, dim_location c,
    dim_calendar d
    where a.fin_year_no = 2017
    ...
    and a.item_no = b.item_no
    and a.location_no = c.location_no
    order by a.fin_year_no, a.fin_week_no, a.item_no, a.location_no, d.calendar_date)

   select /*+ parallel(ld,6) full (sl) full (sc) parallel(ds,6) full (ds) */ sc.fin_year_no, sc.fin_week_no, 
           sc.sk1_item_no, sc.sk1_location_no, max(like_for_like_ind) like_for_like_ind 
   from  rtl_loc_dy ld,  selitm sc
   where ld.sk1_location_no = sc.sk1_location_no 
    and  ld.post_date       = sc.calendar_date
   group by sc.fin_year_no, sc.fin_week_no, sc.sk1_location_no, sc.sk1_item_no;
...
BEGIN
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    from rtl_loc_item_dy_rms_price src, selitm si
    where src.sk1_item_no     = si.sk1_item_no 
    and   src.sk1_location_no = si.sk1_location_no 
    and   src.calendar_date   = si.calendar_date
    group by si.fin_year_no, si.fin_week_no, src.sk1_item_no, src.sk1_location_no)

    select /*+ full (a) parallel (a,4) parallel (c,4) */ a.*
    from  RTL_LOC_ITEM_WK_RMS_dense c,
          selprice a

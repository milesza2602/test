--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_119FIX2NOV15
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_119FIX2NOV15" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      wENDY LYTTLE
--  Purpose:     catch-up fixes from RMS for allocations to be written to RTL_LOC_ITEM_DY_RMS_SPARSE
--
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_119FIX2NOV15';
...
SELALLOC AS (
  select /*+ MATERIALIZE  PARALLEL(ALLOC,8)  */ 
          *
   from  FND_RTL_ALLOCATION ALLOC
          where  CHAIN_CODE = 'DJ'
)
select /*+ MATERIALIZE   PARALLEL(LI) FULL(LI)  FULL(DIH) FULL(DL) FULL(DLH) */

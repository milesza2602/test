--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_195U_FIXAJ1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_195U_FIXAJ1" (p_forall_limit in integer,p_success out boolean) as
-- ******************************************************************************************
--  Date:        January 2017
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_195U_FIXAJ1';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'New Case statemnt test';
...
FOR g_sub IN 0..147
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   merge /*+ parallel(cat,6) */ into dwh_performance.rtl_loc_item_dy_catalog cat using   
     --dwh_datafix.aj_rtl_loc_item_dy_catalog_c cat
  (
         select /*+ parallel (lid,6) full (di) full (dl) full (diu) parallel (rms,6) */ 
          ...
          case when --fd_num_cust_catlg_adj 
               (Case when (case 
              when di.pack_item_ind = 0 and
                   dl.active_store_ind = 1 and
                   diu.product_group_scaling_desc_501 = 'N' and
          ...
          case when nvl(lid.num_facings,0) > 0 then                 
                case when (nvl(lid.boh_adj_qty,0) + abs(nvl(rms.waste_qty,0))) > nvl(lid.num_facings,0) then 1
                     else (nvl(lid.boh_adj_qty,0) + abs(nvl(rms.waste_qty,0))) / nvl(lid.num_facings,0) end
          else  
          case when lid.num_shelf_life_days >= 3 then 
                case when (nvl(lid.boh_adj_qty,0) + abs(nvl(rms.waste_qty,0))) < 3 then 0
                else 1 end
          else lid.fd_num_avail_days_adj end end
          else 0
          end  fd_cust_avail   
...
--    and   lid.sk1_item_no in (63936)
--    and   lid.sk1_location_no in (566)
...
begin
    ...
    l_text := 'Update OF rtl_loc_item_dy_catalog_n STARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_676U_REUSE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_676U_REUSE" 
...
cursor c_prom_loc_sc_dy is
   select   /*+ no_index (sp) full (ia) parallel(sp,8) */
...
FOR G_SUB IN 2..5
  LOOP
   ...
   IF G_SUB = 0 THEN
      ...
      G_END_DATE   := G_DATE - 101;
   END IF;   
   IF G_SUB = 1 THEN
      G_START_DATE := G_DATE - 100;
      G_END_DATE   := G_DATE - 81;
   END IF;
   IF G_SUB = 2 THEN
      G_START_DATE := G_DATE - 80;
      G_END_DATE   := G_DATE - 61;
   END IF;
   IF G_SUB = 3 THEN
      G_START_DATE := G_DATE - 60;
      G_END_DATE   := G_DATE - 41;
   END IF;
   IF G_SUB = 4 THEN
      G_START_DATE := G_DATE - 40;

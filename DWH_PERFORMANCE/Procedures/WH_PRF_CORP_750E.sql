--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_750E
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_750E" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_750E';
...
-- BK12May16 (start) ...     
   select    1, 'FBH', sd2.*, sw2.*             -- AU26Mar18
   from
   (select   sum(this_year_sales) this_year_sales, sum(last_year_sales) last_year_sales
   from      sales 
   where     post_date = g_date

--------------------------------------------------------
--  DDL for Procedure QS_EXTRACT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."QS_EXTRACT" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'QS_EXTRACT';
...
--                 Where  A.this_week_start_date Between ''9 May 11'' And ''20 Jun 11''
    g_count := dwh_generic_file_extract('select /*+ parallel(a,4) */ * from dwh_performance.mart_fds_loc_item_wk_tableau a order by fin_year_no, fin_week_no, location_no, item_no','|','DWH_FILES_OUT','mart_fds_tableau');

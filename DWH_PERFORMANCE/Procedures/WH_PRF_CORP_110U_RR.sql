--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110U_RR
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110U_RR" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as
--**************************************************************************************************
--  Date:        Sept 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_110U_'|| p_from_loc_no||'_R21';
...
cursor c_fnd_rtl_loc_item_dy_rms_sale is
   select /*+ full(di) full(dl) full(dlh) full(dih) full(fnd_li) parallel (fnd_li,8)  */  
          post_date,
          sales_qty,
          sales,
          sales_cost,

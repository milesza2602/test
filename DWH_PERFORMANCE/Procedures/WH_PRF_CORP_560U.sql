--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_560U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_560U" 
                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Sept 2008
--  Author:      Christie Koorts
--  Purpose:     Create location dept day fact table in the performance layer with
--               added value ex foundation layer stock ledger week table.
--  Tables:      Input  -   fnd_rtl_stock_ledger_wk
--               Output -   rtl_loc_dept_dy
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  1 April 2009 - defect 1226 - - no filter on driving cursor
--
--  Naming conventions:
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_560U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE RTL_LOC_DEPT_DY EX FND_RTL_STOCK_LEDGER_WK';
...
with chgrecs as
  (select location_no, department_no, fin_week_end_date
   from   fnd_rtl_stock_ledger_wk
   where  last_updated_date = g_date
   group  by location_no, department_no, fin_week_end_date)

   select dl.sk1_location_no,
          dd.sk1_department_no,
          slw.fin_week_end_date,
          max(dlh.sk2_location_no) sk2_location_no,
          sum(slw.cls_stk_cost) cls_stk_cost
   from   chgrecs cr
   join   fnd_rtl_stock_ledger_wk slw on  slw.location_no       = cr.location_no
                                      and slw.department_no     = cr.department_no
                                      and slw.fin_week_end_date = cr.fin_week_end_date
   join  dim_location dl              on  slw.location_no       = dl.location_no
   join  dim_location_hist dlh        on  slw.location_no       = dlh.location_no
                                      and slw.fin_week_end_date between dlh.sk2_active_from_date and dlh.sk2_active_to_date
   join  dim_department dd            on  slw.department_no     = dd.department_no
   group by dl.sk1_location_no, dd.sk1_department_no, slw.fin_week_end_date;
...
begin
   ...
   g_rec_out.post_date                      := g_rec_in.fin_week_end_date;
   g_rec_out.sk2_location_no                := g_rec_in.sk2_location_no;
   g_rec_out.closing_stock_cost             := g_rec_in.cls_stk_cost;
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      set    sk2_location_no      = a_tbl_update(i).sk2_location_no,
             closing_stock_cost   = a_tbl_update(i).closing_stock_cost,
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_DEPT_DY EX FND_RTL_STOCK_LEDGER_WK STARTED '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_004U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_004U_NEW" (
   p_forall_limit   IN     INTEGER,
   p_success           OUT BOOLEAN)
AS
   --**************************************************************************************************
    -- copy of wh_prf_s4s_004U_NEW - taken on 6/oct/2014 before code to change aviability to cycle
    --**************************************************************************************************
   --  Date:        July 2014
   --  Author:      Wendy lyttle
...
--
   ...
   --**************************************************************************************************
   --
   --  Tables:      Input    - dwh_PERFORMANCE.TST_S4S_LOC_EMP_DY_part3
   --               Output   - TST_EMP_AVAIL_LOC_JOB_DY -- TST_EMP_AVAIL_LOC_JOB_DY
   ...
   g_count            NUMBER := 0;
   g_rec_out          TST_EMP_AVAIL_LOC_JOB_DY%ROWTYPE;
   g_found            BOOLEAN;
   g_date             DATE;
...
g_recs_deleted      integer       :=  0;
      ...
      l_module_name sys_dwh_errlog.log_procedure_name%TYPE := 'WH_PRF_S4S_004U_NEW';
      l_name sys_dwh_log.log_name%TYPE                     := dwh_constants.vc_log_name_rtl_md;
      l_system_name sys_dwh_log.log_system_name%TYPE       := dwh_constants.vc_log_system_name_rtl_prf;
      l_script_name sys_dwh_log.log_script_name%TYPE       := dwh_constants.vc_log_script_rtl_prf_md;
      l_procedure_name sys_dwh_log.log_procedure_name%TYPE := l_module_name;
      l_text sys_dwh_log.log_text%TYPE;
      l_description sys_dwh_log_summary.log_description%TYPE   := 'LOAD THE TST_EMP_AVAIL_LOC_JOB_DY data  EX FOUNDATION';
      l_process_type sys_dwh_log_summary.log_process_type%TYPE := dwh_constants.vc_log_process_type_n;
      -- For output arrays into bulk load forall statements --
      TYPE tbl_array_i
      IS
        TABLE OF TST_EMP_AVAIL_LOC_JOB_DY%ROWTYPE INDEX BY BINARY_INTEGER;
      TYPE tbl_array_u
      IS
        TABLE OF TST_EMP_AVAIL_LOC_JOB_DY%ROWTYPE INDEX BY BINARY_INTEGER;
...
--  I30_TST_EMP_AVL_LC_JB_DY	NONUNIQUE	VALID	NORMAL	N	NO		NO	LAST_UPDATED_DATE
--  I40_TST_EMP_AVL_LC_JB_DY	NONUNIQUE	VALID	NORMAL	N	NO		NO	SK1_EMPLOYEE_ID, SK1_JOB_ID, TRAN_DATE
--  I50_TST_EMP_AVL_LC_JB_DY	NONUNIQUE	VALID	NORMAL	N	NO		NO	SK1_EMPLOYEE_ID, TRAN_DATE
--  K_TST_EMP_AVL_LC_JB_DY	UNIQUE	VALID	NORMAL	N	NO		NO	SK1_LOCATION_NO, SK1_EMPLOYEE_ID, SK1_JOB_ID, TRAN_DATE
  --**************************************************************************************************
...
BEGIN
  ...
  BEGIN
    ...
    WHERE CONSTRAINT_NAME = 'PK_TST_EMP_AVL_LC_JB_DY'
    AND TABLE_NAME        = 'TST_EMP_AVAIL_LOC_JOB_DY';
    
    l_text               := 'alter table dwh_performance.TST_EMP_AVAIL_LOC_JOB_DY drop constraint PK_TST_EMP_AVL_LC_JB_DY';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_performance.TST_EMP_AVAIL_LOC_JOB_DY drop constraint PK_TST_EMP_AVL_LC_JB_DY');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'constraint PK_TST_EMP_AVL_LC_JB_DY does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
     l_text               := 'done';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
  ...
  BEGIN
    ...
    INTO G_name
    FROM DBA_indexes
    WHERE index_NAME = 'I30_TST_EMP_AVL_LC_JB_DY'
    AND TABLE_NAME        = 'TST_EMP_AVAIL_LOC_JOB_DY';
    
    l_text               := 'drop INDEX DWH_PERFORMANCE.I30_TST_EMP_AVL_LC_JB_DY';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('drop INDEX DWH_PERFORMANCE.I30_TST_EMP_AVL_LC_JB_DY');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'index PI30_TST_EMP_AVL_LC_JB_DY does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
  ...
  BEGIN
    ...
    INTO G_name
    FROM DBA_indexes
    WHERE index_NAME = 'I40_TST_EMP_AVL_LC_JB_DY'
    AND TABLE_NAME        = 'TST_EMP_AVAIL_LOC_JOB_DY';
    
    l_text               := 'drop INDEX DWH_PERFORMANCE.I40_TST_EMP_AVL_LC_JB_DY';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('drop INDEX DWH_PERFORMANCE.I40_TST_EMP_AVL_LC_JB_DY');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
        l_text := 'index I40_TST_EMP_AVL_LC_JB_DY does not exist';
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
  ...
  BEGIN
    ...
    INTO G_name
    FROM DBA_indexes
    WHERE index_NAME = 'I50_TST_EMP_AVL_LC_JB_DY'
    AND TABLE_NAME        = 'TST_EMP_AVAIL_LOC_JOB_DY';
    
    l_text               := 'drop INDEX DWH_PERFORMANCE.I50_TST_EMP_AVL_LC_JB_DY';
    DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
    EXECUTE immediate('drop INDEX DWH_PERFORMANCE.I50_TST_EMP_AVL_LC_JB_DY');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
        l_text := 'index I50_TST_EMP_AVL_LC_JB_DY does not exist';
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
...
BEGIN
  ...
  l_text := 'Insert into TST_EMP_AVAIL_LOC_JOB_DY ';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  
          insert /*+ append */
   into DWH_PERFORMANCE.TST_EMP_AVAIL_LOC_JOB_DY
...
with selexta
 ...
 (SELECT
  ...
  JOIN dwh_PERFORMANCE.TST_S4S_LOC_EMP_DY_part3 fc
  ...
  where SK1_EMPLOYEE_ID not in (1105841,1128052,1140310,1081138,1115483)
...
,se2.TRAN_DATE ;                  
        ...
        L_TEXT := 'TST_EMP_AVAIL_LOC_JOB_DY : recs = '||g_recs;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
BEGIN
      l_text          := 'Running GATHER_TABLE_STATS ON TST_EMP_AVAIL_LOC_JOB_DY';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'TST_EMP_AVAIL_LOC_JOB_DY', DEGREE => 8);

      l_text := 'create INDEX DWH_PERFORMANCE.I30_TST_EMP_AVL_LC_JB_DY';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I30_TST_EMP_AVL_LC_JB_DY ON DWH_PERFORMANCE.TST_EMP_AVAIL_LOC_JOB_DY (LAST_UPDATED_DATE)     
      TABLESPACE PRF_MASTER NOLOGGING  PARALLEL');
      Execute Immediate('ALTER INDEX DWH_PERFORMANCE.I30_TST_EMP_AVL_LC_JB_DY LOGGING NOPARALLEL') ;
      
      l_text := 'create INDEX DWH_PERFORMANCE.I40_TST_EMP_AVL_LC_JB_DY';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I40_TST_EMP_AVL_LC_JB_DY ON DWH_PERFORMANCE.TST_EMP_AVAIL_LOC_JOB_DY (SK1_EMPLOYEE_ID, SK1_JOB_ID, TRAN_DATE)     
      TABLESPACE PRF_MASTER NOLOGGING  PARALLEL');
      Execute Immediate('ALTER INDEX DWH_PERFORMANCE.I40_TST_EMP_AVL_LC_JB_DY LOGGING NOPARALLEL') ;

      l_text := 'create INDEX DWH_PERFORMANCE.I50_TST_EMP_AVL_LC_JB_DY';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I50_TST_EMP_AVL_LC_JB_DY ON DWH_PERFORMANCE.TST_EMP_AVAIL_LOC_JOB_DY (SK1_EMPLOYEE_ID, TRAN_DATE)     
      TABLESPACE PRF_MASTER NOLOGGING  PARALLEL');
      Execute Immediate('ALTER INDEX DWH_PERFORMANCE.I50_TST_EMP_AVL_LC_JB_DY LOGGING NOPARALLEL') ;   
...
BEGIN
    ...
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'TST_EMP_AVAIL_LOC_JOB_DY', DEGREE => 8);
      ...
      l_text := 'alter table dwh_performance.TST_EMP_AVAIL_LOC_JOB_DY add constraint PK_TST_EMP_AVL_LC_JB_DY';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('alter table dwh_performance.TST_EMP_AVAIL_LOC_JOB_DY add CONSTRAINT PK_TST_EMP_AVL_LC_JB_DY PRIMARY KEY (SK1_LOCATION_NO, SK1_EMPLOYEE_ID, SK1_JOB_ID, TRAN_DATE)                    
  ...
  l_text := 'Running GATHER_TABLE_STATS ON TST_S4S_LOC_EMP_DY_PART3';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'TST_S4S_LOC_EMP_DY_PART3', DEGREE => 8);
   l_text := 'Completed GATHER_TABLE_STATS ON TST_S4S_LOC_EMP_DY_PART3';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  a_remove_indexes;

  l_text := 'Truncating TST_EMP_AVAIL_LOC_JOB_DY';
  DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
  execute immediate ('truncate table DWH_PERFORMANCE.TST_EMP_AVAIL_LOC_JOB_DY');

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_030F
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_030F" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - fnd_class, dim_department
--               Output - dim_class
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  23 April 2009 - defect 1355 - Change total_descr from plural to singular
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            dim_class%rowtype;
g_rec_in             fnd_class%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_030F';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE DIM_CLASS EX FND_CLASS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_class%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_class%rowtype index by binary_integer;
type tbl_array_u is table of dim_class%rowtype index by binary_integer;
...
---------------------------------------------------------
   ...
   g_rec_out.total_desc                 := 'ALL CLASS';
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_class values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_class
      set    class_name                      = a_tbl_update(i).class_name,
...
begin
   g_found := dwh_valid.dim_class(g_rec_out.class_no,g_rec_out.department_no);

-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_class_no  := merch_hierachy_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF DIM_CLASS EX FND_CLASS STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_class;
    fetch c_fnd_class bulk collect into a_stg_input limit g_forall_limit;

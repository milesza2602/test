--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_505U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_505U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Jan 2009
...
--  Tables:      Input  - rtl_mc_loc_item_dy_rms_stock
--               Output - rtl_mc_loc_item_wk_rms_stock
...
g_count              number        :=  0;
g_rec_out            rtl_mc_loc_item_wk_rms_stock%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_505U';
...
begin
    ...
    l_text := 'ROLLUP OF rtl_mc_loc_item_wk_rms_stock EX DAY LEVEL STARTED '||
...
--**************************************************************************************************
    ...
    DELETE /*+ PARALLEL(ST,8)*/  
    FROM   dwh_performance.rtl_mc_loc_item_wk_rms_stock ST
    WHERE  FIN_YEAR_NO = G_FIN_YEAR_NO
    AND    FIN_WEEK_NO = G_FIN_WEEK_NO;
    
    commit; 
-----------------------------------------------------------------------------------
   l_text := 'DELETE COMPLETED - START ROLLUP:- ';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   INSERT /*+ APPEND */ INTO dwh_performance.rtl_mc_loc_item_wk_rms_stock rtl_liwrs 
...
--MC--          
          LID.	INBND_EXCL_CUST_ORD_COST_LOCAL	AS	INBND_EXCL_CUST_ORD_COST_LOCAL	,
          LID.	INBND_EXCL_CUST_ORD_SELL_LOCAL	AS	INBND_EXCL_CUST_ORD_SELL_LOCAL	,
          LID.	INBND_INCL_CUST_ORD_COST_LOCAL	AS	INBND_INCL_CUST_ORD_COST_LOCAL	,
          LID.	INBND_INCL_CUST_ORD_SELL_LOCAL	AS	INBND_INCL_CUST_ORD_SELL_LOCAL	,
          LID.	SIT_COST_LOCAL	AS	SIT_COST_LOCAL	,
          LID.	SIT_SELLING_LOCAL	AS	SIT_SELLING_LOCAL	,
          LID.	SIT_FR_COST_LOCAL	AS	SIT_FR_COST_LOCAL	,
          LID.	SIT_MARGIN_LOCAL	AS	SIT_MARGIN_LOCAL	,
          LID.	SOH_COST_LOCAL	AS	SOH_COST_LOCAL	,
          LID.	SOH_SELLING_LOCAL	AS	SOH_SELLING_LOCAL	,
          LID.	SOH_FR_COST_LOCAL	AS	SOH_FR_COST_LOCAL	,
          LID.	SOH_MARGIN_LOCAL	AS	SOH_MARGIN_LOCAL	,
          LID.	FRANCHISE_SOH_MARGIN_LOCAL	AS	FRANCHISE_SOH_MARGIN_LOCAL	,
          LID.	BOH_SELLING_LOCAL	AS	BOH_SELLING_LOCAL	,
          LID.	BOH_COST_LOCAL	AS	BOH_COST_LOCAL	,
          LID.	BOH_FR_COST_LOCAL	AS	BOH_FR_COST_LOCAL	,
          LID.	CLEAR_SOH_COST_LOCAL	AS	CLEAR_SOH_COST_LOCAL	,
          LID.	CLEAR_SOH_SELLING_LOCAL	AS	CLEAR_SOH_SELLING_LOCAL	,
          LID.	CLEAR_SOH_MARGIN_LOCAL	AS	CLEAR_SOH_MARGIN_LOCAL	,
          LID.	CLEAR_SOH_FR_COST_LOCAL	AS	CLEAR_SOH_FR_COST_LOCAL	,
          LID.	REG_SOH_SELLING_LOCAL	AS	REG_SOH_SELLING_LOCAL	,
          LID.	REG_SOH_COST_LOCAL	AS	REG_SOH_COST_LOCAL	,
          LID.	REG_SOH_FR_COST_LOCAL	AS	REG_SOH_FR_COST_LOCAL	,
          LID.	REG_SOH_MARGIN_LOCAL	AS	REG_SOH_MARGIN_LOCAL	 
     
   from   rtl_mc_loc_item_dy_rms_stock lid,
          dim_calendar dc

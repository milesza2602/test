--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_714E_AZT_SLAB_LP49
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_714E_AZT_SLAB_LP49" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2014
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_714E_AZT_SLAB_LP49';
...
begin
for i in  1 ..50   loop  -- 50 months back
  SELECT ADD_MONTHS(TRUNC(l_month, 'MM'),-1) + 6, ADD_MONTHS(TRUNC(l_month, 'MM'), 0) + 5,to_char(l_month,'YYYYMM')
  into g_previous_month_day_7, g_current_month_day_6,g_date_yyyymm FROM dual;
  dbms_output.put_line(i||' := g_month_start_6 : ' ||g_previous_month_day_7||' and g_current_month_day_7 : '||g_current_month_day_6||' for extract : ' ||g_date_yyyymm );
  b_extract;
...
--**************************************************************************************************
  ...
  exception
     ...
     when others then
       ...
       p_success := false;
       RAISE;
end WH_PRF_CORP_714E_AZT_SLAB_LP49;

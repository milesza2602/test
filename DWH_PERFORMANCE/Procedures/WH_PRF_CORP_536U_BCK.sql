--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_536U_BCK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_536U_BCK" (
...
--  commit;
  ...
  select  /*+ PARALLEL(lid,6)  PARALLEL(z,6)  */
          ...
          sum(nvl(fd_num_cust_avail_adj,0)) fd_num_cust_avail_adj, -- Chg4256
          sum(nvl(num_facings,0)) num_facings -- Chg4256

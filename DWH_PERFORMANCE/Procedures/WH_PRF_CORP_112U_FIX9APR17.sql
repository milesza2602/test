--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_112U_FIX9APR17
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_112U_FIX9APR17" (p_forall_limit in integer,p_success out boolean) as
--
-- USED FOR datafix 9 april 2017 - reprocessed previous days' batch - wlyttle
--**************************************************************************************************
--  Date:        Sept 2008
--  Author:      Sean Le Roux
...
g_date_MIN1               date          := trunc(sysdate);
...
cursor c_fnd_rtl_loc_item_dy_rms_sale is
   ...
   from   fnd_rtl_loc_item_dy_rms_sale fnd_lid,
          ...
          dim_department dd
   where  fnd_lid.last_updated_date  BETWEEN g_date_MIN1 and G_DATE AND  -- RUN ON 9 APRIL 2017 FOR BATCHDATE=8APRIL 2017 AS DATAFIX
...
--**************************************************************************************************
    ...
    g_date_MIN1  := G_DATE - 1; -- DATAFIX 9 APRIL 2017
--    g_date := '21 APR 2009'; --For testing only
    l_text := 'BATCH DATE BEING PROCESSED - '||g_date_MIN1||' TO '||G_DATE;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
END WH_PRF_CORP_112U_FIX9APR17;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_500U_REUSE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_500U_REUSE" (p_forall_limit in integer,p_success out boolean) as
...
for g_sub in 0..14 loop --REUSE OF ITEMS
...
INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.RTL_LOC_ITEM_WK_RMS_DENSE rtl_liwrd
   ...
   where  lid.post_date         = dc.calendar_date and
          lid.post_date          between g_start_date and g_end_date
                 and      lid.sk1_item_no IN  -- REUSE OF ITEMS THAT MOVED C&H TO FOODS
                (
                182035,
                552207,
                540204,

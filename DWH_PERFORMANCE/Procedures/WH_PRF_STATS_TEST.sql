--------------------------------------------------------
--  DDL for Procedure WH_PRF_STATS_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_STATS_TEST" 
...
l_module_name            sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_STATS_TEST';
...
cursor c_rtl_loc_item_dy_rp_catlg is
   select r.*, trunc(sysdate) active_from_date, trunc(sysdate) active_to_date 
   from   rtl_loc_item_dy_rp_catlg r
   where  1 = 2;
...
--**************************************************************************************************
   ...
   '       0 reg_sales_catlg, '||
   '       stk.reg_soh_qty reg_soh_qty_catlg, '||
   '       stk.reg_soh_selling reg_soh_selling_catlg, '||

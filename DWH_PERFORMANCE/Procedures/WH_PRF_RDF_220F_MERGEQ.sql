--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_220F_MERGEQ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_220F_MERGEQ" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Dec 2010
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_220F_M';
...
begin
 ...
 --   l_text := 'DO A COUNT AT START, LETS TRY enable THE DIRTY BLOCK CLEANOUT ';
 --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
   ...
   merge /*+ parallel(rdf_mart,5) */  into RTL_LOC_ITEM_RDF_DYFCST_L2 rdf_mart 
   using (   
       with pdate as (select /*+ parallel(fcstd,4) */ unique post_date from RTL_LOC_ITEM_RDF_DYFCST_L2 fcstd where fcstd.last_updated_date = g_date)

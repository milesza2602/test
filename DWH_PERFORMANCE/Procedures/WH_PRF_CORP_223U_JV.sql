--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_223U_JV
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_223U_JV" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - tmp_mart_foods_wkly_snpsht_tq
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  21 feb 2014 add pack_item_ind
--
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_223U_JV';
...
--**************************************************************************************************
    ...
    g_date := '16/JUL/15';   --'09/JUL/15';
    l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    g_last_week := '10/JUL/15';   --'03/JUL/15';
...
from   dim_calendar
where  calendar_date = (select last_wk_end_date from dim_control_report);
...
*/
l_53_ly_day7_date := '13/JUL/14';
l_53_ly_day1_date := '07/JUL/14' ;
l_53_ly_fin_week_no := 2;
l_53_ly_fin_year_no := 2015;
l_6wks_wk6_yr := 2015 ; l_6wks_wk6 := 49;
l_6wks_wk5_yr := 2015 ; l_6wks_wk5 := 50;
l_6wks_wk4_yr := 2015 ; l_6wks_wk4 := 51;
l_6wks_wk3_yr := 2015 ; l_6wks_wk3 := 52;
l_6wks_wk2_yr := 2016 ; l_6wks_wk2 := 1;
l_6wks_wk1_yr := 2016 ; l_6wks_wk1 := 2;
l_this_wk_start_date  := '14/JUL/15';
l_last_wk_end_date := '12/JUL/15';
l_last_wk_start_date := '06/JUL/15';
l_ytd_end_date := '12/JUL/15';
l_ytd_start_date := '06/JUL/15';
l_end_6wks_date := '12/JUL/15';
l_less_days := 53;
l_day_no := 5;
l_start_8wks_date := '25/MAY/15';
l_start_6wks_date := '01/JUN/15';
l_6wks_wk1_yr := 2016; l_6wks_wk1 := 2;
l_6wks_wk6_yr := 2015; l_6wks_wk6 := 49;
l_last_wk_fin_year_no := 2016;
l_next_wk_fin_year_no := 2016;
l_next_wk_fin_week_no := 4;
l_fin_week_no := 3;
l_last_week_no := 2;
l_last_fin_year := 2015;
l_fin_year_no := 2016;
g_date := '16/JUL/15';
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table w6005682.tmp_mart_foods_wkly_snpsht_tq');
...
INSERT /*+ APPEND parallel(mart,6) */ INTO w6005682.tmp_mart_foods_wkly_snpsht_tq mart
with item_list as (
...
--like_for_like_loc_ty as (
--   select unique sk1_location_no, sk1_item_no, like_for_like_ind like_for_like_ind_ty
--   from like_for_like_ty, item_list
--),
...
rdf_fcst_measures as (    --- MOVED TO DAY TABLE BELOW
select /*+ PARALLEL(f,4) FULL(f) */
       f.sk1_location_no,
       f.sk1_item_no,
...
select /*+ PARALLEL(f,4) FULL(f) */
       ...
       nvl(sum(f.sales_qty),0) sales_qty_ly_ytd
from   rtl_loc_item_wk_rms_dense f, item_list il, store_list sl, week_list wl
where f.sk1_item_no     = il.sk1_item_no
  and f.sk1_location_no = sl.sk1_location_no
...
--Promotion data
prom_values as (
select /*+ PARALLEL(p,4) FULL(p) */
       p.sk1_location_no,
       p.sk1_item_no,
       max(dp.prom_type) prom_type,
       max(wreward_ind) wreward_ind
       --nvl(sum(p.prom_sales),0) prom_sales,
       --nvl(sum(p.prom_sales_qty),0) prom_sales_qty
...
and p.sk1_prom_period_no = 9749661
--and (p.prom_sales is not null or p.prom_sales_qty is not null)
...
--       f.sk1_item_no,
--       nvl(f.BOH_ADJ_QTY_FLT,0) BOH_ADJ_QTY_FLT
--from   rtl_loc_item_dy_catalog f, item_list il, dim_item_uda iu, store_list_area sl
--where f.sk1_item_no   = il.sk1_item_no
--and f.sk1_item_no     = iu.sk1_item_no     --full outer join ???
--and f.sk1_location_no = sl.sk1_location_no
...
where f.sk1_location_no = sl.sk1_location_no
  and f.sk1_item_no = il.sk1_item_no
  and f.post_date = l_53_ly_day1_date   --WK 53 CODE--
...
select /*+ PARALLEL(f,4) FULL(f) */
       ...
       --f.est_ll_dc_cover_cases stock_dc_cover_cases
from rtl_depot_item_dy f, item_list il, store_list sl
where f.sk1_location_no = sl.sk1_location_no
  and f.sk1_item_no = il.sk1_item_no
...
weeks_no_stock as (
select /*+ full(ros) parallel(ros,4) */ 
       f.sk1_location_no,
       f.sk1_item_no,
...
prom_type_set as (
select /*+ PARALLEL(prom,4)  full(prom) */
...
select /*+ full(ros) parallel(ros,4) */ 
       ros.sk1_location_no, ros.sk1_item_no, ros.avg_units_per_day rate_of_sale_avg_wk, ros.units_per_week rate_of_sale_sum_wk
  from rtl_loc_item_wk_rate_of_sale ros,
       store_list dl,
       item_list di
...
rate_of_sale_6wk as (
select /*+ full(ros) parallel(ros,4) */  
       ros.sk1_location_no, ros.sk1_item_no,
...
from rtl_loc_item_wk_rate_of_sale ros, item_list il, store_list sl
where ((ros.fin_year_no =  l_6wks_wk1_yr and ros.fin_week_no = l_6wks_wk1)
   or (ros.fin_year_no = l_6wks_wk2_yr and ros.fin_week_no = l_6wks_wk2)
   or (ros.fin_year_no = l_6wks_wk3_yr and ros.fin_week_no = l_6wks_wk3)
   or (ros.fin_year_no = l_6wks_wk4_yr and ros.fin_week_no = l_6wks_wk4)
...
select /*+ full(ros) parallel(ros,4) */  f.sk1_location_no, il.sk1_item_no, f.sk1_department_no,
sales_budget sales_budget_dept,
waste_cost_budget waste_cost_budget,
sales_margin_budget sales_margin_budget,
spec_dept_revenue_budget spec_dept_revenue_budget
from w6005682.rtl_loc_dept_wk_qs f,  store_list sl, item_list il
...
sum(spec_dept_revenue_budget) spec_dept_revenue_budget_ytd
from w6005682.rtl_loc_dept_wk_qs f, store_list sl, week_list wl, item_list il
...
select /*+ full(ros) parallel(ros,4) */ f.sk1_location_no, il.sk1_item_no, f.sk1_department_no,
sales_budget sales_budget_dept_ly,
waste_cost_budget waste_cost_budget_ly,
sales_margin_budget sales_margin_budget_ly,
spec_dept_revenue_budget spec_dept_revenue_budget_ly
from w6005682.rtl_loc_dept_wk_qs f,  store_list sl, item_list il
where f.fin_year_no   = l_53_ly_fin_year_no  --WK 53 CODE--
and f.fin_week_no     =  l_53_ly_fin_week_no
and f.sk1_location_no = sl.sk1_location_no
and f.sk1_department_no = il.sk1_department_no
--and (sales_budget <> 0 or waste_cost_budget <>0 or sales_margin_budget <>0)
order by f.sk1_location_no, il.department_no
...
sales_budget_ytd_values_ly as (
select /*+ full(ros) parallel(ros,4) */ f.sk1_location_no, il.sk1_item_no, f.sk1_department_no,
sum(sales_budget) sales_budget_dept_ytd_ly,
sum(spec_dept_revenue_budget) spec_dept_revn_budget_ytd_ly
from w6005682.rtl_loc_dept_wk_qs f, store_list sl, week_list wl, item_list il
where f.fin_year_no   = wl.ly_fin_year_no
and f.fin_week_no     = wl.ly_fin_week_no
and f.sk1_location_no = sl.sk1_location_no
and f.sk1_department_no = il.sk1_department_no
--and f.sales_budget <> 0
group by f.sk1_location_no, il.sk1_item_no, f.sk1_department_no
...
-- POS JV DATA
pos_jv as (
select /*+ PARALLEL(pos,4) full(pos) */ pos.sk1_location_no, pos.sk1_item_no, pos.spec_dept_qty , pos.spec_dept_revenue 
  from dwh_performance.rtl_loc_item_wk_pos_jv pos,
       store_list dl,
       item_list di
where pos.fin_year_no     = l_last_wk_fin_year_no
 and  pos.fin_week_no     = l_last_week_no
 and  pos.sk1_location_no = dl.sk1_location_no
 and  pos.sk1_item_no     = di.sk1_item_no
) --SELECT * FROM POS_JV
,

pos_jv_ly as (
select /*+ PARALLEL(pos,4) full(pos) */ pos.sk1_location_no, pos.sk1_item_no, pos.spec_dept_qty spec_dept_qty_ly, pos.spec_dept_revenue spec_dept_revenue_ly
  from dwh_performance.rtl_loc_item_wk_pos_jv pos,
       store_list dl,
       item_list di
where pos.fin_year_no     = l_53_ly_fin_year_no
 and  pos.fin_week_no     = l_53_ly_fin_week_no
 and  pos.sk1_location_no = dl.sk1_location_no
 and  pos.sk1_item_no     = di.sk1_item_no
) --select * from pos_jv_ly;
,

pos_jv_ytd as (
select /*+ PARALLEL(pos,4) full(pos) */ 
       pos.sk1_location_no, pos.sk1_item_no, 
       sum(pos.spec_dept_qty) spec_dept_qty_ytd, 
       sum(pos.spec_dept_revenue) spec_dept_revenue_ytd
  from dwh_performance.rtl_loc_item_wk_pos_jv pos,
       store_list dl,
       item_list di,
       week_list wl
where pos.fin_year_no     = wl.fin_year_no
 and  pos.fin_week_no     = wl.fin_week_no
 and  pos.sk1_location_no = dl.sk1_location_no
 and  pos.sk1_item_no     = di.sk1_item_no 
 group by pos.sk1_location_no, pos.sk1_item_no
) --select * from pos_jv_ytd;
,

pos_jv_ly_ytd as (
select /*+ PARALLEL(pos,4) */ pos.sk1_location_no, pos.sk1_item_no, 
   sum(pos.spec_dept_qty) spec_dept_qty_ly_ytd, 
   sum(pos.spec_dept_revenue) spec_dept_revenue_ly_ytd
  from dwh_performance.rtl_loc_item_wk_pos_jv pos,
       store_list dl,
       item_list di, 
       week_list wl
where pos.fin_year_no     = wl.ly_fin_year_no
 and  pos.fin_week_no     = wl.ly_fin_week_no
 and  pos.sk1_location_no = dl.sk1_location_no
 and  pos.sk1_item_no     = di.sk1_item_no
  group by pos.sk1_location_no, pos.sk1_item_no
)  --select * from pos_jv_ly_ytd;
,
...
nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                           f2.sk1_location_no),
                                           f3.sk1_location_no),
                                           f4.sk1_location_no),
                                           f6.sk1_location_no),
                                           ...
                                           f22.sk1_location_no),
                                           f23.sk1_location_no),
                                           f24.sk1_location_no),    --START POS JV
                                           f25.sk1_location_no),
                                           f26.sk1_location_no),
                                           f27.sk1_location_no),   -- END POS JV
                                           f28.sk1_location_no),
                                           f29.sk1_location_no)

sk1_location_no,
nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                          f2.sk1_item_no),
                                          f3.sk1_item_no),
                                          f4.sk1_item_no),
                                          f6.sk1_item_no),
                                          ...
                                          f22.sk1_item_no),
                                          f23.sk1_item_no),
                                          f24.sk1_item_no),     -- START POS JV
                                          f25.sk1_item_no),
                                          f26.sk1_item_no),
                                          f27.sk1_item_no),      -- END POS JV
                                          f28.sk1_item_no),
                                          f29.sk1_item_no)
...
f9.stock_cases,
f9.expired_stock,
f9.expiring_stock_wk1_3,
cc.est_ll_short_cases_av,
f9.stock_dc_cover_cases,    --was dc1
f9.DC_OUT_OF_STOCK,
ws.weeks_out_of_stock,
ps.wreward_ind,

f17.rate_of_sale_avg_wk,
f17.rate_of_sale_sum_wk,
f18.rate_of_sale_6wk_sum,
f16.promo_type_MU,
f16.promo_type_SK,
f16.promo_type_TH,
f16.promo_type_MM,
...
--f20.like_for_like_ind_ty
--f21.like_for_like_ind_ly
--f24.rate_of_sale_6wk_avg,
f22.waste_cost_budget,
f22.sales_budget_dept,
f22.sales_margin_budget,
f22.spec_dept_revenue_budget,
f23.sales_budget_dept_ytd,

f23.spec_dept_revenue_budget_ytd,   -- START POS JV / SPECIAL DEPTS
f24.spec_dept_qty,              
f24.spec_dept_revenue,
f25.spec_dept_qty_ly,
f25.spec_dept_revenue_ly,
f26.spec_dept_qty_ytd,
f26.spec_dept_revenue_ytd,
f27.spec_dept_qty_ly_ytd,
f27.spec_dept_revenue_ly_ytd,    
f28.spec_dept_revenue_budget_ly,
f28.sales_budget_dept_ly,
f29.spec_dept_revn_budget_ytd_ly,
f29.sales_budget_dept_ytd_ly        -- END POS JV / SPECIAL DEPTS
...
full outer join rms_sparse_measures      f6 on nvl(nvl(nvl(nvl(f0.sk1_location_no, f1.sk1_location_no),
                                                                                       f2.sk1_location_no),
                                                                                       f3.sk1_location_no),
...
full outer join rms_dense_ytd_measures  fy0 on nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
...
full outer join catalog_measures_ytd fy1 on nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join catalog_measures_ly  fy2  on nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join po_supchain_week fy3 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
 ...
 full outer join item_price_day7 ip1 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
 ...
 full outer join item_price_day7_ly ip2 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
 ...
 full outer join po_supchain_ytd po2 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
 ...
 full outer join po_6wk_avg po3 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
 ...
 full outer join prom_values ps on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
 ...
 full outer join corrected_sales cs on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   po2.sk1_location_no),
                                                                                   po3.sk1_location_no),
                                                                                   ps.sk1_location_no)= cs.sk1_location_no
                                 and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   po2.sk1_item_no),
                                                                                   po3.sk1_item_no),
                                                                                   ps.sk1_item_no) = cs.sk1_item_no


full outer join rms_dense_ytd_measures_ly dly on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   po3.sk1_location_no),
                                                                                   ps.sk1_location_no),
                                                                                   cs.sk1_location_no) = dly.sk1_location_no
                                 and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   po3.sk1_item_no),
                                                                                   ps.sk1_item_no),
                                                                                   cs.sk1_item_no) = dly.sk1_item_no


full outer join rms_sparse_measures_ly f7 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join rms_sparse_ytd_measures f8 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join cover_cases cc on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   dly.sk1_location_no),
                                                                                   f7.sk1_location_no),
                                                                                   f8.sk1_location_no) = cc.sk1_location_no
                                 and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   dly.sk1_item_no),
                                                                                   f7.sk1_item_no),
                                                                                   f8.sk1_item_no) = cc.sk1_item_no

full outer join rms_stock stk on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   f7.sk1_location_no),
                                                                                   f8.sk1_location_no),
                                                                                   cc.sk1_location_no) = stk.sk1_location_no
                                 and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   f7.sk1_item_no),
                                                                                   f8.sk1_item_no),
                                                                                   cc.sk1_item_no) = stk.sk1_item_no

full outer join rms_stock_ly stk_ly on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join weeks_no_stock ws on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   cc.sk1_location_no),
                                                                                   stk.sk1_location_no),
                                                                                   stk_ly.sk1_location_no) = ws.sk1_location_no
                        ...
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   cc.sk1_item_no),
                                                                                   stk.sk1_item_no),
                                                                                   stk_ly.sk1_item_no)= ws.sk1_item_no


full outer join depot_ly dl on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join dc_no_stock_day1 f9 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join catalog_measures1 f10 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join rdf_fcst_measures1 f11 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join prom_type_set f16 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   f9.sk1_location_no),
                                                                                   f10.sk1_location_no),
                                                                                   f11.sk1_location_no) = f16.sk1_location_no
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   f9.sk1_item_no),
                                                                                   f10.sk1_item_no),
                                                                                   f11.sk1_item_no) = f16.sk1_item_no

full outer join rate_of_sale f17 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join rate_of_sale_6wk f18 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join sales_budget_values f22 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                                                                                   ...
                                                                                   f16.sk1_location_no),
                                                                                   f17.sk1_location_no),
                                                                                   f18.sk1_location_no) = f22.sk1_location_no
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   ...
                                                                                   f16.sk1_item_no),
                                                                                   f17.sk1_item_no),
                                                                                   f18.sk1_item_no) = f22.sk1_item_no

full outer join sales_budget_ytd_values f23 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join pos_jv f24 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
full outer join pos_jv_ly f25 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                        ...
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   f2.sk1_item_no),
                                                                                   f3.sk1_item_no),
                                                                                   f4.sk1_item_no),
                                                                                   f6.sk1_item_no),
...
full outer join pos_jv_ytd f26 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                        ...
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   f2.sk1_item_no),
                                                                                   f3.sk1_item_no),
                                                                                   f4.sk1_item_no),
                                                                                   f6.sk1_item_no),
...
full outer join pos_jv_ly_ytd f27 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                        ...
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   f2.sk1_item_no),
                                                                                   f3.sk1_item_no),
                                                                                   f4.sk1_item_no),
                                                                                   f6.sk1_item_no),
...
full outer join sales_budget_values_ly f28 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                        ...
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   f2.sk1_item_no),
                                                                                   f3.sk1_item_no),
                                                                                   f4.sk1_item_no),
                                                                                   f6.sk1_item_no),
...
full outer join sales_budget_ytd_values_ly f29 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
                        ...
                        and nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                                                                   f2.sk1_item_no),
                                                                                   f3.sk1_item_no),
                                                                                   f4.sk1_item_no),
                                                                                   f6.sk1_item_no),
...
-- START POS JV
atg.spec_dept_qty,              
atg.spec_dept_revenue,
atg.spec_dept_qty_ly,
atg.spec_dept_revenue_ly,
atg.spec_dept_qty_ytd,
atg.spec_dept_revenue_ytd,
atg.spec_dept_qty_ly_ytd,
atg.spec_dept_revenue_ly_ytd,

atg.sales + atg.spec_dept_revenue,                        -- market_sales
atg.sales_ly + atg.spec_dept_revenue_ly,                  -- market_sales_ly
atg.sales_ytd + atg.spec_dept_revenue_ytd,                -- market_sales_ytd
atg.sales_ly_ytd + atg.spec_dept_revenue_ly_ytd,          -- market_sales_ytd_ly
atg.sales_qty + atg.spec_dept_qty,                        -- market_sales_qty
atg.sales_qty_ly + atg.spec_dept_qty_ly,                  -- market_sales_qty_ly
atg.sales_qty_ytd + atg.spec_dept_qty_ytd,                -- martket_sales_qty_ytd
atg.sales_qty_ly_ytd + atg.spec_dept_qty_ly_ytd,          -- market_sales_qty_ytd_ly

atg.spec_dept_revenue_budget,                             -- outsource_budget
atg.spec_dept_revenue_budget_ytd,                         -- outsource_budget_ytd
--atg.sales_budget_dept_ly,
--atg.sales_budget_dept_ytd_ly,
nvl(atg.sales_budget_dept,0)        + nvl(atg.spec_dept_revenue_budget,0),          -- market_budget
nvl(atg.sales_budget_dept_ytd,0)    + nvl(atg.spec_dept_revenue_budget_ytd,0),   -- market budget_ytd
nvl(atg.sales_budget_dept_ly,0)     + nvl(atg.spec_dept_revenue_budget_ly,0),          -- market_budget_ly
nvl(atg.sales_budget_dept_ytd_ly,0) + nvl(atg.spec_dept_revn_budget_ytd_ly,0)   -- market budget_ytd_ly

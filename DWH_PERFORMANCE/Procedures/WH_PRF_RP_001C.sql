--------------------------------------------------------
--  DDL for Procedure WH_PRF_RP_001C
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RP_001C" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        November 2008
--  Author:      Alfonso Joshua
--  Purpose:     Create the daily CHBD fast item catalog table with sales in the performance layer
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RP_001C';
...
-- main query
   select cat.sk1_item_no,
          cat.sk1_location_no,
          cat.sk1_product_category_no,
          cat.sk1_criticality_ranking_no,
          cat.sk1_top_mdm_item_catg_no,
          cat.rpl_ind,
          cat.sk1_avail_uda_value_no,
          cat.post_date,
          nvl(sal.reg_sales_qty,0)    reg_sales_qty,
          nvl(sal.reg_sales,0)        reg_sales
   from   rtl_loc_item_dy_rp_catlg cat
   join   dim_item di on
...
--   where  cat.last_update_date              = g_date;

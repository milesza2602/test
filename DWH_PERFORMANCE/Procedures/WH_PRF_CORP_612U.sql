--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_612U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_612U" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - dwh_performance.rtl_loc_item_so_sdn
...
g_rec_out            rtl_loc_item_so_sdn%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_612U';
...
type tbl_array_i is table of rtl_loc_item_so_sdn%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_item_so_sdn%rowtype index by binary_integer;
...
cursor c_fnd_rtl_shipment is
  select /*+ PARALLEL(a,4) */
        ...
        max(a.reg_rsp) as reg_rsp,
        sum(a.sdn_qty) as sdn_qty
   from fnd_rtl_shipment a, dim_item b, dim_location c
   --, fnd_jdaff_dept_rollout d   --must still add live dept check !!
   where a.last_updated_date = g_date
     and a.item_no = b.item_no
     and a.to_loc_no = c.location_no
     and b.business_unit_no = 50
--     and b.department_no = d.department_no
--     and d.department_live_ind = 'Y'
     and a.receive_date is not null
     group by c.sk1_location_no, b.sk1_item_no, a.receive_date
     order by sk1_location_no, sk1_item_no;

g_rec_in             c_fnd_rtl_shipment%rowtype;

-- For input bulk collect --
type fnd_array is table of c_fnd_rtl_shipment%rowtype;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_item_so_sdn values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).receive_date;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_loc_item_so_sdn
       set    received_qty                    = a_tbl_update(i).received_qty,
       ...
       where  sk1_location_no                 = a_tbl_update(i).sk1_location_no
         ...
         and  receive_date                    = a_tbl_update(i).receive_date;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_update(g_error_index).receive_date;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).sk1_location_no = g_rec_out.sk1_location_no and
            ...
            a_tbl_insert(i).receive_date    = g_rec_out.receive_date then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF rtl_loc_item_so_sdn EX fnd_rtl_shipment STARTED AT '||

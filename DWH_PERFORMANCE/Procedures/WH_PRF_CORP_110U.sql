--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110U" (p_forall_limit in integer,p_success out boolean, p_from_loc_no in integer,p_to_loc_no in integer
) as
--**************************************************************************************************
--  Date:        Sept 2008
...
--**************************************************************************************************
  ...
  if g_to_loc_no   <> 0 then     
    ...
    MERGE /*+ parallel(rtl_lidrd,4) */ INTO RTL_LOC_ITEM_DY_RMS_DENSE rtl_lidrd
    USING
    (
    ...
    select  /*+ full(di) full(dl) full(dlh) full(dih) parallel(vr,4) full(fnd_li) parallel(fnd_li,4) */
              ...
              -- sales_incl_vat
              --case when fnd_li.tax_perc is null then                                      -- VAT rate change
              case when fi_vr.vat_rate_perc is null then                                    -- VAT rate change
                  case when dl.vat_region_no = 1000 then
                  ...
                  round(nvl(sales,0) * (100 + fi_vr.vat_rate_perc) / 100,2)                 -- VAT rate change
    ...
    --          end as store_intake_fr_cost,
       ...
       where  fnd_lid.last_updated_date = g_date 
              ...
              --and fnd_lid.item_no = 20105211
              --and fnd_lid.location_no = 2501

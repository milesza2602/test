--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_265U_BK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_265U_BK" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_265U_BK';
...
,  
  ...
  ( select DP.PO_NO, DP.SK1_PO_NO, DP.ORIG_APPROVAL_DATE, DP.PO_STATUS_CODE, DP.NOT_BEFORE_DATE, DP.NOT_AFTER_DATE, DP.INTO_STORE_DATE, DP.LAST_UPDATED_DATE 
     from DIM_PURCHASE_ORDER DP where SK1_PO_NO in (select distinct SK1_PO_NO from w7104429.temp_po_list)) 
  ,
...
from ITEM_LIST a, 
 ...
 where G.RECEIVE_DATE = D.CALENDAR_DATE 
  and G.RECEIVE_DATE >= '24 Aug 2016'
  and G.PO_NO = F.PO_NO 

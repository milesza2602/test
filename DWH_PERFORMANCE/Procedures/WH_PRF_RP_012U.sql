--------------------------------------------------------
--  DDL for Procedure WH_PRF_RP_012U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RP_012U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - rtl_loc_item_wk_rp_catlg
--               Output - rtl_loc_sc_wk_rp_catlg
...
g_count              number        :=  0;
g_rec_out            rtl_loc_sc_wk_rp_catlg%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RP_012U';
...
type tbl_array_i is table of rtl_loc_sc_wk_rp_catlg%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_sc_wk_rp_catlg%rowtype index by binary_integer;
...
cursor c_rtl_loc_item_wk_rp_catlg is
   select   liw.sk1_location_no,
            ...
            max(nvl(liw.num_ch_catalog_ind,0))       as num_ch_catalog_ind,
...
g_rec_in                   c_rtl_loc_item_wk_rp_catlg%rowtype;
-- For input bulk collect --
type stg_array is table of c_rtl_loc_item_wk_rp_catlg%rowtype;
...
begin
   ...
   g_rec_out.num_ch_catalog_ind              := g_rec_in.num_ch_catalog_ind;
   ...
   g_rec_out.last_update_date                := trunc(sysdate);--g_date;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_sc_wk_rp_catlg values a_tbl_insert(i);
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_SC_WK_RP_CATLG EX FOUNDATION STARTED AT '||
...
--**************************************************************************************************
    ...
    l_text := 'START WEEK/YEAR OF ROLLUP IS:- '||g_start_year||' '||g_start_week;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--**************************************************************************************************
-- Bulk fetch loop controlling main program execution
--**************************************************************************************************
    open c_rtl_loc_item_wk_rp_catlg;
    fetch c_rtl_loc_item_wk_rp_catlg bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_rtl_loc_item_wk_rp_catlg;
--**************************************************************************************************

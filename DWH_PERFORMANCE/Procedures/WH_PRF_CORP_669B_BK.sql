--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_669B_BK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_669B_BK" 
                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        May 2009
--  Author:      M Munnik
--  Purpose:     Rollup from rtl_supchain_loc_item_dy to rtl_supchain_loc_item_dy_prsup.
--               Difference between rtl_supchain_loc_item_dy and rtl_supchain_loc_item_dy_prsup -
--               The Supplier on rtl_supchain_loc_item_dy (wh_prf_corp_669u)
--               is the supplier as on the purchase order record.
--               The Supplier on rtl_supchain_loc_item_dy_prsup (this procedure)
--               is the primary supplier for the item, from dim_item.
--  Tables:      Input  - rtl_supchain_loc_item_dy
--               Output - rtl_supchain_loc_item_dy_prsup
...
g_rec_out            rtl_supchain_loc_item_dy_prsup%rowtype;
g_found              boolean;
g_date               date;
g_count              number        :=  0;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_669B_BK';
...
type tbl_array_i is table of rtl_supchain_loc_item_dy_prsup%rowtype index by binary_integer;
type tbl_array_u is table of rtl_supchain_loc_item_dy_prsup%rowtype index by binary_integer;
...
cursor c_spch_loc_item_dy_prsup is
   with chgrecs as (select   tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no
                    from     w7104429.rtl_supchain_loc_item_dy
                    where    last_updated_date = g_date
                    and     sk1_item_no <> 25329804
                    group by tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no
                    union
                    select   tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no
   ...
   from     chgrecs cr
   join     w7104429.rtl_supchain_loc_item_dy lid        on  lid.tran_date           = cr.tran_date
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into w7104429.rtl_supchain_loc_item_dy_prsup values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update w7104429.rtl_supchain_loc_item_dy_prsup
...
-- Check to see if record is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   w7104429.rtl_supchain_loc_item_dy_prsup
...
-- Records on rtl_supchain_loc_item_dy_prsup are a summary of rtl_supchain_loc_item_dy,
-- which is a summary of rtl_po_supchain_loc_item_dy.
-- Summary of rtl_supchain_loc_item_dy_prsup per tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no.
-- Therefore, when the not_before_date of a PO changes, it moves from one tran_date to another.
-- Records on rtl_supchain_loc_item_dy_prsup which belongs to the
...
-- Therefore, the summarised records are first deleted from rtl_supchain_loc_item_dy_prsup.
--**************************************************************************************************
procedure delete_recs as
begin

    delete from w7104429.rtl_supchain_loc_item_dy_prsup r
    where  exists(select d.tran_date, d.sk1_location_no, d.sk1_item_no, d.sk1_supply_chain_no
...
begin
    ...
    l_text := 'ROLLUP OF rtl_supchain_loc_item_dy_prsup EX SUPCHAIN_LOC_ITEM_DY STARTED '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_660U_AJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_660U_AJ" (p_forall_limit in integer,p_success out boolean,p_start_date in date,p_end_date in date) as
--**************************************************************************************************
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_660U_AJ';
...
--    execute immediate 'truncate table dwh_performance.temp_po_list';
    execute immediate 'truncate table dwh_datafix.aj_temp_po_list';
...
--**************************************************************************************************
    g_param_ind := 1;
     insert /*+ append */ into dwh_datafix.aj_temp_po_list
                       with po_suppl_chain as
                             ...
                             where  (po.not_after_date between '01 sep 15' and '30 sep 15' 
                                  or po.not_before_date between '01 sep 15' and '30 sep 15')--po.last_updated_date = g_date
                     ...
                     union
                             ...
                             where  s.last_updated_date between '01 sep 15' and '30 sep 15' --s.last_updated_date = g_date
                              and  di.department_no in (40,64,88)

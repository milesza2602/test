--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_195U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_195U_LOAD" (p_forall_limit in integer,p_success out boolean) as
...
g_start_date         date          := '05 mar 18';
g_end_date           date          := '05 mar 18';

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_195U_LOAD';
...
cursor C_RTL_LOC_ITEM_DY_CATALOG is
    ...
    select /*+ parallel (lid,6) full (di) full (dl) full (diu) parallel (rms,6) */ 
          lid.sk1_item_no,
          lid.sk1_location_no,
          lid.calendar_date,
...
--          dc.fin_day_no,
    ...
    where lid.calendar_date in (select calendar_date from date_x) and
          lid.sk1_item_no                = 64474 and
          lid.sk1_location_no            = 643 ;
--          lid.sk1_item_no                = 231618 and
--          lid.sk1_location_no            = 435 ;
--          lid.calendar_date              = '04 mar 18';
...
-- end new code
--    dbms_output.put_line('fd_num_cust_catlg_adj = '||g_rec_out.fd_num_cust_catlg_adj);
   
   
   if g_rec_out.fd_num_cust_catlg_adj > 0 then -- Chg12260
      if g_rec_in.num_facings > 0 then
         if (g_rec_in.boh_adj_qty + abs(g_rec_in.waste_qty)) > g_rec_in.num_facings then 
            g_rec_out.fd_cust_avail := 1;
         else
            g_rec_out.fd_cust_avail := (g_rec_in.boh_adj_qty + abs(g_rec_in.waste_qty)) / g_rec_in.num_facings; 
         end if;
      elsif g_rec_in.num_shelf_life_days >= 3 then
         if g_rec_in.boh_adj_qty + abs(g_rec_in.waste_qty) < 3 then 
            g_rec_out.fd_cust_avail := 0; 
         else
            g_rec_out.fd_cust_avail := 1;
         end if;
      else
         g_rec_out.fd_cust_avail := g_rec_in.fd_num_avail_days_adj;
      end if;
   else
      g_rec_out.fd_cust_avail := 0;
   end if;
   
   dbms_output.put_line('fd_cust_avail = '||g_rec_out.fd_cust_avail);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    fd_cust_avail                   = a_tbl_update(i).fd_cust_avail,

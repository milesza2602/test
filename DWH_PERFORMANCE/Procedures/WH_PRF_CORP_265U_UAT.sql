--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_265U_UAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_265U_UAT" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_265U_UAT';
...
begin
    ...
    --  and fin_week_no in (1, 2, 3);
    
    G_FROM_DATE := '23/FEB/15';
    G_TO_DATE   := '08/MAR/15';
    
       G_FROM_DATE := '09/MAR/15';
    G_TO_DATE   := '29/MAR/15';
...
where G.RECEIVE_DATE = D.CALENDAR_DATE 
  ...
  and G.seq_no = 1
    ...
    --G.RECEIVE_DATE between G_FROM_DATE and G_TO_DATE  --= G_DATE

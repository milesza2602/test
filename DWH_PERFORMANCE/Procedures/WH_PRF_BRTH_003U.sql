--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_003U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_003U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_BRTH_003U';
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    execute immediate ('truncate table dwh_performance.temp_rtl_area_item_wk_price');
    commit;
    l_text := 'TRUNCATED table dwh_performance.temp_rtl_area_item_wk_price';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'temp_rtl_area_item_wk_price', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_performance.temp_rtl_area_item_wk_price';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'alter session enable parallel dml';
    
    INSERT /*+ APPEND */ INTO dwh_performance.temp_rtl_area_item_wk_price

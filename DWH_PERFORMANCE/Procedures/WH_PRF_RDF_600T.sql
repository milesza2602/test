--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_600T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_600T" 
...
--**************************************************************************************************
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_RDF_600T';
...
*/
  ...
  MERGE /*+ parallel (rdf_mart,8) */ into DWH_PERFORMANCE.RTL_LOC_ITEM_RDF_DYFCST_L2 rdf_mart using (
    WITH SELRDF
                  AS(
                  SELECT /*+ PARALLEL(rtl,8) FULL(rtl) PARALLEL(sys_ilv,8) FULL(sys_ilv) */

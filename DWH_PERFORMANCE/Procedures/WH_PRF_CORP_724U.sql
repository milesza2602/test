--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_724U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_724U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_724U';
...
cursor c_rtl_loc_item_wk_stockown_dcr is
   ...
   where    so.this_week_start_date >= g_this_week_start
...
--**************************************************************************************************
    ...
    l_text := 'START WEEK OF ROLLUP - '||g_this_week_start;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

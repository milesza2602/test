--------------------------------------------------------
--  DDL for Procedure WH_PRF_TRUNC_PARTN_MP_023U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_TRUNC_PARTN_MP_023U" (p_success out boolean) as
--**************************************************************************************************
--  Date:        Sep 2015
--  Author:      K. Lehabe
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_TRUNC_PARTN_MP_023U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'TRUNCATE CURRENT MONTH AND FUTURE PARTITIONS ON RTL_LOC_DEPT_MTH_PLAN_MP';
 ...
 begin
   ...
   for g_sub in 1 .. g_no_of_years
    ...
    g_partn_year := g_partn_year +  g_sub  ;
      ...
      trunc_stmt           :=  '  alter table dwh_performance.RTL_LOC_DEPT_MTH_PLAN_MP truncate partition  for (' || g_partn_year || ')  update global indexes';
...
procedure local_trunc_subpartn as
 ...
 -- For current month it will be fin_month_no + 0, then the next month Fin_month_no + 1, etc
      ...
      trunc_sub_stmt           :=  '  alter table dwh_performance.RTL_LOC_DEPT_MTH_PLAN_MP truncate SUBPARTITION  for (' || g_fin_year_no || ',' || g_subpartn_month||')  update global indexes';
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'TRUNCATE MONTHLY PARTIONS ON  RTL_LOC_DEPT_MTH_PLAN_MP'||' STARTED '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    ---Check how many years in  the future
        ...
        into g_no_of_years
        from  dwh_performance.RTL_LOC_DEPT_MTH_PLAN_MP
    ...
    l_text := 'TRUNCATE MONTLHY PARTITIONS ON RTL_LOC_DEPT_MTH_PLAN_MP ENDED' ||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
end WH_PRF_TRUNC_PARTN_MP_023U;

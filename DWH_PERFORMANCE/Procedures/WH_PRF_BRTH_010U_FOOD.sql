--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_010U_FOOD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_010U_FOOD" 
...
--**************************************************************************************************
    ...
    execute immediate ('truncate table dwh_datafix.rtl14_loc_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_datafix.rtl14_loc_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    INSERT /*+ APPEND */ INTO dwh_datafix.rtl14_loc_item_wk_bridgethorn
     select
                 st_store_type
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                ,0 units_per_week
     from     dwh_datafix.t14_rtl_area_item_wk_catalog raistk  ,
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                ,0 units_per_week
     from     dwh_datafix.t14_rtl_area_item_wk_stock raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
      group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                ,0 units_per_week
     from     dwh_datafix.t14_rtl_area_item_wk_sparse raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                , sum(nvl(units_per_week,0))
     from     dwh_datafix.T14_RATE_OF_SALE_WK roswk,
              dwh_foundation.fnd_pack_item_detail fpid
...
--                ,dl.area_no
     ...
     from     (select    (case when raistk.st_store_type = 'WW'
      ...
      from     dwh_datafix.t14_rtl_area_item_wk_price raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date )
...
--                ,dl.area_no
                ...
                ,sum(nvl(stores_with_sales,0)   )
                ,0 units_per_week
     from     dwh_datafix.t14_rtl_area_item_wk_dense raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
commit;
    ...
    execute immediate ('truncate table dwh_datafix.rtl13_loc_item_wk_bridgethorn');
    commit;
    l_text := 'TRUNCATED table dwh_datafix.rtl13_loc_item_wk_bridgethorn';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   'rtl13_loc_item_wk_bridgethorn', DEGREE => 8);
--    commit;
--    l_text := 'GATHER STATS on dwh_datafix.rtl13_loc_item_wk_bridgethorn';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    INSERT /*+ APPEND */ INTO dwh_datafix.rtl13_loc_item_wk_bridgethorn
     select
                 st_store_type
                ,(case when pack_item_no is null then item_no else pack_item_no end) ITEM_NO
                ,this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                ,0 units_per_week
     from     dwh_datafix.T13_rtl_area_item_wk_catalog raistk  ,
              dwh_performance.dim_product_status dps,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
       and dps.product_status_code = to_char(product_status_1_code)
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                ,0 units_per_week
     from     dwh_datafix.T13_rtl_area_item_wk_stock raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
      group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                ,0 units_per_week
     from     dwh_datafix.T13_rtl_area_item_wk_sparse raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date
...
--                ,dl.area_no
                ...
                ,0 stores_with_sales
                , sum(nvl(units_per_week,0))
     from     dwh_datafix.T13_RATE_OF_SALE_WK roswk,
              dwh_foundation.fnd_pack_item_detail fpid
     where roswk.item_no = fpid.item_no(+)
     group by (case when roswk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     roswk.item_no,fpid.pack_item_no,  roswk.this_week_start_date
...
--                ,dl.area_no
     ...
     from     (select    (case when raistk.st_store_type = 'WW'
                ...
                ,SUM(nvl(reg_rsp_excl_vat_avg,0)) reg_rsp_excl_vat_avg
                ,SUM(NVL(LOC_CNT,0)) LOC_CNT
      from     dwh_datafix.T13_rtl_area_item_wk_price raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date )
...
--                ,dl.area_no
                ...
                ,sum(nvl(stores_with_sales,0)   )
                ,0 units_per_week
     from     dwh_datafix.T13_rtl_area_item_wk_dense raistk,
              dwh_foundation.fnd_pack_item_detail fpid
     where raistk.item_no = fpid.item_no(+)
     group by (case when raistk.st_store_type = 'WW' THEN 'corporate stores' else 'franchise stores' end),
     raistk.item_no,fpid.pack_item_no,  raistk.this_week_start_date

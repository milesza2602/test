--------------------------------------------------------
--  DDL for Procedure WH_PRF_EXT_006U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_EXT_006U" 
...
--  Purpose:     ALLOCATION OPTIMIZATION PROJECT  --  sales AND STOCK  extract 6 weeks - july 2015
-- *********************************
-- SALES SQL BASE FOR 6 WEEKS BACK *
-- VERSION 3: USING CC AS MASTER   *
-- AND USING WK_DENSE AND WK STOCK             *
-- AS AT 17 JULY 2015              *
-- *********************************
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_EXT_006U';
    ...
    g_recs_inserted := 0;
     ...
     INSERT /*+ APPEND */
      into DWH_PERFORMANCE.TMP_LOC_ITEM_WK_ALOPT_SA
              ...
              SELDNS AS (SELECT                                                     --QST (ADDED 'SELDNS')
                                    /*+ PARALLEL(R,4) no_index (R) */
                                          SC.Fin_Year_No,
                                          SC.Fin_Week_No,
                                          SL.Location_No,
               ...
               SELECT                                                                     --QST (ADDED THIS WHOLE SELECT ..
              ...
              with selcal as
                ...
                SELECT
                                    /*+ PARALLEL(R,4) no_index (R) */
                                          ...
                                          NVL(SOH_Qty,0) - NVL(CLEAR_SOH_Qty,0) SOH_Qty      --QST 
                                  FROM SELLOC SL,
                                        SELITM SI,
                                        SELCAL SC,
                                  ...
                                  AND SI.Sk1_Item_No     = R.Sk1_Item_No
                                  --AND NVL(SOH_Qty,0) > 0
                                  AND (NVL(SOH_Qty,0) - NVL(CLEAR_SOH_Qty,0)) > 0             --QST
;

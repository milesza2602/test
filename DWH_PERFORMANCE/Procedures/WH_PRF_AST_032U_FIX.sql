--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_032U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_032U_FIX" 
...
l_module_name           sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_032U_FIX';
...
--**************************************************************************************************
   ...
   g_6wk_bck_start_date := '01 jul 13';
   g_74wk_fwd_end_date := '01 jun 14';
...
*/
   ...
   g_prev_week_end_date := '01 jun 14';
   select ly_fin_week_no, ly_fin_year_no, ly_calendar_date
...
--   execute immediate 'truncate table dwh_performance.mart_ch_ast_chn_sc_wk';
   execute immediate 'alter session enable parallel dml';
      ...
      insert /*+ APPEND parallel (mart,6) */ into dwh_performance.mart_ch_ast_chn_sc_wk mart
...
-- Original Plan - extract2
      orig_data as (
      select /*+ full (orig) parallel (orig,6) */
             orig.sk1_chain_no,
...
-- Actualised Plan - extract3  -- must change to include prom_reg (qty and sell)
      act_data as (
      select /*+ full (act) parallel (act,6) */
             act.sk1_chain_no,
...
-- Pre-Actualised Plan - extract4
      preact_data as (
      select /*+ full (preact) parallel (preact,6) */
             preact.sk1_chain_no,
...
-- Approved Chain StyleColour - extract5
      aplnchain_data as (
      select /*+ full (aplnch) parallel (aplnch,6) */
             aplnch.sk1_chain_no,
...
-- Original Chain StyleColour- - extract6
      origchain_data as (
      select /*+ full (oplnch) parallel (oplnch,6) */
             oplnch.sk1_chain_no,
...
-- Actualised Chain StyleColour - extract7
      actchain_data as (
      select /*+ full (actch) parallel (actch,6) */
             actch.sk1_chain_no,
...
-- Pre-Actualised Chain StyleColour - extract8
      preactchain_data as (
      select /*+ full (preactch) parallel (preactch,6) */
...
-- LY Original Plan - extract9
      ...
      ly_orig_data as (
      select /*+ full (lyorig) parallel (lyorig,6) */
             lyorig.sk1_chain_no,
...
--             'W'||(lyorig.fin_year_no + 1)||lyorig.fin_week_no as fin_week_code,
      ...
      where  lyorig.sk1_plan_type_no  = 62
       ...
       and   cal.fin_day_no         = 3
     group by lyorig.sk1_chain_no, lyorig.sk1_style_colour_no, cal.fin_year_no, cal.fin_week_no, 'W'||(cal.fin_year_no)||cal.fin_week_no),
-- LY Actualised Plan - extract10
      ...
      ly_act_data as (
      select /*+ full (lyact) parallel (lyact,6) */
             lyact.sk1_chain_no,
...
-- Actual RMS Bought - extract11
      rms_act_data as (
         select /*+ full (rmsact) parallel (rmsact,6) */
             rmsact.sk1_chain_no,
      ...
      rms_stock_data as (
      select /*+ full (rmsact) index (stk PK_P_RTL_LC_SC_WK_RMS_STCK) */
             act.sk1_chain_no,

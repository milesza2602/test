--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_046T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_046T" (p_forall_limit in integer,p_success out boolean) as
                                    --"WH_PRF_CORP_046T" (p_forall_limit in integer,p_success out boolean, p_from_loc_no in integer,p_to_loc_no in integer) as
--**************************************************************************************************
--  Date:        Sept 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_046T';
...
begin
   ...
   from   dwh_foundation.fnd_location_item fli,
          ...
          DWH_FOUNDATION.STG_CATMAN_LOCATION_ITEM_CPY STG
   where  fli.item_no                = di.item_no  and
          ...
          fli.primary_supplier_no    = ds.supplier_no and        -- TD-491
           fli.last_updated_date      >= '27/JUN/16' 
       --   and di.item_no = 6009171051211
       --   and dl.location_no = 239
          and stg.item_no = di.item_no
          and stg.location_no = dl.location_no
          and stg.this_wk_catalog_ind = 0 and stg.next_wk_catalog_ind = 0

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_004A_X
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_004A_X" (
   ...
   l_text := 'Running GATHER_TABLE_STATS ON TEMP_S4S_LOC_EMP_DY_PART1';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'TEMP_S4S_LOC_EMP_DY_PART1', DEGREE => 8);
...
---------------------------------------------------------------
 ...
 INSERT /*+ APPEND */
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'TEMP_S4S_LOC_EMP_DY_PART2', DEGREE => 8);
...
WITH selext1 AS
  (SELECT /*+ FULL(a)  PARALLEL(a,6)  */ DISTINCT EMPLOYEE_ID,
    ORIG_CYCLE_START_DATE,
    AVAILABILITY_START_DATE,
    AVAILABILITY_END_DATE ,
    CYCLE_START_DATE,
  ...
  selext2 AS
  (SELECT /*+ FULL(tmp)  PARALLEL(tmp,6)  */ tmp.EMPLOYEE_ID ,

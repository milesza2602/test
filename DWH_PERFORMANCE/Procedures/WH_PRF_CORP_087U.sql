--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_087U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_087U" (p_forall_limit in integer,p_success out boolean) as
...
--
--  Maintenance:
--  28 june 2016 w lyttle - added check to prevent duplicates
--                         ie. take max(to_loc_no)
--  18 Aug 2016  B Kirschner - added additional measure NUM_EXTRA_LEAD_TIME_DAYS in output table to pick up from fnd_zone_item_supp  
--               for FF feed (to populate in rtl_zone_item_supp_hist)                                            REF: BK18AUG2016
--  22 Aug 2016  B Kirschner - added additional measure SUPPLIER_SHELF_LIFE in output table to pick up from fnd_zone_item_supp  
--               for FF feed (to populate in rtl_zone_item_supp_hist)                                            REF: BK22AUG2016
--  23 Aug 2016  B Kirschner - added additional measure FROM_LOC_NO in output table to pick up from fnd_zone_item_supp  
--               for FF feed (to populate in rtl_zone_item_supp_hist)                                            REF: BK23AUG2016
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_087U';
...
cursor c_fnd_zone_item_supp_hist_hist is
 with om as (
              select   /*+ full(fis) parallel(fis,4) */
                        fis.zone_group_no zone_group_no,
                        fis.zone_no zone_no,
                 ...
                 from   dwh_foundation.fnd_zone_item_supp_om fis
                 where 
                   fis.last_updated_date = g_date),  -- select * from om;
 ffdup   as (
                  select  /*+ full(a) parallel(a,4) */
                                        a.ZONE_NO                  ,zone_group_no, ITEM_NO                  ,SUPPLIER_NO
                                  ,    max(TO_LOC_NO) to_loc_no
                  from dwh_foundation.fnd_zone_item_supp a, dim_zone dz
                  where a.last_updated_date = g_date
                  and dz.zone_no       = a.zone_no
                  group by               a.ZONE_NO,zone_group_no                  ,ITEM_NO                  ,SUPPLIER_NO), 
 ff as  
                (select  /*+ full(fis) parallel(fis,4) */
                        dz.zone_group_no,
                        fis.zone_no zone_no,
                        ...
                        fis.to_loc_no  as to_loc_no,
                        fis.num_lead_time_days as num_lead_time_days,
                        fis.NUM_EXTRA_LEAD_TIME_DAYS as NUM_EXTRA_LEAD_TIME_DAYS,           -- BK18AUG2016
                        fis.SUPPLIER_SHELF_LIFE as SUPPLIER_SHELF_LIFE,                     -- BK22AUG2016
                        fis.FROM_LOC_NO as FROM_LOC_NO,                                     -- BK23AUG2016
                        fis.next_wk_cost_price as next_wk_cost_price,
                        fis.this_wk_cost_price as this_wk_cost_price,
                        fis.next_wk_trunk_cost as next_wk_trunk_cost,
                        fis.this_wk_trunk_cost as this_wk_trunk_cost,
                 ...
                 from   dwh_foundation.fnd_zone_item_supp fis,
                        dim_zone dz ,
                        ffdup sd
                 where dz.zone_no       = fis.zone_no
                   and fis.last_updated_date = g_date
                   and sd.zone_no       = fis.zone_no
                   and sd.zone_group_no       = dz.zone_group_no
                   and sd.item_no       = fis.item_no
                   and sd.supplier_no   = fis.supplier_no
                   and sd.to_loc_no = fis.to_loc_no) ,  -- select * from ff;
  ...
  all_together  as (
                select nvl(om.zone_group_no, ff.zone_group_no)  as zone_group_no,
                      ...
                      nvl(ff.NUM_EXTRA_LEAD_TIME_DAYS, 0)                           as NUM_EXTRA_LEAD_TIME_DAYS,      -- BK18AUG2016
                      nvl(ff.SUPPLIER_SHELF_LIFE, 0)                                as SUPPLIER_SHELF_LIFE,           -- BK22AUG2016
                      nvl(ff.FROM_LOC_NO, 0)                                        as FROM_LOC_NO                    -- BK23AUG2016
...
select dz.sk1_zone_group_zone_no,
          ...
          atg.split_perc,
          NUM_EXTRA_LEAD_TIME_DAYS,                                             -- BK18AUG2016
          SUPPLIER_SHELF_LIFE,                                                  -- BK22AUG2016
          FROM_LOC_NO                                                           -- BK23AUG2016
...
/*    minus
  ...
  where atg.zone_group_no = dz.zone_group_no
    ...
    and atg.item_no       = di.item_no
    and atg.supplier_no   = ds.supplier_no   
    and ( (dz.sk1_zone_group_zone_no = 256  and di.sk1_item_no = 66751  and ds.sk1_supplier_no  = 2406 and atg.to_loc_no <> 6110  )
or (dz.sk1_zone_group_zone_no = 254  and di.sk1_item_no = 24898269  and ds.sk1_supplier_no  = 8607  and atg.to_loc_no <> 6060 )
or (dz.sk1_zone_group_zone_no = 254  and di.sk1_item_no = 24970699  and ds.sk1_supplier_no  = 7561 and atg.to_loc_no <> 6070  )
or (dz.sk1_zone_group_zone_no = 255  and di.sk1_item_no = 24970699  and ds.sk1_supplier_no  = 7561 and atg.to_loc_no <> 6070)
or (dz.sk1_zone_group_zone_no = 256  and di.sk1_item_no = 24970699  and ds.sk1_supplier_no  = 7561 and atg.to_loc_no <> 6070 )
or (dz.sk1_zone_group_zone_no = 254  and di.sk1_item_no = 25037493  and ds.sk1_supplier_no  = 5798  and atg.to_loc_no <> 6070 )
or (dz.sk1_zone_group_zone_no = 254  and di.sk1_item_no = 24732459  and ds.sk1_supplier_no  = 5635 and atg.to_loc_no <> 6060  )
or (dz.sk1_zone_group_zone_no = 256  and di.sk1_item_no = 24898269  and ds.sk1_supplier_no  = 8607 and atg.to_loc_no <> 6060  )
or (dz.sk1_zone_group_zone_no = 256  and di.sk1_item_no = 635036  and ds.sk1_supplier_no  = 2406 and atg.to_loc_no <> 6060  )
or (dz.sk1_zone_group_zone_no = 256  and di.sk1_item_no = 25016337  and ds.sk1_supplier_no  = 5635 and atg.to_loc_no <> 6110  )
or (dz.sk1_zone_group_zone_no = 255  and di.sk1_item_no = 24751227  and ds.sk1_supplier_no  = 5448 and atg.to_loc_no <> 6060  ));
 */
...
begin
   ...
   g_rec_out.NUM_EXTRA_LEAD_TIME_DAYS        := g_rec_in.NUM_EXTRA_LEAD_TIME_DAYS;     -- BK18AUG2016
   g_rec_out.SUPPLIER_SHELF_LIFE             := g_rec_in.SUPPLIER_SHELF_LIFE;          -- BK22AUG2016
   g_rec_out.FROM_LOC_NO                     := g_rec_in.FROM_LOC_NO;                  -- BK23AUG2016
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    to_loc_no                       = a_tbl_update(i).to_loc_no,
              ...
              NUM_EXTRA_LEAD_TIME_DAYS        = a_tbl_update(i).NUM_EXTRA_LEAD_TIME_DAYS,       -- BK18AUG2016
              SUPPLIER_SHELF_LIFE             = a_tbl_update(i).SUPPLIER_SHELF_LIFE,            -- BK22AUG2016
              FROM_LOC_NO                     = a_tbl_update(i).FROM_LOC_NO                     -- BK23AUG2016
...
--**************************************************************************************************
   ...
   execute immediate 'alter session enable parallel dml';
       ...
       l_text := 'Running GATHER_TABLE_STATS ON RTL_ZONE_ITEM_SUPP_HIST';
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          
          -- below is the new code for stats collection in 12c - Ref:23Jan2019
    filter_lst.extend(1);
    filter_lst(1).ownname := 'DWH_PERFORMANCE';
    filter_lst(1).objname := 'RTL_ZONE_ITEM_SUPP_HIST';
    DBMS_STATS.GATHER_SCHEMA_STATS(ownname=>'DWH_PERFORMANCE',obj_filter_list=>filter_lst,options=>'gather auto');
    
--           DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
--                                         'RTL_ZONE_ITEM_SUPP_HIST', DEGREE => 8);
--**************************************************************************************************
-- Bulk fetch loop controlling main program execution
--**************************************************************************************************
 l_text := 'Starting processing RTL_ZONE_ITEM_SUPP_HIST';
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

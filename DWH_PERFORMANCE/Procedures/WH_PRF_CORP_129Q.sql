--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_129Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_129Q" (p_forall_limit in integer,p_success out boolean) as
...
g_min_fnd_ff_date    fnd_rtl_loc_item_dy_ff_ord.post_date%type;
g_max_fnd_ff_date    fnd_rtl_loc_item_dy_ff_ord.post_date%type;
g_min_fnd_cust_date  fnd_rtl_loc_item_dy_ff_ord.post_date%type;
g_max_fnd_cust_date  fnd_rtl_loc_item_dy_ff_ord.post_date%type;
g_min_fnd_date       fnd_rtl_loc_item_dy_ff_ord.post_date%type;
g_max_fnd_date       fnd_rtl_loc_item_dy_ff_ord.post_date%type;
...
(
  with MainDriver as                                                           -- BK02Mar2016
    (
      select /*+ parallel(ord,8) parallel(ffd_ord,8) parallel(sdn,8) parallel(prc,8) full(di), full(dl), full(dih) */         --index(prc,PK_P_RTL_LID_RMS_PRCF) index(dir,PK_P_RTL_LC_ITM_DY_STD_ORD)  */
                ord.LOCATION_NO,
                ord.ITEM_NO,
                ord.POST_DATE,
                ord.ROQ_QTY,
...
and rtl_lidoo.post_date between g_min_fnd_date and g_max_fnd_date
)
...
--**************************************************************************************************
    ...
    select /*+ parallel(ord,8) */ min(ord.POST_DATE), max(ord.post_date)
      into g_min_fnd_ff_date, g_max_fnd_ff_date
      from   fnd_rtl_loc_item_dy_ff_ord ord
     where  ord.last_updated_date = g_date;
     
   l_text := 'DATE RANGE FROM FND_RTL_LOC_ITEM_DY_FF_ORD - '||g_min_fnd_ff_date || ' - ' || g_max_fnd_ff_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   
    select /*+ parallel(ord,8) */ min(ord.POST_DATE), max(ord.post_date)
      into g_min_fnd_cust_date, g_max_fnd_cust_date
      from   FND_LOC_ITEM_DY_FF_CUST_ORD ord
     where  ord.last_updated_date = g_date;
     
     l_text := 'DATE RANGE FROM FND_LOC_ITEM_DY_FF_CUST_ORD - '||g_min_fnd_cust_date || ' - ' || g_max_fnd_cust_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     
  --g_min_fnd_date := min(g_min_fnd_ff_date, g_min_fnd_cust_date);
     
   if g_min_fnd_ff_date < g_min_fnd_cust_date then
      g_min_fnd_date := g_min_fnd_ff_date;
   else
      g_min_fnd_date := g_min_fnd_cust_date;
   end if;
   
   if g_max_fnd_ff_date < g_max_fnd_cust_date then
      g_min_fnd_date := g_max_fnd_ff_date;
   else
      g_max_fnd_date := g_max_fnd_cust_date;
   end if;
     
   l_text := 'DATE RANGE FOR RECORDS TO BE PROCESSED ON RTL_LOC_ITEM_DY_OM_ORD - '||g_min_fnd_date || ' - ' || g_max_fnd_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

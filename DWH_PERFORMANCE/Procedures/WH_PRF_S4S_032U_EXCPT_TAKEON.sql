--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_032U_EXCPT_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_032U_EXCPT_TAKEON" (
...
--               Output   - RTL_EMP_CONSTR_LOC_JOB_WK_X
...
--  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_S4S_005U';
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_S4S_032U_EXCPT_TAKEON';
  l_name sys_dwh_log.log_name%type                     := dwh_constants.vc_log_name_rtl_md;
  l_system_name sys_dwh_log.log_system_name%type       := dwh_constants.vc_log_system_name_rtl_prf;
  l_script_name sys_dwh_log.log_script_name%type       := dwh_constants.vc_log_script_rtl_prf_md;
  l_procedure_name sys_dwh_log.log_procedure_name%type := l_module_name;
  l_text sys_dwh_log.log_text%type ;
  l_description sys_dwh_log_summary.log_description%type   := 'LOAD THE RTL_EMP_CONSTR_LOC_JOB_WK_X data  EX FOUNDATION';
...
BEGIN
  ...
  l_text := 'LOAD OF RTL_EMP_CONSTR_LOC_JOB_WK_X  EX FOUNDATION STARTED '|| TO_CHAR(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  INSERT /*+ APPEND */
  INTO DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_X
WITH SELEXT1 AS
          (SELECT
...
SELECT DISTINCT 
              ...
              (SE2.EMPLOYEE_RATE * SE2.STRICT_MIN_HRS_PER_WK) BASE_COST_WK ,
                            SE3.BASE_HEAD_COUNT ,
                            g_date last_updated_date, DERIVE_START_DATE, DERIVE_END_DATE, EMPLOYEE_STATUS
                            
FROM SELEXT2 SE2 ,
      SELEXT3 SE3 
WHERE 
--se2.THIS_WEEK_START_DATE BETWEEN derive_start_date AND derive_end_date
  ----and se2.tran_date between sp.cycle_end_date - (7*mCYCLEweeknum)+1 and sp.cycle_end_date
--AND derive_start_date  IS NOT NULL
--AND 
SE2.FIN_YEAR_NO     = SE3.FIN_YEAR_NO(+)
...
SELECT DISTINCT 
              ...
              (SE2.EMPLOYEE_RATE * SE2.STRICT_MIN_HRS_PER_WK) BASE_COST_WK ,
                            ...
                            , DERIVE_START_DATE, DERIVE_END_DATE, EMPLOYEE_STATUS
FROM SELEXT2 SE2 ,
      SELEXT3 SE3 
WHERE 
--se2.THIS_WEEK_START_DATE BETWEEN derive_start_date AND derive_end_date
...
l_text := 'Running GATHER_TABLE_STATS ON RTL_EMP_CONSTR_LOC_JOB_WK_X';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_CONSTR_LOC_JOB_WK_X', DEGREE => 8);
...
END WH_PRF_S4S_032U_EXCPT_TAKEON;

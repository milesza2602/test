--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_007U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_007U" 
...
--   SKIP OUT AS THIS JOB SHOULD NO LONGER BE RUNNING ------------------------------    
      l_text      := 'THIS JOB IS REDUNDANT AND NEEDS TO BE DE-COMMISSIONED - BYPASS RUN';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      p_success := true;
      return;    
-------------------------------------------------------------------------------------        
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   insert /*+ APPEND parallel(sys,8) */ into DWH_PERFORMANCE.TEMP_LC_ITEM_WK_RDF_SYSFCST52T sys
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    l_text := 'LOADING DATA INTO TEMP TABLE';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    execute immediate 'truncate table DWH_PERFORMANCE.TEMP_26WK_RDF_BIG';
    
    insert /*+ parallel(temp,8) */ into DWH_PERFORMANCE.TEMP_26WK_RDF_BIG temp 
    select /*+ parallel(rdf,8) */ rdf.* from DWH_PERFORMANCE.TEMP_LC_ITEM_WK_RDF_SYSFCST52T rdf;
    
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'TEMP_26WK_RDF_BIG', DEGREE => 16);
    
    l_text := 'TEMP_26WK_RDF_BIG loaded';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       ...
       --
        ...
        USING
         ...
         --  from     DWH_PERFORMANCE.TEMP_LC_ITEM_WK_RDF_SYSFCST52T   sys_ilv 
           from     DWH_PERFORMANCE.TEMP_26WK_RDF_BIG   sys_ilv

--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACT3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACT3" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACT3';
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_count := dwh_generic_file_extract('select /*+ (parallel a,3) */ dl.location_no, di.item_no, a.fin_year_no, a.fin_week_no, a.PROM_SALES_QTY
from rtl_loc_item_wk_rms_sparse a, dim_item di, dim_location dl
where a.fin_year_no = 2014
and   a.fin_week_no between 1 and 13
and   a.sk1_location_no = dl.sk1_location_no
and   a.sk1_item_no     = di.sk1_item_no
and   di.business_unit_no not in (50,70)','|','DWH_FILES_OUT','aj_sparse14_13');

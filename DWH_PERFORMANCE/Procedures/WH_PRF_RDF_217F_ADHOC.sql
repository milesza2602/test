--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_217F_ADHOC
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_217F_ADHOC" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2010
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_217F_ADHOC';
...
cursor c_rtl_loc_item_dy_rdf_fcst is
  ...
  join    RTL_LOC_ITEM_RDF_WKFCST_L2 fcst on
          ...
          fcst.fin_week_no        = sale.fin_week_no and
          fcst.fin_year_no        = 2015 and
          fcst.fin_week_no        between 47 and 48
  join    dim_item di on
...
--    where  calendar_date = g_date - 7;
    ...
    l_text := 'START WEEK/YEAR OF ROLLUP IS:- '||'2015'||' '||'47to48';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

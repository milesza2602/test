--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_052U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_052U_OLD" (p_forall_limit in integer,p_success out boolean) as
...
--  20 Sep 2016 - A Joshua Chg-xxx -- Cater for ETL for measures reg_rsp_excl_vat and cost_price                Ref: AJ-chg1612
...
--              dept.department_live_ind = 'Y'
  ...
  (
    select  ITEM_NO||supplier_no||ZONE_NO as pkey,
            max(NUM_EXTRA_LEAD_TIME_DAYS) NUM_EXTRA_LEAD_TIME_DAYS,
            max(FROM_LOC_NO) FROM_LOC_NO                                                                                -- BK22Aug16
    from    DWH_FOUNDATION.fnd_zone_item_supp zis 
    group by ITEM_NO,supplier_no,ZONE_NO
--    WHERE NOT (ITEM_NO = 200226 AND FROM_LOC_NO IS NULL)
--    where   exists (select 1 from prtA a where a.pkey = zis.ITEM_NO||zis.supplier_no||zis.ZONE_NO)
  ...
  select   ZONE_GROUP_NO
          ...
          ,reg_rsp_excl_vat                                                              -- AJ-chg1612
          ,cost_price                                                                    -- AJ-chg1612
  from    prtA a
  left join 
          prtb b on (a.pkey = b.pkey)
  order by 3,2,1;
  -- BK04Jun16 (end)
...
--   g_rec_out.reg_rsp_excl_vat          := g_rec_in.reg_rsp/(100 + g_vat_rate_perc) * 100;   --100 * (g_rec_in.case_selling_incl_vat / 114
     g_rec_out.reg_rsp_excl_vat          := g_rec_in.reg_rsp_excl_vat;    --AJ-chg1612
     g_rec_out.cost_price                := g_rec_in.cost_price;          --AJ-chg1612
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    --base_retail_ind               = a_tbl_update(i).base_retail_ind,
              ...
              cost_price                    = a_tbl_update(i).cost_price                                          -- AJ-chg1612

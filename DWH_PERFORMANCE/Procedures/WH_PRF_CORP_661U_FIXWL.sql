--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_661U_FIXWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_661U_FIXWL" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_661U_FIXWL';
...
l_description        sys_dwh_log_summary.log_description%type  := 'DATAFIX VAT REGION';
...
WITH SELTST
AS (SELECT /*+ MATERIALIZE */ * FROM DWH_DATAFIX.TEMP_PO_LIST_260815
...
OR (SK1_PO_NO = 23981287 AND SK1_ITEM_NO = 22271763 )*/
   ...
   (select    s.po_no, s.to_loc_no, s.item_no, sum(s.asn_qty) bc_shipment_qty,
    ...
    join      SELTST pl    on  s.po_no = pl.po_no and s.item_no = pl.item_no
...
--    local_bulk_insert;
    ...
    commit;
 --   if g_param_ind = 1 then
  --     delete_invalid_recs; COMMENTED OUT FOR FIX
 --   end if;

--------------------------------------------------------
--  DDL for Procedure KQ_WK_FD_EXTRACT_DEPT_88_SALES
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."KQ_WK_FD_EXTRACT_DEPT_88_SALES" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'KQ_WK_FD_EXTRACT_DEPT_88_SALES';
...
--**************************************************************************************************
  ...
  AND itm.Department_no     = 88
  AND BUSINESS_UNIT_NO      = 50
  AND itm.item_no          IN (20087494, 6008000680257, 20057633, 6009101081226, 6009171229382, 6008000180016, 20111014, 20008529, 20008703, 6008000806657, 20110444, 20091156, 20098551, 6008000179935, 20109622, 20047085, 6001009037091, 20130510, 20008635, 20008666, 20090821, 6008000180061, 20134273, 6009173919649)
  ),
  sales_measures AS
  (SELECT
...
and NVL (NVL (NVL (NVL (F0.FIN_WEEK_NO, F1.FIN_WEEK_NO), F2.FIN_WEEK_NO),F3.FIN_WEEK_NO),F4.FIN_WEEK_NO)                    = F5.FIN_WEEK_NO','|','DWH_FILES_OUT','Veg_24_Items_dept_88_Sales.txt');
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       raise;
end KQ_WK_FD_EXTRACT_DEPT_88_SALES;

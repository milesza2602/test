--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_252U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_252U" (p_forall_limit in integer, p_success out boolean) as 
...
--               Output -   foods_renewal_extract1 FOR CURRENT WEEK TO DATE
...
g_rec_out            DWH_PERFORMANCE.foods_renewal_extract1%rowtype;
g_count              number        :=  0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_252U';
...
type tbl_array_i is table of DWH_PERFORMANCE.foods_renewal_extract1%rowtype index by binary_integer;
type tbl_array_u is table of DWH_PERFORMANCE.foods_renewal_extract1%rowtype index by binary_integer;
...
begin 
    ...
    l_text := 'LOAD OF foods_renewal_extract1 started AT '||
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table DWH_PERFORMANCE.FOODS_RENEWAL_EXTRACT1');  
...
INSERT /*+ APPEND PARALLEL (mart,6) */ INTO DWH_PERFORMANCE.foods_renewal_extract1 mart
WITH item_list AS
  (  select di.item_no, di.sk1_item_no, substr(di.item_desc,60) item_desc, di.sk1_supplier_no, dd.department_no, dd.department_name, di.subclass_no, di.subclass_name 
            from dim_item di, dim_department dd
               where dd.department_no in (44, 88, 41, 47, 68, 75, 85, 40, 6110, 6111, 6112, 6113, 6119)  --(42,46,59,66,83,86,89)   --(41,58,73,81,88,96)   --,44,45,47,50,55,58,72,73,75,76,77,81,87,88,90,93,95,96,97,98,99)
...
group by d.fin_year_no, d.fin_week_no, d.fin_day_no, b.wh_fd_zone_no, c.post_date, b.location_no, a.item_no--, e.supplier_no
  ...
  sales_ly as ( /*+ PARALLEL(a,6) FULL(a) */

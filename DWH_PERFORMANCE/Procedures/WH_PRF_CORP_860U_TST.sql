--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_860U_TST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_860U_TST" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        November 2015
--  Author:      W Lyttle
...
g_date               date;
g_start_date         date          := '05 feb 18';
g_end_date           date          := '07 feb 18';
g_ly_start_date      date;
g_ly_end_date        date;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_860U_TST';
...
BEGIN 
   ...
   DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
          ...
          select /*+ parallel(SRC,8) */ SRC.fin_year_no, SRC.fin_week_no, si.sk1_item_no, sl.sk1_location_no, 
          ...
          group by SRC.fin_year_no, SRC.fin_week_no, si.sk1_item_no, sl.sk1_location_no;
...
BEGIN
   ...
   l_text := 'truncate table  DWH_DATAFIX.AJ_TEMP_FOODS_TAB_2YRS_CATALOG';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   execute immediate('truncate table DWH_DATAFIX.AJ_TEMP_FOODS_TAB_2YRS_CATALOG');
   commit;

   l_text := 'insert starting ';
   DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
   
   insert /*+ append */ into DWH_DATAFIX.AJ_TEMP_FOODS_TAB_2YRS_CATALOG
   with SELITM as (select SK1_ITEM_NO
                   from DIM_ITEM where BUSINESS_UNIT_NO = 50),
        SELLOC as (select LOCATION_NO, SK1_LOCATION_NO
                   from DIM_LOCATION where area_no = 9951),
    ...
    select /*+ parallel(SRC,8) */ SRC.FIN_YEAR_NO, SRC.FIN_WEEK_NO, SRC.SK1_ITEM_NO, SRC.SK1_LOCATION_NO
           ...
           , SUM(NVL(FD_NUM_CUST_CATLG_ADJ,0))  FD_NUM_CUST_CATLG_ADJ
           , SUM(NVL(FD_CUST_AVAIL,0))          FD_CUST_AVAIL
    from  dwh_datafix.AJ_RTL_LOC_ITEM_WK_CATALOG SRC, SELITM SI, SELLOC SL, SELCAL SC
    where src.sk1_item_no = si.sk1_item_no 
    and   src.sk1_location_no = sl.sk1_location_no 
...
BEGIN
   ...
   from  rtl_loc_dy ld, selloc sl, seldat sc, DWH_DATAFIX.AJ_TEMP_FOODS_TAB_2YRS_CATALOG ds
...
--      and sales <> 0 
    union 
    select /*+ full (a) parallel (a,4) full (c) parallel (c,4) */ a.*
    from  DWH_DATAFIX.AJ_TEMP_FOODS_TAB_2YRS_CATALOG c,
          selprice a
    where c.fin_year_no     = a.fin_year_no
      and c.fin_week_no     = a.fin_week_no
      and c.sk1_item_no     = a.sk1_item_no

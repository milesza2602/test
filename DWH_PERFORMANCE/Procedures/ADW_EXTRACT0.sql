--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACT0
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACT0" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACT0';
...
FROM Rtl_loc_item_wk_rms_dense partition (tp_rtl_lcitmrd_m201410) a, locsel dl, itemsel Di
where a.fin_year_no = 2014 
and a.fin_week_no between 40 and 43
...
FROM Rtl_loc_item_wk_rms_sparse partition (tp_rtl_lcitmwrsprs_m201410) a, locsel dl, itemsel Di
where a.fin_year_no = 2014 
and a.fin_week_no between 40 and 43
and a.sk1_location_no = dl.sk1_location_no 
and a.sk1_item_no = di.sk1_item_no
...
FROM Rtl_loc_item_wk_rms_stock partition (tp_rtl_lciwkrmss_m201410) a, locsel dl, itemsel Di
where a.fin_year_no = 2014 
and a.fin_week_no between 40 and 43
and a.sk1_location_no = dl.sk1_location_no 
and a.sk1_item_no = di.sk1_item_no
...
FROM Rtl_loc_item_wk_catalog partition (tp_prf_nliwc_m201410) a, locsel dl, itemsel Di
where a.fin_year_no = 2014 
and a.fin_week_no between 40 and 43
and a.sk1_location_no = dl.sk1_location_no 
and a.sk1_item_no = di.sk1_item_no
...
and cal.fin_week_no between 40 and 43
and a.sk1_location_no = dl.sk1_location_no 
and a.sk1_item_no = di.sk1_item_no
...
select /*+ parallel (Sd,4) parallel (sps,4) parallel (stk,4) parallel (sc,4) parallel (sp,4) */
...
and sp.item_no = sd.item_no','|','DWH_FILES_OUT','aj_spc_raceq4_43');

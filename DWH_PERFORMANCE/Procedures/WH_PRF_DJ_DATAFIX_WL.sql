--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_DATAFIX_WL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_DATAFIX_WL" (p_forall_limit in integer,p_success out boolean
...
--                     eventhough nothing was affected in this proc, have taken opportunity to change to make easier to read
--                     
--
...
g_date_end               date          := trunc(sysdate);
g_start_date         date;
g_end_date           date;
g_rec_out            TEMP_PO_LIST_DJ%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_DATAFIX_WL';
...
l_description        sys_dwh_log_summary.log_description%type  := 'NEW CREATES DISTINCT PO/ITEM LIST FROM PO AND SHIPMENT EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of TEMP_PO_LIST_DJ%rowtype index by binary_integer;
...
begin
    ...
    l_text := 'LOAD OF TEMP_PO_LIST_DJ STARTED '||
...
/* SQL TO CHECK 
"with selloc as 
                        (select distinct  dl.chain_no loc_chain_no, dl.loc_type loc_type, stg.chain_code stg_chain_code, stg.location_no, stg.po_no, LOCATION_NAME, stg.item_no
                        from stg_rms_rtl_purchase_order_arc stg, dim_location dl
                        where stg.location_no = dl.location_no
                        and stg.sys_load_date >= '1 january 2015'
                        ),
selext as (select * from selLOC
              where ( (loc_chain_no = 10 and loc_type = 'W' AND STG_CHAIN_CODE = 'DJ')
              OR (loc_chain_no = 40 and loc_type = 'W' AND STG_CHAIN_CODE IS NULL))
            --  AND PO_NO = 7485839 AND ITEM_NO = 6009189632860 AND LOCATION_NO =3050
             )
              ,
SELEXT2 AS (
             select stg.SYS_SOURCE_BATCH_ID, stg.SYS_SOURCE_SEQUENCE_NO , stg.SYS_LOAD_DATE , stg.PO_NO , stg.ITEM_NO , stg.LOCATION_NO , stg.CHAIN_CODE
              ,  se.LOC_CHAIN_NO,  se.LOC_TYPE,  se.STG_CHAIN_CODE STG_LOCATION_NO2,  se.LOCATION_NO LOC_LOCATION_NO
              , RANK() OVER (PARTITION BY  stg.PO_NO , stg.ITEM_NO , stg.LOCATION_NO ORDER BY stg.SYS_SOURCE_BATCH_ID, stg.SYS_SOURCE_SEQUENCE_NO) RANKVAL
               from stg_rms_rtl_purchase_order_arc stg, selext se
              where stg.po_no = se.po_no
              and stg.location_no = se.location_no
              and stg.item_no = se.item_no
              ORDER BY stg.PO_NO , stg.ITEM_NO , stg.LOCATION_NO, stg.SYS_SOURCE_BATCH_ID, stg.SYS_SOURCE_SEQUENCE_NO
              )
              ,
SELX AS (
              SELECT PO_NO , ITEM_NO , LOCATION_NO , CHAIN_CODE
                ,  LOC_CHAIN_NO,  LOC_TYPE,  STG_LOCATION_NO2,  LOC_LOCATION_NO, MAX(RANKVAL) MAXRANK
                FROM SELEXT2
                GROUP BY PO_NO , ITEM_NO , LOCATION_NO , CHAIN_CODE
                ,  LOC_CHAIN_NO,  LOC_TYPE,  STG_LOCATION_NO2,  LOC_LOCATION_NO
                ),
selfinal as (    SELECT  SE2.PO_NO FIRST_PO_NO, SE2.ITEM_NO FIRST_ITEM_NO , SE2.LOCATION_NO  FIRST_LOCATION_NO , SE2.CHAIN_CODE FIRST_CHAIN_CODE
                ,  SE2.LOC_CHAIN_NO FIRST_LOC_CHAIN_NO,  SE2.LOC_TYPE FIRST_LOC_TYPE 
                ,  SE2.STG_LOCATION_NO2 FIRST_STG_LOCATION_NO2,  SE2.LOC_LOCATION_NO FIRST_LOC_LOCATION_NO, 1 FIRST_MAXRANK ,
SX.PO_NO LAST_PO_NO, SX.ITEM_NO LAST_ITEM_NO , SX.LOCATION_NO  LAST_LOCATION_NO , SX.CHAIN_CODE LAST_CHAIN_CODE
                ,  SX.LOC_CHAIN_NO LAST_LOC_CHAIN_NO,  SX.LOC_TYPE LAST_LOC_TYPE 
                ,  SX.STG_LOCATION_NO2 LAST_STG_LOCATION_NO2,  SX.LOC_LOCATION_NO LAST_LOC_LOCATION_NO, SX.MAXRANK LAST_MAXRANK 
FROM SELEXT2 SE2, SELX SX WHERE RANKVAL = 1 AND SE2.PO_NO = SX.PO_NO(+) and se2.item_no = sx.item_no(+) and se2.location_no = sx.location_no(+)
)
select FIRST_PO_NO,FIRST_ITEM_NO ,FIRST_LOCATION_NO , location_name,  FIRST_CHAIN_CODE
                ,  FIRST_LOC_CHAIN_NO,   FIRST_LOC_TYPE 
                ,  FIRST_STG_LOCATION_NO2,  FIRST_LOC_LOCATION_NO, 1 FIRST_MAXRANK ,
                 LAST_CHAIN_CODE
                ,  LAST_LOC_CHAIN_NO,   LAST_LOC_TYPE 
                ,   LAST_STG_LOCATION_NO2,   LAST_LOC_LOCATION_NO,  LAST_MAXRANK  
                from selfinal sf, dim_location dl 
                where sf.first_location_no = dl.location_no
...
--**************************************************************************************************
    g_param_ind := 1;
     insert /*+ append */ into dwh_performance.TEMP_PO_LIST_DJ
 with po_suppl_chain as
  (select po_no, max(supply_chain_type) supply_chain_type
   ...
   join   dim_dj_purchase_order dp    on po.po_no   = dp.po_no
   join   dim_item di                 on po.item_no = di.item_no
   join   po_suppl_chain ps           on po.po_no   = ps.po_no
   where  
-- COMMENT OUT TO TEST   --po.last_updated_date = g_date
-- COMMENT OUT TO TEST   --  and    
-- COMMENT OUT TO TEST   --  po.not_before_date between '25 Jun 2007' 
-- COMMENT OUT TO TEST   -- g_date + 540   -- partitions for rtl_po_supchain_loc_item_dy, up till 18 months into future
-- COMMENT OUT TO TEST   --  and    
   chain_code = 'DJ'
   ...
   join   dim_dj_purchase_order dp    on s.po_no    = dp.po_no
   join   dim_item di                 on s.item_no  = di.item_no
   join   po_suppl_chain ps           on s.po_no    = ps.po_no
 -- COMMENT OUT TO TEST   --  where  s.last_updated_date = g_date
  ...
  l_text := 'end insert';
    ...
    l_text := 'TABLE TEMP_PO_LIST_DJ STATISTICS UPDATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    ww_dbms_stats.gather_table_stats('dwh_performance','TEMP_PO_LIST_DJ');

/*------------------------------------------------------------------------------
/* the rest of this code is what used to be here
if at any stage we need to run hsitory, etc, then we will need to invok ethe code below
/*---------------------------------------------------------------------------
-- Before June 2007, it was allowed to completely cancel a Purchase Order
-- and then re-use the PO Number with a different location, not_before_date, etc. (therefore more than 1 location per PO).
-- The procedures joining Purchase Orders with Shipments, used to only join on po_no and item_no.
...
cursor c_TEMP_PO_LIST_DJ_hist is
   with po_suppl_chain as
  (select po_no, max(supply_chain_type) supply_chain_type
   from   fnd_rtl_purchase_order
   where  not_before_date > '24 Jun 2007'
...
cursor c_TEMP_PO_LIST_DJ_old_hist is
   with non_dupl_po as
...
cursor c_TEMP_PO_LIST_DJ_ship is
   with non_dupl_po as
  (select   po_no, max(reccnt) from
  (select   po_no, item_no, count(*) reccnt
   from     fnd_rtl_purchase_order
...
g_rec_in             c_TEMP_PO_LIST_DJ%rowtype;
-- For input bulk collect --
type stg_array is table of c_TEMP_PO_LIST_DJ%rowtype;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into TEMP_PO_LIST_DJ values a_tbl_insert(i);
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table dwh_performance.TEMP_PO_LIST_DJ';
    l_text := 'TABLE TEMP_PO_LIST_DJ TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    case g_param_ind
       when 1 then
          open c_TEMP_PO_LIST_DJ;
          fetch c_TEMP_PO_LIST_DJ bulk collect into a_stg_input limit g_forall_limit;
       when 2 then
          open c_TEMP_PO_LIST_DJ_hist;
          fetch c_TEMP_PO_LIST_DJ_hist bulk collect into a_stg_input limit g_forall_limit;
       when 3 then
          open c_TEMP_PO_LIST_DJ_ship;
          fetch c_TEMP_PO_LIST_DJ_ship bulk collect into a_stg_input limit g_forall_limit;
       when 4 then
          open c_TEMP_PO_LIST_DJ_old_hist;
          fetch c_TEMP_PO_LIST_DJ_old_hist bulk collect into a_stg_input limit g_forall_limit;
    end case;
    ...
    case g_param_ind
       when 1 then
          close c_TEMP_PO_LIST_DJ;
       when 2 then
          close c_TEMP_PO_LIST_DJ_hist;
       when 3 then
          close c_TEMP_PO_LIST_DJ_ship;
       when 4 then
          close c_TEMP_PO_LIST_DJ_old_hist;
    end case;

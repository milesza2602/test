--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_223U_T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_223U_T" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - tmp_mart_foods_wkly_snpsht_l
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_223U_T';
...
begin
    ...
    l_text := 'Create Mart table for Foods weekly snapshot (LAUREN) STARTED AT '||
...
l_last_week_no        := 36;
l_fin_week_no         := 37;
l_next_wk_fin_week_no := 38;
l_next_wk_fin_year_no := 2013;
l_last_wk_fin_year_no := 2013;
l_day_no              := 2;
l_this_wk_start_date  := '25/FEB/13';
l_end_6wks            := 36;
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table w6005682.tmp_mart_foods_wkly_snpsht_l');
...
INSERT /*+ APPEND */ INTO w6005682.tmp_mart_foods_wkly_snpsht_l mart
with item_list as (
...
--from rtl_loc_item_dy_rdf_fcst f, item_list il, store_list sl
--where f.sk1_item_no     = il.sk1_item_no
--and f.sk1_location_no = sl.sk1_location_no
--and f.post_date between l_last_wk_start_date and l_last_wk_end_date 
--and f.sales_dly_app_fcst_qty is not null

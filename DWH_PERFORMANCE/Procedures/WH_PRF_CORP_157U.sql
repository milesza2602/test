--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_157U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_157U" 
                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        June 2009
--  Author:      M Munnik
--  Purpose:     Loads location item day Financial Single and Bulk RTV facts in the performance layer with
--               added value ex foundation layer table.
--  Tables:      Input  -   fnd_rtl_rtv
--               Output -   rtl_loc_item_dy_supp_rtv
...
g_rec_out            rtl_loc_item_dy_supp_rtv%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_count              number        :=  0;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_157U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOADS RTL_LOC_ITEM_DY_SUPP_RTV EX FND_RTL_RTV';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of rtl_loc_item_dy_supp_rtv%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_item_dy_supp_rtv%rowtype index by binary_integer;
...
-- belonging to the same group of key fields (location_no, item_no, post_date, supplier_no),
...
cursor c_rtl_loc_item_dy_supp_rtv is
   with changed_rtvs as (select   location_no, item_no, post_date, supplier_no
                         from     fnd_rtl_rtv
                         where    last_updated_date = g_date
                         group by location_no, item_no, post_date, supplier_no)
   select   dl.sk1_location_no,
            ...
            sum(case when (dl.wh_physical_wh_no is not null)
                     and  dl.wh_primary_supply_chain_type = 'XD' and dl.wh_org_hrchy_value_desc = 'CORPORATE'
                     and  rtv.rtv_reason_code = 'W' and rtv.rtv_ref_id like '218%'
                     then rtv.rtv_qty end) fin_single_rtv_qty,
            sum(case when (dl.wh_physical_wh_no is not null)
                     and  dl.wh_primary_supply_chain_type = 'XD' and dl.wh_org_hrchy_value_desc = 'CORPORATE'
                     and  rtv.rtv_reason_code = 'W' and rtv.rtv_ref_id like '218%'
                     then rtv.rtv_selling end) fin_single_rtv_selling,
            sum(case when (dl.wh_physical_wh_no is not null)
                     and  dl.wh_primary_supply_chain_type = 'XD' and dl.wh_org_hrchy_value_desc = 'CORPORATE'
                     and  rtv.rtv_reason_code = 'W' and rtv.rtv_ref_id like '218%'
                     then rtv.rtv_cost end) fin_single_rtv_cost,
            sum(case when (dl.wh_physical_wh_no is not null)
                     and  dl.wh_primary_supply_chain_type = 'XD' and dl.wh_org_hrchy_value_desc = 'CORPORATE'
                     and  rtv.rtv_reason_code = 'W' and rtv.rtv_ref_id like '218%'
                     then null else rtv.rtv_qty end) fin_bulk_rtv_qty,
            sum(case when (dl.wh_physical_wh_no is not null)
                     and  dl.wh_primary_supply_chain_type = 'XD' and dl.wh_org_hrchy_value_desc = 'CORPORATE'
                     and  rtv.rtv_reason_code = 'W' and rtv.rtv_ref_id like '218%'
                     then null else rtv.rtv_selling end) fin_bulk_rtv_selling,
            sum(case when (dl.wh_physical_wh_no is not null)
                     and  dl.wh_primary_supply_chain_type = 'XD' and dl.wh_org_hrchy_value_desc = 'CORPORATE'
                     and  rtv.rtv_reason_code = 'W' and rtv.rtv_ref_id like '218%'
                     then null else rtv.rtv_cost end) fin_bulk_rtv_cost,
   ...
   group by dl.sk1_location_no, di.sk1_item_no, rtv.post_date, ds.sk1_supplier_no;

-- Input record declared as cursor%rowtype
g_rec_in             c_rtl_loc_item_dy_supp_rtv%rowtype;

-- Input bulk collect table declared
type stg_array is table of c_rtl_loc_item_dy_supp_rtv%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into rtl_loc_item_dy_supp_rtv values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update rtl_loc_item_dy_supp_rtv
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_ITEM_DY_SUPP_RTV EX FND_RTL_RTV STARTED '||
...
--**************************************************************************************************
    open c_rtl_loc_item_dy_supp_rtv;
    fetch c_rtl_loc_item_dy_supp_rtv bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_rtl_loc_item_dy_supp_rtv;
--**************************************************************************************************

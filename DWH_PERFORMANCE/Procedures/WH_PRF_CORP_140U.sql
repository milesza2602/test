--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_140U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_140U" -- STORE PROC CHANGE
...
--               Output - RTL_ITEM_SUPPLIER_WK_LIST                                                      -- TABLE NAME CHANGE 
...
g_loop_fin_year_no   number(4)     :=  0;
g_loop_fin_week_no   number(2)     :=  0;
g_fin_year_no        number(4)     :=  0;
g_fin_week_no        number(2)     :=  0;
g_loop_cnt           number(2)     :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_140U';                              -- STORE PROC CHANGE
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_ITEM_SUPPLIER_WK_LIST';    
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

--************************************************************************************************** 
-- Partition Truncation 
--**************************************************************************************************
procedure do_partition_truncate as
begin
    
    select today_fin_day_no into g_wkday from dim_control_report;               
    ...
    begin
       for g_sub in 1 .. g_loop_cnt                                                      -- BK30Sep2016                                                
         ...
         execute immediate 'alter table '|| 'dwh_performance' || '.'|| 'RTL_ITEM_SUPPLIER_WK_LIST' ||' truncate subpartition for ('||G_LOOP_FIN_YEAR_NO||','||G_LOOP_FIN_WEEK_NO||')';
...
begin
   ...
   merge into rtl_item_supplier_wk_list rtl 
   using (
    with 
        loc_list  as 
        ...
        item_list as 
             (select sk1_item_no,
                     ds.sk1_supplier_no
                from dim_item di, dwh_datafix.aj_supplier_bridgethorne ds --dim_supplier_bridgethorne ds
        ...
        sales_measures as 
            ...
            grn_measures as 
            (  
              select /*+ parallel (po,8) full(po) */
                   dl.sk1_location_no,
                   di.sk1_supplier_no,
                   di.sk1_item_no,
                   cal.fin_year_no fin_year_no,
                   cal.fin_week_no fin_week_no,
                   nvl(po.po_grn_selling,0) po_grn_selling,
                   nvl(po.po_grn_qty,0) po_grn_qty,
                   nvl(po.po_grn_cost,0) po_grn_cost,      
                   nvl(po.fillrate_fd_latest_po_qty,0) fillrate_fd_latest_po_qty, 
                   nvl(po.fillrate_fd_po_grn_qty,0) fillrate_fd_po_grn_qty, 
                   nvl(po.shorts_selling,0) shorts_selling,
                   nvl(po.shorts_qty,0) shorts_qty, 
                   nvl(po.shorts_cost,0) shorts_cost       
  ...
  values                                                                                                           -- COLUNM NAME CHANGE 
    ...
    l_text := 'Records Processed = '||g_recs_read||' for '||g_loop_fin_year_no||' - '||g_loop_fin_week_no;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     select fin_year_no, fin_week_no, this_week_start_date  
     into   g_last_wk_fin_year_no, g_last_wk_fin_week_no, g_last_wk_start_date 
     from   dim_calendar 
     where  calendar_date = g_date - 7;
     
    l_text := 'TRUNCATE STARTING - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    do_partition_truncate;
    
    l_text := 'TRUNCATE ENDING - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       raise;
END WH_PRF_CORP_140U                                                                                             -- STORE PROC CHANGE 
;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_ADWSC_FULL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_ADWSC_FULL" 
...
g_rec_out            rtl_loc_sc_wk_rms_dense%rowtype;
g_found              boolean;
g_date               date;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_ADWSC_FULL';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'FIX rtl_loc_sc_wk_rms_dense EX rtl_loc_item_wk_rms_dense';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of rtl_loc_sc_wk_rms_dense%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_sc_wk_rms_dense%rowtype index by binary_integer;
...
group by sk1_location_no,SK1_STYLE_COLOUR_NO
)

select dy.sk1_location_no,
       dy.SK1_STYLE_COLOUR_NO,
       dy.sdn_in_cases,
       dy.sdn_in_qty,
...
from   dy
left outer join rtl_loc_sc_wk_rms_dense dns  on
       fin_year_no         = G_FIN_YEAR_NO 
and    fin_week_no         = G_FIN_WEEK_NO
and    dns.sk1_location_no     = dy.sk1_location_no 
and    dns.SK1_STYLE_COLOUR_NO = dy.SK1_STYLE_COLOUR_NO
...
begin
   ...
   g_rec_out.last_updated_date              := '11 May 1970';
...
--DBMS_OUTPUT.PUT_LINE('UPDATE');
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update rtl_loc_sc_wk_rms_dense 
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_sc_wk_rms_dense values a_tbl_insert(i);
...
begin
    ...
    l_text := 'FIX OF rtl_loc_sc_wk_rms_dense EX rtl_loc_item_wk_rms_dense STARTED '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_237U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_237U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_237U';
...
l_fin_year_no := l_last_wk_fin_year_no;
...
--  sum(prom_sales) prom_sales,
--  sum(prom_sales_qty) prom_sales_qty
-- from rtl_loc_item_wk_rms_sparse, store_list dl, item_list di, dim_prom dp
-- where prom.fin_year_no = l_fin_year_no
-- and prom.fin_week_no = l_last_week_no
-- --where prom.Post_Date between l_last_yr_wk_start_date and l_last_yr_wk_end_date
--   and prom.sk1_location_no = dl.sk1_location_no
--   and prom.sk1_item_no = di.sk1_item_no
--   and prom.sk1_prom_no = dp.sk1_prom_no
-- group by prom.sk1_location_no, prom.sk1_item_no, dp.sk1_prom_no
--),
...
select /*+ PARALLEL(prom,4)  */
 ...
 from rtl_prom_loc_item_dy prom , store_list dl, item_list di, dim_prom dp
 --where prom.fin_year_no = l_last_fin_year
 --and prom.fin_week_no = l_last_week_no
 where prom.Post_Date between l_last_yr_wk_start_date and l_last_yr_wk_end_date
   and prom.sk1_location_no = dl.sk1_location_no
   and prom.sk1_item_no = di.sk1_item_no

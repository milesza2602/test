--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_174X
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_174X" (p_forall_limit in integer,p_success out boolean) as
...
--with Temp1 as
--(SELECT     /*+ FULL(PRC) */
--         max(PRC.SK1_LOCATION_NO) SK1_LOCATION_NO,
--         max(PRC.SK1_ITEM_NO) SK1_ITEM_NO ,
--         WAC.LOCATION_NO,
 --        WAC.ITEM_NO,
--         PRC.CALENDAR_DATE,
--         NVL (max(PRC.NUM_UNITS_PER_TRAY), 1) AS NUM_UNITS_PER_TRAY,
...
--         AND WAC.TRAN_DATE <= PRC.CALENDAR_DATE
--         AND prc.last_updated_date  = g_date 
-- test only         AND PRC.CALENDAR_DATE      >  '04-JUN-09' 

-- GROUP BY WAC.LOCATION_NO,WAC.ITEM_NO, PRC.CALENDAR_DATE
-- )
-- SELECT     /*+ USE_HASH(WAC) */  
--      T.SK1_LOCATION_NO, T.SK1_ITEM_NO, T.CALENDAR_DATE, WAC.WAC, T.NUM_UNITS_PER_TRAY
...
/* (Replaced with new query)
   ...
   from rtl_loc_item_dy_rms_price prc,
        ...
        dim_location dl,
        dim_item di
   where prc.last_updated_date   = g_date and
         prc.sk1_location_no = dl.sk1_location_no and
         prc.sk1_item_no     = di.sk1_item_no and
         dl.location_no      = wac.location_no and
         di.item_no          = wac.item_no and
         wac.tran_date       =
         (
         ...
         where  wac_sub.location_no = dl.location_no and
                ...
                wac_sub.tran_date   <= prc.calendar_date
         );
*/

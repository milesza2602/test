--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_861U_BK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_861U_BK" (p_forall_limit in integer,p_success out boolean) as
...
g_start_date         date          := '06 jul 15';
g_end_date           date          := '06 sep 15';
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_861U_BK';
...
--**************************************************************************************************
    ...
    from dwh_performance.dim_calendar where calendar_date between g_start_date and g_end_date;
    
    
    insert /*+ APPEND parallel (t,6) */ into DWH_DATAFIX.MART_FDS_LI_WK_AJ_REBUILD
    --MART_FDS_LI_WK_TAB_REBUILD t
...
--                                  and fin_week_no = g_fin_week_no 
    ...
    select  /*+ parallel(rl,4), parallel(se,4), full (rl) full (di) full (dl) */
        ...
        ,se.fin_year_no
        ,se.fin_week_no
        ,dc.this_week_start_date --g_this_week_start_date
        ,g_fin_quarter_no
        ,g_fin_month_no 
        ,(se.fin_year_no*100+ se.fin_week_no) fin_yrwk_no
        ,fd_product_no
        ,sales
        ,sales_qty
        ,sales_cost
    ...
    from  selunion se, dwh_performance.dim_location dl, dwh_performance.dim_item di, dim_calendar_wk dc
    where se.sk1_location_no = dl.sk1_location_no
    and   se.sk1_item_no = di.sk1_item_no
    and   se.fin_year_no = dc.fin_year_no
    and   se.fin_week_no = dc.fin_week_no;

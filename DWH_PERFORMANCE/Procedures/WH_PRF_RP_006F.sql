--------------------------------------------------------
--  DDL for Procedure WH_PRF_RP_006F
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RP_006F" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RP_006F';
...
cursor c_fnd_rtl_shipment is
  ...
  WHERE  active_from_date > active_to_date and
...
begin
   ...
   g_rec_out.active_to_date     := g_rec_in.active_from_date;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    active_to_date      = a_tbl_update(i).active_to_date,
              valid_record_ind    = a_tbl_update(i).valid_record_ind,
              last_updated_date   = a_tbl_update(i).last_updated_date
       where  active_from_date    = a_tbl_update(i).active_from_date and
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_update(g_error_index).active_from_date;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;

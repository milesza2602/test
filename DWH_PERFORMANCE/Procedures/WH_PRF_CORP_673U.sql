--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_673U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_673U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        February 2010
--  Author:      M Munnik
...
g_sales_qty            rtl_prom_loc_item_dy.sales_qty%type;
g_actl_store_rcpt_qty  rtl_prom_loc_item_dy.actl_store_rcpt_qty%type;
g_sk1_prom_period_no   rtl_prom_loc_item_dy.sk1_prom_period_no%type;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_673U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'ROLLS SALES DENSE TO PROM/LOC/ITEM/DY';
...
cursor c_prom_loc_item_dy is
   select   dp.sk1_prom_no,
            (case when dn.post_date between dp.prom_start_date and dp.prom_end_date then g_sk1_prom_period_2
   ...
   where    dn.post_date              between g_start_date and g_end_date  and
            Dn.Last_Updated_Date           =  G_Date;
...
begin
   ...
   g_rec_out.sales_fr_cost                       := g_rec_in.sales_fr_cost;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    sk1_prom_period_no              = a_tbl_update(i).sk1_prom_period_no,
              ...
              sales_fr_cost                   = a_tbl_update(i).sales_fr_cost,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1),sum(sales_qty), sum(actl_store_rcpt_qty), sum(sk1_prom_period_no)
   into   g_count,g_sales_qty, g_actl_store_rcpt_qty, g_sk1_prom_period_no
...
-- Place data into and array for later writing to table in bulk
   if not g_found then
      ...
      if nvl(g_sales_qty,0)            <> nvl(g_rec_out.sales_qty,0)           or 
         nvl(g_actl_store_rcpt_qty,0)  <> nvl(g_rec_out.actl_store_rcpt_qty,0) or
         nvl(g_sk1_prom_period_no,0)   <> nvl(g_rec_out.sk1_prom_period_no,0)  then
...
begin
    ...
    l_text := 'LOAD RTL_PROM_LOC_ITEM_DY from SALES DENSE STARTED '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
FOR G_SUB IN 0..5
  LOOP
   ...
   CASE WHEN G_SUB = 0 THEN
        G_START_DATE := G_DATE - 41;
        G_END_DATE   := G_DATE - 35;
        WHEN G_SUB = 1 THEN
        G_START_DATE := G_DATE - 34;
        G_END_DATE   := G_DATE - 28;
        WHEN G_SUB = 2 THEN
        G_START_DATE := G_DATE - 27;
        G_END_DATE   := G_DATE - 21;
        WHEN G_SUB = 3 THEN
        G_START_DATE := G_DATE - 20;
        G_END_DATE   := G_DATE - 14;
        WHEN G_SUB = 4 THEN
        G_START_DATE := G_DATE - 13;
        G_END_DATE   := G_DATE - 7;
        WHEN G_SUB = 5 THEN
        G_START_DATE := G_DATE - 6;
        G_END_DATE   := G_DATE - 0;
   END CASE;

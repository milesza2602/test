--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110U_TESTABORT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110U_TESTABORT" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Test an abort 
--**************************************************************************************************
--  This is soley for the rerun of the spinner part = wh_prf_corp_110u_0
--        to load aborted data from batch_date = 21 october 2014
--  ie. job run on 22nd october 2014
--  problem was that deadlock on table which aborted procedure but Appworx reflects finished succesfully
--
--**************************************************************************************************--  Date:        Sept 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_110U_TESTABORT';
...
cursor c_fnd_rtl_loc_item_dy_rms_sale is
   select /*+ full(di) full(dl) full(dlh) full(dih) full(fnd_li) parallel (fnd_li,4)  */  
          post_date,
          sales_qty,
          sales,
          sales_cost,
...
begin
--l_text := '-1---about to abort in local_bulk_merg----';
--dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--    g_test_abort := 10/0;  
--l_text := '-2---after abort in local_bulk_merg----';
--dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
SET           sales_qty                       = mer_rlidrd.sales_qty,
              ...
              clear_sales_returns_qty         = mer_rlidrd.clear_sales_returns_qty/0,
...
(
   ...
   exception
      when others then
      l_text := '-3---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          l_text := 'g_erro_count - '||sql%bulk_exceptions.count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_text := '-4---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       ...
       dwh_log.record_error(l_module_name,sqlcode,l_message);
             l_text := '-5---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       for i in 1 .. g_error_count loop
          ...
          dwh_log.record_error(l_module_name,sqlcode,l_message);
             ...
             l_text := '-6---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       raise;
             l_text := '-7---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
   ...
   exception
      when dwh_errors.e_insert_error then
                   l_text := '-8---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       l_message := dwh_constants.vc_err_lw_insert||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
                          l_text := '-9---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       raise;
                          l_text := '-10---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

      when others then
       l_message := dwh_constants.vc_err_lw_other||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
                                 l_text := '-11---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       raise;
                                 l_text := '-12---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
l_text := '----**----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
l_text := 'This is soley for the rerun of the spinner part = wh_prf_corp_110u_0';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
l_text := 'to load aborted data from batch_date = 21 october 2014';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
l_text := 'ie. job run on 22nd october 2014';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
l_text := 'problem was that deadlock on table which aborted procedure but Appworx reflects finished succesfully';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
l_text := '----**----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    ...
    loop
      ...
      loop
         ...
         local_write_output;
                                          l_text := '-13---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
  ...
  exception
      when dwh_errors.e_insert_error then
                                                l_text := '-14---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       l_message := dwh_constants.vc_err_mm_insert||sqlcode||' '||sqlerrm;
       dwh_log.record_error(l_module_name,sqlcode,l_message);
                                                       l_text := '-15---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       dwh_log.update_log_summary(l_name,l_system_name,l_script_name,l_procedure_name,l_description,
                                  l_process_type,dwh_constants.vc_log_aborted,'','','','','');
                                  
       rollback;
                                                       l_text := '-16---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       p_success := false;
                                                       l_text := '-17---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       raise;
                                                       l_text := '-18---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

      when others then
                                                      l_text := '-19---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       ...
       dwh_log.update_log_summary(l_name,l_system_name,l_script_name,l_procedure_name,l_description,
                                  l_process_type,dwh_constants.vc_log_aborted,'','','','','');
                                                                                         l_text := '-20---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       rollback;
                                                              l_text := '-21---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       p_success := false;
                                                              l_text := '-22---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
       raise;
                                                              l_text := '-23---after abort in local_bulk_merg----';
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);


END WH_PRF_CORP_110U_TESTABORT;

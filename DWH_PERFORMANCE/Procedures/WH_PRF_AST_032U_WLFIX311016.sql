--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_032U_WLFIX311016
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_032U_WLFIX311016" 
...
l_module_name           sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_032U_WLFIX311016';
...
g_date := '27 JUNE 2016';
    ...
    l_text := 'truncate tablemart_chastchnscwk_WLFIX311016';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

execute immediate ('truncate table DWH_PERFORMANCE.mart_chastchnscwk_WLFIX311016');
...
FOR g_sub IN 0..17
 ...
 --  g_prev_week_end_date := g_this_week_start_date - 1;

   select ly_fin_week_no, ly_fin_year_no 
   ...
   into   g_this_week_start_date_ly, g_this_week_end_date_ly
   from   dim_calendar
   where  fin_year_no = g_fin_year_no_ly and fin_week_no = g_fin_week_no_ly;



      l_text := ' g_start_year='||g_start_year
                   ||' g_start_week='||g_start_week
                   ||' g_this_week_start_date='||g_this_week_start_date
                   ||' g_this_week_end_date='||g_this_week_end_date
                   ||' g_fin_week_code='||g_fin_week_code
                   ||' g_start_month='||g_start_month;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_text := ' SUB='||G_SUB
               ||' g_prev_week_end_date='||g_prev_week_end_date
               ||' g_fin_week_no_ly='||g_fin_week_no_ly
               ||' g_fin_year_no_ly='||g_fin_year_no_ly
               ||' g_this_week_start_date_ly='||g_this_week_start_date_ly
               ||' g_this_week_end_date_ly='||g_this_week_end_date_ly;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--MRT_CACSW_M20132_7
 ...
 --         commit; 
      ...
      insert /*+ APPEND parallel (mart,4) */ into DWH_PERFORMANCE.mart_chastchnscwk_WLFIX311016
-- Approved Plan - extract1
      ...
      group by apln.sk1_chain_no, apln.sk1_style_colour_no, apln.fin_year_no, apln.fin_week_no, G_fin_week_code),
-- Original Plan - extract2
      ...
      where  orig.sk1_plan_type_no = 62
       and   ORIG.fin_year_no      = G_START_YEAR
       and   ORIG.fin_week_no      = G_START_WEEK
      group by orig.sk1_chain_no, orig.sk1_style_colour_no, orig.fin_year_no, orig.fin_week_no, G_fin_week_code),
...
--             sum(act.selling_price) * sum(sit_qty) sit_selling,
      ...
      GROUP BY act.sk1_chain_no, act.sk1_style_colour_no, act.fin_year_no, act.fin_week_no, G_fin_week_code),
-- Pre-Actualised Plan - extract4
      ...
      group by preact.sk1_chain_no, preact.sk1_style_colour_no, preact.fin_year_no, preact.fin_week_no, G_fin_week_code),
...
--             aplnch.chain_closing_stock_selling -- -- 15/02/13 - incorrect plan type
      ...
      where  aplnch.sk1_plan_type_no = 60
       and    aplnch.fin_year_no      = G_START_YEAR
       and    aplnch.fin_week_no      = G_START_WEEK),
-- Original Chain StyleColour- - extract6
      ...
      where  oplnch.sk1_plan_type_no  = 62
       and    oplnch.fin_year_no      = G_START_YEAR
       and    oplnch.fin_week_no      = G_START_WEEK),
-- Actualised Chain StyleColour - extract7
      ...
      where  actch.sk1_plan_type_no   = 63
       and    actch.fin_year_no      = G_START_YEAR
       and    actch.fin_week_no      = G_START_WEEK),
-- Pre-Actualised Chain StyleColour - extract8
      ...
      where  preactch.sk1_plan_type_no   = 64
       and    preactch.fin_year_no      = G_START_YEAR
       and    preactch.fin_week_no      = G_START_WEEK),
-- LY Original Plan - extract9
      selcal as (
                  select distinct fin_year_no, fin_week_no, this_week_start_date, FIN_WEEK_CODE
                  from dim_calendar dc
                  where CALENDAR_DATE = g_this_week_end_date_ly
                  ),
...
--             'W'||(lyorig.fin_year_no + 1)||lyorig.fin_week_no as fin_week_code,
     ...
     group by lyorig.sk1_chain_no, lyorig.sk1_style_colour_no, cal.fin_year_no, cal.fin_week_no, CAL.FIN_WEEK_CODE),
-- LY Actualised Plan - extract10
      selcal as (
                    select distinct fin_year_no, fin_week_no, this_week_start_date
                    from dim_calendar dc
                 where CALENDAR_DATE = g_this_week_end_date_ly
               --   where DC.FIN_WEEK_NO = g_fin_week_no_ly AND DC.FIN_YEAR_NO = g_fin_year_no_ly  
 ),
      ly_act_data as (
      ...
      where  lyact.sk1_plan_type_no  = 63
       ...
       and   cal.fin_day_no     = 3
     group by lyact.sk1_chain_no, lyact.sk1_style_colour_no, cal.fin_year_no, cal.fin_week_no, CAL.FIN_WEEK_CODE),
-- Actual RMS Bought - extract11
      ...
      where  rmsact.sk1_style_colour_no = sc.sk1_style_colour_no
       and    rmsact.fin_year_no      = G_START_YEAR
       and    rmsact.fin_week_no      = G_START_WEEK
      group by rmsact.sk1_chain_no, rmsact.sk1_style_colour_no, rmsact.fin_year_no, rmsact.fin_week_no, G_fin_week_code),

      -- Actual JDA Data for Stock - extract12
      actual_data_stk as (
      select /*+ full (a) parallel (a,4) */ distinct
             a.sk1_chain_no,
    ...
    where  a.fin_year_no = b.fin_year_no
       ...
       AND a.fin_year_no = G_START_YEAR
       and   a.fin_week_no = G_START_WEEK
),

      rms_stock_data as (
      select /*+ index (stk PK_P_RTL_LC_SC_WK_RMS_STCK) */
             act.sk1_chain_no,
      ...
      from   rtl_loc_sc_wk_rms_stock stk,
             dim_location loc
      where  STK.fin_year_no = G_START_YEAR
       and   STK.fin_week_no = G_START_WEEK
        AND     stk.sk1_location_no     = loc.sk1_location_no     and
             loc.chain_no            = 30
...
end wh_prf_ast_032u_WLFIX311016;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_527FX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_527FX" (p_forall_limit in integer,p_success out boolean) as
...
select  /*+ full (lid) PARALLEL(lid,4) */         
  ...
  FROM  
    ...
    and  lid.post_date = '10 JUL 2018'
     and  lid.item_no     = di.item_no 
     and  lid.location_no = loc.location_no 
    and  lid.last_updated_date = '17 APR 2019' 

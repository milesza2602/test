--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_250U_WLFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_250U_WLFIX" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_250U_WLFIX';
...
--**************************************************************************************************
   g_recs_read := 0;
   g_recs_inserted :=  0;
   DELETE FROM DWH_PERFORMANCE.RTL_ITEM_TRADING a
...
--**************************************************************************************************
   ...
   INSERT /*+ APPEND */ INTO dwh_performance.rTL_ITEM_TRADING
with selitem as (select item_no, sk1_item_no, sk1_business_unit_no, business_unit_no 
 ...
 select /*+ PARALLEL(f,4) FULL(f) */
     ...
     from dwh_performance.rtl_loc_item_dy_rms_dense f,
          selitem di
...
--**************************************************************************************************
     ...
     update dwh_performance.rTL_ITEM_TRADING rit 
     set no_of_weeks = (
     ...
     where sk1_item_no in (select sk1_item_no from dwh_performance.dim_item where business_unit_no <> 50);

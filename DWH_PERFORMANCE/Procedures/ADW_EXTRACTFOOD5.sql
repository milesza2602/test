--------------------------------------------------------
--  DDL for Procedure ADW_EXTRACTFOOD5
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_EXTRACTFOOD5" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_EXTRACTFOOD5';
...
'select lid.fin_year_no,lid.fin_week_no,dc.this_week_start_date,location_no,location_name,item_no,sales_cost,sales,sales_qty
from rtl_loc_item_wk_rms_dense lid,
     dim_item di,
     dim_location dl,
     dim_calendar_wk dc
where lid.sk1_location_no = dl.sk1_location_no and
      ...
      lid.sales_qty         <> 0  and lid.sales_qty is not null and        
      di.business_unit_no  = 50 and
      lid.fin_year_no = 2014 and lid.fin_week_no between 10 and 13','|','DWH_FILES_OUT','extr_food_sales_2014_1013');    

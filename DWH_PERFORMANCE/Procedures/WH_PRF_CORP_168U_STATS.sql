--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_168U_STATS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_168U_STATS" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2013
--  Author:      Q. Smit
...
--   select min(trading_date), max(trading_date), min(post_date), max(post_date)
...
--     if g_recs_inserted > 0 then
         ...
         dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         DBMS_STATS.gather_table_stats ('dwh_performance', 'rtl_jdaff_st_plan_analysis_dy', DEGREE => 16);

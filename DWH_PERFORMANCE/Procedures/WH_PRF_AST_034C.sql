--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_034C
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_034C" 
...
--  08 Mar 2018   VAT 2018 (increase) amendements as required                                       REF08Mar2018
--
--  Naming conventions
--  g_  -  Global variable
...
--
    ...
    insert /*+append */ into  dwh_performance.mart_ch_ast_chn_grd_sc_wk m
      with
    --------------------------------------------------------------------------------
    -- last completed week
    --------------------------------------------------------------------------------
   ...
   --                and sk1_chain_no = 243
    ...
    --------------------------------------------------------------------------------
     sixwk as (
              ...
              from dwh_performance.temp_rtl_ast_sc_6wk_034b  w
      ...
      seldays as
              ...
              WKCAT.SK1_STYLE_COLOUR_NO  ,      --     sk1_item_no,
                           ...
                           dcwkcat.fin_half_end_date,
                                   case when dcwkcat.this_week_start_date < '14 nov 2016'   then ch_num_avail_days  
                                   else avail_ch_num_avail_days end  
                           NUM_AVAIL_DAYS_STD_ACT   ,
                                   case when dcwkcat.this_week_start_date < '14 nov 2016'  then ch_num_catlg_days  
...
--- Check VAT Rate    REF08Mar2018            
        case
            when nvl(ac.this_week_start_date,nvl(mg.this_week_start_date, nvl(ai.this_week_start_date, nvl(ss.this_week_start_date,  nvl(sg.this_week_start_date, av.this_week_start_date))))) < '01 Apr 2018'
             then round((max(selling_price) / 1.14),2) 
            else 
                  round((max(selling_price) / 1.15),2)                                      
             end                                                                                  reg_rsp_excl_vat,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_044U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_044U_SS" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
--
--  Tables:      Input    - DWH_PERFORMANCE.RTL_LOC_EMP_JOB_PAYCAT_DY_SS
--               Output   - DWH_PERFORMANCE.rtl_loc_emp_job_paycat_wk_ss
...
g_count              number        :=  0;
g_rec_out            RTL_LOC_EMP_JOB_PAYCAT_WK_SS%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_044U_SS';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_LOC_EMP_JOB_PAYCAT_WK_SS data  EX FOUNDATION';
...
WITH selext1 AS (
        SELECT /*+ full(RTL) parallel(RTL,6) */
          ...
          FROM dwh_performance.RTL_LOC_EMP_JOB_PAYCAT_DY_SS rtl
              ,dim_payment_category dpc
              ,rtl_s4s_business_unit_ss db
              ,rtl_job_ss dj
              ,dim_calendar dc
     ...
     selext3 as (
        ...
        SELECT 
          ...
          FROM SELEXT3 se3
          left outer join dwh_performance.rtl_emp_constr_loc_job_wk_ss rc
                  on rc.sk1_employee_id = se3.sk1_employee_id
                 and rc.fin_year_no     = se3.fin_year_no
                 and rc.fin_week_no     = se3.fin_week_no
          left outer join dwh_performance.rtl_loc_emp_job_paycat_wk_ss flr
          ...
          join dwh_performance.rtl_emp_job_wk_ss ej
     ...
     union
        SELECT 
            distinct 
                ...
                flr.sk1_employee_id rtl_exists
          FROM SELEXT3 se3
          join dwh_performance.rtl_emp_constr_loc_job_wk_ss rc
            on rc.sk1_employee_id = se3.sk1_employee_id
           and rc.fin_year_no = se3.fin_year_no
           and rc.FIN_WEEK_NO = se3.fin_week_no
          left outer join dwh_performance.rtl_loc_emp_job_paycat_wk_ss flr
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_loc_emp_job_paycat_wk_ss  values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
     UPDATE rtl_loc_emp_job_paycat_wk_ss
              SET 
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_EMP_JOB_PAYCAT_DY_SS  EX FOUNDATION STARTED '||
...
--  g_rec_cnt := 0;
 ...
 -- Select count(*) into g_rec_cnt from dwh_foundation.FND_S4S_LOC_EMP_job_PAYCAT_DY where last_updated_date = g_date;
  If g_rec_cnt  > 0 then 
          ...
          l_text := 'Running GATHER_TABLE_STATS ON RTL_LOC_EMP_JOB_PAYCAT_DY_SS';
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
           DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                         'RTL_LOC_EMP_JOB_PAYCAT_DY_SS', DEGREE => 8);
      
              l_text := 'TRUNCATE TABLE DWH_PERFORMANCE.rtl_loc_emp_job_paycat_wk_ss';
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_119U_BCK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_119U_BCK" (p_forall_limit in integer,p_success out boolean) as
...
select /*+ MATERIALIZE  PARALLEL(ALOC,8) PARALLEL(LI) FULL(LI)  FULL(DIH) FULL(DL) FULL(DLH) */
   ...
   where  aloc.item_no                = di.item_no          and
          ...
          di.sk1_item_no              = li.sk1_item_no      and       --NEW
          dl.sk1_location_no          = li.sk1_location_no  and       --NEW

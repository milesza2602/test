--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_861U_TST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_861U_TST" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_861U_TST';
   ...
   --A. Remove previous load periods for reload ...
    begin
       for g_sub in 1 .. g_loop_cnt                                                      -- BK30Sep2016                                                
         ...
         --execute immediate 'alter table '|| 'DWH_DATAFIX' || '.'|| 'AJ_MRT_FDS_LOC_ITEM_WK_TABLEAU' ||' truncate subpartition for ('||G_LOOP_FIN_YEAR_NO||','||G_LOOP_FIN_WEEK_NO||')';
    ...
    insert /*+ APPEND parallel (t,6) */ into DWH_DATAFIX.AJ_MRT_FDS_LOC_ITEM_WK_TABLEAU t
...
-- Catalogue Information
                                  ...
                                  from DWH_DATAFIX.AJ_TEMP_FOODS_TAB_2YRS_CATALOG a
                                  where fin_year_no = g_fin_year_no
                                  and fin_week_no = g_fin_week_no

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_661U_VATWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_661U_VATWL" 
...
--                Output - rtl_po_supchainlocitemdy_vatwl
...
g_rec_out            rtl_po_supchainlocitemdy_vatwl%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_661U_VATWL';
...
type tbl_array_i is table of rtl_po_supchainlocitemdy_vatwl%rowtype index by binary_integer;
type tbl_array_u is table of rtl_po_supchainlocitemdy_vatwl%rowtype index by binary_integer;
      ...
      as
         (select    /*+ materialize full(pi) full(tp) */
                            distinct
                              tp.po_no
                            , tp.item_no
                   from DWH_PERFORMANCE.temp_po_list_VATWL tp 
...
----------------------
   ...
   (
      select    /*+ materialize parallel(di,4) full(di) 
                            ...
                            , vat_rate_perc
                   from          dim_item di
                   join          DWH_PERFORMANCE.temp_po_list_VATWL tp            on  tp.item_no            = di.item_no
     ...
     selship as
       (select   
                 ...
                 from      fnd_rtl_shipment s
                  join      DWH_PERFORMANCE.temp_po_list_VATWL tp    
...
(
   ...
   /*+  MATERIALIZE parallel(P,4) full(TP) */
    ...
    join      DWH_PERFORMANCE.temp_po_list_VATWL tp       on  Tp.item_no                   = p.item_no
...
--**************************************************************************************************
--
-- Do calculations and load into rtl_po_supchainlocitemdy_vatwl
--
--**************************************************************************************************
procedure MERGE_INTO_RTL as
begin

MERGE INTO  DWH_PERFORMANCE.rtl_po_supchainlocitemdy_vatwl RTL
 using ( SELECT A.* , G_DATE+100 GDATE100 FROM     DWH_PERFORMANCE.TEMP_PO_CALC A ) MER_MART
 ...
 when matched then
  update
     set      sk1_contract_no		              = MER_MART.sk1_contract_no,
              ...
              last_updated_date               = MER_MART.GDATE100, --last_updated_date,
  ...
  when not matched then
   ...
   values 
    ...
    g_recs_inserted :=  SQL%ROWCOUNT;
    commit; 
    l_text := 'Recs merged into rtl_po_supchainlocitemdy_vatwl='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    insert into DWH_PERFORMANCE.temp_po_deletes_VATWL
    ...
    from   rtl_po_supchainlocitemdy_vatwl r
    join   DWH_PERFORMANCE.temp_po_list_VATWL t     on  r.sk1_po_no        = t.sk1_po_no
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    delete from DWH_PERFORMANCE.rtl_po_supchainlocitemdy_vatwl r
    where  exists(select t.sk1_po_no
                  from   DWH_PERFORMANCE.temp_po_deletes_VATWL t
   ...
   --    COMMENTED OUT CODE                                                                 --    delete from rtl_po_supchainlocitemdy_vatwl r
                                                                        --    where  exists(select t.po_no
                                                                        --                  from   DWH_PERFORMANCE.temp_po_list_VATWL t
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD OF rtl_po_supchainlocitemdy_vatwl EX FOUNDATION STARTED '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_661U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_661U" 
...
--
-- WENDY LYTTLE 25 AUGUST 2016 - added      WHERE dl.CHAIN_NO IN (10,40)
--                                to ensure loading of only chain_no = 10,40 data
--
...
--
--
--   3 August 2018  A.Ugolini Amend the truncate of the TEMP_PO_DELETES table to reflect TEMP_PO_DELETES_DJ
--
------------------------------------------------------------------------------------------
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_661U';
...
LEFT OUTER JOIN FND_ITEM_VAT_RATE  fi_vr1 on (sp.item_no        = fi_vr1.item_no                                                                           -- VAT rate change
                                         ...
                                         and  (case when IP.fd_discipline_type in('SA','SF') and (SP.not_after_date is not null) then                     -- VAT rate change
                                               SP.not_after_date else SP.not_before_date end)  between fi_vr1.active_from_date and fi_vr1.active_to_date)   -- VAT rate change

WHERE DL.CHAIN_NO IN (10,40)
GROUP BY Sp.sk1_po_no
          ...
          , nvl(ds.sk1_supplier_no, 0) 
           ...
           --, NVL(LI.TAX_PERC, (case when vat_region_no = 1000 then ip.vat_rate_perc else dl.DEFAULT_TAX_REGION_NO_PERC end ) ) ,        --VAT rate change
...
--**************************************************************************************************
 ...
 --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    ...
    execute immediate 'truncate table DWH_PERFORMANCE.temp_po_deletes_dj';
    l_text := 'TABLE DWH_PERFORMANCE.temp_po_deletes_dj TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DELETE_INVALID_RECS;

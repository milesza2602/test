--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_664U_FIXWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_664U_FIXWL" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_664U_FIXWL';
...
cursor c_rtl_loc_item_dy_po_supchain is
   with chgrecs as (select  /*+ MATERIALIZE FULL(D)   PARALLEL(D,4) */  tran_date, sk1_location_no, sk1_item_no 
                    from     rtl_po_supchain_loc_item_dy D
...
/*                    union
                    ...
                    group by tran_date, sk1_location_no, sk1_item_no) */
   select   /*+ FULL(LID) PARALLEL(LID,4) */ lid.sk1_location_no,
            lid.sk1_item_no,
            lid.tran_date,
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into rtl_loc_item_dy_po_supchain values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update rtl_loc_item_dy_po_supchain
      set     original_po_selling		          = a_tbl_update(i).original_po_selling,
...
-- Check to see if record is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_loc_item_dy_po_supchain
...
begin
    ...
    delete from rtl_loc_item_dy_po_supchain r

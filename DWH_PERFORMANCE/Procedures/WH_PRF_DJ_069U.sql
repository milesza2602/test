--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_069U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_069U" 
                                                                (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        April 2015
--  Author:      Alfonso Joshua
--  Purpose:     Create purchase_order dimension table in the performance layer
--               with dim_purchase_order data ex foundation table.
--  Tables:      Input  - dim_purchase_order,
--               Output - dim_dj_purchase_order
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
-- W lyttle 25 aug 2016
-- Due to business process not being followed at times, the same po is created for WW and DJ
-- We create a list of these and they are excluded from processing until fixed
-- See code below
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_069U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE DIM_DJ_PURCHASE_ORDER EX DIM_PURCHASE_ORDER';
...
cursor c_dim_purchase_order is
with selpo as(   select distinct PO_NO
                 from            dim_purchase_order A
                 where last_updated_date = g_date  
                 minus
                 select distinct po_no from DWH_PERFORMANCE.RTL_EXCLUDE_PO z
                                   where z.last_updated_date = g_date
                                   and po_load_seq = g_seq)
   ...
   select          sk1_po_no,
                   ...
                   po_short_desc,
                   po_long_desc
   from            dim_purchase_order A, selpo sp
   where a.po_no = sp.po_no
   ;


g_rec_in             c_dim_purchase_order%rowtype;
-- For input bulk collect --
type stg_array is table of c_dim_purchase_order%rowtype;
...
begin
   ...
   g_rec_out.po_short_desc                   := g_rec_in.po_short_desc;
   g_rec_out.po_long_desc                    := g_rec_in.po_long_desc;
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      set    po_no                           = a_tbl_update(i).po_no,
             into_store_date                 = a_tbl_update(i).into_store_date,
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).sk1_po_no    = g_rec_out.sk1_po_no then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF DIM_PURCHASE_ORDER EX dim_purchase_order STARTED '||
...
--**************************************************************************************************
 ...
 --       G_DATE := '26 AUG 2016';
   ...
   execute immediate 'alter session enable parallel dml';
     ...
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);         
--*****************************************************************************************************
-- Due to business process not being followed at times, the same po is created fro WW and DJ
-- We create a list of these and they are excluded from processing until fixed
-- W lyttle 25 aug 2016
-------------------
      g_seq := 0;
     BEGIN 
      g_seq := TEMP_SEQ.nextval;
      insert /*+ append */ 
      into DWH_PERFORMANCE.RTL_EXCLUDE_PO
      with selext as(select  /*+ full(a) parallel(a,4) */   
             distinct      sk1_po_no,
                         PO_NO
                       from            dim_purchase_order a
                       union
                       ...
                       distinct     sk1_po_no,
                                       PO_NO
                       from dim_dj_purchase_order b),
      selext2 AS (   select po_no, count(*) CNT from selext
                     group by po_no
                     having count(*) > 1)
      select po_no, g_date, g_seq
      from selext2;
  ...
  END;
    ...
    l_text := 'Duplicate PO s excluded from load = '||g_recs_inserted ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    g_recs_inserted := 0;
--**************************************************************************************************
    open c_dim_purchase_order;
    fetch c_dim_purchase_order bulk collect into a_stg_input limit g_forall_limit;

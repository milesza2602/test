--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_662U_VATWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_662U_VATWL" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_662U_VATWL';
...
--**************************************************************************************************
--
-- Do calculations and load into TEMP_POSHIP_CALC
--
--**************************************************************************************************
...
INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.TEMP_POSHIP_CALC
...
(
   ...
   from      fnd_rtl_purchase_order p
    join      dwh_performance.temp_po_list_vatwl tp           on  p.po_no              = tp.po_no
...
select      /*+ materialize */   
                ...
                , dlh.sk2_location_no
   from selpo p 
    join      dwh_performance.dim_location_vatwl l on p.location_no = l.location_no
...
select         /*+ materialize parallel(li,4) parallel(ldd,4) full(di) full(dih)  full(dl) full(dlh) */
    ...
    left join dim_contract dc           on  ps.contract_no        = dc.contract_no
...
--------------------------------
 select
    ...
    commit; 
    l_text := 'Recs inserted into TEMP_POSHIP_CALC='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   exception
     when others then
       l_message := 'error in INSERT_TEMP_POSHIP_CALC '||sqlcode||' '||sqlerrm;
...
MERGE INTO  DWH_PERFORMANCE.rtl_po_supchainlocitemdy_VATWL RTL
 using ( SELECT A.*, G_DATE+200 GDATE200 FROM     DWH_PERFORMANCE.TEMP_POSHIP_CALC A ) MER_MART
 ...
 when matched then
  update
     set     sk1_contract_no		              = MER_MART.sk1_contract_no,
              ...
              last_updated_date               = MER_MART.GDATE200 --last_updated_date 
  ...
  when not matched then
   ...
   values 
         ...
         ,MER_MART.GDATE200 --last_updated_date 
                  );
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table DWH_PERFORMANCE.TEMP_POSHIP_CALC';
    l_text := 'DWH_PERFORMANCE.TEMP_POSHIP_CALC TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

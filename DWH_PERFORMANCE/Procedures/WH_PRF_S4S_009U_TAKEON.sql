--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_009U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_009U_TAKEON" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_009U_TAKEON';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_EMP_JOB_wk data  EX FOUNDATION';
...
begin
    ...
    l_text := 'LOAD OF RTL_EMP_JOB_wk  EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    BEGIN
           delete from DWH_PERFORMANCE.RTL_EMP_JOB_wk
           where (SK1_employee_id, job_start_date) in (select distinct SK1_employee_id, job_start_date from DWH_PERFORMANCE.RTL_EMP_JOB_dy RTL
        ...
        l_text := 'Deleted from DWH_PERFORMANCE.RTL_EMP_JOB_WK recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date;
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
    exception
           when no_data_found then
                  l_text := 'No deletions done for DWH_PERFORMANCE.RTL_EMP_JOB_WK ';
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
     end;          
...
-- might need to remove CONSTRAINT_DATE, no of weeks from table
      ...
      INSERT /*+ APPEND */ INTO DWH_PERFORMANCE.RTL_EMP_JOB_wk
      WITH seldat as
   ...
   commit;
    ...
    l_text := 'Running GATHER_TABLE_STATS ON DWH_PERFORMANCE.RTL_EMP_JOB_wk';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_EMP_JOB_WK', DEGREE => 8);
    l_text := 'Completed GATHER_TABLE_STATS ON DWH_PERFORMANCE.RTL_EMP_JOB_wk';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_199U_TST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_199U_TST" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_199U_TST';
...
--delete from  mart_fd_area_item_dy where calendar_date = g_date;
--g_recs_deleted := g_recs_deleted + sql%rowcount;

--commit;

g_loop_date := '03 jul 17';
...
Insert  /*+ APPEND */ into dwh_datafix.aj_mart_fd_area_item_dy
...
catlg as (
    ...
    from   dwh_performance.rtl_loc_item_dy_catalog  rtl,
           ...
           items,
           dim_item_uda  du
    where  rtl.calendar_date = g_start_date
...
mart_filter as (
    select * from   dwh_datafix.aj_mart_fd_area_item_dy  
    where  calendar_date = g_start_date - 1),
    
final_join as (
select nvl(cat.sk1_area_no,mart.sk1_area_no) sk1_area_no,
       nvl(cat.sk1_item_no,mart.sk1_item_no) sk1_item_no,
       g_start_date calendar_date,
       nvl(cat.area_no,mart.area_no) area_no ,
       nvl(cat.item_no,mart.item_no) item_no ,
      (nvl(cat.fd_num_avail_days_adj,0) + nvl(mart.fd_num_avail_days_adj_ytd,0)) avail_days_adj_ytd,
      (nvl(cat.fd_num_catlg_days_adj,0) + nvl(mart.fd_num_catlg_days_adj_ytd,0)) catlg_days_adj_ytd,
       nvl(cat.commercial_manager_desc_562,mart.commercial_manager_desc_562) ,
       g_start_date - 1 as last_updated_date,
       (nvl(cat.fd_num_cust_avail_adj,0) + nvl(mart.fd_num_cust_avail_adj_ytd,0)) fd_num_cust_avail_adj_ytd,  -- chg-4956
...
where (fj.avail_days_adj_ytd <> 0 or fj.catlg_days_adj_ytd <> 0 or
  ...
  g_recs_inserted :=  g_recs_inserted + SQL%ROWCOUNT;
   ...
   l_text := 'RECORDS PROCESSED :- '||g_recs_read||' '||'for '||g_start_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

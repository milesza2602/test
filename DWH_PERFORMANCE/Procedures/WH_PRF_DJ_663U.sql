--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_663U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_663U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2015
...
--
--  Maintenance:
-- WENDY LYTTLE 25 AUGUST 2016 - added        AND L.CHAIN_NO IN (10,40)
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_663U';
...
cursor c_temp_po_supchain_sum_dj is
   select   *
   from     temp_po_supchain_sum_dj;

g_rec_in          c_temp_po_supchain_sum_dj%rowtype;
type stg_array is table of c_temp_po_supchain_sum_dj%rowtype;
...
begin
   ...
   insert        into dwh_performance.temp_po_supchain_sum_dj
   select        ps.sk1_po_no, ps.sk1_supply_chain_no, ps.sk1_location_no, ps.sk1_item_no, ps.tran_date,
   ...
   from
      (select    tl.sk1_po_no,
       ...
       join      temp_po_list_dj tl          on  r.sk1_po_no                = tl.sk1_po_no
       ...
       AND L.CHAIN_NO IN (10,40)
       group by  tl.sk1_po_no, (case when l.loc_type = 'W' then lv.sk1_location_no else r.sk1_location_no end), r.sk1_item_no) clc
   join          rtl_po_supchain_loc_item_dy_dj ps
   ...
   l_text := 'Records inserted to temp_po_supchain_sum_dj '||g_recs_inserted;
...
--**************************************************************************************************
    load_summary_values;
    open c_temp_po_supchain_sum_dj;
    fetch c_temp_po_supchain_sum_dj bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_temp_po_supchain_sum_dj;
--**************************************************************************************************

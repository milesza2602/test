--------------------------------------------------------
--  DDL for Procedure WH_PRF_FPI_008U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_FPI_008U_LOAD" 
...
--/*+ first_rows parallel(rtl) parallel(upd_rec) */
 ...
 FOR g_sub IN 0..31
  LOOP
   ...
   l_text       := 'Rollup range is:- '||g_loop_start_date||'  To '||g_loop_end_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   l_text       := 'Year/week range is:- '||g_loop_fin_year_no||' '||g_loop_fin_week_no;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   using (
    with items as (
         ...
         calendar_current as (
            ...
            from   dim_calendar 
            where  calendar_date between g_loop_start_date and g_loop_end_date
--            where calendar_date between '14/JAN/2019' and '20/JAN/2019' -- Week 31
            ),
         calendar_list as (
            select distinct
            ...
            from   dim_calendar 
            where  calendar_date between g_loop_start_date and g_loop_end_date
--            where calendar_date between '28/JAN/2019' and '03/FEB/2019' -- Week 31
            ),
...
--                  and po.fin_week_no = cl.fin_week_no
  ...
  values                                                                                                           
   ...
   l_text := 'RECORDS PROCESSED :- '||g_recs_read||' '||g_loop_fin_year_no||' '||g_loop_fin_week_no;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--     from dim_control;
     ...
     from   dim_calendar 
     where  calendar_date = '14 jan 19'; --g_date;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_739U_SHPD1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_739U_SHPD1" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_739U_SHPD1';
...
--**************************************************************************************************
 ...
 l_today_date     := g_date;
          ...
          l_text := 'l_today_date= '||l_today_date;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
-- USE TEST TABLE
 ...
 --                    and     (f.sk1_location_no = 10722	 and f.sk1_item_no = 15995248	and f.post_date = '27/SEP/16')
    ...
    commit;
          ...
          l_text := 'Period='||g_start_date||' - '||g_end_date||' l_today_date= '||l_today_date||' Recs MERGED = '||g_recs_inserted;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    g_date := g_date - 1;
 end loop;   

--------------------------------------------------------
--  DDL for Procedure G_RECS_READ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."G_RECS_READ" 
--**************************************************************************************************
--  Date:        June 2012
--  Author:      Quentin Smit
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_230T';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT RATE OF SALE AT ITEM LOCATION DAY LEVEL';
...
a_count_i           integer       := 0;
a_count_u           integer       := 0;
l_from_date         date := '23/JUL/12';    --trunc(sysdate) - 1;
l_to_date           date := l_from_date;
--in_batch            integer  :=1;
...
begin 
    ...
    l_text := 'LOAD OF rtl_loc_item_dy_rate_of_sale for last 2 days started AT '||

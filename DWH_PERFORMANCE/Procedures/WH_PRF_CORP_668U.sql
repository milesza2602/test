--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_668U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_668U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        March 2009
--  Author:      M Munnik
--  Purpose:     Combines PO and BOC into Contract Chain Item WK.
--               Contract tables will contain data for CHBD only.
--  Tables:      Input  - RTL_CONTRACT_CHAIN_ITEM_WK_po,
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_668U';
...
begin
    ...
    l_text := 'COMBINE PO and BOC to RTL_CONTRACT_CHAIN_ITEM_WK STARTED '||
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table dwh_performance.RTL_CONTRACT_CHAIN_ITEM_WK';
    l_text := 'TABLE RTL_CONTRACT_CHAIN_ITEM_WK TRUNCATED - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    execute immediate 'ALTER TABLE dwh_performance.RTL_CONTRACT_CHAIN_ITEM_WK DISABLE CONSTRAINT PK_N_RTL_CNTRCT_CHN_ITM_WK';
    l_text := 'CONSTRAINT DISABLED ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    insert   /*+ parallel(cont_wk,4) */ into dwh_performance.RTL_CONTRACT_CHAIN_ITEM_WK cont_wk
    select   /*+ PARALLEL(WPO,4) FULL(WPO) PARALLEL(WBOC,4) FULL(WBOC) */
             ...
             wboc.boc_selling,
             wboc.boc_cost,
             0,  --wboc.boc_qty_all_time,       -- DEFAULTED TO '0' AS IT'S DONE IN THE NEXT PROGRAM MANUALLY (WH_PRF_CORP_668B)
             0,  --wboc.boc_selling_all_time,   -- DEFAULTED TO '0' AS IT'S DONE IN THE NEXT PROGRAM MANUALLY (WH_PRF_CORP_668B)
             0,  --wboc.boc_cost_all_time,      -- DEFAULTED TO '0' AS IT'S DONE IN THE NEXT PROGRAM MANUALLY (WH_PRF_CORP_668B)
    ...
    execute immediate 'ALTER TABLE dwh_performance.RTL_CONTRACT_CHAIN_ITEM_WK ENABLE CONSTRAINT PK_N_RTL_CNTRCT_CHN_ITM_WK';

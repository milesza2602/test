--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_285U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_285U" (
...
--  --  Date:      December 2014
  --  Author:      Quentin Smit 
  --  Purpose:     This is basically a copy of WH_PRF_CORP_285U but with some business rule changes for a new table.
  --               Setup all items with no promotions during the last 12 weeks according to certain rules
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_285U';
...
BEGIN
      ...
      l_text := 'LOAD OF TMP_MART_6WKAVG_PROM_DATES STARTED AT '|| TO_CHAR(sysdate,('dd mon yyyy hh24:mi:ss'));
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--
 ...
 ----- END -- this is for testing purposes
  ...
  --**************************************************************************************************
     EXECUTE immediate('truncate table DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES');
     l_text := 'truncate table DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

 --    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
 --                                  'TMP_MART_6WKAVG_PROM_DATES', DEGREE => 8);
 --   commit;
 --   l_text := 'GATHER STATS on dwh_performance.TMP_MART_6WKAVG_PROM_DATES';
 --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    BEGIN
        ...
        AND index_name = 'I10_P_TMP_MRT_6WKAV_PRM_DTS';
              execute immediate('DROP index DWH_PERFORMANCE.I10_P_TMP_MRT_6WKAV_PRM_DTS');
    exception
    ...
    BEGIN
        ...
        from dba_indexes
        where OWNER = 'DWH_PERFORMANCE'
        AND index_name = 'I20_P_TMP_MRT_6WKAV_PRM_DTS';
              execute immediate('DROP index DWH_PERFORMANCE.I20_P_TMP_MRT_6WKAV_PRM_DTS');
    exception
         when no_data_found then
          commit;
    END;
        ...
        from dba_indexes
        where OWNER = 'DWH_PERFORMANCE'
        AND index_name = 'I30_P_TMP_MRT_6WKAV_PRM_DTS';
              execute immediate('DROP index DWH_PERFORMANCE.I30_P_TMP_MRT_6WKAV_PRM_DTS');
    exception
         when no_data_found then
          commit;
    END;
   ...
   --**************************************************************************************************
  -- Do insert into dwh_performance.TMP_MART_6WKAVG_PROM_DATES
  --**************************************************************************************************
    G_RECS := 0;

      INSERT /*+ APPEND  */ INTO DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES aps
...
--10--------------------------------------
     Execute Immediate('CREATE INDEX DWH_PERFORMANCE.I10_P_TMP_MRT_6WKAV_PRM_DTS ON DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES
                            (WEEK_COUNT, ITEM_NO) TABLESPACE PRF_MASTER  NOLOGGING  PARALLEL') ;
 
      Execute Immediate('ALTER INDEX DWH_PERFORMANCE.I10_P_TMP_MRT_6WKAV_PRM_DTS LOGGING NOPARALLEL') ;
   
    l_text := 'create index  DWH_PERFORMANCE.i10_P_TMP_MRT_6WKAV_PRM_DTS ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--20----------------------------
     execute immediate('CREATE INDEX  DWH_PERFORMANCE.I20_P_TMP_MRT_6WKAV_PRM_DTS ON DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES
                            (WEEK_COUNT, SK1_PROM_NO) TABLESPACE PRF_MASTER NOLOGGING PARALLEL') ;
                            
     execute immediate('ALTER INDEX DWH_PERFORMANCE.I20_P_TMP_MRT_6WKAV_PRM_DTS LOGGING NOPARALLEL') ;

  
    l_text := 'create index  DWH_PERFORMANCE.i20_P_TMP_MRT_6WKAV_PRM_DTS ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);


--30-----------------------------------
     execute immediate('CREATE INDEX  DWH_PERFORMANCE.I30_P_TMP_MRT_6WKAV_PRM_DTS ON DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES
                            (ITEM_NO) TABLESPACE PRF_MASTER  NOLOGGING PARALLEL') ;
                            
      execute immediate('ALTER INDEX DWH_PERFORMANCE.I30_P_TMP_MRT_6WKAV_PRM_DTS LOGGING NOPARALLEL') ;
      
      l_text := 'create index DWH_PERFORMANCE.i30_P_TMP_MRT_6WKAV_PRM_DTS ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

 ------------------------------------- 
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'TMP_MART_6WKAVG_PROM_DATES', DEGREE => 8, cascade=>true);
    commit;
    l_text := 'GATHER STATS on DWH_PERFORMANCE.TMP_MART_6WKAVG_PROM_DATES';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

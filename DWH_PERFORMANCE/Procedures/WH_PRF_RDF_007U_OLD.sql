--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_007U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_007U_OLD" (p_forall_limit in integer, p_success out boolean)
...
--               Output - temp_loc_item_wk_rdf_sysfcst52 (unpivot temp table)
...
--        g_from_loc_no := p_from_loc_no;
 ...
 --   end if;
   ...
   insert /*+ parallel(sys,8) */ into dwh_performance.temp_loc_item_wk_rdf_sysfcst52 sys
   select /*+ parallel(wkfcst,8) */
           location_no,
            item_no,
    ...
    from    fnd_rtl_loc_item_wk_rdf_wkfcst wkfcst
...
--and item_no = 20000639
;
    -- order by location_no, item_no, fin_year_no, fin_week_no --desc;
    g_recs_inserted := + SQL%ROWCOUNT;
    commit;

    l_text := 'TEMP TABLE temp_loc_item_wk_rdf_sysfcst52 POPULATED';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text :=  dwh_constants.vc_log_records_inserted||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    commit;
    
    l_text := 'Running GATHER_TABLE_STATS ON temp_loc_item_wk_rdf_sysfcst52';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   DBMS_STATS.gather_table_stats ('dwh_performance', 'temp_loc_item_wk_rdf_sysfcst52', DEGREE => 16);

   l_text := 'GATHER_TABLE_STATS ON temp_loc_item_wk_rdf_sysfcst52 completed';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--   select /*+ parallel (a,4) full(a) */  min(fin_year_no) 
--     into l_temp_min_year
--     from dwh_performance.temp_loc_item_wk_rdf_sysfcst52 a;
--     
--   select /*+ parallel (a,4) full(a) */  min(fin_week_no) 
--     into l_temp_min_wk_min_year
--     from dwh_performance.temp_loc_item_wk_rdf_sysfcst52 a
--     where fin_year_no = l_temp_min_year; 
--
--   select /*+ parallel (a,4) full(a) */  max(fin_year_no) 
--     into l_temp_max_year
--     from dwh_performance.temp_loc_item_wk_rdf_sysfcst52 a;
--     
--   select /*+ parallel (a,4) full(a) */  min(fin_week_no) 
--     into l_temp_min_wk_max_year
--     from dwh_performance.temp_loc_item_wk_rdf_sysfcst52 a
--     where fin_year_no = l_temp_max_year; 
--     
--
--    l_text := 'min year on temp table = '|| l_temp_min_year ||' - min week for min year = ' || l_temp_min_wk_min_year;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--     
--    l_text := 'max year on temp table = '|| l_temp_max_year ||' - min week for max year = ' || l_temp_min_wk_max_year;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
 ...
 -- DO THE UPDATE / INSERT CHECK 
  with aa as (
    select /*+ parallel (a,8) full(a) */ unique c.sk1_LOCATION_NO, b.sk1_ITEM_NO, FIN_YEAR_NO, FIN_WEEK_NO 
      from dwh_performance.temp_loc_item_wk_rdf_sysfcst52 a, dim_item b, dim_location c
 ...
 -- NORMAL PROCESSING CONTINUES..
    ...
    -- C. PRF Table load ...
    -----------------------------------
    --merge /*+ APPEND (tgt,4) */ into dwh_performance.RTL_LOC_ITEM_WK_RDF_FCST52 tgt 
    merge /*+ parallel(tgt,8)  */ into dwh_performance.RTL_LOC_ITEM_WK_RDF_FCST52 tgt 
    USING
      (select  /*+ parallel(sys_ilv,8) full(sys_ilv) full(di) full(dl) full(dlh) full(dih) full(zi) */
                dl.sk1_location_no                                              SK1_LOCATION_NO, 
                di.sk1_item_no                                                  SK1_ITEM_NO,
                sys_ilv.fin_year_no                                             FIN_YEAR_NO,
                sys_ilv.fin_week_no                                             FIN_WEEK_NO,
                dih.sk2_item_no                                                 SK2_ITEM_NO,
                dlh.sk2_location_no                                             SK2_LOCATION_NO,
               ('W'||sys_ilv.fin_year_no||sys_ilv.fin_week_no)                  FIN_WEEK_CODE,
                sys_ilv.sales_wkly_sys_fcst_qty                                 SALES_WK_SYS_FCST_QTY,
                case when (di.standard_uom_code = 'EA' and di.random_mass_ind = 1)
                     then  sys_ilv.sales_wkly_sys_fcst_qty * zi.reg_rsp * di.static_mass
                     else  sys_ilv.sales_wkly_sys_fcst_qty * zi.reg_rsp
                end                                                             SALES_WK_SYS_FCST,
                sys_ilv.sales_wkly_app_fcst_qty                                 SALES_WK_APP_FCST_QTY,
                case when (di.standard_uom_code = 'EA' and di.random_mass_ind = 1)
                     then  sys_ilv.sales_wkly_app_fcst_qty * zi.reg_rsp * di.static_mass
                     else  sys_ilv.sales_wkly_app_fcst_qty * zi.reg_rsp
                end                                                             SALES_WK_APP_FCST,
                g_date                                                          LAST_UPDATED_DATE,
                dc.this_week_start_date                                         THIS_WEEK_START_DATE
      
       from     dwh_performance.temp_loc_item_wk_rdf_sysfcst52   sys_ilv 
...
--     where    sys_ilv.location_no between g_from_loc_no and g_to_loc_no
--      where (
--             (sys_ilv.fin_year_no = l_temp_min_year and sys_ilv.fin_week_no between l_temp_min_wk_min_year and l_temp_max_wk_min_year)
--        or   (sys_ilv.fin_year_no = l_temp_max_year and sys_ilv.fin_week_no between l_temp_min_wk_max_year and l_temp_max_wk_max_year)
--           )
...
/*    l_text := 'ENABLING PK CONSTRAINT - PK_P_NEW_LC_ITM_WK_FCST52';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate 'alter table dwh_performance.RTL_LOC_ITEM_WK_RDF_FCST52 enable constraint PK_P_NEW_LC_ITM_WK_FCST52';

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_556U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_556U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Oct 2008
--  Author:      Christie Koorts
--  Purpose:     Create location dept day fact table in the performance layer with
--               added value ex foundation layer till history table.
...
--
--  Maintenance:
--  1 April 2009 - defect 1225 - - no filter on driving cursor
--
--  Naming conventions:
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_556U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE RTL_LOC_DEPT_DY EX FND_RTL_TILL_HIST';
...
with chgrecs as
  (select    location_no, business_unit_no, tran_date
   ...
   select dl.sk1_location_no,
          ...
          sum(rth.sales_incl_vat - rth.sales_vat) till_analysis_sales,
          sum(rth.num_items) till_analysis_qty,
          sum(rth.num_trans) num_tran
   from   chgrecs cr
   join   fnd_rtl_till_hist rth             on  rth.location_no      = cr.location_no
                                            and rth.business_unit_no = cr.business_unit_no
                                            and rth.tran_date        = cr.tran_date
   join   dim_location dl                   on  rth.location_no      = dl.location_no
   join   dim_location_hist dlh             on  rth.location_no      = dlh.location_no
                                            and rth.tran_date between dlh.sk2_active_from_date and dlh.sk2_active_to_date
   join   dim_department dd                 on  dd.department_no     = (case rth.business_unit_no
                                                                        when 50 then 2100
                                                                        when 51 then 3104
                                                                        when 52 then 3102
                                                                        when 53 then 4100
                                                                        when 54 then 3103
                                                                        else rth.business_unit_no end)
   group by dl.sk1_location_no, dd.sk1_department_no, rth.tran_date;
...
begin
   ...
   g_rec_out.till_analysis_sales            := g_rec_in.till_analysis_sales;
   g_rec_out.till_analysis_qty              := g_rec_in.till_analysis_qty;
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      set sk2_location_no                = a_tbl_update(i).sk2_location_no,
          till_analysis_sales            = a_tbl_update(i).till_analysis_sales,
          till_analysis_qty              = a_tbl_update(i).till_analysis_qty,
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_DEPT_DY EX FND_RTL_TILL_HIST STARTED '||

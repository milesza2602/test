--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_124U_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_124U_TEST" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_124U_TEST';
...
cursor c_fnd_rtl_loc_item_dy_om_ord is
   select /*+ parallel (ord,4) parallel (so,4) parallel (prc,4) parallel (li,4) full (di) full(dl) full (dih) full (dlh) full (li) */
          ord.*,
          di.sk1_item_no,
...
--                                         ord.post_date              = prc.calendar_date                              
          ...
          left outer join rtl_location_item li 
                                      on di.sk1_item_no             = li.sk1_item_no  and

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_251U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_251U" 
...
--  Date:        February 2013
--  Author:      Wendy lyttle
--  Purpose:     Extract first sale date per item for last 6 weeks - update season_first_trade_date
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_251U';
...
cursor c_first_sale_date is
   select /*+ PARALLEL(f,4) FULL(f) */
     ...
     from rtl_loc_item_dy_rms_dense f,
          dim_item di,
          rtl_item_trading rit
    where f.sk1_item_no = di.sk1_item_no
      and f.post_date between g_season_start_date and g_date
      and f.sales != 0
      and f.sales is not null
      and di.business_unit_no <> 50
      and f.sk1_item_no = rit.sk1_item_no

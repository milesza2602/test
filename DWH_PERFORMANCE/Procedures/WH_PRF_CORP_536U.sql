--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_536U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_536U" (
...
as
  ...
  --  01 Feb 2018 - Add Customer Availability Measure (Chg12260 - A Joshua)
  --
  --  Naming Conventions
  --  G_  -  Global Variable
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_536U';
...
--  commit;
  ...
  WITH cal as (
    select /*+ FULL(a) PARALLEL(a,6) */ a.sk1_item_no, a.sk1_location_no, max(a.calendar_date) calendar_date
  ...
  into rtl_loc_item_wk_catalog cat
  select  /*+ FULL(lid) PARALLEL(lid,6)  PARALLEL(z,6)  */

--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_213F
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_213F" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        September 2009
--  Author:      Alfonso Joshua
--  Purpose:     Load daily forecast table in performance layer
--               with input ex RDF Sale table from foundation layer.
--  Tables:      Input  - RTL_LOC_ITEM_RDF_DYFCST_L1
--               Output - RTL_LOC_ITEM_RDF_WKFCST_L1
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  04 Nov 2009 - W Lyttle - td2443 - Rename fields on RTL_LOC_ITEM_RDF_DYFCST_L1
--                                    & RTL_LOC_ITEM_RDF_WKFCST_L1
--  10 Mar 2010 - A. Joshua : TD3556 - new measures required as per ETL (abs_app_fcst_err_qty & abs_sys_fcst_err_qty)
--  11 Mar 2010 - A. Joshua : TD3556 - Change rollup date to sysdate - 7
--
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_213F';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD OF RDF ROLLUP WK FCST FACTS EX RDF DAILY FCST';
...
cursor c_RTL_LOC_ITEM_RDF_DYFCST_L1 is
  ...
  select  fcst.sk1_item_no,
          ...
          max(fcst.sk2_item_no) sk2_item_no,
          max(fcst.sk2_location_no) sk2_location_no,
          sum(fcst.sales_dly_sys_fcst_qty)               sales_dly_sys_fcst_qty,
          sum(fcst.sales_dly_sys_fcst)                   sales_dly_sys_fcst,
          sum(fcst.sales_dly_app_fcst_qty)               sales_dly_app_fcst_qty,
          sum(fcst.sales_dly_app_fcst)                   sales_dly_app_fcst,
          sum(fcst_err_sls_dly_app_fcst_qty)             fcst_err_sls_dly_app_fcst_qty,
          sum(fcst_err_sls_dly_sys_fcst_qty)             fcst_err_sls_dly_sys_fcst_qty,
          sum(fcst_err_sales_dly_app_fcst)               fcst_err_sales_dly_app_fcst,
          sum(fcst_err_sales_dly_sys_fcst)               fcst_err_sales_dly_sys_fcst,
          sum(nvl(fcst.abs_app_fcst_err_qty_ll,0))       abs_app_fcst_err_qty_ll,
          sum(nvl(fcst.abs_rltv_app_fcst_err_qty_ll,0))  abs_rltv_app_fcst_err_qty_ll,
          sum(nvl(fcst.abs_sys_fcst_err_qty_ll,0))       abs_sys_fcst_err_qty_ll,
          sum(nvl(fcst.abs_rltv_sys_fcst_err_qty_ll,0))  abs_rltv_sys_fcst_err_qty_ll,
          sum(fcst.abs_dly_app_fcst_err_qty_peri)        abs_dly_app_fcst_err_qty_peri,
          sum(fcst.abs_dly_sys_fcst_err_qty_peri)        abs_dly_sys_fcst_err_qty_peri,
          sum(fcst.abs_app_fcst_err_qty)                 abs_app_fcst_err_qty,
          sum(fcst.abs_sys_fcst_err_qty)                 abs_sys_fcst_err_qty
...
begin
   ...
   g_rec_out.sales_wk_sys_fcst_qty         := g_rec_in.sales_dly_sys_fcst_qty;
   g_rec_out.sales_wk_sys_fcst             := g_rec_in.sales_dly_sys_fcst;
   g_rec_out.sales_wk_app_fcst_qty         := g_rec_in.sales_dly_app_fcst_qty;
   g_rec_out.sales_wk_app_fcst             := g_rec_in.sales_dly_app_fcst;
   g_rec_out.abs_app_fcst_err_qty_ll       := g_rec_in.abs_app_fcst_err_qty_ll;
   g_rec_out.abs_rltv_app_fcst_err_qty_ll  := g_rec_in.abs_rltv_app_fcst_err_qty_ll;
   g_rec_out.abs_sys_fcst_err_qty_ll       := g_rec_in.abs_sys_fcst_err_qty_ll;
   g_rec_out.abs_rltv_sys_fcst_err_qty_ll  := g_rec_in.abs_rltv_sys_fcst_err_qty_ll;
   g_rec_out.abs_dly_app_fcst_err_qty_peri := g_rec_in.abs_dly_app_fcst_err_qty_peri;
   g_rec_out.abs_dly_sys_fcst_err_qty_peri := g_rec_in.abs_dly_sys_fcst_err_qty_peri;
   g_rec_out.abs_app_fcst_err_qty          := g_rec_in.abs_app_fcst_err_qty;
   g_rec_out.abs_sys_fcst_err_qty          := g_rec_in.abs_sys_fcst_err_qty;
   g_rec_out.fcst_err_sls_dly_app_fcst_qty := g_rec_in.fcst_err_sls_dly_app_fcst_qty;
   g_rec_out.fcst_err_sls_dly_sys_fcst_qty := g_rec_in.fcst_err_sls_dly_sys_fcst_qty;
   g_rec_out.fcst_err_sales_dly_app_fcst   := g_rec_in.fcst_err_sales_dly_app_fcst;
   g_rec_out.fcst_err_sales_dly_sys_fcst   := g_rec_in.fcst_err_sales_dly_sys_fcst;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into RTL_LOC_ITEM_RDF_WKFCST_L1 values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    sales_wk_sys_fcst_qty         = a_tbl_update(i).sales_wk_sys_fcst_qty,
              ...
              abs_dly_app_fcst_err_qty_peri = a_tbl_update(i).abs_dly_app_fcst_err_qty_peri,
              ...
              fcst_err_sls_dly_app_fcst_qty = a_tbl_update(i).fcst_err_sls_dly_app_fcst_qty,
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_text := 'LOAD RTL_LOC_ITEM_RDF_WKFCST_L1 ITSELF TABLES STARTED '||

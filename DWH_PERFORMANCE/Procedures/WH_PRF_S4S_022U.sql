--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_022U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_022U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
--               Output   - DWH_PERFORMANCE.RTL_LOC_JOB_WK  
...
g_count              number        :=  0;
g_rec_out            dwh_performance.RTL_LOC_JOB_WK%rowtype;
g_found              boolean;
g_date               date;
...
g_sub                integer       :=  0;
g_loop_cnt           integer       :=  30; -- Number of partitions to be truncated/replaced


l_message            sys_dwh_errlog.log_text%type;
l_procedure_name     sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_022U';
l_table_name         all_tables.table_name%type                := 'RTL_LOC_JOB_WK';
...
type tbl_array_i is table of dwh_performance.RTL_LOC_JOB_WK%rowtype index by binary_integer;
type tbl_array_u is table of dwh_performance.RTL_LOC_JOB_WK%rowtype index by binary_integer;
...
with selext as (SELECT  
          ...
          WHERE FLR.LOCATION_NO = DL.LOCATION_NO
            ...
            and flr.business_date between DE.sk1_effective_from_date and DE.sk1_effective_to_date
        ...
        FROM selext se,             
             DWH_PERFORMANCE.RTL_LOC_JOB_WK rtl
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions     
       insert into dwh_performance.RTL_LOC_JOB_WK  values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
             UPDATE dwh_performance.RTL_LOC_JOB_WK
                    SET
...
begin
    ...
    l_text := 'LOAD OF '||l_table_name||'  EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_S4S_HYP_LOC_JOB_WK', DEGREE => 8);
...
--**************************************************************************************************
   ...
   local_bulk_update;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats (l_table_owner,
                                   l_table_name, DEGREE => 8);

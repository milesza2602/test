--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_002U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_002U" 
...
--                        dim_item
--                        dim_location
--               Output - temp_rtl_area_item_wk_dense
...
g_rec_out            temp_rtl_area_item_wk_dense%rowtype;
g_found              boolean;
g_date               date;
g_start_date         date;
g_end_date         date;
...
l_description        sys_dwh_log_summary.log_description%type  := 'BRIDGETHORN ROLLUP TO TEMP_DENSE';
...
begin
    ...
    l_text := 'ROLLUP OF temp_rtl_loc_item_wk_dense EX WEEK LEVEL STARTED '||
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    execute immediate ('truncate table dwh_performance.temp_rtl_area_item_wk_dense');
    commit;
    l_text := 'TRUNCATED table dwh_performance.temp_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'temp_rtl_area_item_wk_dense', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_performance.temp_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

     execute immediate 'alter session enable parallel dml';

    INSERT /*+ APPEND */ INTO dwh_performance.temp_rtl_area_item_wk_dense

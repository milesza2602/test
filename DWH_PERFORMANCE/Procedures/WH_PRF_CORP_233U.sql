--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_233U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_233U" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - TMP_MART_FD_LOC_ITEM_WK
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_233U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'Create Foods Loc Item Week Mart - TEST TO CHECK SALES LY YTD values';    --QST
...
begin
    ...
    l_text := 'Create Foods Loc Item Week Mart STARTED AT '||
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table TMP_MART_FD_LOC_ITEM_WK');
...
INSERT /*+ APPEND */ INTO TMP_MART_FD_LOC_ITEM_WK mart
with item_list as (
...
--    rtl_loc_item_wk_rdf_fcst f, item_list il, store_list sl  -- level1/2 rename chg-2218
...
from RTL_LOC_ITEM_RDF_WKFCST_L1 f, item_list il, store_list_area sl -- level1/2 rename chg-2218
where f.sk1_item_no     = il.sk1_item_no
and f.sk1_location_no = sl.sk1_location_no
...
from RTL_LOC_ITEM_RDF_DYFCST_L2 f, item_list il, store_list_area sl -- level1/2 rename chg-2218
where f.sk1_item_no     = il.sk1_item_no
and f.sk1_location_no = sl.sk1_location_no
...
nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                          f2.sk1_location_no),
                                          f3.sk1_location_no),
                                          f6.sk1_location_no),
                                          fy0.sk1_location_no),
...
sk1_location_no,
nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                          f2.sk1_item_no),
                                          f3.sk1_item_no),
                                          f6.sk1_item_no),
                                          fy0.sk1_item_no),

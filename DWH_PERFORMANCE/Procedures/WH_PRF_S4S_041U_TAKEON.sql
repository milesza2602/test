--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_041U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_041U_TAKEON" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_041U_TAKEON';
...
--**************************************************************************************************
    ...
    BEGIN
           delete from DWH_PERFORMANCE.RTL_EMP_LOC_STATUS_wk
           where (SK1_employee_id, effective_start_date) in (select distinct SK1_employee_id, effective_start_date from DWH_PERFORMANCE.RTL_EMP_LOC_STATUS_DY RTL
        ...
        l_text := 'Deleted from DWH_PERFORMANCE.RTL_EMP_LOC_STATUS_WK recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
    exception
           when no_data_found then
                  l_text := 'No deletions done for DWH_PERFORMANCE.RTL_EMP_LOC_STATUS_WK ';
        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
     end;          
...
-- might need to remove CONSTRAINT_DATE, no of weeks from table
   ...
   commit;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_EMP_LOC_STATUS_WK', DEGREE => 8);

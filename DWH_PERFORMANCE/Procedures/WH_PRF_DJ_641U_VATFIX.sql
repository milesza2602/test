--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_641U_VATFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_641U_VATFIX" 
  (
...
as
  ...
  --                           dim_item
  --                           Dim_calendar
  --                           temp_fnd_boc_contract_list_dj
  --               Output -    temp_fnd_boc_contract_wk_dj
  ...
  --
  --  PROCESSING FLOW
  --   1. Clear temp_fnd_boc_contract_wk_dj
  --   2. Select contracts from temp_fnd_boc_contract_wk_dj
  --  With
  ...
  -- Insert into temp_fnd_boc_contract_list_dj
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_DJ_641U_VATFIX';
  ----
...
--------------
 ...
 --                 sum(contract_qty * di.base_rsp_excl_vat) contract_selling,
             ...
             from dwh_foundation.fnd_rtl_contract frc,
                  dwh_performance.temp_fnd_boc_contract_list_dj tf,
...
--    tf.sk2_item_no                     ,
    ...
    tf.fin_week_code                   ,
    tf.contract_status_code
     from  dwh_performance.temp_fnd_boc_contract_list_dj tf
 group by tf.sk1_contract_no               ,
    tf.contract_no               ,
    tf.item_no                         ,
    tf.sk1_item_no                     ,
...
g_rec_out    dwh_performance.temp_fnd_boc_contract_wk_dj%rowtype;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of temp_fnd_boc_contract_wk_dj%rowtype index by binary_integer;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into  dwh_performance.temp_fnd_boc_contract_wk_dj values a_tbl_insert(i);
...
begin
  ...
  l_text := 'LOAD OF TEMP_FND_BOC_CONTRACT_WK_DJ EX fnd_rtl_contract AND RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ STARTED '|| to_char ( sysdate,('dd mon yyyy hh24:mi:ss') ) ;
  dwh_log.write_log ( l_name,l_system_name,l_script_name,l_procedure_name,l_text ) ;
  ...
  dwh_log.write_log ( l_name,l_system_name,l_script_name,l_procedure_name,l_text ) ;
  --**************************************************************************************************
  -- TRUNCATE temp_fnd_boc_contract_list_dj
  -- DWH_LOG.WRITE_LOG   : temp_fnd_boc_contract_list_dj truncated
  --**************************************************************************************************
  execute immediate('truncate table  temp_fnd_boc_contract_wk_dj');
  commit;
  l_text :='TRUNCATED TEMP_FND_BOC_CONTRACT_WK_DJ AT '||to_char ( sysdate,('dd mon yyyy hh24:mi:ss') ) ;
  dwh_log.write_log ( l_name,l_system_name,l_script_name,l_procedure_name,l_text ) ;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_502FIX2NOV15
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_502FIX2NOV15" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_502FIX2NOV15';
...
--RTL_LCITMWRSPRS_M20131_1

    g_part_period := 'M'||g_fin_year_no||g_fin_month_no||'_'||g_fin_week_no;

    l_text      := 'Truncate started - for subpartition RTL_LCITMWRSPRS_'||g_part_period;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    g_stmt      := 'Alter table  DWH_PERFORMANCE.RTL_LOC_ITEM_WK_RMS_SPARSE truncate  subpartition RTL_LCITMWRSPRS_'||g_part_period;
...
INSERT 
/*+ APPEND */ 
INTO dwh_performance.rtl_loc_item_wk_rms_sparse 

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_263B
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_263B" (p_forall_limit in integer,p_success out boolean) as
...
--                        rtl_zone_item_om
--                        rtl_depot_item_wk
--               Output - MART_SUPPLIER_MASTER_DATA_HISQ
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_263B';
...
begin
    ...
    l_text := 'LOAD OF MART_SUPPLIER_MASTER_DATA_HISQ STARTED AT '||
...
MERGE  INTO W6005682.MART_SUPPLIER_MASTER_DATA_HISQ rtl_mart USING
(
with cal as (
     ...
     item_list as (select a.sk1_item_no,a.item_no,a.item_desc,a.department_no, a.subclass_no,
                   ...
                   from dim_item a, dim_department b, fnd_jdaff_dept_rollout c 
                    where a.business_unit_no = 50 
                      ...
                      and a.sk1_item_no in (select /*+ parallel(hist,4) full(hist) */ distinct hist.sk1_item_no
...
,
  ...
  ,                      
    deliv_pat as (select /*+ full(fzis) PARALLEL(fzis,4)  */ unique 
...
--                       and ll3.sk1_zone_group_zone_no     = zone_item_supp_hist.sk1_zone_group_zone_no
     ...
     left join loc_list ll_2 on ll_2.location_no            = zone_item_om.from_loc_no
     --join loc_list ll_2 on ll_2.location_no            = zone_item_om.from_loc_no                            -- BK03May16

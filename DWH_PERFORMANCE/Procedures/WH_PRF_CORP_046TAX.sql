--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_046TAX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_046TAX" (p_forall_limit in integer,p_success out boolean) as
...
begin
   ...
   merge /*+ parallel (li,16) */ into rtl_location_item li
   using (
   with li_list as
   (select   /*+  full(vat) parallel(vat,16) */
             item_no,
             vat_region_no,
...
--   where     vat.last_updated_date = g_date
   ...
   select /*+ full(vat) parallel(vat,16) full(di) */
          di.sk1_item_no,
          dl.sk1_location_no,
          vat.vat_rate_perc
   from   fnd_item_vat_rate vat,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_229U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_229U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
-- This procedure has been changed to cater for FOODS only
-- Wendy - 14 Feb 2013
--***************************************************************************************************
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_229U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT FIRST SALE DATE PER ITEM FOODS';
...
cursor c_first_sale_date is
   select /*+ PARALLEL(f,6) FULL(f) */
    ...
    where f.sk1_item_no = di.sk1_item_no
      ...
      and f.sk1_item_no not in(select rit.sk1_item_no from rtl_item_trading_new RIT)
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into rtl_item_trading_new values a_tbl_insert(i);
...
begin
    ...
    l_text := 'LOAD OF rtl_item_trading_new FOODS STARTED AT '||
...
-- Is used by FOODs only for the time being
--**************************************************************************************************

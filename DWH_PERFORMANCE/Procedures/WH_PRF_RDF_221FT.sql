--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_221FT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_221FT" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Dec 2010
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_221FT';
...
cursor c_RTL_LOC_ITEM_RDF_DYFCST_L2 is
  ...
  from      RTL_LOC_ITEM_RDF_DYFCST_L2 fcst  --, dim_location b, dim_item c 
  where     fcst.post_date between g_start_date and g_end_date and
            fcst.sales_dly_app_fcst_qty is not null
           -- and       fcst.sk1_location_no = b.sk1_location_no
      --and fcst.sk1_item_no = c.sk1_item_no
      --and b.location_no in (127)
      --and c.item_no in (20007928)
...
/*   l_text := 'sk1_item_no - '||g_rec_out.sk1_item_no;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'sales_dly_app_fcst_qty_flt_av - '||g_rec_out.sales_dly_app_fcst_qty_av ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'sales_dly_app_fcst_qty_flt_av - '||g_rec_out.sales_dly_app_fcst_qty_flt_av;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
*/
   exception
      when others then

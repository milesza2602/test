--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_281U_LOC
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_281U_LOC" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_281U_LOC';
...
--**************************************************************************************************
 ...
 ----- END -- this is for testing purposes
   ...
   EXECUTE immediate('truncate table dwh_performance.MART_FD_ITEM_6WKAVG_LOC');
   l_text := 'truncate table dwh_performance.MART_FD_ITEM_6WKAVG_LOC';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'MART_FD_ITEM_6WKAVG_LOC', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_performance.MART_FD_ITEM_6WKAVG_LOC';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
 ...
 insert /*+ APPEND parallel (aps,2) */ into dwh_performance.MART_FD_ITEM_6WKAVG_LOC
            WITH 
            selcal AS
                          (
            ...
            selmaxp AS
                          (
                            SELECT /*+ parallel(tmpm,2) */
                                        SK1_ITEM_NO,    item_no,    fin_year_no,    fin_week_no,    week_count,    MAX(sk1_prom_no) maxprom, SK1_LOCATION_NO, LOCATION_NO
                            FROM dwh_performance.temp_mart_6wkavg_prom_dates tmpm
                            GROUP BY    SK1_ITEM_NO,  item_no,  fin_year_no,  fin_week_no,  week_count, SK1_LOCATION_NO, LOCATION_NO
                            ORDER BY  SK1_ITEM_NO,   MAX(sk1_prom_no),item_no,  fin_year_no,   fin_week_no,  week_count, SK1_LOCATION_NO, LOCATION_NO
                           )
                           ,
             selzeroes AS
              -- rule 1--
                          (
                                SELECT
                                 SK1_ITEM_NO,    item_no,    COUNT(DISTINCT week_count) diswkcnt,  SK1_LOCATION_NO, LOCATION_NO
                          FROM selmaxp sp
                          WHERE maxprom   IS NOT NULL
                            AND week_count   < 7
                          GROUP BY SK1_ITEM_NO,    item_no,  SK1_LOCATION_NO, LOCATION_NO
              ...
              -- all recs where rule 1 does not apply
                         (
                            SELECT DISTINCT sk1_item_no,    item_no,    fin_year_no,    fin_week_no,    week_count,  SK1_LOCATION_NO, LOCATION_NO
                            FROM selmaxp sp2
                            WHERE maxprom IS NULL
                              AND NOT EXISTS (SELECT sk1_item_no,SK1_LOCATION_NO  FROM selzeroes sz WHERE sp2.sk1_item_no = sz.sk1_item_no
                              AND sp2.sk1_LOCATION_no = sz.sk1_LOCATION_no )
              ...
              selrnk AS
                          ...
                          SELECT srk.sk1_item_no,    srk.item_no,    srk.fin_year_no,    srk.fin_week_no,    srk. week_count , srk.rnk,  srk.SK1_LOCATION_NO, srk.LOCATION_NO
                          FROM
                                (SELECT sk1_item_no,      item_no,      fin_year_no,      fin_week_no,      week_count ,SK1_LOCATION_NO, LOCATION_NO,
                                  dense_rank() over (partition BY sk1_item_no,SK1_LOCATION_NO order by week_count) rnk
                                FROM selext se ) srk
                          WHERE rnk <= 6
              ...
              -- distinct NON-rule 1 items
                            (
                                SELECT sk1_item_no,   SK1_LOCATION_NO, COUNT(*) wkcnt
                            FROM selrnk
      ...
      SELECT SK1_ITEM_NO
                  ...
                  ITEM_NO, SK1_LOCATION_NO, LOCATION_NO
      FROM selzeroes
      UNION ALL
      SELECT    SK1_ITEM_NO
                , g_START_12WK
      ...
      FROM
                (SELECT
                  /*+ parallel(tmp,2) parallel(dns,2) full(dns) parallel(sps,2) full(sps) */
                      ...
                      TMP.ITEM_NO ITEM_NO,
                      si.wkcnt,  TMP.SK1_LOCATION_NO, TMP.LOCATION_NO
                FROM SELitm Si
                JOIN sELrnk Sr
                   ON sr.SK1_ITEM_NO = si.SK1_ITEM_NO AND sr.SK1_LOCATION_NO = si.SK1_LOCATION_NO
                ...
                GROUP BY TMP.SK1_ITEM_NO , TMP.ITEM_NO, si.wkcnt, TMP.SK1_LOCATION_NO, TMP.LOCATION_NO) ;

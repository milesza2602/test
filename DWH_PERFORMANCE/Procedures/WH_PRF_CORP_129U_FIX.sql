--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_129U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_129U_FIX" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_129U_FIX';
...
cursor c_fnd_rtl_loc_item_dy_ff_ord is
   select ord.*,
          di.sk1_item_no,
   ...
   from   dwh_foundation.fnd_rtl_loc_item_dy_ff_ord ord
          ...
          left outer join rtl_loc_item_so_sdn sdn on dl.sk1_location_no        = sdn.sk1_location_no
          ...
          left outer join fnd_rtl_loc_item_dy_ff_st_ord so
                                       on ord.item_no                = so.item_no
                                      and ord.location_no            = so.location_no
                                      and ord.post_date              = so.post_date

          left outer join fnd_loc_item_dy_ff_ord ffd_ord
                                       on di.item_no             = ffd_ord.item_no
                                      and dl.location_no         = ffd_ord.location_no
                                      and ord.post_date          = ffd_ord.post_date
  ...
  --   and ord.location_no = 602
  --   and di.item_no = 8714800025713
   ;


g_rec_in             c_fnd_rtl_loc_item_dy_ff_ord%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_rtl_loc_item_dy_ff_ord%rowtype;
...
--**************************************************************************************************
    open c_fnd_rtl_loc_item_dy_ff_ord;
    fetch c_fnd_rtl_loc_item_dy_ff_ord bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_rtl_loc_item_dy_ff_ord;
--**************************************************************************************************

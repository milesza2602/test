--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_051U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_051U_SS" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
...
--                        use date only.
--
--               nb. FULL OUTER JOIN done between sumed(dwh_PERFORMANCE.RTL_SCH_LOC_EMP_JB_DY_SS)
--                              and RTL_ABSENCE_EMP_WK
--                               is because someone can be anbent for a week 
--                                   but have no schedule
--
--
--  Tables:      Input    - dwh_PERFORMANCE.RTL_SCH_LOC_EMP_JB_DY_SS
--               Output   - DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS  
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_051U_SS';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_SCH_LOC_EMP_JB_WK_SS data EX FOUNDATION';
...
WITH selext1a AS (
        SELECT /*+ full(flr) parallel(flr,6) */
          ...
          FROM dwh_PERFORMANCE.RTL_SCH_LOC_EMP_JB_DY_SS flr
          JOIN dwh_PERFORMANCE.DIM_CALENDAR DC
            ON TRUNC(flr.shift_clock_in) = dc.calendar_date
          JOIN DWH_PERFORMANCE.RTL_EMP_JOB_WK_SS RE
     ...
     selext1b AS (
        SELECT 
            DISTINCT 
               ...
               el.sk1_location_no,
               CASE
                WHEN dli.LEAVE_TYPE_ID  = 1000185 AND DAI.ABSENCE_TYPE_ID = 1 
                THEN ABSENCE_HOURS
                ELSE 0
               END TRAINING_HOURS ,
               employee_rate,
     ...
     selext3 AS (
        SELECT SE1.SK1_JOB_ID ,
                  ...
                  EMPLOYEE_RATE
          FROM SELEXT1 SE1
          JOIN DWH_PERFORMANCE.RTL_EMP_CONSTR_LOC_JOB_WK_SS EC
     ...
     selext4 AS (
        ...
        SELECT
          ...
          FROM selext4 se4,
               DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS rtl
         WHERE se4.sk1_LOCATION_NO = rtl.sk1_LOCATION_NO(+)
           AND se4.sk1_JOB_ID        = rtl.sk1_JOB_ID(+)
           AND se4.sk1_employee_id   = rtl.sk1_employee_id(+)
           AND se4.fin_year_no       = rtl.fin_year_no (+)
           AND se4.fin_week_no       = rtl.fin_week_no (+)
...
begin
        ...
        INTO g_run_seq_no
        FROM dwh_foundation.FND_S4S_SCHLOCEMPJBDY_DLLST_SS
      ...
      g_run_seq_no := 1;
        ...
        BEGIN
           delete from DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS B
           where EXISTS  (select distinct SK1_employee_id,fin_year_no, fin_week_no from DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_DY_SS RTL,
           dim_calendar dc
          ...
          l_text         := 'Deleted from DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
          dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
          
        EXCEPTION
        WHEN no_data_found THEN
                    l_text := 'No deletions done for DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS ';
                    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
        END;
        ...
        BEGIN
          ...
          DELETE
            FROM DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS b
          WHERE EXISTS (
                SELECT DISTINCT SK1_EMPLOYEE_ID, fin_year_no, fin_week_no 
                  FROM dwh_performance.rtl_ABSENCE_EMP_wk a
          ...
          l_text         := 'abscenec data changes for Schedule - Deleted from DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
          dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
          
        EXCEPTION
                WHEN no_data_found THEN
                  l_text := 'No Schedule deletions done for DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS ';
                  dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
        END;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into dwh_performance.RTL_SCH_LOC_EMP_JB_WK_SS  values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
     UPDATE dwh_performance.RTL_SCH_LOC_EMP_JB_WK_SS
        SET  
...
begin
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD OF RTL_SCH_LOC_EMP_JB_WK_SS  EX FOUNDATION STARTED '||
...
--g_date := trunc(sysdate);
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_SCH_LOC_EMP_JB_DY_SS', DEGREE => 8);

        l_text := 'TRUNCATE TABLE DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
EXECUTE IMMEDIATE('TRUNCATE TABLE DWH_PERFORMANCE.RTL_SCH_LOC_EMP_JB_WK_SS');
   l_text := 'Running GATHER_TABLE_STATS ON RTL_SCH_LOC_EMP_JB_WK_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_SCH_LOC_EMP_JB_WK_SS', DEGREE => 8);
--**************************************************************************************************
-- delete process
--**************************************************************************************************

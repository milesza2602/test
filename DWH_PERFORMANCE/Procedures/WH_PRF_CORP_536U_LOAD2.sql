--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_536U_LOAD2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_536U_LOAD2" (
...
as
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_536U_LOAD2';
...
--  commit;
  ...
  from  rtl_loc_item_dy_catalog aa,
        dim_calendar c,
        CAL ,
        w6005682.wrong_rtl_cataloged_items_sk fix
  ...
  where aa.calendar_date    = CAL.CALENDAR_DATE
    ...
    AND AA.SK1_ITEM_NO      = FIX.SK1_ITEM_NO
    AND AA.SK1_LOCATION_NO  = FIX.SK1_LOCATION_NO

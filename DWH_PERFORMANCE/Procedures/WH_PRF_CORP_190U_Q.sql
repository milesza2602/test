--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_190U_Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_190U_Q" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_190U_TEST';
...
g_part_rec_count    integer;
l_cnt_string        varchar2(200);
...
--    local_bulk_update;
    ...
    select /*+parallel(dp,6) */ subobject_name 
      into g_part_name
      from dba_objects dp
     where data_object_id=(select dbms_rowid.rowid_object(rowid) from rtl_loc_item_dy_catalog dp 
    ...
    l_cnt_string := 'select count(*) from rtl_loc_item_dy_catalog partition (' || g_part_name || ')';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_cnt_string);
    
    execute immediate l_cnt_string into g_part_rec_count;
    
    l_cnt_string := 'Records in partition ' || g_part_name || ' = ' || g_part_rec_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_cnt_string);
    
    --dbms_stats.gather_table_stats ('DWH_PERFORMANCE','RTL_LOC_ITEM_DY_CATALOG', g_part_name, granularity => 'PARTITION',degree => 4);
    
--    begin
--      dbms_stats.SET_TABLE_STATS( OWNNAME=>'DWH_PERFORMANCE', 
--                                  tabname=>'RTL_LOC_ITEM_DY_CATALOG',
--                                  PARTNAME=>g_part_name,
--                                  NUMROWS=>g_part_rec_count,
--                                  NUMBLKS=>10147 ,
--                                  AVGRLEN=> 140);
--    end;

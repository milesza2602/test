--------------------------------------------------------
--  DDL for Procedure WH_PRF_MP_023U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MP_023U_FIX" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MP_023U_FIX';
...
--**************************************************************************************************
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      ...
      -- first delete these entries from the rtl table then reload all from staging
      delete /*+ parallel(rtl,4) */
        from rtl_loc_dept_mth_plan_mp rtl
       where fin_year_no = 2017
         and fin_month_no = 3;
         --and sk1_department_no = 204
         --and sk1_plan_type_no = 56;
      ...
      SELECT /*+ full (fnd) parallel (fnd,2) */
        ...
        from  dwh_foundation.fnd_rtl_loc_dept_mth_plan_mp fnd,
              ...
              dim_department dept,
              dim_plan_type pln
        where fnd.fin_year_no = 2017
          and fnd.fin_month_no = 3
          --and fnd.plan_type_no = 56
          --and fnd.department_no = 105

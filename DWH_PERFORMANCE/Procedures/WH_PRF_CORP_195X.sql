--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_195X
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_195X" (p_forall_limit in integer,p_success out boolean) as
...
--  14 Mar 2018 - Add Customer Availibility Measure and Num Facing (Chg12260 - A Joshua)
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_195X';
...
cursor C_RTL_LOC_ITEM_DY_CATALOG is
   select /*+ full (li) parallel (li,6) */
          ...
          nvl(li.num_facings,0) num_facings -- Chg4256
   ...
   where  lid.last_updated_date          = g_date and
          ...
          lid.sk1_item_no                = li.sk1_item_no and
          lid.sk1_location_no            = li.sk1_location_no ;
--          lid.sk1_item_no                = 262758 and
--          lid.sk1_location_no            = 676 and
--          lid.calendar_date              = '22 mar 18';
...
begin
   ...
   g_rec_out.sk1_location_no                 := g_rec_in.sk1_location_no;
          ...
          g_rec_out.fd_num_cust_avail_adj     := 0; --new 23/3/18
          g_rec_out.fd_num_cust_catlg_adj     := 0; --new 23/3/18
          g_rec_out.fd_cust_avail             := 0; --new 23/3/18
...
---- Start Chg4256
--   if g_rec_out.fd_num_cust_catlg_adj > 0 then -- chg_12260
--      if g_rec_in.num_facings > 0 then
--         if (g_rec_in.boh_adj_qty + abs(g_waste_qty)) > g_rec_in.num_facings then 
--            g_rec_out.fd_num_cust_avail_adj := 1;
--         else
----            dbms_output.put_line('BOH '||g_rec_in.boh_adj_qty);
----            dbms_output.put_line('Waste '||g_waste_qty);
----            dbms_output.put_line('BOH '||g_rec_in.num_facings);
--            g_rec_out.fd_num_cust_avail_adj := (g_rec_in.boh_adj_qty + abs(g_waste_qty)) / g_rec_in.num_facings;
----            dbms_output.put_line('Answer '||g_rec_out.fd_num_cust_avail_adj);
--         end if;
--      elsif g_rec_in.num_shelf_life_days >= 3 then
--         if (g_rec_in.boh_adj_qty + abs(g_waste_qty)) < 3 then 
--            g_rec_out.fd_num_cust_avail_adj := 0;
--         else
--            g_rec_out.fd_num_cust_avail_adj := 1;
--         end if;
--      else
--         g_rec_out.fd_num_cust_avail_adj := g_rec_out.fd_num_avail_days_adj;
--      end if;
--   else
--      g_rec_out.fd_num_cust_avail_adj := 0;
--dbms_output.put_line('Enter fd_num_cust_avail_adj '||g_rec_out.fd_num_cust_avail_adj);  
--dbms_output.put_line('Enter fd_num_catlg_days_adj '||g_rec_out.fd_num_catlg_days_adj);   
--dbms_output.put_line('Enter loc_type '||g_rec_in.loc_type);  

--if g_rec_in.loc_type = 'S' and g_rec_in.active_store_ind = 1 then       
  -- Start Chg4256          
   if g_rec_out.fd_num_catlg_days_adj > 0 then
      if g_rec_in.num_facings > 0 then
         if g_rec_in.boh_adj_qty > g_rec_in.num_facings then
            g_rec_out.fd_num_cust_avail_adj := 1;
         else
            g_rec_out.fd_num_cust_avail_adj := g_rec_in.boh_adj_qty / g_rec_in.num_facings;
         end if;
      elsif g_rec_in.num_shelf_life_days >= 3 then
         if g_rec_in.boh_adj_qty < 3 then
            g_rec_out.fd_num_cust_avail_adj := 0;
         else
            g_rec_out.fd_num_cust_avail_adj := 1;
         end if;
...
-- End Chg4256
--dbms_output.put_line('Exit fd_num_cust_avail_adj '||g_rec_out.fd_num_cust_avail_adj);  
--dbms_output.put_line('Exit fd_num_catlg_days_adj '||g_rec_out.fd_num_catlg_days_adj); 
   exception
      when others then
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set /*   fd_num_avail_days               = a_tbl_update(i).fd_num_avail_days,
              ...
              fd_num_cust_avail_adj           = a_tbl_update(i).fd_num_cust_avail_adj,  --chg4256
--              num_facings                     = A_TBL_UPDATE(I).num_facings,
              fd_cust_avail                   = a_tbl_update(i).fd_cust_avail,          --Chg12260
              fd_num_cust_catlg_adj           = a_tbl_update(i).fd_num_cust_catlg_adj   --Chg12260

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_003U_WLTST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_003U_WLTST" (p_forall_limit in integer,p_success out boolean
--,P_end_date in date
)
as
  --**************************************************************************************************
--  TEST ROLLING -ISSUE - JULY 2017 - 2WKROLL
-- back of dim_calendar = dwh_performance.dm_calendar_bck11jul17
-- alter table dwh_performance.DIM_CALENDAR_WLTST rename to DM_CALENDAR_WLTST

-- create table dwh_performance.dm_calendar_bck11jul17
-- as select * from dwh_performance.dim_calendar

-- select  CALENDAR_DATE,	FIN_YEAR_NO,	FIN_MONTH_NO,	FIN_WEEK_NO,	FIN_DAY_NO,	ROLL_2WK_IND
--  from dwh_performance.dim_calendar
--  --dwh_performance.DM_CALENDAR_WLTST 
--  where roll_2wk_ind = 1
--  order by CALENDAR_DATE
 
--  update dim_calendar a
--  set a.roll_2wk_ind = (select b.roll_2wk_ind from dwh_performance.DM_CALENDAR_WLTST  b where a.calendar_date = b.calendar_date)
  --**************************************************************************************************
--  Date:        march 2009
--  Author:      Wendy Lyttle
--  Purpose:     Create a DIM_CALENDAR_WLTST table loaded with
...
--                          total of 30 months
--  Tables:      Input  - DIM_CALENDAR_WLTST, dim_control
--               Output - DIM_CALENDAR_WLTST
--  Packages:    dwh_constants, dwh_log, dwh_valid
--
--  Maintenance:
--  12 MAY 2009 - Defect 1541 - Add New Fields to Cater for Calendar Roll Time
--                               Periods to DIM_CALENDAR_WLTST
--  26 May 2009 - defect 1657 - Change Cube_A FDP from 5 weeks to 6 weeks
--
--  04 July 2009 - defect 1981 - DIM_CALENDAR_WLTST to have 3wks-rolling instead
--                               of 2wks-rolling
--  10 July 2009 - defect 2031 - DIM_CALENDAR_WLTST roll_2wk_ind should not be
...
--  13 june 2011 - defect 4354 - FIN YR-END 2011 - changes to generation of DIM_CALENDAR_WLTST
--
--  Naming conventions
--  g_  -  Global variable
...
--  l_module_name sys_dwh_errlog.log_procedure_name%type     := 'WH_PRF_DM_CALENDAR_WLTST_LOAD';
  l_module_name sys_dwh_errlog.log_procedure_name%type     := 'WH_PRF_CORP_003U_WLTST';
  l_name sys_dwh_log.log_name%type                         := dwh_constants.vc_log_name_rtl_md;
  l_system_name sys_dwh_log.log_system_name%type           := dwh_constants.vc_log_system_name_rtl_fnd;
  l_script_name sys_dwh_log.log_script_name%type           := dwh_constants.vc_log_script_rtl_fnd_md;
  l_procedure_name sys_dwh_log.log_procedure_name%type     := l_module_name;
  l_text sys_dwh_log.log_text%type ;
  l_description sys_dwh_log_summary.log_description%type   := 'LOAD OF DM_CALENDAR_WLTST';
...
begin
      ...
      l_text := 'LOAD OF DM_CALENDAR_WLTST ex DM_CALENDAR_WLTST STARTED AT '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--g_date := '15 aug 2011';
  ...
  --**************************************************************************************************
  -- Clear DM_CALENDAR_WLTST 2-WK-ROLL-IND, 6-WK-ROLL-IND, 9-WK-ROLL-IND, 13-WK-ROLL-IND
  --**************************************************************************************************
       update dwh_performance.DM_CALENDAR_WLTST
      ...
      commit;
      l_text           := '** all rolling indicators updated to null on DM_CALENDAR_WLTST';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  --**************************************************************************************************
      execute immediate('update dwh_performance.DM_CALENDAR_WLTST dc
                            set roll_6wk_ind = 0,
                                roll_9wk_ind = 0,
                                roll_13wk_ind = 0
      ...
      update dwh_performance.DM_CALENDAR_WLTST dc
            set dc.roll_2wk_ind = 0,
                 dc.roll_6wk_ind = 0,
                 dc.roll_9wk_ind = 0,
                 dc.roll_13wk_ind = 0
      where dc.calendar_date in (select dct.ly_calendar_date
                                 from dwh_performance.DM_CALENDAR_WLTST dct
...
--                                                      from dwh_performance.DM_CALENDAR_WLTST dct2
...
--                                                    ' );
 ...
 --
   --  --   g_recs_inserted :=0;
    ...
    --   from dwh_performance.DM_CALENDAR_WLTST
 ...
 --
  ...
  --**************************************************************************************************
      ...
      -- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
      -- check values on DM_CALENDAR_WLTST - written to log
      -- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
      --
      -- ROLL_2WK_IND
      --
...
--      OR g_cnt > 13 then
  ...
  --**************************************************************************************************
      ...
      SELECT COUNT(*) INTO g_recs_updated FROM dwh_performance.DM_CALENDAR_WLTST

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_280Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_280Q" (p_forall_limit IN INTEGER, p_success OUT BOOLEAN)
AS
  --**************************************************************************************************
...
--  --  Date:        October 2013
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_CORP_280Q';
...
--
 ...
 ----- END -- this is for testing purposes
  ...
  --**************************************************************************************************
     EXECUTE immediate('truncate table W6005682.TEMP_MART_6WKAVG_PROM_DATESQ');
     l_text := 'truncate table W6005682.TEMP_MART_6WKAVG_PROM_DATESQ';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

 --    DBMS_STATS.gather_table_stats ('W6005682',
 --                                  'TEMP_MART_6WKAVG_PROM_DATESQ', DEGREE => 8);
 --   commit;
 --   l_text := 'GATHER STATS on W6005682.TEMP_MART_6WKAVG_PROM_DATESQ';
 --   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    BEGIN
        ...
        into g_seq
        from dba_indexes
        where OWNER = 'W6005682'
        AND index_name = 'I10_Q_TMP_MRT_6WKAVG_PRM_DTS';
              execute immediate('DROP index W6005682.I10_Q_TMP_MRT_6WKAVG_PRM_DTS');
    exception
    ...
    BEGIN
        ...
        from dba_indexes
        where OWNER = 'W6005682'
        AND index_name = 'I20_Q_TMP_MRT_6WKAVG_PRM_DTS';
              execute immediate('DROP index W6005682.I20_Q_TMP_MRT_6WKAVG_PRM_DTS');
    exception
         when no_data_found then
          commit;
    END;
        ...
        from dba_indexes
        where OWNER = 'W6005682'
        AND index_name = 'I30_Q_TMP_MRT_6WKAVG_PRM_DTS';
              execute immediate('DROP index W6005682.I30_Q_TMP_MRT_6WKAVG_PRM_DTS');
    exception
         when no_data_found then
          commit;
    END;
   ...
   --**************************************************************************************************
  -- Do insert into W6005682.TEMP_MART_6WKAVG_PROM_DATESQ
  --**************************************************************************************************
    G_RECS := 0;

      INSERT /*+ parallel(aps,4) */ INTO W6005682.TEMP_MART_6WKAVG_PROM_DATESQ aps
                ...
                selitm as
                           (SELECT /*+  PARALLEL(FIU,4) full(FIU) */ distinct sk1_item_no, di.item_no
                ...
                selcat as (
                          ...
                          ,
                selprm as (
                select   /*+ FULL(DPIA)  paraLlel(dpia,4) parallel(fpl,4) parallel(dp,4) full(dp) full(fpl) */
...
--10--------------------------------------
     Execute Immediate('CREATE INDEX W6005682.I10_Q_TMP_MRT_6WKAVG_PRM_DTS ON W6005682.TEMP_MART_6WKAVG_PROM_DATESQ
                            (WEEK_COUNT, ITEM_NO) TABLESPACE DWH_TS_WORKAREA2  NOLOGGING  PARALLEL') ;
 
      Execute Immediate('ALTER INDEX W6005682.I10_Q_TMP_MRT_6WKAVG_PRM_DTS LOGGING NOPARALLEL') ;
   
    l_text := 'create index  W6005682.i10_Q_tmp_mrt_6wkavg_PRM_DTS ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--20----------------------------
     execute immediate('CREATE INDEX  W6005682.I20_Q_TMP_MRT_6WKAVG_PRM_DTS ON W6005682.TEMP_MART_6WKAVG_PROM_DATESQ
                            (WEEK_COUNT, SK1_PROM_NO) TABLESPACE DWH_TS_WORKAREA2 NOLOGGING PARALLEL') ;
                            
     execute immediate('ALTER INDEX W6005682.I20_Q_TMP_MRT_6WKAVG_PRM_DTS LOGGING NOPARALLEL') ;

  
    l_text := 'create index  W6005682.i20_Q_tmp_mrt_6wkavg_PRM_DTS ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);


--30-----------------------------------
     execute immediate('CREATE INDEX  W6005682.I30_Q_TMP_MRT_6WKAVG_PRM_DTS ON W6005682.TEMP_MART_6WKAVG_PROM_DATESQ
                            (ITEM_NO) TABLESPACE DWH_TS_WORKAREA2  NOLOGGING PARALLEL') ;
                            
      execute immediate('ALTER INDEX W6005682.I30_Q_TMP_MRT_6WKAVG_PRM_DTS LOGGING NOPARALLEL') ;
      
      l_text := 'create index W6005682.i30_q_tmp_mrt_6wkavg_PRM_DTS ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

 ------------------------------------- 
    DBMS_STATS.gather_table_stats ('W6005682', 'TEMP_MART_6WKAVG_PROM_DATESQ', DEGREE => 32, cascade=>true);
    commit;
    l_text := 'GATHER STATS on W6005682.TEMP_MART_6WKAVG_PROM_DATESQ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

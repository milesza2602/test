--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_762U_WL2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_762U_WL2" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        May 2009
--  Author:      M Munnik
...
--  Maintenance:
--
--  August 2016 - Rewritten as bulk merge in order to improve performance.
--                Q.Smit
--
--  Naming conventions
--  g_  -  Global variable
...
--**************************************************************************************************
  ...
  MERGE /*+ full(rtl) parallel(rtl,5) */ into rtl_loc_item_dy_wh_plan rtl using
  (
   with selcal as(select * from dim_calendar where calendar_date between g_start_date and g_end_date),
        selitem as(select /*+ full(x) */ item_no, sk1_item_no from dim_item x where group_no in(1,2)),
        selfnd as (select  /*+ materialize full(jdaff) parallel(jdaff,4)*/ a*
                  a.LOCATION_NO
, a.ITEM_NO
, sk1_item_no
, dc.calendar_date calendar_date
, a.DC_LOC_NO
...
, a.LAST_UPDATED_DATE
                  from dwh_foundation.fnd_loc_item_jdaff_wh_plan jdaff a, selitem di, selcal dc
   ...
   unpivot  include nulls (sysdata for syscol in (week_1_day_1_cases,
                                                  ...
                                                  week_3_day_6_cases,
                                                  week_3_day_7_cases))) 
   SELECT /*+ full(owp) parallel(owp,5) full(di) full(dl) full(dlh) full(dih) full(dc) */
  ...
  commit;
      ...
      l_text := 'group in(1,2) recs='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  (
   ...
   (select  /*+ full(jdaff) parallel(jdaff,4)*/
            ...
            syscol,
            sysdata
   from     dwh_foundation.fnd_loc_item_jdaff_wh_plan jdaff (select /*+ full(x) */ item_no from dim_item x where group_no in (3)) b
  ...
  commit;
        l_text := 'group in(3) recs='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  (
   ...
   (select  /*+ full(jdaff) parallel(jdaff,4)*/
            ...
            syscol,
            sysdata
   from     dwh_foundation.fnd_loc_item_jdaff_wh_plan jdaff (select /*+ full(x) */ item_no from dim_item x where group_no in (4,5)) b
   where a.item_no = b.item_no
   unpivot  include nulls (sysdata for syscol in (week_1_day_1_cases,
                                                  week_1_day_2_cases,
                                                  week_1_day_3_cases,
  ...
  commit;
      l_text := 'group in(4,5) recs='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
  ...
  (
   ...
   (select  /*+ full(jdaff) parallel(jdaff,4)*/
            ...
            syscol,
            sysdata
   from     dwh_foundation.fnd_loc_item_jdaff_wh_plan jdaff (select /*+ full(x) */ item_no from dim_item x where group_no in (6,8,10)) b
   where a.item_no = b.item_no
   unpivot  include nulls (sysdata for syscol in (week_1_day_1_cases,
                                                  week_1_day_2_cases,
                                                  week_1_day_3_cases,
  ...
  commit;
        ...
        l_text := 'group in(6,8,10) recs='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  (
   ...
   (select  /*+ full(jdaff) parallel(jdaff,4)*/
            ...
            syscol,
            sysdata
   from     dwh_foundation.fnd_loc_item_jdaff_wh_plan jdaff (select /*+ full(x) */ item_no from dim_item x where group_no in (7,9)) b
   where a.item_no = b.item_no
   unpivot  include nulls (sysdata for syscol in (week_1_day_1_cases,
                                                  week_1_day_2_cases,
                                                  week_1_day_3_cases,
  ...
  commit;
        ...
        l_text := 'group in(7,9) recs='||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

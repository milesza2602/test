--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_223U_201552
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_223U_201552" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_223U_201552';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'Create Mart table for Foods weekly snapshot for FY 2016 WK 5';
...
l_last_week_no        := 5;
l_fin_week_no         := 6;
l_next_wk_fin_week_no := 7;
l_next_wk_fin_year_no := 2016;
l_last_wk_fin_year_no := 2016;
l_day_no              := 2;
l_this_wk_start_date  := '03/AUG15';
l_end_6wks            := 5;
...
l_53_ly_day1_date := '28/JUL/2014';
l_53_ly_day7_date := '03/AUG/2014';
...
l_text := 'year and week being processed = '|| l_last_wk_fin_year_no ||' '||l_last_week_no;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table W6005682.TMP_MART_FOODS_WKLY_SNPSHT_52');
    l_text := 'Truncate Mart table completed '||to_char(sysdate,('dd mon yyyy hh24:mi:ss')) ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    l_text := 'DISABLING CONSTRAINT STARTED AT - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'alter table W6005682.TMP_MART_FOODS_WKLY_SNPSHT_52 disable constraint PK_T_MRT_FDS_WK_SNPSHT_52';
    l_text := 'DISABLING CONSTRAINT ENDED AT - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
INSERT /*+ APPEND */ INTO W6005682.TMP_MART_FOODS_WKLY_SNPSHT_52 mart
with item_list as (
...
from rtl_loc_item_wk_rdf_fcst f, item_list il, store_list sl    --RDF L1/L2 remapping change
...
from rtl_loc_item_dy_rdf_fcst f, item_list il, store_list_area sl   --RDF L1/L2 remapping change
...
where atg.sk1_item_no     = itm.sk1_item_no
  ...
  g_recs_inserted := g_recs_inserted + SQL%ROWCOUNT;
    ...
    l_text := 'ENABLING CONSTRAINT STARTED AT - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'alter table W6005682.TMP_MART_FOODS_WKLY_SNPSHT_52 enable constraint PK_T_MRT_FDS_WK_SNPSHT_52';
    l_text := 'ENABLING CONSTRAINT ENDED AT - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

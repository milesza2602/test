--------------------------------------------------------
--  DDL for Procedure WH_PRF_MP_028U_RELOD2016
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MP_028U_RELOD2016" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MP_028U_RELOD2016';
...
l_fin_year_loop      integer       :=  0;
l_fin_week_loop      integer       :=  0;
...
begin
  ...
  L_TEXT := 'Updating RMS Stock data ....';
   ...
   merge /*+ parallel(mart,4) */   into  dwh_performance.MART_TMP_LOC_SUBC_PLN_WK_RMS mart
    
   using (  select /*+ parallel(stck,4) full lev1, full loc */
...
begin
 ...
 merge /*+ parallel(mart,4) */   into  dwh_performance.MART_TMP_LOC_SUBC_PLN_WK_RMS mart
   using
   (select /*+ parallel (sprs,4) full lev1, full loc */
...
begin
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   l_text := 'LOAD OF MART_TMP_LOC_SUBC_PLN_WK_RMS MERGE!!!  STARTED AT '||
...
/*    L_TEXT := '-------------';
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    L_TEXT := '-------------';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);   
  /*  L_TEXT := 'stats started';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
    begin
      DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','MART_TMP_LOC_SUBC_PLN_WK_RMS',estimate_percent=>1, DEGREE => 32);
      commit;
    end;
    L_TEXT := 'stats completed';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);   

   
   for l_fin_year_loop in reverse 2016..2016 loop
     g_fin_year_no := l_fin_year_loop;

     for l_fin_week_loop in reverse 1..52 loop
       g_fin_week_no := l_fin_week_loop;
       
       g_recs_read     := 0;
       g_recs_inserted := 0;
     
       select distinct fin_week_no, fin_year_no, this_week_start_date, this_week_end_date
         into g_start_fin_week_no, g_start_fin_year_no, g_this_week_start_date, g_this_week_end_date
         from dim_calendar
        where fin_year_no = g_fin_year_no
          and fin_week_no = g_fin_week_no;
     
       l_text := 'Data extract '|| g_start_fin_week_no || ' Week  '|| g_start_fin_year_no;
       dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
*/
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
    ...
    L_TEXT := '-------------';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);    
    L_TEXT := '2015 started - CONTINUE';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
    L_TEXT := '-------------';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
   
   for l_fin_year_loop in reverse 2015..2015 loop
     g_fin_year_no := l_fin_year_loop;

     for l_fin_week_loop in reverse 1..17 loop
       g_fin_week_no := l_fin_week_loop;

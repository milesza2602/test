--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_125U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_125U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_125U';
...
type tbl_array_m is table of rtl_mc_loc_item_dy_om_ord%rowtype index by binary_integer;
type tbl_array_u is table of rtl_mc_loc_item_dy_om_ord%rowtype index by binary_integer;
...
cursor c_fnd_loc_item_dy_ff_ord is
   select /*+ materialize cardinality (ord,300000) */  ord.*,
          di.sk1_item_no,
   ...
   from   fnd_loc_item_dy_ff_ord ord
          ...
          left outer join rtl_location_item prc
                                      on di.sk1_item_no             = prc.sk1_item_no  and
                                         dl.sk1_location_no         = prc.sk1_location_no 
                                         
          left outer join FND_MC_LOC_ITEM_DY_RMS_WAC wac   --
                                      on ord.item_no                = wac.item_no  and
                                         ord.location_no            = wac.location_no and
                                         ord.post_date              = wac.tran_date                               

   where  ord.last_updated_date      = g_date
   AND    ORD.POST_DATE              > G_DATE - 8
...
begin
   ...
   -- OTHER MEASURES ARE DERIVED IN WH_PRF_MC_129U
...
-- Foods Renewal Fields
   ...
   g_rec_out.num_units_per_tray2             := g_rec_in.num_units_per_tray2;
      ...
      if g_rec_in.standard_uom_code = 'EA' and g_rec_in.random_mass_ind = 1 then
         ...
         g_rec_out.sod_boh_selling_local  := g_rec_out.sod_boh_qty * g_rec_in.reg_rsp_excl_vat_local * g_rec_in.static_mass;
      else
         g_rec_out.sod_boh_selling        := g_rec_out.sod_boh_qty * g_rec_in.reg_rsp_excl_vat;
         g_rec_out.sod_boh_selling_local  := g_rec_out.sod_boh_qty * g_rec_in.reg_rsp_excl_vat_local;
      end if;
      g_rec_out.sod_boh_cost       := g_rec_out.sod_boh_qty * g_rec_in.wac;
      g_rec_out.sod_boh_cost_local := g_rec_out.sod_boh_qty * g_rec_in.wac_local;
...
merge into rtl_mc_loc_item_dy_om_ord rtl_lidoo USING
(select a_tbl_merge(i).sk1_location_no           as	sk1_location_no,
        ...
        a_tbl_merge(i).sod_boh_selling_local     as	sod_boh_selling_local,
        a_tbl_merge(i).sod_boh_cost_local        as	sod_boh_cost_local,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_613U_1803
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_613U_1803" (p_forall_limit in integer,p_success out boolean) as
...
rdf_fcst as (
 select /*+ full(f) PARALLEL(f,4) */
        ...
        sum(f.DY_01_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day1_estimate,
        sum(f.DY_02_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day2_estimate,
        sum(f.DY_03_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day3_estimate,
        sum(f.DY_01_APP_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as weekly_estimate1, -- to DAY7
        sum(f.DY_08_APP_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as weekly_estimate2, -- to DAY 14
        sum(f.DY_04_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day4_estimate,
        sum(f.DY_05_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day5_estimate,
        sum(f.DY_06_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day6_estimate,
        sum(f.DY_07_SYS_FCST_QTY * ((fli.reg_rsp * 100 / (100 + di.vat_rate_perc)))) as day7_estimate,
   ...
   from fnd_loc_item_rdf_dyfcst_l2 f, dim_item di, dim_location dl, loc_item fli
--   , fnd_jdaff_dept_rollout dptr --must still add live dept check !! -- Rollout change
   --from fnd_rtl_loc_item_wk_rdf_dyfcst f, dim_item di, dim_location dl, loc_item fli, fnd_jdaff_dept_rollout dptr --must still add live dept check !!
  where f.item_no     = di.item_no
    and f.location_no = dl.location_no
--    and f.last_updated_date = g_date
    and f.post_date between l_min_date and l_max_date   --QS
    and f.item_no     = fli.item_no
    and f.location_no = fli.location_no
--    and di.department_no = dptr.department_no
...
--**************************************************************************************************
    ...
    l_text := 'DATE BEING PROCESSED IS:- '||l_min_date ||' ' || l_max_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

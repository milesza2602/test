--------------------------------------------------------
--  DDL for Procedure WH_PRF_BRTH_002U_FOOD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_BRTH_002U_FOOD" 
...
--                        dim_item
--                        dim_location
--               Output - T14_rtl_area_item_wk_dense
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  20 oct 2010 - added gather stats
...
--g_rec_out            T14_rtl_area_item_wk_dense%rowtype;
g_found              boolean;
g_date               date;
g_start_date         date;
g_end_date         date;
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_BRTH_002U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'BRIDGETHORN ROLLUP TO T14_DENSE';
...
begin
    ...
    l_text := 'ROLLUP OF T14_rtl_loc_item_wk_dense EX WEEK LEVEL STARTED '||
...
--**************************************************************************************************
    ...
    execute immediate ('truncate table dwh_DATAFIX.T14_rtl_area_item_wk_dense');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.T14_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
                                   'T14_rtl_area_item_wk_dense', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_DATAFIX.T14_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

     execute immediate 'alter session enable parallel dml';

    INSERT /*+ APPEND */ INTO dwh_DATAFIX.T14_rtl_area_item_wk_dense
       ...
       dl.area_no
                ...
                ,sum(nvl(rai.sales_qty,0)) sales_qty
                ,sum(nvl(rai.sales,0)) sales
                ,sum(nvl(rai.sales_cost,0)) sales_cost
                ,sum(case when nvl(rai.sales_qty,0) > 0 then 1 when nvl(rai.sales,0) > 0 then 1 else 0 end) stores_with_sales
     from     dwh_performance.rtl_loc_item_wk_rms_dense rai,
...
commit;
 ...
 SELECT MIN(THIS_WEEK_START_DATE), MAX(THIS_WEEK_END_DATE) 
    ...
    execute immediate ('truncate table dwh_DATAFIX.T13_rtl_area_item_wk_dense');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.T13_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
                                   'T13_rtl_area_item_wk_dense', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_DATAFIX.T13_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

     execute immediate 'alter session enable parallel dml';

    INSERT /*+ APPEND */ INTO dwh_DATAFIX.T13_rtl_area_item_wk_dense
       select  /*+ full(rai) parallel (rai	4) */
       dl.area_no
                ,di.item_no
                ,rai.fin_year_no
...
commit;
    ...
    execute immediate ('truncate table dwh_DATAFIX.t18_rtl_area_item_wk_dense');
    commit;
    l_text := 'TRUNCATED table dwh_DATAFIX.t18_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--
--    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
--                                   'T14_rtl_area_item_wk_dense', DEGREE => 8);
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    INSERT /*+ APPEND */ INTO dwh_DATAFIX.t18_rtl_area_item_wk_dense
       select  /*+ full(rai) parallel (rai	4) */
       dl.area_no
                ,di.item_no
                ,rai.fin_year_no
...
commit;
    DBMS_STATS.gather_table_stats ('DWH_DATAFIX',
                                   't18_rtl_area_item_wk_dense', DEGREE => 8);
    commit;
    l_text := 'GATHER STATS on dwh_DATAFIX.t18_rtl_area_item_wk_dense';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

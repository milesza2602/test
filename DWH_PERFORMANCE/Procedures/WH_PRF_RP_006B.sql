--------------------------------------------------------
--  DDL for Procedure WH_PRF_RP_006B
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RP_006B" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - fnd_rtl_shipment, fnd_uda_value, fnd_item_uda
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RP_006B';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RPL FAST AVAIL CATALOG EX FOUNDATION';
...
with
 ship_validation as
 (select tship.actl_rcpt_date,
         di.sk1_item_no,
         dl.sk1_location_no,
         fndv.uda_value_no
  from   temp_shipment_ch_avail tship,
         dim_item di,
         dim_location dl,
         dim_country dc,
         fnd_uda_value fndv,
         fnd_item_uda fndi
  where  tship.item_no                     = di.item_no and
         di.business_unit_no        NOT IN (50,55) and
         di.rpl_ind                        = 0 and
         tship.to_loc_no                   = dl.location_no and
         dl.loc_type                       = 'S' and
         dl.sk1_country_code               = dc.sk1_country_code and
         dc.country_code                   = 'ZA' and
         tship.item_no                     = fndi.item_no and
         fndi.uda_no                       = fndv.uda_no and
         fndi.uda_value_no_or_text_or_date = fndv.uda_value_no and
         fndv.uda_no                       = 104 and
         fndv.uda_value_no               in (4,5,8)) ,

  ship_valid as (
  select shipe.sk1_item_no,
         shipe.sk1_location_no,
         shipe.actl_rcpt_date,
         shipe.uda_value_no
  from   ship_validation  shipe
  where not exists (select 1
                    from  rtl_loc_item_dy_ch_avail avl
                    where shipe.sk1_item_no      = avl.sk1_item_no and
                          shipe.sk1_location_no  = avl.sk1_location_no)),
--                          shipe.actl_rcpt_date   = avl.active_from_date)),

  ship_max as
 (select avail.sk1_item_no,
         avail.sk1_location_no,
         max(avail.active_to_date) active_to_date
  from   rtl_loc_item_dy_ch_avail avail,
         ship_validation shipv
  where  avail.sk1_item_no     = shipv.sk1_item_no and
         avail.sk1_location_no = shipv.sk1_location_no
  group by avail.sk1_item_no,
           avail.sk1_location_no)

  select a.actl_rcpt_date,
         a.sk1_item_no,
         a.sk1_location_no,
         a.uda_value_no,
         dcal.fin_day_no,
  ...
  from   ship_valid a
  join   dim_calendar dcal on
         a.actl_rcpt_date                  = dcal.calendar_date
  left outer join   ship_max b on
         a.sk1_item_no                     = b.sk1_item_no and
         a.sk1_location_no                 = b.sk1_location_no and
         a.actl_rcpt_date                  > b.active_to_date;
...
/*
   begin
      select fin_day_no, this_week_end_date + 1
      into   g_fin_day_no, g_next_wk_start_date
      from   dim_calendar
      where  calendar_date = g_rec_in.actl_rcpt_date;
   end;
*/
-- Let start date commence the following week should first receipt of stock into store be post Thursday
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    sk1_avail_uda_value_no   = a_tbl_update(i).sk1_avail_uda_value_no,

--------------------------------------------------------
--  DDL for Procedure ADW_MC_HIST_ZIP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."ADW_MC_HIST_ZIP" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'ADW_MC_HIST_ZIP';
...
cursor   c_prf_zones is
select   sk1_zone_group_zone_no  
from     dim_zone
--where    sk1_zone_group_zone_no < 257    --restart point--
order by sk1_zone_group_zone_no;
...
--**************************************************************************************************
  ...
  execute immediate 'alter session enable parallel dml';
   ...
   for zone_record in c_prf_zones
   loop

   
UPDATE  /*+ parallel(xyz,12) */
        RTL_ZONE_ITEM_OM xyz
SET     
--        CASE_COST_PRICE_LOCAL       = CASE_COST_PRICE,
...
--        MULTI_UNIT_RSP_OPR          = MULTI_UNIT_RSP         
        where SK1_ZONE_GROUP_ZONE_NO   = ZONE_RECORD.SK1_ZONE_GROUP_ZONE_NO
...
--        and   NVL(REG_RSP_OPR,0) <>  NVL(REG_RSP,0)
      ...
      l_text := 'UPDATED ZONE_ITEM_OM ' ||ZONE_RECORD.SK1_ZONE_GROUP_ZONE_NO||'  '||

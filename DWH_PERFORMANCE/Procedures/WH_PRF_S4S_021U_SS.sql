--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_021U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_021U_SS" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
--               Output   - DWH_PERFORMANCE.RTL_LOC_BU_WK_SS
--  Packages:    dwh_constants, dwh_log, dwh_valid
--
--  Maintenance:
--
-- S4S New way forward SS 30112018
--  Naming conventions
--  g_  -  Global variable
...
g_count                number        :=  0;
g_rec_out              dwh_performance.RTL_LOC_BU_WK_SS%rowtype;
g_found                boolean;
g_date                 date;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_021U_SS';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RTL_LOC_BU_WK_SS data  EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dwh_performance.RTL_LOC_BU_WK_SS%rowtype index by binary_integer;
type tbl_array_u is table of dwh_performance.RTL_LOC_BU_WK_SS%rowtype index by binary_integer;
...
WITH SELEXT AS (SELECT  
               ...
               FROM DWH_FOUNDATION.FND_S4S_HYP_LOC_BU_WK FLR,
                    ...
                    DWH_PERFORMANCE.RTL_S4S_BUSINESS_UNIT_SS DE
            ...
            SELECT   
               /*+ full(rtl) parallel(rtl,6) parallel(se,6) */  
                DISTINCT 
                   SE.*
                  , RTL.SK1_LOCATION_NO RTL_EXISTS
            FROM SELEXT SE,
                 DWH_PERFORMANCE.RTL_LOC_BU_WK_SS RTL
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
        insert into dwh_performance.RTL_LOC_BU_WK_SS  values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions             
               UPDATE dwh_performance.RTL_LOC_BU_WK_SS
                    SET
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_BU_WK_SS  EX FOUNDATION STARTED '||
...
--**************************************************************************************************
   ...
   local_bulk_insert;
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   local_bulk_update;
    l_text := 'Running GATHER_TABLE_STATS ON RTL_LOC_BU_WK_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE',
                                   'RTL_LOC_BU_WK_SS', DEGREE => 8);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_016U_WLFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_016U_WLFIX" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_016U_WLFIX';
...
FOR g_sub IN 0..10
  LOOP
   ...
   from dim_calendar
   where this_week_start_date = g_this_week_start_date
   ...
   from dim_calendar
   where this_week_end_date = g_this_week_end_date
   group by ly_fin_year_no, ly_fin_week_no, fin_week_no, fin_year_no;

   l_text := 'Current Data extract from '||g_this_week_start_date|| ' to '||g_this_week_end_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--RTL_LGGSWAA_M20173_13
          g_partition_name :=   'RTL_LGGSWAA_M'||g_START_YEAR||g_START_month||'_'||g_START_week;    
          g_sql_trunc_partition := 'alter table dwh_performance.RTL_LOC_GEO_GRD_SC_WK_AST_ACT truncate SUBPARTITION '||g_partition_name;

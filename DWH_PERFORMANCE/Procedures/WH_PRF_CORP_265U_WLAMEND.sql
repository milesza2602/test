--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_265U_WLAMEND
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_265U_WLAMEND" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_265U_WLAMEND';
...
execute immediate 'alter session enable parallel dml';
    ...
    l_text := 'TRUNCATE TABLE MART_FOODS_SUPPLIER_WLAMEND' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

EXECUTE IMMEDIATE('TRUNCATE TABLE DWH_DATAFIX.MART_FOODS_SUPPLIER_WLAMEND');
COMMIT;

INSERT /*+ APPEND */ INTO DWH_DATAFIX.MART_FOODS_SUPPLIER_WLAMEND rtl_mart 
with ITEM_LIST AS
  (
  SELECT DI.ITEM_NO,
    DI.SK1_ITEM_NO,
...
from ITEM_LIST a, 
 ...
 where G.RECEIVE_DATE = D.CALENDAR_DATE 
  AND (  G.RECEIVE_DATE = G_DATE OR G.LAST_UPDATED_DATE = G_DATE) 
  and G.PO_NO = F.PO_NO 

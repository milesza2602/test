--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_281U_WL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_281U_WL" 
...
--------------------------------
--------------------------------
-- Rule 2&3 - include records where not on promotion for the  6 weeks
...
--------------------------------
 insert /*+ APPEND parallel (aps,2) */ into dwh_performance.MART_FD_ITEM_6WKAVG
            WITH selmaxp AS
                          (
                            SELECT /*+ parallel(tmpm,2) */
                                        SK1_ITEM_NO,    item_no,    fin_year_no,    fin_week_no,    week_count,    MAX(sk1_prom_no) maxprom, sum(nvl(sales,0)) totsales
                            FROM dwh_performance.temp_mart_6wkavg_prom_VALUES tmpm
                        --   where sk1_item_no in (27782,66235,189433,231416,269796,306577,320690,17511049,   61400,14319513, 16386749)
      ...
      FROM
                (SELECT
                  /*+ parallel(tmp,2) parallel(dns,2) parallel(sprs,2) */
                      TMP.SK1_ITEM_NO ,
                ...
                JOIN dwh_performance.temp_mart_6wkavg_prom_values TMP

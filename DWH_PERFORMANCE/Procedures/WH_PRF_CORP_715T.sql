--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_715T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_715T" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - RTL_LOC_ITEM_WK_SPACERACT
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_715T';
...
--    execute immediate 'truncate table DWH_PERFORMANCE.RTL_LOC_ITEM_WK_SPACERACT';
--   l_text := ' DWH_PERFORMANCE.RTL_LOC_ITEM_WK_SPACERACT truncated ' ;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
 insert /*+ parallel(dest,4) */ into DWH_PERFORMANCE.RTL_LOC_ITEM_WK_SPACERACT dest
...
;  
    ...
    commit;
--
    l_text := 'TABLE DWH_PERFORMANCE.RTL_LOC_ITEM_WK_SPACERACT POPULATED';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text :=  dwh_constants.vc_log_records_inserted||g_recs_inserted;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
--    l_text := 'Running GATHER_TABLE_STATS ON RTL_LOC_ITEM_WK_SPACERACT';
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--   
--   EXECUTE IMMEDIATE 'ALTER SESSION enable PARALLEL DML';
--
--   DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_LOC_ITEM_WK_SPACERACT', CASCADE => TRUE, DEGREE => 16);
--
--   l_text := 'GATHER_TABLE_STATS ON RTL_LOC_ITEM_WK_SPACERACT completed';
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 

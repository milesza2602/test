--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_105U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_105U" 
...
-- 1. Removed hash join hints in MERGE in local_bulk_merge
-- **************************************************************************************************
...
--               with input ex RMS Stock table from foundation layer AFRICA DATA ONLY.
--  Tables:      Input  - fnd_mc_loc_item_dy_rms_stk_ch
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_105U';
...
type tbl_array_m is table of rtl_mc_loc_item_dy_rms_stock%rowtype index by binary_integer;
...
cursor c_fnd_rtl_loc_item_dy_rms_stk is
   select 
          ...
          dih.sk2_item_no,
          dlh.sk2_location_no
   from   fnd_mc_loc_item_dy_rms_stk_ch fnd_lid,
          dim_item di,
          dim_location dl,
          dim_item_hist dih,
...
begin
   ...
   g_rec_out.clear_soh_selling               := g_rec_in.clear_soh_selling_opr;
   g_rec_out.clear_soh_cost                  := g_rec_in.clear_soh_cost_opr;
...
--MC--   
    g_rec_out.clear_soh_cost_local            := g_rec_in.clear_soh_cost_local;
    g_rec_out.clear_soh_selling_local         := g_rec_in.clear_soh_selling_local;
...
--MC--   
   ...
   g_rec_out.clear_soh_fr_cost_local               := '';
...
--Multi currency --   
   ...
   g_rec_out.clear_soh_margin_local    := nvl(g_rec_out.clear_soh_selling_local,0) - nvl(g_rec_out.clear_soh_cost_local,0);
   g_rec_out.reg_soh_margin_local      := nvl(g_rec_out.reg_soh_selling_local,0) - nvl(g_rec_out.reg_soh_cost_local,0);
   ...
   if g_rec_in.chain_no = 20 then
      ...
      g_rec_out.clear_soh_fr_cost_local    := nvl(g_rec_out.clear_soh_cost_local,0) + round((nvl(g_rec_out.clear_soh_cost_local,0) * g_debtors_commission_perc / 100),2);
...
begin
    ...
    forall i in a_tbl_merge.first .. a_tbl_merge.last
       save exceptions
-- PW 2019-01-23 12.2 upgrade performance tuning - remove hash
--       merge /*+ USE_HASH(rtl_lid ,mer_lid)*/ into rtl_mc_loc_item_dy_rms_stock rtl_lid
       merge into rtl_mc_loc_item_dy_rms_stock rtl_lid
       using
...
--MC--                
                a_tbl_merge(i).	INBND_EXCL_CUST_ORD_COST_LOCAL	AS	INBND_EXCL_CUST_ORD_COST_LOCAL	,
                a_tbl_merge(i).	INBND_EXCL_CUST_ORD_SELL_LOCAL	AS	INBND_EXCL_CUST_ORD_SELL_LOCAL	,
                a_tbl_merge(i).	INBND_INCL_CUST_ORD_COST_LOCAL	AS	INBND_INCL_CUST_ORD_COST_LOCAL	,
                a_tbl_merge(i).	INBND_INCL_CUST_ORD_SELL_LOCAL	AS	INBND_INCL_CUST_ORD_SELL_LOCAL	,
                a_tbl_merge(i).	SIT_COST_LOCAL	AS	SIT_COST_LOCAL	,
                a_tbl_merge(i).	SIT_SELLING_LOCAL	AS	SIT_SELLING_LOCAL	,
                a_tbl_merge(i).	SIT_FR_COST_LOCAL	AS	SIT_FR_COST_LOCAL	,
                a_tbl_merge(i).	SIT_MARGIN_LOCAL	AS	SIT_MARGIN_LOCAL	,
                a_tbl_merge(i).	SOH_COST_LOCAL	AS	SOH_COST_LOCAL	,
                a_tbl_merge(i).	SOH_SELLING_LOCAL	AS	SOH_SELLING_LOCAL	,
                a_tbl_merge(i).	SOH_FR_COST_LOCAL	AS	SOH_FR_COST_LOCAL	,
                a_tbl_merge(i).	SOH_MARGIN_LOCAL	AS	SOH_MARGIN_LOCAL	,
                a_tbl_merge(i).	FRANCHISE_SOH_MARGIN_LOCAL	AS	FRANCHISE_SOH_MARGIN_LOCAL	,
                a_tbl_merge(i).	BOH_SELLING_LOCAL	AS	BOH_SELLING_LOCAL	,
                a_tbl_merge(i).	BOH_COST_LOCAL	AS	BOH_COST_LOCAL	,
                a_tbl_merge(i).	BOH_FR_COST_LOCAL	AS	BOH_FR_COST_LOCAL	,
                a_tbl_merge(i).	CLEAR_SOH_COST_LOCAL	AS	CLEAR_SOH_COST_LOCAL	,
                a_tbl_merge(i).	CLEAR_SOH_SELLING_LOCAL	AS	CLEAR_SOH_SELLING_LOCAL	,
                a_tbl_merge(i).	CLEAR_SOH_MARGIN_LOCAL	AS	CLEAR_SOH_MARGIN_LOCAL	,
                a_tbl_merge(i).	CLEAR_SOH_FR_COST_LOCAL	AS	CLEAR_SOH_FR_COST_LOCAL	,
                a_tbl_merge(i).	REG_SOH_SELLING_LOCAL	AS	REG_SOH_SELLING_LOCAL	,
                a_tbl_merge(i).	REG_SOH_COST_LOCAL	AS	REG_SOH_COST_LOCAL	,
                a_tbl_merge(i).	REG_SOH_FR_COST_LOCAL	AS	REG_SOH_FR_COST_LOCAL	,
                a_tbl_merge(i).	REG_SOH_MARGIN_LOCAL	AS	REG_SOH_MARGIN_LOCAL	
...
--MC--              
              INBND_EXCL_CUST_ORD_COST_LOCAL	= MER_LID.	INBND_EXCL_CUST_ORD_COST_LOCAL	,
              INBND_EXCL_CUST_ORD_SELL_LOCAL	= MER_LID.	INBND_EXCL_CUST_ORD_SELL_LOCAL	,
              INBND_INCL_CUST_ORD_COST_LOCAL	= MER_LID.	INBND_INCL_CUST_ORD_COST_LOCAL	,
              INBND_INCL_CUST_ORD_SELL_LOCAL	= MER_LID.	INBND_INCL_CUST_ORD_SELL_LOCAL	,
              SIT_COST_LOCAL	                = MER_LID.	SIT_COST_LOCAL	,
              SIT_SELLING_LOCAL	              = MER_LID.	SIT_SELLING_LOCAL	,
              SIT_FR_COST_LOCAL	              = MER_LID.	SIT_FR_COST_LOCAL	,
              SIT_MARGIN_LOCAL	              = MER_LID.	SIT_MARGIN_LOCAL	,
              SOH_COST_LOCAL	                = MER_LID.	SOH_COST_LOCAL	,
              SOH_SELLING_LOCAL	              = MER_LID.	SOH_SELLING_LOCAL	,
              SOH_FR_COST_LOCAL	              = MER_LID.	SOH_FR_COST_LOCAL	,
              SOH_MARGIN_LOCAL	              = MER_LID.	SOH_MARGIN_LOCAL	,
              FRANCHISE_SOH_MARGIN_LOCAL	    = MER_LID.	FRANCHISE_SOH_MARGIN_LOCAL	,
              BOH_SELLING_LOCAL	              = MER_LID.	BOH_SELLING_LOCAL	,
              BOH_COST_LOCAL	                = MER_LID.	BOH_COST_LOCAL	,
              BOH_FR_COST_LOCAL	              = MER_LID.	BOH_FR_COST_LOCAL	,
              CLEAR_SOH_COST_LOCAL	          = MER_LID.	CLEAR_SOH_COST_LOCAL	,
              CLEAR_SOH_SELLING_LOCAL	        = MER_LID.	CLEAR_SOH_SELLING_LOCAL	,
              CLEAR_SOH_MARGIN_LOCAL	        = MER_LID.	CLEAR_SOH_MARGIN_LOCAL	,
              CLEAR_SOH_FR_COST_LOCAL       	= MER_LID.	CLEAR_SOH_FR_COST_LOCAL	,
              REG_SOH_SELLING_LOCAL	          = MER_LID.	REG_SOH_SELLING_LOCAL	,
              REG_SOH_COST_LOCAL	            = MER_LID.	REG_SOH_COST_LOCAL	,
              REG_SOH_FR_COST_LOCAL	          = MER_LID.	REG_SOH_FR_COST_LOCAL	,
              REG_SOH_MARGIN_LOCAL	          = MER_LID.	REG_SOH_MARGIN_LOCAL	 
...
begin
    ...
    l_text := 'LOAD OF rtl_mc_loc_item_dy_rms_stock EX FOUNDATION STARTED '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_001A
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_001A" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_001A';
...
-- Testing parameters ...
--and fr.last_updated_date                                  = '27 JAN 2018'
...
begin
      -- REF03Mar2018 (strt)
      select /*+ parallel(vr,2) */  
             MAX(vat_rate_perc) VAT_RATE_PERC
      into   g_rec_in.vat_rate_perc 
      from   fnd_item_vat_rate   vr
      where  item_no       =  g_rec_in.item_no
      and    vat_region_no =  g_rec_in.vat_region_no
      and    g_rec_in.post_date_calc between active_from_date and active_to_date;

         exception
            when no_data_found then
              g_rec_in.vat_rate_perc  := g_rec_in.vat_rate_perc;
      -- REF03Mar2018 (end)

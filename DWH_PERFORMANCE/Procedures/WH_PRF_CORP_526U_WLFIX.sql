--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_526U_WLFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_526U_WLFIX" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        FEB 2014
--  Author:      Alastair de Wet
--  Purpose:     Create LOC_BU_DY_SHRINKAGE rollup fact table in the performance layer
...
--
--  Maintenance:
--  08 May 2014 - xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
--
--  Naming conventions
--  g_  -  Global variable
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            rtl_loc_bu_dy_shrinkage%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_526U_WLFIX';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE RMS DENSE PERFORMANCE to LOC_BU_DAY';
...
begin
    ...
    l_text := 'ROLLUP OF rtl_loc_bu_dy_shrinkage EX DAY LEVEL STARTED AT '||
...
--RLIWKRMSA_M20173_11
    ...
    DELETE FROM dwh_performance.rtl_loc_bu_dy_shrinkage
    WHERE post_date BETWEEN g_this_week_start_date AND g_this_week_END_date;

    g_recs_deleted := g_recs_deleted + SQL%ROWCOUNT;

    l_text := 'DELETED PREVIOUS DATA:- '||g_this_week_start_date||'  to '||g_this_week_END_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
commit;  --ADDED 25/12/2014 DUE TO AN ORA-12839 ERROR BEING GENERATED
...
insert /*+ APPEND parallel (rtl_liwrd,4) */ into dwh_performance.rtl_loc_bu_dy_shrinkage rtl_liwrd
   ...
   FROM   rtl_loc_item_dy_rms_dense lid,
          dim_item di
   WHERE  lid.sk1_item_no       = di.sk1_item_no AND
          lid.post_date          BETWEEN g_this_week_start_date AND g_this_week_END_date AND
          sales_qty             is not null

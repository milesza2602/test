--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_566U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_566U" 
                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Oct 2008
--  Author:      Christie Koorts
--  Purpose:     Create location dept day fact table in the performance layer with
--               added value ex foundation layer fin location recon 3 day table.
--  Tables:      Input  -   fnd_fin_loc_recon_3_dy
--               Output -   rtl_loc_comp_dy
--  Packages:    constants, dwh_log, dwh_valid
--  Comments:    Single DML could be considered for this program.
--
--  Maintenance:
--  21 Feb 2009 - QC934 - Changed source table from rtl_loc_dept_dy to rtl_loc_comp_dy.
--  1 April 2009 - defect 1229 - - no filter on driving cursor
--  Naming conventions:
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_566U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE RTL_LOC_COMP_DY EX FND_FIN_LOC_RECON_3_DY';
...
cursor c_fnd_fin_loc_recon_3_dy is
   with chgrecs as
  (select   location_no, tran_date
   from     fnd_fin_loc_recon_3_dy
   where    last_updated_date = g_date
   and      record_id         = 'D'
   group by location_no, tran_date)
   
   select   dl.sk1_location_no,
            dc.sk1_company_no,
            flr.tran_date,
            max(dlh.sk2_location_no) sk2_location_no,
            sum(flr.tran_value) cra_tender_sales
   from     chgrecs cr
   join     fnd_fin_loc_recon_3_dy flr   on  cr.location_no    = flr.location_no 
   ...
   join     dim_company dc               on  1                 = 1
   where    dc.company_no = 1
   group by dl.sk1_location_no, dc.sk1_company_no, flr.tran_date;

-- Input record declared as cursor%rowtype
g_rec_in             c_fnd_fin_loc_recon_3_dy%rowtype;

-- Input bulk collect table declared
type stg_array is table of c_fnd_fin_loc_recon_3_dy%rowtype;
...
begin
   ...
   g_rec_out.cra_tender_sales               := g_rec_in.cra_tender_sales;
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      set sk1_location_no                = a_tbl_update(i).sk1_location_no,
          ...
          cra_tender_sales               = a_tbl_update(i).cra_tender_sales,
...
begin
    ...
    l_text := 'LOAD OF RTL_LOC_COMP_DY EX FND_FIN_LOC_RECON_3_DY STARTED '||
...
--**************************************************************************************************
    open c_fnd_fin_loc_recon_3_dy;
    fetch c_fnd_fin_loc_recon_3_dy bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_fin_loc_recon_3_dy;
--**************************************************************************************************

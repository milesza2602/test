--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_006U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_006U" 
...
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;
 ...
 TYPE year_sub_rec is RECORD (
  ...
  TYPE loop_list_tab IS TABLE OF year_sub_rec;   
  l_loop_list               loop_list_tab;
...
--**************************************************************************************************
  g_rec_cnt := 0;
    Select count(*) into g_rec_cnt from dwh_foundation.FND_S4S_LOC_EMP_JOB_PAYCAT_DY where last_updated_date >= g_date-7; --Weekly Load Test
    --Select count(*) into g_rec_cnt from dwh_foundation.FND_S4S_LOC_EMP_JOB_PAYCAT_DY where last_updated_date = g_date;  --Weekly Load Test PROD!!!!!
...
--        DWH_PERFORMANCE.DWH_S4S.get_subpartition_list (l_table_name, L_table_owner, g_date,  g_subpart_column_name, l_loop_list);
--dbms_output.put_line(g_sub||', l_table_name = '||l_table_name||', l_table_owner = '||l_table_owner||', g_date = '||g_date||', g_subpart_column_name = '||g_subpart_column_name);
--return;
    ...
    begin
       for g_sub in 1 .. g_loop_cnt                                                                                                  
         loop 
           ...
           --  dbms_output.put_line(g_sub||', l_table_name = '||l_table_name||', l_table_owner = '||l_table_owner||', g_date = '||g_date||', g_subpart_type = '||g_subpart_type||', g_subpart_column_name = '||g_subpart_column_name||', G_LOOP_FIN_YEAR_NO = '||G_LOOP_FIN_YEAR_NO||', G_LOOP_FIN_WEEK_NO = '||G_LOOP_FIN_WEEK_NO);
           else
...
--             where  calendar_date = (g_date) - (g_sub * 7);  
       --**************************************************************************************************
      -- Truncate Table Partition (Remove previous load periods for reload)
      --**************************************************************************************************  
      dbms_output.put_line ('p_name is : ' ||l_name);
   dbms_output.put_line ('p_system_name is : ' ||l_system_name);
      dbms_output.put_line ('p_script_name is : ' ||l_script_name);
            dbms_output.put_line ('p_procedure_name is : ' ||l_procedure_name);
                        dbms_output.put_line ('P_table_name is : ' ||l_table_name);

                                      dbms_output.put_line ('P_owner_name is : ' ||l_table_owner);
                                                    dbms_output.put_line ('p_fin_year_no is : ' ||G_LOOP_FIN_YEAR_NO);
                                                                  dbms_output.put_line ('p_fin_sub_no is : ' ||G_LOOP_FIN_WEEK_NO);
...
--           FOR rec in c1
--           LOOP
--               execute immediate rec.enable_stmt;               
--           END LOOP;       

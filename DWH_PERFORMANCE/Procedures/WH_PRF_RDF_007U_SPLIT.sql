--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_007U_SPLIT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_007U_SPLIT" (p_forall_limit in integer, p_success out boolean)
...
--   select /*+ parallel(wkfcst,8) full(di) full(dl)*/
--            location_no,
--            item_no,
...
--        USING
--          (select  /*+ parallel(sys_ilv,8) full(sys_ilv) full(di) full(dl) full(dlh) full(dih) full(zi) full(dc) */

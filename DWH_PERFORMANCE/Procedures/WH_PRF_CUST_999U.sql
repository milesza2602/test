--------------------------------------------------------
--  DDL for Procedure WH_PRF_CUST_999U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CUST_999U" 
...
-- WHEN PROCEDURE RAN, HAD INCORRECT NAME - so search for WH_PRF_BRTH_900U in sys_dwh_log
...
begin
 G_SQL := '';
    ...
    select  to_char(TRAN_DATE,''yyyy-mm-dd'')||'',''||
            ...
            TO_CHAR(TRAN_SELLING,''fm99999999990.00'') TRAN_SELLING 
...
-------------------------------------- CORPORATE   
  G_SQL := '';
    G_SQL :=    
'select RECTYPE||''|''||RECTYPE_DATETIME||''|''||RECDESC 
    ...
    union all
     select RECTYPE||''|''||
            RECCNT||''|''||
            TRAN_SELLING
     from dwh_cust_performance.cust_vmp_sales_ms_corp_FTR';

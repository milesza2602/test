--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110U_VAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110U_VAT" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as
--**************************************************************************************************
--  Date:        Sept 2008
...
--               Output - W6005682.RTL_LOC_ITEM_DY_RMS_DENSEQ
...
g_rec_out            W6005682.RTL_LOC_ITEM_DY_RMS_DENSEQ%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_110U_'|| p_from_loc_no || '_VAT';
...
type tbl_array_m is table of W6005682.RTL_LOC_ITEM_DY_RMS_DENSEQ%rowtype index by binary_integer;
...
cursor c_fnd_rtl_loc_item_dy_rms_sale is
   select /*+ full(di) full(dl) full(dlh) full(dih) full(fnd_li) parallel(fnd_li,6)  */  
          ...
          invoice_adj_cost,
          prom_discount_no,
          di.standard_uom_code,di.business_unit_no,
         
          case when fnd_li.tax_perc is null then
              case when vr.vat_rate_perc is null then
                  dl.default_tax_region_no_perc
              else
                  vr.vat_rate_perc
              end
          else 
              fnd_li.tax_perc                                  -- di.vat_rate_perc,
          end as tax_perc,
   ...
   from   fnd_rtl_loc_item_dy_rms_sale fnd_lid
          ...
          join dim_location_VATWL dl on fnd_lid.location_no     = dl.location_no 
          ...
          left outer join fnd_item_vat_rate vr on di.item_no              = vr.item_no
...
g_process_no := 1;
   g_rec_out.sales_incl_vat        := round(nvl(g_rec_out.sales,0) * (100 + g_rec_in.tax_perc) / 100,2);
...
begin
    forall i in a_tbl_merge.first .. a_tbl_merge.last
       save exceptions
MERGE /*+ USE_HASH(rtl_lidrd ,mer_rlidrd)*/ INTO W6005682.RTL_LOC_ITEM_DY_RMS_DENSEQ rtl_lidrd
USING
(
...
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'LOAD OF W6005682.RTL_LOC_ITEM_DY_RMS_DENSEQ EX FOUNDATION STARTED '||
...
--select sum(sales) 
--into g_prf_sale
--from W6005682.RTL_LOC_ITEM_DY_RMS_DENSEQ

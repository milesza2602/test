--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_223U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_223U" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - tmp_mart_foods_wkly_snpsht
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  21 feb 2014 add pack_item_ind
--  09 Nov 2016 - A Joshua - Level1 rename (chg-2218)
--  04 Apr 2017 - A Joshua   - Add 3 new measures for Customer Availability (Chg-5359)
--  05 Feb 2018 - Chg-12260  - Add 6 new measures for Customer Availability (Chg_12260)
--
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_223U_TST';
...
----------------------------------------------------------------------------------------------------
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE IMMEDIATE('truncate table dwh_performance.tmp_mart_foods_wkly_snpsht');
...
INSERT /*+ APPEND */ INTO dwh_performance.tmp_mart_foods_wkly_snpsht mart
with item_list as (
...
item_pclass as (
select /*+ full(uda) */ di.sk1_item_no,
       di.item_no,
       SUBSTR(di.item_desc,1,60) AS item_desc,
       di.department_no,
       di.department_name,
       di.subclass_no,
       di.subclass_name,
...
from RTL_LOC_ITEM_RDF_WKFCST_L1 f, item_list il, store_list sl 
--from rtl_loc_item_wk_rdf_fcst f, item_list il, store_list sl    --RDF L1/L2 remapping chg-2218
...
--from rtl_loc_item_dy_rdf_fcst f, item_list il, store_list_area sl   --RDF L1/L2 remapping chg-2218
...
catalog_measures as (
select /*+ PARALLEL(f,4)  full(iu) full(f) */
       f.sk1_location_no,
       f.sk1_item_no,
       ...
       nvl(f.fd_num_cust_avail_adj,0) fd_num_cust_avail_adj, -- chg5359
       nvl(f.fd_num_cust_catlg_adj,0) fd_num_cust_catlg_adj, -- Chg12260
       nvl(f.fd_cust_avail,0) fd_cust_avail, -- Chg12260
...
select /*+ PARALLEL(f,4) FULL(f) full(iu) */
       f.sk1_location_no,
       f.sk1_item_no,
...
select /*+ PARALLEL(f,4)  */
       ...
       nvl(f.fd_num_cust_avail_adj,0) fd_num_cust_avail_adj_ly, -- chg5359
       nvl(f.fd_num_cust_catlg_adj,0) fd_num_cust_catlg_adj_ly, -- Chg12260
       nvl(f.fd_cust_avail,0) fd_cust_avail_ly, -- Chg12260
       case when sl.location_no in (2070,2200,3050,4000,6010,6060,6110) then
...
select /*+ PARALLEL(f,4) FULL(f) */
       ...
       sum(nvl(f.fd_num_cust_avail_adj,0)) fd_num_cust_avail_adj_ytd, -- chg5359
       sum(nvl(f.fd_num_cust_catlg_adj,0)) fd_num_cust_catlg_adj_ytd, -- Chg12260
       sum(nvl(f.fd_cust_avail,0)) fd_cust_avail_ytd -- Chg12260
...
select /*+ full(ros) parallel(ros,4) */ ros.sk1_location_no, ros.sk1_item_no, ros.avg_units_per_day rate_of_sale_avg_wk, ros.units_per_week rate_of_sale_sum_wk
...
rate_of_sale_6wk as (
select /*+ full(ros) parallel(ros,4) */ ros.sk1_location_no, ros.sk1_item_no,
...
select /*+ full(f) parallel(f,4) */ f.sk1_location_no, il.sk1_item_no, f.sk1_department_no,
...
nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                           f2.sk1_location_no),
                                           f3.sk1_location_no),
                                           f4.sk1_location_no),
                                           f6.sk1_location_no),
...
sk1_location_no,
nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_item_no,f1.sk1_item_no),
                                          f2.sk1_item_no),
                                          f3.sk1_item_no),
                                          f4.sk1_item_no),
                                          f6.sk1_item_no),
...
fy1.fd_num_cust_avail_adj_ytd, -- chg5359
fy1.fd_num_cust_catlg_adj_ytd, -- Chg12260
fy1.fd_cust_avail_ytd, -- Chg12260
...
fy2.fd_num_cust_avail_adj_ly, -- chg5359
fy2.fd_num_cust_catlg_adj_ly, -- Chg12260
fy2.fd_cust_avail_ly, -- Chg12260
f0.boh_ll_adj_qty,
f0.fd_num_cust_avail_adj, -- chg5359
f0.fd_num_cust_catlg_adj, -- Chg12260
f0.fd_cust_avail, -- Chg12260
f6.waste_cost,
f7.waste_cost_ly,
f8.waste_cost_ytd,
...
/*
full outer join rdf_dly_fcst_measures f24 on nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(nvl(f0.sk1_location_no,f1.sk1_location_no),
                                                                                   f2.sk1_location_no),
                                                                                   f3.sk1_location_no),
                                                                                   f4.sk1_location_no),
                                                                                   f6.sk1_location_no),
...
il.pack_item_ind,
atg.fd_num_cust_avail_adj, -- chg5359
atg.fd_num_cust_avail_adj_ytd,  -- chg5359
atg.fd_num_cust_avail_adj_ly, -- chg5359
atg.fd_num_cust_catlg_adj, -- Chg12260
atg.fd_num_cust_catlg_adj_ytd,  -- Chg12260
atg.fd_num_cust_catlg_adj_ly, -- Chg12260
atg.fd_cust_avail, -- Chg12260
atg.fd_cust_avail_ytd,  -- Chg12260
atg.fd_cust_avail_ly -- Chg12260

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_195U_TST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_195U_TST" (p_forall_limit in integer,p_success out boolean) as
...
g_count              number        :=  0;
g_rec_out            dwh_datafix.aj_rtl_loc_item_dy_catalog%rowtype;
g_found              boolean;
...
g_start_date         date          := '04 mar 18';
g_end_date           date          := '04 mar 18';

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_195U_TST';
...
type tbl_array_i is table of dwh_datafix.aj_rtl_loc_item_dy_catalog%rowtype index by binary_integer;
type tbl_array_u is table of dwh_datafix.aj_rtl_loc_item_dy_catalog%rowtype index by binary_integer;
...
cursor C_RTL_LOC_ITEM_DY_CATALOG is
   select /*+ parallel (lid,6) full (di) full (dl) full (diu) full (dc) */ 
          lid.sk1_item_no,
          lid.sk1_location_no,
          lid.calendar_date,
...
--          lid.sk1_item_no                = 18697162 and
--          lid.sk1_location_no in (692,693,694) and
          lid.calendar_date              = '05 mar 18';
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into dwh_datafix.aj_rtl_loc_item_dy_catalog values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update dwh_datafix.aj_rtl_loc_item_dy_catalog
       set /*   fd_num_avail_days             = a_tbl_update(i).fd_num_avail_days,
              ...
              fd_num_cust_catlg_adj           = a_tbl_update(i).fd_num_cust_catlg_adj
--              fd_num_cust_avail_adj           = a_tbl_update(i).fd_num_cust_avail_adj

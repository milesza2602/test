--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_196U_LOAD2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_196U_LOAD2" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        Feb 2009
--  Author:      Alastair de Wet
--  Purpose:     Update RMS Food Catalog fact table in the performance layer
--               with input ex RMS Dense table sales
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_196U_LOAD2';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE FOOD CATALOG EX RMS DENSE';
...
cursor c_rtl_loc_item_dy_rms_dense is
   ...
   Where
          ...
          --lid.post_date                  = g_date and 
          lid.sk1_item_no                = di.sk1_item_no and
          di.business_unit_no            = 50 
          --and di.department_no      in (16,34,42,52,62,85)
          and lid.sk1_item_no            = fix.sk1_item_no
          and lid.sk1_location_no        = fix.sk1_location_no
   )
   select lid.sk1_item_no,
          lid.sk1_location_no,
          lid.calendar_date,
   ...
   where  lid.sk1_item_no                = lid_list.sk1_item_no and
          ...
          lid.calendar_date              = lid_list.post_date;
...
begin
   ...
   g_rec_out.WEIGHTED_AVAIL_SALES            := null;
   g_rec_out.WEIGHTED_AVAIL_SALES_QTY        := null;
   g_rec_out.WEIGHTED_ADJ_AVAIL_SALES        := null;
   g_rec_out.WEIGHTED_ADJ_AVAIL_SALES_QTY    := null;

   if  g_rec_in.fd_num_avail_days = 1 then
       g_rec_out.WEIGHTED_AVAIL_SALES            := g_rec_in.sales;
       g_rec_out.WEIGHTED_AVAIL_SALES_QTY        := g_rec_in.sales_qty;
   end if;
   if  g_rec_in.fd_num_avail_days_adj = 1 then
       g_rec_out.WEIGHTED_ADJ_AVAIL_SALES        := g_rec_in.sales;
       g_rec_out.WEIGHTED_ADJ_AVAIL_SALES_QTY    := g_rec_in.sales_qty;
   end if;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    WEIGHTED_AVAIL_SALES            = a_tbl_update(i).WEIGHTED_AVAIL_SALES,
              WEIGHTED_ADJ_AVAIL_SALES        = a_tbl_update(i).WEIGHTED_ADJ_AVAIL_SALES,
              WEIGHTED_AVAIL_SALES_QTY        = a_tbl_update(i).WEIGHTED_AVAIL_SALES_QTY,
              WEIGHTED_ADJ_AVAIL_SALES_QTY    = a_tbl_update(i).WEIGHTED_ADJ_AVAIL_SALES_QTY,
...
begin
    ...
    l_text := 'LOAD OF rtl_loc_item_dy_catalog  FOODS EX DENSESTARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_169U_SSFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_169U_SSFIX" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2013
--  Author:      Q. Smit
...
--**************************************************************************************************
   ...
   select min(trading_date), max(trading_date)
     ...
     from dwh_foundation.FND_JDAFF_ST_PLAN_ANALYSIS_WK
  ...
  INSERT /*+ APPEND parallel (jdast,4) full(jdast)  */
   ...
   SELECT /*+ full(di) full(dl)  full (jdaff) parallel (jdaff,4) */
            di.sk1_item_no,
            dl.sk1_location_no,
            jdaff.trading_date,
            jdaff.POST_DATE,

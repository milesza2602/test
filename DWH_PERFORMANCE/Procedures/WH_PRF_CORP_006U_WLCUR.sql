--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_006U_WLCUR
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_006U_WLCUR" (
...
AS
  ...
  --
  --  maintenance:
  --  10 feb 2009 - defect 782- dim_calendar_WLCUR concatination spacings
  ...
  --                             - on column = day_name
  -- 19 march 2009 - defect 1171 - update etl comments for some dim_calendar_WLCUR ...
  --                                _desc fields
  ...
  --                               dim_calendar_WLCUR & dim_calendar_WLCUR_wk
  -- 25 march 2009 - defect 1239 - cal_month_short_desc on dim_calendar_WLCUR
  --                                & dim_calendar_WLCUR_wk
  ...
  --                               a monday (all cubes)
  -- 7 june 2011 - defect 4342 - add extra logging for dim_calendar_WLCUR processing
  --                             add auto generate of new fin year
--  13 june 2011 - defect 4354 - FIN YR-END 2011 - changes to generation of dim_calendar_WLCUR
--
  --  naming conventions:
  --  g_  -  global variable
  ...
  g_rec_out dim_calendar_WLCUR%ROWTYPE;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%TYPE := 'WH_PRF_CORP_006U_CUR';
  l_name sys_dwh_log.log_name%TYPE                     := dwh_constants.vc_log_name_rtl_md;
  l_system_name sys_dwh_log.log_system_name%TYPE       := dwh_constants.vc_log_system_name_rtl_prf;
  l_script_name sys_dwh_log.log_script_name%TYPE       := dwh_constants.vc_log_script_rtl_prf_md;
  l_procedure_name sys_dwh_log.log_procedure_name%TYPE := l_module_name;
  l_text sys_dwh_log.log_text%TYPE ;
  l_description sys_dwh_log_summary.log_description%TYPE   := 'CREATE dim_calendar_WLCUR EX FND_CALENDAR';
...
TYPE tbl_array_i
IS
  TABLE OF dim_calendar_WLCUR%ROWTYPE INDEX BY binary_integer;
TYPE tbl_array_u
IS
  TABLE OF dim_calendar_WLCUR%ROWTYPE INDEX BY binary_integer;
...
BEGIN
  ...
  SAVE EXCEPTIONS
  INSERT INTO dim_calendar_WLCUR VALUES a_tbl_insert    (i    );
...
AS
BEGIN
--  g_found := dwh_valid.dim_calendar_WLCUR(g_rec_out.calendar_date);
G_FOUND := TRUE;
...
BEGIN
    ...
    update dim_calendar_WLCUR
    set RSA_PUBLIC_HOLIDAY_IND = 1
    where calendar_date in(select calendar_date+1 from dim_calendar_WLCUR where RSA_PUBLIC_HOLIDAY_IND = 2)
    ;
    commit;
    update dim_calendar_WLCUR
    set RSA_PUBLIC_HOLIDAY_IND = 1
...
BEGIN
  ...
  l_text := 'LOAD OF dim_calendar_WLCUR EX FND_CALENDAR STARTED AT '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  --**************************************************************************************************
  -- check current financial-year vs those on dim_calendar_WLCUR
  --**************************************************************************************************
  ...
  l_text             := '** Current Fin_year in dim_calendar_WLCUR = '||g_current_fin_year;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  --**************************************************************************************************
  -- check maximum financial-year on dim_calendar_WLCUR
  --**************************************************************************************************
  g_min_fin_year      := 0;
  g_max_fin_year      := 0;
  ...
  l_text := '** Maximum Fin_year and date in dim_calendar_WLCUR = '||g_max_fin_year||'  -  '||g_max_calendar_date;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  --**************************************************************************************************
  -- count no. of weeks and days per fin_year on dim_calendar_WLCUR
  --**************************************************************************************************

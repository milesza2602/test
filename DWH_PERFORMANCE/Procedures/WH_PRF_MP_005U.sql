--------------------------------------------------------
--  DDL for Procedure WH_PRF_MP_005U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MP_005U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        October 2008
--  Author:      Alfonso Joshua
--  Purpose:     Create the product chain subclass plan type table in the performance layer
--               with input ex MP table from foundation layer.
--  Tables:      Input  - fnd_chain_subclass_wk_plan
--               Output - rtl_chain_subclass_wk_mp_plan
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  08 Jan 2008 - defect 453 - Change to fnd_chain_subclass_wk_plan structure
--              - defect 454 - Change to rtl_chain_subclass_wk_mp_plan structure
...
g_count              number        :=  0;
g_rec_out            rtl_chain_subclass_wk_mp_plan%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MP_005U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE MP PRODUCT CHAIN SUBCLASS PLAN TYPE FACTS EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For output arrays into bulk load forall statements --
type tbl_array_i is table of rtl_chain_subclass_wk_mp_plan%rowtype index by binary_integer;
type tbl_array_u is table of rtl_chain_subclass_wk_mp_plan%rowtype index by binary_integer;
...
--   g_rec_out.pln_chain_rtv_cost              := g_rec_in.pln_chain_rtv_cost;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into rtl_chain_subclass_wk_mp_plan values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update rtl_chain_subclass_wk_mp_plan
...
--              pln_chain_rtv_cost             = a_tbl_update(i).pln_chain_rtv_cost,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_chain_subclass_wk_mp_plan
...
begin
    ...
    l_text := 'LOAD OF RTL_CHAIN_SUBCLASS_WK_MP_PLAN EX FOUNDATION STARTED AT '||
...
begin
  DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE','RTL_CHAIN_SUBCLASS_WK_MP_PLAN',estimate_percent=>1, DEGREE => 32);
  commit;
end;
   l_text := 'RTL_CHAIN_SUBCLASS_WK_MP_PLAN completed';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

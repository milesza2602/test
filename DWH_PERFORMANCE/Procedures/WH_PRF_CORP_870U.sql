--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_870U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_870U" -- STORE PROC CHANGE
...
--               Output - rtl_item_supplier_wk_list                                                       
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_870U';                              -- STORE PROC CHANGE
...
--************************************************************************************************** 
-- Truncate data as per 6 week data refresh
--**************************************************************************************************
procedure do_truncate as
begin
    ...
    with 
        loc_list  as 
             (select sk1_location_no
               ...
               where chain_no = 10),
        item_list as 
             (select rtl.sk1_item_no,
                     ds.sk1_supplier_no
                from rtl_item_supplier rtl,
                     dim_item di,                     
--                   DWH_DATAFIX.AJ_SUPPLIER_BRIDGETHORNE DS
                     rtl_supplier_lookup ds
               where di.business_unit_no in (50)
                   and rtl.sk1_supplier_no = ds.sk1_supplier_no                   
                   and rtl.sk1_item_no     = di.sk1_item_no
                   and rtl.sk1_item_no not in (24367063,24367064,22336983,22336984,24008436,23993978)
                  ), 
        calendar_list as
        ...
        sales_measures as 
            ...
            (  
              ...
              where spa.sk1_item_no = di.sk1_item_no
                ...
                and spa.waste_cost is not null
            ),
            stock_measures as 
            (  
            ...
            (  
              ...
              and (po.po_grn_selling is not null or
                   po.po_grn_qty is not null)
   ...
   update set                                                                                                      -- COLUNM NAME CHANGE 
          ...
          rtl.this_week_start_date       = g_start_date,
          rtl.last_updated_date          = g_end_date
...
--**************************************************************************************************
    ...
    l_text := 'TRUNCATE DATA STARTING:- ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
     
    do_truncate;
    
    l_text := 'TRUNCATE DATA ENDED:- ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       RAISE;
end WH_PRF_CORP_870U                                                                                             -- STORE PROC CHANGE 
;

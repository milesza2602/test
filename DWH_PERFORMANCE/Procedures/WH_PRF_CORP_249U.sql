--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_249U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_249U" (p_forall_limit in integer, p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_249U';
...
-- Partition Stats
--**------------------------------------------------
l_text := 'Start of stats collection : ' ;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
l_text := 'End of stats collection : ' ;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--**------------------------------------------------
-- Insert
--**------------------------------------------------
...
bb_ly as (
select /*+ materialize cardinality (pr,300000) FULL(pr) */
...
aa as (select /*+ materialize cardinality (pr,300000) FULL(pr) */
       pr.sk1_location_no,
       pr.sk1_item_no,
       max(pr.calendar_date) last_prom_date
from rtl_loc_item_dy_rms_price pr,
...
bb as (
select /*+ materialize cardinality (cbi,300000) FULL(pr) */
       unique pr.sk1_location_no,
       pr.sk1_item_no,
       pr.prom_rsp,

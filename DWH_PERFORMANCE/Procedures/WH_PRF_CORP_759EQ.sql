--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_759EQ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_759EQ" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_759EQ';
...
--   and       l.area_no            = 9951
   ...
   select    2 rectype, u.commercial_manager_desc_562 merch_cat, r.calendar_date,u.product_class_desc_507 prod_class,
             ...
             sum(r.fd_num_avail_days_adj) fd_num_avail_days_adj, sum(r.fd_num_catlg_days_adj) fd_num_catlg_days_adj
   ...
   union all
   select    62 seq_no, 'P.WST%' descr,
             case when sum(nvl(sales,0)) = 0 then 0 else round(((sum(nvl(waste_cost,0)) / sum(sales)) * 100),1) end perc
   from      sales
   where     rectype   = 1
   and       merch_cat = 'TRADING GROUP 1'
   union all
   select    71 seq_no, 'L.WST%' descr,
             case when nvl(sales,0) = 0 then 0 else round(((nvl(waste_cost,0) / sales) * 100),1) end perc
   from      sales
   where     rectype   = 1
   and       post_date = g_date
   ...
   union all
   select    72 seq_no, 'L.WST%' descr,
             case when sum(nvl(sales,0)) = 0 then 0 else round(((sum(nvl(waste_cost,0)) / sum(sales)) * 100),1) end perc
   from      sales
   where     rectype   = 1
   and       merch_cat = 'TRADING GROUP 2'
   union all
   select    21 seq_no, 'T.EOD%' descr,
   ...
   and       post_date = g_date
   union all
   select    22 seq_no, 'T.EOD%' descr,
             case when sum(nvl(fd_num_catlg_days_adj,0)) = 0 then 0
                  else round(((sum(nvl(fd_num_avail_days_adj,0)) / sum(fd_num_catlg_days_adj)) * 100),1) end perc
   from      sales
   where     rectype   = 2
   union all
   select    31 seq_no, 'P.EOD%' descr,
             case when sum(nvl(fd_num_catlg_days_adj,0)) = 0 then 0
                  else round(((sum(nvl(fd_num_avail_days_adj,0)) / sum(fd_num_catlg_days_adj)) * 100),1) end perc
   from      sales
   where     rectype   = 2
...
if g_rec_in.seq_no in (21,31,41,51,61,71) then
   g_sms_string := g_sms_string||'#'||g_rec_in.descr||' '||trim(to_char(g_rec_in.perc,'99990.0'));
end if;
if g_rec_in.seq_no in (22,52,62,72) then
   g_sms_string := g_sms_string||' '||trim(to_char(g_rec_in.perc,'99990.0'));
end if;
if g_rec_in.seq_no in (32,33,34,42,43,44) then
   g_sms_string := g_sms_string||' '||g_rec_in.descr||trim(to_char(g_rec_in.perc,'99990.0'));
end if;

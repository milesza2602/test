--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_223U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_223U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        Dec 2010
--  Author:      Alastair de Wet
--  Purpose:     Load Wkly forecast Table LL 13 week avrg measure in performance layer
--               with input ex FCST & Dim tables from performance layer.
--  Tables:      Input  - rtl_loc_item_wk_rdf_fcst
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_223U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD OF RDF 13 WKLY AVRG EX RDF WKLY FCST ';
...
cursor c_rtl_loc_item_dy_rdf_fcst is
  ...
  select    fcst.sk1_item_no,
            ...
            avg(fcst.sales_wk_app_fcst_qty)  sales_wk_app_fcst_qty_av,
            avg(fcst.sales_wk_app_fcst_qty_flt)  sales_wk_app_fcst_qty_flt_av
  from      rtl_loc_item_wk_rdf_fcst fcst
  join      dim_item di
  on        di.sk1_item_no = fcst.sk1_item_no
  where     fcst.this_week_start_date between g_start_date and
            case
             when di.handling_method_code = 'S' then
             g_end_date
             else
             g_peri_end_date
             end   and
            fcst.sales_wk_app_fcst_qty is not null
...
begin
    ...
    l_text := 'LOAD RTL_LOC_ITEM_WK_RDF_FCST EX SELF TABLES STARTED '||
...
--QC4000
--	The 13 wks avg stored on the weekly forecast table needs to be calculated for current week, but using the
--  forecast starting from the following week, i.e. current week +1.
--  So the 13wks avg calc for week 28 should be based on weeks 29 to 41.
   ...
   for g_sub in 0..1
    ...
    select fin_year_no,fin_week_no,this_week_start_date + 7
    into   g_fin_year_no,g_fin_week_no,g_start_date
    from   dim_calendar
    where calendar_date = g_cal_date  ;

    g_end_date      := g_start_date + 84 ;
    g_peri_end_date := g_start_date + 14 ;

    l_text := 'Week range being aggregated - '||g_start_date|| ' to '||g_end_date||' for long life';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'Week range being aggregated - '||g_start_date|| ' to '||g_peri_end_date||' for perishables';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text := 'And written away to Yr/Wk- '||g_fin_year_no|| '   '||g_fin_week_no ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

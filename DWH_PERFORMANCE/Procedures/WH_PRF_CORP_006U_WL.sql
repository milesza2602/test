--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_006U_WL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_006U_WL" (
...
AS
  ...
  --
  --  maintenance:
  --  10 feb 2009 - defect 782- dwh_performance.dim_calendar_wl concatination spacings
  ...
  --                             - on column = day_name
  -- 19 march 2009 - defect 1171 - update etl comments for some dwh_performance.dim_calendar_wl ...
  --                                _desc fields
  ...
  --                               dwh_performance.dim_calendar_wl & dwh_performance.dim_calendar_wl_wk
  -- 25 march 2009 - defect 1239 - cal_month_short_desc on dwh_performance.dim_calendar_wl
  --                                & dwh_performance.dim_calendar_wl_wk
  ...
  --                               a monday (all cubes)
  -- 7 june 2011 - defect 4342 - add extra logging for dwh_performance.dim_calendar_wl processing
  --                             add auto generate of new fin year
--  13 june 2011 - defect 4354 - FIN YR-END 2011 - changes to generation of dwh_performance.dim_calendar_wl
--
  --  naming conventions:
  --  g_  -  global variable
  ...
  g_rec_out dwh_performance.dim_calendar_wl%ROWTYPE;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%TYPE := 'WH_PRF_CORP_006U_WL';
  l_name sys_dwh_log.log_name%TYPE                     := dwh_constants.vc_log_name_rtl_md;
  l_system_name sys_dwh_log.log_system_name%TYPE       := dwh_constants.vc_log_system_name_rtl_prf;
  l_script_name sys_dwh_log.log_script_name%TYPE       := dwh_constants.vc_log_script_rtl_prf_md;
  l_procedure_name sys_dwh_log.log_procedure_name%TYPE := l_module_name;
  l_text sys_dwh_log.log_text%TYPE ;
  l_description sys_dwh_log_summary.log_description%TYPE   := 'CREATE dwh_performance.dim_calendar_wl EX FND_CALENDAR';
...
TYPE tbl_array_i
IS
  TABLE OF dwh_performance.dim_calendar_wl%ROWTYPE INDEX BY binary_integer;
TYPE tbl_array_u
IS
  TABLE OF dwh_performance.dim_calendar_wl%ROWTYPE INDEX BY binary_integer;
...
BEGIN
  ...
  SAVE EXCEPTIONS
  INSERT INTO dwh_performance.dim_calendar_wl VALUES a_tbl_insert    (i    );
...
BEGIN
  ...
  SAVE EXCEPTIONS
  UPDATE dwh_performance.dim_calendar_wl
...
BEGIN
    ...
    update dwh_performance.dim_calendar_wl
    set RSA_PUBLIC_HOLIDAY_IND = 1
    where calendar_date in(select calendar_date+1 from dwh_performance.dim_calendar_wl where RSA_PUBLIC_HOLIDAY_IND = 2)
    ;
    commit;
    update dwh_performance.dim_calendar_wl
    set RSA_PUBLIC_HOLIDAY_IND = 1
...
BEGIN
  ...
  l_text := 'LOAD OF dwh_performance.dim_calendar_wl EX FND_CALENDAR STARTED AT '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  IF g_today_cal_month_no = 6 THEN
    ...
    FROM dwh_performance.dim_calendar_wl
  ...
  --**************************************************************************************************
  -- check current financial-year vs those on dwh_performance.dim_calendar_wl
  --**************************************************************************************************
  ...
  l_text             := '** Current Fin_year in dwh_performance.dim_calendar_wl = '||g_current_fin_year;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  --**************************************************************************************************
  -- check maximum financial-year on dwh_performance.dim_calendar_wl
  --**************************************************************************************************
  g_min_fin_year      := 0;
  g_max_fin_year      := 0;
  ...
  l_text := '** Maximum Fin_year and date in dwh_performance.dim_calendar_wl = '||g_max_fin_year||'  -  '||g_max_calendar_date;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  --**************************************************************************************************
  -- count no. of weeks and days per fin_year on dwh_performance.dim_calendar_wl
  --**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_197U_LOAD2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_197U_LOAD2" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_197U_LOAD2';
...
cursor c_rtl_location_item is
 ...
 (select /*+ full(li)  */ distinct
   ...
   where li.sk1_location_no              = dl.sk1_location_no and
         ...
         and di.sk1_item_no              = fix.sk1_item_no
         and dl.sk1_location_no          = fix.sk1_location_no
         ),
 ...
 (select dl.sk1_location_no,
  ...
  (select stk.sk1_item_no,
   ...
   where  stk.post_date       = g_date and
          ...
          --and di.department_no      in (16,34,42,52,62,85)
          and stk.sk1_item_no              = fix.sk1_item_no
          and stk.sk1_location_no          = fix.sk1_location_no)

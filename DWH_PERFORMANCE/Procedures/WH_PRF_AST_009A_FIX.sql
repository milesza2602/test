--------------------------------------------------------
--  DDL for Procedure WH_PRF_AST_009A_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_AST_009A_FIX" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        June 2012
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_AST_009A_FIX';
...
--
  Cursor C_fnd_ast_lev1_diff1 Is
   ...
   with style_list as (
        select a.style_colour_no
        from fnd_ast_lev1_diff1 a, dim_ast_lev1_diff1 b
        where a.style_colour_no = b.style_colour_no
         and  a.subclass_no = b.subclass_no
         and  a.class_no <> b.class_no
--         and  a.style_colour_no = 99990212628434
        order by a.style_colour_no
        )
    ...
    from    fnd_ast_lev1_diff1 ast,
            ...
            dim_calendar cal,
            style_list x
     where  ast.style_colour_no                      = x.style_colour_no

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_005U_SS_HSP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_005U_SS_HSP" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--**************************************************************************************************
--  Date:        July 2014
--  Author:      Wendy lyttle
...
--
--
--  Tables:      Input    - dwh_foundation.FND_S4S_EMP_JOB_SS_2019
...
BEGIN
  ...
  BEGIN
    ...
    and TABLE_NAME        = 'RTL_EMP_JOB_DY';

    l_text               := 'alter table dwh_performance.RTL_EMP_JOB_DY drop constraint PK_P_RTL_EMP_JB_DY_SS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_performance.RTL_EMP_JOB_DY drop constraint PK_P_RTL_EMP_JB_DY_SS');
    COMMIT;
...
BEGIN
  ...
  l_text := 'Insert into RTL_EMP_JOB_DY';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

          insert
            /*+ APPEND parallel(X,8) */
          into dwh_performance.RTL_EMP_JOB_DY X
            WITH
            SELEXT1 as (
        ...
        L_TEXT := 'RTL_EMP_JOB_DY : recs = '||g_recs;
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
BEGIN
      ...
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I30_RTL_EMP_JB_DY_SS ON DWH_PERFORMANCE.RTL_EMP_JOB_DY (LAST_UPDATED_DATE)     
      ...
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('CREATE INDEX DWH_PERFORMANCE.I40_RTL_EMP_JB_DY_SS ON DWH_PERFORMANCE.RTL_EMP_JOB_DY (SK1_EMPLOYEE_ID, TRAN_DATE)     
      TABLESPACE PRF_MASTER NOLOGGING  PARALLEL');
...
--  BEGIN
--
--    INSERT /*+ APPEND parallel(X,8) */
--    into dwh_performance.PRF_S4S_EMPJOB_DEL_LIST_SS X
...
--                    LAST_UPDATED_DATE
--                  from dwh_performance.RTL_EMP_JOB_DY a,
...
--                                    from dwh_foundation.FND_S4S_EMP_JOB_SS_2019 a
...
--      EXCEPTION
--      WHEN no_data_found THEN
--        l_text := 'No deletions done for DWH_PERFORMANCE.RTL_EMP_JOB_DY ';
--        dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
--      END;
--
--      g_recs_inserted :=0;
...
BEGIN
    ...
    DBMS_STATS.gather_table_stats ('DWH_PERFORMANCE', 'RTL_EMP_JOB_DY', DEGREE => 8);
      ...
      l_text := 'alter table dwh_performance.RTL_EMP_JOB_DY add constraint PK_P_RTL_EMP_JB_DY_SS';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('alter table dwh_performance.RTL_EMP_JOB_DY add CONSTRAINT PK_P_RTL_EMP_JB_DY_SS PRIMARY KEY (SK1_JOB_ID, SK1_EMPLOYEE_ID, TRAN_DATE)                    
...
BEGIN
  ...
  l_text := 'truncate table dwh_performance.RTL_EMP_JOB_DY';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  execute immediate('truncate table dwh_performance.RTL_EMP_JOB_DY');

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_118U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_118U" (p_forall_limit in integer,p_success out boolean) as
...
cursor c_fnd_rtl_allocation is
   select /*+ FULL(aloc) full(fnd_li) parallel(aloc,4) parallel(fnd_li,4) */
          ...
          max(nvl(ALOC.reg_rsp_excl_vat,0)) reg_rsp_excl_vat,
          max(nvl(wac,0)) wac,

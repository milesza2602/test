--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_700U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_700U_OLD" (
...
--------------------------------NEW VERSION--------------------------------------------------------------------
    ...
    type tbl_array_i
    IS
      TABLE OF TEMP_LOC_ITM_WK_RDF_SYSFCST_L1%rowtype INDEX BY binary_integer;
    type tbl_array_u
    IS
      TABLE OF TEMP_LOC_ITM_WK_RDF_SYSFCST_L1%rowtype INDEX BY binary_integer;
    ...
    BEGIN
      ...
      l_text := 'LOAD TEMP_LOC_ITM_WK_RDF_SYSFCST_L1 EX FOUNDATION STARTED '|| TO_CHAR(sysdate,('dd mon yyyy hh24:mi:ss'));
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      ...
      --**************************************************************************************************
      MERGE /*+ parallel (rdf_mart,8) */ INTO dwh_performance.RTL_LOC_ITEM_RDF_WKFCST_L1 rdf_mart USING (
      
        SELECT
        
        /*+ PARALLEL(TMP,8) FULL(TMP)) */
          DL.SK1_LOCATION_NO,
          DI.SK1_ITEM_NO ,
          TMP.FIN_YEAR_NO ,
          TMP.FIN_WEEK_NO ,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_110U3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_110U3" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_110U3';
...
cursor c_fnd_rtl_loc_item_dy_rms_sale is
 select /*+ full(di) full(dl) full(dlh) full(dih) parallel(fnd_li,6)  */  
   ...
   where  fnd_lid.last_updated_date = g_date 
          and fnd_lid.location_no BETWEEN G_FROM_LOC_NO AND G_TO_LOC_NO
...
begin
    forall i in a_tbl_merge.first .. a_tbl_merge.last
       save exceptions
MERGE /*+ USE_HASH(rtl_lidrd ,mer_rlidrd) PARALLEL(rtl_lidrd,4) */ INTO rtl_loc_item_dy_rms_dense rtl_lidrd
USING
(
...
select sum(sales) 
into g_prf_sale
from rtl_loc_item_dy_rms_dense A, DIM_LOCATION B
where post_date = g_date
AND A.SK1_LOCATION_NO = B.SK1_LOCATION_NO
AND Location_no BETWEEN G_FROM_LOC_NO AND G_TO_LOC_NO;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_669B_VATFIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_669B_VATFIX" 
                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        April 2015
--  Author:      Alfonso Joshua
--  Purpose:     Rollup from rtl_supchain_loc_item_dy_dj to rtl_supchn_loc_item_dy_sup_dj.
--               Difference between rtl_supchain_loc_item_dy_dj and rtl_supchn_loc_item_dy_sup_dj -
--               The Supplier on rtl_supchain_loc_item_dy_dj (wh_prf_corp_669u)
--               is the supplier as on the purchase order record.
--               The Supplier on rtl_supchn_loc_item_dy_sup_dj (this procedure)
--               is the primary supplier for the item, from dim_item.
--  Tables:      Input  - rtl_supchain_loc_item_dy_dj
--               Output - rtl_supchn_loc_item_dy_sup_dj
...
g_rec_out            rtl_supchn_loc_item_dy_sup_dj%rowtype;
g_found              boolean;
g_date               date;
g_count              number        :=  0;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_669B_VATFIX';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP SUPCHAIN_LOC_ITEM_DY TO SUPCHAIN_LOC_ITEM_DY_PRSUP';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of rtl_supchn_loc_item_dy_sup_dj%rowtype index by binary_integer;
type tbl_array_u is table of rtl_supchn_loc_item_dy_sup_dj%rowtype index by binary_integer;
...
cursor c_spch_loc_item_dy_prsup is
   with chgrecs as (select   tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no, sk1_chain_code_ind
                    from     rtl_supchain_loc_item_dy_dj
                    where    last_updated_date = g_date
                    group by tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no, sk1_chain_code_ind
                    union
                    select   tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no, sk1_chain_code_ind
                    from     temp_po_deletes_dj
                    group by tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no, sk1_chain_code_ind)
   select   lid.tran_date,
            lid.sk1_location_no,
            lid.sk1_item_no,
            lid.sk1_supply_chain_no,
            lid.sk1_chain_code_ind,
            max(i.sk1_supplier_no) sk1_supplier_no,
            ...
            sum(lid.case_cost) case_cost,
            g_date last_updated_date
   from     chgrecs cr
   join     rtl_supchain_loc_item_dy_dj lid     on  lid.tran_date           = cr.tran_date
...
-- we think it was added to prevent loading into partitions which did not exist
   join     dim_item i                          on  lid.sk1_item_no         = i.sk1_item_no
   group by lid.tran_date,
            lid.sk1_location_no,
            lid.sk1_item_no,
            lid.sk1_supply_chain_no,
            lid.sk1_chain_code_ind;
...
g_rec_in             c_spch_loc_item_dy_prsup%rowtype;

-- Input bulk collect table declared
type stg_array is table of c_spch_loc_item_dy_prsup%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into rtl_supchn_loc_item_dy_sup_dj values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update rtl_supchn_loc_item_dy_sup_dj
...
-- Check to see if record is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   rtl_supchn_loc_item_dy_sup_dj
...
-- Records on rtl_supchn_loc_item_dy_sup_dj are a summary of rtl_supchain_loc_item_dy_dj,
-- which is a summary of rtl_po_supchain_loc_item_dy.
-- Summary of rtl_supchn_loc_item_dy_sup_dj per tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no.
-- Therefore, when the not_before_date of a PO changes, it moves from one tran_date to another.
-- Records on rtl_supchn_loc_item_dy_sup_dj which belongs to the
-- tran_date, sk1_location_no, sk1_item_no, sk1_supply_chain_no combination of
-- any of the records which have been deleted from rtl_po_supchain_loc_item_dy, have to be re-summarised.
-- Therefore, the summarised records are first deleted from rtl_supchn_loc_item_dy_sup_dj.
--**************************************************************************************************
procedure delete_recs as
begin

    delete from rtl_supchn_loc_item_dy_sup_dj r
    where  exists(select d.tran_date, d.sk1_location_no, d.sk1_item_no, d.sk1_supply_chain_no, d.sk1_chain_code_ind
...
begin
    ...
    l_text := 'ROLLUP OF RTL_SUPCHAIN_LOC_ITEM_DY_DJ EX SUPCHAIN_LOC_ITEM_DY STARTED '||
...
--**************************************************************************************************
 --   delete_recs;
    ...
    open c_spch_loc_item_dy_prsup;
    fetch c_spch_loc_item_dy_prsup bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_spch_loc_item_dy_prsup;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_127U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_127U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - fnd_rtl_loc_item_dy_wwo_sale
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_127U';
...
cursor c_fnd_rtl_loc_item_dy_wwo_sale is
    with wwo as
   (select  location_no,
            item_no,
            post_date,
            sum(online_total_sales_qty) online_total_sales_qty,
            sum(online_total_sales) online_total_sales

   from
   ( select location_no,
            dwh_performance.dwh_lookup.item_convert(itb.item_no) as item_no,
            post_date,
            online_total_sales_qty,
            online_total_sales

       from  fnd_rtl_loc_item_dy_wwo_sale   itb
   ...
   select wwo.location_no,
          ...
          dd.jv_dept_ind,dd.packaging_dept_ind,
          dl.sk1_location_no,
          dih.sk2_item_no,
          dlh.sk2_location_no
   from   wwo,
   ...
   where  wwo.item_no               = di.item_no  and
          di.department_no          = dd.department_no and
          wwo.location_no           = dl.location_no   and
          wwo.item_no               = dih.item_no and
          wwo.post_date              between dih.sk2_active_from_date and dih.sk2_active_to_date and
          wwo.location_no            = dlh.location_no and
          wwo.post_date              between dlh.sk2_active_from_date and dlh.sk2_active_to_date
          ;
...
type stg_array is table of c_fnd_rtl_loc_item_dy_wwo_sale%rowtype;
...
--**************************************************************************************************
    ...
    fetch c_fnd_rtl_loc_item_dy_wwo_sale bulk collect into a_stg_input limit g_forall_limit;

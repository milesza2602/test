--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_280U_WL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_280U_WL" (
 ...
 ----- END -- this is for testing purposes
  ...
  --**************************************************************************************************
    ...
    END;
        ...
        from dba_indexes
        where OWNER = 'DWH_PERFORMANCE'
        AND index_name = 'I40_P_TMP_MRT_6WKAVG_PRM_DTS';
              execute immediate('DROP index dwh_performance.I40_P_TMP_MRT_6WKAVG_PRM_DTS');
    exception
         when no_data_found then
          commit;
    END;
        ...
        from dba_indexes
        where OWNER = 'DWH_PERFORMANCE'
        AND index_name = 'I50_P_TMP_MRT_6WKAVG_PRM_DTS';
              execute immediate('DROP index dwh_performance.I50_P_TMP_MRT_6WKAVG_PRM_DTS');
    exception
         when no_data_found then
          commit;
    END; 
  ...
  --**************************************************************************************************
    G_RECS := 0;
     ...
     INSERT /*+ APPEND  */ INTO dwh_performance.temp_mart_6wkavg_prom_dates aps
                ...
                selitm as
                           (SELECT distinct sk1_item_no, di.item_no
                ...
                selcat as (
                          ...
                          ,
                selprm as (
                select   /*+ paralel(dpia,2) parallel(fpl,2) parallel(dp,2) */
                                          distinct sct.FIN_YEAR_NO,sct.FIN_WEEK_NO  ,sct.SK1_ITEM_NO
                                        ,sct.SK1_LOCATION_NO  
                                        , 99 sk1_prom_no
                            from selcat sct
                            join dwh_performance.DIM_CALENDAR_WL dc
    ...
    l_text := 'temp_mart_6wkavg_PROM_DATES INSERT  ** g_recs='||g_recs;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     execute immediate('create index dwh_performance.i40_p_tmp_mrt_6wkavg_PRM_DTS on dwh_performance.temp_mart_6wkavg_prom_dates
                             (sk1_prom_no) TABLESPACE PRF_MASTER') ;
    l_text := 'create index dwh_performance.i40_p_tmp_mrt_6wkavg_PRM_DTS';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_RDF_001B
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RDF_001B" 
...
--               Output - temp_loc_item_dy_rdf_appfcst
...
g_rec_out            temp_loc_item_dy_rdf_appfcst%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RDF_001B';
...
l_description        sys_dwh_log_summary.log_description%type  := 'TEMP LOAD RDF DAILY APP FCST FACTS EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of temp_loc_item_dy_rdf_appfcst%rowtype index by binary_integer;
type tbl_array_u is table of temp_loc_item_dy_rdf_appfcst%rowtype index by binary_integer;
...
cursor c_fnd_rtl_loc_item_wk_dyfcst is
        ...
        select location_no,
                item_no,
                post_date,
               (post_date + to_number(substr(appcol,4,2)) -1) post_date_calc,
                appcol,
                appdata
         from   fnd_rtl_loc_item_wk_rdf_dyfcst
             unpivot include nulls (appdata for appcol in (dy_01_app_fcst_qty,
                                                           dy_02_app_fcst_qty,
                                                           dy_03_app_fcst_qty,
                                                           dy_04_app_fcst_qty,
                                                           dy_05_app_fcst_qty,
         ...
         where  last_updated_date             = g_date and
                to_number(substr(appcol,4,2)) > (select today_fin_day_no - 1 from dim_control)
...
begin
   ...
   g_rec_out.sales_dly_app_fcst_qty          := g_rec_in.appdata;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into temp_loc_item_dy_rdf_appfcst values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update temp_loc_item_dy_rdf_appfcst
...
-- Check to see if item is present on table and update/insert accordingly
   if g_fnd_count > 0 then  /* check done to determine multiple staging loads into foundation - TD-1143 */
      ...
      select count(1)
      into   g_count
      from   temp_loc_item_dy_rdf_appfcst
...
begin
    ...
    l_text := 'LOAD TEMP_LOC_ITEM_DY_RDF_APPFCST EX FOUNDATION STARTED '||
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table dwh_performance.temp_loc_item_dy_rdf_appfcst';
    l_text := 'TABLE temp_loc_item_dy_rdf_appfcst TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    prior to data population (temp_loc_item_dy_rdf_appfcst)
*/
    select count(*)
    into g_fnd_count
    from

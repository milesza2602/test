--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_746J
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_746J" (p_forall_limit in integer,p_success out boolean) as
...
-- Altered hints on cursor c_fnd_zone_item_supp_om_po_pl
-- **************************************************************************************************
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_746J';
...
cursor c_fnd_zone_item_supp_om_po_pl is
  select /*+ full(prc) full(di) full(dih) full(zi) full(fi_vr) */

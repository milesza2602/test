--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_660U_VATWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_660U_VATWL" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:         February 2009
--  Author:       M Munnik
...
g_start_date         date;
g_end_date           date;
g_rec_out            TEMP_PO_LIST_DJ_VATWL%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_DJ_660U_VATWL';
...
type tbl_array_i is table of TEMP_PO_LIST_DJ_VATWL%rowtype index by binary_integer;
...
begin
    ...
    l_text := 'LOAD OF TEMP_PO_LIST_DJ_VATWL STARTED '||
...
*/
    ...
    execute immediate 'truncate table dwh_performance.TEMP_PO_LIST_DJ_VATWL';
    l_text := 'TABLE TEMP_PO_LIST_DJ_VATWL TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    g_param_ind := 1;
     insert /*+ append */ into dwh_performance.TEMP_PO_LIST_DJ_VATWL
 with po_suppl_chain as
  ...
  l_text := 'end insert';
    ...
    l_text := 'TABLE TEMP_PO_LIST_DJ_VATWL STATISTICS UPDATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    ww_dbms_stats.gather_table_stats('dwh_performance','TEMP_PO_LIST_DJ_VATWL');
...
cursor c_TEMP_PO_LIST_DJ_VATWL is
   with po_suppl_chain as
  (select po_no, max(supply_chain_type) supply_chain_type
   from   fnd_rtl_purchase_order
   where  not_before_date > '24 Jun 2007'
...
cursor c_TEMP_PO_LIST_DJ_VATWL_hist is
   with po_suppl_chain as
  (select po_no, max(supply_chain_type) supply_chain_type
   from   fnd_rtl_purchase_order
   where  not_before_date > '24 Jun 2007'
...
cursor c_TEMP_PO_LIST_DJ_VATWL_old_hist is
   with non_dupl_po as
...
cursor c_TEMP_PO_LIST_DJ_VATWL_ship is
   with non_dupl_po as
  (select   po_no, max(reccnt) from
  (select   po_no, item_no, count(*) reccnt
   from     fnd_rtl_purchase_order
...
g_rec_in             c_TEMP_PO_LIST_DJ_VATWL%rowtype;
-- For input bulk collect --
type stg_array is table of c_TEMP_PO_LIST_DJ_VATWL%rowtype;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into TEMP_PO_LIST_DJ_VATWL values a_tbl_insert(i);
...
--**************************************************************************************************
    case g_param_ind
       when 1 then
          open c_TEMP_PO_LIST_DJ_VATWL;
          fetch c_TEMP_PO_LIST_DJ_VATWL bulk collect into a_stg_input limit g_forall_limit;
       when 2 then
          open c_TEMP_PO_LIST_DJ_VATWL_hist;
          fetch c_TEMP_PO_LIST_DJ_VATWL_hist bulk collect into a_stg_input limit g_forall_limit;
       when 3 then
          open c_TEMP_PO_LIST_DJ_VATWL_ship;
          fetch c_TEMP_PO_LIST_DJ_VATWL_ship bulk collect into a_stg_input limit g_forall_limit;
       when 4 then
          open c_TEMP_PO_LIST_DJ_VATWL_old_hist;
          fetch c_TEMP_PO_LIST_DJ_VATWL_old_hist bulk collect into a_stg_input limit g_forall_limit;
    end case;
    ...
    case g_param_ind
       when 1 then
          close c_TEMP_PO_LIST_DJ_VATWL;
       when 2 then
          close c_TEMP_PO_LIST_DJ_VATWL_hist;
       when 3 then
          close c_TEMP_PO_LIST_DJ_VATWL_ship;
       when 4 then
          close c_TEMP_PO_LIST_DJ_VATWL_old_hist;
    end case;

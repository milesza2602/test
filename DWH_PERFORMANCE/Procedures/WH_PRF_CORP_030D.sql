--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_030D
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_030D" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - fnd_subgroup, fnd_group
--               Output - dim_subgroup
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  23 April 2009 - defect 1357 - Change total_descr from plural to singular
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            dim_subgroup%rowtype;
g_rec_in             fnd_subgroup%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_030D';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE DIM_SUBGROUP EX FND_SUBGROUP';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_subgroup%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_subgroup%rowtype index by binary_integer;
type tbl_array_u is table of dim_subgroup%rowtype index by binary_integer;
...
begin
   ...
   dwh_lookup.dim_group_hierachy(g_rec_out.group_no,g_rec_out.group_name,g_rec_out.sk1_group_no,
...
---------------------------------------------------------
   ...
   g_rec_out.total_desc                 := 'ALL SUBGROUP';
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_subgroup values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_subgroup
      set    subgroup_name                   = a_tbl_update(i).subgroup_name,
...
begin
   g_found := dwh_valid.dim_subgroup(g_rec_out.subgroup_no);


-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_subgroup_no  := merch_hierachy_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF DIM_SUBGROUP EX FND_SUBGROUP STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_subgroup;
    fetch c_fnd_subgroup bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_S4S_014C_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_S4S_014C_SS" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        10 July 2014
--  Author:      Lwazi Ntloko
--  Purpose:     Load LabourRole heirachy data to DIM_JOBGROUP from STG Heirachy'.
--
--  DIM Tables:  Input  - FND_S4S_JOBGROUP_SS
--               Output - RTL_JOBGROUP_SS
...
g_rec_out            RTL_JOBGROUP_SS%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_S4S_014C_SS';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD jOBgroup dimenstion Data';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For output arrays into bulk load forall statements --
type tbl_array_i is table of RTL_JOBGROUP_SS%rowtype index by binary_integer;
type tbl_array_u is table of RTL_JOBGROUP_SS%rowtype index by binary_integer;
...
SELECT 
    ...
    WG.S4S_BUSINESS_UNIT_NAME,
    JG.EFFECTIVE_FROM_DATE,
    JG.EFFECTIVE_TO_DATE
FROM FND_S4S_JOBGROUP_SS JG,
     RTL_WORKGROUP_SS WG
WHERE JG.S4S_BUSINESS_UNIT_NO   = WG.S4S_BUSINESS_UNIT_NO
  AND JG.S4S_BUSINESS_UNIT_NAME = WG.S4S_BUSINESS_UNIT_NAME
...
begin
      ...
      g_rec_out.TOTAL_DESC               := 'ALL JOBGROUP ID';
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into RTL_JOBGROUP_SS values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).S4S_BUSINESS_UNIT_NAME||
                       ' '||a_tbl_insert(g_error_index).SK1_EFFECTIVE_FROM_DATE||
                       ' '||a_tbl_insert(g_error_index).SK1_EFFECTIVE_TO_DATE||
                       ' '||'INS';
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
...
--dbms_output.put_line(g_count||':'||g_rec_out.JOBGROUP_NAME);
...
-- Place data into and array for later writing to table in bulk
   if not g_found then
	  g_rec_out.SK1_JOBGROUP_ID := LABOUR_HIERACHY_SEQ.nextval;
...
begin
  if p_forall_limit is not null and p_forall_limit > 1000  then ------dwh_constants.vc_forall_minimum
    ...
    l_text := 'LOADING PRF jOBgROUP data'|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

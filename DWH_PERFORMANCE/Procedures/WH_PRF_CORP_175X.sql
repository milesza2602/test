--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_175X
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_175X" (p_forall_limit in integer,p_success out boolean) as
...
g_wac                fnd_rtl_loc_item_dy_rms_wac.wac%type;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_175U';
...
--         AND prc.last_updated_date  between '10 jun 2009' and '24 jun 2009' 
         AND PRC.WAC IS NULL;
...
SELECT wac into g_wac FROM
( SELECT wac FROM fnd_rtl_loc_item_dy_rms_wac
  where item_no     = g_rec_in.item_no and
        location_no = g_rec_in.location_no and
        tran_date   between g_date_yr and g_rec_in.calendar_date and
        rownum               = 1 
...
--execute immediate 'alter session set sort_area_size=250000000';

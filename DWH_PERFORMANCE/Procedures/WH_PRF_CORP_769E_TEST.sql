...
g_rec_out            w6005682.rtl_sms_publish_supp_q%rowtype;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_769E_TEST';
...
p_cell_no            w6005682.fnd_sp_sms_recipients_q.cell_no%type;
p_supplier_no        dwh_foundation.fnd_supplier.supplier_no%type;
--p_supplier_no        number := 11105;
g_control_no         w6005682.rtl_sms_publish_supp_q.control_no%type;


cursor c_supplier is
   select supplier_no, cell_no from w6005682.fnd_sp_sms_recipients_q where active_ind = 'Y' order by supplier_no;
...
--         insert into rtl_sms_publish_supp_q values g_rec_out;
--         g_recs_inserted  := g_recs_inserted + sql%rowcount;
--      end if;
--   else
      insert into w6005682.rtl_sms_publish_supp_q values g_rec_out;
      g_recs_inserted  := g_recs_inserted + sql%rowcount;
--   end if;
...
--   g_l_dc_date :=  to_char(trunc(l_dc_date),'dd/mm/yyyy');
   ...
   update      w6005682.rtl_sms_publish_supp_q
   set         processed_ind = 1
   ...
   select nvl(max(control_no),0) into g_control_no from w6005682.rtl_sms_publish_supp_q;

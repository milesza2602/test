--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_129T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_129T" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_129T';
...
merge into dwh_datafix.aj_rtl_loc_item_dy_om_ord rtl_lidoo USING
...
--               ord.cust_order_cases * nvl(prc.case_cost,0) AS cust_order_cost,
  ...
  Custorders as
    (
       select /*+ parallel(co,4) parallel (prc,4) parallel (ffd_ord,4) full (di), full (dl), full (dih) */
              co.LOCATION_NO,
              co.ITEM_NO,
              co.POST_DATE,

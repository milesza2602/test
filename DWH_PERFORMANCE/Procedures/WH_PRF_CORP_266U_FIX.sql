--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_266U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_266U_FIX" (p_forall_limit in integer, p_success out boolean)
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_266U_FIX';
...
--WHERE a.Sk1_Item_No < 10511654 and A.Sk1_Item_No    = B.Sk1_Item_No
                ...
                FROM  Fnd_4f_Loc_Dy_Stocktake A,
                      ...
                      item_too c
                WHERE a.cycle_no       = 34 -- (SELECT MAX(CYCLE_NO) FROM Fnd_4f_Loc_Dy_Stocktake)  ******34 ADDED *******
                AND   A.Stocktake_Date = B.Stocktake_Date
                and   A.Location_No    = B.Location_No
                
                and   B.Item_No        = C.Item_No 
                  --And B.Location_No = 112
                  --And C.Post_Date Between Sysdate-90 And Sysdate
                ),

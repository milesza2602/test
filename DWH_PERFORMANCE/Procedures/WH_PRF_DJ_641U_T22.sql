--------------------------------------------------------
--  DDL for Procedure WH_PRF_DJ_641U_T22
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_DJ_641U_T22" 
  (
...
as
  ...
  --                           dim_item
  --                           Dim_calendar
  --                           temp_fnd_boc_contractlistdjT22
  --               Output -    temp_fnd_boc_contractwkdj_T22
  --  Packages:    Constants, Dwh_Log, Dwh_Valid
  --  Comments:    Single Dml Could Be Considered For This Program.
  --
  --  Maintenance:
  --  20 Jan 2008   - defect 351  - Change to temp_fnd_boc_contractwkdj_T22 structure
  ...
  --
  --  PROCESSING FLOW
  --   1. Clear temp_fnd_boc_contractwkdj_T22
  --   2. Select contracts from temp_fnd_boc_contractwkdj_T22
  --  With
  ...
  -- Insert into temp_fnd_boc_contractlistdjT22
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_PRF_DJ_641U_T22';
  ----
...
--------------
 ...
 --                 sum(contract_qty * di.base_rsp_excl_vat) contract_selling,
             ...
             from dwh_foundation.fnd_rtl_contract frc,
                  dwh_performance.temp_fnd_boc_contractlistdjT22 tf,
...
--    tf.sk2_item_no                     ,
    ...
    tf.fin_week_code                   ,
    tf.contract_status_code
     from  dwh_performance.temp_fnd_boc_contractlistdjT22 tf
 group by tf.sk1_contract_no               ,
    tf.contract_no               ,
    tf.item_no                         ,
    tf.sk1_item_no                     ,
...
g_rec_out    dwh_performance.temp_fnd_boc_contractwkdj_T22%rowtype;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of temp_fnd_boc_contractwkdj_T22%rowtype index by binary_integer;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into  dwh_performance.temp_fnd_boc_contractwkdj_T22 values a_tbl_insert(i);
...
begin
  ...
  l_text := 'LOAD OF temp_fnd_boc_contractwkdj_T22 EX fnd_rtl_contract AND RTL_PO_SUPCHAIN_LOC_ITEM_DY_DJ STARTED '|| to_char ( sysdate,('dd mon yyyy hh24:mi:ss') ) ;
  dwh_log.write_log ( l_name,l_system_name,l_script_name,l_procedure_name,l_text ) ;
  ...
  dwh_log.write_log ( l_name,l_system_name,l_script_name,l_procedure_name,l_text ) ;
  --**************************************************************************************************
  -- TRUNCATE temp_fnd_boc_contractlistdjT22
  -- DWH_LOG.WRITE_LOG   : temp_fnd_boc_contractlistdjT22 truncated
  --**************************************************************************************************
  execute immediate('truncate table  temp_fnd_boc_contractwkdj_T22');
  commit;
  l_text :='TRUNCATED temp_fnd_boc_contractwkdj_T22 AT '||to_char ( sysdate,('dd mon yyyy hh24:mi:ss') ) ;
  dwh_log.write_log ( l_name,l_system_name,l_script_name,l_procedure_name,l_text ) ;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_RP_006A
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_RP_006A" (p_forall_limit in integer,p_success out boolean) as
...
g_rec_out            temp_shipment_ch_avail%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_RP_006A';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD TEMP SHIPMENT FOR CH FAST AVAIL CATALOG EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For output arrays into bulk load forall statements --
type tbl_array_i is table of temp_shipment_ch_avail%rowtype index by binary_integer;
type tbl_array_u is table of temp_shipment_ch_avail%rowtype index by binary_integer;
...
cursor c_fnd_rtl_shipment is
  ...
  where  last_updated_date     = g_date and
         shipment_status_code  = 'R' and
         actl_rcpt_date  between (g_date - 13) and (g_date);
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into temp_shipment_ch_avail values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).actl_rcpt_date;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update temp_shipment_ch_avail
       set    active_from_date         = a_tbl_update(i).active_from_date,
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if  a_tbl_insert(i).sk1_item_no     = g_rec_out.sk1_item_no and
             a_tbl_insert(i).sk1_location_no = g_rec_out.sk1_location_no then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF TEMP_SHIPMENT_CH_AVAIL EX FOUNDATION STARTED AT '||
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table dwh_performance.temp_shipment_ch_avail';
    l_text := 'TABLE temp_shipment_ch_avail TRUNCATED.';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

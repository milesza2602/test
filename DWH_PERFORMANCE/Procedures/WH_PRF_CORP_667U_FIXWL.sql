--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_667U_FIXWL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_667U_FIXWL" 
                                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        March 2009
--  Author:      M Munnik
--  Purpose:     Rollup to rtl_contract_chain_item_wk_po.
--               Only for CHBD
--  Tables:      Input  - rtl_po_supchain_loc_item_dy
--               Output - rtl_contract_chain_item_wk_po
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_667U_FIXWL';
...
begin
    ...
    l_text := 'ROLLUP OF rtl_contract_chain_item_wk_po EX PO SUPCHAIN STARTED '||
...
--**************************************************************************************************
    ...
    execute immediate 'truncate table dwh_performance.rtl_contract_chain_item_wk_po';
    l_text := 'TABLE rtl_contract_chain_item_wk_po TRUNCATED - '||to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    insert /* append */ into rtl_contract_chain_item_wk_po

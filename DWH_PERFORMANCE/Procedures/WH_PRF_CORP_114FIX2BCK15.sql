--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_114FIX2BCK15
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_114FIX2BCK15" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_114FIX2BCK15';
...
cursor c_fnd_rtl_shipment is
   ...
   where  shp.last_updated_date  >= '4 MAY 2015' AND 
          ...
          shp.actl_rcpt_date     is not null 
           and (shp.from_loc_no in (207,400,305,4202,4201,4206)
           or shp.to_loc_no in (207,400,305,4202,4201,4206,212))

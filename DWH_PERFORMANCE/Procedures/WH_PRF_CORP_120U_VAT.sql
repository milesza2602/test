--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_120U_VAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_120U_VAT" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_120U_VAT';
...
type tbl_array_i is table of w6005682.rtl_loc_item_dy_rms_denseq%rowtype index by binary_integer;
type tbl_array_u is table of w6005682.rtl_loc_item_dy_rms_denseq%rowtype index by binary_integer;
 ...
 */  
   ...
   select sum(nvl(shp.received_qty,0)) as received_qty,
          ...
          sum(case when rli.tax_perc is null then
                ...
                nvl(shp.received_qty,0) * (shp.reg_rsp * 100 / (100 + rli.tax_perc  ))                                -- di.vat_rate_perc,
   ...
   from   fnd_rtl_shipment shp
          ...
          join dwh_performance.dim_location dl on lid_list.to_loc_no             = dl.location_no 
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into w6005682.rtl_loc_item_dy_rms_denseq values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update w6005682.rtl_loc_item_dy_rms_denseq
...
begin
    ...
    l_text := 'LOAD OF w6005682.rtl_loc_item_dy_rms_denseq EX FOUNDATION STARTED AT '||

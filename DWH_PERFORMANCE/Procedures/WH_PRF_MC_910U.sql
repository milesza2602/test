--------------------------------------------------------
--  DDL for Procedure WH_PRF_MC_910U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_MC_910U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2018
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_MC_910U';
...
--/*+ parallel(di,4) parallel(dih,4)  full(dl) full(dlh)  parallel(vr,4) parallel(fi_vr,4)  parallel(fnd_li,4) parallel(fnd_lid,4)*/
    ...
    select /*+ parallel(SPRS,4) */ *
       from   RTL_MC_LOC_ITEM_DY_RMS_SPARSE SPRS

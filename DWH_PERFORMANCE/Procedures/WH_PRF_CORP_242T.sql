--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_242T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_PERFORMANCE"."WH_PRF_CORP_242T" (p_forall_limit in integer, p_success out boolean) as
...
--               Output -   rtl_loc_item_dy_rate_of_sale    --w6005682
...
g_rec_out            rtl_loc_item_dy_rate_of_sale%rowtype;
g_count              number        :=  0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CORP_242T';
...
type tbl_array_i is table of rtl_loc_item_dy_rate_of_sale%rowtype index by binary_integer;
type tbl_array_u is table of rtl_loc_item_dy_rate_of_sale%rowtype index by binary_integer;
...
a_count_i           integer       := 0;
a_count_u           integer       := 0;
l_from_date         date:= trunc(sysdate) - 20;   --41
--in_batch            integer  :=1;
...
uda_3502_item_list as (
  ...
  from rtl_item_uda_dy uda_3502, item_list il
   where uda_3502.calendar_date between '04/MAR/13' and '10/MAR/13'   --l_from_date and g_date
...
from rtl_loc_item_dy_catalog cat,
     ...
     uda_item_list uda
where cat.calendar_date between '04/MAR/13' and '10/MAR/13'   --l_from_date and g_date
...
dense_stuff as (
select /*+ PARALLEL(dense,4) FULL(dense) */
...
from rtl_loc_item_dy_rms_dense dense,
     ...
     uda_3502_item_list uda_3502,
     uda_item_list uda
where dense.post_date between '04/MAR/13' and '10/MAR/13'   --l_from_date and g_date
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into rtl_loc_item_dy_rate_of_sale values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update rtl_loc_item_dy_rate_of_sale
...
begin
    ...
    l_text := 'LOAD OF rtl_loc_item_dy_rate_of_sale for 6 weeks started AT '||
...
--**************************************************************************************************
    ...
    l_text := 'Date range being processed : ' || '04/MAR/13' || ' - ' || '10/MAR/13';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

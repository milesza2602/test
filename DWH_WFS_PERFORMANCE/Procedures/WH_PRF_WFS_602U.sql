--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_602U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_602U" (p_forall_limit in integer,p_success out boolean) as
...
--  2016-12-19 N Chauhan - fix for unique key violation.
--
--  Naming conventions
--  g_  -  Global variable
           ...
           FROM dwh_wfs_performance.wfs_open_application apps
                ...
                (SELECT /*+ full(cus) parallel(cus, 4) */ -- changed added full(cus)
                       APPLICATION_NO,
                        ...
                        DATE_FIRST_PURCH,
                        CASE
                             WHEN product_code_no IN 
                               (1, 2, 3, 7, 8, 9, 2, 4, 5, 6, 21) 
                             THEN
                                'StoreCard'
                             WHEN product_code_no IN (11, 15, 16)
                             THEN
                                'PersonalLoan'
                             ELSE
                                NULL
                        end as product_name
                   FROM dwh_cust_performance.dim_wfs_all_prod cus) all_prd
                   ON apps.app_number = all_prd.APPLICATION_NO and apps.product_name=all_prd.product_name),
     activations_all
     AS (SELECT /*+  full(open_app_cus) full(tran) parallel(t1,4) full(t1) */ -- changed added full(t1)
               oa_cus.app_number,
                t1.product_code_no,
                CASE
                   WHEN t1.product_code_no IN 
                        (1, 2, 3, 7, 8, 9, 2, 4, 5, 6, 21) 
                   THEN
                      'StoreCard'

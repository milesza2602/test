--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_610U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_610U" (p_forall_limit in integer,p_success out boolean) as
...
--  2017-08-15 S Ismail - Schedule job run based on APEX_WFS_STMT_ABSA_BILL_CYC.
...
--**************************************************************************************************   
  ...
  select LAST_BILLING_DATE+1  INTO g_run_date
    from APEX_APP_WFS_01.APEX_WFS_STMT_ABSA_BILL_CYC
	where CAL_YEAR_MONTH_NO = TO_CHAR (sysdate,'YYYYMM');
    ...
    l_text := 'LOAD TABLE: '||'WFS_STMT_CR_CRD' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

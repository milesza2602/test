--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_690U_20181018
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_690U_20181018" (p_forall_limit in integer,p_success out boolean)
as
  --**************************************************************************************************
...
begin
  ...
  SELECT wfs_year_week,
    ...
    dea_ind,
    latest_dea_date,
    first_wf_poi_date_success,
    first_wf_poi_date_unsuccessful,
    first_wf_poi_date,
    first_fica_pending_date_wf,
    first_fica_successful_date_wf,
    latest_poi_decline_date_wf,
    poi_expiry_date_wf,
    booked_account_date,
    risk_category,
    solicitation,
    ...
    product_activated_same_day,
    both_selected_and_accepted
  FROM
    (SELECT wfs_year_week,      --        AS Fin Week,
      wfs_year,                 --        AS Fin Year,
      wfs_year_quarter,         --        AS Fin Quater,
      wfs_year_month,           --        AS Fin Month,
      entered_time_stamp,       --        AS Split App Date,
      app_date,                 --        AS App Date,
      master_app_number,        --        AS Master App No,
      split_app,                --        AS Split App,
      app_number,               --        AS App No,
      channel,                  --        AS Channel,
      app_channel,              --        AS App Channel,
      campaign_id,              --        AS Campaign Id,
      promotion_id,             --        AS Promotion Id,
      product_name,             --        AS Product,
      request_type,             --        AS Request Type,
      credit_application_status,--        AS App Status,
      product_purpose,          --        AS Product Status,
      insurance_type,           --        AS Insurance Type,
      poi_required,             --        AS POI Required,
      CASE
        WHEN app = 1
      ...
      pre_agreement_accept_date   AS paa_date,
      product_activated           AS activated,
      sa_id                       AS sa_id,
      guest_number                AS staff_number,
    ...
    FROM
      ...
      FROM
        (SELECT
          ...
          main_appl.pre_agreement_accept_agent AS paa_user,
          ...
          CASE
            ...
            ELSE 1
          END AS product_activated
        FROM dwh_wfs_performance.wfs_nbr_tmp_main_product_appl main_appl

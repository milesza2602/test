--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_180U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_180U" (p_forall_limit IN INTEGER,p_success OUT BOOLEAN) AS
...
--  2019-02-18 N Chauhan - Restrict update of customer_no and atg_cust_no if value is null
--  2019-03-18 N Chauhan - Snapshot check removed as snapshot table decommissioned.
--
--
   ...
   INTO dwh_wfs_performance.dim_oneapp_subscriber tgt USING 
     (
      ...
      SELECT  /*+ parallel(t,4) full(t) */ 
         ...
         T.oneapp_subscriber_key,
         T.oneapp_api_no,

         CASE      -- avoid update if null
           WHEN S.atg_customer_no IS NULL 
         ...
         CASE      -- avoid update if null
           WHEN S.customer_no IS NULL 
            AND T.customer_no IS NOT NULL THEN
             to_number(T.customer_no)
           ELSE
             S.customer_no 
         END 
         as customer_no,
         ...
         T.subscriber_email,
         T.email_address_verified,
         T.subscriber_first_name,
         T.subscriber_surname,
         T.first_subscr_login_date,
         T.last_subscr_login_date,

--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_652U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_652U" (p_forall_limit in integer,p_success out boolean) as
      ...
      from 
		...
		*/
          select 
            a.*
          from 
            dwh_cust_performance.wfs_all_prod_mnth a
          where 
  ...
  */
  wfs_portfolios as(
    ...
    (
      ...
      from 
        DWH_CUST_PERFORMANCE.DIM_CUSTOMER_PORTFOLIO cst_prt
      where
  ...
  LOYALTY_TIER as (
    ...
    from 
      DWH_CUST_PERFORMANCE.CUST_WOD_TIER_MTH_DETAIL LT
    where
...
--    execute immediate 'alter session enable parallel dml';
    ...
    g_date_to_do:= to_date(to_char(g_yr_mth_cmplt)||'01', 'YYYYMMDD');
    -- first day of last completed calendar month

    select max(CAL_YEAR_MONTH_NO) 
     into g_yr_mth_done
    from WFS_MART_SALES_CUST_ACC_MLY;
    if g_yr_mth_done >= g_yr_mth_cmplt then
       l_text      := 'Data already exists for YearMonth '||g_yr_mth_cmplt||
                      ' - load abandoned';
       dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--dbms_output.put_line( l_text );
       p_success := true;

--------------------------------------------------------
--  DDL for Procedure TMP_ONEAPP_ACTIVITY_TRL_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."TMP_ONEAPP_ACTIVITY_TRL_TAKEON" (p_forall_limit IN INTEGER,p_success OUT BOOLEAN) AS
-- renamed to TMP_ONEAPP_ACTIVITY_TRL_TAKEON in prod.
...
--  2018-04-16 N Chauhan - adapted for take-on into a temp table.  retain logging as will be run under dwh_datafix.
--                         add run control
--  2018-04-24 N Chauhan - ***** take-on into actual LIVE table  !!!!!!!!!!!!!
--                          Priv on PROC_RUN_CTL will be needed to be granted to DWH_DATAFIX
--
--
--
...
l_module_name        sys_dwh_errlog.log_procedure_name%TYPE    := 'TMP_ONEAPP_ACTIVITY_TRL_TAKEON';
...
select   -- stop processing if past timelimit or flagged to stop
    case 
    when sysdate > p.END_TIME 
      or p.RUN_IND <> 'Y' then
     'N'
    else
     'Y'
    ...
    as proceed_status
     into g_proceed
from w7071603.PROC_RUN_CTL p
where p.PROC_NAME = 'TMP_ONEAPP_ACTIVITY_TRL_TAKEON'
;
...
If g_proceed<> 'Y' then  -- aborted 
   l_text :=  'Subscribers load terminated';
...
END TMP_ONEAPP_ACTIVITY_TRL_TAKEON;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_682U_20190214
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_682U_20190214" (
...
AS
  ...
  --  2018-10-09 N Dlamini - Commented the final DEA_IND business rule and used the initial rule, Added Maturation columns & other NBR missing columns.
  --
  --  Naming conventions
  --  g_  -  Global variable
  ...
  l_description sys_dwh_log_summary.log_description%type   := 'WFS NBR Main Product Applications Test';
...
WITH fnd_wfs_tmp_master_application AS
  ...
  (SELECT
    ...
    CASE
      WHEN dea.latest_dea_date IS NULL
      THEN 'No'
      ELSE 'Yes'
    END       AS dea_ind,
  ...
  AND main_appl.product_name          = undecided.product_type
    --left outer join dwh_wfs_performance.wfs_tmp_customer_card_in_hand card -- Created a Temp table based on production view for testing by Nhlaka Dlamini 13/09/2018
    --on trim(main_appl.sa_id) = trim(card.customer_id_no)
  )
SELECT app_number,
  ma_entered_time_stamp,
  ma_app_number,

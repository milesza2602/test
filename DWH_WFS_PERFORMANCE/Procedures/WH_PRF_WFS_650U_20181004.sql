--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_650U_20181004
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_650U_20181004" (p_forall_limit in integer,p_success out boolean) as
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    where cnt > 1000;   -- faster than using "having in previous group query
...
--**************************************************************************************************
     ...
     execute immediate 
       ...
       ||'  ON "DWH_WFS_PERFORMANCE"."WFS_MART_SALES_BASKET_ITEM"'
       ||'  ("TRAN_NO")'
       ||'  NOlogging TABLESPACE "'||l_tablespace||'"    LOCAL PARALLEL (degree 16)';

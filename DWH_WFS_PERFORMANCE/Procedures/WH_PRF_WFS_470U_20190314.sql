--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_470U_20190314
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_470U_20190314" (
         ...
         correct_evaluations 
          ...
          all_evalution_scores 
           ...
           WHERE ev.call_id          IS NOT NULL  -- Fix to ignore null values, as this is a Key on the Perofmance table N. Dlamini 07/11/2018
             AND ev.contact_start_time >= p_day_to_do
             AND ev.contact_start_time  < p_day_to_do + 1
    ) new_rec 
    ON 
    ( 
      new_rec.evaluation_key = tgt.evaluation_key AND new_rec.call_id = tgt.call_id AND new_rec.contact_start_time = tgt.contact_start_time
    )

  WHEN MATCHED THEN

    UPDATE
    SET tgt.evaluation_date            = new_rec.evaluation_date,
...
--    g_recs_call_eval_performance := g_recs_call_eval_performance + SQL%rowcount;
--    COMMIT;
    g_success := TRUE;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_650U_FIX3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_650U_FIX3" 
...
--  2018-10-05 N Chauhan - remove filter on new captured TENDER_TYPE_DETAIL_CODE's 
--
--  2018-10-31 N Chauhan - "FIX" version of proc created to run for specific days passed as parameters.
--  2018-10-31 N Chauhan - "FIX2" version of proc created to run filter basket data on TRAN_DATE
--                          for basket/tender last_updated_date out-of-sync to due prod issues.
--  2018-11-01 N Chauhan - "FIX3" version of proc created to handle dups due to multiple custmer_nos per bsk
--
--
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_WFS_650U_FIX3';
   ...
   into DWH_WFS_PERFORMANCE.WFS_MART_SALES_BASKET_ITEM ti using (
      ...
      dly_bsk_tender_1 as (
         select /*+ parallel (cbk,4)   full(cbk)  */
                 ...
                 cbk.till_no,
                 cbk.tran_no,
-- get customer_no from tender instead of basket where there could be multiple                 
cbk.customer_no,                 
...
--          where bsk.last_updated_date = p_date_to_do  
          where bsk.tran_date = p_date_to_do  
...
--           and bsk.tran_date between sysdate-g_late_window and sysdate
      ...
      basket_items_summary as (
         select /*+ parallel(i,4) */
--            customer_no,
-- cust no now retrieved from tender
            tran_date,
            location_no,
            till_no,
            tran_no,
...
-- cust no now retrieved from tender
-- to fix for multiple cust no per basket

                     when b.customer_no is null then 99999999999999
                     else b.customer_no
             end as customer_no,
             a.item_tran_qty,
             a.item_tran_amt,
...
g_date_end:= p_date_end;
    ...
    loop
       ...
       l_text :=  'For tran_date '||to_char(g_date_to_do, 'YYYY-MM-DD')||'  Merged:  '||g_recs_cnt_day;
       dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_PRF_WFS_180U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_PERFORMANCE"."WH_PRF_WFS_180U_TAKEON" 
 (p_forall_limit in integer,p_success out boolean, 
   p_date_start IN DATE, p_date_end in Date default null ) as

--**************************************************************************************************
-- Description: OneApp usage and navigations - Load/update subscriber's details
--
-- TAKE-ON Version to insert snapshot into WFS_ONEAPP_SUBSCR_LOGIN_DLY each day too.
--
-- Date:        2017-12-21
...
--
--  2018-08-02 N Chauhan - Adapted for once-off mini take-on fix, keeping snapshots in sync.
--  2018-08-03 N Chauhan - Adapted for once-off mini take-on fix, keeping summary in sync.
--  2018-11-26 N Chauhan - Provision for start/end dates as parameters.
--  2018-12-05 N Chauhan - Fix getting prev snapshot date correctly.
--  2019-04-08 N Chauhan - Remove saving snapshots as table was decommissioned.
--  2019-04-12 N Chauhan - for uat test/prep load from fnd..@dwhprd.
--  2019-04-17 N Chauhan - remove @dwhprd for prod version.
--
--
--
--
...
l_module_name        sys_dwh_errlog.log_procedure_name%TYPE    := 'WH_PRF_WFS_180U_TAKEON';
l_name               sys_dwh_log.log_name%TYPE                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%TYPE          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%TYPE          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%TYPE       := l_module_name;
l_text               sys_dwh_log.log_text%TYPE ;
l_description        sys_dwh_log_summary.log_description%TYPE  := 'ONEAPP USAGE AND NAVIGATIONS - LOAD/UPDATE SUBSCRIBER DETAILS AND LOGIN SNAPSHOTS';
...
g_recs_snapshot_day NUMBER := 0;
g_recs_summary_day NUMBER := 0;
...
--          dwh_wfs_foundation.fnd_wfs_oneapp_api@dwhprd fnd
      WHERE 
...
PROCEDURE SAVE_SUMMARY(p_day_to_do DATE) AS
   ...
   (
      ...
      subscr_status AS (
         ...
         CASE WHEN customer_no IS NULL
          THEN 1 ELSE 0 END
         AS is_unlinked,
         CASE WHEN TRUNC(p_day_to_do)-TRUNC(last_subscr_login_date) < 45 
          THEN 1 ELSE 0 END
         AS is_active ,
         CASE WHEN TRUNC(p_day_to_do)-TRUNC(last_subscr_login_date) >= 45 
          THEN 1 ELSE 0 END
      ...
      SELECT  /*+ parallel(s,4) full(s) */ 
         TRUNC(p_day_to_do) AS subscr_summary_date ,
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ...
      ROLLBACK;
--      SYS.DBMS_OUTPUT.PUT_LINE(to_char(sysdate,'yyyy-mm-dd hh24:mi:ss  ')||'Error '||sqlcode||' '||sqlerrm );
      l_text :=  'ONEAPP - SAVE_SUMMARY sub proc fails';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      l_message := dwh_constants.vc_err_mm_other||SQLCODE||' '||sqlerrm;
      dwh_log.record_error(l_module_name,SQLCODE,l_message);
...
g_date_end:= nvl(p_date_end, p_date_start);
g_date_to_do := g_date_start;

    if g_date_start > g_date_end then
       p_success := true;
       l_text :=  dwh_constants.vc_log_ended ||' - no data to process - '||to_char(g_date_start,'YYYY-MM-DD')||' data already processed';
       dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   LOOP
      ...
      -- ****** update WFS_ONEAPP_SUBSCR_SUMMARY_DLY **********
      SAVE_SUMMARY(g_date_to_do);
      -- **********************************************

      g_recs_summary_day := SQL%rowcount;
      COMMIT;  


      l_text :=  'For day '||to_char(g_date_to_do, 'YYYY-MM-DD')||'  WFS_ONEAPP_SUBSCR_SUMMARY_DLY inserted:  '||g_recs_summary_day;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

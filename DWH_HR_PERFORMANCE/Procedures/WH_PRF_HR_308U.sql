--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_308U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_308U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_emp_management_cntl_dy fact table in the performance layer
--               with added value ex foundation layer fnd_hr_emp_management_cntl.
--  Tables:      Input  - fnd_hr_emp_management_cntl
--               Output - hr_emp_management_cntl_dy
...
--  Change     : Add sk1_bee_business_unit_code
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            hr_emp_management_cntl_dy%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_308U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_emp_management_cntl_dy EX EMP MASTER';
...
type tbl_array_i is table of hr_emp_management_cntl_dy%rowtype index by binary_integer;
type tbl_array_u is table of hr_emp_management_cntl_dy%rowtype index by binary_integer;
...
cursor c_fnd_hr_emp_management_cntl is
   select man.*,
          emp.sk1_employee_id,
          deh.sk2_employee_id,
          mc.sk1_management_control_code,
          dpt.sk1_bee_business_unit_code
   from   fnd_hr_emp_management_cntl man,
          dim_hr_employee emp,
          dim_hr_employee_hist deh,
          dim_hr_management_control mc,
          dim_hr_department dpt
   where  man.management_control_code  = mc.management_control_code and
          man.employee_id              = emp.employee_id  and
          man.employee_id              = deh.employee_id  and
          dpt.sk1_department_code  = emp.sk1_department_code and
          man.effective_date     between deh.sk2_active_from_date and deh.sk2_active_to_date and
          emp.employee_status_code     = 'A'  and
          emp.company_code             in ('WW','UPN','WHL') and
          man.last_updated_date        >  g_date-7 ;

g_rec_in                   c_fnd_hr_emp_management_cntl%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_hr_emp_management_cntl%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_emp_management_cntl_dy values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_emp_management_cntl_dy
      set    employee_count              = a_tbl_update(i).employee_count,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_emp_management_cntl_dy
...
begin
    ...
    l_text := 'LOAD OF hr_emp_management_cntl_dy EX fnd_hr_emp_management_cntl STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_emp_management_cntl;
    fetch c_fnd_hr_emp_management_cntl bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_hr_emp_management_cntl;
--**************************************************************************************************

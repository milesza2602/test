--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_356U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_356U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_skill_dev_spend_bu fact table in the performance layer
--               with added value ex foundation layer hr_bee_skill_dev_spend.
--  Tables:      Input  - hr_bee_skill_dev_spend
--               Output - hr_bee_skill_dev_spend_bu
--  Packages:    constants, dwh_log, dwh_valid
--
--      Date      : Jul 2014
--     Changed by : Kgomotso Lehabe
--     Purpose    : Add all the cost columns to the summary table
--                  and sk1_citizenship_status_code
--                  Bring in proper race codes instead of classiying A,C,I as W
--
--
--
--  Naming conventions:
...
g_rec_out            hr_bee_skill_dev_spend_bu%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_356U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_bee_skill_dev_spend_bu EX EMP MASTER';
...
type tbl_array_i is table of hr_bee_skill_dev_spend_bu%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_skill_dev_spend_bu%rowtype index by binary_integer;
...
cursor c_skill_dev_spend_sds is
   ...
   from   hr_bee_skill_dev_spend sds,
          dim_hr_department dd,
          dim_hr_employee de,
          dim_hr_course dc,
          dim_hr_bee_category cat
   where  sds.sk1_employee_id           = de.sk1_employee_id and
       ( de.department_code            = dd.department_code  and
         de.company_code  =              dd.company_code) and
...
g_rec_in                   c_skill_dev_spend_sds%rowtype;
-- For input bulk collect --
type stg_array is table of c_skill_dev_spend_sds%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_skill_dev_spend_bu values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_bee_skill_dev_spend_bu
      set    TOTAL_COST                  = a_tbl_update(i).TOTAL_COST,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_bee_skill_dev_spend_bu
...
begin
    ...
    l_text := 'LOAD OF hr_bee_skill_dev_spend_bu EX hr_bee_skill_dev_spend STARTED AT '||
...
--**************************************************************************************************
    open c_skill_dev_spend_sds;
    fetch c_skill_dev_spend_sds bulk collect into a_stg_input limit g_forall_limit;

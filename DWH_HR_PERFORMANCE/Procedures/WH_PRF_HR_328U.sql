--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_328U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_328U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_enterprise_dev_loan fact table in the performance layer
--               with added value ex foundation layer fnd_hr_bee_enterprise_dev_loan.
--  Tables:      Input  - fnd_hr_bee_enterprise_dev_loan
--               Output - hr_bee_enterprise_dev_loan
...
--  Change      :	Remove the evaluation on ED_Category  while deriving Outstanding_amount_total
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            hr_bee_enterprise_dev_loan%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_328U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE HR_BEE_ENTERPRISE_DEV_LOAN EX FOUNDATION';
...
type tbl_array_i is table of hr_bee_enterprise_dev_loan%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_enterprise_dev_loan%rowtype index by binary_integer;
...
cursor c_fnd_hr_bee_edl is
   select edl.*,
          ...
          debh.sk2_ed_beneficiary_id,
          deb.interest_fee_loan_ind,
          deb.std_loans_black_owned_ind,
          deb.standard_loan_other_ind,
          deb.ed_category
   from   fnd_hr_bee_enterprise_dev_loan  edl,
          dim_hr_bee_ed_beneficiary deb,
          dim_hr_bee_ed_beneficiary_hist debh
   where  edl.last_updated_date  > g_date - 70 and
          edl.account_no = deb.loan_account_no and
          edl.account_no = debh.loan_account_no and
          edl.gl_date    between debh.sk2_active_from_date and debh.sk2_active_to_date;
...
begin
   ...
   g_rec_out.loan_amount            := g_rec_in.loan_amount;
   g_rec_out.outstanding_amount     := g_rec_in.outstanding_amount;
   g_rec_out.gl_date                := g_rec_in.gl_date ;
   g_rec_out.CUSTOMER_ID            := g_rec_in.CUSTOMER_ID ;
   g_rec_out.ACCOUNT_ID             := g_rec_in.ACCOUNT_ID ;
   g_rec_out.ACCOUNT_NO             := g_rec_in.ACCOUNT_NO ;
   ...
   if g_rec_in.interest_fee_loan_ind = 1 then
      select sk1_benefit_factor_id
      into   g_rec_out.sk1_benefit_factor_id
      from   dim_hr_bee_benefit_factor
      where  benefit_factor_id = 6005;
   end if;
   if g_rec_in.std_loans_black_owned_ind = 1 then
      select sk1_benefit_factor_id ,ed_percentage
      into   g_rec_out.sk1_benefit_factor_id,g_ed_percentage
      from   dim_hr_bee_benefit_factor
      where  benefit_factor_id = 6006;
   end if;
   if g_rec_in.standard_loan_other_ind = 1 then
      select sk1_benefit_factor_id,ed_percentage
      into   g_rec_out.sk1_benefit_factor_id,g_ed_percentage
      from   dim_hr_bee_benefit_factor
      where  benefit_factor_id = 6007;
   end if;

      g_rec_out.outstanding_amount_total  := 0;

      g_rec_out.outstanding_amount_total  := (g_rec_out.outstanding_amount *  g_ed_percentage / 100) ;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_enterprise_dev_loan values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).customer_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_bee_enterprise_dev_loan
      set     CUSTOMER_ID              = a_tbl_update(i).CUSTOMER_ID ,
              ACCOUNT_ID               = a_tbl_update(i).ACCOUNT_ID  ,
              outstanding_amount       = a_tbl_update(i).outstanding_amount ,
              outstanding_amount_total = a_tbl_update(i).outstanding_amount_total ,
              loan_amount              = a_tbl_update(i).loan_amount ,
              account_no               = a_tbl_update(i).account_no ,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_bee_enterprise_dev_loan
...
begin
    ...
    l_text := 'LOAD OF hr_bee_enterprise_dev_loan EX fnd_hr_bee_enterprise_dev_loan STARTED AT '||

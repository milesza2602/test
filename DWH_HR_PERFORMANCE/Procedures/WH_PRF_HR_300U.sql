--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_300U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_300U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_employee_shares_mth fact table in the performance layer
--               with added value ex foundation layer fnd_hr_bee_share_ownship.
--  Tables:      Input  - fnd_hr_bee_share_ownship
--               Output - hr_bee_employee_shares_mth
...
--  Change      :	 Add sk1_bee_business_unit_code to hr_bee_employee_shares_mth
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            hr_bee_employee_shares_mth%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_300U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_bee_employee_shares_mth EX FOUNDATION';
...
type tbl_array_i is table of hr_bee_employee_shares_mth%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_employee_shares_mth%rowtype index by binary_integer;
...
cursor c_fnd_hr_bee_share_ownship is
   select sc.*,
          de.sk1_employee_id,
          deh.sk2_employee_id,
          dept.sk1_bee_business_unit_code
   from   fnd_hr_bee_share_ownship sc
          ...
          on   sc.employee_id              = deh.employee_id and
               sc.effective_date     between deh.sk2_active_from_date and deh.sk2_active_to_date
               and  deh.sk2_active_to_date ='01/JAN/3000'
          join dim_hr_department dept
          on dept.sk1_department_code = de.sk1_department_code
   where  de.employee_status_code = 'A' and
          sc.last_updated_date    > g_date-7  ;

g_rec_in                   c_fnd_hr_bee_share_ownship%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_hr_bee_share_ownship%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_employee_shares_mth values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_bee_employee_shares_mth
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_bee_employee_shares_mth
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).sk1_employee_id     = g_rec_out.sk1_employee_id and
            a_tbl_insert(i).this_mn_end_date    = g_rec_out.this_mn_end_date  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF hr_bee_employee_shares_mth EX fnd_hr_bee_share_ownship STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_bee_share_ownship;
    fetch c_fnd_hr_bee_share_ownship bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_hr_bee_share_ownship;
--**************************************************************************************************

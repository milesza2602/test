--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_100U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_100U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create dim_hr_gender dimention table in the performance layer
--               with added value ex foundation layer fnd_hr_gender.
--  Tables:      Input  - fnd_hr_gender
--               Output - dim_hr_gender
...
g_rec_out            dim_hr_gender%rowtype;
g_rec_in             fnd_hr_gender%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_100U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE dim_hr_gender EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_hr_gender%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_hr_gender%rowtype index by binary_integer;
type tbl_array_u is table of dim_hr_gender%rowtype index by binary_integer;
...
--   DWH_LOOKUP.dim_gender_hierachy(g_rec_out.gender_no,g_rec_out.gender_name,g_rec_out.sk1_gender_no,
...
---------------------------------------------------------
   ...
   g_rec_out.gender_long_desc  := g_rec_in.gender_code||' - '||g_rec_out.gender_desc;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_hr_gender values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_hr_gender
      set    gender_code                  = a_tbl_update(i).gender_code,
             gender_desc                  = a_tbl_update(i).gender_desc,
             gender_long_desc             = a_tbl_update(i).gender_long_desc,
...
begin
   g_found := dwh_hr_valid.dim_hr_gender(g_rec_out.gender_code);

-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_gender_code  := hr_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF dim_hr_gender EX fnd_hr_gender STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_gender;
    fetch c_fnd_hr_gender bulk collect into a_stg_input limit g_forall_limit;

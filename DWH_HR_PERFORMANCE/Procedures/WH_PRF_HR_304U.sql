--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_304U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_304U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        January 2012
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_skill_dev_spend fact table in the performance layer
--               with added value ex foundation layer fnd_hr_bee_skill_dev_spend.
--  Tables:      Input  - fnd_hr_bee_skill_dev_spend
--               Output - hr_bee_skill_dev_spend
...
--  Change      :	 Add sk1_citizenship_status_code to hr_bee_skill_dev_spend
...
g_rec_out            hr_bee_skill_dev_spend%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_304U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_bee_skill_dev_spend EX FOUNDATION';
...
type tbl_array_i is table of hr_bee_skill_dev_spend%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_skill_dev_spend%rowtype index by binary_integer;
...
cursor c_fnd_hr_bee_skill_dev_spend is
   select sc.*,
          de.sk1_employee_id,
          dc.sk1_course_code,
          deh.sk2_employee_id,
          de.sk1_citizenship_status_code
   from   fnd_hr_bee_skill_dev_spend sc
          ...
          on   sc.employee_id           = deh.employee_id and
               sc.course_start_date     between deh.sk2_active_from_date and deh.sk2_active_to_date
   where  sc.last_updated_date  > g_date-7
   and     de.company_code in ('WW','UPN');

g_rec_in                   c_fnd_hr_bee_skill_dev_spend%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_hr_bee_skill_dev_spend%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_skill_dev_spend values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).sk1_course_code;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    l_text := 'LOAD OF hr_bee_skill_dev_spend EX fnd_hr_bee_skill_dev_spend STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_bee_skill_dev_spend;
    fetch c_fnd_hr_bee_skill_dev_spend bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_hr_bee_skill_dev_spend;
--**************************************************************************************************

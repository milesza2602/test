--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_406U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_406U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        January 2015
--  Author:      Kgomotso
--  Purpose:     Create hr_mdl_mgmt_ctrl_pbu_ee fact table in the performance layer
--               with added value ex foundation layer fnd_hr_mdl_mgmt_ctrl_pbu_ee.
--  Tables:      Input  - fnd_hr_mdl_mgmt_ctrl_pbu_ee
--               Output - hr_mdl_mgmt_ctrl_pbu_ee
...
g_rec_out            hr_mdl_mgmt_ctrl_pbu_ee%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_406U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_mdl_mgmt_ctrl_pbu_ee EX FOUNDATION';
...
type tbl_array_i is table of hr_mdl_mgmt_ctrl_pbu_ee%rowtype index by binary_integer;
type tbl_array_u is table of hr_mdl_mgmt_ctrl_pbu_ee%rowtype index by binary_integer;
...
cursor  c_fnd_mdl_mgmt_ctl_pnt_bu_ee is
   select mgt.*,
          dpbu.sk1_bee_prnt_busness_unit_code,
  ...
  from   dwh_hr_foundation.fnd_hr_mdl_mgmt_ctrl_pbu_ee mgt ,
         dwh_hr_performance.dim_hr_bee_parent_bu dpbu,
  ...
  where dpbu.bee_parent_business_unit_code = mgt.bee_parent_business_unit_code
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_mdl_mgmt_ctrl_pbu_ee values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_mdl_mgmt_ctrl_pbu_ee
       ...
       where  fin_year_no                 = a_tbl_update(i).fin_year_no  and
              sk1_bee_prnt_busness_unit_code  = a_tbl_update(i).sk1_bee_prnt_busness_unit_code and
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_mdl_mgmt_ctrl_pbu_ee
   where  fin_year_no                 = g_rec_out.fin_year_no  and
          sk1_bee_prnt_busness_unit_code  = g_rec_out.sk1_bee_prnt_busness_unit_code and
...
begin
    ...
    l_text := 'LOAD OF hr_mdl_mgmt_ctrl_pbu_ee EX fnd_hr_mdl_mgmt_ctrl_pbu_ee STARTED AT '||

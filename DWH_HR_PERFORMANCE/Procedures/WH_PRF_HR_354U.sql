--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_354U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_354U" 
...
--               Output - HR_BEE_INVOICE_SUPP_VMP
...
--  Description: Change process to report on Suppliers even if there are no transactions for the month.
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            HR_BEE_INVOICE_SUPP_VMP%rowtype;

g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;
g_count              number        := 0;
g_value_adding       number        := 0;
g_ed_benefit         number        := 0;
g_black_prof         number        := 0;
g_fin_year_no        number        := 0;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_354U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE HR_BEE_INVOICE_SUPP_VMP EX FOUNDATION';
...
type tbl_array_i is table of HR_BEE_INVOICE_SUPP_VMP%rowtype index by binary_integer;
type tbl_array_u is table of HR_BEE_INVOICE_SUPP_VMP%rowtype index by binary_integer;
...
cursor c_fnd_hr_bee_sed is
   ...
   from  dates dc
  left outer join   dwh_hr_foundation.fnd_hr_ap_invoice ai
  on     (dc.supplier_id = ai.supplier_id and
   ...
   group by dc.this_mn_end_date,ds.sk1_supplier_id,dc.fin_month_code,dc.fin_month_no,dc.fin_year_no
    ;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into HR_BEE_INVOICE_SUPP_VMP values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update HR_BEE_INVOICE_SUPP_VMP
...
begin
    ...
    l_text := 'LOAD OF HR_BEE_INVOICE_SUPP_VMP EX fnd_hr_ap_invoice STARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_326U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_326U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_socio_economic_dev fact table in the performance layer
--               with added value ex foundation layer fnd_hr_bee_socio_economic_dev.
--  Tables:      Input  - fnd_hr_bee_socio_economic_dev
--               Output - hr_bee_socio_economic_dev
...
g_rec_out            hr_bee_socio_economic_dev%rowtype;
g_company_code       varchar2(3 byte);
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_326U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE HR_BEE_SOCIO_ECONOMIC_DEV EX FOUNDATION';
...
type tbl_array_i is table of hr_bee_socio_economic_dev%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_socio_economic_dev%rowtype index by binary_integer;
...
cursor c_fnd_hr_bee_sed is
   ...
   from   fnd_hr_bee_socio_economic_dev sc ,
          DIM_HR_BEE_BENEFIT_FACTOR DBF,
          DIM_HR_CONTR_TYPE DCT
   where  sc.last_updated_date  > g_date - 70  and
          6001                      = DBF.BENEFIT_FACTOR_ID AND
          SC.CONTRIBUTION_ID        = DCT.CONTRIBUTION_ID ;
...
begin
   ...
   if g_rec_in.ledger_id = 2022 then
      g_company_code                          := 'WW';
   else
      g_company_code                          := 'WW';
   end if;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_socio_economic_dev values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_bee_socio_economic_dev
      set     nett_surplus          = a_tbl_update(i).nett_surplus ,
              sk1_benefit_factor_id = a_tbl_update(i).sk1_benefit_factor_id,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_bee_socio_economic_dev
...
begin
    ...
    l_text := 'LOAD OF hr_bee_socio_economic_dev EX fnd_hr_bee_socio_economic_dev STARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_228U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_228U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        Jun 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create dim_hr_contract dimension in the performance layer
--
--  Tables:      Input  - fnd_hr_contract
--               Output - dim_hr_contract
...
g_rec_out            dim_hr_contract%rowtype;
g_rec_in             dwh_hr_foundation.fnd_hr_contract%rowtype;
g_count              number        := 0;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_228U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE dim_hr_contract EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of dwh_hr_foundation.fnd_hr_contract%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_hr_contract%rowtype index by binary_integer;
type tbl_array_u is table of dim_hr_contract%rowtype index by binary_integer;
...
begin
   ...
   g_rec_out.last_updated_date               := g_date;
    ...
    begin
      select sk1_supplier_id
      into   g_rec_out.sk1_supplier_id
      from   dim_hr_bee_supplier
      where  supplier_id = g_rec_in.supplier_id;
    exception
         when no_data_found then
              g_rec_out.sk1_supplier_id := 0;
   end;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_hr_contract values a_tbl_insert(i);
...
-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_contract_id  := hr_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF dim_hr_contract EX fnd_hr_contract STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_contract;
    fetch c_fnd_hr_contract bulk collect into a_stg_input limit g_forall_limit;

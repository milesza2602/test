--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_404U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_404U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        January 2015
--  Author:      Kgomotso
--  Purpose:     Create hr_mdl_mgmt_ctrl_bu_ee fact table in the performance layer
--               with added value ex foundation layer fnd_hr_mdl_mgmt_ctrl_bu_ee.
--  Tables:      Input  - fnd_hr_mdl_mgmt_ctrl_bu_ee
--               Output - hr_mdl_mgmt_ctrl_bu_ee
...
g_rec_out            hr_mdl_mgmt_ctrl_bu_ee%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_404U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_mdl_mgmt_ctrl_bu_ee EX FOUNDATION';
...
type tbl_array_i is table of hr_mdl_mgmt_ctrl_bu_ee%rowtype index by binary_integer;
type tbl_array_u is table of hr_mdl_mgmt_ctrl_bu_ee%rowtype index by binary_integer;
...
cursor c_fnd_hr_mdl_mgmt_ctrl_bu_ee is
   select mgt.*,
          ...
          dd.sk1_disability_no

  from   dwh_hr_foundation.fnd_hr_mdl_mgmt_ctrl_bu_ee mgt ,
         dwh_hr_performance.dim_hr_bee_bu dbu,
         dwh_hr_performance.dim_hr_occupation_level ocl,
         dwh_hr_performance.dim_hr_race dr,
         dwh_hr_performance.dim_hr_gender dg,
         dwh_hr_performance.dim_hr_citizenship_status dc,
         dwh_hr_performance.dim_hr_disability dd
  where dbu.bee_business_unit_code = mgt.bee_business_unit_code
  and   ocl.occupation_level_code = mgt.occupational_level_code
  ...
  and   dd.disabled_ind = mgt.disabled_ind;

g_rec_in                   c_fnd_hr_mdl_mgmt_ctrl_bu_ee%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_hr_mdl_mgmt_ctrl_bu_ee%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_mdl_mgmt_ctrl_bu_ee values a_tbl_insert(i);
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      update hr_mdl_mgmt_ctrl_bu_ee
       ...
       where  fin_year_no                 = a_tbl_update(i).fin_year_no  and
              sk1_bee_business_unit_code  = a_tbl_update(i).sk1_bee_business_unit_code and
              sk1_occupation_level_code   = a_tbl_update(i).sk1_occupation_level_code and
              sk1_race_code               = a_tbl_update(i).sk1_race_code and
              sk1_gender_code             = a_tbl_update(i).sk1_gender_code and
              sk1_citizenship_status_code = a_tbl_update(i).sk1_citizenship_status_code and
              sk1_disability_no           = a_tbl_update(i).sk1_disability_no;
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_mdl_mgmt_ctrl_bu_ee
   where  fin_year_no                 = g_rec_out.fin_year_no  and
          sk1_bee_business_unit_code  = g_rec_out.sk1_bee_business_unit_code and
          sk1_occupation_level_code   = g_rec_out.sk1_occupation_level_code and
          sk1_race_code               = g_rec_out.sk1_race_code and
          sk1_gender_code             = g_rec_out.sk1_gender_code and
          sk1_citizenship_status_code = g_rec_out.sk1_citizenship_status_code and
         sk1_disability_no            = g_rec_out.sk1_disability_no;
...
begin
    ...
    l_text := 'LOAD OF hr_mdl_mgmt_ctrl_bu_ee EX fnd_hr_mdl_mgmt_ctrl_bu_ee STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_mdl_mgmt_ctrl_bu_ee;
    fetch c_fnd_hr_mdl_mgmt_ctrl_bu_ee bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_hr_mdl_mgmt_ctrl_bu_ee;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_224U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_224U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        Jun 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create dim_hr_ap_invoice dimension in the performance layer
--
--  Tables:      Input  - fnd_hr_ap_invoice
--               Output - dim_hr_ap_invoice
...
g_rec_out            dim_hr_ap_invoice%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_224U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE dim_hr_ap_invoice EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_hr_ap_invoice%rowtype index by binary_integer;
type tbl_array_u is table of dim_hr_ap_invoice%rowtype index by binary_integer;
...
cursor c_fnd_hr_ap_invoice is
   select  api.invoice_id,
             api.invoice_no,
             app.payment_gl_period,
             app.paid_ind,
             api.post_date,
             api.invoice_date,
             app.payment_date,
             app.payment_date - api.invoice_date days_to_pay
   from   fnd_hr_ap_invoice_payment  app,
          fnd_hr_ap_invoice api,
          dim_hr_bee_supplier ds,
          dim_hr_bee_supplier_hist dsh,
        --  dim_hr_bee_ed_beneficiary deb,
          dim_calendar_WK dc
   where  app.last_updated_date  > g_date - 400 and
      --    ds.supplier_no         = to_char(deb.vendor_no) and
          app.invoice_id         = api.invoice_id and
          app.supplier_id        = ds.supplier_id and
          app.supplier_id        = dsh.supplier_id and
          payment_date     between dsh.sk2_active_from_date and dsh.sk2_active_to_date and
          app.fin_year_no  = dc.fin_year_no and
          app.fin_week_no  = dc.fin_week_no;

  g_rec_in                   c_fnd_hr_ap_invoice%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_hr_ap_invoice%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_hr_ap_invoice values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_hr_ap_invoice
         SET  invoice_no             = a_tbl_update(i).invoice_no,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   dim_hr_ap_invoice
   where  invoice_id           = g_rec_out.invoice_id  and
          payment_date         = g_rec_out.payment_date;
...
-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_invoice_id  := hr_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF dim_hr_ap_invoice EX fnd_hr_ap_invoice STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_ap_invoice;
    fetch c_fnd_hr_ap_invoice bulk collect into a_stg_input limit g_forall_limit;

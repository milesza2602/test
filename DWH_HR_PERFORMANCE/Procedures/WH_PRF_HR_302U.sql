--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_302U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_302U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_share_capital_mth fact table in the performance layer
--               with added value ex foundation layer fnd_hr_bee_share_capital.
--  Tables:      Input  - fnd_hr_bee_share_capital
--               Output - hr_bee_share_capital_mth
...
g_rec_out            hr_bee_share_capital_mth%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_302U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_bee_share_capital_mth EX FOUNDATION';
...
type tbl_array_i is table of hr_bee_share_capital_mth%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_share_capital_mth%rowtype index by binary_integer;
...
cursor c_fnd_hr_bee_share_capital is
   select sc.*,dc.sk1_company_code
   from   fnd_hr_bee_share_capital sc,
          dim_hr_company dc
   where  sc.company_code  = dc.company_code
--     and  sc.last_updated_date  = g_date
   ;


g_rec_in                   c_fnd_hr_bee_share_capital%rowtype;
-- For input bulk collect --
type stg_array is table of c_fnd_hr_bee_share_capital%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_share_capital_mth values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_bee_share_capital_mth
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).sk1_company_code = g_rec_out.sk1_company_code and
...
begin
    ...
    l_text := 'LOAD OF hr_bee_share_capital_mth EX fnd_hr_bee_share_capital STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_bee_share_capital;
    fetch c_fnd_hr_bee_share_capital bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_hr_bee_share_capital;
--**************************************************************************************************

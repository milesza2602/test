--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_118U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_118U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create dim_hr_employee dimention table in the performance layer
--               with added value ex foundation layer fnd_hr_employee.
--  Tables:      Input  - fnd_hr_employee
--               Output - dim_hr_employee
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--      Date      : Jun 2014
--     Changed by : Kgomotso Lehabe
--     Purpose     :Add columns to dim_hr_employee
--                 Employee_class_code
...
g_rec_out            dim_hr_employee%rowtype;
g_rec_in             fnd_hr_employee%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_118U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE dim_hr_employee EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_hr_employee%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_hr_employee%rowtype index by binary_integer;
type tbl_array_u is table of dim_hr_employee%rowtype index by binary_integer;
...
--ADD SK1 VALUES TO OUTPUT EMPLOYEE MASTER
   ...
   begin
     select sk1_employee_class_code
     into   g_rec_out.sk1_employee_class_code
     from   dim_hr_employee_class
     where  employee_class_code          = g_rec_out.employee_class_code ;

     exception
         when no_data_found then
              g_rec_out.sk1_employee_class_code := 0;
   end;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_hr_employee values a_tbl_insert(i);
...
begin
   g_found := dwh_hr_valid.dim_hr_employee(g_rec_out.employee_id);

-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_employee_id  := hr_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF dim_hr_employee EX fnd_hr_employee STARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_312U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_312U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        November 2011
--  Author:      Alastair de Wet
--  Purpose:     Create hr_bee_skill_dev_spend_lrn fact table in the performance layer
--               with added value ex foundation layer fnd_hr_bee_skill_dev_spend_lrn.
--  Tables:      Input  - fnd_hr_bee_skill_dev_spend_lrn
--               Output - hr_bee_skill_dev_spend_lrn
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--      Date      : Jul 2014
--     Changed by : Kgomotso Lehabe
--     Purpose    :    Derive Citizenship status by assuming that Ethinc group W are foreing nationals
--                !!!NB!!! This logic is incorrect, this is because we have to excluse whites, and foreing national from the calculations.
--                 : Add filter for companies WW and UPN only
--
--  Naming conventions:
--  g_  -  Global variable
...
g_rec_out            hr_bee_skill_dev_spend_lrn%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_312U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE hr_bee_skill_dev_spend_lrn EX EMP MASTER';
...
type tbl_array_i is table of hr_bee_skill_dev_spend_lrn%rowtype index by binary_integer;
type tbl_array_u is table of hr_bee_skill_dev_spend_lrn%rowtype index by binary_integer;
...
cursor c_skill_dev_spend_lrn is
   select TRUNC(lrn.effective_date) effective_date ,
          dg.sk1_gender_code,
           dr.sk1_race_code,
          lrn.disabled_ind,
          cat.sk1_bee_category_code,
          SUM(LRN.learnership_salaries) learnership_salaries,
          dbu.sk1_bee_business_unit_code,
         (case
          when lrn.ethnicity_code = 'W'
          then 'E'
          else 'C'
         end) citizenship_status_code
   from   fnd_hr_bee_skill_dev_spend_lrn lrn,
          dim_hr_bee_bu dbu,
          fnd_hr_ps_bee_bu_map bu_map,
          dim_hr_gender dg,
          dim_hr_bee_category cat,
           dim_hr_race dr
   where  lrn.company_code              = bu_map.company_code and
          lrn.ps_business_unit_code     = bu_map.ps_business_unit_code  and
          bu_map.bee_business_unit_code = dbu.bee_business_unit_code and
          lrn.gender_code               = dg.gender_code and
          lrn.bee_category_code         = cat.bee_category_code and
          lrn.ethnicity_code            = dr.race_code       and
          lrn.company_code in ('WW','UPN') and
          lrn.last_updated_date         > g_date-7
   GROUP BY
          lrn.effective_date,
          dr.sk1_race_code,
          dg.sk1_gender_code,
          ...
          cat.sk1_bee_category_code,
          dbu.sk1_bee_business_unit_code,
          (case when lrn.ethnicity_code = 'W'  then 'E' else 'C' end) ;


g_rec_in                   c_skill_dev_spend_lrn%rowtype;
-- For input bulk collect --
type stg_array is table of c_skill_dev_spend_lrn%rowtype;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into hr_bee_skill_dev_spend_lrn values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update hr_bee_skill_dev_spend_lrn
      set    learnership_salaries        = a_tbl_update(i).learnership_salaries,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   hr_bee_skill_dev_spend_lrn
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if
          a_tbl_insert(i).sk1_bee_business_unit_code  = g_rec_out.sk1_bee_business_unit_code  and
          a_tbl_insert(i).sk1_race_code               = g_rec_out.sk1_race_code          and
          a_tbl_insert(i).sk1_gender_code             = g_rec_out.sk1_gender_code        and
          a_tbl_insert(i).sk1_disability_no           = g_rec_out.sk1_disability_no      and
          a_tbl_insert(i).sk1_bee_category_code       = g_rec_out.sk1_bee_category_code  and
          a_tbl_insert(i).effective_date              = g_rec_out.effective_date
...
begin
    ...
    l_text := 'LOAD OF hr_bee_skill_dev_spend_lrn EX fnd_hr_bee_skill_dev_spend_lrn STARTED AT '||
...
--**************************************************************************************************
    open c_skill_dev_spend_lrn;
    fetch c_skill_dev_spend_lrn bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_134U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_134U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        June 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create dim_hr_poi_type dimension table in the performance layer
--  Tables:      Input  - fnd_hr_poi_type
--               Output - dim_hr_poi_type
...
g_rec_out            dim_hr_poi_type%rowtype;
g_rec_in             DWH_HR_FOUNDATION.fnd_hr_poi_type%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_134U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE DIM_HR_POI_TYPE EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of DWH_HR_FOUNDATION.fnd_hr_poi_type%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_hr_poi_type%rowtype index by binary_integer;
type tbl_array_u is table of dim_hr_poi_type%rowtype index by binary_integer;
...
cursor c_fnd_hr_poi_type is
   select *
   from DWH_HR_FOUNDATION.fnd_hr_poi_type;
...
---------------------------------------------------------
   ...
   g_rec_out.poi_type_long_desc  := g_rec_in.poi_type||' - '||g_rec_out.poi_type_desc;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_hr_poi_type values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_hr_poi_type
      set    poi_type               = a_tbl_update(i).poi_type,
             poi_type_desc          = a_tbl_update(i).poi_type_desc,
             poi_type_long_desc     = a_tbl_update(i).poi_type_long_desc,
...
begin
   g_found := dwh_hr_valid.dim_hr_poi_type(g_rec_out.poi_type);

-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_poi_type  := hr_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF dim_hr_poi_type EX fnd_hr_poi_type STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_hr_poi_type;
    fetch c_fnd_hr_poi_type bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_PRF_HR_352U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_PERFORMANCE"."WH_PRF_HR_352U" (p_forall_limit in integer,p_success out boolean) AS
...
g_rec_out            dwh_hr_performance.hr_bee_ed_payment_mn%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_HR_352U';
...
type tbl_array_i is table of dwh_hr_performance.hr_bee_ed_payment_mn%rowtype index by binary_integer;
type tbl_array_u is table of dwh_hr_performance.hr_bee_ed_payment_mn%rowtype index by binary_integer;
...
cursor c_fnd_hr_ap_pay is
  with dates as
    (select distinct ai.sk1_supplier_id,
                sk1_benefit_factor_id1,
                ai.sk2_ed_beneficiary_id,
                ai.sk1_ed_beneficiary_id,
      ...
      and   sk1_benefit_factor_id1 <> 0
      ),

    payments as
   (select   app.sk1_supplier_id,
          app.sk1_benefit_factor_id1,
...
--          sum(app.ed_settlement_discount) ed_settlement_discount
   ...
   where  app.sk1_supplier_id        = ds.sk1_supplier_id     and
          ...
          app.sk1_benefit_factor_id1 = bf.sk1_benefit_factor_id and
         app.sk1_benefit_factor_id1 <> 0
...
--          sum(app.ed_settlement_discount) ed_settlement_discount
    ...
    and pay.sk1_benefit_factor_id1 = dt.sk1_benefit_factor_id1
    and pay.this_mn_end_date = dt.this_mn_end_date
     )
--   where  dt.sk1_ed_beneficiary_id != 716712
     group by  dt.sk1_benefit_factor_id1, pay.ed_category,  dt.this_mn_end_date,  dt.sk1_ed_beneficiary_id,   dt.fin_year_no, dt.fin_month_no;
...
begin
   ...
   g_rec_out.sk1_benefit_factor_id    := g_rec_in.sk1_benefit_factor_id1;
   g_rec_out.payment_mn_end_date      := g_rec_in.this_mn_end_date ;
   g_rec_out.ed_payment_contribution  := g_rec_in.ed_payment_terms;
--   g_rec_out.ed_payment_contrtibution := g_rec_in.ed_settlement_discount;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dwh_hr_performance.hr_bee_ed_payment_mn values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dwh_hr_performance.hr_bee_ed_payment_mn

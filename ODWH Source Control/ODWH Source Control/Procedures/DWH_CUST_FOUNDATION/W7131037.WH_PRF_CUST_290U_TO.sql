-- ****** Object: Procedure W7131037.WH_PRF_CUST_290U_TO Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_290U_TO" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        May 2016
--  Author:      Theo Filander
--  Purpose:     Create Dept Involvement Scores for the current month using Depth and Breath Tables
--  Tables:      Input  - cust_db_dept_month
--                      - dim_calendar
--                      - temp_cust_dept_item_ranking
--                      - temp_cust_dept_sale_ranking
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_290U_TO';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CUST_DB_DEPT_MONTH_INVOLVE EX CUST_DB_DEPT_MONTH';
...
begin
    ...
    l_text := 'LOAD OF CUST_DB_DEPT_MONTH_INVOLVE EX CUST_DB_DEPT_MONTH STARTED AT '||
...
--**************************************************************************************************
   ...
   l_text := 'CUST_DB_DEPT_MONTH data being processed is Financial Year  : '|| g_year_no|| ' Month  ' || g_month_no;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   l_text := 'CUST_DB_DEPT_MONTH_INVOLVE data being rolled is Financial Year  '|| g_prev_year_no|| ' Month  ' || g_prev_month_no;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    g_stmt := 'TRUNCATE table W7131037.TEMP_CUST_DEPT_SALE_RANKING';
    execute immediate g_stmt;

    g_stmt := 'TRUNCATE table W7131037.TEMP_CUST_DEPT_ITEM_RANKING';
    execute immediate g_stmt;

    l_text := 'UPDATE STATS ON ALL TEMP TABLES';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('W7131037','TEMP_CUST_DEPT_SALE_RANKING',estimate_percent=>1, DEGREE => 32);
    DBMS_STATS.gather_table_stats ('W7131037','TEMP_CUST_DEPT_ITEM_RANKING',estimate_percent=>1, DEGREE => 32);
    COMMIT;

    l_text := 'Populate Sale Ranking for CUST_DB_DEPT_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    INSERT INTO W7131037.TEMP_CUST_DEPT_SALE_RANKING (fin_year_no,fin_month_no,department_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                    ...
                    FROM W7131037.cust_db_dept_month a
         ...
         customer_cnt AS
                 (SELECT /*+ PARALLEL(cust,8) FULL(cust) */
                    ...
                    FROM cust
                   GROUP BY fin_year_no, fin_month_no,department_no
                 ),
         customer_item_cnt AS
                 (SELECT /*+ PARALLEL(c,8) PARALLEL(d,8) FULL(c) FULL(d) */
                   ...
                   GROUP BY c.fin_year_no,c.fin_month_no,c.department_no,c.sales_sum,d.customer_cnt
                 ),
         customer_item_ranking AS
                 (SELECT /*+ PARALLEL(a,8) FULL(a) */
                         ...
                         a.customer_cnt,
                         a.customer_item_cnt,
                         SUM(a.customer_item_cnt) OVER (PARTITION BY a.fin_year_no, a.fin_month_no, a.department_no ORDER BY a.fin_year_no, a.fin_month_no, a.department_no, a.sales_sum ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) - a.customer_item_cnt AS diff_customer_item_cnt
    ...
    l_text := 'Populate Item Ranking for CUST_DB_DEPT_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    INSERT INTO W7131037.TEMP_CUST_DEPT_ITEM_RANKING (fin_year_no,fin_month_no,department_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                        2016 fin_year_no,
                        12 fin_month_no,
                        department_no,
                 ...
                 (SELECT /*+ PARALLEL(c,8) PARALLEL(d,8) FULL(c) FULL(d) */
                   ...
                   INNER JOIN customer_cnt d
                      ON c.fin_year_no = d.fin_year_no AND
                         c.fin_month_no = d.fin_month_no AND
                         c.department_no = d.department_no
                   GROUP BY c.fin_year_no,c.fin_month_no,c.department_no,c.item_sum,d.customer_cnt
                 ),
         customer_item_ranking AS
                 (SELECT /*+ PARALLEL(a,8) FULL(a) */
                         a.fin_year_no,
                         ...
                         a.customer_cnt,
                         a.customer_item_cnt,
                         SUM(a.customer_item_cnt) OVER (PARTITION BY a.fin_year_no, a.fin_month_no, a.department_no ORDER BY a.fin_year_no, a.fin_month_no, a.department_no, a.item_sum ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) - a.customer_item_cnt AS diff_customer_item_cnt
                    FROM customer_item_cnt a
                 )
    SELECT /*+ PARALLEL(a,8) FULL(a) */
           fin_year_no,
    ...
    l_text := 'Populate CUST_DB_DEPT_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--**************************************************************************************************
-- Execute the insert
--**************************************************************************************************
    insert /*+ APPEND Parallel(a,8) */ into W7131037.cust_db_dept_month_involve a
    select /*+ Parallel(cma,8) Parallel(cmb,8) Parallel(cmc,8) Parallel(cmd,8) Full(cma) Full(cmb) Full(cmc) Full(cmd) */
          ...
          from W7131037.cust_db_dept_month cma
               left join
               W7131037.temp_cust_dept_item_ranking cmb on (cma.num_item_yr1_mn02 + cma.num_item_yr1_mn03 + cma.num_item_yr1_mn04+cma.num_item_yr1_mn05 + cma.num_item_yr1_mn06 +
                                                                         cma.num_item_yr1_mn07 + cma.num_item_yr1_mn08 + cma.num_item_yr1_mn09+cma.num_item_yr1_mn10 + cma.num_item_yr1_mn11 + cma.num_item_yr1_mn12 +
                                                                         cma.num_item_yr2_mn01) = cmb.item_sum AND
                                                                        cmb.fin_year_no = 2016 AND
                                                                        cmb.fin_month_no = 12 AND
                                                                        cma.department_no = cmb.department_no
               left join
               W7131037.temp_cust_dept_sale_ranking cmc on ROUND(cma.sales_yr1_mn02 + cma.sales_yr1_mn03 + cma.sales_yr1_mn04 + cma.sales_yr1_mn05 + cma.sales_yr1_mn06 +
                                                                             cma.sales_yr1_mn07 + cma.sales_yr1_mn08 + cma.sales_yr1_mn09 + cma.sales_yr1_mn10 + cma.sales_yr1_mn11 + cma.sales_yr1_mn12 +
                                                                             cma.sales_yr2_mn01) = cmc.sales_sum AND
                                                                        cmc.fin_year_no = 2016 AND
                                                                        cmc.fin_month_no = 12 AND
                                                                        cma.department_no = cmc.department_no
               left join
               W7131037.cust_db_dept_month_involve cmd on cma.primary_customer_identifier = cmd.primary_customer_identifier AND
                                                                      cma.department_no = cmd.department_no and
   ...
   --l_text := 'UPDATE STATS ON CUST_DB_DEPT_MONTH_INVOLVE TABLES';
   --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

-- ****** Object: Procedure W7131037.WH_PRF_CUST_E20_6E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_E20_6E" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_E20_6E';
...
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT E20 PERSONALISATION INTERFACE (NUM OF TENDERS) TO FLAT FILE';
...
begin
    ...
    l_text := 'EXTRACT FOR E20 PERSONALISATION INTERFACE-NUMBER OF TENDERS STARTED AT '||
...
--    select *
--      into l_period
--      from W7131037.keystone
--     where program_name = 'WH_PRF_CUST_E20_6E';
--
--     l_text := 'PROCESSING SEQUENCE NO '||l_period.seq_num||' FOR DATE: '||l_period.curr_date;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    l_period.seq_num   := l_period.seq_num+1;
--    l_period.curr_date := l_period.curr_date-1;
...
--**************************************************************************************************
    dbms_output.put_line('NUMBER OF TENDERS');
    g_count := dwh_performance.dwh_generic_file_extract(
         q'[select /*+ Parallel(cbt,6) Full(cbt) */ distinct
                   *
              from W7131037.temp_e20_basket_tender_cnt cbt
       ]','|','DWH_FILES_OUT','e20_basket_tender_cnt_'||l_period.fin_year_no||l_period.fin_month_no||'.txt');
        l_text :=  'Records extracted to E20_BASKET_TENDER_CNT '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

-- ****** Object: Procedure W7131037.WH_PRF_CUST_110FIX Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_110FIX" (p_forall_limit in integer,p_success out boolean) as
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (prf,2) */ into cust_loc_item_dy_prc_override prf

  with b_aux as (
      select /*+  parallel (aux,2) */
              aux.*
      from    cust_basket_aux aux
      where   ppc_code in(6,24)
      and     aux.last_updated_date = '5 dec 2015'
                )

      select /*+  parallel (cb,2) */
             ca.location_no,
             ca.till_no,
             ca.tran_no,
             ca.aux_seq_no,
       ...
       where ca.location_no = cb.location_no
         ...
         and cb.item_seq_no = (ca.aux_seq_no+1)
       group by
             ca.location_no,
             ca.till_no,
             ca.tran_no,
...
--   +- 6000 deletes when you rerun makes it more efficient to first delete and then only insert
    ...
    where last_updated_date = '5 dec 2015';

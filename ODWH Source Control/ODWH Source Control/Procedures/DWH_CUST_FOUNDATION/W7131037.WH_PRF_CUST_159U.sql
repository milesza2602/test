-- ****** Object: Procedure W7131037.WH_PRF_CUST_159U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_159U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_159U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD TALKTOME_HIST  EX FND DAILY';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


cursor c_cust_talktome is
select /*+ FULL(fnd)  parallel (fnd,2) */
              fnd.*
      from    cust_talktome fnd,
              cust_talktome_hist prf
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (prf,2) */ into cust_talktome_hist prf
      select /*+ FULL(fnd)  parallel (fnd,2) */
             	fnd.*
       from   cust_talktome fnd
       where
       not exists
      (select /*+ nl_aj */ * from cust_talktome_hist
...
FOR upd_rec IN c_cust_talktome
   loop
     update   cust_talktome_hist prf
     set      prf.	wfs_customer_no	=	upd_rec.	wfs_customer_no	,
              ...
              prf.	closed_charged_off_ind	=	upd_rec.	closed_charged_off_ind	,

-- ****** Object: Procedure W7131037.WH_PRF_CUST_339U1T Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_339U1T" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_sclass_qtr_profile fact table in the performance layer
...
--               Output - cust_mart_sclass_qtr_profile
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_sclass_qtr_profile%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_339U1';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE BASKET ITEM to SUBCLASS PROFILE QTR MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_sclass_qtr_profile EX cust_basket_item LEVEL STARTED AT '||
...
--**************************************************************************************************
   ...
   if to_char(sysdate,'DDMM') NOT IN  ('1007','1010','1001','1004') then
      l_text      := 'This job only runs QUARTERLY ON THE 10TH and today '||to_char(sysdate,'DDMM')||' is not that day !';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      p_success := true;
--      return;
   end if;
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    delete from cust_mart_sclass_qtr_profile where fin_year_no = g_yr_00 and fin_quarter_no =  g_qt_00;

    l_text := 'TABLE CLEARED FOR NEW QUARTER INSERTS '  ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

insert  into cust_mart_sclass_qtr_profile csm
with  prof as (
select  /*+ parallel (8) */
           customer_no,
...
select /*+ FULL(cbi) FULL(di) parallel (cbi,12) */
         primary_customer_identifier,
         di.department_no,di.class_no,di.subclass_no,
...
select  /*+ FULL(a)  FULL(dc)  FULL(dcr)  FULL(prf) FULL(hbt) FULL(ttm)  parallel (8) */
         g_yr_00,
         g_qt_00,
         a.department_no,a.class_no,a.subclass_no,
...
group by a.department_no,a.class_no,a.subclass_no
order by a.department_no,a.class_no,a.subclass_no
         ;

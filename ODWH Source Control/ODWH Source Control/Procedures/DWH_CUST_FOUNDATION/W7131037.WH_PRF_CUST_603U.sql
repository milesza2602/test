-- ****** Object: Procedure W7131037.WH_PRF_CUST_603U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_603U" (p_forall_limit in integer,p_success out boolean) AS
...
--**************************************************************************************************
   ...
   MERGE  INTO cust_svoc_campaign_fb_summ prf USING
   (
    SELECT  max(ACCOUNT_ID) ACCOUNT_ID,
            max(OYB_ACCOUNT_ID) OYB_ACCOUNT_ID,
            max(LIST_ID) LIST_ID,
            min(EVENT_DATE) EVENT_DATE,
            EMAIL_NAME,
            EMAIL_SUBJECT
    FROM   W7131037.cust_svoc_campaign_feedback
...
--            prf.	EVENT_DATE	        =	mer_rec.	EVENT_DATE	, --Not needed as updated event date will not be less than what was originally inserted

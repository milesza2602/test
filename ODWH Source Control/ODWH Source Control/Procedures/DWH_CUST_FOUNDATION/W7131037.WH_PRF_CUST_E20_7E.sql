-- ****** Object: Procedure W7131037.WH_PRF_CUST_E20_7E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_E20_7E" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_E20_7E';
...
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT E20 PERSONALISATION INTERFACE (DIM_DEPARTMENT) TO FLAT FILE';
...
begin
    ...
    l_text := 'EXTRACT FOR E20 PERSONALISATION INTERFACE-DIM_DEPARTMENT STARTED AT '||
...
--**************************************************************************************************
    dbms_output.put_line('DIM_DEPARTMENT');
    g_count := dwh_performance.dwh_generic_file_extract(
         q'[select /*+ Parallel(dd,6) Full(dd) */ distinct
                   department_no,
                   department_name,
                   subgroup_no,
                   subgroup_name,
                   ...
                   business_unit_no,
                   business_unit_name
              from dim_department@dwhprd dd
               where dd.business_unit_no =ANY (51,52,53,54,55)
       ]','|','DWH_FILES_OUT','e20_department.txt');
        l_text :=  'Records extracted to E20_DEPARTMENT '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

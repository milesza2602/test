-- ****** Object: Procedure W7131037.WH_PRF_CUST_615U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_615U" (p_forall_limit in integer,p_success out boolean) AS
...
--**************************************************************************************************
   ...
   (
    SELECT *
    FROM   W7131037.fnd_customer_mapping
    WHERE last_updated_date = g_date
   ) MER_REC
    ON    (  dim.	source_key	          =	mer_rec.	source_key and
             dim. source                = mer_rec.  source)
   WHEN MATCHED THEN
   UPDATE SET
            dim.	subscriber_key	        =	mer_rec.	subscriber_key	,

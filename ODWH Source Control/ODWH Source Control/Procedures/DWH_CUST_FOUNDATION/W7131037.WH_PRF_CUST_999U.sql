-- ****** Object: Procedure W7131037.WH_PRF_CUST_999U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_999U" 
...
begin
 G_SQL := '';
    G_SQL := 'select RECTYPE||''|''||RECTYPE_DATETIME||''|''||RECDESC
           from W7131037.cust_vmp_sales_ms_ENGN_hdr
    union all
    select  CUSTOMER_NO||''|''||
            TRAN_DATE||''|''||
            LOCATION_NO||''|''||
            ...
            TRAN_SELLING||''|''||
            C2_CUSTOMER_NO
           from W7131037.cust_vmp_sales_myschl_engn
    union all
     select RECTYPE||''|''||
            RECCNT||''|''||
            TRAN_SELLING
     from W7131037.cust_vmp_sales_ms_ENGN_FTR';
...
-------------------------------------- CORPORATE
 ...
 G_SQL := '';
  G_SQL := 'select RECTYPE||''|''||RECTYPE_DATETIME||''|''||RECDESC
           from W7131037.cust_vmp_sales_ms_CORP_hdr
    union all
    select  CUSTOMER_NO||''|''||
            TRAN_DATE||''|''||
            LOCATION_NO||''|''||
            ...
            TRAN_SELLING||''|''||
            C2_CUSTOMER_NO
           from W7131037.cust_vmp_sales_myschl_CORP
    union all
     select RECTYPE||''|''||
            RECCNT||''|''||
            TRAN_SELLING
     from W7131037.cust_vmp_sales_ms_CORP_FTR';

g_count := 0;
    g_count := dwh_generic_file_extract(G_SQL,'|','DWH_FILES_OUT','cust_wod_vmp.txt');

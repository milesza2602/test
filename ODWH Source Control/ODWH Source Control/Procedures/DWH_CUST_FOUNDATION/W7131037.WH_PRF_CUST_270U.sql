-- ****** Object: Procedure W7131037.WH_PRF_CUST_270U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_270U" (p_forall_limit in integer,p_success out boolean) AS
...
--   g_stmt      := 'Alter table  W7131037.CUST_DB_DEPT_MONTH truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_DB_DEPT_MONTH_1';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_DB_DEPT_MONTH_2';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_DB_DEPT_MONTH_3';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_DB_DEPT_MONTH_4';
    execute immediate g_stmt;

   l_text := 'UPDATE STATS ON ALL TABLES';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   DBMS_STATS.gather_table_stats ('W7131037','TEMP_DB_DEPT_MONTH_1',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('W7131037','TEMP_DB_DEPT_MONTH_2',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('W7131037','TEMP_DB_DEPT_MONTH_3',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('W7131037','TEMP_DB_DEPT_MONTH_4',estimate_percent=>1, DEGREE => 32);
   DBMS_STATS.gather_table_stats ('W7131037','CUST_DB_DEPT_MONTH',estimate_percent=>1, DEGREE => 32);
   COMMIT;
...
--MAIN INSERT STATEMENT USING UNION ALL ACCROSS ALL DATE RANGES --
   ...
   insert /*+ APPEND parallel (prf,8) */ into temp_db_dept_month_1 prf
   with     bskt1 as
   (
   ...
   insert /*+ APPEND parallel (prf,12) */ into temp_db_dept_month_2 prf
   with         bskt10 as
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */
            primary_customer_identifier,
   ...
   insert /*+ APPEND parallel (prf,12) */ into temp_db_dept_month_3 prf
   with         bskt19 as
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */
            primary_customer_identifier,
   ...
   insert /*+ APPEND parallel (prf,12) */ into temp_db_dept_month_4 prf
   with         bskt28 as
   (
   select  /*+ FULL(cbi) parallel (cbi,16)  full(di) */
            primary_customer_identifier,
   ...
   insert /*+ APPEND parallel (prf,12) */ into cust_db_dept_month  prf
   with        cust_union_all as
   (
   select  /*+ FULL(t1)  parallel (t1,8)  */  *   from temp_db_dept_month_1  t1
   union all
   select  /*+ FULL(t2)  parallel (t2,8)  */  *   from temp_db_dept_month_2  t2
   union all
   select  /*+ FULL(t3)  parallel (t3,8)  */  *   from temp_db_dept_month_3  t3
   union all
   select  /*+ FULL(t4)  parallel (t4,8)  */  *   from temp_db_dept_month_4  t4
   )
   select /*+ FULL(cua)  parallel (cua,8)  */
            g_yr_00,g_mn_00,
            primary_customer_identifier,

-- ****** Object: Procedure W7131037.WH_PRF_CUST_520U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_520U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        September 2010
--  Author:      Alastair de Wet
--  Purpose:     Create Chanel Inbound dimention table in the performance layer
--               with added value ex foundation layer Chanel Inbound table.
--  Tables:      Input  - fnd_cust_cl_chanel_inbound
--               Output - dim_cust_cl_chanel_inbound
...
g_rec_out            dim_cust_cl_chanel_inbound%rowtype;
g_rec_in             fnd_cust_cl_chanel_inbound%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_520U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE dim_cust_cl_chanel_inbound EX FOUNDATION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_cust_cl_chanel_inbound%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dim_cust_cl_chanel_inbound%rowtype index by binary_integer;
type tbl_array_u is table of dim_cust_cl_chanel_inbound%rowtype index by binary_integer;
...
cursor c_fnd_cust_cl_chanel_inbound is
   select *
   from fnd_cust_cl_chanel_inbound;
...
---------------------------------------------------------
   ...
   g_rec_out.channel_inbound_long_desc  := g_rec_in.channel_inbound_no||' - '||g_rec_out.channel_inbound_desc;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into dim_cust_cl_chanel_inbound values a_tbl_insert(i);
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      save exceptions
      update dim_cust_cl_chanel_inbound
      set    channel_inbound_desc      = a_tbl_update(i).channel_inbound_desc,
             channel_inbound_long_desc = a_tbl_update(i).channel_inbound_long_desc,
...
begin
   g_found := dwh_cust_valid.dim_cust_cl_chanel_inbound(g_rec_out.channel_inbound_no);

-- Place record into array for later bulk writing
   if not g_found then
      g_rec_out.sk1_channel_inbound_no  := cust_seq.nextval;
...
begin
    ...
    l_text := 'LOAD OF dim_cust_cl_chanel_inbound EX fnd_cust_cl_chanel_inbound STARTED AT '||
...
--**************************************************************************************************
    open c_fnd_cust_cl_chanel_inbound;
    fetch c_fnd_cust_cl_chanel_inbound bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_fnd_cust_cl_chanel_inbound;
--**************************************************************************************************

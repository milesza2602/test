-- ****** Object: Procedure W7131037.WH_PRF_CUST_300U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_300U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        January 2014
--  Author:      Alastair de Wet
--  Purpose:     Create wfs_cust_vmp_sales fact table in the performance layer
--               with input ex basket transactions
--  Tables:      Input  - cust_basket
--               Output - cust_vmp_sales
...
g_tran_date          date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_300U';
...
select   /*+ FULL(a)  FULL(c)  parallel (4) */
         a.location_no,
         a.till_no,
         a.tran_no,
         a.tran_date,
         max(case when  substr(a.loyalty_ext_swipe_no,1,4) = 5900 then
                        a.loyalty_ext_swipe_no
                  when  a.loyalty_ww_swipe_no is not null then
                        a.loyalty_ww_swipe_no
                  else  a.primary_customer_identifier
             end) primary_customer_identifier,
         max(c.business_unit_no) business_unit_no,
         sum(b.item_tran_selling-b.discount_selling) total_sales,
         sum(b.item_tran_qty) total_qty,
         sum (case when business_unit_no  = 50 then (b.item_tran_selling-b.discount_selling) else 0 end) foods_sales,
         sum (case when business_unit_no <> 50 then (b.item_tran_selling-b.discount_selling) else 0 end) candh_sales,
...
and      a.tran_date         between (g_date - 7) and g_date
--and      a.tran_date         between '25 jun 2016' and '30 jun 2016'
--and      a.last_updated_date = g_date - 1
and      b.item_no           = c.item_no
and      c.department_no    <> 546
and      a.vmp_ind           = 1
...
--insert /*+ APPEND parallel (prf,4) */ into cust_vmp_sales prf

insert /*+ APPEND parallel (prf,8) */ into cust_vmp_sales prf
select /*+ FULL(a)  FULL(c)  parallel (a,8) parallel (b,8) parallel(c,8) */
         a.tran_date,
         a.location_no,
         a.tran_no,
         a.till_no,
...
and      a.tran_no           = b.tran_no
and      a.tran_date         = b.tran_date
and      a.tran_date         between  g_tran_date  and g_date
--and      a.last_updated_date = g_date - 1
and      b.item_no           = c.item_no
and      c.department_no    <> 546
and      a.VMP_IND           = 1
and      not exists
      (select /*+ nl_aj */ * from cust_vmp_sales
...
FOR upd_rec IN c_cust_basket
   loop
     ...
     set    prf.tran_time	          =	upd_rec.	tran_time,
            ...
            prf.textile_selling	    =	upd_rec.	candh_sales,
     ...
     where  prf.tran_date	          =	upd_rec.	tran_date   and
            ...
            (upd_rec.	total_sales <> prf.tran_selling);
...
--**************************************************************************************************
    ...
    g_tran_date := g_date - 10;

    l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date||g_tran_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

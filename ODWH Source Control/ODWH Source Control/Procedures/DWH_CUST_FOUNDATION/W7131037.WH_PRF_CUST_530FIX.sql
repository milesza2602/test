-- ****** Object: Procedure W7131037.WH_PRF_CUST_530FIX Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_530FIX" (p_forall_limit in integer,p_success out boolean) AS
...
cursor c_fnd_cust_cl_inquiry is
   select fnd_inq.*
   from   cust_cl_inquiry fnd_inq

   where  sk1_location_no = 411 and svc_cia_store_no is not null ;
...
----------------------------------------------------------------------------------
   if not  dwh_valid.fnd_location(g_rec_out.svc_cia_store_no) then
        g_rec_out.svc_cia_store_no := 101;
        g_rec_out.sk1_location_no   := 411;
   end if;
   dwh_lookup.dim_location_sk1(g_rec_out.svc_cia_store_no,g_rec_out.sk1_location_no);
----------------------------------------------------------------------------------
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      set   sk1_location_no                 = a_tbl_update(i).sk1_location_no,
            last_updated_date               = '11 May 1950'   --a_tbl_update(i).last_updated_date

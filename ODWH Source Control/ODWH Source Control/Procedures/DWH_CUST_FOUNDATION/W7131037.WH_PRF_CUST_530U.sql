-- ****** Object: Procedure W7131037.WH_PRF_CUST_530U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_530U" (p_forall_limit in integer,p_success out boolean) AS
...
--
--  Maintenance:
--   14 OCTOBER 2015  - wl ---- remove derivation for sk1_cl_inq_sub_cat_no and add rather to wh_fnd_cust_530u
--                         where records not found will go to hospital
--                         PBLM – We cannot load data at the moment due to the fact that since Sunday night, we have been receiving fact data with missing DIMS.
...
--                         Seems fine, but I will have to then do a datafix to fnd_cust_cl_inquiry to complete the change.
...
cursor c_fnd_cust_cl_inquiry is
   ...
   from   fnd_cust_cl_inquiry fnd_inq,
          ...
          dim_cust_cl_sub_cat DSC
   where
   --fnd_inq.last_updated_date  >= '9 oct 2015' and
          fnd_inq.last_updated_date  = g_date and
          ...
          fnd_inq.cl_status_no       = ds.cl_status_no AND
          DSC.cl_inq_sub_cat_no      = FND_inq.DERIVED_INQ_TYPE_NO;
 --AND INQUIRY_NO = 432773246         ;
...
--   g_rec_out.inq_type_no                     := g_rec_in.inq_type_no;
   ...
   g_rec_out.inq_details_CLOB                := g_rec_in.inq_details;
   g_rec_out.inq_details                     := SUBSTR(g_rec_in.inq_details,1,4000);
   g_rec_out.inq_details1                    := SUBSTR(g_rec_in.inq_details,4001,8000);
...
-----------------------------------------------------------------------------------
--LOCAL BULK ERROR - POSITION, CODE and RECORD KEY:- 1 1 ORA-01400: cannot insert NULL into () 432773246  52171 52171 16813 16803 4658
...
begin
   ...
   forall i in a_tbl_update.first .. a_tbl_update.last
      ...
      set   sk1_cl_inq_sub_cat_no           = a_tbl_update(i).sk1_cl_inq_sub_cat_no,
            ...
            inq_details_CLOB                = a_tbl_update(i).inq_details_CLOB,
            inq_details                     = a_tbl_update(i).inq_details,
            inq_details1                    = a_tbl_update(i).inq_details1,

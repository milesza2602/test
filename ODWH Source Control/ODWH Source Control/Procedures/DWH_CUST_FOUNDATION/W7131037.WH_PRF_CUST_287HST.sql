-- ****** Object: Procedure W7131037.WH_PRF_CUST_287HST Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_287HST" (p_forall_limit in integer,p_success out boolean) AS
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE LIFE STYLE SEGMENT BASE TABLE FOR C481 ';
...
LOOP
    ...
    G_DATE := G_DATE - 30;
     ...
     g_year_no := 2015;
     g_month_no := g_month_no - 1;
--*************************************

     l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date||g_year_no||g_month_no;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--***********************************************************************************************************
    ...
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS1_PCI_CH';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS1_PCI_PFC_CH';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS1_PCI_FINAL_CH';
    execute immediate g_stmt;
    ...
    DBMS_STATS.gather_table_stats ('W7131037','TEMP_LSS1_PCI_CH',estimate_percent=>1, DEGREE => 32);
...
-- CREATE A TEMP TABLE WITH PCI/PFC'S THAT NEED TO BE CALCULATED IN NEXT PROCESS
    insert  /*+ APPEND parallel(tmp,16) */ into W7131037.temp_lss1_pci_pfc_ch tmp
    ...
    DBMS_STATS.gather_table_stats ('W7131037','TEMP_LSS1_PCI_PFC_CH',estimate_percent=>1, DEGREE => 32);
...
--************************************************************************************************************************
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--************************************************************************************************************************
    INSERT  /*+ APPEND parallel(tmp,8) */ INTO W7131037.TEMP_LSS1_PCI_FINAL_CH TMP
    WITH PTHEME AS (
...
--************************************************************************************************************************
    ...
    FROM    W7131037.TEMP_LSS1_PCI_FINAL_CH FIN,
            W7131037.DIM_LSS_CLUSTER_CENTRE CC

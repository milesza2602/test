-- ****** Object: Procedure W7131037.WH_PRF_CUST_157_0510 Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_157_0510" (p_forall_limit in integer,p_success out boolean) AS
...
select    /*+ FULL(dc)  parallel (4) */
          ...
          nvl(max((case when  dy.product_code_no = 1  and  dy.account_status  in ('Z','W') AND g_stcard_open     = 1 then 1 end)),0)  charge_off_ind
from      dim_customer dc
          join
          dim_customer_portfolio  cp
          on   dc.customer_no     = cp.customer_no
          left outer join
          fnd_wfs_cust_perf_60dy  dy
          on   dy.run_date = g_max_run_date
          and  dc.wfs_customer_no = dy.wfs_customer_no
WHERE     dc.last_transaction_date = '3 OCT 2015' and
          cp.product_code_no       in (1,2,3,4,6,9,19,20,21,24,28,30,80,92,99) and
          dy.product_code_no       in (1,2,3,4,6,9,21,80)
...
-- CREDITCARD_OTB
   ...
   into    g_rec_out.creditcard_otb, g_max_legal_date,g_rec_out.deceased_ind,g_rec_out.delinquent_ind,g_rec_out.debt_review_ind

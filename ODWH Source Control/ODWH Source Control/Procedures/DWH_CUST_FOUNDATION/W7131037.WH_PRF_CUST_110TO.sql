-- ****** Object: Procedure W7131037.WH_PRF_CUST_110TO Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_110TO" (p_forall_limit in integer,p_success out boolean) as
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (prf,10) */ into cust_loc_item_dy_prc_override prf

  with b_aux as (
      select /*+  parallel (aux,10) */
              aux.*
      from    cust_basket_aux aux
      where   ppc_code in(6,24)
--      and     aux.tran_date  between '26 Oct 2015' and '25 Dec 2015'
      AND     aux.tran_date  between  '1 jan 2014' and '30 Jun 2014'
                )

      select /*+  full(ca) full(cb) full(di) parallel(10) */
             ca.location_no,
             ca.till_no,
             ca.tran_no,
             ca.aux_seq_no,

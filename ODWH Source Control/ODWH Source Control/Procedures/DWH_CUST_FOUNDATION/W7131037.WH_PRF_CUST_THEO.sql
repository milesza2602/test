-- ****** Object: Procedure W7131037.WH_PRF_CUST_THEO Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_THEO" (p_forall_limit in integer,p_success out boolean)
as
...
--
--  Maintenance: Setup keystone table to manage data to transmit (Theo Filander)
--               Delete this when Stored Procedure complete
--
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_THEO';
...
--**************************************************************************************************
    ...
    select fin_year_no,
           ...
           this_mn_start_date,
           this_mn_end_date,
           NVL((select seq_num+1 from "W7131037"."KEYSTONE"),1) seq_num
      into l_period
      from dim_calendar
     where calendar_date = NVL((select this_mn_start_date-1 START_DATE from "W7131037"."KEYSTONE"),trunc(sysdate)-20 );
...
-- WOD_TIER_MTH_DETAIL
-----------------------------------------------------------------------
--   dbms_output.put_line('Extract CUST_WOD_TIER_MTH_DETAIL');

--   l_text :=  'EXTRACT CUST_WOD_TIER_MTH_DETAIL STARTED AT '|| to_char(sysdate,('dd Mon yyyy hh24:mi:ss'));
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--    l_text :=  'Records extracted to cust_wod_tier_mth_detail '||g_count;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--    -- g_count := 0;
--    l_text :=  'EXTRACT CUST_WOD_TIER_MTH_DETAIL ENDED AT '|| to_char(sysdate,('dd Mon yyyy hh24:mi:ss'));
--    DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);

   ----------------------------------------------------------------------
-- DIM_CUSTOMER
-----------------------------------------------------------------------
    dbms_output.put_line('Extract DIM_CUSTOMER');
...
-- DIM_LOCATION
-----------------------------------------------------------------------
--dbms_output.put_line('Extract DIM_LOCATION');
...
--       '|',
--       'DWH_FILES_OUT',
--       'cust_basket_item_sample_'||l_period.seq_num||'.det');

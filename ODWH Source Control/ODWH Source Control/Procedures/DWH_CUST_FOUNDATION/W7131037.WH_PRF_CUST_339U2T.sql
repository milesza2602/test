-- ****** Object: Procedure W7131037.WH_PRF_CUST_339U2T Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_339U2T" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_sclass_half_profile fact table in the performance layer
...
--               Output - cust_mart_sclass_half_profile
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_sclass_half_profile%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_339U2';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE BASKET ITEM to SUBCLASS PROFILE HALF MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_sclass_half_profile EX cust_basket_item LEVEL STARTED AT '||
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    delete from cust_mart_sclass_half_profile where fin_year_no = g_yr_00 and fin_half_no =  g_hf_00;

    l_text := 'TABLE CLEARED FOR NEW HALF YEAR INSERTS '  ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

insert  into cust_mart_sclass_half_profile csm
with  prof as (
select  /*+ parallel (8) */
           customer_no,

-- ****** Object: Procedure W7131037.WH_PRF_CUST_291U_RRN Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_291U_RRN" (p_forall_limit in integer,p_success out boolean,p_date in date) AS
...
--**************************************************************************************************
    select fin_year_no,fin_month_no, this_mn_start_date, this_mn_end_date
     ...
     where calendar_date in (select this_mn_start_date-1
                               from dim_calendar
                              where calendar_date = trunc(g_date));
...
--   end if;
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    select count(*)
      into g_recs_read
      from W7131037.cust_db_subgroup_month_involve
    ...
    g_stmt := 'ALTER TABLE W7131037.cust_db_subgroup_month_involve TRUNCATE SUBPARTITION FOR ('||g_year_no||','||g_month_no||')';
    l_text := g_stmt;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--    execute immediate g_stmt;
...
--**************************************************************************************************
    ...
    select /*+ Parallel(cma,8) Parallel(cmb,8) Parallel(cmc,8) Parallel(cmd,8) Full(cma) Full(cmb) Full(cmc) Full(cmd) */
          ...
          from W7131037.cust_db_subgroup_month cma
               left join
               W7131037.temp_cust_sgrp_item_ranking cmb on cma.num_item_yr1_mn01 +
               ...
               left join
               W7131037.temp_cust_sgrp_sale_ranking cmc on ROUND(cma.sales_yr1_mn01 +
                                                                             cma.sales_yr1_mn02 +
                                                                             cma.sales_yr1_mn03 +
                                                                             cma.sales_yr1_mn04 +
                                                                             cma.sales_yr1_mn05 +
    ...
    g_stmt := 'ALTER TABLE W7131037.cust_db_subgroup_month_involve TRUNCATE SUBPARTITION FOR ('||g_trunc_year_no||','||g_trunc_month_no||')';
    l_text := g_stmt;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--    execute immediate g_stmt;

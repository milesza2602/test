-- ****** Object: Procedure W7131037.WH_PRF_CUST_383E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_383E" (p_forall_limit in integer,p_success out boolean,p_run_date in date)
as
--**************************************************************************************************
--  Date:        APR 2016
...
--**************************************************************************************************
    if p_run_date is not null or p_run_date <> '' then
      ...
      g_run_date := g_this_mn_end_date - 12;
...
---
    ...
    execute immediate 'truncate table "W7131037"."TEMP_CUST_INSIGHTS"';
    commit;

    l_text := 'UPDATE STATS ON TEMP_CUST_INSIGHTS TABLES';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('W7131037','OUT_DWH_CUST_INSIGHTS_SK',estimate_percent=>1, DEGREE => 32);
    ...
    execute immediate 'truncate table "W7131037"."OUT_DWH_CUST_INSIGHTS_SK"';
    commit;
    ...
    l_text := 'Populate TEMP_CUST_INSIGHTS_SK. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
---

--insert into W7131037.out_dwh_cust_insights_sk sk
insert into W7131037.temp_cust_insights
select /* Parallel(6)  */
  ...
  from (
                select subscriber_key,
                      ...
                      from (
                            select /*+ Parallel(ci,6) Full(ci) Parallel(sm,6) */ distinct
                                   ...
                                   sum(ci.foods_preferred_store)          foods_preferred_store,
                                   min(ci.non_foods_preferred_store)      non_foods_preferred_store,
                                   ...
                                   min(ci.months_since_last_foods_pur)    months_since_last_foods_pur,
                                   min(ci.months_since_last_cgm_purchase) months_since_last_cgm_purchase,
                                   min(ci.months_since_last_crg_purchase) months_since_last_crg_purchase,
                              ...
                              from W7131037.out_dwh_cust_insights ci
       ...
       left join W7131037.fnd_segment_mapping sa     on non.non_food_life_seg_code       = sa.segment_code and sa.segment_name = 'NON_FOOD_LIFE_SEG_CODE'
       left join W7131037.fnd_segment_mapping sb     on fds.food_life_seg_code           = sb.segment_code and sb.segment_name = 'FOOD_LIFE_SEG_CODE'
       left join W7131037.fnd_segment_mapping sc     on non.nfshv_current_seg            = sc.segment_code and sc.segment_name = 'NFSHV_CURRENT_SEG'
       left join W7131037.fnd_segment_mapping sd     on non.fshv_current_seg             = sd.segment_code and sd.segment_name = 'FSHV_CURRENT_SEG'
       left join W7131037.fnd_segment_mapping se     on non.shopping_habit_segment_no    = se.segment_code and se.segment_name = 'SHOPPING_HABIT_SEGMENT_NO'
       left join W7131037.fnd_segment_mapping sf     on non.ly_foods_value_seg           = sf.segment_code and sf.segment_name = 'LY_FOODS_VALUE_SEGMENT'
       left join W7131037.fnd_segment_mapping sg     on non.ly_non_foods_value_seg       = sg.segment_code and sg.segment_name = 'LY_NON_FOODS_VALUE_SEGMENT'
       left join W7131037.fnd_segment_mapping sh     on non.ty_summer_non_foods_val_seg  = sh.segment_code and sh.segment_name = 'TY_SUMMER_NON_FOODS_VAL_SEG'
       left join W7131037.fnd_segment_mapping si     on non.ly_summer_non_foods_val_seg  = si.segment_code and si.segment_name = 'LY_SUMMER_NON_FOODS_VAL_SEG'
       left join W7131037.fnd_segment_mapping sj     on non.ty_winter_non_foods_val_seg  = sj.segment_code and sj.segment_name = 'TY_WINTER_NON_FOODS_VAL_SEG'
       left join W7131037.fnd_segment_mapping sk     on non.ly_winter_non_foods_val_seg  = sk.segment_code and sk.segment_name = 'LY_WINTER_NON_FOODS_VAL_SEG'
       where seq_no = 1;
---
 ...
 insert /*+ Parallel(sk,6) */ into W7131037.OUT_DWH_CUST_INSIGHTS_SK sk

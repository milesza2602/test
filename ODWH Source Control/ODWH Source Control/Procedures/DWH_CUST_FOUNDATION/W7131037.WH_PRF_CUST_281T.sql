-- ****** Object: Procedure W7131037.WH_PRF_CUST_281T Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_281T" (p_forall_limit in integer,p_success out boolean) AS
...
--***********************************************************************************************************
    ...
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS_PRODUCT_FAMILY_FD';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS_FINAL_CALCS_FD';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS_PFC_EXCEPTIONS';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.TEMP_LSS_PFC_NULL_FD';
    execute immediate g_stmt;
    g_stmt      := 'TRUNCATE table  W7131037.CUST_LSS_PFC_FEEDBACK';
    execute immediate g_stmt;
    ...
    DBMS_STATS.gather_table_stats ('W7131037','TEMP_LSS_PRODUCT_FAMILY_FD',estimate_percent=>1, DEGREE => 32);
...
--************************************************************************************************************************
    ...
    from   W7131037.temp_lss_product_family_fd;

-- CREATE A TEMP TABLE WITH PFC'S THAT NEED TO BE CALCULATED AT CUST/ITEM_L1 LEVEL --
    insert /*+ APPEND parallel(tmp,12) */ into W7131037.temp_lss_pfc_null_fd tmp
    select /*+ FULL(pff2)   parallel(pff2,12) */
             pff2.primary_customer_identifier,
             pff2.item_level1_no
    from     W7131037.temp_lss_product_family_fd pff2
    where    pff2.product_family_code is null;

    DBMS_STATS.gather_table_stats ('W7131037','TEMP_LSS_PFC_NULL_FD',estimate_percent=>1, DEGREE => 32);
...
--************************************************************************************************************************
    ...
    INSERT /*+ APPEND parallel(tmp,12) */ into W7131037.temp_lss_final_calcs_fd tmp
    with     assoc_var as (
    select /*+ FULL(pff)   parallel(pff,12) */
             pff.association_variable,
             count(distinct pff.primary_customer_identifier) num_customers
    from     W7131037.temp_lss_product_family_fd pff
    ...
    from     W7131037.temp_lss_product_family_fd pff1
...
--             W7131037.temp_lss_product_family_fd l1, ** Alternative join method back to same table is slightly slower **
             W7131037.temp_lss_product_family_fd pff3,
             assoc_var av,
             prod_fam pf
...
--YULE SCORE CALC --
             yule as         (
    select   /*+ FULL(pf) full(di)  parallel(12) */
             pf.item_level1_no,
...
--************************************************************************************************************************
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--************************************************************************************************************************
    DBMS_STATS.gather_table_stats ('W7131037','TEMP_LSS_FINAL_CALCS_FD',estimate_percent=>1, DEGREE => 32);
    commit;

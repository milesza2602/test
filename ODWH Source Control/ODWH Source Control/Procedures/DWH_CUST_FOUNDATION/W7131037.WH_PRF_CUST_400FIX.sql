-- ****** Object: Procedure W7131037.WH_PRF_CUST_400FIX Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_400FIX" (p_forall_limit in integer,p_success out boolean) AS 
...
select today_,
  ...
  from (
        select to_char(calendar_date,'DAY') TODAY_,
               ...
               case
                    when to_char(calendar_date,'D') in (2) then             -- Monday
                         calendar_date - 5
                    when to_char(calendar_date,'D') in (3) then             -- Tuesday
                         calendar_date - 4
                    when to_char(calendar_date,'D') in (4,5,6) then         -- Wednesday,Thursday,Friday
                         calendar_date - 3
...
--**************************************************************************************************       
     ...
     l_text := 'Processing for '||trim(to_char(g_rundate,'Day'))||'. Extract Start Date : '||to_char(g_start_date,'dd/Mon/yyyy')||
...
--**************************************************************************************************    
       ...
       select 'decode(upper(dcm.derived_race), '||WM_CONCAT(race_name)||',0)'
         into g_race_case
         from (select race_no,''''||upper(race_name)||''','||race_no race_name from apex_app_cust_01.derived_race@dwhprd);
       ...
       select replace('case '||WM_CONCAT(case_stmt)||' end',',','')  
         ...
         from (select ' when dcm.age >= '||minimum_age||' and dcm.age <= '||maximum_age||' then '||age_band case_stmt 
                 from apex_app_cust_01.apex_age_band@dwhprd);
           ...
           select survey_code,
              ...
              from (
                    select /*+ Full(apd) */ apd.survey_code,apd.product_location_other_desc,
                     ...
                     from apex_app_cust_01.apex_mkt_csats_summary@dwhprd aps
                    inner join apex_app_cust_01.apex_mkt_csats_detail@dwhprd apd on (aps.survey_code = apd.survey_code)
...
--    dbms_output.put_line('Inserted into ALL_PARAMS - '||sql%rowcount);
    ...
    g_stmt := ' insert /*+ Append */ into temp_survey_master
                ...
                select aps.survey_code,aps.survey_name,aps.survey_area,aps.minimum_spend_criteria,aps.minimum_units_criteria,aps.priority_order,
                  ...
                  from apex_app_cust_01.apex_mkt_csats_summary aps inner join
                       (select *
                          from (select survey_code, apd.product_location_other_desc, apd.product_location_other_no
                                  from apex_app_cust_01.apex_mkt_csats_detail@dwhprd apd
                 ...
                 left join apex_app_cust_01.apex_age_band@dwhprd ab on (apd.age_band = ab.age_band)
                 where upper(aps.survey_status) like ''ACTIVE%'' ';
        ...
        g_stmt :=' insert /*+ Append Parallel(8)*/ into temp_basket_item 
                   select /*+Parallel(cbi,8) Parallel(di,8) Parallel(cm,8) Parallel(dcm,8) Parallel(dr,8) Parallel(csm,8) Parallel(cw,8) Parallel(cs,8) Parallel(sd,8) Parallel(ap,8) Parallel(dl,8)*/ 
                    ...
                    inner join (select master_subscriber_key,identity_document_no,
                                  ...
                                  from dim_customer_master@dwhprd dcm) dcm on (cm.subscriber_key = dcm.master_subscriber_key)
                   ...
                   inner join (select /*+Parallel(piv,8) */
                                 ...
                                 from (select /*+Parallel(8) */
                                         ...
                                         from dwh_cust_performance.cust_csm_value_segment@dwhprd 
                   ...
                   where cbi.tran_date between '''||case when upper(c_survey.survey_type) <> 'WEBSITE' then g_start_date else g_online_start_date end ||''' and '''||case when upper(c_survey.survey_type) <> 'WEBSITE' then g_end_date else g_online_end_date end||''' 
            ...
            g_stmt :=' insert /*+ Append Parallel(8) */ into temp_csats_survey_detail 
                       select /*+ Parallel(bi,8)*/ distinct *
                                 ...
                                 left join (select /*+ Parallel */ 
                                              ...
                                              from (
                                                    select /*+ Parallel(csd,8) Parallel(anc,8) */ 
                                                      ...
                                                      from dwh_cust_performance.cust_csats_survey_detail@dwhprd csd
                                                      left join apex_app_cust_01.apex_non_csats_survey@dwhprd anc on (csd.master_subscriber_key = anc.subscriber_key)
                        ...
                        left join (select /*+ Parallel(ao,8)  */ 
                                     ...
                                     from apex_app_cust_01.apex_mkt_csats_opt_out@dwhprd ao
                                  ) oo on (cb.master_subscriber_key = oo.opt_out_subscriber_key or 
                                           cb.identity_document_no = cast(oo.opt_out_identity_document_no as varchar2(20)) or 

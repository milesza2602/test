-- ****** Object: Procedure W7131037.WH_PRF_CUST_290U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_290U" (p_forall_limit in integer,p_success out boolean) AS
...
--**************************************************************************************************
    ...
    g_stmt := 'ALTER TABLE W7131037.cust_db_dept_month_involve TRUNCATE SUBPARTITION FOR ('||g_year_no||','||g_month_no||')';
    l_text := g_stmt;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate g_stmt;
...
--**************************************************************************************************
  ...
  select /*+ Parallel(cma,8) Parallel(cmb,8) Parallel(cmc,8) Parallel(cmd,8) Full(cma) Full(cmb) Full(cmc) Full(cmd) */
          ...
          from W7131037.cust_db_dept_month cma
               left join
               W7131037.temp_cust_dept_item_ranking cmb on cma.num_item_yr1_mn01 +
               ...
               left join
               W7131037.temp_cust_dept_sale_ranking cmc on ROUND(cma.sales_yr1_mn01 +
                                                                             cma.sales_yr1_mn02 +
                                                                             cma.sales_yr1_mn03 +
                                                                             cma.sales_yr1_mn04 +
                                                                             cma.sales_yr1_mn05 +
...
--**************************************************************************************************
-- Perform cleanup of old subpartion
--**************************************************************************************************
    g_stmt := 'ALTER TABLE W7131037.cust_db_dept_month_involve TRUNCATE SUBPARTITION FOR ('||g_trunc_year_no||','||g_trunc_month_no||')';
    l_text := g_stmt;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate g_stmt;

--**************************************************************************************************
-- We need to update Statistics on current subpartition only
-- This query gets that partition name
--**************************************************************************************************
    select /*+Parallel(dp,6) */
           subobject_name
      into g_partname
      from dba_objects do
     where data_object_id=(select /*+Parallel(dp,16) */
                                  dbms_rowid.rowid_object(rowid)
                             from cust_db_dept_month_involve dp
                            where fin_year_no = g_year_no
                              and fin_month_no = g_month_no
                              and rownum = 1
                           );
   ...
   l_text := 'UPDATE STATS ON CUST_DB_DEPT_MONTH_INVOLVE SUBPARTITION - '||g_partname;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   DBMS_STATS.gather_table_stats ('W7131037','CUST_DB_DEPT_MONTH_INVOLVE',g_partname,estimate_percent=>0.1, DEGREE => 16);
   commit;

-- ****** Object: Procedure W7131037.WH_PRF_CUST_270BU Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_270BU" (p_forall_limit in integer,p_success out boolean) AS
...
--**************************************************************************************************
   ...
   g_stmt      := 'Alter table  W7131037.CUST_DB_DEPT_MONTH truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
   l_text      := g_stmt;

   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    g_stmt      := 'TRUNCATE table  W7131037.CUST_DB_DEPT_MONTH ';
   ...
   insert /*+ APPEND parallel (prf,8) */ into cust_db_dept_month prf
   with     bskt1 as
   (
   select  /*+ FULL(cbi) parallel (cbi,8) parallel (di,8) */
   ...
   group by primary_customer_identifier,di.department_no
   ),
            bskt2 as
   (
   select  /*+ FULL(cbi) parallel (cbi,8) parallel (di,8) */
            primary_customer_identifier,
            di.department_no   department_no,
            max(customer_no)   customer_no,
            0 item_01,0 sales_01,0 visit_01,
            sum(item_tran_qty) item_02,
            sum(item_tran_selling - discount_selling) sales_02,
            count(unique tran_no) visit_02,

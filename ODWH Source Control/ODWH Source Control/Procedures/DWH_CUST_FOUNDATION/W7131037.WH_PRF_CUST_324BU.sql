-- ****** Object: Procedure W7131037.WH_PRF_CUST_324BU Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_324BU" (p_forall_limit in integer,p_success out boolean) as
...
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_BASKET_ITEM to HOME48 BU MART';
...
--    g_stmt      := 'Alter table  W7131037.cust_mart_home_and_away_bu truncate  subpartition for ('||g_yr_00||','||g_qt_00||') update global indexes';
--    l_text      := g_stmt;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--              bih.location_no,
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no between 51 and 55
     group by bih.primary_customer_identifier, di.business_unit_no, s2.location_no),
              away as  (
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no between 51 and 55
     group by primary_customer_identifier,business_unit_no,location_no)
...
select   /*+ FULL(home) FULL(away) parallel(8)  */
         ...
         sum(away_location_qty),
         sum(away_location_value),
         g_date
from     home,
         away
where    home.primary_customer_identifier = away.primary_customer_identifier
and      home.business_unit_no            = away.business_unit_no
and      home.location_no <> away.location_no
group by home.location_no,
         away.location_no,
         home.business_unit_no
having   sum(away_location_qty) > 99 and
         sum(away_location_value) / (sum(away_location_value) + sum(home_location_value)) >= 0.1
...
-- The sql below is more simple as there is only 1 BU and as such the SOP is the same and always present in the home result set
...
insert /*+ APPEND parallel(csm,4)*/ into cust_mart_home_and_away_bu csm
     ...
     select   /*+ parallel(8) */
              sop.primary_customer_identifier,
              sop.business_unit_no,
              max(location_no) location_no,
              max(sop.basket_value) home_location_value,
              max(sop.basket_qty)   home_location_qty
     from     sop, sop1
     where    sop.basket_value                = sop1.max_basket_value
     and      sop.primary_customer_identifier = sop1.primary_customer_identifier
     and      sop.business_unit_no            = sop1.business_unit_no
     group by sop.primary_customer_identifier,
              sop.business_unit_no
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no = 50
     group by primary_customer_identifier,location_no,business_unit_no)
...
from     home,
         away
where    home.primary_customer_identifier = away.primary_customer_identifier
and      home.business_unit_no            = away.business_unit_no and
         home.location_no <> away.location_no
group by home.location_no,
         away.location_no,
         home.business_unit_no
having   sum(away_location_qty) > 99 and

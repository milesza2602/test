-- ****** Object: Procedure W7131037.WH_PRF_CUST_350U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_350U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        OCT 2017
--  Author:      Alastair de Wet
--  Purpose:     Create CUST_MART_VOUCHERS_PROMOTION fact table in the performance layer
...
--               Output - CUST_MART_VOUCHERS_PROMOTION
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            CUST_MART_VOUCHERS_PROMOTION%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_350U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_FV_VOUCHER to CUST_MART_VOUCHERS_PROMOTION';
...
begin
    ...
    l_text := 'ROLLUP OF CUST_MART_VOUCHERS_PROMOTION EX cust_fv_vouchers LEVEL STARTED AT '||
...
execute immediate 'alter session enable parallel dml';
    ...
    g_stmt      := 'TRUNCATE table  W7131037.CUST_MART_VOUCHERS_PROMOTION';
    l_text      := g_stmt;
...
INSERT  /*+ append */ INTO CUST_MART_VOUCHERS_PROMOTION CSM

WITH  CNT AS  (
SELECT   /*+ parallel(VC,16) FULL(VC) */
         VC.CAMPAIGN_NO,
         VC.PROMOTION_NO,
         MAX(VC.VOUCHER_TYPE_NO) VOUCHER_TYPE_NO,
...
GROUP BY VC.CAMPAIGN_NO,VC.PROMOTION_NO
               ),
      CAMPS AS (
SELECT   /*+ parallel(16) */
         CAMPAIGN_NO,PROMOTION_NO,
         MAX(PROMOTION_START_DATE) PROMOTION_START_DATE,
         MAX(PROMOTION_END_DATE) PROMOTION_END_DATE
FROM     FND_FV_CAMPAIGN_PROMOTION
GROUP BY CAMPAIGN_NO,PROMOTION_NO
               ),
      WEEKS AS (
SELECT  /*+ parallel(4) */
        FIN_YEAR_NO,
...
FROM    CAMPS CMP,
        DIM_CALENDAR_WK
WHERE   THIS_WEEK_END_DATE   >=  PROMOTION_START_DATE
AND     THIS_WEEK_START_DATE <=  PROMOTION_END_DATE
                ),
      ISSUED AS (
SELECT  /*+ parallel(4) */
        FIN_YEAR_NO,
...
AND     CNT.PROMOTION_NO = WEEKS.PROMOTION_NO
                 )
...
--================================================================================
    ...
    DBMS_STATS.gather_table_stats ('W7131037','CUST_MART_VOUCHERS_PROMOTION',estimate_percent=>1, DEGREE => 32);
COMMIT;
...
--================================================================================
    ...
    MERGE  /*+ PARALLEL(4) */ INTO CUST_MART_VOUCHERS_PROMOTION CSM
    USING (
    ...
    ON    (  CSM.	FIN_YEAR_NO	          =	MER_REC.	FIN_YEAR_NO AND
             ...
             CSM.	CAMPAIGN_NO	          =	MER_REC.	CAMPAIGN_NO AND
             CSM.	PROMOTION_NO	        =	MER_REC.	PROMOTION_NO)
    WHEN MATCHED THEN
    UPDATE SET

-- ****** Object: Procedure W7131037.WH_PRF_CUST_370E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_370E" (p_forall_limit in integer,p_success out boolean,p_run_date in date)
as
--**************************************************************************************************
--  Date:        February 2016
...
--  May 2017  - Theo Filander Split the extract into 2 on Tuesday extract last Mon to Wed. Thu extract last Wed to Sun
--              BCB-258
...
--                                  where calendar_date = trunc(sysdate));
       ...
       select calendar_date, case when to_char(g_run_date, 'd') = 2 then (calendar_date + 4)
                                  when to_char(g_run_date, 'd') = 5 then (calendar_date + 1)
                                  else calendar_date
                             end end_date
        into g_start_date, g_end_date
        from dim_calendar
       where calendar_date = case when to_char(g_run_date, 'd') = 2 then trunc(g_run_date) - 5
                                  when to_char(g_run_date, 'd') = 5 then trunc(g_run_date) - 3
                                  else g_run_date
                             end;
--    end if;

     dbms_output.put_line('StartDate = '||g_run_date||' - '||'EndDate = '||g_end_date);

    g_file_ext := to_char(g_run_date, 'YYYYMONddhh24mi');
    g_weekday := to_char(g_run_date, 'd');
    dbms_output.put_line('Day Number is '||g_weekday);
    dbms_output.put_line('RUN DATE is '||p_run_date);

    if (g_weekday  <> 2 or g_weekday <> 5) and p_run_date is null then
...
--**************************************************************************************************
    ...
    g_count := dwh_performance.dwh_generic_file_extract(
          'select /*+ Full(bi) Parallel(bi,4) Full(di) Parallel(di,4) Full(dl) Parallel(dl,4) Index(fl PK_F_FND_LCTN_ITM) Parallel(fl,4)*/
             ...
             from cust_basket_item@dwhprd bi
            inner join dim_item@dwhprd di on bi.item_no = di.item_no
            inner join dim_location@dwhprd dl on bi.location_no = dl.location_no
             left join fnd_location_item@dwhprd fl on bi.item_no     = fl.item_no and
                                     bi.location_no = fl.location_no
                    where bi.tran_date between '''||g_start_date||''' and '''||g_end_date||'''
                      and di.business_unit_no in (50,51,52,54,55)

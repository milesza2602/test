-- ****** Object: Procedure W7131037.WH_PRF_CUST_E20_3E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_E20_3E" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_E20_3E';
...
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT E20 PERSONALISATION INTERFACE (DIM_CUSTOMER_PORTFOLIO) TO FLAT FILE';
...
begin
    ...
    l_text := 'EXTRACT FOR E20 PERSONALISATION INTERFACE-DIM_CUSTOMER_PORTFOLIO STARTED AT '||
...
--**************************************************************************************************
    dbms_output.put_line('DIM_CUSTOMER_PORTFOLIO');
    g_count := dwh_performance.dwh_generic_file_extract(
         q'[select /*+ Parallel(dcp,6) Full(dcp) Parallel(ata,6) Full(ata)*/
                     dcp.customer_no,
                     dcp.product_code_no,
                     dcp.portfolio_status_desc,
                     dcp.portfolio_create_date,
                     dcp.portfolio_close_date,
                     dcp.last_updated_date
                from dim_customer_portfolio@dwhprd dcp inner join
                   (select /*+ Parallel(vs,6) Full(vs)*/ customer_no from W7131037.temp_e20_customer vs
                     ) ata on dcp.customer_no = ata.customer_no
       ]','|','DWH_FILES_OUT','e20_customer_portfolio.txt');
        l_text :=  'Records extracted to E20_CUSTOMER_PORTFOLIO '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

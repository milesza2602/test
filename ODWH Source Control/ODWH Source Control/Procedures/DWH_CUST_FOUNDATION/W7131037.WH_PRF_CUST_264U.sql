-- ****** Object: Procedure W7131037.WH_PRF_CUST_264U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_264U" (p_forall_limit in integer,p_success out boolean) AS
...
pivot           (
     ...
     from   cust_csm_st_wk@dwhprd csm
     where   (
...
begin
   ...
   from   fnd_csm_avg_basket_value_band@dwhprd
   ...
   from   fnd_csm_visit_pattern@dwhprd
   ...
   from    fnd_csm_shopping_habit_matrix@dwhprd
...
--**************************************************************************************************
    ...
    from     dim_calendar
    where    fin_year_no  =  2016 and --(select last_yr_fin_year_no from dim_control)  and
             fin_month_no =  7; --(select last_mn_fin_month_no from dim_control);
...
--    g_stmt      := 'Alter table  W7131037.CUST_CSM_SHOPPING_HABITS truncate  subpartition for ('||g_end_year_no||','||g_end_month_no||') update global indexes';
--    l_text      := g_stmt;

-- ****** Object: Procedure W7131037.WH_PRF_CUST_341U1_V2 Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_341U1_V2" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_dept_qtr_profile fact table in the performance layer
...
--               Output - cust_mart_dept_qtr_profile
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_dept_qtr_profile%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_341U1';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE BASKET ITEM to DEPARTMENT PROFILE QTR MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_dept_qtr_profile EX cust_basket_item LEVEL STARTED AT '||
...
--**************************************************************************************************
   ...
   if to_char(sysdate,'DDMM') NOT IN  ('1007','1010','1001','1004') then
...
--execute immediate 'alter session set sort_area_size=100000000';
    ...
    delete from cust_mart_dept_qtr_profile where fin_year_no = g_yr_00 and fin_quarter_no =  g_qt_00;

    l_text := 'TABLE CLEARED FOR NEW QUARTER INSERTS '  ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

insert  into cust_mart_dept_qtr_profile csm
with  prof as (
select  /*+ parallel (8) */
           customer_no,
...
-- ADD INVOLVE SCORES
   ...
   MERGE  INTO CUST_MART_DEPT_QTR_PROFILE CSM
   USING (
     WITH DNB AS (
      SELECT  /*+  FULL(db) parallel(db,16) */
         DB.PRIMARY_CUSTOMER_IDENTIFIER,
         DB.DEPARTMENT_NO,
         SUM ( SALES_YR1_MN01 + SALES_YR1_MN02 + SALES_YR1_MN03 ) SUM_SALES
      FROM     CUST_DB_DEPT_MONTH  DB
      WHERE    SALES_YR1_MN01 + SALES_YR1_MN02 + SALES_YR1_MN03 <> 0
      GROUP BY DB.PRIMARY_CUSTOMER_IDENTIFIER,DB.DEPARTMENT_NO
            )
     SELECT  /*+  FULL(invl)  FULL(db) parallel(invl,16) parallel(db,16) */
         INVL.DEPARTMENT_NO,
         SUM ( CASE     WHEN INVOLVEMENT_SCORE_YR1_MN01 = 3 AND
                      (SUM_SALES <> 0)   THEN 1 ELSE 0  END)  HIGH_INVOLVEMENT,
         SUM ( CASE     WHEN INVOLVEMENT_SCORE_YR1_MN01 = 2 AND
                      (SUM_SALES <> 0)   THEN 1 ELSE 0  END)  MEDIUM_INVOLVEMENT,
         SUM ( CASE     WHEN INVOLVEMENT_SCORE_YR1_MN01 = 1 AND
                      (SUM_SALES <> 0)   THEN 1 ELSE 0  END)  LOW_INVOLVEMENT,
         SUM ( CASE     WHEN INVOLVEMENT_SCORE_YR1_MN01 = 3 AND
                      SUM_SALES <> 0   THEN
                      SUM_SALES ELSE 0  END)  HIGH_INVOLVEMENT_VALUE,
         SUM ( CASE     WHEN INVOLVEMENT_SCORE_YR1_MN01 = 2 AND
                      SUM_SALES <> 0   THEN
                      SUM_SALES ELSE 0  END)  MEDIUM_INVOLVEMENT_VALUE,
         SUM ( CASE     WHEN INVOLVEMENT_SCORE_YR1_MN01 = 1 AND
                      SUM_SALES <> 0   THEN
                      SUM_SALES ELSE 0  END)  LOW_INVOLVEMENT_VALUE
FROM    CUST_DB_DEPT_MONTH_INVOLVE  INVL,
        DNB   DB
WHERE   INVL.FIN_YEAR_NO       = G_YR_00
AND     INVL.FIN_MONTH_NO      = G_END_MONTH
AND     INVL.PRIMARY_CUSTOMER_IDENTIFIER = DB.PRIMARY_CUSTOMER_IDENTIFIER
AND     INVL.DEPARTMENT_NO     = DB.DEPARTMENT_NO
GROUP BY INVL.DEPARTMENT_NO
         ) MER_REC
   ON    (  CSM.	DEPARTMENT_NO	          =	MER_REC.	DEPARTMENT_NO AND
            CSM.	FIN_YEAR_NO	            =	G_YR_00   AND
            CSM.	FIN_QUARTER_NO	        =	G_QT_00)
   WHEN MATCHED THEN
   UPDATE SET
            CSM.	HIGH_INVOLVEMENT	        =	MER_REC.	HIGH_INVOLVEMENT	,
            CSM.	MEDIUM_INVOLVEMENT	      =	MER_REC.	MEDIUM_INVOLVEMENT,
            CSM.	LOW_INVOLVEMENT	          =	MER_REC.	LOW_INVOLVEMENT,
            CSM.	HIGH_INVOLVEMENT_VALUE	  =	MER_REC.	HIGH_INVOLVEMENT_VALUE	,
            CSM.	MEDIUM_INVOLVEMENT_VALUE	=	MER_REC.	MEDIUM_INVOLVEMENT_VALUE,
            CSM.	LOW_INVOLVEMENT_VALUE	    =	MER_REC.	LOW_INVOLVEMENT_VALUE
          ;

-- ****** Object: Procedure W7131037.WH_PRF_CUST_113U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_113U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2017
--  Author:      A DE WET
...
begin
  ...
  update /*+ PARALLEL(dim,4)*/ W7131037.dim_customer_master dim
...
begin
  l_text := 'UPDATE UCOUNT_CUST_TYPE TO UCOUNT CUSTOMER ON W7131037.dim_customer_master STARTED AT '||
  ...
  where dim.master_subscriber_key in
                          (select /*+ PARALLEL(cbt,8) PARALLEL(mp,8) FULL(cbt) */
                           ...
                           from  W7131037.cust_basket_tender cbt,
                                 W7131037.dim_customer_mapping mp
  ...
  l_text := 'UPDATE UCOUNT_CUST_TYPE TO UCOUNT REDEEMED ON W7131037.dim_customer_master STARTED AT '||
  to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);


   update W7131037.dim_customer_master dc
   set ucount_cust_type = 'UCount Redeemed'
   where dc.master_subscriber_key in (
                         select /*+ Parallel(cbt,8) Full(cbt) Parallel(apx,4) Full(apx) Parallel(mp,8)  */
                                 distinct mp.subscriber_key --cbt.customer_no
                           from  W7131037.cust_basket_tender cbt
                           inner join
                                 W7131037.dim_customer_mapping mp
                                 on cbt.customer_no = mp.source_key
                           inner join
                                (
...
begin
  update /*+ PARALLEL(dim,4)*/ W7131037.dim_customer_master dim
   ...
   where dim.master_subscriber_key  in (select /*+ PARALLEL(cust,8) PARALLEL(mp,8) FULL(cust)*/
                                mp.subscriber_key
                           from W7131037.cust_interaction cust
                           inner join
                                 W7131037.dim_customer_mapping mp
                                 on account_contact_id = mp.source_key

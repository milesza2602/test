-- ****** Object: Procedure W7131037.WH_PRF_CUST_298U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_298U" (p_forall_limit in integer,p_success out boolean) AS
...
--  Tables:      Input  - temp_db_bu_month_involve
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_298U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CUST_DB_BU_MONTH_INVOLVE EX TEMP_DB_BU_MONTH_INVOLVE';
...
begin
    ...
    l_text := 'LOAD OF CUST_DB_BU_MONTH_INVOLVE EX temp_db_bu_month_involve STARTED AT '||
...
--**************************************************************************************************
   ...
   g_stmt      := 'Alter table  W7131037.CUST_DB_BU_MONTH_INVOLVE truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
   l_text      := g_stmt;
...
--    execute immediate g_stmt;
  /* g_run_date := g_this_mn_end_date + 10;
   ...
   insert /*+ APPEND parallel (prf,4) */ into cust_db_bu_month_involve prf
    with     bskt as
    (
   ...
   select /*+ FULL(prf)  parallel (prf,4) parallel (bskt,4) */
            ...
            nvl(prf.business_unit_no,bskt.business_unit_no) business_unit_no,
            ...
            g_date
   from     cust_db_bu_month_involve prf
   full outer join
            bskt
   on       prf.primary_customer_identifier = bskt.primary_customer_identifier
   and      PRF.BUSINESS_UNIT_NO            = BSKT.BUSINESS_UNIT_NO

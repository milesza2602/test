-- ****** Object: Procedure W7131037.WH_PRF_CUST_180U1 Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_180U1" (p_forall_limit in integer,p_success out boolean) as
...
select   /*+ Full(cbcnt)   Parallel(cbcnt,8) */
         cbcnt.location_no,
         sum ( case
                when cbcnt.loyalty_ww_swipe_no     is not null then
                     1
                else
                     0
...
select   /*+ full(bk) full(bi) full(di) Parallel(bk,8) Parallel(bi,8) */
         ...
         sum ( case
                when bk.loyalty_ww_swipe_no     is not null then
...
--    g_stmt   := 'Alter table  W7131037.cust_wod_reward_swipe_cnt_wk truncate  subpartition for ('||g_last_mn_fin_year_no||','||g_last_mn_fin_month_no||') update global indexes';
--    l_text   := g_stmt;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--    execute immediate g_stmt;
    ...
    commit;
--    DBMS_STATS.gather_table_stats ('W7131037','CUST_WOD_REWARD_SWIPE_CNT_WK',estimate_percent=>1, DEGREE => 32);
--    commit;

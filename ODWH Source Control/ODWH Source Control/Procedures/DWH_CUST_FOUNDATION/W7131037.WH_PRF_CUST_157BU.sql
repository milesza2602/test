-- ****** Object: Procedure W7131037.WH_PRF_CUST_157BU Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_157BU" (p_forall_limit in integer,p_success out boolean) AS
...
from   dim_customer a
WHERE     a.last_transaction_date = '30 SEP 2015' ;  -------FOR TESTING ---------
...
--WHAT ACCOUNTS DO I HAVE
  for dp_rec in c_dim_portfolio
    loop
    if  dp_rec.portfolio_status_desc <> 'Closed' then
    case dp_rec.product_code_no
        when 1    then g_rec_out.storcard_ind       := 1 ;
        when 2    then g_rec_out.storcard_ind       := 1 ;
        when 3    then g_rec_out.storcard_ind       := 1 ;
        when 4    then g_rec_out.storcard_ind       := 1 ;
        when 6    then g_rec_out.storcard_ind       := 1 ;
        when 9    then g_rec_out.storcard_ind       := 1 ;
        when 21   then g_rec_out.storcard_ind       := 1 ;
        when 80   then g_rec_out.storcard_ind       := 1 ;
        when 20   then g_rec_out.creditcard_ind     := 1 ;
        when 28   then g_rec_out.differencecard_ind := 1 ;
        when 30   then g_rec_out.differencecard_ind := 1 ;
        when 19   then g_rec_out.myschool_ind       := 1 ;
        when 92   then g_rec_out.vitality_ind       := 1 ;
        when 99   then g_rec_out.littleworld_ind    := 1 ;
        when 24   then g_other_open                 := 1 ;
     end case;
     end if;
    end loop;
...
--STORE_OTB
   begin
   select  NVL(sum(credit_limit - current_balance),0)    storecard_otb, count(*),max(chgoff_date)
   ...
   and     product_code_no in ('001','002','003','004','006','009','021','080');
...
--DECEASED,DELINQUENT AND DEBT REVIEW
   ...
   and     g_rec_in.wfs_customer_no = wfs_customer_no
   and     product_code_no in ('001','002','003','004','006','009','021','080')
   and     block_code_1 in ('D','K');
   exception
            when no_data_found then
               g_rec_out.deceased_ind                  := g_rec_out.deceased_ind ;   -- dont change as it may already be = 1 ex ABSA crd_daily
   end;

   begin
   select  distinct 1 as delinquent_ind
   into    g_rec_out.delinquent_ind
   from    fnd_wfs_cust_perf_60dy
   where   run_date = g_max_run_date
   and     g_rec_in.wfs_customer_no = wfs_customer_no
   and     product_code_no in ('001','002','003','004','006','009','021','080')
   and     delinquency_cycle not in ('0','1','2','3')
   and     account_status in ('A','D','I');
   exception
            when no_data_found then
               g_rec_out.delinquent_ind                 := g_rec_out.delinquent_ind ;    -- dont change as it may already be = 1 ex ABSA crd_daily
   end;
   ...
   and     g_rec_in.wfs_customer_no = wfs_customer_no
   and     product_code_no in ('001','002','003','004','006','009','021','080')
   and     block_code_1 in ('T','U','Q')
   and     account_status not in ('A','D','I');
   exception
            when no_data_found then
               g_rec_out.debt_review_ind                := g_rec_out.debt_review_ind ;   -- dont change as it may already be = 1 ex ABSA crd_daily
   end;
...
-- PROCESS STORE CARD TO SEE IF CHARGED OFF
    if g_stcard_open     = 1 then
      for dp_rec1 in c_dim_portfolio_choff
        loop
         ...
         and     g_rec_in.wfs_customer_no = wfs_customer_no
         and     dp_rec1.product_no       = wfs_account_no
         and     dp_rec1.product_code_no  = product_code_no
       ...
       if  g_chgoff_count = g_open_count then
           g_stcard_open    := 0;
       end if;
    end if;
  end if;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert /*+ APPEND parallel (4) */ into cust_talktome values a_tbl_insert(i);

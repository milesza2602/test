-- ****** Object: Procedure W7131037.WH_PRF_CUST_297U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_297U" (p_forall_limit in integer,p_success out boolean) AS
...
--  Tables:      Input  - temp_db_group_month_involve
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_297U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CUST_DB_GROUP_MONTH_INVOLVE EX TEMP_DB_GROUP_MONTH_INVOLVE';
...
begin
    ...
    l_text := 'LOAD OF CUST_DB_GROUP_MONTH_INVOLVE EX TEMP_DB_GROUP_MONTH_INVOLVE STARTED AT '||
...
--**************************************************************************************************
   ...
   g_stmt      := 'Alter table  W7131037.CUST_DB_GROUP_MONTH_INVOLVE truncate  subpartition for ('||g_yr_00||','||g_mn_00||') update global indexes';
   l_text      := g_stmt;
...
--    execute immediate g_stmt;
  /* g_run_date := g_this_mn_end_date + 10;
   ...
   insert /*+ APPEND parallel (prf,4) */ into cust_db_group_month_involve prf
    with     bskt as
    (
      select   /*+ FULL(fnd) parallel (fnd,4) */
          ...
          sales_sum ,
          involvement_score
     from temp_db_group_month_involve
   )
   select /*+ FULL(prf)  parallel (prf,4) parallel (bskt,4) */
            ...
            nvl(prf.group_no,bskt.group_no) group_no,

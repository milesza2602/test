-- ****** Object: Procedure W7131037.WH_PRF_CUST_375E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_375E" 
...
--  Tables:      Input  - DIM_ITEM, DIM_SUPPLIER, RTL_LOC_ITEM_WK_RMS_DENSE
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_375E';
...
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT E20 PERSONALISATION INTERFACE (SUPPLIER) TO FLAT FILE';
...
begin
    ...
    l_text := 'EXTRACT FOR E20 PERSONALISATION INTERFACE-SUPPLIER STARTED AT '||
...
--**************************************************************************************************
    ...
    g_count := dwh_performance.dwh_generic_file_extract(
         q'[select /* Parallel (b,4) Full(b) */
                   sk1_supplier_no,
                   supplier_no,
                   supplier_name,
                   parent_supplier_no,
                   last_active_date
              from dwh_performance.dim_supplier@dwhprd b
             where supplier_no in (select /* Parallel (a,4) Full(a) */
                                          primary_supplier_no
                                     from dwh_performance.dim_item@dwhprd a
                                    where business_unit_no in (50,51,52,54,55)
                                  )]','|','DWH_FILES_OUT','e20_cust_supplier.txt');
    l_text :=  'Records extracted to E20_CUST_SUPPLIER '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

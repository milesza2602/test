-- ****** Object: Procedure W7131037.WH_PRF_CUST_230U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_230U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        April 2011
--  Author:      Alastair de Wet
--  Purpose:     Roll up for 3,6 & 12 months values
--  Tables:      Input  - cust_db_subgroup_mn
--               Output - temp_cust_subgroup_mn_1
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_230U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE temp_cust_subgroup_mn_1 EX cust_db_subgroup_mn';
...
begin
    ...
    l_text := 'LOAD OF temp_cust_subgroup_mn_1 EX cust_db_subgroup_mn STARTED AT '||
...
--**************************************************************************************************
   ...
   l_text := 'Months being processed:= '||
             g_yr_06||g_mn_06||' '||
             g_yr_07||g_mn_07||' '||g_yr_08||g_mn_08||' '||
             g_yr_09||g_mn_09||' '||g_yr_10||g_mn_10||' '||g_yr_11||g_mn_11;

   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   execute immediate 'truncate table W7131037.temp_cust_subgroup_mn_1';
   l_text := 'Truncate temp table  W7131037.temp_cust_subgroup_mn_1';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   insert into temp_cust_subgroup_mn_1
   ...
   from     cust_db_subgroup_mn
   where    (fin_year_no = g_yr_07 and fin_month_no = g_mn_07) or (fin_year_no = g_yr_08 and fin_month_no = g_mn_08) or
            (fin_year_no = g_yr_09 and fin_month_no = g_mn_09) or (fin_year_no = g_yr_10 and fin_month_no = g_mn_10) or
            (fin_year_no = g_yr_11 and fin_month_no = g_mn_11) or (fin_year_no = g_yr_06 and fin_month_no = g_mn_06)

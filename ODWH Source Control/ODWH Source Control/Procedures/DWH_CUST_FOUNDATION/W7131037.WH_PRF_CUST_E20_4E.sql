-- ****** Object: Procedure W7131037.WH_PRF_CUST_E20_4E Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_E20_4E" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_E20_4E';
...
l_description        sys_dwh_log_summary.log_description%type  := 'EXTRACT E20 PERSONALISATION INTERFACE (CUST_BASKET_ITEM) TO FLAT FILE';
...
begin
    ...
    l_text := 'EXTRACT FOR E20 PERSONALISATION INTERFACE-CUST_BASKET_ITEM STARTED AT '||
...
--**************************************************************************************************
    ...
    select fin_year_no,
           ...
           this_mn_start_date,
           this_mn_end_date,
           NVL((select seq_num+1 from "W7131037"."KEYSTONE" where program_name = 'WH_PRF_CUST_E20_4E'),1) seq_num,
           'WH_PRF_CUST_E20_4E' program_name,
           this_mn_start_date curr_date
      into l_period
      from dim_calendar
     where calendar_date = NVL((select this_mn_start_date-1 START_DATE from "W7131037"."KEYSTONE" where program_name = 'WH_PRF_CUST_E20_4E'),trunc(sysdate)-20 );

     l_text := 'PROCESSING SEQUENCE NO '||l_period.seq_num||' FOR PERIOD: '||l_period.this_mn_start_date||' TO '||l_period.this_mn_end_date;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    dbms_output.put_line('CUST_BASKET_ITEM');
    g_count := dwh_performance.dwh_generic_file_extract(
         q'[select /*+ Parallel(cbi,6) Full(cbi) */ distinct
                    *
              from W7131037.temp_e20_basket_item cbi
             where tran_date between (select this_mn_start_date from W7131037.keystone where program_name = 'WH_PRF_CUST_E20_4E')
               and                   (select this_mn_end_date from W7131037.keystone where program_name = 'WH_PRF_CUST_E20_4E')
       ]','|','DWH_FILES_OUT','e20_basket_item_'||l_period.fin_year_no||l_period.fin_month_no||'.txt');
        l_text :=  'Records extracted to E20_BASKET_ITEM '||g_count;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

-- ****** Object: Procedure W7131037.WH_PRF_CUST_235U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_235U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        April 2011
--  Author:      Alastair de Wet
--  Purpose:     Create cust_db_subgroup_mn rollup ex temp tables
--  Tables:      Input  - temp_cust_subgroup_mn_1,2,3,c
--               Output - cust_db_subgroup_mn
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_235U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'CREATE cust_db_subgroup_mn EX temp_cust_subgroup_mn table';
...
begin
    ...
    l_text := 'LOAD OF cust_db_subgroup_mn   EX temp_cust_subgroup_mn tables STARTED AT '||
...
--  truncate the current month partition instead of deleting if put into production.
    ...
    g_partition_name := 'CST_SGRP_MN'||'_'||g_month_code;
    g_stmt           := 'alter table '||'cust_db_subgroup_mn'||' truncate SUBPARTITION '||g_partition_name;
...
-------------------------------
--    delete from cust_db_subgroup_mn
--    where fin_year_no  = g_yr_00 and
--          fin_month_no = g_mn_00;

--   execute immediate 'truncate table W7131037.cust_db_subgroup_mn'; ---For testing only---
--   l_text := 'Delete from table  W7131037.cust_db_subgroup_mn' ;
--   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--*********************************************************************************************

   insert into cust_db_subgroup_mn
   with cust_dept_union as
   (
   select * from temp_cust_subgroup_mn_1
   union all
   select * from temp_cust_subgroup_mn_2
   union all
   select * from temp_cust_subgroup_mn_3
   union all
   select * from temp_cust_subgroup_mn_c
   )
   ...
   group by fin_year_no,fin_month_no,primary_customer_identifier,subgroup_no;

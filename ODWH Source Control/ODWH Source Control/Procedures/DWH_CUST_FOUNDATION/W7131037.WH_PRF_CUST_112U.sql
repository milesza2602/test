-- ****** Object: Procedure W7131037.WH_PRF_CUST_112U Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_112U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        July 2016
--  Author:      Mariska Matthee
--  Purpose:     Update dim_customer with additional information
--
...
--
--  Maintenance:
--  November 2016 - Mariska Matthee - Swop W7131037.apex_ap3_param_1 for apex_app_cust_01.apex_ap3_param_1
--  May 2017      - Theo Filander   - Remove the filters on the dates. Only use the last 2 campaigns. BCB-15
--
--  Naming conventions
--  g_  -  Global variable
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_112U';
...
begin
  update /*+ PARALLEL(dim,4)*/ W7131037.dim_customer dim
...
begin
  l_text := 'UPDATE UCOUNT_CUST_TYPE TO UCOUNT CUSTOMER ON W7131037.DIM_CUSTOMER STARTED AT '||
  to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

  update /*+ PARALLEL(dim,4)*/ W7131037.dim_customer dim
     set ucount_cust_type = 'UCount Potential'
   where customer_no in (select /*+ PARALLEL(cbt,4) FULL(cbt) */
                                customer_no
                           from W7131037.cust_basket_tender cbt
                          where cbt.tran_date between add_months(g_date,-6) and g_date
  ...
  l_text := 'UPDATE UCOUNT_CUST_TYPE TO UCOUNT REDEEMED ON W7131037.DIM_CUSTOMER STARTED AT '||
  to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

-- Theo Filander 23 May 2017
-- Query no longer valid.
-- BCB-15: Remove filters on date ranges. Only use the last 2 campaigns.
--
--update /*+ PARALLEL(dim,4)*/ W7131037.dim_customer dim
--   set ucount_cust_type = 'UCount Redeemed'
-- where customer_no in (select /*+ PARALLEL(cbt,4) PARALLEL(cam,4) FULL(cbt) FULL(cam) */
--                              customer_no
--                         from W7131037.cust_basket_tender cbt
--                        inner join apex_app_cust_01.apex_ap3_param_1 cam
--                           on cbt.tran_date between cam.start_measurement_date and cam.end_measurement_date and
--                              substr(cam.pir_no,1,2) = 'UC' and
--                              cam.start_measurement_date <= g_date and
--                              cam.end_measurement_date >= add_months(g_date,-18)
--                        where floor(cbt.tender_no/10000000000) in (533823)
--                          and cbt.customer_no is not null
--                          and cbt.tran_date between add_months(g_date,-6) and g_date)
--   and ucount_cust_type = 'UCount Customer';

  update W7131037.dim_customer dc
     set ucount_cust_type = 'UCount Redeemed'
   where customer_no in (
                         select /*+ Parallel(cbt,4) Full(cbt) Parallel(apx,4) Full(apx) Parallel(dc,4) Full(dc)  */
                                distinct cbt.customer_no
                           from W7131037.cust_basket_tender cbt
                           inner join
                                (
                                 select /*+ Parallel(aa,4) Full(aa) */
                                  ...
                                  where substr(aa.campaign_description,1,2) = 'UC'
                                    and aa.end_measurement_date <=trunc(sysdate)
...
--     and ucount_cust_type = 'UCount Customer'; Removed 23/May/17 As per
...
begin
  update /*+ PARALLEL(dim,4)*/ W7131037.dim_customer dim
   ...
   where customer_no in (select /*+ PARALLEL(cust,4) FULL(cust)*/
                                account_contact_id
                           from W7131037.cust_interaction cust

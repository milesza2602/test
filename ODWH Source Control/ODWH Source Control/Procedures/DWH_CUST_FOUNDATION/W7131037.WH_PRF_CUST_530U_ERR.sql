-- ****** Object: Procedure W7131037.WH_PRF_CUST_530U_ERR Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_530U_ERR" (p_forall_limit in integer,p_success out boolean) AS
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_530U_ERR';
...
---------------------------------------------------------
   if g_rec_in.inq_type_level_ind = 2 then
      begin
         ...
         exception
                     when no_data_found then
                        l_text := 'Inquiry Type Number1 not found '||g_rec_in.inq_type_no;
                        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
                        dbms_output.put_line('Inquiry Type Number1 not found '||g_rec_in.inq_type_no);
      end;
      ...
      begin
         ...
         g_rec_in.inq_type_no := g_rec_in.inq_type_no + 99990000;
            exception
                     when no_data_found then
                        l_text := 'Inquiry Type Number2 not found '||g_rec_in.inq_type_no;
                        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
                        dbms_output.put_line('Inquiry Type Number2 not found '||g_rec_in.inq_type_no);
      end;
   end if;
   ...
   if g_rec_in.inq_type_level_ind = 1 then
      begin
         ...
         g_rec_in.inq_type_no := g_rec_in.inq_type_no + 99990000;
          exception
                   when no_data_found then
                      l_text := 'Inquiry Type Number3 not found '||g_rec_in.inq_type_no;
                      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
                      dbms_output.put_line('Inquiry Type Number3 not found '||g_rec_in.inq_type_no);
      end;
   end if;
  ...
  begin
   ...
   exception
               when no_data_found then
                  l_text := 'Inquiry Type Number4 not found '||g_rec_in.inq_type_no;
                  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
                  dbms_output.put_line('Inquiry Type Number4 not found '||g_rec_in.inq_type_no);
    end;
---------------------------------------------------------------------------------
   ...
   exception
               when no_data_found then
                  dbms_output.put_line('Supplier Number not found '||g_rec_in.supplier_no);
  end;
----------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------
   ...
   exception
               when no_data_found then
                  dbms_output.put_line('Department Number not found '||g_rec_in.department_no);
  end;
-----------------------------------------------------------------------------------

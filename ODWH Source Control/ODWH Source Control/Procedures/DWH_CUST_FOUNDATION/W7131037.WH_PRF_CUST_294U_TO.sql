-- ****** Object: Procedure W7131037.WH_PRF_CUST_294U_TO Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_294U_TO" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        May 2016
--  Author:      Theo Filander
--  Purpose:     Create Company Involvement Scores for the current month using Depth and Breath Tables
--  Tables:      Input  - cust_db_company_month
--                      - dim_calendar
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_294U_TO';
...
--**************************************************************************************************
   ...
   l_text := 'CUST_DB_COMPANY_MONTH data being processed is Financial Year  : '|| g_year_no|| ' Month  ' || g_month_no;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   l_text := 'CUST_DB_COMPANY_MONTH_INVOLVE data being rolled is Financial Year  '|| g_prev_year_no|| ' Month  ' || g_prev_month_no;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    g_stmt := 'TRUNCATE table W7131037.TEMP_CUST_COMP_SALE_RANKING';
    execute immediate g_stmt;

    g_stmt := 'TRUNCATE table W7131037.TEMP_CUST_COMP_ITEM_RANKING';
    execute immediate g_stmt;

    l_text := 'UPDATE STATS ON ALL TEMP TABLES';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    DBMS_STATS.gather_table_stats ('W7131037','TEMP_CUST_COMP_SALE_RANKING',estimate_percent=>1, DEGREE => 32);
    DBMS_STATS.gather_table_stats ('W7131037','TEMP_CUST_COMP_ITEM_RANKING',estimate_percent=>1, DEGREE => 32);
    COMMIT;

    l_text := 'Populate Sale Ranking for CUST_DB_COMPANY_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    INSERT INTO W7131037.TEMP_CUST_COMP_SALE_RANKING (fin_year_no,fin_month_no,company_no,sales_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                    ...
                    FROM W7131037.cust_db_company_month a
    ...
    l_text := 'Populate Item Ranking for CUST_DB_COMPANY_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    INSERT INTO W7131037.TEMP_CUST_COMP_ITEM_RANKING (fin_year_no,fin_month_no,company_no,item_sum,customer_item_cnt,customer_cnt,diff_customer_item_cnt,ranking,last_updated_date)
    WITH cust AS (SELECT /*+ PARALLEL(a,8) FULL(a) */
                        2016 fin_year_no,
                        12 fin_month_no,
                        company_no,
    ...
    l_text := 'Populate CUST_DB_COMPANY_MONTH_INVOLVE. ' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--**************************************************************************************************
-- Execute the insert
--**************************************************************************************************
    insert /*+ APPEND Parallel(a,8) */ into W7131037.cust_db_company_month_involve a
    select /*+ Parallel(cma,8) Parallel(cmb,8) Parallel(cmc,8) Parallel(cmd,8) Full(cma) Full(cmb) Full(cmc) Full(cmd) */
          ...
          from W7131037.cust_db_company_month cma
               left join
               W7131037.temp_cust_comp_item_ranking cmb on (cma.num_item_yr1_mn02 + cma.num_item_yr1_mn03 + cma.num_item_yr1_mn04+cma.num_item_yr1_mn05 + cma.num_item_yr1_mn06 +
                                                                         cma.num_item_yr1_mn07 + cma.num_item_yr1_mn08 + cma.num_item_yr1_mn09+cma.num_item_yr1_mn10 + cma.num_item_yr1_mn11 + cma.num_item_yr1_mn12 +
                                                                         cma.num_item_yr2_mn01) = cmb.item_sum AND
                                                                        cmb.fin_year_no = 2016 AND
                                                                        cmb.fin_month_no = 12 AND
                                                                        cma.company_no = cmb.company_no
               left join
               W7131037.temp_cust_comp_sale_ranking cmc on ROUND(cma.sales_yr1_mn02 + cma.sales_yr1_mn03 + cma.sales_yr1_mn04 + cma.sales_yr1_mn05 + cma.sales_yr1_mn06 +
                                                                             cma.sales_yr1_mn07 + cma.sales_yr1_mn08 + cma.sales_yr1_mn09 + cma.sales_yr1_mn10 + cma.sales_yr1_mn11 + cma.sales_yr1_mn12 +
                                                                             cma.sales_yr2_mn01) = cmc.sales_sum AND
                                                                        cmc.fin_year_no = 2016 AND
                                                                        cmc.fin_month_no = 12 AND
                                                                        cma.company_no = cmc.company_no
               left join
               W7131037.cust_db_company_month_involve cmd on cma.primary_customer_identifier = cmd.primary_customer_identifier AND
                                                                         cma.company_no = cmd.company_no and
   ...
   --l_text := 'UPDATE STATS ON CUST_DB_COMPANY_MONTH_INVOLVE TABLES';
   --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--   DBMS_STATS.gather_table_stats ('W7131037','CUST_DB_COMPANY_MONTH_INVOLVE',estimate_percent=>1, DEGREE => 32);

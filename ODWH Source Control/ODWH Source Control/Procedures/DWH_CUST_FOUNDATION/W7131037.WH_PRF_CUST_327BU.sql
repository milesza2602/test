-- ****** Object: Procedure W7131037.WH_PRF_CUST_327BU Script Date: 13/03/2019 04:41:17 PM ******
CREATE OR REPLACE PROCEDURE "W7131037"."WH_PRF_CUST_327BU" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        NOV 2015
--  Author:      Alastair de Wet
--  Purpose:     Create cust_mart_home_and_away_dept fact table in the performance layer
...
--               Output - cust_mart_home_and_away_dept
...
g_count              number        :=  0;
g_sub                integer       :=  0;
g_rec_out            cust_mart_home_and_away_dept%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_PRF_CUST_327U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'ROLL UP THE CUST_BASKET_ITEM to HOME48 DEPT MART';
...
begin
    ...
    l_text := 'ROLLUP OF cust_mart_home_and_away_dept EX cust_st_wk LEVEL STARTED AT '||
...
--    g_stmt      := 'Alter table  W7131037.cust_mart_home_and_away_dept truncate  subpartition for ('||g_yr_00||','||g_qt_00||') update global indexes';
--    l_text      := g_stmt;
--    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
insert /*+ APPEND parallel(csm,4)*/ into cust_mart_home_and_away_dept csm
     with     sop as  (
...
--              bih.location_no,
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no between 51 and 55
     group by bih.primary_customer_identifier, di.department_no, s2.location_no),
              away as  (
     ...
     and      tran_type          not in ('P','N','L','R','Q')
     and      di.business_unit_no between 51 and 55
     group by primary_customer_identifier,department_no,location_no)
...
and      home.department_no              = away.department_no

--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_150U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_150U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
-- Description: Load OM4 Application data
...
--  2018-10-26 S Ismail - Added field INCOME_ESTIMATOR_T102 and BLAZE_FLOW_ID for POI project
--
--
...
for upd_rec in c_stg_wfs_om4_omapp_dly
   loop
     ...
     where  fnd.credit_applications_id     = upd_rec.credit_applications_id and
            ...
            nvl(fnd. paper_app_created_by_user, 0) <> upd_rec. paper_app_created_by_user OR
            nvl(fnd. t102_predicted_income, 0) <> upd_rec. t102_predicted_income OR
...
--**************************************************************************************************
    ...
    l_text := 'LOAD TABLE: '||'FND_WFS_OM4_APPLICATION' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

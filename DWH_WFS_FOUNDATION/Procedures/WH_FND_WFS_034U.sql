--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_034U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_034U" (p_forall_limit in integer,p_success out boolean) AS 
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_vsn_rep_hsp.sys_process_msg%type;
g_rec_out            fnd_wfs_rep%rowtype;
g_rec_in             stg_vsn_rep_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
L_MODULE_NAME        SYS_DWH_ERRLOG.LOG_PROCEDURE_NAME%TYPE    := 'WH_FND_WFS_034U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE REP DIM EX VISION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_vsn_rep_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wfs_rep%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wfs_rep%rowtype index by binary_integer;
...
type staging_array1 is table of stg_vsn_rep_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_vsn_rep_cpy.sys_source_sequence_no%type 
                                                       index by binary_integer;
...
cursor c_stg_vsn_rep is
   select *
   from stg_vsn_rep_cpy
...
begin
   ...
   g_rec_out.rep_id                   := g_rec_in.rep_id;
   g_rec_out.rep_name                 := g_rec_in.rep_name;
   g_rec_out.rep_staff_level          := g_rec_in.rep_staff_level;
...
begin
   ...
   insert into stg_vsn_rep_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wfs_rep values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).rep_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);  
       end loop;   
       raise;        
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last 
       save exceptions
       update fnd_wfs_rep
       set    rep_name               = a_tbl_update(i).rep_name,
              rep_staff_level        = a_tbl_update(i).rep_staff_level ,
              dummy_ind              = a_tbl_update(i).dummy_ind ,
              last_updated_date      = a_tbl_update(i).last_updated_date
       where  rep_id                 = a_tbl_update(i).rep_id;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).rep_id;
          dwh_log.record_error(l_module_name,sqlcode,l_message);  
       end loop;   
       raise;        
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).rep_id  = g_rec_out.rep_id  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin 
    ...
    l_text := 'LOAD OF fnd_wfs_rep EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_vsn_rep;
    fetch c_stg_vsn_rep bulk collect into a_stg_input limit g_forall_limit;

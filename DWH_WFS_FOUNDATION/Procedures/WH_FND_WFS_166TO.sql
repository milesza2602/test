--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_166TO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_166TO" (
...
AS
  ...
  -- Purpose:     Create fnd_wfs_om4_pa_expense table in the foundation layer
  --              with input ex staging table from WFS.
  -- Tables:      Input  - stg_om4_pa_expense
  --              Output - fnd_wfs_om4_pa_expense
  ...
  g_pa_c_id stg_om4_pa_expense.credit_applications_id%type;
  g_pa_p_id stg_om4_pa_expense.personal_applicant_id%type;
  g_pa_exp_id stg_om4_pa_expense.expense_id%type;
  
  
  g_date DATE := TRUNC(sysdate);
  
  L_MESSAGE SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_WFS_166U';
  l_name sys_dwh_log.log_name%type                     := dwh_constants.vc_log_name_rtl_facts;
  l_system_name sys_dwh_log.log_system_name%type       := dwh_constants.vc_log_system_name_rtl_fnd;
  l_script_name sys_dwh_log.log_script_name%type       := dwh_constants.vc_log_script_rtl_fnd_facts;
  l_procedure_name sys_dwh_log.log_procedure_name%type := l_module_name;
  L_TEXT SYS_DWH_LOG.LOG_TEXT%TYPE ;
  l_description sys_dwh_log_summary.log_description%type   := 'LOAD WFS PA EXPENSE DATA';
  l_process_type sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;
  CURSOR stg_dup
IS
    SELECT /*+ PARALLEL(8) */ *
    FROM stg_om4_pa_expense
    WHERE (credit_applications_id,personal_applicant_id,expense_id) IN
      (SELECT credit_applications_id,personal_applicant_id,expense_id
      FROM stg_om4_pa_expense
      GROUP BY credit_applications_id,personal_applicant_id,expense_id
      HAVING COUNT(*) > 1
      )
  ORDER BY credit_applications_id,personal_applicant_id,expense_id,
  ...
  IS
    SELECT
      ...
      stg.*
    FROM stg_om4_pa_expense stg,
      fnd_wfs_om4_pa_expense fnd
    WHERE stg.credit_applications_id = fnd.credit_applications_id
    and   stg.personal_applicant_id  = fnd.personal_applicant_id
    and   stg.expense_id             = fnd.expense_id
    AND stg.sys_process_code    = 'N'
      -- Any further validation goes in here - like xxx.ind in (0,1) ---
    ORDER BY stg.credit_applications_id,stg.personal_applicant_id,stg.expense_id,
...
BEGIN
  ...
  LOOP
    IF dupp_record.credit_applications_id = g_pa_c_id and
       ...
       dupp_record.expense_id             = g_pa_exp_id THEN
      UPDATE stg_om4_pa_expense stg
    ...
    g_pa_exp_id := dupp_record.expense_id;
  END LOOP;
  COMMIT;
EXCEPTION
WHEN OTHERS THEN
...
BEGIN
  ...
  INSERT
    /*+ APPEND parallel (fnd,4) */
  INTO fnd_wfs_om4_pa_expense fnd
  SELECT
    /*+ FULL(cpy)  parallel (cpy,4) */
...
BEGIN
  ...
  LOOP    
    UPDATE fnd_wfs_om4_pa_expense fnd
    SET fnd. personal_applicant_id         = upd_rec. personal_applicant_id ,
      fnd. expense_id                            = upd_rec. expense_id ,      
      fnd. expense_type             = upd_rec. expense_type ,    
      fnd. monthly_payment                           = upd_rec. monthly_payment ,   
    ...
    and   fnd.expense_id                    = upd_rec.expense_id;

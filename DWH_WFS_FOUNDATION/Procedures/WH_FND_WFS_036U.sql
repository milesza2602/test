--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_036U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_036U" (p_forall_limit in integer,p_success out boolean) AS 
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_vsn_tran_hsp.sys_process_msg%type;
g_rec_out            fnd_wfs_tran%rowtype;
g_rec_in             stg_vsn_tran_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
L_MODULE_NAME        SYS_DWH_ERRLOG.LOG_PROCEDURE_NAME%TYPE    := 'WH_FND_WFS_036U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE TRAN DIM EX VISION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_vsn_tran_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wfs_tran%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wfs_tran%rowtype index by binary_integer;
...
type staging_array1 is table of stg_vsn_tran_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_vsn_tran_cpy.sys_source_sequence_no%type 
                                                       index by binary_integer;
...
cursor c_stg_vsn_tran is
   select *
   from stg_vsn_tran_cpy
...
begin
   ...
   g_rec_out.tran_desc                := g_rec_in.tran_desc;
   g_rec_out.tran_code_desc           := g_rec_in.tran_code_desc;
...
begin
   ...
   insert into stg_vsn_tran_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wfs_tran values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last 
       save exceptions
       UPDATE fnd_wfs_tran
       SET    tran_desc                 = a_tbl_update(i).tran_desc,
              tran_code_desc            = a_tbl_update(i).tran_code_desc ,
              dummy_ind                 = a_tbl_update(i).dummy_ind ,
              last_updated_date         = a_tbl_update(i).last_updated_date
       where  tran_code                 = a_tbl_update(i).tran_code;
...
-- Check to see if present on table and update/insert accordingly
   ...
   into   g_count
   from   fnd_wfs_tran
   where  tran_code = g_rec_out.tran_code;
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).tran_code  = g_rec_out.tran_code  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin 
    ...
    l_text := 'LOAD OF fnd_wfs_tran EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_vsn_tran;
    fetch c_stg_vsn_tran bulk collect into a_stg_input limit g_forall_limit;

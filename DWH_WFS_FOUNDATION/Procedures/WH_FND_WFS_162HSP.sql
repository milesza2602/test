--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_162HSP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_162HSP" (
...
AS
  ...
  -- Tables:      Input  - stg_om4_personal_applicant
  ...
  g_pa_id stg_om4_personal_applicant.personal_applicant_id%type;
...
IS
    SELECT *
    FROM stg_om4_personal_applicant
  ...
  IS
    SELECT
      ...
      stg.*
    FROM stg_om4_personal_applicant stg,
...
BEGIN
  ...
  LOOP
    IF dupp_record.personal_applicant_id = g_pa_id THEN
      UPDATE stg_om4_personal_applicant stg
...
BEGIN
  ...
  FROM stg_om4_personal_applicant cpy
  WHERE NOT EXISTS
    (SELECT
      /*+ nl_aj */
      *
...
--    AND   UPD_REC.SYS_LOAD_DATE > FND.LAST_UPDATED_DATE
    AND ( 
      ...
      NVL(fnd.	not_contacted_for_marketing	,0) <>	upd_rec.	not_contacted_for_marketing	);

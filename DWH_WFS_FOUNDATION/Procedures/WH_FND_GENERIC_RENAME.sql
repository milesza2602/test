...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_GENERIC_RENAME" ( p_table_name VARCHAR2,
...
--               2016-08-16 N Chauhan - Tables with CLOB/BLOB data types now accommodated.
--
--   
--  Naming conventions
--  g_  -  Global variable
...
and index_type not in ('LOB');                -- This line added to accommodate CLOB/BLOB data types
...
--
    ...
    g_stmt := 'GRANT SELECT,UPDATE,INSERT,DELETE ON '|| p_table_name||
              ' TO DWH_AIT_ETL';
--
    l_text := g_stmt||' PERMISSIONS GRANTED DWH_AIT_ETL '||
    to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--
    ...
    dbms_stats.gather_table_stats('DWH_WFS_FOUNDATION',g_cpy_table_name);
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_OUTPUT.PUT_LINE('24=');
    
    l_text := 'LOCK TABLE STATS on '||  g_cpy_table_name;
    dbms_stats.lock_table_stats('DWH_WFS_FOUNDATION',g_cpy_table_name);
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_OUTPUT.PUT_LINE('24A=');        

--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_116U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_116U" (p_forall_limit in integer,p_success out boolean) AS 
...
--  Tables:      Input  - stg_absa_sbu_sub_segment_cpy
--               Output - fnd_wfs_sbu_sub_segment
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_absa_sbu_sub_segment_hsp.sys_process_msg%type;
g_rec_out            fnd_wfs_sbu_sub_segment%rowtype;
g_rec_in             stg_absa_sbu_sub_segment_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
L_MODULE_NAME        SYS_DWH_ERRLOG.LOG_PROCEDURE_NAME%TYPE    := 'WH_FND_WFS_116U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE SBU SUB SEGMENT DIM EX ABSA';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_absa_sbu_sub_segment_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wfs_sbu_sub_segment%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wfs_sbu_sub_segment%rowtype index by binary_integer;
...
type staging_array1 is table of stg_absa_sbu_sub_segment_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_absa_sbu_sub_segment_cpy.sys_source_sequence_no%type 
                                                       index by binary_integer;
...
cursor c_stg_absa_sbu_sub_segment is
   select *
   from stg_absa_sbu_sub_segment_cpy
...
begin
   ...
   g_rec_out.sbu_sub_segment_code                 := g_rec_in.sbu_sub_segment_code;
   g_rec_out.sbu_sub_segment_desc                 := g_rec_in.sbu_sub_segment_desc;
...
begin
   ...
   insert into stg_absa_sbu_sub_segment_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wfs_sbu_sub_segment values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).sbu_sub_segment_code;
          dwh_log.record_error(l_module_name,sqlcode,l_message);  
       end loop;   
       raise;        
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last 
       save exceptions
       update fnd_wfs_sbu_sub_segment
       set    sbu_sub_segment_desc                 = a_tbl_update(i).sbu_sub_segment_desc,
              last_updated_date                             = a_tbl_update(i).last_updated_date
       where  sbu_sub_segment_code                 = a_tbl_update(i).sbu_sub_segment_code;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_cust_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).sbu_sub_segment_code;
          dwh_log.record_error(l_module_name,sqlcode,l_message);  
       end loop;   
       raise;        
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_absa_sbu_sub_segment_cpy      
...
-- Check to see if present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_wfs_sbu_sub_segment
   where  sbu_sub_segment_code = g_rec_out.sbu_sub_segment_code;
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).sbu_sub_segment_code  = g_rec_out.sbu_sub_segment_code  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin 
    ...
    l_text := 'LOAD OF fnd_wfs_sbu_sub_segment EX ABSA STARTED AT '||
...
--**************************************************************************************************
    open c_stg_absa_sbu_sub_segment;
    fetch c_stg_absa_sbu_sub_segment bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_absa_sbu_sub_segment;
--**************************************************************************************************  

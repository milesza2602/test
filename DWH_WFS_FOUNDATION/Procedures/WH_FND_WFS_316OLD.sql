--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_316OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_316OLD" (p_forall_limit in integer,p_success out boolean) as
...
g_information_date                 stg_absa_crd_txn_dly_cpy.information_date%type;  
g_account_number                   stg_absa_crd_txn_dly_cpy.account_number%type; 
g_txn_date                         stg_absa_crd_txn_dly_cpy.txn_date%type;  
...
cursor stg_dup is
select * from stg_absa_crd_txn_dly_cpy
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.information_date        = g_information_date and
        ...
        update stg_absa_crd_txn_dly_cpy stg
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (fnd,2) */ into fnd_wfs_crd_txn_dly fnd
      ...
      where  not exists 
      (select /*+ nl_aj */ * from fnd_wfs_crd_txn_dly 
...
for upd_rec in c_stg_absa_crd_txn_dly
   loop
     update fnd_wfs_crd_txn_dly fnd 
...
begin
      ...
      insert /*+ APPEND parallel (hsp,2) */ into stg_absa_crd_txn_dly_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */ 
...
--**************************************************************************************************
    ...
    DELETE FROM fnd_wfs_crd_txn_dly WHERE INFORMATION_DATE >= G_DELETE_DATE;
    
    g_recs_deleted := g_recs_deleted + sql%rowcount;
    
--    select trunc(sysdate) - (to_char(sysdate,'DD') - 1) from dual;
...
--    update stg_absa_crd_txn_dly_cpy

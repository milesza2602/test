--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_032U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_032U" (p_forall_limit in integer,p_success out boolean) AS 
...
--  Tables:      Input  - stg_vsn_product_cpy
--               Output - fnd_wfs_product
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_vsn_product_hsp.sys_process_msg%type;
g_rec_out            fnd_wfs_product%rowtype;
g_rec_in             stg_vsn_product_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
L_MODULE_NAME        SYS_DWH_ERRLOG.LOG_PROCEDURE_NAME%TYPE    := 'WH_FND_WFS_032U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE PRODUCT DIM EX VISION';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_vsn_product_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wfs_product%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wfs_product%rowtype index by binary_integer;
...
type staging_array1 is table of stg_vsn_product_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_vsn_product_cpy.sys_source_sequence_no%type 
                                                       index by binary_integer;
...
cursor c_stg_vsn_product is
   select *
   from stg_vsn_product_cpy
...
begin
   ...
   g_rec_out.product_group_no                := g_rec_in.product_group_no;
   g_rec_out.product_group_desc              := g_rec_in.product_group_desc;
...
begin
   ...
   insert into stg_vsn_product_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wfs_product values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last 
       save exceptions
       update fnd_wfs_product
       set    product_code_desc               = a_tbl_update(i).product_code_desc,
              product_group_no                = a_tbl_update(i).product_group_no,
              product_group_desc              = a_tbl_update(i).product_group_desc,
...
-- Check to see if present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_wfs_product
   where  product_code_no = g_rec_out.product_code_no;
...
begin 
    ...
    l_text := 'LOAD OF fnd_wfs_product EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_vsn_product;
    fetch c_stg_vsn_product bulk collect into a_stg_input limit g_forall_limit;

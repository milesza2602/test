--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_074U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_074U" (p_forall_limit in integer,p_success out boolean) AS 
...
--  Tables:      Input  - stg_absa_country_cpy
--               Output - fnd_wfs_country
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_absa_country_hsp.sys_process_msg%type;
g_rec_out            fnd_wfs_country%rowtype;
g_rec_in             stg_absa_country_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
...
L_MODULE_NAME        SYS_DWH_ERRLOG.LOG_PROCEDURE_NAME%TYPE    := 'WH_FND_WFS_074U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE COUNTRY DIM EX ABSA';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_cust_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_absa_country_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_wfs_country%rowtype index by binary_integer;
type tbl_array_u is table of fnd_wfs_country%rowtype index by binary_integer;
...
type staging_array1 is table of stg_absa_country_cpy.SYS_SOURCE_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_absa_country_cpy.SYS_SOURCE_sequence_no%type 
                                                       index by binary_integer;
...
cursor c_stg_absa_country is
   select *
   from stg_absa_country_cpy
...
begin
   ...
   insert into stg_absa_country_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_wfs_country values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last 
       save exceptions
       update fnd_wfs_country
       set    country_desc      = a_tbl_update(i).country_desc,
...
begin 
    ...
    l_text := 'LOAD OF fnd_wfs_country EX CUST CENTRAL STARTED AT '||
...
--**************************************************************************************************
    open c_stg_absa_country;
    fetch c_stg_absa_country bulk collect into a_stg_input limit g_forall_limit;

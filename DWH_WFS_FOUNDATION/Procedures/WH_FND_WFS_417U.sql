--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_417U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_417U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
-- Description: Load Credit Limit Increase (CLI) data
-- Tran_type: ODWH: ¿CLIOFN   AIT: ¿CLIOFFN
--
-- Date:       2016-06-14
-- Author:      Naresh Chauhan
-- Purpose:     update table FND_WFS_CLI_OFFR_NOTE in the foundation layer
--              with input ex staging table from WFS.
-- Tables:      Input  - STG_C2_CLI_OFFR_NOTE_CPY
--              Output - FND_WFS_CLI_OFFR_NOTE
...
g_unique_key_field_val       dwh_wfs_foundation.stg_c2_cli_offr_note_cpy.cli_offer_note_no%type;

g_date               date          := trunc(sysdate);

L_MESSAGE            SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_WFS_417U';
...
cursor stg_dup is
   select * from  dwh_wfs_foundation.stg_c2_cli_offr_note_cpy
   where (cli_offer_note_no)
   in
   (select cli_offer_note_no 
    from dwh_wfs_foundation.stg_c2_cli_offr_note_cpy
    group by  cli_offer_note_no
    having count(*) > 1) 
   order by
    cli_offer_note_no,
...
cursor c_stg is
   select /*+ FULL(stg)  parallel (stg,2) */  
              stg.*
      from    dwh_wfs_foundation.stg_c2_cli_offr_note_cpy stg,
              dwh_wfs_foundation.fnd_wfs_cli_offr_note fnd
--              dwh_wfs_foundation.fnd_wfs_cli_offr dep
      where   stg.cli_offer_note_no   = fnd.cli_offer_note_no     -- only ones existing in fnd
--              and stg.cli_offer_no  =  dep.cli_offer_no       -- only those existing in depended table
              and stg.sys_process_code         = 'N'  
-- Any further validation goes in here - like xxx.ind in (0,1) ---              
      order by
              stg.cli_offer_note_no,
...
begin
   ...
   for dupp_record in stg_dup
    loop
       ...
       if  dupp_record.cli_offer_note_no  = g_unique_key_field_val then
        update dwh_wfs_foundation.stg_c2_cli_offr_note_cpy stg
       ...
       g_unique_key_field_val   := dupp_record.cli_offer_note_no;
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ append parallel (fnd,2) */ into fnd_wfs_cli_offr_note fnd
      SELECT /*+ FULL(cpy)  parallel (cpy,2) */
         cpy. cli_offer_note_no ,
         cpy. cli_offer_no ,
         cpy. note_desc ,
         cpy. change_date ,
         cpy. changed_by ,
      ...
      from  dwh_wfs_foundation.stg_c2_cli_offr_note_cpy cpy
--         inner join dwh_wfs_foundation.fnd_wfs_cli_offr dep  on dep.cli_offer_no = cpy.cli_offer_no
         left outer join dwh_wfs_foundation.fnd_wfs_cli_offr_note fnd on fnd.cli_offer_note_no = cpy.cli_offer_note_no
      where fnd.cli_offer_note_no is null
...
for upd_rec in c_stg
   loop
     update fnd_wfs_cli_offr_note fnd 
     set    
         fnd. cli_offer_note_no = upd_rec. cli_offer_note_no ,
         fnd. cli_offer_no = upd_rec. cli_offer_no ,
         fnd. note_desc = upd_rec. note_desc ,
     ...
     where  fnd.cli_offer_note_no = upd_rec.cli_offer_note_no and
        ( 
         nvl(fnd. cli_offer_note_no, 0) <> upd_rec. cli_offer_note_no OR
         nvl(fnd. cli_offer_no, 0) <> upd_rec. cli_offer_no OR
         nvl(fnd. note_desc, 0) <> upd_rec. note_desc OR
...
begin
      ...
      insert /*+ append parallel (hsp,2) */ into dwh_wfs_foundation.stg_c2_cli_offr_note_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */ 

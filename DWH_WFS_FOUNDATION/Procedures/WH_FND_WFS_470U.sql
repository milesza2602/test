--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_470U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_470U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
-- Description: Call Data - Load WFS Call Evaluation data
-- Tran_type: ODWH: IVRQARPT    AIT: IVRQARPT 
--
-- Date:        2018-09-05
-- Author:      Safiyya Ismail
-- Purpose:     update table FND_WFS_CALL_EVALUATION in the Foundation layer
--              with input ex staging table from WFS.
-- Tables:      Input  - STG_VRNT_CALL_EVALUATION_CPY
--              Output - FND_WFS_CALL_EVALUATION
--              Dependency on  -   none
...
g_unique_key1_field_val  DWH_WFS_FOUNDATION.STG_VRNT_CALL_EVALUATION_CPY.EVALUATION_KEY%type;
g_unique_key2_field_val  DWH_WFS_FOUNDATION.STG_VRNT_CALL_EVALUATION_CPY.EVALUATION_DATE%type;

g_date               date          := trunc(sysdate);

L_MESSAGE            SYS_DWH_ERRLOG.LOG_TEXT%TYPE;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_WFS_470U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
L_TEXT               SYS_DWH_LOG.LOG_TEXT%TYPE ;
l_description        sys_dwh_log_summary.log_description%type  := 'VERINT DATA - LOAD WFS CALL EVALUATION DATA';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

cursor c_stg_dup is
   select * from  dwh_wfs_foundation.stg_vrnt_call_evaluation_cpy
   where (    evaluation_key, evaluation_date  )
   in
   (select     evaluation_key, evaluation_date 
    from dwh_wfs_foundation.stg_vrnt_call_evaluation_cpy
    group by      evaluation_key, evaluation_date
    having count(*) > 1) 
   order by
    evaluation_key, evaluation_date
    ,sys_source_batch_id desc ,sys_source_sequence_no desc;

cursor c_stg is
   select /*+ FULL(stg)  parallel (stg,2) */  
              stg.*
      from    dwh_wfs_foundation.stg_vrnt_call_evaluation_cpy stg
              ,dwh_wfs_foundation.fnd_wfs_call_evaluation fnd
     where
   ...
   for dupp_record in c_stg_dup
    loop
       if 
           ...
           and dupp_record.evaluation_date  = g_unique_key2_field_val
       then 
        update dwh_wfs_foundation.stg_vrnt_call_evaluation_cpy stg
       ...
       g_unique_key2_field_val :=  dupp_record.evaluation_date;
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ append parallel (fnd,2) */ into fnd_wfs_call_evaluation fnd
      SELECT /*+ FULL(cpy)  parallel (cpy,2) */
         ...
         cpy. handset_login ,
         cpy. agent_name ,
         cpy. customer_line_identification ,
         cpy. call_id ,
      ...
      from  dwh_wfs_foundation.stg_vrnt_call_evaluation_cpy cpy
         left outer join dwh_wfs_foundation.fnd_wfs_call_evaluation fnd on (
...
for upd_rec in c_stg
   loop
     update fnd_wfs_call_evaluation fnd 
     set    
         ...
         fnd. evaluator_name = upd_rec. evaluator_name ,
         fnd. evaluation_form = upd_rec. evaluation_form ,
         fnd. handset_login = upd_rec. handset_login ,
         fnd. agent_name = upd_rec. agent_name ,
         fnd. customer_line_identification = upd_rec. customer_line_identification ,
         fnd. call_id = upd_rec. call_id ,
     ...
     where 
        ...
        ( 
         ...
         nvl(fnd. contact_start_time, '01 JAN 1900') <> upd_rec. contact_start_time OR
         nvl(fnd. evaluator_name, 0) <> upd_rec. evaluator_name OR
         ...
         nvl(fnd. customer_line_identification, 0) <> upd_rec. customer_line_identification OR
         nvl(fnd. call_id, 0) <> upd_rec. call_id OR
         ...
         nvl(fnd. wfs_organization, 0) <> upd_rec. wfs_organization
         );         
...
begin
      ...
      insert /*+ append parallel (hsp,2) */ into dwh_wfs_foundation.stg_vrnt_call_evaluation_hsp hsp
      select /*+ FULL(cpy)  parallel (cpy,2) */ 
...
--**************************************************************************************************
    ...
    l_text := 'LOAD TABLE: '||'FND_WFS_CALL_EVALUATION' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

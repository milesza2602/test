--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_170U_20170601
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_170U_20170601" (p_forall_limit in integer,p_success out boolean) as
   ...
   loop
     ...
     where  fnd.credit_applications_id     = upd_rec.credit_applications_id and
            ( 
             ...
             nvl(fnd. tot_monthly_income_avg,0) <> upd_rec. tot_monthly_income_avg
            );         

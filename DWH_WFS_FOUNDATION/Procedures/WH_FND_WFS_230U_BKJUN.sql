--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_230U_BKJUN
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_230U_BKJUN" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
-- Barry  - 1 june 2016 - - load fix data from arc into cpy - reprocess 4 - sets
--**************************************************************************************************
--  Date:        March 2013
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_WFS_230U_BKJUN';
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND  */ into fnd_wfs_tran_day fnd
      select /*+ FULL(cpy)  parallel (cpy,4) */
       ...
       from  stg_vsn_tran_day_cpy cpy
       where  not exists 
      (select /*+ nl_aj */ * from fnd_wfs_tran_day A
...
-- Any further validation goes in here - like xxx.ind in (0,1) ---  
    ...
    l_text := 'INSERT COMPLETED '||g_recs_inserted  ;
...
/*     g_sys_source_batch_id := 244;
     g_date := '09 mar 2016';

    g_sys_source_batch_id := 251;
     g_date := '17 mar 2016';

    g_sys_source_batch_id := 255;
     g_date := '22 mar 2016';

     g_sys_source_batch_id := 261;
     g_date := '29 mar 2016'; 
 */    

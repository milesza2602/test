--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_318U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_318U" (p_forall_limit in integer,p_success out boolean) as
...
begin
 --     g_rec_out.last_updated_date         := g_date;
      ...
      insert /*+ APPEND parallel (fnd,4) */ into fnd_wfs_crd_txn_mly fnd
...
--**************************************************************************************************
    l_text := 'DELETE CURRENT MONTH FROM TXN_MLY STARTED AT '||
    ...
    if g_delete_ind = 1 then 
       ...
       DELETE /*+ PARALLEL(TXN,8) */ FROM fnd_wfs_crd_txn_mly TXN WHERE INFORMATION_DATE = G_DELETE_DATE;
    ...
    update /*+ parallel (cpy,4) */ stg_absa_crd_txn_mly_cpy cpy
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    l_text :=  'RECORDS DELETED '||g_recs_deleted;            --Bulk load--

--------------------------------------------------------
--  DDL for Procedure WH_FND_WFS_164TO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_WFS_FOUNDATION"."WH_FND_WFS_164TO" (
...
AS
  ...
  -- Tables:      Input  - stg_om4_pa_income
  ...
  g_pa_inc_id stg_om4_pa_income.income_id%type;
  g_pa_c_id   stg_om4_pa_income.credit_applications_id%type;
  g_pa_p_id   stg_om4_pa_income.personal_applicant_id%type;
...
IS
    SELECT /*+ PARALLEL(8) */ *
    FROM stg_om4_pa_income
    WHERE (credit_applications_id,personal_applicant_id,income_id) IN
      (SELECT /*+ PARALLEL(8) */ credit_applications_id,personal_applicant_id,income_id
      FROM stg_om4_pa_income

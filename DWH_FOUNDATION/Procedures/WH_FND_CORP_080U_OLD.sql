--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_080U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_080U_OLD" 
...
g_rec_in             stg_rms_location_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
type stg_array is table of stg_rms_location_item_cpy%rowtype;
...
type staging_array1 is table of stg_rms_location_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_location_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   g_rec_out.primary_country_code            := g_rec_in.primary_country_code;
   ...
   if not dwh_valid.indicator_field(g_rec_out.clearance_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;

   if not dwh_valid.indicator_field(g_rec_out.taxable_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;

   if not dwh_valid.indicator_field(g_rec_out.wh_supply_chain_type_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;
...
begin
   ...
   insert into stg_rms_location_item_hsp values g_rec_in;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_location_item_cpy

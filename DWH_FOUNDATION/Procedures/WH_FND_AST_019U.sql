--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_019U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_019U" 
...
--               Output - stg_ast_geo_grd_sc_fpln_wk_hsp
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_019U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ASSORT FORECAST HOSPITAL FACTS';
...
begin
   ...
   l_text := 'LOAD OF STG_AST_GEO_GRD_SC_PLN_WK_HSP STARTED AT '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
   ...
   where calendar_date = (select to_date(sysdate, 'dd mon yy') from dual);
      ...
      insert /*+ APPEND parallel (ast,4) */ into stg_ast_geo_grd_sc_fpln_wk_hsp ast
      select /*+ full (apln) parallel (apln,4) */
             ...
             apln.adjusted_sales,
             apln.adjusted_sales_qty,
             apln.adjusted_sales_cost
      from   stg_ast_geo_grd_sc_fpln_wk_cpy apln,

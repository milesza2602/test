--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014U_TAKEON" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014U_TAKEON';
...
FROM (SELECT DISTINCT 
 ...
 GROUP BY WORKGROUP_ID HAVING COUNT(*) > 1
 ),
SELJG
...
FROM (SELECT DISTINCT 
 ...
 GROUP BY JOBGROUP_ID HAVING COUNT(*) > 1
 )
,
SELJ
...
FROM (SELECT DISTINCT 
 ...
 GROUP BY LABOUR_ROLE_ID HAVING COUNT(*) > 1
 )
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).S4S_BUSINESS_UNIT_NO  = g_rec_out.S4S_BUSINESS_UNIT_NO  and 
              ...
              a_tbl_insert(i).LABOUR_ROLE_ID = g_rec_out.LABOUR_ROLE_ID 
              then
            g_found := TRUE;
         end if;
      end loop;

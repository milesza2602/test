--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_004U_BCK22012015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_004U_BCK22012015" (p_forall_limit in integer,p_success out boolean) as
...
--  WHERE EMPLOYEE_ID = '7080600' AND AVAILABILITY_START_DATE = '09/DEC/13'--AND CYCLE_START_DATE = '20/JAN/14'
...
/*
   WITH selbat AS
  (SELECT /*+ full(stg) parallel(stg,6)  */
...
-- and sys_source_batch_id >= 148
  ...
  selall AS
  (
  SELECT /*+ full(stg) parallel(stg,6)  full(ss) parallel(ss,6)*/ 
  /*SYS_SOURCE_BATCH_ID,
    STG.EMPLOYEE_ID ,
    STG.AVAILABILITY_START_DATE ,
    STG.AVAILABILITY_end_DATE , stg.cycle_start_date
  FROM selbat ss,
  ...
  AND stg.AVAILABILITY_START_DATE = sS.AVAILABILITY_START_DATE
    AND stg.cycle_START_DATE = sS.cycle_START_DATE
...
END WH_FND_S4S_004U_BCK22012015;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_613U_HIST1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_613U_HIST1" (
...
AS
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_613U_HIST1';
...
BEGIN
  ...
  select distinct this_week_start_date into g_start_date
  from dim_calendar 
  where calendar_date = G_Date - 126;
  ...
  l_text := 'Period PROCESSED - '||g_start_date||' - '||g_end_date;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------
    ...
    INTO dwh_foundation.TEMP_ALLOC_SHIP3
                    SELECT
                          ...
                          DISTINCT a.alloc_no, null
                    FROM fnd_rtl_allocation a
                    WHERE a.po_no    IS NULL -- IE. supply_chain_code = 'WH'
                    AND release_date between g_start_date and g_end_date
              ;
    g_recs_read     := 0;
    g_recs_inserted := 0;
...
--------------------------------------------------------------------------------------
    ...
    INSERT INTO DWH_FOUNDATION.TEMP_ALLOC_SHIP4
          WITH selalloc AS
                    (SELECT
                      /*+ materialize */
                      alloc_no

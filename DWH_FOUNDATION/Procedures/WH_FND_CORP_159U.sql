--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_159U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_159U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_intactix_loc_item
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_intactix_loc_item_hsp.sys_process_msg%type;
g_rec_out            fnd_location_item%rowtype;
g_rec_in             stg_intactix_loc_item%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_159U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE LOC ITEM MASTERDATA EX INTACTIX';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_intactix_loc_item%rowtype;
...
type staging_array1 is table of stg_intactix_loc_item.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_intactix_loc_item.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_intactix_loc_item is
/*   select a.*
   from stg_intactix_loc_item_cpy a, fnd_jdaff_dept_rollout b, dim_item c
   ...
   select a.*
   from stg_intactix_loc_item_cpy a
   where sys_process_code = 'N'
   order by sys_source_batch_id,sys_source_sequence_no;
...
begin
   ...
   insert into stg_intactix_loc_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    model_stock                 = a_tbl_update(i).model_stock,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_intactix_loc_item_cpy
...
--**************************************************************************************************
    open c_stg_intactix_loc_item;
    fetch c_stg_intactix_loc_item bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_106U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_106U" (p_forall_limit in integer,p_success out boolean) as
...
--  November 2017 - Addition of multi-currency columns for multi-currency (Bhavesh Valodia)
...
set       base_retail_ind           = mer_mart.base_retail_ind,
          ...
          currency_code2             = mer_mart.currency_code2,
          currency_reg_rsp2          = mer_mart.currency_reg_rsp2,
          reg_rsp_local                                = mer_mart.reg_rsp_local,
          reg_rsp_opr                                 =  mer_mart.reg_rsp_opr,
          selling_unit_rsp_local                = mer_mart.selling_unit_rsp_local,
          selling_unit_rsp_opr                   = mer_mart.selling_unit_rsp_opr,
          multi_unit_rsp_local                    = mer_mart.multi_unit_rsp_local,
          multi_unit_rsp_opr                      = mer_mart.multi_unit_rsp_opr
...
(         mer_mart.zone_group_no,
          ...
          g_date,
          mer_mart.currency_code2,
          mer_mart.currency_reg_rsp2,
          mer_mart.reg_rsp_local                      ,
          mer_mart.reg_rsp_opr                        ,
          mer_mart.selling_unit_rsp_local      ,
          mer_mart.selling_unit_rsp_opr        ,
          mer_mart.multi_unit_rsp_local        ,
          mer_mart.multi_unit_rsp_opr 
          )  
  ;
...
--**************************************************************************************************
   ...
   insert /*+ APPEND parallel (hsp,2) */ into stg_rms_zone_item_hsp hsp 
   select /*+ FULL(TMP) */  TMP.sys_source_batch_id,
                            ...
                            TMP.currency_code2,
                            TMP.currency_reg_rsp2,
                            TMP.reg_rsp_local,
                            TMP.reg_rsp_opr,
                            TMP.selling_unit_rsp_local,
                            TMP.selling_unit_rsp_opr ,
                            TMP.multi_unit_rsp_local ,
                            TMP. multi_unit_rsp_opr  
    from  stg_rms_zone_item_cpy  TMP 

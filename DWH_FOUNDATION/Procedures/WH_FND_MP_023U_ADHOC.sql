--------------------------------------------------------
--  DDL for Procedure WH_FND_MP_023U_ADHOC
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MP_023U_ADHOC" (p_forall_limit in integer , p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MP_023U_ADHOC';
      ...
      insert /*+ APPEND parallel (finfct,4) */ into fnd_rtl_loc_dept_mth_plan_mp finfct
       ...
       WITH STG_LOAD AS
          ...
          SELECT   /*+ full (stg) parallel (stg, 4) */
              ...
              pln_tot_facings
             from stg_mp_chain_dept_mth_plan
...
begin
   ...
   insert /*+ APPEND parallel (hsp,4) */ into stg_mp_chain_dept_mth_plan_hsp hsp
    select /*+ full (apln) parallel (stg,4) */
           ...
           stg.pln_disp_prop_factor,
           stg.pln_tot_facings
    from   stg_mp_chain_dept_mth_plan stg,
             fnd_chain chn,
             fnd_location loc,
             fnd_department dept,
             fnd_plan_type pln,

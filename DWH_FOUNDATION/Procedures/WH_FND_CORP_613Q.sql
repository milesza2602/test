--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_613Q
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_613Q" (p_forall_limit IN INTEGER,p_success OUT BOOLEAN)
AS
  --**************************************************************************************************
  --  Date:        may 2015
  --  Author:      W Lyttle
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_613Q';
...
BEGIN
  ...
  --**************************************************************************************************
  l_text := 'truncate table W6005682.TEMP_ALLOC_SHIP1Q' ;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  EXECUTE immediate 'truncate table W6005682.TEMP_ALLOC_SHIP1Q';
  
    INSERT /*+ append */
    INTO W6005682.TEMP_ALLOC_SHIP1Q
                    WITH selitm AS
                                (SELECT
                                  /*+   parallel(di,2) */
                                  item_no
...
--------------------------------------------------------------------------------------
    ...
    INTO dwh_foundation.TEMP_ALLOC_SHIP3
              ...
              UNION
                    ...
                    MINUS
                    SELECT b.* FROM W6005682.TEMP_ALLOC_SHIP1Q b
                    ) ;
    g_recs_read     := 0;
    g_recs_inserted := 0;
    g_recs_read     := g_recs_read     + SQL%ROWCOUNT;

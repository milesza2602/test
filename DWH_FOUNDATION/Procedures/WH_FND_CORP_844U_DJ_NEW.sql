--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_844U_DJ_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_844U_DJ_NEW" (
...
AS
  ...
  --               For CHBD only.
  --  Tables:      Input  - temp_alloc_tracker_actl_rcpt_DJ1,temp_alloc_tracker_actl_rcpt_DJ2,temp_alloc_tracker_actl_rcpt_DJ3,temp_alloc_tracker_actl_rcpt_DJ4
  --               Output - fnd_alloc_tracker_actl_rcpt_DJ
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_844U_DJ_NEW';
...
--
      ...
      where calendar_date  between g_start_wk_date and g_end_wk_date;
       ...
       l_text := 'FULL REFRESH ** truncate table dwh_foundation.fnd_alloc_tracker_actl_rcpt_dj';
       dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);   
        execute immediate ('truncate table dwh_foundation.fnd_alloc_tracker_actl_rcpt_dj');     
--**************************************************************************************************
...
--**************************************************************************************************
      ...
      for g_sub in 0..G_COUNT_DAYS loop
                      ...
                      where calendar_date  = G_SUB_DATE;
                        ...
                        G_SUBPART_NAME := 'FND_ATANO_'||G_PART_DT;
                        
                        g_sql_trunc_partition := 'alter table dwh_foundation.FND_ALLOC_TRACKER_ACTL_RCPT_DJ truncate SUBPARTITION '||G_SUBPART_NAME;
     ...
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     l_text := 'Running GATHER_TABLE_STATS ON FND_ALLOC_TRACKER_ACTL_RCPT_DJ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
--                                   'FND_ALLOC_TRACKER_ACTL_RCPT_DJ', DEGREE => 8);  

-- below is the new code for stats collection in 12c - Ref: 23Jan2019
     DWH_FOUNDATION.GENERIC_GATHER_TABLE_STATS(l_procedure_name,'FND_ALLOC_TRACKER_ACTL_RCPT_DJ');
...
--********************************************************************************************* 
    ...
    INTO dwh_foundation.fnd_alloc_tracker_actl_rcpt_DJ z
      SELECT
        /*+ full(a)  */
              *
            FROM dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ1 a
     ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
     ...
     l_text := 'TEMP_A_TRACK_ACTL_RCPT_DJ1 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_actl_rcpt_DJ z     
      SELECT
        /*+ full(B)  */
              *
            FROM dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ2 b
     ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;     
     l_text := 'TEMP_A_TRACK_ACTL_RCPT_DJ2 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_actl_rcpt_DJ z 
      SELECT
        /*+ full(C)  */
              *
            FROM dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ3 c
     ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_ACTL_RCPT_DJ3 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------          
    ...
    INTO dwh_foundation.fnd_alloc_tracker_actl_rcpt_DJ z 
      SELECT
        /*+ full(D)  */
              *
            FROM dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ4 d;
           ...
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_ACTL_RCPT_DJ4 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

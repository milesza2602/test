--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_840U_NDJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_840U_NDJ" (
...
--**************************************************************************************************
      ...
      for g_sub in 0..G_COUNT_DAYS loop
                      ...
                      where calendar_date  = G_SUB_DATE;
                        ...
                        G_SUBPART_NAME := 'FND_ATAND_'||G_PART_DT;
                        
                        g_sql_trunc_partition := 'alter table dwh_foundation.fnd_alloc_tracker_alloc_NDJ truncate SUBPARTITION '||G_SUBPART_NAME;
     ...
     l_text := 'Running GATHER_TABLE_STATS ON FND_ALLOC_TRACKER_ALLOC_NDJ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                   'FND_ALLOC_TRACKER_ALLOC_NDJ', DEGREE => 8);       
...
--********************************************************************************************* 
    ...
    INTO dwh_foundation.FND_ALLOC_TRACKER_ALLOC_NDJ z
      SELECT
        /*+ full(a)  */
              *
            FROM dwh_foundation.temp_alloc_tracker_alloc_NDJ1 a
     ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
     ...
     l_text := 'temp_alloc_tracker_alloc_NDJ1 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.FND_ALLOC_TRACKER_ALLOC_NDJ z     
      SELECT
        /*+ full(B)  */
              *
            FROM dwh_foundation.temp_alloc_tracker_alloc_NDJ2 b
     ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;     
     l_text := 'temp_alloc_tracker_alloc_NDJ2 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.FND_ALLOC_TRACKER_ALLOC_NDJ z 
      SELECT
        /*+ full(C)  */
              *
            FROM dwh_foundation.temp_alloc_tracker_alloc_NDJ3 c
     ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'temp_alloc_tracker_alloc_NDJ3 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------          
    ...
    INTO dwh_foundation.FND_ALLOC_TRACKER_ALLOC_NDJ z 
      SELECT
        /*+ full(D)  */
              *
            FROM dwh_foundation.temp_alloc_tracker_alloc_NDJ4 d;
           ...
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'temp_alloc_tracker_alloc_NDJ4 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
l_text := 'Running GATHER_TABLE_STATS ON TEMP_SHIPMENTS_MIN_NDJ';
  ...
  l_text := 'Truncating TEMP_SHIPMENTS_MIN_NDJ';
  DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
  execute immediate ('truncate table DWH_FOUNDATION.TEMP_SHIPMENTS_MIN_NDJ');

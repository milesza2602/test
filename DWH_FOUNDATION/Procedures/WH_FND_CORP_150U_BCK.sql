--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_150U_BCK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_150U_BCK" (p_forall_limit in integer,p_success out boolean) as
...
--**************************************************************************************************
  ...
  --------------------------------------------------------------------------------------------------------
   ...
   merge /*+ parallel (fnd_mart,6) */ 
    ...
    using (
         with aa as ( 
            select /*+ MATERIALIZE FULL(A) PARALLEL(A,4) */

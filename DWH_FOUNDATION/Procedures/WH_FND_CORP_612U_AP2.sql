--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_612U_AP2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_612U_AP2" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        August 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_612U_AP2';
...
type staging_array1 is table of stg_rms_rtl_shipment_cpx.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_rtl_shipment_cpx.sys_source_sequence_no%type
                                                       index by binary_integer;
...
WITH selext1 AS
            (SELECT
              /*+ materialize full(a) parallel(a,4) */
                  ...
                  ,FINAL_LOC_NO
            FROM stg_rms_rtl_shipment_cpx a
            WHERE (to_loc_no      between g_from_loc_no and g_to_loc_no
            OR (g_from_loc_no = 0 AND TO_LOC_NO IS NULL))
         --   AND (PO_NO >= 3000000 OR PO_NO IS NULL) -- INTERIM MEASURE TO PREVENT OLD PO'S BEING OVERWRITTEN WITH RE-CYLCLED PO'S
          --  where dist_no = 301504341
          --  WHERE SYS_SOURCE_BATCH_ID IS NULL
        --    WHERE to_loc_no BETWEEN 1 AND 400
            ) ,
  SELEXT2 AS
            (SELECT
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_shipment_ap values a_tbl_insert(i);

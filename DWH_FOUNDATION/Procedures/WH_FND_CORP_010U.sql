--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_010U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_010U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Alastair de Wet
--  Purpose:     Create supplier unit dimention table in the foundation layer
--               with input ex staging table from Walker.
--  Tables:      Input  - stg_walker_supplier_cpy
--               Output - fnd_supplier
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_walker_supplier_hsp.sys_process_msg%type;
g_rec_out            fnd_supplier%rowtype;
g_rec_in             stg_walker_supplier_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_010U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE SUPPLIER MASTERDATA EX WALKER';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_walker_supplier_cpy%rowtype;
...
type staging_array1 is table of stg_walker_supplier_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_walker_supplier_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_walker_supplier is
   select *
   from stg_walker_supplier_cpy
...
begin
   ...
   g_rec_out.contact_name                    := ' ';
   g_rec_out.supplier_name                   := ' ';
   g_rec_out.rtv_wh_no                       := 0;
   g_rec_out.edi_cost_chg_var_amt            := 0;
   g_rec_out.deal_mngmt_code                 := ' ';
   g_rec_out.freight_terms_id                := ' ';
   g_rec_out.fd_fax_plan_pattern_code        := ' ';
   g_rec_out.fd_fax_order_pattern_code       := ' ';
   g_rec_out.currency_code                   := ' ';
   g_rec_out.delivery_policy_code            := ' ';
   g_rec_out.contact_phone                   := ' ';
...
--   end if;
   ...
   if g_rec_out.control_entity_code <> 'WWTP' and g_rec_out.control_entity_code <> 'WWGL' then
     g_hospital      := 'Y';
     g_hospital_text := 'INVALID CONTROL ENTITY - MUST BE WWTP OR WWGL'  ;
   end if;
...
begin
   ...
   insert into stg_walker_supplier_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_supplier
       set    control_entity_code             = a_tbl_update(i).control_entity_code,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_walker_supplier_cpy
...
begin
   g_found := dwh_valid.fnd_supplier(g_rec_out.supplier_no);

-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).supplier_no = g_rec_out.supplier_no then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
--**************************************************************************************************
    open c_stg_walker_supplier;
    fetch c_stg_walker_supplier bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_660U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_660U_FIX" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
-- DATA FIX - reprocess all stg_cpy recss
--**************************************************************************************************
--  Date:        March 2010
--  Author:      B.Bartlett
...
--               Output - temp_deal_ACTUAL_DETAIL_RTV
...
g_rec_out            temp_deal_ACTUAL_DETAIL_RTV%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_660U_FIX';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE temp_deal_ACTUAL_DETAIL_RTV DATA EX RMS';
...
type tbl_array_i is table of temp_deal_ACTUAL_DETAIL_RTV%rowtype index by binary_integer;
type tbl_array_u is table of temp_deal_ACTUAL_DETAIL_RTV%rowtype index by binary_integer;
...
--   from DWH_FOUNDATION.STG_RMS_DEAL_ACTL_DETL_RTV_arc
--         WHERE SYS_LOAD_DATE BETWEEN '12 june 2011' AND '17 june 2011'
--   where SYS_LOAD_DATE BETWEEN '1 JUNE 2011' AND '15 june 2011' 
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into temp_deal_ACTUAL_DETAIL_RTV values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update temp_deal_ACTUAL_DETAIL_RTV
...
begin
  ...
  g_found := false;
   ...
   select count(1)
   into   v_count
   from   temp_deal_ACTUAL_DETAIL_RTV
...
begin
    ...
    l_text := 'LOAD OF temp_deal_ACTUAL_DETAIL_RTV EX RMS STARTED AT '||
...
--**************************************************************************************************
  ...
  --
--  EXECUTE IMMEDIATE('TRUNCATE TABLE dwh_foundation.temp_deal_ACTUAL_DETAIL_RTV');
--  COMMIT;
--  l_text := 'TRUNCATE COMPLETED OF temp_deal_ACTUAL_DETAIL_RTV';
--  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--  COMMIT;
  
  DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','temp_deal_ACTUAL_DETAIL_RTV', DEGREE => 8);
  COMMIT;
  l_text := 'GATHER STATS COMPLETED';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--l_text := 'sys_load_date between 27 MAY 2011 and 30 MAY 2011';
--  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--  COMMIT; 

l_text := 'cpy_table reprocess';
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  COMMIT; 

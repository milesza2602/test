--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_080U_INS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_080U_INS" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_080U_INS';
...
merge /*+ parallel (fnd_mart,6) */ 
    ...
    using (
    select /*+ FULL(TMP) */ tmp.*
    from stg_rms_location_item_hsp  tmp 
    ...
    where tmp.sys_process_code = 'Y'
      and tmp.sys_load_date >= '18/AUG/15'

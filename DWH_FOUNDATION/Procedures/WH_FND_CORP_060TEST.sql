--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_060TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_060TEST" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Sean Le Roux
--  Purpose:     Update ITEM unit dimension table in the foundation layer
--               with input ex staging table from Order Management (OM).
--  Tables:      AIT load - stg_om_item
--               Input    - stg_om_item_cpy
--               Output   - fnd_item
--  Packages:    dwh_constants, dwh_log, dwh_valid
--  Maintenance:
--  10 Feb 2009 - defect 837- Changing segregation_in validation to reject if Null -
--                            indicator should have values 1-9 or should contain a value
--
--  26 Feb 2009 - defect 843  - Default selling_ind and buying_ind on FND_ITEM on insert
--  11 May 2011 - defect 2981 - Add 2 new measures to table (min/max_shelf_life_tolerance)
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_om_item_hsp.sys_process_msg%type;
g_rec_out            fnd_item%rowtype;
g_rec_in             stg_om_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_060TEST';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'UPDATE THE ITEM MASTERDATA EX OM';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_om_item_cpy%rowtype;
...
type staging_array1 is table of stg_om_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_om_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_om_item is
   select a.* 
   from stg_om_item_cpy a, fnd_jdaff_dept_rollout b, dim_item b
...
begin
   ...
   g_rec_out.buying_ind	              := nvl(g_rec_in.buying_ind,0);
   g_rec_out.selling_ind	            := nvl(g_rec_in.selling_ind,0);
...
--   g_rec_out.max_shelf_life_tolerance := g_rec_in.max_shelf_life_tolerance;
   g_rec_out.min_shelf_life_tolerance	:= g_rec_in.min_tolerance;
   g_rec_out.max_shelf_life_tolerance := g_rec_in.max_tolerance;
   ...
   if g_rec_out.segregation_ind is null then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;
   if not dwh_valid.indicator_field(g_rec_out.buying_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;

   if not dwh_valid.indicator_field(g_rec_out.selling_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;

   if not dwh_valid.indicator_field(g_rec_out.store_scanned_order_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;
...
begin
   ...
   insert into stg_om_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_item
       set    fd_product_no	          = a_tbl_update(i).fd_product_no,
...
-- Data will only update fnd_item table
   g_found := TRUE ;
...
begin
    ...
    l_text := 'LOAD OF FND_ITEM EX OM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_om_item;
    fetch c_stg_om_item bulk collect into a_stg_input limit g_forall_limit;

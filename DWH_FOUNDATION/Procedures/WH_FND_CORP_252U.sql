--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_252U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_252U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Sean Le Roux
--  Purpose:     Load catalog transaction table in the foundation layer
--               with input ex staging table from CATMAN.
--  Tables:      Input  - stg_catman_loc_dept_wk_cpy
--               Output - fnd_rtl_loc_dept_wk
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_catman_loc_dept_wk_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_dept_wk%rowtype;
g_rec_in             stg_catman_loc_dept_wk_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_252U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD CATALOG TRANSACTIONS EX CATMAN';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_catman_loc_dept_wk_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_dept_wk%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_dept_wk%rowtype index by binary_integer;
...
type staging_array1 is table of stg_catman_loc_dept_wk_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_catman_loc_dept_wk_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_catman_loc_dept_wk is
   select *
   from stg_catman_loc_dept_wk_cpy
...
begin
   ...
   g_rec_out.num_catalog_additions           := g_rec_in.num_catalog_additions;
   g_rec_out.num_catalog_deletions           := g_rec_in.num_catalog_deletions;
   g_rec_out.num_catalog_total               := g_rec_in.num_catalog_total;
   g_rec_out.num_catalog_old_total           := g_rec_in.num_catalog_old_total;
...
begin
   ...
   insert into stg_catman_loc_dept_wk_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_dept_wk values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_dept_wk
       set    num_catalog_additions           = a_tbl_update(i).num_catalog_additions,
              num_catalog_deletions           = a_tbl_update(i).num_catalog_deletions,
              num_catalog_total               = a_tbl_update(i).num_catalog_total,
              num_catalog_old_total           = a_tbl_update(i).num_catalog_old_total,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_catman_loc_dept_wk_cpy
...
begin
    ...
    l_text := 'LOAD OF fnd_rtl_loc_dept_wk EX CATMAN STARTED AT '||
...
--**************************************************************************************************
    open c_stg_catman_loc_dept_wk;
    fetch c_stg_catman_loc_dept_wk bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_catman_loc_dept_wk;
--**************************************************************************************************

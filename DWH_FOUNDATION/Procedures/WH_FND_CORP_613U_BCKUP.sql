--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_613U_BCKUP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_613U_BCKUP" (
...
AS
  ...
  --               Output - temp_alloc_first_dc_no
  --  Packages:    constants, dwh_log, dwh_valid
  --
  --  Maintenance:
  --
  --    ALTER TABLE DWH_FOUNDATION.TEMP_ALLOC_SHIP RENAME TO TEMP_ALLOC_SHIP1
  --    ALTER TABLE DWH_FOUNDATION.TEMP_ALLOC_ALLOC RENAME TO TEMP_ALLOC_SHIP2
  --    ALTER TABLE DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP RENAME TO TEMP_ALLOC_SHIP3
  --    ALTER TABLE DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP_SHIP RENAME TO TEMP_ALLOC_SHIP4
  --  Naming conventions
  --  g_  -  Global variable
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_613U_BCKUP';
...
BEGIN
  ...
  --**************************************************************************************************
  l_text := 'truncate table DWH_FOUNDATION.TEMP_ALLOC_SHIP' ;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  EXECUTE immediate 'truncate table dwh_foundation.temp_alloc_SHIP';
  
    INSERT /*+ append */
    INTO dwh_foundation.temp_alloc_ship
                    WITH selitm AS
                                (SELECT
                                  /*+   parallel(di,2) */
                                  item_no
  ...
  l_text                 := 'Insert complete -- temp_alloc_shipc='||g_recs_read;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  
  COMMIT;
  --------------------------------------------------------------------------------------
  l_text := 'truncate table DWH_FOUNDATION.TEMP_ALLOC_ALLOC' ;
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  EXECUTE immediate 'truncate table dwh_foundation.temp_alloc_ALLOC';
  
  INSERT /*+ append */
  INTO dwh_foundation.temp_alloc_alloc
              WITH selitm AS
                        (SELECT
                              /*+  MATERIALIZE */
                              item_no
              ...
              SELECT
                ...
                AND release_date BETWEEN '20-FEB-15' AND '21-MAY-15'
                 ;
    g_recs_read     := 0;
    g_recs_inserted := 0;
    g_recs_read     := g_recs_read     + SQL%ROWCOUNT;
    ...
    l_text          := 'Insert complete -- extra ship into temp_alloc_alloc='||g_recs_read;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    COMMIT;
--------------------------------------------------------------------------------------
    l_text := 'truncate table DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate 'truncate table dwh_foundation.temp_alloc_ALLOC_SHIP';
    
    INSERT /*+ append */
    INTO dwh_foundation.temp_alloc_alloc_SHIP
                    SELECT
                          /*+   parallel(a,4) parallel(si,4) */
                          DISTINCT a.alloc_no,
                          a.item_no
                    FROM fnd_rtl_allocation_WL a,
                         dwh_foundation.temp_alloc_ship si
                    WHERE a.po_no    IS NULL -- IE. supply_chain_code = 'WH'
                    AND a.item_no     = si.item_no
                    AND release_date IS NOT NULL
                    AND a.alloc_no    = si.alloc_no
              UNION
                    ( SELECT a.* FROM dwh_foundation.temp_alloc_alloc a
                    MINUS
                    SELECT b.* FROM dwh_foundation.temp_alloc_ship b
                    ) ;
    g_recs_read     := 0;
    g_recs_inserted := 0;
    g_recs_read     := g_recs_read     + SQL%ROWCOUNT;
    ...
    COMMIT;
--------------------------------------------------------------------------------------
    l_text := 'truncate table DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP_SHIP' ;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        EXECUTE immediate 'truncate table dwh_foundation.temp_alloc_ALLOC_SHIP_SHIP';
        
    INSERT INTO DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP_ship
          WITH selalloc AS
                    (SELECT
                      /*+ materialize */
                      DISTINCT alloc_no
                    FROM dwh_foundation.temp_alloc_alloc_ship
                    ORDER BY alloc_no
                    )
                  --,
                  --selship as
...
--------------------------------------------------------------------------------------
  ...
  insert into dwh_foundation.temp_alloc_first_dc_no
 with    
                   selwh1 AS
                                      (SELECT 
                                    ...
                                    FROM DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP_SHIP s
                                    JOIN dim_location l

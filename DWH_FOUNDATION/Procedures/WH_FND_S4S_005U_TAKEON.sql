--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_005U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_005U_TAKEON" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_005U_TAKEON';
...
WITH selover AS
             (SELECT DISTINCT stg2.employee_id,
                       stg2.JOB_start_date
                FROM DWH_FOUNDATION.STG_S4S_EMP_JOB_dy_cpy stg2 ,
   ...
   where stg.EMPLOYEE_ID = fe.employee_id(+)
     ...
     AND SYS_SOURCE_BATCH_ID = 100
       and stg.emlpoyee_id = se.emlpoyee_id(+)
     and stg.JOB_start_date = se.JOB_start_date(+)
 --AND STG.LOCATION_NO IN (119,127,106)
 --    AND STG.EMPLOYEE_ID IN ('1002194','1002198','1002609','7072436','7075640')

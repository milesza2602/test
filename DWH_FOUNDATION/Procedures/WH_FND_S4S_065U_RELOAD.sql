--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_065U_RELOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_065U_RELOAD" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        January 2016
--  Author:      Lwazi Ntloko
--  Purpose:     Create iterative procedure to reprocess WRNC schedule data for S4S
--  Tables:      Input  - STG_S4S_SCHED_XCPTN_EMP_DY_CPY (ARC)
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_065U_RELOAD';
...
begin
    ...
    (select /*+ full(b1) parallel(d1,6) */ distinct 
      ...
      from dwh_foundation.STG_S4S_SCHED_XCPTN_EMP_DY_ARC d1
     ...
     where exists 
            (select distinct g1.EMPLOYEE_ID, g1.EXCEPTION_TYPE_ID, g1.EXCEPTION_DATE, g1.LOCATION_NO, g1.JOB_ID
             from dwh_foundation.STG_S4S_SCHED_XCPTN_EMP_DY_ARC g1
...
begin
  ...
  INSERT /*+ append */ into dwh_foundation.STG_S4S_SCHED_XCPTN_EMP_DY_HSP 
        with selstg as 
                         ...
                         FROM DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_EMP_DY_ARC stg,
                                  fnd_location fl,
...
--      coding below commented out due to decision made 15 april 2015 that deletes are not based upon location_no and exception_date
        ...
        where  trunc(f.exception_date) > G_DATE; --Change in logic from >= to just > was agreed by Mojeed on 23 Nov 2015
...
BEGIN
     for v_cur in (
                ...
                from DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_EMP_DY_ARC
                where sys_source_batch_id >= 1210			--HARDCODED FOR REPROCESSING (CHG-41308)

--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_040U_FX12052017
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_040U_FX12052017" 
(p_forall_limit in integer,p_success out boolean)
as
--**************************************************************************************************
-- Fix datasend from JDA ASST - may 2017 - wendy
------------------------------
-- 1. copy stg_ast_loc_item_dy_catlg_cpy to stg_astlcitmdy_ctlg_bck120517
--
--              create table dwh_foundation.stg_astlcitmdy_ctlg_bck120517
--                tablespace FND_MASTER as
--              select *from stg_ast_loc_item_dy_catlg_cpy 
--Table DWH_FOUNDATION.STG_ASTLCITMDY_CTLG_BCK120517 created.
-- recs=3446680
--
-- 2. truncate stg_ast_loc_item_dy_catlg_cpy
--Table DWH_FOUNDATION.STG_AST_LOC_ITEM_DY_CATLG_CPY truncated.
--
-- 3. copy stg_ast_loc_item_dy_catlg to stg_ast_loc_item_dy_catlg_cpy
--    insert /*+ append */ into dwh_foundation.stg_ast_loc_item_dy_catlg_cpy
--    select * from dwh_foundation.stg_ast_loc_item_dy_catlg
--
--                   also made back of stg with fix
--                   create table dwh_foundation.stg_astlcitmdy_ctlg_fix120517
--                   tablespace FND_MASTER as--
--                   select distinct post_date from stg_ast_loc_item_dy_catlg

-- 4. run load for post_date = 12 may 2017
--**************************************************************************************************
--  Date:        Jan 2013
--  Author:      Wendy Lyttle
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_040U_FX12052017';
...
end wh_fnd_ast_040u_FX12052017;

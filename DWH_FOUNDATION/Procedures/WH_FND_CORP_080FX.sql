--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_080FX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_080FX" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_080U';
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    for dupp_record in stg_dup
       loop
            ...
            where          location_no    = dupp_record.location_no and
                           item_no        = dupp_record.item_no;
            --sys_source_batch_id    = dupp_record.sys_source_batch_id and
...
merge  /*+ parallel (fnd_mart,6) */
    ...
    join fnd_supplier ds       on tmp.primary_supplier_no = ds.supplier_no
...
--**************************************************************************************************
   ...
   select /*+ FULL(TMP) */  TMP.sys_source_batch_id,
                            ...
                            TMP.REG_RSP_LOCAL,
                            TMP.REG_RSP_OPR,
                            TMP.SELLING_RSP_LOCAL,
                            TMP.SELLING_RSP_OPR,
                            TMP.PROM_RSP_LOCAL,
                            TMP.PROM_RSP_OPR,
                            TMP.PROM_SELLING_RSP_LOCAL,
                            TMP.PROM_SELLING_RSP_OPR
    from  stg_rms_location_item_cpy  TMP 

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_206U_FIX_SUN
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_206U_FIX_SUN" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer)  as
...
--  Tables:      Input  - stg_rms_stock_ch_FIX_SUN
...
g_rec_in             dwh_foundation.stg_rms_stock_ch_FIX_SUN%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_206U_FIX_SUN_'||p_from_loc_no;
...
type stg_array is table of dwh_foundation.stg_rms_stock_ch_FIX_SUN%rowtype;
...
type staging_array1 is table of dwh_foundation.stg_rms_stock_ch_FIX_SUN.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of dwh_foundation.stg_rms_stock_ch_FIX_SUN.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_stock_ch is
   select *
   from dwh_foundation.stg_rms_stock_ch_FIX_SUN
...
begin
    ...
    l_text := 'INPUT = stg_rms_stock_ch_FIX_SUN_fri';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

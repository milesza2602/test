--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014U_FIX" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014U_FIX';
...
WITH 
SELWG
AS (SELECT WORKGROUP_ID, COUNT(*)
    FROM (SELECT DISTINCT 
              lh.S4S_BUSINESS_UNIT_NO ,
              lh.WORKGROUP_ID,
              lh.EFFECTIVE_FROM_DATE,
              lh.EFFECTIVE_TO_DATE
          from stg_s4s_labour_hierarchy_cpy LH
          )
    GROUP BY WORKGROUP_ID,EFFECTIVE_FROM_DATE,EFFECTIVE_TO_DATE HAVING COUNT(*) > 1
    ),
SELJG
AS (SELECT JOBGROUP_ID, COUNT(*)
    FROM (SELECT DISTINCT 
              lh.WORKGROUP_ID ,
              lh.JOBGROUP_ID,
              lh.EFFECTIVE_FROM_DATE,
    ...
    GROUP BY JOBGROUP_ID,EFFECTIVE_FROM_DATE,EFFECTIVE_TO_DATE HAVING COUNT(*) > 1
    ),
SELJ
AS (SELECT JOB_ID, COUNT(*)
    FROM (SELECT DISTINCT 
              lh.JOBGROUP_ID ,
              lh.JOB_ID,
              lh.EFFECTIVE_FROM_DATE,
    ...
    GROUP BY JOB_ID,EFFECTIVE_FROM_DATE,EFFECTIVE_TO_DATE HAVING COUNT(*) > 1
--    ),
--SELLR
...
WHERE STG.WORKGROUP_ID = SWG.WORKGROUP_ID(+)
  ...
  and stg.workgroup_id <> 1000133     
  and stg.job_id not in (1000346,1000241,1000236,1000226)
--  and stg.jobgroup_id <> 10001330
...
begin
      ...
      and   job_id not in (1000346,1000241,1000236,1000226)
      ;

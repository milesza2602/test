--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_156U_MERGE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_156U_MERGE" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_156M';
...
g_location_no       stg_jdaff_location_item_cpy.location_no%type; 
g_item_no           stg_jdaff_location_item_cpy.item_no%TYPE; 

cursor stg_dup is
     select a.* from stg_jdaff_location_item_cpy a, fnd_jdaff_dept_rollout b, dim_item c
      where a.item_no = c.item_no
        ...
        and (a.location_no, a.item_no)
          in (select location_no,item_no
               from stg_jdaff_location_item_cpy 
...
--**************************************************************************************************
procedure process_recs_merge as
begin
...
when matched then
update
set       NEXT_WK_DELIV_PATTERN_CODE  = mer_mart.next_wk_deliv_pattern_code,
...
--************************************************************************************************** 
    ...
    for dupp_record in stg_dup
       loop
        ...
        if  dupp_record.location_no   = g_location_no and
            ...
            update stg_jdaff_location_item_cpy stg
  ...
  ---------------------
   ...
   insert /*+ APPEND parallel (hsp,2) */ into stg_jdaff_location_item_hsp hsp 
   select /*+ FULL(TMP) */  TMP.sys_source_batch_id,
                            ...
                            g_date
    from  stg_jdaff_location_item_cpy  TMP 

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_141U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_141U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_jdaff_zone_item_cpy
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_jdaff_zone_item_hsp.sys_process_msg%type;
g_rec_out            fnd_zone_item%rowtype;
g_rec_in             stg_jdaff_zone_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_141U';
...
type stg_array is table of stg_jdaff_zone_item_cpy%rowtype;
...
type staging_array1 is table of stg_jdaff_zone_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_jdaff_zone_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_jdaff_zone_item is
/*   select a.*
     from stg_jdaff_zone_item_cpy a, fnd_jdaff_dept_rollout b, dim_item c
     ...
     select a.*
     from stg_jdaff_zone_item_cpy a
     where sys_process_code = 'N'
     order by sys_source_batch_id,sys_source_sequence_no;
...
begin
   ...
   insert into stg_jdaff_zone_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    num_shelf_life_days         = a_tbl_update(i).num_shelf_life_days,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_jdaff_zone_item_cpy
...
--**************************************************************************************************
    open c_stg_jdaff_zone_item;
    fetch c_stg_jdaff_zone_item bulk collect into a_stg_input limit g_forall_limit;

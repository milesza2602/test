--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_620X
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_620X" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_620U';
...
cursor c_stg_rms_rtl_allocation is
   ...
   from stg_rms_rtl_allocation_cpy stg, 
        dim_item di,  
        fnd_location_item li
   where sys_process_code = 'N'
     and stg.item_no      = di.item_no(+)  
     and stg.to_loc_no    = li.location_no(+) 
     and stg.item_no      = li.item_no(+) 
...
/*
   begin
         select vat_rate_perc,business_unit_no,base_rsp
         into   g_vat_rate_perc,g_business_unit_no,g_rec_out.reg_rsp_excl_vat
         from   dim_item
...
-- Remove WAC lookup when wh_fnd_corp_621u goes into service which does the WAC join in a very efficient manner-- A de Wet 05 2009
   if g_business_unit_no = 50 then
      g_start_date := g_rec_out.release_date - 365;
      begin
         select wac
         into   g_rec_out.wac
         from   fnd_rtl_loc_item_dy_rms_wac
         where  location_no         = g_rec_out.to_loc_no and
                item_no             = g_rec_out.item_no   and
--                tran_date          <= g_date and
                tran_date           between g_start_date and g_rec_out.release_date and
                rownum              = 1
...
/*
   ...
   begin
         ...
         exception
         when no_data_found then
           g_rec_out.reg_rsp_excl_vat   := g_rec_out.reg_rsp_excl_vat;
   end;
   end if;
*/   

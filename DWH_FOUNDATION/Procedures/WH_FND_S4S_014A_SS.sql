--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014A_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014A_SS" (p_forall_limit in integer,p_success out boolean) as
...
--               Output - FND_S4S_BUSINESS_UNIT_SS
...
g_rec_out            FND_S4S_BUSINESS_UNIT_SS%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014A_SS';
...
type tbl_array_i is table of FND_S4S_BUSINESS_UNIT_SS%rowtype index by binary_integer;
type tbl_array_u is table of FND_S4S_BUSINESS_UNIT_SS%rowtype index by binary_integer;
...
Newtable as (    
select bu.*, 
      LAG(effective_from_date-1, 1, date_candidate) OVER (PARTITION BY s4s_business_unit_no ORDER BY s4s_business_unit_no desc) effective_to_date
from bu_list bu
order by s4s_business_unit_no, effective_from_date desc)
select 
    s4s_business_unit_no,
    s4s_business_unit_name,
    business_unit_no,
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into FND_S4S_BUSINESS_UNIT_SS values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
        update FND_S4S_BUSINESS_UNIT_SS

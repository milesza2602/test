--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_005U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_005U_SS" (p_forall_limit in integer,p_success out boolean) as
...
--                        TRUNCATE TABLE dwh_foundation.STG_S4S_EMP_JOB_DY_TMP_SS
--
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
--  Maintenance:
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_005U_SS';
...
WITH selbat as (
    select
            ...
            employee_id
       from stg_s4s_emp_job_dy_cpy --2361
--       from stg_s4s_emp_job_dy_hsp --2468
--       where sys_load_date > '25 jun 17'
--       and  sys_process_msg = 'JOB_ID NOT FOUND'
--       and job_id not in (2681,4403,4584,5706,999985,1000101,1000164)
   ...
   selseq as
       (select 
            ...
            stg.job_start_date
        from selbat sb,
             stg_s4s_emp_job_dy_cpy stg --391,925
--             stg_s4s_emp_job_dy_hsp stg --6963
...
--           stg_s4s_emp_job_dy_hsp stg --6963
   ...
   from stg_s4s_emp_job_dy_cpy stg, --475,903
--   from stg_s4s_emp_job_dy_hsp stg, --7803
...
begin
      ...
      from dwh_foundation.fnd_s4s_emp_job_del_list_ss;
      ...
      BEGIN
           insert /*+ append */ into dwh_foundation.fnd_s4s_emp_job_del_list_ss
           with selstg as 
                  (select distinct employee_id from stg_s4s_emp_job_dy_cpy )--2361 --391,925
--                  (select distinct employee_id from stg_s4s_emp_job_dy_hsp )--6506 --43,803
           select g_run_date, g_date, g_run_seq_no, f.*
              from dwh_foundation.fnd_s4s_emp_job_ss f, selstg s
              where f.employee_id = s.employee_id;
                ...
                l_text := 'Insert into FND_S4S_EMP_JOB_del_list_ss recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
                dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

               delete from dwh_foundation.fnd_s4s_emp_job_ss
               where employee_id IN (select distinct employee_id from stg_s4s_emp_job_dy_cpy);
--               where employee_id IN (select distinct employee_id from stg_s4s_emp_job_dy_hsp);

                g_recs :=SQL%ROWCOUNT ;
                COMMIT;
                g_recs_deleted := g_recs;

            l_text := 'Deleted from dwh_foundation.fnd_s4s_emp_job_ss recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
            dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
          exception
               when no_data_found then
                      l_text := 'No deletions done for dwh_foundation.fnd_s4s_emp_job_ss ';
            dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
         end;          
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_s4s_emp_job_ss  values a_tbl_insert(i);
...
begin
   ...
   select count(1)
    into   v_count
    from DWH_FOUNDATION.fnd_s4s_emp_job_ss
...
--**************************************************************************************************
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   THEN 
        delete_fnd;
   ELSE
        L_Text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_EMP_JOB_DEL_LIST_SS';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Execute Immediate ('TRUNCATE TABLE dwh_foundation.FND_S4S_EMP_JOB_DEL_LIST_SS');
        L_Text := 'TRUNCATE TABLE dwh_foundation.fnd_s4s_emp_job_ss';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Execute Immediate ('TRUNCATE TABLE dwh_foundation.fnd_s4s_emp_job_ss');
        L_Text := 'Running GATHER_TABLE_STATS ON fnd_s4s_emp_job_ss';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Dbms_Stats.Gather_Table_Stats ('DWH_FOUNDATION',
                                       'fnd_s4s_emp_job_ss', Degree => 8);

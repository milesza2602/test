--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_058U_WLFX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_058U_WLFX" (p_forall_limit in integer,p_success out boolean) as
...
--  QC ???? -- 19 Jun 2015 -- Add code to tranlate product_class alpha to number
--  08 May 2017 - Update the data integrity fix
--  09 MAY 2017 - fix commented out again - code change reversed out
--  10 Jul 2017 - Uncomment the data integrity fix for reclass
...
g_class_no_2         fnd_item.class_no%type;
g_department_no_2    fnd_item.department_no%type;
...
--   select a.*,c.department_live_ind
...
-- This line checks only previous data. leaving it out checks all data
          ...
          (style_no       <> g_style_no or
           ...
           class_no       <> g_class_no_2 or
           department_no  <> g_department_no_2);
...
--   g_rec_out.base_rsp                        := g_rec_in.base_rsp;
  ...
  -- SS  Start 19/JUN/15     only translate the prduct_class for Live departments
      if g_rec_in.product_class in ('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H') then
          case g_rec_in.product_class
            when 'A' then g_rec_out.product_class := '1';
            when 'B' then g_rec_out.product_class := '2';
            when 'C' then g_rec_out.product_class := '3';
            when 'D' then g_rec_out.product_class := '4';
            when 'E' then g_rec_out.product_class := '5';
            when 'F' then g_rec_out.product_class := '6';
            when 'G' then g_rec_out.product_class := '7';
            when 'H' then g_rec_out.product_class := '8';
...
--
   ...
   loop
       ...
       g_class_no_2        := v_fnd_item.class_no;
       g_department_no_2   := v_fnd_item.department_no;

       for  v_fnd_item_old in c_fnd_item_old
       loop
          l_text := 'New data - '||v_fnd_item.style_colour_no||'  '||v_fnd_item.style_no||'  '||v_fnd_item.subclass_no||'  '||v_fnd_item.class_no||'  '||v_fnd_item.department_no||
                    ' Previous bad data - '||v_fnd_item_old.style_colour_no||'  '||v_fnd_item_old.style_no||'  '||v_fnd_item_old.subclass_no||'  '||v_fnd_item_old.class_no||'  '||v_fnd_item_old.department_no||
   ...
   loop
       ...
       g_class_no_2        := v_fnd_item_s.class_no;
       g_department_no_2   := v_fnd_item_s.department_no;

       for  v_fnd_item_old_s in c_fnd_item_old_s
       loop
          l_text := 'New data - '||v_fnd_item_s.style_no||'  '||v_fnd_item_s.subclass_no||'  '||v_fnd_item_s.class_no||'  '||v_fnd_item_s.department_no||
                    ' Previous bad data - '||v_fnd_item_old_s.style_no||'  '||v_fnd_item_old_s.subclass_no||'  '||v_fnd_item_old_s.class_no||'  '||v_fnd_item_old_s.department_no||
                    ' for item '||v_fnd_item_old_s.item_no;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          g_recs_rejected  := g_recs_rejected + 1;
          
        -- AUTO UPDATE TO FIX THE ISSUE -- USE IF ANY PROBLEMS AFTER RECLASS
        update fnd_item

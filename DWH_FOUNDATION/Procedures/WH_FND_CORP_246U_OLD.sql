--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_246U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_246U_OLD" 
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_wac_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_rms_wac%rowtype;
g_rec_in             stg_rms_wac_cpy%rowtype;
g_found              boolean;
...
type stg_array is table of stg_rms_wac_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_rms_wac%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_rms_wac%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_wac_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_wac_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   insert into stg_rms_wac_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_rms_wac values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_rms_wac
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_loc_item_dy_rms_wac
...
-- DUPS SHOULD NEVER HAPPEN ON WAC SO REMOVE FOR PERFORMANCE
/*
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_DY_RMS_WAC EX ITB STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_wac;
    fetch c_stg_rms_wac bulk collect into a_stg_input limit g_forall_limit;

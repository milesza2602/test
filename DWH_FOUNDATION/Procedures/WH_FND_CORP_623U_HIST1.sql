--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_623U_HIST1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_623U_HIST1" (p_forall_limit in integer,p_success out boolean)
as
--**************************************************************************************************
--  Date:        may 2015
--  Author:      W Lyttle
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_623U_HIST1';
...
-- run update  
     update dwh_foundation.fnd_rtl_allocation fnd
             set fnd.first_dc_no = (select  /*+ full(tmp) */ 
                                             min(tmp.first_dc_no) from dwh_foundation.temp_alloc_first_dc_no tmp
                                            where tmp.alloc_no = fnd.alloc_no)
     where po_no is null
...
-- run update  
     update dwh_foundation.fnd_rtl_allocation fnd
             set fnd.first_dc_no = (select  /*+ full(dl) */  
                                             WH_PHYSICAL_WH_NO from dwh_performance.dim_location dl
                                            where dl.location_no = fnd.wh_no)

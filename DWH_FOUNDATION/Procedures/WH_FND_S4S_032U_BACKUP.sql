--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_032U_BACKUP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_032U_BACKUP" (p_forall_limit in integer,p_success out boolean) as
...
/*WITH selover AS
             (SELECT DISTINCT stg2.employee_id,
                ...
                WHERE sel.employee_id = stg2.EMPLOYEE_id
                  AND sel.calendar_date BETWEEN stg2.constraint_start_date AND NVL(stg2.constraint_END_DATE - 1, g_eff_end_date)
              )
*/   select SYS_SOURCE_BATCH_ID,
 ...
 --    and stg.CONSTRAINT_START_DATE = SEO.CONSTRAINT_START_DATE(+)
       --   AND SYS_SOURCE_BATCH_ID = 115

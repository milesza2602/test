--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_743U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_743U_LOAD" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_743U_LOAD';
...
cursor c_stg_jdaff_po_plan is
   select a.*
   from stg_jdaff_po_plan_arc a, fnd_jdaff_dept_rollout b, dim_item c
   where a.sys_source_batch_id = 800  --sys_process_code = 'Y'
     and a.item_no = c.item_no
     and c.department_no = b.department_no
     and b.department_live_ind = 'Y'
     and a.sys_load_date = g_date
...
begin
   ...
   g_hospital                               := 'N';
    ...
    g_rec_out.from_loc_no                     := g_rec_in.to_loc_no;

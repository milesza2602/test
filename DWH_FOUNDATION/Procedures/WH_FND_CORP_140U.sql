--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_140U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_140U" 
...
--  Tables:      Input  - stg_om_zone_item_supp_cpy
--               Output - fnd_zone_item_supp_om
...
g_rec_out            fnd_zone_item_supp_om%rowtype;
g_rec_in             stg_om_zone_item_supp_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_140U';
...
type stg_array is table of stg_om_zone_item_supp_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_zone_item_supp_om%rowtype index by binary_integer;
type tbl_array_u is table of fnd_zone_item_supp_om%rowtype index by binary_integer;
...
type staging_array1 is table of stg_om_zone_item_supp_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_om_zone_item_supp_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_om_zone_item_supp is
   --select *
   --from stg_om_zone_item_supp_cpy
   --where sys_process_code = 'N'
   --order by sys_source_batch_id,sys_source_sequence_no;
   
    select a.*
   from stg_om_zone_item_supp_cpy a, dim_item b, fnd_jdaff_dept_rollout c
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_zone_item_supp_om values a_tbl_insert(i);
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_om_zone_item_supp_cpy
...
begin
    ...
    l_text := 'LOAD OF FND_ZONE_ITEM_SUPP_OM EX OM STARTED AT '||

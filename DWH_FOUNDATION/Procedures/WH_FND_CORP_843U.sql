--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_843U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_843U" 
...
-- 1. Replaced hints on cursor c_fnd_alloc_ship - creating indexes on fnd_rtl_shipment(dist_no, final_loc_no, item_no) 
...
--  Date:        February 2010
--  Author:      M Munnik
--  Purpose:     Load Shipment data - receipts at first warehouse - to allocation tracker table (supply_chain_code 'WH').
--               For CHBD only.
--  NB !! NB !!  This procedure does not truncate the fnd_alloc_tracker_frst_rcpt table.
--  NB !! NB !!  However, fnd_alloc_tracker_frst_rcpt is truncated by wh_fnd_corp_842u.
--  NB !! NB !!  If this procdure fails, but has already committed some inserts,
--  NB !! NB !!  both wh_fnd_corp_842u and wh_fnd_corp_843u must be re-run.
--  Tables:      Input  - fnd_alloc_tracker_alloc, fnd_rtl_shipment
--               Output - fnd_alloc_tracker_frst_rcpt
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  12 may 2010 - change joins
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_843U_'|| p_from_loc_no;
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOADS Receipts at First Warehouse (WH) TO ALLOC TRACKER TABLE';
...
--SELECT /*+ parallel(a 4) */ 
SELECT /*+ parallel(4) use_nl_with_index(s2 I10_F_FND_RTL_SHIPMENT) index(s I15_F_FND_RTL_SHIPMENT) */
  a.release_date,
  a.alloc_no,
  s2.to_loc_no,
  ...
  g_date last_updated_date, s2.final_loc_no,
...
ON s.item_no = s2.item_no
  --        and s.from_dc_no    = s2.final_loc_no
AND s.du_id                       = s2.du_id
--AND a.first_dc_no                 = s2.from_loc_no
AND a.first_whse_supplier_no                = s2.from_loc_no
...
begin
    ...
    l_text := 'LOAD OF fnd_alloc_tracker_frst_rcpt (WH) from fnd_rtl_shipment STARTED '||

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_040U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_040U_TAKEON" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_040U_TAKEON';
...
WITH selover AS
             (SELECT DISTINCT stg2.employee_id,
                ...
                WHERE sel.employee_id = stg2.elpoyee_id
                  AND sel.calendar_date BETWEEN stg2.effective_start_date AND NVL(stg2.FLR.EFFECTIVE_END_DATE - 1, g_eff_end_date)
              )

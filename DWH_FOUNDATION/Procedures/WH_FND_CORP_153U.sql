--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_153U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_153U" (p_forall_limit in integer,p_success out boolean) as
...
--               with input ex staging table from AMOS.
--  Tables:      Input  - stg_amos_zone_item
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_amos_zone_item_hsp.sys_process_msg%type;
g_rec_out            fnd_zone_item%rowtype;
g_rec_in             stg_amos_zone_item%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_153U';
...
type stg_array is table of stg_amos_zone_item%rowtype;
...
type staging_array1 is table of stg_amos_zone_item.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_amos_zone_item.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_amos_zone_item is
   select a.*
   from stg_amos_zone_item_cpy a
...
begin
   ...
   insert into stg_amos_zone_item_hsp values g_rec_in;
...
begin
    ...
    --l_text :=  'UPDATE - item = ' || g_rec_out.item_no || ' zone_no = ' || g_rec_out.zone_no || ' zone_group_no = ' || g_rec_out.zone_group_no;
    --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    --l_text := 'min order qty = ' || g_rec_out.min_order_qty;
    --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    --l_text := 'g_recs_updated (1) = ' || g_recs_updated;
    --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_zone_item
       set    tray_size_code            = a_tbl_update(i).tray_size_code,
       ...
       --l_text := 'g_recs_updated (2) = ' || g_recs_updated;
       --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
begin
    ...
    l_text := 'LOAD OF fnd_zone_item EX AMOS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_amos_zone_item;
    fetch c_stg_amos_zone_item bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_580U_020516
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_580U_020516" (p_forall_limit in integer,p_success out boolean) as
...
where (area_no,fin_year_no, FIN_WEEK_NO)
in
(select area_no,fin_year_no, FIN_WEEK_NO
from stg_excel_fd_fcst_ratio_cpy
group by area_no,fin_year_no, FIN_WEEK_NO
having count(*) > 1)
order by area_no,fin_year_no,FIN_WEEK_NO,sys_source_batch_id desc ,sys_source_sequence_no desc ;
...
select /*+ FULL(stg)  parallel (stg,2) */
      ...
      where   stg.area_no  = fnd.area_no and
              ...
              stg.fin_week_no      = fnd.fin_week_no
              and
              stg.sys_process_code = 'N'
-- Any further validation goes in here - like xxx.ind in (0,1) ---
      order by
              stg.area_no,stg.fin_year_no,stg.sys_source_batch_id,stg.sys_source_sequence_no ;
...
for dupp_record in stg_dup
   loop
    ...
    if  dupp_record.area_no   = g_area_no and
        ...
        dupp_record.fin_week_no       = g_fin_week_no then
   ...
   loop
     ...
     where  fnd.area_no       = upd_rec.area_no and
            ...
            fnd.fin_week_no           = upd_rec.fin_week_no  and
              (
            upd_rec.forecast_sales <> fnd.forecast_sales );
...
begin
      ...
      not exists
        (select * from dim_calendar dc
         ...
         and cpy.fin_week_no          = dc.fin_week_no) or
      not exists

--------------------------------------------------------
--  DDL for Procedure WH_FND_MP_005U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MP_005U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MP_005U';
...
cursor c_stg_mp_chain_subc_wk_plan is
   select *
   from stg_mp_chain_subc_wk_plan_cpy;
...
-- remove 4 measures for Total Planning - March release 2015
...
-- 3 new measures Total Planning - March release 2015
   ...
   if g_rec_out.plan_type_no not in (50,51,52,53,54) then
     g_hospital      := 'Y';
...
begin
   ...
   insert into stg_mp_chain_subc_wk_plan_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_chain_subclass_wk_plan values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_chain_subclass_wk_plan
...
-- 3 new measures Total Planning - March release 2015 QC 5043
       ...
       where  chain_no                       = a_tbl_update(i).chain_no and
              ...
              subclass_no                    = a_tbl_update(i).subclass_no and
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_mp_chain_subc_wk_plan_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_chain_subclass_wk_plan
   where  chain_no        = g_rec_out.chain_no      and
          department_no   = g_rec_out.department_no and
          class_no        = g_rec_out.class_no      and
          subclass_no     = g_rec_out.subclass_no   and
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).chain_no      = g_rec_out.chain_no       and
            ...
            a_tbl_insert(i).subclass_no   = g_rec_out.subclass_no    and
...
--**************************************************************************************************
    open c_stg_mp_chain_subc_wk_plan;
    fetch c_stg_mp_chain_subc_wk_plan bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_mp_chain_subc_wk_plan;
--**************************************************************************************************

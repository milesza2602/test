--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_844C_DJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_844C_DJ" (
...
--                 For CHBD only.
  ...
  --               Output - TEMP_A_TRACK_ACTL_RCPT_DJ3
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_844C_DJ';
...
--**************************************************************************************************
      ...
      l_text := 'truncate table dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ3' ;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'truncate table dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ3';
...
--**************************************************************************************************
    ...
    INSERT /*+ APPEND */
    INTO dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_DJ3
     with  selcal AS (   
           ...
           SELSHIP AS (
            ...
            ,
          selall as (
                          select /*+   full(a) full(s) parallel(a,4)  parallel(s,4)  */
                                              a.release_date
                                              ,
                                              a.alloc_no,
                                              s.to_loc_no,
                           ...
                           group by           a.release_date,
                                              ...
                                              a.item_no
                  union all
                              select /*+  full(a) full(s2) parallel(a,4)  parallel(s2,4)  */
                                              a.release_date,
                                              a.alloc_no,
                                              s2.to_loc_no,

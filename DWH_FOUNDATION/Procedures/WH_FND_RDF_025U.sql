--------------------------------------------------------
--  DDL for Procedure WH_FND_RDF_025U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RDF_025U" 
...
--  Purpose:     Create location forecast indicator table in the foundation layer
--               with input ex staging table from RDF.
--  Tables:      Input  - stg_rdf_location_cpy
--               Output - fnd_location
--  Packages:    dwh_constants, dwh_log, dwh_valid
--
--  Maintenance:
--  Mar 2009 - A Joshua - Td-973 : Validate forecastable_ind and change to update only
--
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rdf_location_cpy.sys_process_msg%type;
g_rec_out            fnd_location %rowtype;
g_rec_in             stg_rdf_location_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RDF_025U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE DAILY LOCATION FORECAST IND INDICATOR EX RDF';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rdf_location_cpy%rowtype;
...
type staging_array1 is table of stg_rdf_location_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rdf_location_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rdf_loc_fcst is
   select *
   from stg_rdf_location_cpy
...
begin
   ...
   if not dwh_valid.indicator_field(g_rec_out.forecastable_ind) then
     g_hospital      := 'Y';
...
begin
   ...
   insert into stg_rdf_location_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_location
       set    forecastable_ind               = a_tbl_update(i).forecastable_ind,
...
-- Check if insert of location already in insert array and change to put duplicate in update array
...
begin
    ...
    l_text := 'LOAD OF FND_LOCATION EX RDF STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rdf_loc_fcst;
    fetch c_stg_rdf_loc_fcst bulk collect into a_stg_input limit g_forall_limit;

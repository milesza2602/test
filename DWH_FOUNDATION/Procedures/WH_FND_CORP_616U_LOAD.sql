--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_616U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_616U_LOAD" 
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_rtv_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_rtv%rowtype;
g_rec_in             stg_rms_rtv_cpy%rowtype;
g_found              boolean;
g_date               date;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_616U_LOAD';
...
type stg_array is table of stg_rms_rtv_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_rtv%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_rtv%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_rtv_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_rtv_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_rtv is
   select *
   from stg_rms_rtv_arc
   where sys_load_date = '10 aug 14'
    and  sys_source_batch_id = 1981
...
begin
   ...
   insert into stg_rms_rtv_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_rtv values a_tbl_insert(i);
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_RTV EX OM STARTED '||
...
--**************************************************************************************************
    open c_stg_rms_rtv;
    fetch c_stg_rms_rtv bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_064U_FIX1DEC2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_064U_FIX1DEC2015" (p_forall_limit in integer,p_success out boolean) as
...
--  Date:        December 2014
--  Author:      Wendy Lyttle
--  Purpose:     Update schedule exception LOCATION day information for Scheduling for Staff(S4S)
--
--  Tables:      AIT load - STG_S4S_SCHED_XCPTN_LOC_DY 
--               Input    - STG_S4S_SCHED_XCPTN_LOC_DY_cpy
--               Output   - FND_S4S_SCHED_XCPTN_LOC_DY
...
--
--
--  w lyttle 16 apr 2015 - change delete to reflect where excpetion_date >= batch_date(ie. g_date) regardless of location
...
g_hospital_text      DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_LOC_DY_hsp.sys_process_msg%type;
g_rec_out            DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_LOC_DY%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_064U_FIX1DEC2015';
...
begin 
      ...
      l_text := 'truncate table dwh_foundation.FND_S4S_SCHED_XCPTN_LOC_DEL';
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text); 
      
      execute immediate('truncate table dwh_foundation.FND_S4S_SCHED_XCPTN_LOC_DEL');
      
      DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                   'FND_S4S_SCHED_XCPTN_LOC_DEL', DEGREE => 8);

 /*     select max(run_seq_no)+1 into g_run_seq_no
      from dwh_foundation.FND_S4S_SCHED_XCPTN_LOC_DEL;
...
begin
     insert /*+ append */ into dwh_foundation.FND_S4S_SCHED_XCPTN_LOC_DEL
     select /*+ full(f) parallel(f,6) */ g_run_date
             , g_date
             , g_run_seq_no
             , f.*
     from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_LOC_DY f
     where trunc(exception_date) >= G_MINEXCPTDATE;
--
--
...
--            distinct location_no, trunc(exception_date) exception_date
---            from STG_S4S_SCHED_XCPTN_LOC_DY_cpy a 
--            where sys_source_batch_id = g_sys_source_batch_id)
--     select /*+ full(f) parallel(f,6) */ g_run_date
--             , g_date
--             , g_run_seq_no
--             , f.*
--     from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_LOC_DY f, selstg s
--   where f.location_no = s.location_no
...
--               ;
     ...
     l_text := 'Insert into FND_S4S_SCHED_XCPTN_LOC_DEL recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
     dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

     delete from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_LOC_DY b
     where exists   (select distinct location_no, trunc(exception_date) 
                      from dwh_foundation.FND_S4S_SCHED_XCPTN_LOC_DEL a
      ...
      l_text := 'Deleted from FND_S4S_SCHED_XCPTN_LOC_DY recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      
    exception
         when no_data_found then
                l_text := 'No deletions done for DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_LOC_DY ';
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
    end;          
...
-- g_date := trunc(sysdate);
  ...
  for v_cur in (
	...
	from DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_LOC_DY_CPY
--  where sys_source_batch_id <= 106   -- WAITING FOR DATAFIX
...
--  if TO_CHAR(V_CUR.MINEXCPTDATE,'DY') = 'SUN'
--  THEN 
...
--   dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
 -- ELSE  
   ...
   g_recs := null;
 --------------------------------  
      insert /*+ append */ into DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_LOC_DY
             ...
             G_DATE
					     from DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_LOC_DY_cpy stg,
...
--**************************************************************************************************
  INSERT /*+ append */ into dwh_foundation.STG_S4S_SCHED_XCPTN_LOC_DY_HSP 
        with selstg as 
...
END WH_FND_S4S_064U_FIX1DEC2015;

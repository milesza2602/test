--------------------------------------------------------
--  DDL for Procedure WH_FND_MC_654U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MC_654U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - STG_RMS_MC_DEAL_ACT_DETAIL_cpy
--               Output - fnd_mc_deal_actual_detail
...
g_hospital           char(1)       := 'N';
g_hospital_text      STG_RMS_MC_DEAL_ACT_DETAIL_hsp.sys_process_msg%type;
g_rec_out            fnd_mc_deal_actual_detail%rowtype;
g_rec_in             STG_RMS_MC_DEAL_ACT_DETAIL_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MC_654U';
...
type stg_array is table of STG_RMS_MC_DEAL_ACT_DETAIL_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_mc_deal_actual_detail%rowtype index by binary_integer;
type tbl_array_u is table of fnd_mc_deal_actual_detail%rowtype index by binary_integer;
...
type staging_array1 is table of STG_RMS_MC_DEAL_ACT_DETAIL_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of STG_RMS_MC_DEAL_ACT_DETAIL_cpy.sys_source_sequence_no%type 
                                                       index by binary_integer;
...
cursor c_stg_rms_deal_actual_detail is
   select *
   from STG_RMS_MC_DEAL_ACT_DETAIL_cpy
...
begin
   ...
   insert into STG_RMS_MC_DEAL_ACT_DETAIL_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_mc_deal_actual_detail values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last 
       save exceptions
       update fnd_mc_deal_actual_detail
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update STG_RMS_MC_DEAL_ACT_DETAIL_cpy      
...
begin
  ...
  g_found := false;
   ...
   select count(1)
   into   v_count
   from   fnd_mc_deal_actual_detail
...
begin 
    ...
    l_text := 'LOAD OF FND_deal_actual_detail EX RMS STARTED AT '||

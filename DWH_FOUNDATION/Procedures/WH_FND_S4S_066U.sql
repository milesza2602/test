--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_066U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_066U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  PLEASE NOTE THIS [PROCESS IS TEMPORARY UNTIL WE HAVE LOADED HISTORY AND IT IS CORRECT
-- THIS WILL RUN IN PARALLE TO WH_FND_065U FOR NOW:
--**************************************************************************************************
--**************************************************************************************************
...
--               Output   - FND_S4S_SCHED_XCPTN_EMPXDY
...
g_rec_out            DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMPXDY%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_066U';
...
begin 
  ...
  --    l_text := 'truncate table dwh_foundation.FND_S4S_SCHED_XCPTN_EMPXDEL';
  --    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text); 
      
   ---   execute immediate('truncate table dwh_foundation.FND_S4S_SCHED_XCPTN_EMPXDEL');
      
      DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                   'FND_S4S_SCHED_XCPTN_EMPXDEL', DEGREE => 8);

      select max(run_seq_no)+1 into g_run_seq_no
      from dwh_foundation.FND_S4S_SCHED_XCPTN_EMPXDEL;
...
--
--     insert /*+ append */ into dwh_foundation.FND_S4S_SCHED_XCPTN_EMPXDEL--
--    
--     select /*+ full(f) parallel(f,6) */ distinct g_run_date
--             , g_date
--             , g_run_seq_no
--             , f.*
--     from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMPXDY F
--     where  trunc(f.exception_date) >= G_MINEXCPTDATE
--                ;
                
     insert /*+ append */ into dwh_foundation.FND_S4S_SCHED_XCPTN_EMPXDEL
--    
     with selstg
            as (select /*+ materialize full(a) parallel(a,6) */ 
           distinct 
          -- employee_id, 
           trunc(exception_date) exception_date
           ,exception_type_id
     ...
     select /*+ full(f) parallel(f,6) */ distinct g_run_date
             ...
             , g_run_seq_no
             , f.*
     from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMPXDY F, SELSTG S
     where f.exception_type_id = s.exception_type_id
     ...
     l_text := 'Insert into FND_S4S_SCHED_XCPTN_EMPXDEL recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
     dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

     delete from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMPXDY b
     where exists   (select distinct employee_id, trunc(exception_date) 
                      from dwh_foundation.FND_S4S_SCHED_XCPTN_EMPXDEL a
      ...
      l_text := 'Deleted from FND_S4S_SCHED_XCPTN_EMPXDY recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      
    exception
         when no_data_found then
                l_text := 'No deletions done for DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMPXDY ';
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
    end;          
...
-- g_date := trunc(sysdate);
 ...
 -- where sys_source_batch_id >= 77
  GROUP BY sys_source_batch_id,sys_load_date
   ...
   l_text := ' g_sys_source_batch_id ='|| g_sys_source_batch_id||' g_sys_load_date ='|| g_sys_load_date;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
-------------------------------------------------------------------------------------------
  ...
  ELSE
   ...
   delete_fnd;
   DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_S4S_SCHED_XCPTN_EMPXDY', DEGREE => 8);
 
   g_recs_inserted := null;
   g_recs := null;
 --------------------------------  
      insert /*+ append */ into DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMPXDY
   ...
   dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
    ...
    l_text := 'Running GATHER_TABLE_STATS ON FND_S4S_SCHED_XCPTN_EMPXDY';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_S4S_SCHED_XCPTN_EMPXDY', DEGREE => 8);

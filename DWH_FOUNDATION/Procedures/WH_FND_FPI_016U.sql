--------------------------------------------------------
--  DDL for Procedure WH_FND_FPI_016U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_FPI_016U" 
                                    (p_forall_limit in integer,p_success out boolean) as
                                                                
--**************************************************************************************************
--  Date:        February 2018
--  Author:      Francisca De Vaal
--  Tables:      Input  - STG_FPI_COMP_ALLERGEN_CPY                                                            
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_FPI_016U';				
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STG_FPI_COMP_ALLERGEN_CPY EX FPI';
...
begin
   ...
   using (
    ...
    from   stg_fpi_comp_allergen_cpy cpy1,
           fnd_supplier sup,
           fnd_item itm,
    ...
    where  cpy1.supplier_no   = sup.supplier_no and
          ...
          (select /*+ PARALLEL(8) */ sys_source_batch_id, sys_source_sequence_no,
           ...
           from (select /*+ PARALLEL(tmp,8) */ tmp.*,
                 ...
                 as rank
                 from stg_fpi_comp_allergen_cpy tmp                                                             -- STG TABLE NAME CHANGE 
           )
           where rank = 1)
    ...
    update set                                                                                                  -- COLUNM NAME CHANGE
             fnd.contains	        =	mer_rec.contains,
             fnd.may_contains	    =	mer_rec.may_contains,
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   STG_FPI_COMP_ALLERGEN_CPY                                                            	    -- STG TABLE NAME CHANGE
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       RAISE;
end wh_fnd_fpi_016u													-- STORE PROC CHANGE
;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_620U_AP2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_620U_AP2" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_rms_rtl_allocation_cpx
...
g_rec_out            fnd_rtl_allocation_ap%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_620U_AP2';
...
type tbl_array_i is table of fnd_rtl_allocation_ap%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_allocation_ap%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_rtl_allocation_cpx.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_rtl_allocation_cpx.sys_source_sequence_no%type
                                                       index by binary_integer;
...
g_from_loc_no   number  := 491;
g_to_loc_no     number  := 99999;
...
--           case when li.tax_perc is null then                                 -- REF03Mar2018
          case when vr.VAT_RATE_PERC is null then                               -- REF03Mar2018
              ...
              vr.VAT_RATE_PERC                                                  -- REF03Mar2018
--               li.tax_perc                                                    -- REF03Mar2018
   ...
   from stg_rms_rtl_allocation_cpx stg
  ...
  --                               and nvl(dl.vat_region_no, dl2.vat_region_no) = vr.vat_region_no  
         left join fnd_item_vat_rate vr                                                                                -- REF03Mar2018
                                        on (vr.item_no = stg.item_no and vr.vat_region_no = dl.VAT_REGION_NO and        -- REF03Mar2018
                                            stg.RELEASE_DATE between active_from_date and active_to_date)             -- REF03Mar2018
   where --sys_process_code = 'N' and
         TO_LOC_NO      BETWEEN g_from_loc_no AND g_to_loc_no
-- TO_LOC_NO = 9512
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_allocation_ap values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    release_date                    = a_tbl_update(i).release_date,
              ...
              first_dc_no                     = a_tbl_update(i).first_dc_no 
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_rtl_allocation_cpx
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_allocation_ap
   where  alloc_no    = g_rec_out.alloc_no  and

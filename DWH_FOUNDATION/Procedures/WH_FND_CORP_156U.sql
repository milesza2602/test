--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_156U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_156U" (p_forall_limit in integer,p_success out boolean) as
...
cursor stg_dup is
      select a.* from stg_jdaff_location_item_cpy a
...
merge /*+ parallel(fnd_mart,6) */ 
    ...
    using (   
       select /*+ parallel(a,6) */  a.*
         from stg_jdaff_location_item_cpy a
--         , fnd_jdaff_dept_rollout b, dim_item c

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_076U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_076U" 
...
--  Purpose:     Create item_supplier dimention table in the foundation layer
--               with input ex staging table from RMS.
--  Tables:      Input  - stg_rms_item_supplier_cpy
--               Output - fnd_item_supplier
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_item_supplier_hsp.sys_process_msg%type;
g_rec_out            fnd_item_supplier%rowtype;
g_rec_in             stg_rms_item_supplier_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_076U';
...
type stg_array is table of stg_rms_item_supplier_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_item_supplier%rowtype index by binary_integer;
type tbl_array_u is table of fnd_item_supplier%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_item_supplier_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_item_supplier_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_item_supplier is
   select *
   from stg_rms_item_supplier_cpy
...
begin
   ...
   if not dwh_valid.indicator_field(g_rec_out.direct_shipment_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     return;
   end if;
...
begin
   ...
   insert into stg_rms_item_supplier_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_item_supplier values a_tbl_insert(i);
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_item_supplier_cpy
...
begin
    ...
    l_text := 'LOAD OF FND_ITEM_SUPPLIER EX OM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_item_supplier;
    fetch c_stg_rms_item_supplier bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_145U_PROD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_145U_PROD" (p_forall_limit in integer,
...
g_rec_out            dwh_foundation.fnd_loc_item_dy_ff_ord%rowtype;
g_rec_in             stg_cam_location_item%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_145U_'||p_from_loc_no;
...
type stg_array is table of stg_cam_location_item%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dwh_foundation.fnd_loc_item_dy_ff_ord%rowtype index by binary_integer;
type tbl_array_u is table of dwh_foundation.fnd_loc_item_dy_ff_ord%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cam_location_item.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cam_location_item.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cam_location_item is
   select a.*
     from stg_cam_location_item_cpy a, 
--     fnd_jdaff_dept_rollout b, 
     dim_item c, dim_location d
    where 
  --  sys_process_code = 'N' -- commented out prior to rollout check 
  --    and                  -- commented out prior to rollout check 
      a.item_no = c.item_no
--      and c.department_no = b.department_no
--      and b.department_live_ind = 'Y'
      and a.location_no = d.location_no
      and a.location_no between g_from_loc_no and g_to_loc_no   --p_from_loc_no and p_to_loc_no
...
begin
 ...
 -- All invalid locations are excluded by the join to dim_location in the cursor
...
-- All invalid items are excluded by the join to dim_item in the cursor
...
begin
   ...
   insert into stg_cam_location_item_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_loc_item_dy_ff_ord values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).num_units_per_tray;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_loc_item_dy_ff_ord
       set    num_units_per_tray          = a_tbl_update(i).num_units_per_tray ,
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_update(g_error_index).num_units_per_tray;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_cam_location_item_cpy
...
begin
    ...
    l_text := 'LOAD OF dwh_foundation.fnd_loc_item_dy_ff_ord EX OM STARTED AT '||
...
--**************************************************************************************************
    ...
    l_text := 'BATCH DATE BEING PROCESSED IS:- '||g_date||' LOCATION RANGE : '||g_from_loc_no||' TO '||g_to_loc_no;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    open c_stg_cam_location_item;
    fetch c_stg_cam_location_item bulk collect into a_stg_input limit g_forall_limit;

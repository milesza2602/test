--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_087J
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_087J" (p_forall_limit in integer,p_success out boolean) as
-- ******************************************************************************************
--  Date:        June 2016
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_087J';
...
begin
  ...
  (
   select /*+ parallel(fiz,6) full (fiz) full(dl) full (di) full (jda) */ 
          fiz.item_no,
          fiz.num_shelf_life_days,
          fiz.num_units_per_tray,
          g_date as last_updated_date,
          dl.location_no,
          di.primary_supplier_no,
          di.department_no
...
--     and  jda.department_live_ind = 'Y'
 ...
 when matched then
  update 
     set num_shelf_life_days             = mer_mart.num_shelf_life_days,
...
/*      where (
            ...
            primary_supplier_no             <> mer_mart.primary_supplier_no or

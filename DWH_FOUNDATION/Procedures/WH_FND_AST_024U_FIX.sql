--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_024U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_024U_FIX" 
                                                                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        June 2012
--  Author:      A Joshua
--  Purpose:     Load Assort Actualised Plan from Staging.
--  Tables:      Input  - w7067814.takeon_grd_geo_apln_mo
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_024U_FIX';
...
--**************************************************************************************************
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      ...
      insert /*+ APPEND parallel (ast,4) */ into fnd_ast_geo_grd_sc_wk_act ast
      select /*+ full (apln) parallel (apln,4) */
             ...
             apln.selling_price,
             apln.cost_price,
             null, --apln.prom_reg_qty,
             null, --apln.prom_reg_cost,
             null, --apln.prom_reg_selling,
      ...
      from   w7012356.TKN_GRD_GEO_SC_WK_APLN_MO apln,
             fnd_ast_lev1_diff1_real sc,
             fnd_ast_geography geo,
             fnd_ast_grade grd,
             fnd_plan_type pln
      where apln.stylcolour                   = sc.style_colour_no
--       and  substr(apln.stylcolour,1,2)       <> 99
       and  apln.geography                    = geo.geography_no

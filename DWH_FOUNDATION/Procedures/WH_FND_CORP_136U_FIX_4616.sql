--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_136U_FIX_4616
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_136U_FIX_4616" (
    p_forall_limit IN INTEGER,
    p_success OUT BOOLEAN)
AS
  --**************************************************************************************************
  -- qc4616 - datafix - active_date
  --
  --**************************************************************************************************
  --  Date:         February 2012
  --  Author:       W Lyttle
...
--  COMMIT;
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  IF G_Cnt > 0 THEN
    UPDATE dwh_foundation.Fnd_Rtl_Clearance
    SET Active_Date   = '05/MAR/2012'
    WHERE Active_Date = '06/MAR/2012';
    G_Recs_updated   := Sql%Rowcount ;
    COMMIT;
  End If;
      L_Text           := 'Fnd_Rtl_Clearance - No. of recs updated = '|| G_Recs_Updated;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
  IF G_Cnt > 0 THEN
    UPDATE dwh_foundation.stg_rms_rtl_clearance_arc
    SET Active_Date   = '05/MAR/2012'
    WHERE Active_Date = '06/MAR/2012';
    G_Recs_Updated   := Sql%Rowcount ;
    COMMIT;
  END IF;
     L_Text           := 'Stg_rms_rtl_clearance_ARC - No. of recs updated = '|| G_Recs_updated;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
  IF G_Cnt > 0 THEN
    UPDATE dwh_foundation.stg_rms_rtl_clearance_CPY
    SET Active_Date   = '05/MAR/2012'
    WHERE Active_Date = '06/MAR/2012';
    G_Recs_Updated   := Sql%Rowcount ;
    COMMIT;
  END IF;
      L_Text           := 'Stg_rms_rtl_clearance_CPY - No. of recs updated = '|| G_Recs_Updated;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
  IF G_Cnt > 0 THEN
    UPDATE dwh_foundation.stg_rms_rtl_clearance_HSP
    SET Active_Date   = '05/MAR/2012'
    WHERE Active_Date = '06/MAR/2012';
    G_Recs_Updated   := Sql%Rowcount ;
    COMMIT;
  END IF;
      L_Text           := 'Stg_rms_rtl_clearance_HSP - No. of recs updated = '|| G_Recs_Updated;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  ...
  Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
  IF G_Cnt > 0 THEN
    UPDATE dwh_foundation.stg_rms_rtl_clearance
    SET Active_Date   = '05/MAR/2012'
    WHERE Active_Date = '06/MAR/2012';
    G_Recs_Updated   := Sql%Rowcount ;
    COMMIT;
  END IF;
    L_Text           := 'Stg_rms_rtl_clearance - No. of recs updated = '|| G_Recs_Updated;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_RDF_010U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RDF_010U" 
...
--  Purpose:     Create Alert table in the foundation layer
--               with input ex staging table from RDF.
--  Tables:      Input  - stg_rdf_alert_cpy
--               Output - fnd_rtl_loc_item_wk_rdf_alert
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rdf_alert_cpy.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_wk_rdf_alert %rowtype;
g_rec_in             stg_rdf_alert_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RDF_010U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WEEKLY ALERT INDICATOR EX RDF';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rdf_alert_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_wk_rdf_alert %rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_wk_rdf_alert %rowtype index by binary_integer;
...
type staging_array1 is table of stg_rdf_alert_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rdf_alert_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cam_fcst is
   select *
   from stg_rdf_alert_cpy
...
begin
   ...
   g_rec_out.forecast_alert_ind              := g_rec_in.forecast_alert_ind;
...
begin
   ...
   insert into stg_rdf_alert_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_wk_rdf_alert  values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_wk_rdf_alert
       set    this_week_start_date           = a_tbl_update(i).this_week_start_date,
              forecast_alert_ind             = a_tbl_update(i).forecast_alert_ind,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_loc_item_wk_rdf_alert
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_DY_RDF_ALERT EX CAM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_cam_fcst;
    fetch c_stg_cam_fcst bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_210U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_210U_LOAD" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as
...
--  Tables:      Input  - stg_om_loc_item_dy_ord_cpy
--               Output - fnd_rtl_loc_item_dy_om_ord
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_om_loc_item_dy_ord_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_om_ord%rowtype;
g_rec_in             stg_om_loc_item_dy_ord_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_210U_LOAD'|| p_from_loc_no;
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ROQ/CUST ORDERS FACTS EX OM';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_om_loc_item_dy_ord_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_om_ord%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_om_ord%rowtype index by binary_integer;
...
type staging_array1 is table of stg_om_loc_item_dy_ord_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_om_loc_item_dy_ord_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_om_loc_item_dy_ord is
   --select *
   --from stg_om_loc_item_dy_ord_cpy
   --where sys_process_code = 'N' and
   --      location_no      between p_from_loc_no and p_to_loc_no
   --order by sys_source_batch_id,sys_source_sequence_no;
   
  select a.*
   from stg_om_loc_item_dy_ord_arc a, dim_item b, fnd_jdaff_dept_rollout c
   where a.sys_load_date = '10/AUG/14'
     and a.sys_process_code = 'N'
     and a.sys_source_batch_id = 1996
...
begin
   ...
   if g_rec_in.weigh_ind = 1 then
      g_rec_out.roq_qty                := g_rec_out.roq_qty/1000;
   end if;
...
begin
   ...
   insert into stg_om_loc_item_dy_ord_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_om_ord values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_om_ord
       set    roq_qty                    = a_tbl_update(i).roq_qty,
              cust_order_cases           = a_tbl_update(i).cust_order_cases ,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_om_loc_item_dy_ord_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_loc_item_dy_om_ord
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_DY_OM_ORD EX OM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_om_loc_item_dy_ord;
    fetch c_stg_om_loc_item_dy_ord bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_om_loc_item_dy_ord;
--**************************************************************************************************

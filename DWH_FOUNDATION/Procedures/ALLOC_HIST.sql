--------------------------------------------------------
--  DDL for Procedure ALLOC_HIST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."ALLOC_HIST" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        April 2008
--  Author:      Alastair de Wet
--  Purpose:     Populate alloc hist
...
g_rec_in             fnd_rtl_allocation%rowtype;
g_found              boolean;
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ALLOC FACTS EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of fnd_rtl_allocation%rowtype;
...
cursor c_stg_rms_rtl_contract is
   ...
   where wac is null  and
         release_date = '02 jan 2009';
...
begin
   ...
   g_rec_out.wac                             := 0;
   g_start_date                              := g_rec_in.release_date - 30 ;
   
   begin
         select wac
         into   g_rec_out.wac
         FROM   fnd_rtl_loc_item_dy_rms_wac   
         where  location_no         =  g_rec_in.to_loc_no    and
                item_no             =  g_rec_in.item_no      and
                tran_date           <= g_rec_in.release_date and
                tran_date           >  g_start_date and
                rownum               = 1
...
begin
    ...
    l_text := 'LOAD OF fnd_rtl_allocation PRICE HIST STARTED AT '||

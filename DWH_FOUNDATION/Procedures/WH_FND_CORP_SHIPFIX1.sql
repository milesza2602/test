--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_SHIPFIX1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_SHIPFIX1" 
(p_forall_limit in integer,p_success out boolean)
as
--**************************************************************************************************
--  Date:        AUG 2016
--  Author:      Wendy Lyttle
--  Purpose:     shipments datafix - cal_year 2015
...
where log_procedure_name = 'WH_FND_CORP_SHIPFIX1'
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_SHIPFIX1';
...
BEGIN
  ...
  BEGIN
    ...
    WHERE CONSTRAINT_NAME = 'PK_P_RMS_SHPMNT_SHPSKU'
    AND TABLE_NAME        = 'RMS_SHIPMENT_SHIPSKU';
    
    l_text               := 'drop constraint PK_P_RMS_SHPMNT_SHPSKU';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_FOUNDATION.RMS_SHIPMENT_SHIPSKU drop constraint PK_P_RMS_SHPMNT_SHPSKU');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'constraint PK_P_RMS_SHPMNT_SHPSKU does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
...
------------------------------------------------------------------------------
--               3. extract into DWH_FOUNDATION.RMS_SHIPMENT_SHIPSKU
--                  from SUPP_SMOT.SHIPMENT, SUPP_SMOT.SHIPSKU
--                  joined on shipment, seq_no
--                  and where ship_date >= '1 jan 2015'
--????????                  and status <> 'D' ?????
------------------------------------------------------------------------------
procedure A3_RMS_join_hdr_det as
begin

    insert /*+ append */ into DWH_FOUNDATION.RMS_SHIPMENT_SHIPSKU
      ...
      commit;
   l_text := 'insert RMS_shipment_shipsku = '||g_recs_inserted;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
end A3_RMS_join_hdr_det;

------------------------------------------------------------------------------
--                  4. Add primary key and indexes from DWH_FOUNDATION.RMS_SHIPMENT_SHIPSKU
------------------------------------------------------------------------------
procedure A4_ADD_INDEXES as
BEGIN
      l_text          := 'Running GATHER_TABLE_STATS ON RMS_SHIPMENT_SHIPSKU';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'RMS_SHIPMENT_SHIPSKU', DEGREE => 8);

      l_text := 'add constraint PK_P_RMS_SHPMNT_SHPSKU';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('alter table dwh_FOUNDATION.RMS_SHIPMENT_SHIPSKU add CONSTRAINT PK_P_RMS_SHPMNT_SHPSKU PRIMARY KEY (hSHIPMENT,dSEQ_NO,dITEM)                    
...
------------------------------------------------------------------------------
--                  5. Remove primary key and indexes from DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT
------------------------------------------------------------------------------
...
BEGIN
  ...
  BEGIN
    ...
    INTO G_name
    FROM ALL_CONSTRAINTS
    WHERE CONSTRAINT_NAME = 'PK_P_RMS_ODWH_STG_SHPMNT'
    AND TABLE_NAME        = 'RMS_ODWH_STG_SHIPMENT';
    
    l_text               := 'alter table dwh_FOUNDATION.RMS_ODWH_STG_SHIPMENT drop constraint PK_P_RMS_ODWH_STG_SHPMNT';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    EXECUTE immediate('alter table dwh_FOUNDATION.RMS_ODWH_STG_SHIPMENT drop constraint PK_P_RMS_ODWH_STG_SHPMNT');
    COMMIT;
    
  EXCEPTION
  WHEN no_data_found THEN
    l_text := 'constraint PK_P_RMS_ODWH_STG_SHPMNT does not exist';
    dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
  END;
...
------------------------------------------------------------------------------
--               6. extract into DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT
--                  from DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT, DWH_FOUNDATION.FND_RTL_SHIPMENT
--                  left outer join on shipment, seq_no, item_no
------------------------------------------------------------------------------
procedure A6_RMS_join_ODWH as
begin

     insert /*+ append */ into DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT
            WITH SELCAL AS (SELECT CALENDAR_DATE 
                            FROM DIM_CALENDAR 
                            WHERE CALENDAR_DATE between g_start_date and g_end_date),
                 SELFND AS (
            ...
            SELECT /*+ full(RMS) PARALLEL(RMS,4) */ HSHIPMENT
                  ,HORDER_NO
                  ,HBOL_NO
                  ,HASN
                  ,HSHIP_DATE
                  ...
                  ,CHAIN_CODE
            FROM DWH_FOUNDATION.RMS_SHIPMENT_SHIPSKU RMS, SELFND FND
            WHERE RMS.hSHIPMENT = FND.SHIPMENT_NO(+)
            AND RMS.dSEQ_NO = FND.SEQ_NO(+)
            AND TO_NUMBER(RMS.dITEM) = TO_CHAR((nvl(FND.ITEM_NO,0)));

      g_recs_inserted := 0;
      g_recs_inserted := sql%rowcount;

      commit;
   l_text := 'insert RMS_ODWH_stg_shipment = '||g_recs_inserted;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
end A6_RMS_join_ODWH;

------------------------------------------------------------------------------
--                  7. Add primary key and indexes from DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT
------------------------------------------------------------------------------
procedure A7_ADD_INDEXES as
BEGIN

      l_text          := 'Running GATHER_TABLE_STATS ON RMS_ODWH_STG_SHIPMENT';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'RMS_ODWH_STG_SHIPMENT', DEGREE => 8);

      l_text := 'add constraint PK_P_RMS_ODWH_STG_SHPMNT';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      EXECUTE immediate('alter table dwh_FOUNDATION.RMS_ODWH_STG_SHIPMENT add CONSTRAINT PK_P_RMS_ODWH_STG_SHPMNT PRIMARY KEY (SHIPMENT_NO,hSHIPMENT,dSEQ_NO,dITEM )                    
      USING INDEX tABLESPACE FND_MASTER  ENABLE');
...
------------------------------------------------------------------------------
--               8. extract into DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX
--                  from DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT
--                  the records we want to fix
------------------------------------------------------------------------------
procedure A8_ODWH_extract_fix as
begin

      insert /*+ append */ into DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX
            select /*+  PARALLEL(RMS,4) */ 
                    HSHIPMENT
                  ,	DSHIPMENT
                  ,	SHIPMENT_NO
                  ,	DSEQ_NO
                  ...
                  ,	TO_LOC_DEBT_COMM_PERC
                  ,	TSF_PO_LINK_NO
      from DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT RMS
...
union all
    ...
    ;
      ...
      commit;
      l_text := 'insert ODWH_stg_shipment_fix = '||g_recs_inserted;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
end A8_ODWH_extract_fix ;

------------------------------------------------------------------------------
--               9. extract into DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX_CPY
--                  from DWH_FOUNDATION.ODWH_STG_SHIPMENT_fix
--                  the records we want to fix in the correct format for loading 
------------------------------------------------------------------------------
 procedure A9_ODWH_format_fix_stg_cpy  as
   begin

        insert /*+ append */ into DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX_CPY
            SELECT /*+ full(ODW) PARALLEL(ODW,4) */
              7777777777 SYS_SOURCE_BATCH_ID
            ...
            FROM DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX ODW;
      g_recs_inserted := 0;
      g_recs_inserted := sql%rowcount;

      commit;
   l_text := 'insert ODWH_stg_shipment_fix_cpy = '||g_recs_inserted;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
end A9_ODWH_format_fix_stg_cpy;
...
------------------------------------------------------------------------------
 procedure A10_cnt_diff  as
   begin
        ...
        insert /*+ append */ into DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX_diff
with 
selcnt as (   select /*+  full(RMS) PARALLEL(RMS,4) */ trunc(hship_date) ship_date,
...
;
      ...
      commit;
      l_text := 'insert ODWH_stg_shipment_fix_diff = '||g_recs_inserted;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    g_cnt := 0;
    g_cnt := g_cnt + 1;
--   g_start_date := '29 DEC 2014';
--   g_end_date := '4 jan 2015';
--   g_start_date := '9 feb 2015';
--   g_end_date := '15 feb 2015';

   g_start_date := '23 nov 2015';
   g_end_date := '29 nov 2015';          
/*                          l_text := '**- cnt'||g_cnt||'- - - -*- - - - - - -*- - - - - -*- - - - -*- - -**' ;
                            dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
                            ...
                            l_text := 'Period = '||g_start_date||' to '||g_end_date;
                             ...
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
   A2_REMOVE_INDEXES;
                             l_text := 'truncate RMS_SHIPMENT_SHIPSKU' ;
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   execute immediate 'truncate table DWH_FOUNDATION.RMS_SHIPMENT_SHIPSKU';
   A3_RMS_JOIN_HDR_DET;
   A4_ADD_INDEXES;
                             ...
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
  A5_REMOVE_INDEXES;
                             l_text := 'truncate RMS_ODWH_STG_SHIPMENT' ;
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
  execute immediate 'truncate table DWH_FOUNDATION.RMS_ODWH_STG_SHIPMENT';
  A6_RMS_JOIN_ODWH;
  A7_ADD_INDEXES;

                             l_text := '-------------';
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
                             l_text := 'truncate ODWH_STG_SHIPMENT_FIX' ;
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   execute immediate 'truncate table DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX';
   A8_ODWH_EXTRACT_FIX;

                             l_text := '-------------';
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
                             l_text := 'truncate ODWH_STG_SHIPMENT_FIX_CPY' ;
                             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    execute immediate 'truncate table DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX_CPY';
    A9_ODWH_FORMAT_FIX_STG_CPY;
                            l_text          := 'Running GATHER_TABLE_STATS ON ODWH_STG_SHIPMENT_FIX_CPY';
                            dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'ODWH_STG_SHIPMENT_FIX_CPY', DEGREE => 8);
*/
--**************************************************************************************************
...
--**************************************************************************************************
  for g_sub in 0..4 loop
             ...
             A7_ADD_INDEXES;
                                         ...
                                         dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
             execute immediate 'truncate table DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX';
                                          l_text          := 'Running GATHER_TABLE_STATS ON ODWH_STG_SHIPMENT_FIX';
                                          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
             DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'ODWH_STG_SHIPMENT_FIX', DEGREE => 8);
             A8_ODWH_EXTRACT_FIX;
...
--************************************************************************************************   
                                 ...
                                 l_text := 'truncate ODWH_STG_SHIPMENT_FIX_DIFF' ;
                                 dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     execute immediate 'truncate table DWH_FOUNDATION.ODWH_STG_SHIPMENT_FIX_DIFF';
     A10_CNT_DIFF;

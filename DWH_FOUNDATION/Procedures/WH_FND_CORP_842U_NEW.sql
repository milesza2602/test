--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_842U_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_842U_NEW" 
...
-- 1. Removed FTS hints and forced use of index on S2 in cursor c_fnd_alloc_ship
-- 2. Suggest creating indexes on fnd_rtl_shipment(dist_no, final_loc_no, item_no) and fnd_rtl_shipment(tsf_alloc_no, final_loc_no, item_no) and assess impact
-- 3. Suggest removing the duplicate row check in local_write_output and updating local_bulk_insert to use a merge
-- **************************************************************************************************


-- **************************************************************************************************
--  Date:        February 2010DECLARE
--  Author:      M Munnik
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_842U_NEW_'|| p_from_loc_no;
...
SELECT
  /*+ index(s2 I10_F_FND_RTL_SHIPMENT) parallel(a 4) */
  a.release_date,
  a.alloc_no,
  s2.to_loc_no,

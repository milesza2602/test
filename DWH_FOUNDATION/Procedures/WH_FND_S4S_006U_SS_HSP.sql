--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_006U_SS_HSP
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_006U_SS_HSP" (p_forall_limit IN INTEGER, p_success OUT BOOLEAN)AS
  --**************************************************************************************************
  --  Date:        July 2014
  --  Author:      Wendy lyttle
...
begin
    ...
    with selstg
     ...
     select g_run_date, g_date, g_run_seq_no, f.*
        from DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX f, selstg s
        where f.employee_id = s.employee_id
         ...
         delete from DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX
      ...
      l_text := 'Deleted from DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      dbms_output.put_line('Write 3 ');
    exception
         when no_data_found then
                l_text := 'No deletions done for DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX ';
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
   end;          
...
BEGIN
  ...
  SAVE exceptions
  INSERT
  INTO DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX VALUES a_tbl_insert(i);
...
BEGIN
  ...
  SAVE exceptions
  UPDATE DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX
...
BEGIN
  ...
  SELECT COUNT(1)
  INTO v_count
   FROM DWH_FOUNDATION.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX
...
--**************************************************************************************************
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   ELSE
        ...
        l_text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        EXECUTE IMMEDIATE ('TRUNCATE TABLE dwh_foundation.FND_S4S_LOC_EMP_JB_PYCT_DY_SSX');
        l_text := 'Running GATHER_TABLE_STATS ON FND_S4S_LOC_EMP_JB_PYCT_DY_SSX';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_S4S_LOC_EMP_JB_PYCT_DY_SSX', DEGREE => 8);

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_204U_LOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_204U_LOAD" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as


--**************************************************************************************************
--  Date:        April 2008
--  Author:      Alastair de Wet
--  Purpose:     Create RMS Stock Foods fact table in the foundation layer
--               with input ex staging table from RMS.
--  Tables:      Input  - stg_rms_stock_fds_cpy
--               Output - fnd_rtl_loc_item_dy_rms_stk_fd
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_stock_fd_hsp.sys_process_msg%type;
g_item_level_no      fnd_item.item_level_no%type;
g_tran_level_no      fnd_item.tran_level_no%type;
g_rec_out            fnd_rtl_loc_item_dy_rms_stk_fd%rowtype;
g_rec_in             stg_rms_stock_fd_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_204U_LOAD'|| p_from_loc_no;
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE RMS STOCK FD FACTS EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rms_stock_fd_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_rms_stk_fd%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_rms_stk_fd%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_stock_fd_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_stock_fd_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_stock_fd is
   select *
   from stg_rms_stock_fd_cpy
   where sys_process_code = 'Y' and
...
begin
   ...
   g_rec_out.boh_selling                     := g_rec_in.boh_selling;
   g_rec_out.boh_cost                        := g_rec_in.boh_cost;
...
--   g_rec_out.source_data_status_code         := g_rec_in.source_data_status_code;
   ...
   if not  dwh_valid.fnd_calendar(g_rec_out.post_date) then
    ...
    if not  dwh_valid.indicator_field(g_rec_out.com_flag_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     l_text          := dwh_constants.vc_invalid_indicator||g_rec_out.com_flag_ind ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     return;
   end if;
...
begin
   ...
   insert into stg_rms_stock_fd_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_rms_stk_fd values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_rms_stk_fd
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_DY_STOCK EX RMS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_stock_fd;
    fetch c_stg_rms_stock_fd bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_MP_015U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MP_015U" 
...
--  Purpose:     Create Weekly Authorised Plan table in the foundation layer
--               with input ex staging table from MP.
--  Tables:      Input  - stg_mp_chain_sc_wk_auth_cpy
--               Output - fnd_rtl_chain_sc_wk_mp_auth
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_mp_chain_sc_wk_auth_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_chain_sc_wk_mp_auth%rowtype;
g_rec_in             stg_mp_chain_sc_wk_auth_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MP_015U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WEEKLY AUTHORISED PLAN EX MP';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_mp_chain_sc_wk_auth_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_chain_sc_wk_mp_auth%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_chain_sc_wk_mp_auth%rowtype index by binary_integer;
...
type staging_array1 is table of stg_mp_chain_sc_wk_auth_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_mp_chain_sc_wk_auth_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_mp_chain_sc_wk_auth is
   select *
   from stg_mp_chain_sc_wk_auth_cpy
...
begin
   ...
   g_rec_out.auth_sales_qty                  := g_rec_in.auth_sales_qty;
   g_rec_out.auth_sales_selling              := g_rec_in.auth_sales_selling;
   g_rec_out.auth_ord_qty                    := g_rec_in.auth_ord_qty;
   g_rec_out.auth_ord_selling                := g_rec_in.auth_ord_selling;
...
begin
   ...
   insert into stg_mp_chain_sc_wk_auth_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_chain_sc_wk_mp_auth values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_chain_sc_wk_mp_auth
       set    auth_sales_qty                 = a_tbl_update(i).auth_sales_qty,
              auth_sales_selling             = a_tbl_update(i).auth_sales_selling,
              auth_ord_qty                   = a_tbl_update(i).auth_ord_qty,
              auth_ord_selling               = a_tbl_update(i).auth_ord_selling,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_mp_chain_sc_wk_auth_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_chain_sc_wk_mp_auth
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_CHAIN_SC_WK_MP_AUTH EX MP STARTED AT '||
...
--**************************************************************************************************
    open c_stg_mp_chain_sc_wk_auth;
    fetch c_stg_mp_chain_sc_wk_auth bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_mp_chain_sc_wk_auth;
--**************************************************************************************************

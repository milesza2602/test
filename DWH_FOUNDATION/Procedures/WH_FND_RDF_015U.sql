--------------------------------------------------------
--  DDL for Procedure WH_FND_RDF_015U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RDF_015U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Alfonso Joshua
--  Purpose:     Create CAM Daily Foods Forecast table in the foundation layer
--               with input ex staging table from CAM.
--  Tables:      Input  - stg_cam_fcst_cpy
--               Output - fnd_rtl_loc_item_dy_cam_fcst
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_cam_fcst_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_cam_fcst%rowtype;
g_rec_in             stg_cam_fcst_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RDF_015U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE DAILY FOODS FORECAST EX CAM';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_cam_fcst_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_cam_fcst%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_cam_fcst%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cam_fcst_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cam_fcst_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   insert into stg_cam_fcst_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_cam_fcst values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_cam_fcst
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_loc_item_dy_cam_fcst
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_DY_CAM_FCST EX RDF STARTED AT '||

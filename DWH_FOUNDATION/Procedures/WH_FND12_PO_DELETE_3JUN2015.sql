--------------------------------------------------------
--  DDL for Procedure WH_FND12_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND12_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND12_PO_DELETE_3JUN2015';
...
FOR V_CUR IN
            ...
            and ship_date <= '19/MAY/09'
)
LOOP


      begin
            DELETE /*+ FULL(A)  */ FROM dwh_foundation.FND_RTL_SHIPMENT a
              where a.PO_NO = V_CUR.PO_NO;
              
              g_recs_DELETED_FRS := g_recs_DELETED_FRS + SQL%ROWCOUNT;
              commit;
              
                                 if g_recs_DELETED_FRS >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_FRS;
        ...
        when no_data_found then
          g_recs_DELETED_FRS := g_recs_DELETED_FRS ;
      end;      
...
l_text := 'FND_RTL_SHIPMENT recs deleted = '||g_recS_deleted_FRS;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
END WH_FND12_PO_DELETE_3JUN2015;

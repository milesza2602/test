--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_112U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_112U_FIX" 
...
--               Input    - temp_stg_rms_prom_location
...
g_rec_in             temp_stg_rms_prom_location%rowtype;
g_found              boolean;
g_valid              boolean;
g_date               date          := trunc(sysdate);
g_batch_id           temp_stg_rms_prom_location.sys_source_batch_id%type:=9014;
g_from_rec           NUMBER(10,0)  := 210001;
g_to_rec             NUMBER(10,0)  := 2100000;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_112U_FIX';
...
type stg_array is table of stg_rms_prom_location_cpy%rowtype;
...
cursor c_temp_stg_rms_prom_location is
   select *
   from temp_stg_rms_prom_location
   where sys_process_code = 'N'
   and sys_source_batch_id = g_batch_id
   and sys_source_sequence_no between g_from_rec and g_to_rec
...
begin
   ...
   insert into stg_rms_prom_location_hsp values g_rec_in;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update temp_stg_rms_prom_location
...
begin
    ...
    l_text := 'LOAD OF FND_PROM_LOCATION EX RMS STARTED '||
    to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
    l_text := 'BATCH = '|| g_batch_id || ' From seq = ' || g_from_rec || ' To seq = ' || g_to_rec;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
    open c_temp_stg_rms_prom_location;
    fetch c_temp_stg_rms_prom_location bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_temp_stg_rms_prom_location;
--**************************************************************************************************

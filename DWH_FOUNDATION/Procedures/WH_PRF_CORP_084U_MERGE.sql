--------------------------------------------------------
--  DDL for Procedure WH_PRF_CORP_084U_MERGE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_PRF_CORP_084U_MERGE" (p_forall_limit in integer,p_success out boolean) as
-- ******************************************************************************************
--  Date:        Sept 2008
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_084M';
...
begin
  ...
  merge /*+ parallel(fli,4) */ into w6005682.fnd_location_item_qs fli using 
  (
   select /*+ parallel(fiz,4) */ 
          fiz.*,
          location_no
   ...
   and   (
            fli.next_wk_deliv_pattern_code      <> mer_mart.reg_delivery_pattern_code or
            fli.this_wk_deliv_pattern_code      <> mer_mart.reg_delivery_pattern_code or
            fli.num_shelf_life_days             <> mer_mart.num_shelf_life_days or
            fli.num_units_per_tray              <> mer_mart.num_units_per_tray or
            fli.next_wk_deliv_pattern_code      is null or
            fli.this_wk_deliv_pattern_code      is null or
            fli.num_shelf_life_days             is null or
            fli.num_units_per_tray              is null 
          )
      )

--------------------------------------------------------
--  DDL for Procedure WH_FND_MC_204U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MC_204U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        November 2017
--  Author:      Bhavesh Valodia
--  Purpose:     Update STG_RMS_MC_STOCK_FD dimention table in the foundation layer
--               with input ex staging table from RMS.
--  Tables:      Input  - stg_rms_mc_stock_fd_cpy
--               Output - FND_MC_LOC_ITEM_DY_RMS_STK_FD
...
g_hospital           char(1)       := 'N';
g_hospital_text      dwh_foundation.stg_rms_mc_stock_fd_hsp.sys_process_msg%type;
mer_mart             dwh_foundation.stg_rms_mc_stock_fd_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MC_204U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STOCK_FD MASTERDATA EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

g_location_no     stg_rms_mc_stock_fd_cpy.location_no%type; 
g_item_no           stg_rms_mc_stock_fd_cpy.item_no%type; 
g_post_date           stg_rms_mc_stock_fd_cpy.post_date%type;


   cursor stg_dup is
      select * 
        from dwh_foundation.stg_rms_mc_stock_fd_cpy
       where (location_no, item_no, post_date)
          in
     (select location_no, item_no, post_date
        from stg_rms_mc_stock_fd_cpy 
...
begin
    ...
    l_text := 'LOAD OF MC_STOCK_FD EX RMS STARTED AT '||
    ...
    for dupp_record in stg_dup
       loop
        ...
        if  dupp_record.location_no = g_location_no and
            ...
            then
            update stg_rms_mc_stock_fd_cpy stg
...
merge /*+ parallel (fnd_mart,6) */ 
    into DWH_FOUNDATION.FND_MC_LOC_ITEM_DY_RMS_STK_FD fnd_mart 
    using (
    select /*+ FULL(TMP) */ tmp.*
    from DWH_FOUNDATION.STG_RMS_MC_STOCK_FD_CPY  tmp 
...
set           SOH_QTY                        = mer_mart.soh_qty,
              ...
              BOH_QTY                        = mer_mart.boh_qty,
              FD_NUM_AVAIL_DAYS              = mer_mart.fd_num_avail_days,
       --       SOURCE_DATA_STATUS_CODE      = mer_mart.source_data_status_code,
              LAST_UPDATED_DATE              = g_date,
              BOH_COST_LOCAL                 = mer_mart.boh_cost_local,
              BOH_COST_OPR                   = mer_mart.boh_cost_opr,
              BOH_SELLING_LOCAL              = mer_mart.boh_selling_local,
              BOH_SELLING_OPR                = mer_mart.boh_selling_opr,
...
(         mer_mart.LOCATION_NO,
          ...
          mer_mart.BOH_QTY,
          mer_mart.FD_NUM_AVAIL_DAYS,
     --     mer_mart.SOURCE_DATA_STATUS_CODE,
          g_date,
          mer_mart.BOH_COST_LOCAL,
          mer_mart.BOH_COST_OPR,
          mer_mart.BOH_SELLING_LOCAL,
          mer_mart.BOH_SELLING_OPR,
...
--**************************************************************************************************
   ...
   insert /*+ APPEND parallel (hsp,2) */ into STG_RMS_MC_STOCK_FD_HSP hsp 
   select /*+ FULL(TMP) */  TMP.sys_source_batch_id,
                            ...
                            TMP.BOH_QTY,
                            TMP.FD_NUM_AVAIL_DAYS,
                 --           TMP.SOURCE_DATA_STATUS_CODE,
                            TMP.BOH_COST_LOCAL,
                            TMP.BOH_COST_OPR,
                            TMP.BOH_SELLING_LOCAL,
                            TMP.BOH_SELLING_OPR,
   ...
   from  STG_RMS_MC_STOCK_FD_CPY  TMP 

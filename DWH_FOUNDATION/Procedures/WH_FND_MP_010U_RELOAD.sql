--------------------------------------------------------
--  DDL for Procedure WH_FND_MP_010U_RELOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MP_010U_RELOAD" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MP_010U_RELOAD';
...
--**************************************************************************************************
    ...
    EXECUTE IMMEDIATE('TRUNCATE TABLE  DWH_FOUNDATION.fnd_chain_dept_wk_plan');
       l_text := 'Truncate DWH_FOUNDATION.fnd_chain_dept_wk_plan';
       dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
       
       select count(*) into g_recs from dwh_foundation.stg_mp_chain_dept_wk_plan_cpy;
       g_recs_read := g_recs;
       l_text := 'recs read = '|| g_recs;
       dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
        g_recs  := 0;
        
    insert /*+ append */ inTO DWH_FOUNDATION.fnd_chain_dept_wk_plan
    SELECT /*+ FULL(A) PARALLEL(A,8) */ 
        ...
        from dwh_foundation.stg_mp_chain_dept_wk_plan_cpy A,
              dwh_foundation.fnd_department fi,
              dwh_foundation.fnd_chain fc
        where plan_type_no  in (50,51,52,53,54,55) 

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014B_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014B_TAKEON" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014B_TAKEON';
...
WITH SELEXT AS
  (SELECT  workgroup_id ,
            MAX(workgroup_seq) workgroup_seq
  FROM fnd_s4s_labour_hierarchy
  GROUP BY workgroup_id )
  SELECT  FND.workgroup_id ,
  ...
  FROM fnd_s4s_labour_hierarchy FND,
    SELEXT SE
  WHERE FND.workgroup_id  = SE.workgroup_id
    and  FND.workgroup_seq = SE.workgroup_seq
    group by fND.workgroup_id ,
          FND.s4s_business_unit_no ,
           FND.s4s_business_unit_nAME ,
          FND.workgroup_name,
          SE.workgroup_seq;

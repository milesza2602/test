--------------------------------------------------------
--  DDL for Procedure WH_FND_RDF_800U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RDF_800U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        February 2015
--  Author:      Quentin Smit
--  Purpose:     Create RDF Weekly Foods Forecast LEVEL 2(DEPARTMENT LEVEL) table in the foundation layer
--               with input ex staging table from RDF.
--               3-weeks extract
--  Tables:      Input  - stg_rdf_rtl_wkfcst_CPY
--               Output - FND_LOC_ITEM_RDF_WKFCST_L2
...
g_hospital           char(1)       := 'N';
g_hospital_text      STG_RDF_WKFCST_L2_HSP.sys_process_msg%type;
g_rec_out            FND_LOC_ITEM_RDF_WKFCST_L2%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RDF_800U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WEEKLY FOODS FORECAST LEVEL2 EX RDF';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For output arrays into bulk load forall statements --
type tbl_array_i is table of FND_LOC_ITEM_RDF_WKFCST_L2%rowtype index by binary_integer;
type tbl_array_u is table of FND_LOC_ITEM_RDF_WKFCST_L2%rowtype index by binary_integer;
...
type staging_array1 is table of STG_RDF_WKFCST_L2_CPY.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of STG_RDF_WKFCST_L2_CPY.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rdf_rtl_wkfcst is
   select /*+ full(st) */ ST.*, FI.ITEM_NO FI_ITEM_NO, FL.LOCATION_NO FL_LOCATION_NO, DC.fin_year_no dc_fin_year_no
   from STG_RDF_WKFCST_L2_CPY ST
...
g_rec_in             c_stg_rdf_rtl_wkfcst%rowtype;
-- For input bulk collect --
type stg_array is table of c_stg_rdf_rtl_wkfcst%rowtype;
...
begin
  ...
  if g_rec_in.FL_LOCATION_NO IS NULL then
   ...
   if g_rec_in.DC_fin_year_no IS NULL then
     g_hospital      := 'Y';
...
begin
   ...
   insert into STG_RDF_WKFCST_L2_HSP values
                               ( G_REC_IN.SYS_SOURCE_BATCH_ID
                                ...
                                , G_REC_IN.WK_02_APP_FCST_QTY
                                , G_REC_IN.WK_03_APP_FCST_QTY
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into FND_LOC_ITEM_RDF_WKFCST_L2 values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update FND_LOC_ITEM_RDF_WKFCST_L2
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   FND_LOC_ITEM_RDF_WKFCST_L2
...
begin
    ...
    l_text := 'LOAD THE WEEKLY FOODS FORECAST LEVEL2 EX RDF EX RDF STARTED AT '||

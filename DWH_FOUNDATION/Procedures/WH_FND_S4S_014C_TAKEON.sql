--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014C_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014C_TAKEON" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014C_TAKEON';
...
WITH SELEXT AS
  (SELECT  jobgroup_id ,
            MAX(jobgroup_seq) jobgroup_seq
  FROM fnd_s4s_labour_hierarchy
  GROUP BY jobgroup_id  )
  SELECT  FND.jobgroup_id ,
          FND.workgroup_id ,
          FND.workgroup_name ,
          FND.jobgroup_name,
          SE.jobgroup_seq,
  ...
  FROM fnd_s4s_labour_hierarchy FND,
    SELEXT SE
  WHERE FND.jobgroup_id  = SE.jobgroup_id
        AND FND.jobgroup_seq = SE.jobgroup_seq
  group by FND.jobgroup_id ,
          FND.workgroup_id ,
          FND.workgroup_name ,
          FND.jobgroup_name,
          SE.jobgroup_seq,

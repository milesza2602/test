--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_648U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_648U" 
...
--  Date:        August 2008
--  Author:      Sean Le Roux
--  Purpose:     Load deal_item location information in the foundation layer
--               with input ex staging table from RMS.
--  Tables:      Input  - stg_rms_deal_item_loc_cpy
--               Output - fnd_deal_item_loc
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_deal_item_loc_hsp.sys_process_msg%type;
g_rec_out            fnd_deal_item_loc%rowtype;
g_rec_in             stg_rms_deal_item_loc_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_648U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE FND_DEAL_ITEM_LOC DATA EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rms_deal_item_loc_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_deal_item_loc%rowtype index by binary_integer;
type tbl_array_u is table of fnd_deal_item_loc%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_deal_item_loc_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_deal_item_loc_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_deal_item_loc is
   select *
   from stg_rms_deal_item_loc_cpy
...
begin
   ...
   g_rec_out.merch_hrchy_level_no            := g_rec_in.merch_hrchy_level_no;
   g_rec_out.company_ind                     := g_rec_in.company_ind;
   ...
   g_rec_out.org_hrchy_level_no              := g_rec_in.org_hrchy_level_no;
   ...
   g_rec_out.exclude_from_deal_ind           := g_rec_in.exclude_from_deal_ind;
...
begin
   ...
   insert into stg_rms_deal_item_loc_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_deal_item_loc values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_deal_item_loc
       set    merch_hrchy_level_no            = a_tbl_update(i).merch_hrchy_level_no,
              company_ind                     = a_tbl_update(i).company_ind,
              ...
              org_hrchy_level_no              = a_tbl_update(i).org_hrchy_level_no,
              ...
              exclude_from_deal_ind           = a_tbl_update(i).exclude_from_deal_ind,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_deal_item_loc_cpy
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).deal_no = g_rec_out.deal_no
          ...
          and a_tbl_insert(i).seq_no = g_rec_out.seq_no   then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF FND_DEAL_item_loc EX RMS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_deal_item_loc;
    fetch c_stg_rms_deal_item_loc bulk collect into a_stg_input limit g_forall_limit;

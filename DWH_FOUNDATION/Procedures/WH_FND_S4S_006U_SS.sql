--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_006U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_006U_SS" (p_forall_limit IN INTEGER, p_success OUT BOOLEAN)AS
  --**************************************************************************************************
  --  Date:        July 2014
  --  Author:      Wendy lyttle
...
l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_S4S_006U_SS';
...
type staging_array1
IS
  TABLE OF STG_S4S_LOC_EMP_JB_PYCT_DY_HSP.sys_source_batch_id%type INDEX BY binary_integer;
type staging_array2
IS
  TABLE OF STG_S4S_LOC_EMP_JB_PYCT_DY_HSP.sys_source_sequence_no%type INDEX BY binary_integer;
  ...
  is
    select /*+ full(stg) parallel(stg,8) */ 
        sys_source_batch_id,
      ...
      from dwh_foundation.stg_s4s_loc_emp_jb_pyct_dy_hsp stg
          ,fnd_location fl
     ...
     where stg.location_no          = fl.location_no(+)
       ...
       and stg.sys_load_date > '25 jun 17'
       and stg.sys_process_msg = 'JOB_ID NOT FOUND'
       and stg.job_id not in (2681,4085,4093,4161,4403,4584,4607,5706,999985)
...
begin
     dbms_output.put_line('Write 1 ');
     g_recs_inserted := 0;

      select max(run_seq_no)+1 into g_run_seq_no
      from dwh_foundation.FND_S4S_LCEMPJBPYCTDY_DL_LT_SS;
      
      g_run_date := trunc(sysdate);
     BEGIN
     insert /*+ append */ 

     into dwh_foundation.FND_S4S_LCEMPJBPYCTDY_DL_LT_SS
    with selstg
            as (select distinct employee_id, pay_week_date from STG_S4S_LOC_EMP_JB_PYCT_DY_HSP)
     select g_run_date, g_date, g_run_seq_no, f.*
        from DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS f, selstg s
        where f.employee_id = s.employee_id
          and f.pay_week_date = s.pay_week_date;
          g_recs :=SQL%ROWCOUNT ;
          COMMIT;
          dbms_output.put_line('Write 2 ');
          g_recs_inserted := g_recs;
                
          l_text := 'Insert into FND_S4S_LCEMPJBPYCTDY_DL_LT_SS recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
          dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

         delete from DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS
         where (employee_id, pay_week_date) in (select distinct employee_id, pay_week_date from dwh_foundation.FND_S4S_LCEMPJBPYCTDY_DL_LT_SS
      ...
      l_text := 'Deleted from DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      dbms_output.put_line('Write 3 ');
    exception
         when no_data_found then
                l_text := 'No deletions done for DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS ';
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
   end;          
...
BEGIN
  ...
  SAVE exceptions
  INSERT
  INTO DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS VALUES a_tbl_insert(i);
...
BEGIN
  ...
  SAVE exceptions
  UPDATE DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS
...
BEGIN
  ...
  SELECT COUNT(1)
  INTO v_count
   FROM DWH_FOUNDATION.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS
...
--**************************************************************************************************
  dbms_output.put_line('Display data take on ');
  G_DATA_TAKEON := 'N';
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   If g_data_takeon <> 'Y' OR g_data_takeon IS NULL
       THEN 
             delete_fnd;
   ELSE
        l_text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_LCEMPJBPYCTDY_DL_LT_SS';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        EXECUTE IMMEDIATE ('TRUNCATE TABLE dwh_foundation.FND_S4S_LCEMPJBPYCTDY_DL_LT_SS');
        l_text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        EXECUTE IMMEDIATE ('TRUNCATE TABLE dwh_foundation.FND_S4S_LOC_EMP_JOB_PYCT_DY_SS');
        l_text := 'Running GATHER_TABLE_STATS ON FND_S4S_LOC_EMP_JOB_PYCT_DY_SS';
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_S4S_LOC_EMP_JOB_PYCT_DY_SS', DEGREE => 8);
...
--        EXECUTE IMMEDIATE ('TRUNCATE TABLE dwh_foundation.STG_S4S_LOC_EMP_JB_PYCT_DY_HSP');
  ...
  DWH_LOG.WRITE_LOG(L_NAME,L_SYSTEM_NAME,L_SCRIPT_NAME,L_PROCEDURE_NAME,L_TEXT);
  DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'FND_S4S_LOC_EMP_JOB_PYCT_DY_SS', DEGREE => 8);
      END IF;

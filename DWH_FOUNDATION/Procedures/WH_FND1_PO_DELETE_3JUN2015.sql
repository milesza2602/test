--------------------------------------------------------
--  DDL for Procedure WH_FND1_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND1_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND1_PO_DELETE_3JUN2015';
...
begin
  ...
  execute immediate 'alter session enable parallel dml';
            ...
            DELETE /*+  full(a)  */ FROM dwh_FOUNDATION.FND_RTL_INVOICE_MATCHING  a
              where a.order_no in (SELECT PO_NO
                          FROM DIM_PURCHASE_ORDER
                          WHERE 
                          PO_NO BETWEEN 1000000 AND 1500000
...
l_text := 'FND_RTL_INVOICE_MATCHING recs deleted = '||g_recs_deleted;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
 DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_RTL_INVOICE_MATCHING', DEGREE => 8);
            ...
            DELETE /*+  full(a)  */ FROM dwh_FOUNDATION.FND_RTL_INVOICE_MATCHING  a
              where a.order_no in (SELECT PO_NO
                          FROM DIM_PURCHASE_ORDER
                          WHERE 
                          PO_NO BETWEEN 1500000 AND 2000000
                          AND PO_STATUS_CODE = 'C'
                          );
 ...
 DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_RTL_INVOICE_MATCHING', DEGREE => 8);
            ...
            DELETE /*+  full(a)  */ FROM dwh_FOUNDATION.FND_RTL_INVOICE_MATCHING  a
              where a.order_no in (SELECT PO_NO
                          FROM DIM_PURCHASE_ORDER
                          WHERE 
                          PO_NO BETWEEN 2000000 AND 2500000
                          AND PO_STATUS_CODE = 'C'
                          );
 ...
 DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_RTL_INVOICE_MATCHING', DEGREE => 8);
           ...
           DELETE /*+  full(a)  */ FROM dwh_FOUNDATION.FND_RTL_INVOICE_MATCHING  a
              where a.order_no in (SELECT PO_NO
                          FROM DIM_PURCHASE_ORDER
                          WHERE 
                          PO_NO BETWEEN 2500000 AND 2999999
                          AND PO_STATUS_CODE = 'C'
                          );
...
-------------------------------------------------------------------------------------------------------------------------------
            ...
            DELETE /*+  full(a)  */ FROM dwh_FOUNDATION.FND_RTL_PURCHASE_ORDER  a
...
l_text := 'FND_RTL_PURCHASE_ORDER recs deleted = '||g_recs_deleted;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
 DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_RTL_PURCHASE_ORDER', DEGREE => 8);
...
l_text := 'FND_RTL_STOCKOWN_DCR recs deleted = '||g_recs_deleted;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
 DBMS_STATS.gather_table_stats ('DWH_FOUNDATION','FND_RTL_STOCKOWN_DCR', DEGREE => 8);
...
END WH_FND1_PO_DELETE_3JUN2015;

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_065U_LOAD25APR
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_065U_LOAD25APR" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  PLEASE NOTE THE FOLLOWING IF DOING A HISTORY RELOAD :
-----------------------------------------------------------
-- Until such time that the history is correct, we will be loading data sent for batch running on MOnday night 20th April 2015.
--**************************************************************************************************
--  PLEASE NOTE THE FOLLOWING IF DOING A HISTORY RELOAD :
-----------------------------------------------------------
...
--
     ...
     where  trunc(f.exception_date) >= G_MIN_EXCEPTION_DATE
              ;
...
--  g_date := '22 apr 2015';
  ...
  --
  -- Further caveat -- eventhough we have setup a cursor to laod the data in, we cannot allow more than one batch to load
  --                   as we have to set the batch_date eachtime and cannot be guaranteed that the date will always be the day before.
  --**************************************************************************************************
--421 20 april
--444 21 april
--445 22 april
  ...
  for v_cur in (
  select  sys_source_batch_id,sys_load_date
  , MIN(TRUNC(EXCEPTION_DATE)) MINEXCPTDATE
	from DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_EMP_DY_CPY
 -- where  
  --exception_date >= g_date
  --where sys_source_batch_id = 445
  GROUP BY sys_source_batch_id,sys_load_date
   ...
   G_MIN_EXCEPTION_DATE := V_CUR.MINEXCPTDATE;
   l_text := '------------------------------------|-------------------------------------';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   l_text := ' G_MIN_EXCEPTION_DATE ='|| G_MIN_EXCEPTION_DATE;
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--  ELSE
 ...
 --------------------------------  
      insert /*+ append */ into DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMP_DY
            ...
            AND stg.EXCEPTION_DATE >= G_MIN_EXCEPTION_DATE
                     ;
...
--**************************************************************************************************
 ...
 -----------------------------------------------------------------------------------------
-- temporary snapshot check
--
insert /*+ append */ into DWH_FOUNDATION.FND_S4S_SCHEDXCPTNEMPDY_snap
select g_date, g_SYS_SOURCE_BATCH_ID, a.* from DWH_FOUNDATION.FND_S4S_SCHED_XCPTN_EMP_DY a
;
commit;
    l_text := 'Creating snapshot of FND_S4S_SCHED_XCPTN_EMP_DY';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
-----------------------------------------------------------------------------------------	  

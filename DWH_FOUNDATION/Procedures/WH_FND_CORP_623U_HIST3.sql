--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_623U_HIST3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_623U_HIST3" (p_forall_limit in integer,p_success out boolean)
as
--**************************************************************************************************
--  Date:        may 2015
--  Author:      W Lyttle
--  Purpose:     Update fnd_rtl_allocations with derived first_dc_no for supply_chain_code = 'WH'
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_623U_HIST3';
...
--
      ...
      for v_cur in (
                WITH selall AS
                            ( SELECT /*+ PARALLEL(FND,4) */ DISTINCT alloc_no, WH_NO
                            FROM fnd_rtl_allocation FND
                            WHERE (first_dc_no is null or po_no is not null)
                            AND release_date BETWEEN g_start_wk_date AND g_END_wk_date
                            ) ,
                    seltmp AS
                            (
                             select  /*+ full(dl) */  
                                             WH_PHYSICAL_WH_NO FIRST_DC_NO, LOCATION_NO from dwh_performance.dim_location dl
                                            where loc_type = 'W'
                            )
                   SELECT DISTINCT A.alloc_no,
                          ...
                          FROM selall a,
                            seltmp b
                          WHERE B.location_no = A.wh_no )
                          loop


-- run update  
     update dwh_foundation.fnd_rtl_allocation fnd
             set fnd.first_dc_no = v_cur.first_dc_no,
             last_updated_date = '27 may 2015'
             where fnd.alloc_no = v_cur.alloc_no
             and fnd.first_dc_no is null
    ;
  ...
  --    G_alloc_CALC := G_alloc_CALC + 1;
      
  --     if G_alloc_CALC >= 999 then
  --                        l_text := 'alloc cnt='||G_alloc_CALC;
  --                        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
  --        G_alloc_CALC := 0;
   --    end if;

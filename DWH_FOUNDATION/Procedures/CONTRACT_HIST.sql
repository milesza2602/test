--------------------------------------------------------
--  DDL for Procedure CONTRACT_HIST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."CONTRACT_HIST" (p_forall_limit in integer,p_success out boolean) as
...
g_rec_in             fnd_rtl_contract%rowtype;
g_found              boolean;
...
type stg_array is table of fnd_rtl_contract%rowtype;
...
cursor c_stg_rms_rtl_contract is
   ...
   where reg_rsp_excl_vat is null ;
...
begin
   ...
   g_start_date                              := g_rec_in.create_date ;
   g_end_date                                := g_rec_in.contract_end_date + 30;
   begin
         select (UNIT_RETAIL * 100 / 114) reg_rsp_excl_vat
         into   g_rec_out.reg_rsp_excl_vat
         FROM   PRICE_HIST  
         where  loc                 BETWEEN 100 AND 150 and
                item                = TO_CHAR(g_rec_in.item_no)   and
                action_date         <  g_end_date and
                tran_type           IN (0,4) and
                rownum               = 1
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    reg_rsp_excl_vat                = a_tbl_update(i).reg_rsp_excl_vat ,
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_CONTRACT PRICE HIST STARTED AT '||
...
--**************************************************************************************************
    ...
    loop
      ...
      loop
         ...
         if g_rec_out.reg_rsp_excl_vat <> 0 then
            local_write_output;
         end if;
      end loop;
    fetch c_stg_rms_rtl_contract bulk collect into a_stg_input limit g_forall_limit;

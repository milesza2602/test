--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_050U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_050U" (
...
--**************************************************************************************************
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   If g_data_takeon = 'Y' 
       THEN 
        L_Text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_SCH_LOC_EMP_JB_DY';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Execute Immediate ('TRUNCATE TABLE dwh_foundation.FND_S4S_SCH_LOC_EMP_JB_DY');
        L_Text := 'Running GATHER_TABLE_STATS ON FND_S4S_SCH_LOC_EMP_JB_DY';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
         Dbms_Stats.Gather_Table_Stats ('DWH_FOUNDATION',
                                       'FND_S4S_SCH_LOC_EMP_JB_DY', Degree => 8);
        L_Text := 'TRUNCATE TABLE dwh_foundation.STG_S4S_EMP_LOC_JOB_SHED_HSP';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Execute Immediate ('TRUNCATE TABLE dwh_foundation.STG_S4S_EMP_LOC_JOB_SHED_HSP');
   END IF;
...
--                          and stg.employee_id not in (7011529,7009875) -- duplication failure 15/01/16
                          ;

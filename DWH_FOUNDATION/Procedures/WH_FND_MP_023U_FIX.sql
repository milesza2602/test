--------------------------------------------------------
--  DDL for Procedure WH_FND_MP_023U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MP_023U_FIX" (p_forall_limit in integer , p_success out boolean) as   --, p_sys_load_date in date) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MP_023U_FIX';
...
begin
      ...
      -- first delete these entries from the fnd table then reload all from staging
      delete /*+ parallel(stg,4) */
        from fnd_rtl_loc_dept_mth_plan_mp stg
       where fin_year_no = 2017
         and fin_month_no = 3;
         --and department_no = 105
         --and plan_type_no = 56;
       ...
       WITH STG_LOAD AS
          ...
          SELECT   /*+ full (stg) parallel (stg, 4) */
            ...
            where fin_year_no = 2017
              and fin_month_no = 3
              --and department_no = 105
              --and sys_load_date = p_sys_load_date
              --and plan_type_no = 56
          )

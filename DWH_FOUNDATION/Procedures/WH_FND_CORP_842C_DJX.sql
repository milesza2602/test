--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_842C_DJX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_842C_DJX" (
...
--                 For CHBD only.
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_842C_DJX';
    ...
    INTO dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_DJ3
        with  selcal AS (   
          ...
          selalloc as (
            ...
            ,
          SELEXT1A AS (
                       select /*+ MATERIALIZE  full(a) FULL(S) parallel(a,2)  parallel(s,4)   */
                                a.*,
                                s.du_id SHIP_DU_ID1 , 

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_720U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_720U_FIX" 
...
--  Purpose:     Create stockown dcr fact table in the foundation layer
--               with input ex staging table from Allpoints.
--  Tables:      Input  - stg_allpoints_stockown
--               Output - fnd_rtl_stockown_dcr
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_allpoints_stockown_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_stockown_dcr%rowtype;
g_rec_in             stg_allpoints_stockown_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_720U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STOCKOWN DCR FACTS EX ALLPOINTS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_allpoints_stockown_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_stockown_dcr%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_stockown_dcr%rowtype index by binary_integer;
...
type staging_array1 is table of stg_allpoints_stockown_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_allpoints_stockown_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_allpoints_stockown is
--   select *
--   from stg_allpoints_stockown_cpy
--   WHERE SYS_PROCESS_CODE = 'N'
--   order by sys_source_batch_id,sys_source_sequence_no;
     select * from  stg_allpoints_stockown_arc
     where  sys_load_date = '12/JUN/15'
       and  create_date between '20 apr 2015' and '31 may 2015'
       and  detail_check_ind = '1'
...
begin
   ...
   g_rec_out.to_location_no                  := g_rec_in.to_location_no;
...
--   end if;
   ...
   if not dwh_valid.indicator_field(g_rec_out.detail_check_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
   end if;


   if not dwh_valid.indicator_field(g_rec_out.damage_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
   end if;

   if not dwh_valid.fnd_location(g_rec_out.from_location_no) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_location_not_found;
     l_text          := dwh_constants.vc_location_not_found||g_rec_out.from_location_no ;
   ...
   if not dwh_valid.fnd_supplier(g_rec_out.supplier_no) then
    ...
    if not dwh_valid.fnd_location(g_rec_out.to_location_no) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_location_not_found;
     l_text          := dwh_constants.vc_location_not_found||g_rec_out.to_location_no ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     return;
   end if;
...
begin
   ...
   insert into stg_allpoints_stockown_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_stockown_dcr values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    order_no                        = a_tbl_update(i).order_no,
              ...
              to_location_no                  = a_tbl_update(i).to_location_no,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_allpoints_stockown_cpy
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).asn_no     = g_rec_out.asn_no   and
            a_tbl_insert(i).du_id      = g_rec_out.du_id   and
            a_tbl_insert(i).di_no      = g_rec_out.di_no   and
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_STOCKOWN_DCR EX ALLPOINTS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_allpoints_stockown;
    fetch c_stg_allpoints_stockown bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_allpoints_stockown;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_106U_OLD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_106U_OLD" 
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_zone_item_hsp.sys_process_msg%type;
g_rec_out            fnd_zone_item%rowtype;
g_rec_in             stg_rms_zone_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_106U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ZONE_ITEM MASTERDATA EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rms_zone_item_cpy%rowtype;
...
type staging_array1 is table of stg_rms_zone_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_zone_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   g_rec_out.market_basket_code              := g_rec_in.market_basket_code;
   g_rec_out.item_zone_link_code             := g_rec_in.item_zone_link_code;
   g_rec_out.multi_selling_uom_code          := g_rec_in.multi_selling_uom_code;
   g_rec_out.selling_unit_rsp                := g_rec_in.selling_unit_rsp;
   g_rec_out.multi_unit_rsp                  := g_rec_in.multi_unit_rsp;
   g_rec_out.multi_qty                       := g_rec_in.multi_qty;
   g_rec_out.source_data_status_code         := g_rec_in.source_data_status_code;
   g_rec_out.currency_code                   := g_rec_in.currency_code;
   g_rec_out.currency_reg_rsp                := g_rec_in.currency_reg_rsp;
   ...
   if not dwh_valid.indicator_field(g_rec_out.base_retail_ind) then
     g_hospital      := 'Y';
...
begin
   ...
   insert into stg_rms_zone_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    base_retail_ind           = a_tbl_update(i).base_retail_ind,
              ...
              currency_reg_rsp          = a_tbl_update(i).currency_reg_rsp,
...
begin
    ...
    l_text := 'LOAD OF FND_ZONE_ITEM EX RMS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_zone_item;
    fetch c_stg_rms_zone_item bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_613U_WLTEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_613U_WLTEST" (
...
AS
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_613U_WLTEST';
    ...
    INSERT INTO DWH_FOUNDATION.TEMP_ALLOC_ALLOC_SHIP_ship
          WITH selalloc AS
                    (SELECT
                      /*+ materialize */
                      DISTINCT alloc_no
                    FROM dwh_foundation.fnd_rtl_allocation_wl
                    where po_no is null
                    ORDER BY alloc_no
                    )
                  --,

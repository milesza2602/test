--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_250U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_250U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Sean Le Roux
--  Purpose:     Load merchandinse and trading square meters in the foundation layer
--               with input ex staging table ex excel spread sheet.
--  Tables:      Input  - stg_excel_loc_dept_wk_cpy
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_loc_dept_wk_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_dept_wk%rowtype;
g_rec_in             stg_excel_loc_dept_wk_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_250U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD SQUARE METERS EX FINANCE SPREADSHEET';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_loc_dept_wk_cpy%rowtype;
...
type staging_array1 is table of stg_excel_loc_dept_wk_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_loc_dept_wk_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_loc_dept_wk is
   select *
   from stg_excel_loc_dept_wk_cpy
...
begin
   ...
   g_rec_out.merch_square_meters_value       := g_rec_in.merch_square_meters_value;
   g_rec_out.trading_square_meters_value     := g_rec_in.trading_square_meters_value;
...
begin
   ...
   insert into stg_excel_loc_dept_wk_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    merch_square_meters_value   = a_tbl_update(i).merch_square_meters_value,
              trading_square_meters_value = a_tbl_update(i).trading_square_meters_value ,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_excel_loc_dept_wk_cpy
...
begin
    ...
    l_text := 'LOAD OF SQUARE METERS fnd_rtl_loc_dept_wk EX FINANCE SPREADSHEET STARTED AT '||
...
--**************************************************************************************************
    ...
    fetch c_stg_excel_loc_dept_wk bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_744U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_744U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Alastair de Wet
--  Purpose:     Create cam loc_item_wk table in the foundation layer
--               with input ex staging table from CAM lldf (Old whfood072u.4gl).
--  Tables:      Input  - stg_cam_loc_item_wk
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_cam_loc_item_wk_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_wk_fd%rowtype;
g_rec_in             stg_cam_loc_item_wk_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_744U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CAM LLDF FOODS FACTS EX CAM PLANNING';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_cam_loc_item_wk_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_wk_fd%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_wk_fd%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cam_loc_item_wk_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cam_loc_item_wk_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cam_loc_item_wk is
  select a.*
   from stg_cam_loc_item_wk_cpy a, fnd_jdaff_dept_rollout b, dim_item c
    ...
    if not  dwh_valid.fnd_supplier(g_rec_out.supplier_no) then
     ...
     return;
        ...
        when no_data_found then
            ...
            l_text          := dwh_constants.vc_date_not_found||g_rec_out.fin_year_no||g_rec_out.fin_week_no;
            dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            return;
            END;
...
begin
   ...
   insert into stg_cam_loc_item_wk_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_wk_fd values a_tbl_insert(i);
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_cam_loc_item_wk_cpy
...
begin
    ...
    l_text := 'LOAD OF fnd_rtl_loc_item_wk_fd EX CAM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_cam_loc_item_wk;
    fetch c_stg_cam_loc_item_wk bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_211U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_211U" (p_forall_limit in integer,p_success out boolean) as
...
MERGE /*+ full(fnd_lidfford) PARALLEL(fnd_lidfford,10) */ INTO dwh_foundation.fnd_rtl_loc_item_dy_ff_ord fnd_lidfford USING
     (
      select /*+ PARALLEL(a,10) full(a) full(b) full(c)*/  b.location_no, c.item_no, a.post_date, sum(a.store_order1) roq_qty, 
               0 cust_order_cases,    --sum(e.amended_po_cases) cust_order_cases, 
               c.var_weight_ind, a.last_updated_date
        from rtl_loc_item_dy_st_dir_ord a, dim_location b, dim_item c
        --, fnd_jdaff_dept_rollout d
       where a.sk1_location_no = b.sk1_location_no
         and a.sk1_item_no     = c.sk1_item_no
         --and a.last_updated_date = g_date -- commented out prior to rollout check

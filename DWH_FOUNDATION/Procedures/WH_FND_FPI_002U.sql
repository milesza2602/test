--------------------------------------------------------
--  DDL for Procedure WH_FND_FPI_002U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_FPI_002U" -- STORE PROC CHANGE
       (p_forall_limit in integer,p_success out boolean) as
                                                                
--**************************************************************************************************
--  Date:        February 2018
--  Author:      Francisca De Vaal
--  Tables:      Input  - STG_FPI_INGR_SUPPLIER_CPY                                                             
--               Output - FND_ITEM_SUP_INGREDIENT                                                               
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_FPI_002U';                              
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STG_FPI_INGR_SUPPLIER_CPY EX FPI';    
...
begin
    ...
    merge  into fnd_item_sup_ingredient fnd     		                                                         
    using (
   
    select /*+ PARALLEL(CPY1,8) */ cpy1.*
    from stg_fpi_ingr_supplier_cpy cpy1,
         fnd_supplier sup,
         fnd_item itm,
    ...
    where  cpy1.supplier_no   = sup.supplier_no and
          ...
          (sys_source_batch_id, sys_source_sequence_no,
           ...
           ingredient_id,orbc_record_id,ingredient_supplier)
       in
         (select /*+ PARALLEL(tmp,8) */ sys_source_batch_id, sys_source_sequence_no,
          ...
          from (select /*+ PARALLEL(tmp,8) */ tmp.*,
			    ...
			    order by sys_source_batch_id desc, sys_source_sequence_no desc)
                    as rank
                from stg_fpi_ingr_supplier_cpy tmp                                                               
           )
          where rank = 1
          )
   ...
   on    (
            ...
            fnd.ingredient_id	      =	mer_rec.ingredient_id 		and
            fnd.orbc_record_id	      =	mer_rec.orbc_record_id      and
            fnd.ingredient_supplier	  =	mer_rec.ingredient_supplier
	       )
   when matched then 
   update set                                                                                                      -- COLUNM NAME CHANGE 
            fnd.supplier_location	 =	mer_rec.supplier_location,
            fnd.country_of_origin	 =	mer_rec.country_of_origin,
            fnd.finish_colour		 =	mer_rec.finish_colour,
            fnd.specification		 =	mer_rec.specification,
  ...
  values                                                                                                           -- COLUNM NAME CHANGE 
          (         
            ...
            mer_rec.orbc_record_id,
            mer_rec.ingredient_supplier,
            mer_rec.supplier_location,
            mer_rec.country_of_origin,
            mer_rec.finish_colour,
            mer_rec.specification,
            g_date
          )           
          ;  
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   STG_FPI_INGR_SUPPLIER_CPY                                                                           -- STG TABLE NAME CHANGE 
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       RAISE;
end wh_fnd_fpi_002u                                                                                             -- STORE PROC CHANGE 
;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_248U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_248U" (
...
IS
  ...
  IS
  WITH seldup as(select fin_year_no, fin_week_no, fin_day_no, max(sys_source_batch_id) sys_source_batch_id
                      from STG_EXCEL_LIKE_4_LIKE_CPY
                       group by fin_year_no, fin_week_no, fin_day_no  ),
  selext AS
...
-- from   DWH_DATAFIX.stg_excel_like_4_like_WL210713 a,
      dim_calendar c, seldup e
    WHERE c.fin_year_no = a.fin_year_no
    and c.fin_week_no   = a.fin_week_no
    and a.sys_source_batch_id = e.sys_source_batch_id
    and e.fin_year_no = a.fin_year_no
    and e.fin_week_no   = a.fin_week_no
    and e.fin_day_no   = a.fin_day_no
    )
...
and SYS_SOURCE_BATCH_ID NOT IN (20300,20301, 20303,20305,20307)

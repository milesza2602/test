--------------------------------------------------------
--  DDL for Procedure WH_FND11_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND11_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND11_PO_DELETE_3JUN2015';
...
/*       l_text := 'Running GATHER_TABLE_STATS ON FND_RTL_INVOICE_MATCHING';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_RTL_INVOICE_MATCHING', DEGREE => 8);


       l_text := 'Running GATHER_TABLE_STATS ON FND_RTL_SHIPMENT';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_RTL_SHIPMENT', DEGREE => 8);

       l_text := 'Running GATHER_TABLE_STATS ON FND_DEAL_ACTUAL_DETAIL';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_DEAL_ACTUAL_DETAIL', DEGREE => 8);
...
FOR V_CUR IN
            ...
            SELECT /*+ FULL(A)  */ DISTINCT b.PO_NO, b.sk1_PO_NO
            FROM dwh_foundation.FND_RTL_INVOICE_MATCHING A,
                  selext B
            WHERE a.ORDER_NO = b.PO_NO
            AND  post_date <= '27/SEP/07'
)
LOOP

      --------------------------------------
      --FND_RTL_INVOICE_MATCHING
     --             l_text := 'FND_RTL_INVOICE_MATCHING';
     --             dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
      begin
            DELETE /*+ FULL(A)  */ FROM dwh_foundation.FND_RTL_INVOICE_MATCHING a
              where a.ORDER_NO = V_CUR.PO_NO
              
;
              
              g_recs_DELETED_FRIM := g_recs_DELETED_FRIM + SQL%ROWCOUNT;
              commit;
              
                   if g_recs_DELETED_FRIM >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_FRIM;
        ...
        when no_data_found then
          g_recs_DELETED_FRim := g_recs_DELETED_FRim ;
      end;      
...
l_text := 'FND_RTL_INVOICE_MATCHING recs deleted = '||g_recS_deleted_FRIM;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
END WH_FND11_PO_DELETE_3JUN2015;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_082U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_082U" (p_forall_limit in integer,p_success out boolean) as
...
--
--  25 April 2016 - Barry - replaced 'update .. where' clause in line 182, removed where conditions for update.
--  30 June 2016  - Quentin - changed cleardown process to run daily and not when recieved record count > 500000
...
when matched then
  update set
           fli.next_wk_deliv_pattern_code      = mer_mart.next_wk_deliv_pattern_code,
           fli.this_wk_deliv_pattern_code      = mer_mart.this_wk_deliv_pattern_code,
           fli.this_wk_catalog_ind             = mer_mart.this_wk_catalog_ind,
           fli.next_wk_catalog_ind             = mer_mart.next_wk_catalog_ind,
           fli.num_shelf_life_days             = mer_mart.num_shelf_life_days,
           fli.num_units_per_tray              = mer_mart.num_units_per_tray,
           fli.direct_perc                     = mer_mart.direct_perc,
           fli.model_stock                     = mer_mart.model_stock,
           fli.this_wk_cross_dock_ind          = mer_mart.this_wk_cross_dock_ind,
           fli.next_wk_cross_dock_ind          = mer_mart.next_wk_cross_dock_ind,
           fli.this_wk_direct_supplier_no      = nvl(mer_mart.this_wk_direct_supplier_no,0),
           fli.next_wk_direct_supplier_no      = nvl(mer_mart.next_wk_direct_supplier_no,0),
           fli.unit_pick_ind                   = mer_mart.unit_pick_ind,
           fli.store_order_calc_code           = mer_mart.store_order_calc_code,
           fli.safety_stock_factor             = mer_mart.safety_stock_factor,
           fli.min_order_qty                   = mer_mart.min_order_qty,
           fli.weigh_ind                       = mer_mart.weigh_ind,
           fli.last_updated_date               = g_date,   --sysdate,
           fli.primary_supplier_no             = 0,
           fli.primary_country_code            = 'ZA'
...
-- changed this to do the cleardown regardless of number of records sent and to only update OM (non-JDA) items
    ...
    if g_stg_count > 0 then      --500000
      ...
      where  (this_wk_catalog_ind = 1  or next_wk_catalog_ind = 1) and
        ...
        and item_no in (
         SELECT c.ITEM_NO 
           FROM FND_JDAFF_DEPT_ROLLOUT b, dim_item c
          where b.department_no = c.department_no

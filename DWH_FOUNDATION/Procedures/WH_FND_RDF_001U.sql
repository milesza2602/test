--------------------------------------------------------
--  DDL for Procedure WH_FND_RDF_001U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RDF_001U" 
...
--  Tables:      Input  - stg_rdf_rtl_dyfcst_cpy
--               Output - fnd_rtl_loc_item_wk_rdf_dyfcst
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rdf_rtl_dyfcst_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_wk_rdf_dyfcst%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RDF_001U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE DAILY FOODS FORECAST EX RDF';
...
type tbl_array_i is table of fnd_rtl_loc_item_wk_rdf_dyfcst%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_wk_rdf_dyfcst%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rdf_rtl_dyfcst_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rdf_rtl_dyfcst_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rdf_rtl_dyfcst is
   ...
   from stg_rdf_rtl_dyfcst_cpy stg
   LEFT OUTER JOIN fnd_rtl_loc_item_wk_rdf_dyfcst FND
   ON   STG.location_no                    = FND.location_no
   AND STG.item_no                         = FND.item_no
   AND STG.post_date                       = FND.post_date
...
begin
   ...
   insert into stg_rdf_rtl_dyfcst_hsp values
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_wk_rdf_dyfcst values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_wk_rdf_dyfcst
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_WK_RDF_DYFCST EX RDF STARTED AT '||
...
/*    G_DATE := '16 SEPTEMBER 2013';
     ...
     UPDATE  dwh_foundation.stg_rdf_rtl_dyfcst_cpy
    SET SYS_PROCESS_CODE = 'Y';
       g_recs_UPDATED :=  sql%rowcount;
           l_text := 'recs updated to Y:- '||g_recs_UPDATED;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    COMMIT;

    UPDATE  dwh_foundation.stg_rdf_rtl_dyfcst_cpy
    SET SYS_PROCESS_CODE = 'N'
    WHERE SYS_LOAD_DATE = G_DATE +1;
      g_recs_UPDATED :=  sql%rowcount;
                 l_text := 'recs updated to N:- '||g_recs_UPDATED;
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    COMMIT;
------ FOR TESTING PURPOSES -----
      g_recs_UPDATED := 0;

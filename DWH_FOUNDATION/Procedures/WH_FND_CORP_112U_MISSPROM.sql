--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_112U_MISSPROM
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_112U_MISSPROM" (p_forall_limit in integer,p_success out boolean) as
...
g_rec_in             stg_rms_prom_location_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_112U_MISSPROM';
...
cursor c_stg_rms_prom_location is
   select ts.*
   from dwh_foundation.TEMP_STG_RMS_PROM_LOCATIOn ts,
   dim_location dl
   where ts.location_no = dl.location_no
...
begin
 --  g_hospital                            := 'N';
   g_rec_out.last_updated_date           := g_rec_in.sys_load_date;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_prom_location_cpy

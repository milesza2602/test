--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_145U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_145U" (p_forall_limit in integer,
...
g_post_date           date;
   ...
   cursor stg_dup is
      select * from dwh_foundation.stg_cam_location_item_cpy
      where (location_no, item_no, post_date)
         in
      (select location_no,item_no, POST_DATE
         from dwh_foundation.stg_cam_location_item_cpy
...
--**************************************************************************************************
   ...
   select /*+ FULL(TMP) */  TMP.sys_source_batch_id,
                            ...
                            TMP.NUM_UNITS_PER_TRAY2, -- old code
                            tmp.CASE_MASS,
                            tmp.CASE_SELLING_INCL_VAT,
                            tmp.REG_RSP_INCL_VAT,
                            tmp.CASE_COST_PRICE,
                            tmp.REG_RSP_EXCL_VAT,
                            tmp.CASE_SELLING_EXCL_VAT,
                            tmp.COST_PRICE,
                            tmp.CASE_COST_PRICE_LOCAL,
                            tmp.CASE_COST_PRICE_OPR,
                            tmp.CASE_SELLING_EXCL_VAT_LOCAL,
                            tmp.CASE_SELLING_EXCL_VAT_OPR,
                            tmp.CASE_SELLING_INCL_VAT_LOCAL,
                            tmp.CASE_SELLING_INCL_VAT_OPR,
                            tmp.COST_PRICE_LOCAL,
                            tmp.COST_PRICE_OPR,
                            tmp.REG_RSP_EXCL_VAT_LOCAL,
                            tmp.REG_RSP_EXCL_VAT_OPR,
                            tmp.REG_RSP_INCL_VAT_LOCAL,
                            tmp.REG_RSP_INCL_VAT_OPR
    from  stg_cam_location_item_cpy  TMP 
    where not exists
          (select *
           from   fnd_item di

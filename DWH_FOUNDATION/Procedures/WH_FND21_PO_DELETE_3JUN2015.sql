--------------------------------------------------------
--  DDL for Procedure WH_FND21_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND21_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND21_PO_DELETE_3JUN2015';
...
begin
 ...
 /*      l_text := 'Running GATHER_TABLE_STATS ON FND_RTL_STOCKOWN_DCR';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_RTL_STOCKOWN_DCR', DEGREE => 8);
  

       l_text := 'Running GATHER_TABLE_STATS ON FND_RTL_PURCHASE_ORDER';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_RTL_PURCHASE_ORDER', DEGREE => 8);

       l_text := 'Running GATHER_TABLE_STATS ON FND_RTL_ALLOCATION';
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                       'FND_RTL_ALLOCATION', DEGREE => 8);
...
FOR V_CUR IN
            ...
            FROM dwh_foundation.FND_RTL_STOCKOWN_DCR A,
                  selext B
            WHERE a.ORDER_NO = b.PO_NO
                and create_date <= '05/MAY/08'
)
LOOP

      --------------------------------------
      --FND_RTL_STOCKOWN_DCR
   --               l_text := 'FND_RTL_STOCKOWN_DCR';
   --               dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);  
      begin
            DELETE /*+ FULL(A)  */ FROM dwh_foundation.FND_RTL_STOCKOWN_DCR A
                where a.order_NO = V_CUR.PO_NO

                ;
                
                g_recs_DELETED_FRSD := g_recs_DELETED_FRSD + SQL%ROWCOUNT;
                commit;
                
                                              if g_recs_DELETED_FRSD >= 100000
        THEN 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_FRSD;
        ...
        when no_data_found then
          g_recs_DELETED_FRSD := g_recs_DELETED_FRSD ;
      end;
...
l_text := 'FND_RTL_STOCKOWN_DCR recs deleted = '||g_recS_deleted_FRSD;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
END WH_FND21_PO_DELETE_3JUN2015;

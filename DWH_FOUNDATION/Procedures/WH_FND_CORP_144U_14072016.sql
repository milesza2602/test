--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_144U_14072016
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_144U_14072016" (p_forall_limit in integer,p_success out boolean) as
...
g_rec_out            fnd_loc_item_jdaff_wh_plan%rowtype;
g_rec_in             stg_jdaff_wh_plan%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
type stg_array is table of stg_jdaff_wh_plan%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_loc_item_jdaff_wh_plan%rowtype index by binary_integer;
type tbl_array_u is table of fnd_loc_item_jdaff_wh_plan%rowtype index by binary_integer;
...
type staging_array1 is table of stg_jdaff_wh_plan.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_jdaff_wh_plan.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_jdaff_wh_plan is
   select a.*
     from stg_jdaff_wh_plan_CPY a, fnd_jdaff_dept_rollout b, dim_item c
...
begin
   ...
   g_rec_out.dc_loc_no                        := g_rec_in.dc_loc_no;
   --g_rec_out.dc_plan_store_cases              := g_rec_in.dc_plan_store_cases;
   ...
   case l_day_no
     when 1 then
        -------------------------------------------------------------------
        -- do the zero checks for the first 3 day cases values sent by JDA
        -------------------------------------------------------------------
         ...
         if l_cnt > 0 then
            select week_1_day_1_cases, week_1_day_2_cases, week_1_day_3_cases
         ...
         -- check if passed day 1 value is zero and if so, get the value from the previous day
         ...
         -- check if passed day 2 value is zero and if so, get the value from the previous day
         ...
         -- check if passed day 3 value is zero and if so, get the value from the previous day
     ...
     when 2 then       
        -----------------------------------------------
        --  do the zero checks for the first 3 days
        -----------------------------------------------
        if g_rec_in.week_1_day_1_cases = 0
        or g_rec_in.week_1_day_2_cases = 0
        or g_rec_in.week_1_day_3_cases = 0 then
            ...
            --l_text := 'g_rec_in.week_1_day_1_cases = ' || g_rec_in.week_1_day_1_cases;
            --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            --l_text := 'g_rec_in.week_1_day_2_cases = ' || g_rec_in.week_1_day_2_cases;
            --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            --l_text := 'g_rec_in.week_1_day_3_cases = ' || g_rec_in.week_1_day_3_cases;
            --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         ...
         if l_cnt > 0 then
            --l_text := 'ONE OR MORE OF FIRST 3 VALUES ARE 0, GET PREVIOUS DAYS VALUES';
            --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            
            select week_1_day_2_cases, week_1_day_3_cases, week_1_day_4_cases
              into l_week_1_day_1_cases, l_week_1_day_2_cases, l_week_1_day_3_cases
              from fnd_loc_item_jdaff_wh_plan
             where item_no        = g_rec_out.item_no 
               and location_no    = g_rec_out.location_no
         ...
         --l_text := 'l_week_1_day_1_cases = ' || l_week_1_day_1_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         --l_text := 'l_week_1_day_2_cases = ' || l_week_1_day_2_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         --l_text := 'l_week_1_day_3_cases = ' || l_week_1_day_3_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         
         -- check if passed day 1 value is zero and if so, get the value from the previous day
         IF g_rec_in.week_1_day_1_cases = 0 then
            if l_cnt > 0 then
               g_rec_in.week_1_day_1_cases    := l_week_1_day_1_cases;
               --l_text := 'replaced w1d1 cases';
               --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
            end if;
         end if;
         ...
         IF g_rec_in.week_1_day_2_cases = 0 then
            if l_cnt > 0 then
              g_rec_in.week_1_day_2_cases     := l_week_1_day_2_cases;
              --l_text := 'replaced w1d2 cases';
              --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
           end if;
         end if;
         ...
         IF g_rec_in.week_1_day_3_cases = 0 then
            if l_cnt > 0 then
              g_rec_in.week_1_day_3_cases     := l_week_1_day_3_cases;
              --l_text := 'replaced w1d3 cases';
              --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
           end if;
         end if;
         
         --l_text := 'g_rec_in.week_1_day_1_cases NOW = ' || g_rec_in.week_1_day_1_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         --l_text := 'g_rec_in.week_1_day_2_cases NOW = ' || g_rec_in.week_1_day_2_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         --l_text := 'g_rec_in.week_1_day_3_cases NOW = ' || g_rec_in.week_1_day_3_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     when 3 then
        ...
        -----------------------------------------------
        -- now do the zero checks for the first 3 days
        -----------------------------------------------
        if g_rec_in.week_1_day_1_cases = 0
        or g_rec_in.week_1_day_2_cases = 0
        or g_rec_in.week_1_day_3_cases = 0 then
         ...
         --l_text := 'l_week_1_day_1_cases = (week_1_day_3_cases from '|| l_prev_date || ' )' || l_week_1_day_1_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         --l_text := 'l_week_1_day_2_cases = (week_1_day_4_cases from '|| l_prev_date || ' )'  || l_week_1_day_2_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         --l_text := 'l_week_1_day_3_cases = (week_1_day_5_cases from '|| l_prev_date || ' )'   || l_week_1_day_3_cases;
         --dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);   
...
begin
   ...
   insert into stg_jdaff_wh_plan_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_loc_item_jdaff_wh_plan values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_loc_item_jdaff_wh_plan
       set    dc_loc_no                   = a_tbl_update(i).dc_loc_no ,
              week_1_day_1_cases          = a_tbl_update(i).week_1_day_1_cases,
              week_1_day_2_cases          = a_tbl_update(i).week_1_day_2_cases,
              week_1_day_3_cases          = a_tbl_update(i).week_1_day_3_cases,
              week_1_day_4_cases          = a_tbl_update(i).week_1_day_4_cases,
              week_1_day_5_cases          = a_tbl_update(i).week_1_day_5_cases,
              week_1_day_6_cases          = a_tbl_update(i).week_1_day_6_cases,
              week_1_day_7_cases          = a_tbl_update(i).week_1_day_7_cases,
              week_2_day_1_cases          = a_tbl_update(i).week_2_day_1_cases,
              week_2_day_2_cases          = a_tbl_update(i).week_2_day_2_cases,
              week_2_day_3_cases          = a_tbl_update(i).week_2_day_3_cases,
              week_2_day_4_cases          = a_tbl_update(i).week_2_day_4_cases,
              week_2_day_5_cases          = a_tbl_update(i).week_2_day_5_cases,
              week_2_day_6_cases          = a_tbl_update(i).week_2_day_6_cases,
              week_2_day_7_cases          = a_tbl_update(i).week_2_day_7_cases,
              week_3_day_1_cases          = a_tbl_update(i).week_3_day_1_cases,
              week_3_day_2_cases          = a_tbl_update(i).week_3_day_2_cases,
              week_3_day_3_cases          = a_tbl_update(i).week_3_day_3_cases,
              week_3_day_4_cases          = a_tbl_update(i).week_3_day_4_cases,
              week_3_day_5_cases          = a_tbl_update(i).week_3_day_5_cases,
              week_3_day_6_cases          = a_tbl_update(i).week_3_day_6_cases,
              week_3_day_7_cases          = a_tbl_update(i).week_3_day_7_cases,
...
begin
    ...
    l_text := 'LOAD OF fnd_loc_item_jdaff_wh_plan EX JDA STARTED AT '||
...
--**************************************************************************************************
    open c_stg_jdaff_wh_plan;
    fetch c_stg_jdaff_wh_plan bulk collect into a_stg_input limit g_forall_limit;

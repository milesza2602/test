--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_232U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_232U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        August 2008
--  Author:      Christie Koorts
--  Purpose:     Create the location week fact table in the foundation layer
--               with input ex staging table from STAFFPLANNER.
--  Tables:      Input  - stg_stpl_fcst_cpy
--               Output - fnd_rtl_loc_wk_staffplan_fcst
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_stpl_fcst_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_wk_staffplan_fcst%rowtype;

g_rec_in             stg_stpl_fcst_cpy%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_232U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE LOC WEEK FACTS EX STAFFPLANNER';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For input bulk collect --
type stg_array is table of stg_stpl_fcst_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_wk_staffplan_fcst%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_wk_staffplan_fcst%rowtype index by binary_integer;
...
type staging_array1 is table of stg_stpl_fcst_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_stpl_fcst_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor stg_stpl_fcst is
   select stg.*
   from   stg_stpl_fcst_cpy stg
   where  stg.sys_process_code     = 'N'
   order by stg.sys_source_sequence_no;
-- order by only where sequencing is essential to the correct loading of data

--g_rec_in1             stg_stpl_fcst%rowtype;
...
begin
   ...
   insert into stg_stpl_fcst_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_wk_staffplan_fcst values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_wk_staffplan_fcst
       set    fte_budget                     = a_tbl_update(i).fte_budget,
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into g_count
   from fnd_rtl_loc_wk_staffplan_fcst
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_WK_STAFFPLAN_FCST EX POS STARTED AT '||
...
--**************************************************************************************************
    open stg_stpl_fcst;
    fetch stg_stpl_fcst bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_736U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_736U" (p_forall_limit in integer,p_success out boolean) as
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_om_pick_acc2_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_trcps_pick%rowtype;
g_rec_in             stg_om_pick_acc2_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_736U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE PICK ACCURACY FOODS FACTS EX OM';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_om_pick_acc2_cpy%rowtype;
...
type staging_array1 is table of stg_om_pick_acc2_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_om_pick_acc2_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_om_pick_acc2 is
   select *
   from stg_om_pick_acc2_cpy
...
begin
   ...
   insert into stg_om_pick_acc2_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    special_cases                   = a_tbl_update(i).special_cases,
              forecast_cases                  = a_tbl_update(i).forecast_cases,
              safety_cases                    = a_tbl_update(i).safety_cases,
              over_cases                      = a_tbl_update(i).over_cases,
...
begin
    ...
    l_text := 'LOAD OF fnd_rtl_loc_item_dy_trcps_pick EX ALLPOINTS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_om_pick_acc2;
    fetch c_stg_om_pick_acc2 bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_086U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_086U" 
...
--  Tables:      Input  - stg_catman_loc_item_cpy
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_catman_loc_item_hsp.sys_process_msg%type;
g_rec_out            fnd_location_item%rowtype;
g_rec_in             stg_catman_loc_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_086U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE LOCATION_ITEM MASTERDATA EX CATMAN';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_catman_loc_item_cpy%rowtype;
...
type staging_array1 is table of stg_catman_loc_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_catman_loc_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_catman_loc_item is
   select *
   from stg_catman_loc_item_cpy
...
-- CATMAN only updates so the location_item must be there in order for it to be a valid CATMAN transaction
...
begin
   ...
   insert into stg_catman_loc_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    profile_id                      = a_tbl_update(i).profile_id,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_catman_loc_item_cpy
...
--**************************************************************************************************
    open c_stg_catman_loc_item;
    fetch c_stg_catman_loc_item bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND13_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND13_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND13_PO_DELETE_3JUN2015';
...
FOR V_CUR IN
            ...
            FROM  dwh_foundation.FND_DEAL_ACTUAL_DETAIL A,
                  selext B
            WHERE a.PO_NO = b.PO_NO
          AND TRAN_DATE <= '27/SEP/07'
)
LOOP

      begin
            DELETE /*+ FULL(A)  */ FROM dwh_foundation.FND_DEAL_ACTUAL_DETAIL a
              where a.PO_NO = V_CUR.PO_NO
    
              ;
              
              g_recs_DELETED_FDAD := g_recs_DELETED_FDAD + SQL%ROWCOUNT;
              commit;
              
                                            if g_recs_DELETED_FDAD >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_FDAD;
        ...
        when no_data_found then
          g_recs_DELETED_FDAD := g_recs_DELETED_FDAD ;
      end;      
...
l_text := 'FND_DEAL_ACTUAL_DETAIL recs deleted = '||g_recS_deleted_FDAD;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
END WH_FND13_PO_DELETE_3JUN2015;

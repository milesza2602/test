--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_041U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_041U" 
...
--               Output - stg_ast_loc_item_dy_catlg_hsp
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_041U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_roll;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_roll;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD ASSORT DAILY CATALOG HOSPITAL DATA';
...
begin
   ...
   l_text := 'LOAD OF stg_ast_loc_item_dy_catlg_HSP STARTED AT '||
...
--**************************************************************************************************
   ...
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      ...
      insert /*+ APPEND parallel (ast,2) */ into stg_ast_loc_item_dy_catlg_HSP ast
      select /*+ full (AST) parallel (AST,2) */
             ...
             ,'INVALID PRIMARY MEASURE - ITEM/LOCATION/DATE'
      ...
      where  ast.item_no = fi.item_no (+)
       ...
       or  (case when ast.location_no is not null then 1 else 0 end) = 0
       or  (case when ast.post_date is not null then 1 else 0 end) = 0)
       ;

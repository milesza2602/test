--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014D_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014D_SS" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        10 July 2014
--  Author:      Lwazi Ntloko
--  Purpose:     Create S4S jobid hierachy Data on the foundation layer
--               with input from the Labour hierachy staging tables.
--
--  DIM Tables:  Input  - fnd_s4s_LABOUR_HIERARCHY_SS
--               Output - FND_S4S_JOB_SS
...
g_rec_out            FND_S4S_job_ss%rowtype;
g_found              boolean;
g_date               date          := trunc(sysdate);
g_yesterday          date          := trunc(sysdate) - 1;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014D_SS';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_prf;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_prf_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD S4S jobid';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of FND_S4S_job_ss%rowtype index by binary_integer;
type tbl_array_u is table of FND_S4S_job_ss%rowtype index by binary_integer;
...
Newtable as (    
select jl.*, 
      LAG(effective_from_date-1, 1, date_candidate) OVER (PARTITION BY job_id,effective_from_date ORDER BY job_id, effective_from_date desc) effective_to_date
from jb_list jl
...
select 
    ...
    effective_from_date,
    effective_to_date,
    case when effective_to_date > g_date then 'Y' else 'N' end active_ind,
    g_date
from Newtable;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into FND_S4S_JOB_SS values a_tbl_insert(i);
...
dbms_output.put_line('Message XY1 '||l_message);
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
        update FND_S4S_JOB_SS
           set job_name                 = a_tbl_update(i).job_name,
...
-- check if insert of workroup Id is lready in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if  a_tbl_insert(i).job_id= g_rec_out.job_id
    then
            g_found := true;
         end if;
      end loop;
...
begin
  if p_forall_limit is not null and p_forall_limit > 1000 then
    ...
    l_text := 'loading S4S jobid data started at '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

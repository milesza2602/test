--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_185U_WL2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_185U_WL2" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_185U_WL2';
...
cursor c_STG_apx_item_link is
   WITH SELBAT AS (SELECT  /*+ FULL(A) PARALLEL(A,6) */ CHAIN_CODE
                      , PROD_LINK_REF_NO
                      , PROD_LINK_IND
                      , CHAIN_NO
                      , ITEM_NO
...
,'12-Jul-17')
        ...
        SELSEQ AS (SELECT  /*+ FULL(C) PARALLEL(C,6) */ C.CHAIN_CODE
                      , C.PROD_LINK_REF_NO
                      , C.PROD_LINK_IND
                      , C.CHAIN_NO
                      , C.ITEM_NO
                  ...
                  FROM STG_apx_item_link_ARC C, SELBAT SB
...
,'12-Jul-17')
                  GROUP BY C.CHAIN_CODE
                      ...
                      , trunc(C.POST_DATE) 
                      , MAXBAT)
   select /*+ FULL(A) PARALLEL(A,6) */  A.SYS_SOURCE_BATCH_ID
   ...
   from STG_apx_item_link_ARC a , SELSEQ SS

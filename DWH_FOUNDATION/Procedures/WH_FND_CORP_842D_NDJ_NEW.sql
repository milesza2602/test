--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_842D_NDJ_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_842D_NDJ_NEW" (
...
--                 For CHBD only.
  ...
  --               Output - temp_alloc_tracker_frst_rcpt_XD_NDJ4
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_842D_NDJ_NEW';
...
--**************************************************************************************************
      ...
      l_text := 'truncate table dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ4' ;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'truncate table dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ4';
...
--**************************************************************************************************
 ...
 --     g_sql_trunc_partition := 'alter table dwh_foundation.fnd_alloc_tracker_alloc truncate SUBPARTITION '||g_partition_name;
         ...
         l_text := 'Running GATHER_TABLE_STATS ON TEMP_A_TRACK_FRST_RCPT_XD_NDJ4';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--     DBMS_STATS.GATHER_TABLE_STATS ('DWH_FOUNDATION',
--                                   'TEMP_A_TRACK_FRST_RCPT_XD_NDJ4', DEGREE => 8);     

-- below is the new code for stats collection in 12c - Ref: 23Jan2019
     DWH_FOUNDATION.GENERIC_GATHER_TABLE_STATS(l_procedure_name,'TEMP_A_TRACK_FRST_RCPT_XD_NDJ4');
--**************************************************************************************************
...
--**************************************************************************************************
    ...
    INSERT /*+ APPEND */
    INTO dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ4
    with  selcal AS (   
                  ...
                  UNION ALL
                              select /*+ full(a)  full(s2) parallel(a,2)  parallel(s2,4)  */
                                              ...
                                              SUM(nvl(s2.cancelled_qty,0)) cancelled_qty,
                                                A.SUPPLY_CHAIN_CODE
                           FROM SELEXT1A A
                                  JOIN DWH_FOUNDATION.TEMP_SHIPMENTS_MIN_NDJ S2

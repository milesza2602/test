--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_040U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_040U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_040U';
 ...
 --  g_date := g_date - 2;
   ...
   --    AND  SYS_SOURCE_BATCH_ID = 6223
    --   AND SYS_SOURCE_BATCH_ID = 1 AND SYS_SOURCE_SEQUENCE_NO = 106
       AND POST_DATE <> '19 NOV 2018'  -- FIX TO EXCLUDE DATA ALREADY ON OUTPUT FAILING WITH UNIQUE CONSTRAINT ADW
       ;

--------------------------------------------------------
--  DDL for Procedure WH_FND_RDF_020U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RDF_020U" 
...
--  Purpose:     Create item forecast indicator table in the foundation layer
--               with input ex staging table from RDF.
--  Tables:      Input  - stg_rdf_item_cpy
--               Output - fnd_item
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rdf_item_cpy.sys_process_msg%type;
g_rec_out            fnd_item %rowtype;
g_rec_in             stg_rdf_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RDF_020U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE DAILY ITEM FORECAST IND INDICATOR EX RDF';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rdf_item_cpy%rowtype;
...
type staging_array1 is table of stg_rdf_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rdf_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
--   g_rec_out.rdf_forecst_ind                 := g_rec_in.rdf_forecst_ind;
   g_rec_out.rdf_forecst_ind                 := nvl(g_rec_in.rdf_forecst_ind,0);
...
begin
   ...
   insert into stg_rdf_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_item
       set    rdf_forecst_ind                = a_tbl_update(i).rdf_forecst_ind,
...
begin
    ...
    l_text := 'LOAD OF FND_ITEM EX RDF STARTED AT '||

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_656U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_656U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_rms_deal_accrual_dy_cpy
--               Output - fnd_deal_loc_item_dy
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_deal_accrual_dy_hsp.sys_process_msg%type;
g_rec_out            fnd_deal_loc_item_dy%rowtype;
g_rec_in             stg_rms_deal_accrual_dy_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_656U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE FND_DEAL_LOC_ITEM_DY DATA EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rms_deal_accrual_dy_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_deal_loc_item_dy%rowtype index by binary_integer;
type tbl_array_u is table of fnd_deal_loc_item_dy%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_deal_accrual_dy_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_deal_accrual_dy_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_deal_accrual_dy is
   select *
   from stg_rms_deal_accrual_dy_cpy
...
begin
   ...
   g_rec_out.accrual_cost                    := g_rec_in.accrual_cost;
...
begin
   ...
   insert into stg_rms_deal_accrual_dy_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_deal_loc_item_dy values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_deal_loc_item_dy
       set    accrual_qty                     = a_tbl_update(i).accrual_qty,
              accrual_cost                    = a_tbl_update(i).accrual_cost,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_deal_accrual_dy_cpy
...
begin
    ...
    l_text := 'LOAD OF FND_DEAL_LOC_ITEM_DY EX RMS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_deal_accrual_dy;
    fetch c_stg_rms_deal_accrual_dy bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_rms_deal_accrual_dy;
--**************************************************************************************************

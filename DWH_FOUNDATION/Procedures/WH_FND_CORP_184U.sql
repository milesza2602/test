--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_184U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_184U" (p_forall_limit in integer,p_success out boolean) as
...
cursor stg_dup is
      select * from stg_intactix_pln_store_cpy
     ...
     (select location_no, planogram_id, planogram_status, live_date, floorplan_key, planogram_department, 
             fixture_seq_no, planogram_db_key
      from stg_intactix_pln_store_cpy a
...
begin
  ...
  merge  /*+ parallel (fli,4) */ 
into dwh_foundation.fnd_loc_planogram_wk fli using (
     select /*+ PARALLEL(a,4) FULL(a) */ 
            ...
            size_break
      from  stg_intactix_pln_store_cpy a, fnd_location b, dim_calendar c
...
--**************************************************************************************************
  ...
  --**************************************************************************************************
    ...
    for dupp_record in stg_dup
       loop
        ...
        if  dupp_record.location_no          = g_location_no and
            ...
            update stg_intactix_pln_store_cpy stg
   ...
   select /*+ FULL(TMP) */  tmp.sys_source_batch_id,
                            ...
                            tmp.floorplan_key,
                            tmp.floorplan_display_method,
                            tmp.spaceplan_display_method,
                            tmp.size_break

    from  stg_intactix_pln_store_cpy  tmp
    where
         not exists
           (select *
...
--          and tmp.location_no    = 480 
--          and tmp.planogram_id   = 320480
          ;

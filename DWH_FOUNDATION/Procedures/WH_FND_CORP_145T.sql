--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_145T
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_145T" (p_forall_limit in integer,
...
mer_mart             w6005682.STG_CAM_LOCATION_ITEM_QPY%rowtype;
g_found              boolean;
g_valid              boolean;

g_date               date          := trunc(sysdate);

p_from_loc_no         integer;
p_to_loc_no           integer;

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_145T';
...
g_location_no       W6005682.STG_CAM_LOCATION_ITEM_QPY.location_no%type; 
g_item_no           W6005682.STG_CAM_LOCATION_ITEM_QPY.item_no%TYPE; 
...
g_post_date           date;
   ...
   cursor stg_dup is
      select * from W6005682.STG_CAM_LOCATION_ITEM_QPY
      where (location_no, item_no, post_date)
         in
      (select location_no,item_no, POST_DATE
         from W6005682.STG_CAM_LOCATION_ITEM_QPY
...
--            
--            update W6005682.STG_CAM_LOCATION_ITEM_QPY stg
...
merge /*+ parallel(fnd_mart,6) */ 
    into W6005682.FND_LOC_ITEM_DY_FF_ORD_QS fnd_mart 
    using (
    select /*+ parallel(tmp,6) FULL(TMP) */ distinct tmp.item_no,
          tmp.location_no,
          tmp.post_date,
          tmp.source_data_status_code,
          tmp.num_units_per_tray,
          tmp.num_units_per_tray2
      from w6005682.STG_CAM_LOCATION_ITEM_QPY  tmp 

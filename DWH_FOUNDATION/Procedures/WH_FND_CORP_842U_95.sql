--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_842U_95
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_842U_95" 
...
INSERT /*+ APPEND */ INTO dwh_performance.fnd_alloc_tracker_frst_rcpt
      SELECT
      ...
      group by a.release_date, a.alloc_no, s2.to_loc_no, s2.actl_rcpt_date, s.du_id, a.first_dc_no, a.item_no
               , dwh_lookup.no_of_workdays(a.release_date,s2.actl_rcpt_date), 0, 
               g_date, s2.final_loc_no, a.first_whse_supplier_no, a.supply_chain_code
UNION ALL
      SELECT
      /*+ full(s) full(s2) parallel (s2,4) parallel (s,4)  */
              a.release_date,
      ...
      WHERE a.trunk_ind                 = 1
            ...
            AND s.received_qty                > 0
            AND a.release_date BETWEEN g_start_date AND g_date
      GROUP BY a.release_date,  a.alloc_no, s2.to_loc_no,  s2.actl_rcpt_date,
                s.du_id,  a.first_dc_no,  a.item_no,
                dwh_lookup.no_of_workdays(a.release_date, s2.actl_rcpt_date),
                s2.final_loc_no ,  a.first_whse_supplier_no, a.supply_chain_code;

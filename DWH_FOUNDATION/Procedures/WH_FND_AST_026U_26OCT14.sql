--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_026U_26OCT14
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_026U_26OCT14" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
-- used for data catch_up - batch-date = '26 oct 2014'
--**************************************************************************************************
--  Date:        May 2012
--  Author:      A Joshua
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_026U_26OCT14';
...
--**************************************************************************************************
  ...
  g_date := '26 oct 2014';
   ...
   where calendar_date = (select to_date(sysdate, 'dd mon yy') from dual);
      ...
      insert /*+ APPEND parallel (x,2) */ into fnd_ast_chain_sc_plan_wk x
      ...
      where  ast.style_colour_no                         = sc.style_colour_no (+)
       ...
       and   ast.fin_year_no = cal.fin_year_no

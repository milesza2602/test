--------------------------------------------------------
--  DDL for Procedure WH_FND23_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND23_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND23_PO_DELETE_3JUN2015';
...
FOR V_CUR IN
            ...
            WHERE a.po_NO = b.PO_NO
             and release_date <= '07/FEB/08'
)
LOOP

 
      begin
            DELETE /*+ FULL(A) parallel(a,4) */ FROM dwh_foundation.FND_RTL_ALLOCATION a
              where a.PO_NO = V_CUR.PO_NO
             
              ;
              
              g_recs_DELETED_FRA := g_recs_DELETED_FRA + SQL%ROWCOUNT;
              commit;
              
                                                              if g_recs_DELETED_FRA >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_FRA;
        ...
        when no_data_found then
          g_recs_DELETED_FRa := g_recs_DELETED_FRa ;
      end;      
...
l_text := 'FND_RTL_ALLOCATION recs deleted = '||g_recS_deleted_FRA;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text); 
...
END WH_FND23_PO_DELETE_3JUN2015;

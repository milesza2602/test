--------------------------------------------------------
--  DDL for Procedure WH_FND_RP_001U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RP_001U_FIX" 
...
--  Tables:      Input  - stg_rp_loc_item_dy_catlg_arc
--               Output - fnd_rtl_loc_item_dy_rp_catlg
--  Packages:    dwh_constants, dwh_log, dwh_valid
--
--  Maintenance:
--  02 Feb 2009 : TD-713 Source_data_status_code is not sent from source,
--                       therefore the error-check on the procedure should allow for nulls.
--
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rp_loc_item_dy_catlg_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_rp_catlg%rowtype;
g_rec_in             stg_rp_loc_item_dy_catlg_arc%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RP_001U_FIX';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE DAILY C&H CATALOG EX RP';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rp_loc_item_dy_catlg_arc%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_rp_catlg%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_rp_catlg%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rp_loc_item_dy_catlg_arc.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rp_loc_item_dy_catlg_arc.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rp_loc_item_dy_catlg is
   select *
   from stg_rp_loc_item_dy_catlg_arc
   where sys_load_date = '23 jan 13';
...
begin
   ...
   g_rec_out.criticality_code                := g_rec_in.criticality_code;
   g_rec_out.active_from_date                := g_rec_in.active_from_date;
   g_rec_out.active_to_date                  := g_rec_in.active_to_date;
...
begin
   ...
   insert into stg_rp_loc_item_dy_catlg_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_rp_catlg values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_rp_catlg
       set    criticality_code                = a_tbl_update(i).criticality_code,
              active_from_date                = a_tbl_update(i).active_from_date,
              active_to_date                  = a_tbl_update(i).active_to_date,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rp_loc_item_dy_catlg_arc
...
begin
   ...
   select count(1)
   into   g_count
   from   fnd_rtl_loc_item_dy_rp_catlg
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_ITEM_DY_RP_CATLG EX RP STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rp_loc_item_dy_catlg;
    fetch c_stg_rp_loc_item_dy_catlg bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_rp_loc_item_dy_catlg;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014D_SS1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014D_SS1" (p_forall_limit in integer,p_success out boolean) as
...
cursor c_fnd_s4s_labour_hierarchy is
    With FromDate as (
    select FND.job_id, max(FND.effective_from_date) effective_from_date
    ...
    SELECT  
      ...
      FROM fnd_s4s_labour_hierarchy FND,
          FromDate fd
     WHERE FND.job_id = fd.job_id
       AND FND.effective_from_date = fd.effective_from_date
   group by FND.job_id ,
             FND.jobgroup_id ,
             FND.job_name,
             FND.workgroup_id,
             ...
             FND.s4s_business_unit_name,
             FND.effective_from_date
    ORDER BY fnd.job_id,fnd.jobgroup_id,fnd.workgroup_id,fnd.s4s_business_unit_no;
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into FND_S4S_job values a_tbl_insert(i);

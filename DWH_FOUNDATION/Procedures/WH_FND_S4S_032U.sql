--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_032U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_032U" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_032U';
...
BEGIN
     ...
     select g_run_date, g_date, g_run_seq_no, f.*
        ...
        where f.employee_id = s.employee_id
         ...
         delete from DWH_FOUNDATION.FND_S4S_EMP_CONSTR_WK B
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   ...
   If g_data_takeon <> 'Y' OR g_data_takeon IS NULL
       THEN 
             delete_fnd;
   ELSE
        L_Text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_EMP_CONSTR_WK_DEL_LIST';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Execute Immediate ('TRUNCATE TABLE dwh_foundation.FND_S4S_EMP_CONSTR_WK_DEL_LIST');
        L_Text := 'TRUNCATE TABLE dwh_foundation.FND_S4S_EMP_CONSTR_WK';
        Dwh_Log.Write_Log(L_Name,L_System_Name,L_Script_Name,L_Procedure_Name,L_Text);
        Execute Immediate ('TRUNCATE TABLE dwh_foundation.FND_S4S_EMP_CONSTR_WK');

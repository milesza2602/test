--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_844A_NDJ_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_844A_NDJ_NEW" (
...
AS
  ...
  --  Date:        May 2015
  --  Author:      W Lyttle
  --  Purpose:     Load NON-DJ(David Jones) Allocation data to 'allocation tracker receipts in store table'
...
--                 For CHBD only.
  ...
  --               Output - TEMP_A_TRACK_ACTL_RCPT_NDJ1
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_844A_NDJ_NEW';
...
--**************************************************************************************************
      ...
      l_text := 'truncate table dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_NDJ1' ;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'truncate table dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_NDJ1';
...
--**************************************************************************************************
 ...
 --     g_sql_trunc_partition := 'alter table dwh_foundation.fnd_alloc_tracker_alloc truncate SUBPARTITION '||g_partition_name;
        ...
        L_TEXT := 'Running GATHER_TABLE_STATS ON TEMP_A_TRACK_ACTL_RCPT_NDJ1';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--     DBMS_STATS.GATHER_TABLE_STATS ('DWH_FOUNDATION',
--                                   'TEMP_A_TRACK_ACTL_RCPT_NDJ1', DEGREE => 8);  

    -- below is the new code for stats collection in 12c - Ref: 23Jan2019
     DWH_FOUNDATION.GENERIC_GATHER_TABLE_STATS(l_procedure_name,'TEMP_A_TRACK_ACTL_RCPT_NDJ1');
...
--**************************************************************************************************
    ...
    INSERT /*+ APPEND */
    INTO dwh_foundation.TEMP_A_TRACK_ACTL_RCPT_NDJ1
    with  selcal AS (   

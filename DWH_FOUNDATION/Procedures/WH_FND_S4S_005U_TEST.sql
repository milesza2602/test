--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_005U_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_005U_TEST" (p_forall_limit in integer,p_success out boolean) as
  ...
  selover AS
             (SELECT DISTINCT stg2.employee_id,
                ...
                FROM SELALL stg2 ,
                        ...
                        --STG.EMPLOYEE_ID , calendar_date, COUNT(*)
     /*                   FROM DWH_FOUNDATION.STG_S4S_EMP_JOB_DY_cpy STG , 
                        dim_calendar dc, selseq ss
                        WHERE (dc.calendar_date BETWEEN stg.JOB_start_date AND NVL(stg.JOB_END_DATE, G_JOB_end_date)
                        and  ss.maxseq = stg.sys_source_sequence_no
                         and  ss.maxbat = stg.sys_source_batch_id
                          and  ss.EMPLOYEE_ID = stg.EMPLOYEE_ID
                          and  ss.JOB_START_DATE = stg.JOB_START_DATE
                          AND STG.EMPLOYEE_STATUS IN ('H','I','R'))
                          OR (dc.calendar_date =  stg.JOB_start_date 
                        and  ss.maxseq = stg.sys_source_sequence_no
                         and  ss.maxbat = stg.sys_source_batch_id
                          and  ss.EMPLOYEE_ID = stg.EMPLOYEE_ID
                          and  ss.JOB_START_DATE = stg.JOB_START_DATE
                ...
                WHERE sel.employee_id = stg2.eMPLoyee_id
                  AND sel.calendar_date BETWEEN stg2.JOB_start_date AND NVL(stg2.JOB_END_DATE - 1, G_JOB_end_date)
           )
              */

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_297U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_297U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_of_fin_dbtrs_comm_cpy
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_of_fin_dbtrs_comm_hsp.sys_process_msg%type;
g_rec_out            fnd_of_fin_debtors_comm%rowtype;
g_rec_in             stg_of_fin_dbtrs_comm_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_297U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE DEBTORS COMM FACTS EX ORACLE FINANCE';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_of_fin_dbtrs_comm_cpy%rowtype;
...
type staging_array1 is table of stg_of_fin_dbtrs_comm_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_of_fin_dbtrs_comm_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor stg_of_fin_dbtrs_comm is
   select *
   from stg_of_fin_dbtrs_comm_cpy
...
begin
   ...
   insert into stg_of_fin_dbtrs_comm_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       and   debtors_business_id = a_tbl_update(i).debtors_business_id
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_of_fin_dbtrs_comm_cpy
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if (a_tbl_insert(i).debtors_business_id = g_rec_out.debtors_business_id and
             ...
             a_tbl_insert(i).store_billing_id = g_rec_out.store_billing_id and
...
begin
    ...
    l_text := 'LOAD OF fnd_of_fin_debtors_comm EX POS STARTED AT '||
...
--**************************************************************************************************
    open stg_of_fin_dbtrs_comm;
    fetch stg_of_fin_dbtrs_comm bulk collect into a_stg_input limit g_forall_limit;

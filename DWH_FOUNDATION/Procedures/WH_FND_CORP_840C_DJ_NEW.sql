--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_840C_DJ_NEW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_840C_DJ_NEW" (
...
AS
  ...
  --               Output - temp_alloc_tracker_alloc_dj3
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_840C_DJ_NEW';
...
--**************************************************************************************************
      ...
      l_text := 'truncate table dwh_foundation.temp_alloc_tracker_alloc_dj3' ;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      execute immediate 'truncate table dwh_foundation.temp_alloc_tracker_alloc_dj3';
...
--**************************************************************************************************
 ...
 --     g_sql_trunc_partition := 'alter table dwh_foundation.fnd_alloc_tracker_alloc truncate SUBPARTITION '||g_partition_name;
      l_text := 'Running GATHER_TABLE_STATS ON TEMP_ALLOC_TRACKER_ALLOC_DJ3';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
--     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
--                                   'TEMP_ALLOC_TRACKER_ALLOC_DJ3', DEGREE => 8);       

-- below is the new code for stats collection in 12c - Ref: 23Jan2019
     DWH_FOUNDATION.GENERIC_GATHER_TABLE_STATS(l_procedure_name,'TEMP_ALLOC_TRACKER_ALLOC_DJ3');
...
--**************************************************************************************************
    ...
    INSERT /*+ APPEND */
    INTO dwh_foundation.temp_alloc_tracker_alloc_dj3
       SELECT
        /*+   parallel(Fa,4) parallel(Di,4)   */
            ...
            AND (fa.CHAIN_CODE  = 'DJ' ) -- COMMENTED OUT FOR TESTING , MUST BE UNCOMMNTED WHEN GOING TO PROD
              ;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_140U_HIST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_140U_HIST" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - dwh_foundation.stg_om_zone_item_supp_cpj
--               Output - dwh_foundation.fnd_zone_item_supp_om_h
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_om_zone_item_supp_hsp.sys_process_msg%type;
g_rec_out            dwh_foundation.fnd_zone_item_supp_om_h%rowtype;
g_rec_in             dwh_foundation.stg_om_zone_item_supp_cpj%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_140U_HIST';
...
type stg_array is table of dwh_foundation.stg_om_zone_item_supp_cpj%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dwh_foundation.fnd_zone_item_supp_om_h%rowtype index by binary_integer;
type tbl_array_u is table of dwh_foundation.fnd_zone_item_supp_om_h%rowtype index by binary_integer;
...
type staging_array1 is table of dwh_foundation.stg_om_zone_item_supp_cpj.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of dwh_foundation.stg_om_zone_item_supp_cpj.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_om_zone_item_supp is
   select a.*
     from dwh_foundation.stg_om_zone_item_supp_cpj a, fnd_jdaff_dept_rollout b, dim_item c
    where sys_process_code = 'N'
      and a.item_no = c.item_no
      and c.department_no = b.department_no
      and b.department_live_ind = 'N'
...
begin
   ...
   insert into stg_om_zone_item_supp_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into dwh_foundation.fnd_zone_item_supp_om_h values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update dwh_foundation.fnd_zone_item_supp_om_h
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update dwh_foundation.stg_om_zone_item_supp_cpj
...
begin
   ...
   select count(1)
   into   g_count
   from   dwh_foundation.fnd_zone_item_supp_om_h
...
begin
    ...
    l_text := 'LOAD OF dwh_foundation.fnd_zone_item_supp_om_h EX OM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_om_zone_item_supp;
    fetch c_stg_om_zone_item_supp bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_050U_TAKEON
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_050U_TAKEON" (
...
g_recs_deleted      integer       :=  0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_S4S_050U_TAKEON';
...
begin
     ...
     with selstg
            as (select distinct employee_id, trunc(SHIFT_CLOCK_IN) SHIFT_CLOCK_IN from STG_S4S_EMP_LOC_JOB_SHED_cpy)
     select g_run_date, g_date, g_run_seq_no, f.*
        ...
        where f.employee_id = s.employee_id
         ...
         delete from DWH_FOUNDATION.FND_S4S_SCH_LOC_EMP_JB_DY
         where (employee_id, trunc(SHIFT_CLOCK_IN)) in (select distinct employee_id, trunc(SHIFT_CLOCK_IN) from dwh_foundation.FND_S4S_SCHLOCEMPJBDY_del_list

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_842U_NDJ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_842U_NDJ" (
...
AS
  ...
  --  Date:        May 2015
  --  Author:      W Lyttle
  --  Purpose:     Load NDJ(NON-David Jones) Allocation data to 'allocation tracker receipts at first warehouse table'
  ...
  --               For CHBD only.
  --  Tables:      Input  - temp_alloc_tracker_frst_rcpt_XD_NDJ1,temp_alloc_tracker_frst_rcpt_XD_NDJ2,temp_alloc_tracker_frst_rcpt_XD_NDJ3,temp_alloc_tracker_frst_rcpt_XD_NDJ4
  --               Output - fnd_alloc_tracker_frst_rcpt_NDJ
  --  Packages:    constants, dwh_log, dwh_valid
  --
  --  Maintenance:
  --  9 march 2016 - wendy - added 'to_loc_no <> final_loc_no' to WHS extracts as the errored record might not 
  --                          get fixed. To my knowledge this condition should not exist.
  -- 
  --
  --  Naming conventions
  --  g_  -  Global variable
 ...
 -- p_to_loc_no integer := 0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_CORP_842U_NDJ';
...
--**************************************************************************************************
      ...
      for g_sub in 0..G_COUNT_DAYS loop
                      ...
                      where calendar_date  = G_SUB_DATE;
                        ...
                        G_SUBPART_NAME := 'FND_FTAND_'||G_PART_DT;
                        
                        g_sql_trunc_partition := 'alter table dwh_foundation.FND_ALLOC_TRACKER_FRSTRCPT_NDJ truncate SUBPARTITION '||G_SUBPART_NAME;
                        
                        EXECUTE IMMEDIATE g_sql_trunc_partition;
                        commit; 
                        DBMS_LOCK.sleep(0.25);      -- to prevent shared pool issues
                    if g_sub = 0 then
     ...
     l_text := 'Running GATHER_TABLE_STATS ON FND_ALLOC_TRACKER_FRSTRCPT_NDJ';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     DBMS_STATS.gather_table_stats ('DWH_FOUNDATION',
                                   'FND_ALLOC_TRACKER_FRSTRCPT_NDJ', DEGREE => 8);       
     g_tot_ins := 0; 
...
--********************************************************************************************* 
    INSERT /*+ APPEND */
      INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ
      SELECT
        /*+ full(a)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ1 a
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
     ...
     l_text := 'TEMP_A_TRACK_FRST_RCPT_XD_NDJ1 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------------------------------------     
    INSERT /*+ APPEND parallel(z,4) */
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(b)  */
                *
              FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ2 b
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_XD_NDJ2 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(c)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ3 c
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_XD_NDJ3 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(d)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_XD_NDJ4 d
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_XD_NDJ4 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(e)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_WH_NDJ1 e
            where to_loc_no <> final_loc_no
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           g_recs_read     :=SQL%ROWCOUNT;
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_WH_NDJ1 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(f)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_WH_NDJ2 f
            where to_loc_no <> final_loc_no
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           ...
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_WH_NDJ2 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(g)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_WH_NDJ3 g
            where to_loc_no <> final_loc_no
            ;
           g_recs_read     := 0;
           g_recs_inserted := 0;
           ...
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_WH_NDJ3 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--------------------------------------------------------------------------------------     
    ...
    INTO dwh_foundation.fnd_alloc_tracker_frstrcpt_NDJ z 
      SELECT
        /*+ full(h)  */
                *
            FROM dwh_foundation.TEMP_A_TRACK_FRST_RCPT_WH_NDJ4 h
            where to_loc_no <> final_loc_no
            ;
           ...
           g_recs_inserted :=SQL%ROWCOUNT;
            COMMIT;
     g_tot_ins := g_tot_ins +   g_recs_inserted;  
     l_text := 'TEMP_A_TRACK_FRST_RCPT_WH_NDJ4 inserted='||g_recs_inserted;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

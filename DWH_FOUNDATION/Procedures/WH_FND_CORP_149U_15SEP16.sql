--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_149U_15SEP16
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_149U_15SEP16" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer) as
...
g_hospital           char(1)       := 'N';
g_count              number        :=  0;
g_hospital_text      stg_jdaff_st_ord_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_ff_st_ord%rowtype;
g_rec_in             stg_jdaff_st_ord_cpy%rowtype;
g_found              boolean;
G_Valid              Boolean;
...
type stg_array is table of stg_jdaff_st_ord_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_ff_st_ord%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_ff_st_ord%rowtype index by binary_integer;
...
type staging_array1 is table of stg_jdaff_st_ord_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_jdaff_st_ord_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_jdaff_st_ord is
   select a.*
   from dwh_foundation.stg_jdaff_st_ord_cpy a
...
begin
  -- g_rec_in.weigh_ind                 := nvl(g_rec_in.weigh_ind,0);
   ...
   g_rec_out.item_no                  := g_rec_in.item_no;
    ...
    --Store order fields
   g_rec_out.boh_qty1                 := g_rec_in.boh_qty1;
   g_rec_out.boh_1_ind                := g_rec_in.boh_1_ind;
   g_rec_out.boh_qty2                 := g_rec_in.boh_qty2;
   g_rec_out.boh_qty3                 := g_rec_in.boh_qty3;
...
begin
   ...
   insert into stg_jdaff_st_ord_hsp
        values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_ff_st_ord values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_ff_st_ord
       set    boh_qty1                        = a_tbl_update(i).boh_qty1,
              boh_1_ind                       = a_tbl_update(i).boh_1_ind,
              boh_qty2                        = a_tbl_update(i).boh_qty2,
              boh_qty3                        = a_tbl_update(i).boh_qty3,
              ...
              special_cases                   = a_tbl_update(i).special_cases,
...
--**************************************************************************************************
    open c_stg_jdaff_st_ord;
    fetch c_stg_jdaff_st_ord bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_206U_FIX_SAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_206U_FIX_SAT" (p_forall_limit in integer,p_success out boolean,p_from_loc_no in integer,p_to_loc_no in integer)  as


--**************************************************************************************************
-- datafix  - copy from wh_fnd_corp_206u
-- fix saturday data, batch=sun
--**************************************************************************************************
--  Date:        AUGUST 2014
--  Author:      Wendy Lyttle
--  Purpose:     Create RMS Stock C&H fact table in the foundation layer
--               with input ex staging table from RMS.
--  Tables:      Input  - stg_rms_stock_ch_FIX_SAT
...
g_rec_in             dwh_foundation.stg_rms_stock_ch_FIX_SAT%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_206U_FIX_SAT_'||p_from_loc_no;
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'FIX STOCK 2';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of dwh_foundation.stg_rms_stock_ch_FIX_SAT%rowtype;
...
type staging_array1 is table of dwh_foundation.stg_rms_stock_ch_FIX_SAT.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of dwh_foundation.stg_rms_stock_ch_FIX_SAT.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_stock_ch is
   select *
   from dwh_foundation.stg_rms_stock_ch_FIX_SAT
...
begin
    ...
    l_text := 'INPUT = stg_rms_stock_ch_FIX_SAT_fri';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_040U_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_040U_TEST" 
...
WITH  
 selbat AS
  (SELECT /*+ FULL(STG)  */ MAX(sys_source_batch_id) maxbat,
  employee_id
  FROM dwh_foundation.STG_S4S_EMP_LOC_STATUS_cpy STG
  WHERE SYS_SOURCE_BATCH_ID = 432
  GROUP BY employee_id
  )
  ,
  selseq AS
  (
  SELECT /*+ FULL(STG)  */ MAX(sys_source_sequence_no) maxseq,
  maxbat,
  ...
  selall AS
  (
  SELECT /*+ FULL(STG)  */ SYS_SOURCE_BATCH_ID,
  ...
  ,
selover AS
             (SELECT /*+ FULL(STG2) parallel(stg2,4)  */ DISTINCT stg2.employee_id,
                       stg2.effective_start_date
                FROM DWH_FOUNDATION.STG_S4S_EMP_LOC_STATUS_cpy stg2 ,
                        (SELECT /*+ FULL(STG) parallel(stg,4)  */ STG.EMPLOYEE_ID , calendar_date, COUNT(*)
                        FROM DWH_FOUNDATION.STG_S4S_EMP_LOC_STATUS_cpy STG , 

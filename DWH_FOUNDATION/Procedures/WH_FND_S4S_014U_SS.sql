--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_014U_SS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_014U_SS" 
...
--               Output - fnd_s4s_LABOUR_HIERARCHY_SS
...
--
-- WIP for S4S redo.... Labour Hierarchy Slowly Changing Dimension SS , LK 22/11/2018
--  Naming conventions``  
--  g_  -  Global variable
...
g_rec_out            fnd_s4s_LABOUR_HIERARCHY_SS%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_014U_SS';
...
type tbl_array_i is table of fnd_s4s_LABOUR_HIERARCHY_SS%rowtype index by binary_integer;
type tbl_array_u is table of fnd_s4s_LABOUR_HIERARCHY_SS%rowtype index by binary_integer;
...
WITH SELBU AS -- Duplicate Business Unit --SS and LK 23112018! DONE
(SELECT S4S_BUSINESS_UNIT_NO,EFFECTIVE_FROM_DATE,COUNT(*)
    ...
    GROUP BY  S4S_BUSINESS_UNIT_NO,EFFECTIVE_FROM_DATE HAVING COUNT(*) > 1
    ),

SELWG -- duplicate workgroup_id --SS and LK 23112018! DONE
AS (SELECT WORKGROUP_ID,EFFECTIVE_FROM_DATE,COUNT(*)
    FROM (SELECT DISTINCT 
              S4S_BUSINESS_UNIT_NO,
              WORKGROUP_ID ,
              WORKGROUP_NAME ,
    ...
    GROUP BY  WORKGROUP_ID,EFFECTIVE_FROM_DATE HAVING COUNT(*) > 1
    )
    ,
SELJG -- duplicate jobgroup_id --SS and LK 23112018! DONE
AS (SELECT JOBGROUP_ID,EFFECTIVE_FROM_DATE, COUNT(*)
    FROM (SELECT  DISTINCT
              S4S_BUSINESS_UNIT_NO,
              WORKGROUP_ID ,
              JOBGROUP_ID,
    ...
    GROUP BY JOBGROUP_ID,EFFECTIVE_FROM_DATE HAVING COUNT(*) > 1
    ),
SELJ -- duplicate job_id --SS and LK 23112018! DONE
AS (SELECT JOB_ID,SYS_LOAD_DATE, EFFECTIVE_FROM_DATE, ROLE_COUNT,COUNT(*)
    FROM (SELECT  
              S4S_BUSINESS_UNIT_NO,
              WORKGROUP_ID ,
              JOBGROUP_ID,
              ...
              SYS_LOAD_DATE,
              EFFECTIVE_FROM_DATE,
              COUNT(LABOUR_ROLE_ID) ROLE_COUNT
          FROM STG_S4S_LABOUR_HIERARCHY_CPY     
    ...
    GROUP BY JOB_ID,ROLE_COUNT, EFFECTIVE_FROM_DATE,SYS_LOAD_DATE HAVING COUNT(*) > ROLE_COUNT
   ),
  
  SELLR AS  -- duplicate labour role --SS,FDV and LK 10122018! DONE
    (SELECT LABOUR_ROLE_ID,JOB_ID,SYS_LOAD_DATE, EFFECTIVE_FROM_DATE, ROLE_COUNT,COUNT(*)
    FROM (SELECT  
              S4S_BUSINESS_UNIT_NO,
              WORKGROUP_ID ,
              JOBGROUP_ID,
    ...
    GROUP BY LABOUR_ROLE_ID,JOB_ID,ROLE_COUNT, EFFECTIVE_FROM_DATE,SYS_LOAD_DATE HAVING COUNT(*) > ROLE_COUNT
   ),
   
   SELDF AS --conflict with existing primary key
       (SELECT LABOUR_ROLE_ID,JOB_ID,EFFECTIVE_FROM_DATE, COUNT(*)
        from (
            select 
                   stg.LABOUR_ROLE_ID,
                   stg.JOB_ID,
                   stg.EFFECTIVE_FROM_DATE
              from  fnd_s4s_LABOUR_HIERARCHY_SS fnd
              inner join stg_s4s_labour_hierarchy_cpy stg 
                      on fnd.EFFECTIVE_FROM_DATE= stg.EFFECTIVE_FROM_DATE 
                     and fnd.labour_role_id = stg.labour_role_id 
                     and fnd.job_id = stg.job_id
              where (fnd.S4S_BUSINESS_UNIT_NO <> stg.S4S_BUSINESS_UNIT_NO
              or    fnd.WORKGROUP_ID <> stg.WORKGROUP_ID
              or       fnd.JOBGROUP_ID <> stg.JOBGROUP_ID)
                )
    GROUP BY LABOUR_ROLE_ID,JOB_ID, EFFECTIVE_FROM_DATE 
   )
SELECT STG.SYS_SOURCE_BATCH_ID,     
       ...
       SB.S4S_BUSINESS_UNIT_NO  DUP_S4S_BUSINESS_UNIT_NO,
       ...
       sdf.EFFECTIVE_FROM_DATE CONFLICT_DATE
...
WHERE STG.WORKGROUP_ID = SWG.WORKGROUP_ID(+)
  ...
  AND STG.S4S_BUSINESS_UNIT_NO = SB.S4S_BUSINESS_UNIT_NO(+)
  AND STG.JOB_ID = SJ.JOB_ID(+)
  AND STG.LABOUR_ROLE_ID = SLR.LABOUR_ROLE_ID(+)
  AND STG.LABOUR_ROLE_ID = SDF.LABOUR_ROLE_ID(+)
  AND STG.JOB_ID = SDF.JOB_ID(+)
  AND STG.EFFECTIVE_FROM_DATE = SDF.EFFECTIVE_FROM_DATE(+) 
...
begin
      ...
      into  g_S4S_BUSINESS_UNIT_NO,
            ...
            g_EFFECTIVE_TO_DATE
      from  fnd_s4s_LABOUR_HIERARCHY_SS
      where EFFECTIVE_FROM_DATE = (select max(EFFECTIVE_FROM_DATE) from fnd_s4s_LABOUR_HIERARCHY_SS where labour_role_id = G_REC_OUT.LABOUR_ROLE_ID)
...
*/
      IF G_REC_IN.DUP_S4S_BUSINESS_UNIT_NO IS NOT NULL 
             THEN 
         g_hospital      := 'Y';
         g_hospital_text := 'Duplicate S4S_BUSINESS_UNIT_NO ';
         l_text          := 'Duplicate S4S_BUSINESS_UNIT_NO '
          ||' '||g_rec_out.S4S_BUSINESS_UNIT_NO 
          ||' '||g_rec_out.WORKGROUP_ID  
          ||' '||g_rec_out.JOBGROUP_ID 
          ||' '||g_rec_out.JOB_ID
     ...
     IF G_REC_IN.DUP_JOBGROUP_ID IS NOT NULL 
      ...
      IF G_REC_IN.CONFLICT_DATE IS NOT NULL 
         THEN 
         g_hospital      := 'Y';
         g_hospital_text := 'Duplicate EFFECTIVE_FROM_DATE ';
         l_text          := 'Duplicate EFFECTIVE_FROM_DATE '
          ||' '||g_rec_out.S4S_BUSINESS_UNIT_NO 
          ||' '||g_rec_out.WORKGROUP_ID  
          ||' '||g_rec_out.JOBGROUP_ID 
          ||' '||g_rec_out.JOB_ID
...
begin
  ...
  insert into STG_S4S_LABOUR_HIERARCHY_HSPSS values 
...
begin
   ...
   forall i in a_tbl_insert.first .. a_tbl_insert.last
      save exceptions
      insert into fnd_s4s_LABOUR_HIERARCHY_SS values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
        update fnd_s4s_LABOUR_HIERARCHY_SS
        set  

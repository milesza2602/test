--------------------------------------------------------
--  DDL for Procedure WH_FND_FPI_030U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_FPI_030U" 
...
--
--  Tables:      Input  - STG_FPI_PACK_COMP_COUNTRY_CPY                                                    
--               Output - FND_ITEM_SUP_COMP_PACK_COUNTRY                                                    
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_FPI_030U';                              -- STORE PROC CHANGE
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STG_FPI_PACK_COMP_COUNTRY EX FPI';  -- STG TABLE NAME CHANGE
...
begin
   ...
   merge  into fnd_item_sup_comp_pack_country fnd     		                                                           -- FND TABLE NAME CHANGE
   using ( 
    select /*+ PARALLEL(CPY1,8) */ cpy1.*
    from   stg_fpi_pack_comp_country_cpy cpy1,
           fnd_supplier sup,
           fnd_item itm,
         ...
         (select /*+ PARALLEL(8) */ sys_source_batch_id, sys_source_sequence_no,
          ...
          from (select /*+ PARALLEL(tmp,8) */ tmp.*,
		      ...
		      order by sys_source_batch_id desc, sys_source_sequence_no desc)
                    as rank
                  from stg_fpi_pack_comp_country_cpy tmp                                                               -- STG TABLE NAME CHANGE 
           )
          where rank = 1)
    ...
    when matched then 
    update set                                                                                                       
      fnd.country_code	            =	mer_rec.country_code,
      fnd.country_name	            =	mer_rec.country_name,
      fnd.country_type              =	mer_rec.country_type,
   ...
   values                                                                                                            
     (         
      ...
      mer_rec.country_name,	
      mer_rec.country_type,	
      g_date
     )           
          ;  
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   stg_fpi_pack_comp_country_cpy                                                                           -- STG TABLE NAME CHANGE 
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       raise;
end wh_fnd_fpi_030u                                                                                             -- STORE PROC CHANGE 
;

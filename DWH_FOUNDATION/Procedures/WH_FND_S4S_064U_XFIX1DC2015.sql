--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_064U_XFIX1DC2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_064U_XFIX1DC2015" (p_forall_limit in integer,p_success out boolean) as
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_064U_XFIX1DC2015';
...
-- g_date := trunc(sysdate);
  ...
  WITH SELEXT AS
            (SELECT /*+ PARALLEL(A,8) FULL(A) */ location_no, Exception_TYPE_ID, Exception_date, MAX(SYS_SOURCE_BATCH_ID) MAXBAT
             FROM DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_LOC_DY_CPY A
             GROUP BY location_no, Exception_TYPE_ID, Exception_date)
	select  /*+ PARALLEL(A,8) FULL(A) */ A.LOCATION_NO,
              A.EXCEPTION_TYPE_ID,
              A.EXCEPTION_DATE,
              A.EXCEPTION_START_TIME,
              A.EXCEPTION_END_TIME,
              G_DATE
	from DWH_FOUNDATION.STG_S4S_SCHED_XCPTN_LOC_DY_CPY A, SELEXT SE,
                dwh_performance.dim_calendar f,
							  dwh_performance.dim_exception_type e,
							  dwh_foundation.fnd_location d
  where A.sys_source_batch_id = SE.MAXBAT
  AND A.Exception_TYPE_ID = SE.Exception_TYPE_ID
  AND A.Exception_DATE = SE.Exception_DATE
  AND A.LOCATION_NO = SE.LOCATION_NO
  and A.LOCATION_NO = d.LOCATION_NO
	and A.EXCEPTION_TYPE_ID  = e.EXCEPTION_TYPE_ID
	and A.EXCEPTION_DATE = f.calendar_date;
...
END WH_FND_S4S_064U_XFIX1DC2015;

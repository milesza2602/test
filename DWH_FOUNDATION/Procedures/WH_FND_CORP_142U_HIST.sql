--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_142U_HIST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_142U_HIST" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - dwh_foundation.stg_jdaff_zone_item_supp_cpj
--               Output - dwh_foundation.fnd_zone_item_supp_hist
...
g_hospital           char(1)       := 'N';
g_hospital_text      dwh_foundation.stg_jdaff_zone_item_supp_hsp.sys_process_msg%type;
g_rec_out            dwh_foundation.fnd_zone_item_supp_hist%rowtype;
g_rec_in             dwh_foundation.stg_jdaff_zone_item_supp_cpj%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_142U_HIST';
...
type stg_array is table of dwh_foundation.stg_jdaff_zone_item_supp_cpj%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of dwh_foundation.fnd_zone_item_supp_hist%rowtype index by binary_integer;
type tbl_array_u is table of dwh_foundation.fnd_zone_item_supp_hist%rowtype index by binary_integer;
...
type staging_array1 is table of dwh_foundation.stg_jdaff_zone_item_supp_cpj.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of dwh_foundation.stg_jdaff_zone_item_supp_cpj.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_zone_item_supp is
   select a.*
     from dwh_foundation.stg_jdaff_zone_item_supp_cpj a, fnd_jdaff_dept_rollout b, dim_item c
...
--   select * 
--     from dwh_foundation.stg_jdaff_zone_item_supp_cpj_cpy
--    where sys_process_code = 'N'
-- order by sys_source_batch_id,sys_source_sequence_no;
...
begin
   ...
   insert into dwh_foundation.stg_jdaff_zone_item_supp_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into dwh_foundation.fnd_zone_item_supp_hist values a_tbl_insert(i);
   ...
   exception
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_text := 'Item :- '||a_tbl_insert(g_error_index).item_no ;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          l_text := 'supplier_no :- '||a_tbl_insert(g_error_index).supplier_no; 
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          l_text := 'zone_no :- '||a_tbl_insert(g_error_index).zone_no ;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          l_text := 'to_loc_no :- '||a_tbl_insert(g_error_index).to_loc_no ;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          l_text := 'last_updated_date :- '||a_tbl_insert(g_error_index).last_updated_date ;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
          l_text := 'next_wk_cost_price :- '||a_tbl_insert(g_error_index).next_wk_cost_price ;
          dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       save exceptions
       update dwh_foundation.fnd_zone_item_supp_hist
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update dwh_foundation.stg_jdaff_zone_item_supp_cpj
...
begin
   ...
   select count(1)
   into   g_count
   from   dwh_foundation.fnd_zone_item_supp_hist    --dwh_foundation.stg_jdaff_zone_item_supp_cpj_cpy
...
begin
    ...
    l_text := 'LOAD OF dwh_foundation.fnd_zone_item_supp_hist EX OM STARTED AT '||
...
--**************************************************************************************************
    open c_zone_item_supp;
    fetch c_zone_item_supp bulk collect into a_stg_input limit g_forall_limit;

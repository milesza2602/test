--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_742U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_742U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        August 2008
--  Author:      Alastair de Wet
--  Purpose:     Create OM pick fact table in the foundation layer
--               with input ex staging table from OM.
--  Tables:      Input  - stg_om_po_plan
--               Output - fnd_zone_item_supp_om_po_plan
...
g_count              number        :=  0;
g_count_trunc        number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_om_po_plan_hsp.sys_process_msg%type;
g_rec_out            fnd_zone_item_supp_om_po_plan%rowtype;
g_rec_in             stg_om_po_plan_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_742U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE PO PLAN FOODS FACTS EX OM';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_om_po_plan_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_zone_item_supp_om_po_plan%rowtype index by binary_integer;
type tbl_array_u is table of fnd_zone_item_supp_om_po_plan%rowtype index by binary_integer;
...
type staging_array1 is table of stg_om_po_plan_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_om_po_plan_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_om_po_plan is
  -- select *
  -- from stg_om_po_plan_cpy;
   
 select a.*
   from stg_om_po_plan_cpy a, dim_item b, fnd_jdaff_dept_rollout c
   where a.sys_process_code = 'N'
     and a.item_no = b.item_no
     and b.department_no = c.department_no;
 

--   process_code ignored as truncation occurs on fnd table and rerun must pick up all input staging
...
--   end if;
   ...
   if g_rec_out.to_loc_no is not null then
...
begin
   ...
   insert into stg_om_po_plan_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_zone_item_supp_om_po_plan values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_zone_item_supp_om_po_plan
       ...
       where  zone_group_no                   = a_tbl_update(i).zone_group_no and
              zone_no                         = a_tbl_update(i).zone_no and
...
-- Check to see if item is present on table and update/insert accordingly
   ...
   where  supplier_no   = g_rec_out.supplier_no    and
          zone_no       = g_rec_out.zone_no     and
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).supplier_no     = g_rec_out.supplier_no   and
...
begin
    ...
    l_text := 'LOAD OF fnd_zone_item_supp_om_po_plan EX ALLPOINTS STARTED AT '||
...
--**************************************************************************************************
    ...
    if g_count_trunc > 0 then
      begin
        l_text := 'TRUNCATED TABLE fnd_zone_item_supp_om_po_plan '||
        to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
        dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
        execute immediate 'truncate table fnd_zone_item_supp_om_po_plan';
      end;
    end if;
...
--**************************************************************************************************
    open c_stg_om_po_plan;
    fetch c_stg_om_po_plan bulk collect into a_stg_input limit g_forall_limit;

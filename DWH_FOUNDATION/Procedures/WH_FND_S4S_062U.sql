--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_062U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_062U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        December 2014
--  Author:      Lwazi Ntloko
--  Purpose:     Update actual exception location day information for Scheduling for Staff(S4S)
--
--  Tables:      AIT load - STG_S4S_ACTL_XCPTN_LOC_DY 
--               Input    - STG_S4S_ACTL_XCPTN_LOC_DY_cpy
--               Output   - FND_S4S_ACTL_XCPTN_LOC_DY
--  Packages:    dwh_constants, dwh_log, dwh_valid
--
--  Maintenance:
--  w lyttle 24 feb 2015 - SRS from Teddy
--                        a.	S4S SP: usp_Stg_Outbound_Exception_Schedule_Loc
--                        b.	Trans Type:  S4SAEN
--                        c.	BI Interface: 
--                        d.	BI Logic:
...
g_hospital           char(1)       := 'N';
g_hospital_text      DWH_FOUNDATION.STG_S4S_ACTL_XCPTN_LOC_DY_hsp.sys_process_msg%type;
g_rec_out            DWH_FOUNDATION.FND_S4S_ACTL_XCPTN_LOC_DY%rowtype;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_S4S_062U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE actual exception location day  data ex S4S';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;

  -- For output arrays into bulk load forall statements --
type tbl_array_i is table of FND_S4S_ACTL_XCPTN_LOC_DY%rowtype index by binary_integer;
type tbl_array_u is table of FND_S4S_ACTL_XCPTN_LOC_DY%rowtype index by binary_integer;
...
type staging_array1 is table of STG_S4S_ACTL_XCPTN_LOC_DY_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of STG_S4S_ACTL_XCPTN_LOC_DY_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_STG_S4S_EMP_JOB is
   select SYS_SOURCE_BATCH_ID,
          ...
          STG.EXCEPTION_TYPE_ID STG_EXCEPTION_ID,
   ...
   from DWH_FOUNDATION.STG_S4S_ACTL_XCPTN_LOC_DY_cpy stg,
        FND_LOCATION DL,
        dim_calendar dc2,
...
begin
      ...
      from dwh_foundation.FND_S4SACTL_XCPTN_LOCDELETE;
     ...
     BEGIN
	    insert /*+ append  into dwh_foundation.FND_S4SACTL_XCPTN_LOCDELETE
			select f.*, g_run_date, g_date, g_run_seq_no 
			from DWH_FOUNDATION.FND_S4S_ACTL_XCPTN_LOC_DY f
		  ...
		  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------
		  l_text := 'Insert into FND_S4SACTL_XCPTN_LOCDELETE recs='||g_recs_inserted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
          dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);

         delete from DWH_FOUNDATION.FND_S4S_ACTL_XCPTN_LOC_DY
         where (LOCATION_NO, EXCEPTION_TYPE_ID, EXCEPTION_DATE) in (select distinct LOCATION_NO, EXCEPTION_TYPE_ID, EXCEPTION_DATE from dwh_foundation.FND_S4SACTL_XCPTN_LOCDELETE
         where run_seq_no = g_run_seq_no);
     
          g_recs :=SQL%ROWCOUNT ;
          COMMIT;
          g_recs_deleted := g_recs;
                l_text := 'Deleted from DWH_FOUNDATION.FND_S4S_ACTL_XCPTN_LOC_DY recs='||g_recs_deleted||' - run_date='||g_run_date||' - batch_date='||g_date||' - run_seq_no='||g_run_seq_no;
			dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
      
      exception
         when no_data_found then
                l_text := 'No deletions done for DWH_FOUNDATION.FND_S4S_ACTL_XCPTN_LOC_DY';
      dwh_log.write_log (l_name, l_system_name, l_script_name,l_procedure_name, l_text);
   end;          
...
begin
    ...
    g_rec_out.EXCEPTION_TYPE_ID    := g_rec_in.STG_EXCEPTION_ID;
...
begin
   ...
   g_rec_in.sys_process_msg       := g_hospital_text;
      ...
      insert into dwh_foundation.STG_S4S_ACTL_XCPTN_LOC_DY_hsp values
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into FND_S4S_ACTL_XCPTN_LOC_DY  values a_tbl_insert(i);
...
begin
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
             UPDATE FND_S4S_ACTL_XCPTN_LOC_DY
                SET  EXCEPTION_START_TIME = a_tbl_update(i).EXCEPTION_START_TIME,
...
begin
   ...
   select count(1)
   into   v_count
   from   DWH_FOUNDATION.FND_S4S_ACTL_XCPTN_LOC_DY
...
begin
    ...
    l_text := 'LOAD THE actual exception location day  data ex S4S STARTED AT '||
...
--**************************************************************************************************
 ...
 --   g_date := trunc(sysdate);
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     ...
     select min(sys_source_batch_id), max(sys_source_batch_id) into g_SYS_SOURCE_BATCH_ID, g_max from DWH_FOUNDATION.STG_S4S_ACTL_XCPTN_LOC_DY_cpy;

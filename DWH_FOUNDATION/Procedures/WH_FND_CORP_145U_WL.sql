--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_145U_WL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_145U_WL" (p_forall_limit in integer,
...
g_post_date           date;
   ...
   cursor stg_dup is
      select * from dwh_foundation.tmp_stg_cam_location_item
      where (location_no, item_no, post_date)
         in
      (select location_no,item_no, POST_DATE
         from dwh_foundation.tmp_stg_cam_location_item
...
--**************************************************************************************************
    ...
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      ...
      l_text := 'truncate tmp';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   execute immediate ('truncate table DWH_FOUNDATION.tmp_stg_CAM_LOCATION_ITEM');
   commit;
       
    l_text := 'insert into tmp';
    dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    
   insert /*+ append */ into dWH_FOUNDATION.tmp_stg_CAM_LOCATION_ITEM

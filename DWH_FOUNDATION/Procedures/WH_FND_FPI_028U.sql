--------------------------------------------------------
--  DDL for Procedure WH_FND_FPI_028U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_FPI_028U" 
...
--
--  Tables:      Input  - STG_FPI_PACK_COMP_CPY                                                            
--               Output - FND_ITEM_SUP_COMP_PACK                                                            
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_FPI_028U';                              
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STG_FPI_PACK_COMP EX FPI';    
...
--/*+ first_rows parallel(fnd) parallel(upd_rec) */
   ...
   merge  into fnd_item_sup_comp_pack fnd     		                                                           
   using (
   
    select /*+ PARALLEL(CPY1,8) */ cpy1.*
    from   stg_fpi_pack_comp_cpy cpy1,
           fnd_supplier sup,
           fnd_item itm,
    ...
    where  cpy1.supplier_no   = sup.supplier_no and
         ...
         (select /*+ PARALLEL(8) */ sys_source_batch_id, sys_source_sequence_no,
	      	      item_no,supplier_code,site_code,supplier_no,component_name, spec_version, pack_comp_id
          from (select /*+ PARALLEL(tmp,8) */ tmp.*,
		      ...
		      order by sys_source_batch_id desc, sys_source_sequence_no desc)
                    as rank
                  from stg_fpi_pack_comp_cpy tmp                                                                
           )
          where rank = 1)
    ...
    on    (  
        ...
        fnd.pack_comp_id	          =	mer_rec.pack_comp_id
	 )
    when matched then 
    update set                                                                                                       
      fnd.description	            =	mer_rec.description,
      fnd.certification	            =	mer_rec.certification,
      fnd.level_ind                 =	mer_rec.level_ind,
      fnd.material_of_construction	=	mer_rec.material_of_construction,
      fnd.food_contact	            =	mer_rec.food_contact,
      fnd.perc_recycled	            =	mer_rec.perc_recycled,
      fnd.print_method	            =	mer_rec.print_method,
      fnd.specification_name        =	mer_rec.specification_name,
      fnd.supplier_name	            =	mer_rec.supplier_name,
      fnd.weight	                =	mer_rec.weight,
   ...
   values                                                                                                           -- COLUNM NAME CHANGE 
     (         
      ...
      mer_rec.pack_comp_id,	
      mer_rec.description,	
      mer_rec.certification,	
      mer_rec.level_ind,	
      mer_rec.material_of_construction,	
      mer_rec.food_contact,	
      mer_rec.perc_recycled,	
      mer_rec.print_method,	
      mer_rec.specification_name,	
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   stg_fpi_pack_comp_cpy                                                                           -- STG TABLE NAME CHANGE 
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       raise;
end wh_fnd_fpi_028u                                                                                             -- STORE PROC CHANGE 
;

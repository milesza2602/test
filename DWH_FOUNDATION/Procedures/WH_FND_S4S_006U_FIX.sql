--------------------------------------------------------
--  DDL for Procedure WH_FND_S4S_006U_FIX
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_S4S_006U_FIX" (p_forall_limit IN INTEGER, p_success OUT BOOLEAN)AS
  --**************************************************************************************************
-- DATA FIX 24 MARCH 2016 - ISSUE WITH JOB NOT RUNNING BECAUSE OF SCHEDULE CHANGES
  --**************************************************************************************************
  --  Date:        July 2014
  --  Author:      Wendy lyttle
...
--
  ...
  --               Input    - WL_S4S_LOC_EMP_JB_PYCT_DY_CPY
...
g_recs_deleted      integer       :=  0;
  ...
  l_module_name sys_dwh_errlog.log_procedure_name%type := 'WH_FND_S4S_006U_FIX';
...
type staging_array1
IS
  TABLE OF WL_S4S_LOC_EMP_JB_PYCT_DY_CPY.sys_source_batch_id%type INDEX BY binary_integer;
type staging_array2
IS
  TABLE OF WL_S4S_LOC_EMP_JB_PYCT_DY_CPY.sys_source_sequence_no%type INDEX BY binary_integer;
  ...
  is
    ...
    FROM DWH_FOUNDATION.WL_S4S_LOC_EMP_JB_PYCT_DY_CPY stg,
      fnd_location fl,
...
begin
    ...
    with selstg
            as (select distinct employee_id, pay_week_date from DWH_FOUNDATION.WL_S4S_LOC_EMP_JB_PYCT_DY_CPY)

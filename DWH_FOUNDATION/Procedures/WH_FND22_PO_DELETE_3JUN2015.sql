--------------------------------------------------------
--  DDL for Procedure WH_FND22_PO_DELETE_3JUN2015
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND22_PO_DELETE_3JUN2015" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND22_PO_DELETE_3JUN2015';
...
FOR V_CUR IN
            ...
            FROM dwh_foundation.FND_RTL_PURCHASE_ORDER A,
                  selext B
            WHERE a.po_NO = b.PO_NO
            and into_store_date <= '18/MAR/09'
)
LOOP
      ...
      begin
            DELETE /*+ FULL(A)  */ FROM dwh_foundation.FND_RTL_PURCHASE_ORDER a
              where a.PO_NO = V_CUR.PO_NO

              ;
              
              g_recs_DELETED_FRPO := g_recs_DELETED_FRPO + SQL%ROWCOUNT;
              commit;
              
                                                    if g_recs_DELETED_FRPO >= 100000
        then 
                  g_gtot_recs := g_gtot_recs + g_recs_DELETED_FRPO;
        ...
        when no_data_found then
          g_recs_DELETED_FRPO := g_recs_DELETED_FRPO ;
      end;      
...
l_text := 'FND_RTL_PURCHASE_ORDER recs deleted = '||g_recS_deleted_FRPO;
dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);    
...
END WH_FND22_PO_DELETE_3JUN2015;

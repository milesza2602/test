--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_185U_WL3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_185U_WL3" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_185U_WL3';
...
cursor c_STG_apx_item_link is
   WITH SELARC AS (SELECT  /*+ FULL(A) PARALLEL(A,4) */  *
                  FROM STG_apx_item_link_ARC A
                  WHERE A.POST_DATE IN (
 '22 JULY 2017'
 )
 /*'05-Jun-17'
,'07-Jun-17'
...
*/
       ...
       SELBAT AS (SELECT  /*+ FULL(A) PARALLEL(A,4) */ CHAIN_CODE
                      , PROD_LINK_REF_NO
                      , PROD_LINK_IND
                      , CHAIN_NO
                      , ITEM_NO
        ...
        SELSEQ AS (SELECT  /*+ FULL(C) PARALLEL(C,4) */ C.CHAIN_CODE
                      , C.PROD_LINK_REF_NO
                      , C.PROD_LINK_IND
                      , C.CHAIN_NO
                      , C.ITEM_NO
                  ...
                  GROUP BY C.CHAIN_CODE
                      ...
                      , trunc(C.POST_DATE) 
                      , MAXBAT)
   select /*+ FULL(A) PARALLEL(A,4) */  A.SYS_SOURCE_BATCH_ID
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set    CREATE_DATE                  = a_tbl_update(i).CREATE_DATE ,
              ...
              last_updated_date            = a_tbl_update(i).POST_DATE
...
--**************************************************************************************************
  ...
  dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    DELETE FROM FND_apx_item_link_chn_item_dy WHERE POST_DATE = '22 JULY 2017';
    COMMIT;
 --   g_date := '3 oct 2016';

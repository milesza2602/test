--------------------------------------------------------
--  DDL for Procedure WH_FND_FPI_010U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_FPI_010U" 
                                    (p_forall_limit in integer,p_success out boolean) as
                                                                
--**************************************************************************************************
--  Date:        February 2018
--  Author:      Francisca De Vaal
--  Tables:      Input  - STG_FPI_SPEC_INSTRUCT_CPY                                                            
--               Output - FND_ITEM_SUP_SPEC_INSTR                                                              
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_FPI_010U';                              -- STORE PROC CHANGE
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STG_FPI_SPEC_INSTRUCT_CPY EX FPI';    
...
begin
 ...
 merge  into fnd_item_sup_spec_instr fnd     		                                                         
   using (
   
    select /*+ PARALLEL(CPY1,8) */ cpy1.*
    from   stg_fpi_spec_instruct_cpy cpy1,
           fnd_supplier sup,
           fnd_item itm,
    ...
    where  cpy1.supplier_no   = sup.supplier_no and
         ...
         (select /*+ PARALLEL(8) */ sys_source_batch_id, sys_source_sequence_no,
	      	      item_no, supplier_code, site_code, supplier_no, component_name, spec_version, attribute_name
          from (select /*+ PARALLEL(tmp,8) */ tmp.*,
		      ...
		      order by sys_source_batch_id desc, sys_source_sequence_no desc)
                    as rank
                  from stg_fpi_spec_instruct_cpy tmp                                                                
           )
          where rank = 1)
   ...
   when matched then 
   update set                                                                                                       
        fnd.step	            =	mer_rec.step,
        fnd.catb	            =	mer_rec.catb,
        fnd.catd	            =	mer_rec.catd,
        fnd.cate	            =	mer_rec.cate,
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   STG_FPI_SPEC_INSTRUCT_CPY                                                                           -- STG TABLE NAME CHANGE 
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       RAISE;
end wh_fnd_fpi_010u                                                                                             -- STORE PROC CHANGE 
;

--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_147U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_147U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_cam_zone_item_cpy
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_cam_zone_item_hsp.sys_process_msg%type;
g_rec_out            fnd_zone_item%rowtype;
g_rec_in             stg_cam_zone_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_147U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE ZONE_ITEM MASTERDATA EX CAM';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_cam_zone_item_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_zone_item%rowtype index by binary_integer;
type tbl_array_u is table of fnd_zone_item%rowtype index by binary_integer;
...
type staging_array1 is table of stg_cam_zone_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_cam_zone_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_cam_zone_item_ is
   select a.*
     from stg_cam_zone_item_cpy a
...
begin
   ...
   g_rec_out.zone_group_no                  := 1;
   g_rec_out.zone_no                        := g_rec_in.zone_no;
   g_rec_out.item_no                        := g_rec_in.item_no;
   g_rec_out.case_mass                      := g_rec_in.case_mass;
   g_rec_out.case_selling_incl_vat          := g_rec_in.case_selling_incl_vat;
   g_rec_out.reg_rsp_incl_vat               := g_rec_in.reg_rsp_incl_vat;    --(g_rec_in.reg_rsp_incl_vat/100);
   g_rec_out.case_cost_price                := g_rec_in.case_cost_price;
   ...
   g_rec_out.reg_rsp                        := g_rec_in.reg_rsp_incl_vat;
-- FOODS ONLY THEREFORE NORMAL RAND FIELD = OPR FIELD-- ADW  
    ...
    g_rec_out.CASE_COST_PRICE_OPR           := g_rec_in.CASE_COST_PRICE;
    g_rec_out.CASE_SELLING_EXCL_VAT_OPR     := g_rec_in.CASE_SELLING_EXCL_VAT;
    g_rec_out.CASE_SELLING_INCL_VAT_OPR     := g_rec_in.CASE_SELLING_INCL_VAT;
    g_rec_out.COST_PRICE_LOCAL              := g_rec_in.COST_PRICE_LOCAL;
    g_rec_out.COST_PRICE_OPR                := g_rec_in.COST_PRICE;
    g_rec_out.REG_RSP_EXCL_VAT_OPR          := g_rec_in.REG_RSP_EXCL_VAT;
    g_rec_out.REG_RSP_INCL_VAT_OPR          := g_rec_in.REG_RSP_INCL_VAT;
...
/*   g_rec_out.num_shelf_life_days            := 0;
   g_rec_out.BASE_RETAIL_IND                := 0;
   g_rec_out.REG_RSP                        := 0;
   g_rec_out.SELLING_UOM_CODE               := ' ';
   g_rec_out.MARKET_BASKET_CODE             := ' ';
   g_rec_out.ITEM_ZONE_LINK_CODE            := ' ';
   g_rec_out.MULTI_SELLING_UOM_CODE         := ' ';
   g_rec_out.SELLING_UNIT_RSP               := 0;
   g_rec_out.MULTI_UNIT_RSP                 := 0;
   g_rec_out.MULTI_QTY                      := 0;
   g_rec_out.PRODUCT_STATUS_CODE            := ' ';
   g_rec_out.PRODUCT_STATUS_1_CODE          := ' ';
   g_rec_out.TRAY_SIZE_CODE                 := ' ';
   g_rec_out.MU_PACK_HEIGHT_CODE            := ' ';
   g_rec_out.num_supplier_leadtime_days     := 0;
   g_rec_out.num_extra_leadtime_days        := 0;
   g_rec_out.reg_deliver_pattern_code       := ' ';
*/
...
/*   
   begin
         ...
         exception
         when no_data_found then g_vat_rate_perc   := null;
   end;
   
   g_rec_out.case_selling_excl_vat          := g_rec_in.case_selling_incl_vat * 100 / (100 + g_vat_rate_perc);  
*/
   g_rec_out.case_selling_excl_vat          := g_rec_in.case_selling_excl_vat;
   
   -- This code added due to Defect 140 raised by Edwina Thomas
-- added in PROD on 26 February 2015 by  Q. Smit.
...
begin
   ...
   insert into stg_cam_zone_item_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_zone_item values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_zone_item
       set    case_mass                 = a_tbl_update(i).case_mass,
              case_selling_incl_vat     = a_tbl_update(i).case_selling_incl_vat,
              case_selling_excl_vat     = a_tbl_update(i).case_selling_excl_vat,
              reg_rsp_incl_vat          = a_tbl_update(i).reg_rsp_incl_vat,
              reg_rsp                   = a_tbl_update(i).reg_rsp,
              case_cost_price           = a_tbl_update(i).case_cost_price,
...
begin
   ...
   into   g_count
   from   fnd_zone_item
   where  zone_group_no = g_rec_out.zone_group_no and
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).zone_group_no   = g_rec_out.zone_group_no and
...
begin
    ...
    l_text := 'LOAD OF fnd_zone_item EX CAM STARTED AT '||
...
--**************************************************************************************************
    open c_stg_cam_zone_item_;
    fetch c_stg_cam_zone_item_ bulk collect into a_stg_input limit g_forall_limit;

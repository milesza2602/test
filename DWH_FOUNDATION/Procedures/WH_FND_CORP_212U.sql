--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_212U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_212U" (p_forall_limit in integer,p_success out boolean) as


--**************************************************************************************************
--  Date:        April 2008
--  Author:      Alastair de Wet
--  Purpose:     Create WW online fact table in the foundation layer
--               with input ex staging table from In the Bag.
--  Tables:      Input  - stg_itb_sales_cpy
--               Output - fnd_rtl_loc_item_dy_wwo_sale
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_itb_sales_hsp.sys_process_msg%type;
g_rec_out            fnd_rtl_loc_item_dy_wwo_sale%rowtype;
g_rec_in             stg_itb_sales_cpy%rowtype;
g_found              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_212U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE WW ONLINE SALES FACTS EX ITB';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_itb_sales_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_rtl_loc_item_dy_wwo_sale%rowtype index by binary_integer;
type tbl_array_u is table of fnd_rtl_loc_item_dy_wwo_sale%rowtype index by binary_integer;
...
type staging_array1 is table of stg_itb_sales_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_itb_sales_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_itb_sales is
   select *
   from stg_itb_sales_cpy
...
begin
   ...
   g_rec_out.online_total_sales_cost   := g_rec_in.online_total_sales_cost ;
...
begin
   ...
   insert into stg_itb_sales_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_rtl_loc_item_dy_wwo_sale values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_rtl_loc_item_dy_wwo_sale
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_rtl_loc_item_dy_wwo_sale
...
begin
    ...
    l_text := 'LOAD OF FND_RTL_LOC_ITEM_DY_WWO_SALE EX ITB STARTED AT '||
...
--**************************************************************************************************
    open c_stg_itb_sales;
    fetch c_stg_itb_sales bulk collect into a_stg_input limit g_forall_limit;

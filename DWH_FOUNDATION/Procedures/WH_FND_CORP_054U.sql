--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_054U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_054U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        August 2008
--  Author:      Christie Koorts
--  Purpose:     Create class dimension table in the foundation layer
--               with input ex staging table from RMS.
--  Tables:      Input  - stg_rms_class_cpy
--               Output - fnd_class
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_class_hsp.sys_process_msg%type;
g_rec_out            fnd_class%rowtype;
g_rec_in             stg_rms_class_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_054U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE CLASS MASTERDATA EX RMS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_rms_class_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_class%rowtype index by binary_integer;
type tbl_array_u is table of fnd_class%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_class_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_class_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_class is
   select *
   from stg_rms_class_cpy
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_class values a_tbl_insert(i);
...
begin
   ...
   g_found := dwh_valid.fnd_class(g_rec_out.class_no,g_rec_out.department_no);

-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if (a_tbl_insert(i).class_no = g_rec_out.class_no and
...
begin
    ...
    l_text := 'LOAD OF FND_CLASS EX RMS STARTED AT '||
...
--**************************************************************************************************
    open c_stg_rms_class;
    fetch c_stg_rms_class bulk collect into a_stg_input limit g_forall_limit;

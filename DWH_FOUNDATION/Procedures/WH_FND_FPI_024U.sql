--------------------------------------------------------
--  DDL for Procedure WH_FND_FPI_024U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_FPI_024U" 								
...
--
--  Tables:      Input  - STG_FPI_COMP_INGR_ATTRIB_CPY                                                             
--               Output - FND_ITEM_SUP_COMP_ATTRB                                                                  
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_FPI_024U';                                 -- STORE PROC CHANGE
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_facts;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_facts;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE STG_FPI_COMP_INGR_ATTRIB_CPY EX FPI';    -- STG TABLE NAME CHANGE
...
begin
   ...
   merge  into fnd_item_sup_ingr_attrb fnd   		                                                              
   using (
   
    select /*+ PARALLEL(CPY1,8) */ cpy1.*
    from   stg_fpi_ingr_attribute_cpy cpy1,
           fnd_supplier sup,
           fnd_item itm,
    ...
    where  cpy1.supplier_no   = sup.supplier_no and
          ...
          (sys_source_batch_id, sys_source_sequence_no,
	        ...
	        parent_type, attribute_id, attribute_name, attribute_code, attribute_value
             ) in
          (select /*+ PARALLEL(8) */ sys_source_batch_id, sys_source_sequence_no,
           ...
           from (select /*+ PARALLEL(tmp,8) */ tmp.*,
                 ...
                 as rank
                 from stg_fpi_ingr_attribute_cpy tmp                                                             -- STG TABLE NAME CHANGE 
           )
           where rank = 1)
   ...
   on    (
            ...
            fnd.parent_type 	  =	mer_rec.parent_type    and
            fnd.attribute_id 	  =	mer_rec.attribute_id   and
            fnd.attribute_name 	  =	mer_rec.attribute_name and
...
--    remove_duplicates;
    ...
    select count(*)
    into   g_recs_read
    from   stg_fpi_ingr_attribute_cpy                                                                           -- STG TABLE NAME CHANGE 
...
--**************************************************************************************************
  ...
  exception
      ...
      when others then
       ...
       p_success := false;
       RAISE;
end wh_fnd_fpi_024u                                                                                             -- STORE PROC CHANGE 
;

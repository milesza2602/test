--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_023U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_023U" 
                                                                                (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        June 2012
--  Author:      A Joshua
--  Purpose:     Load Assort Original Plan hospital data from staging.
--  Tables:      Input  - stg_ast_geo_grd_sc_opln_wk_cpy
--               Output - stg_ast_geo_grd_sc_opln_wk_hsp
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_023U';
...
begin
   ...
   l_text := 'LOAD OF STG_AST_GEO_GRD_SC_OPLN_WK_HSP STARTED AT '|| to_char(sysdate,('dd mon yyyy hh24:mi:ss'));
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
...
--**************************************************************************************************
   ...
   where calendar_date = (select to_date(sysdate, 'dd mon yy') from dual);
      ...
      insert /*+ APPEND  */ into stg_ast_geo_grd_sc_opln_wk_hsp ast
      select /*+ full (opln) parallel (opln,2) */
             opln.sys_source_batch_id,
             opln.sys_source_sequence_no,
             g_date,
             'Y',
             'DWH',
             opln.sys_middleware_batch_id,
      ...
      from   stg_ast_geo_grd_sc_opln_wk_cpy opln,
             dim_calendar_wk cal
      where  opln.fin_year_no      = cal.fin_year_no

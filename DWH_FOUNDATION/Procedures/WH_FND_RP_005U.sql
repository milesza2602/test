--------------------------------------------------------
--  DDL for Procedure WH_FND_RP_005U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_RP_005U" 
...
--  Purpose:     Update ITEM dimension table with season indicator measures in the foundation layer
--               with input ex staging table from RP.
--  Tables:      AIT load - stg_mdm_item
--               Input    - stg_mdm_item_cpy
--               Output   - fnd_item
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_mdm_item_hsp.sys_process_msg%type;
g_rec_out            fnd_item%rowtype;
g_rec_in             stg_mdm_item_cpy%rowtype;
g_found              boolean;
g_valid              boolean;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_RP_005U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'UPDATE THE ITEM MASTERDATA EX RP';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_mdm_item_cpy%rowtype;
...
type staging_array1 is table of stg_mdm_item_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_mdm_item_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_mdm_item is
   select *
   from stg_mdm_item_cpy
...
begin
   ...
   insert into stg_mdm_item_hsp values g_rec_in;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_item
       set    rpl_merch_season_no     = a_tbl_update(i).rpl_merch_season_no,
...
begin
    ...
    l_text := 'LOAD OF FND_ITEM EX RP STARTED AT '||
...
--**************************************************************************************************
    open c_stg_mdm_item;
    fetch c_stg_mdm_item bulk collect into a_stg_input limit g_forall_limit;

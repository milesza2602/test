--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_169U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_169U" (p_forall_limit in integer,p_success out boolean) as
--**************************************************************************************************
--  Date:        February 2013
--  Author:      Quentin Smit
--  Purpose:     Create FND_JDAFF_ST_PLAN_ANALYSIS_WK table in the foundation layer
--               with input ex staging table from JDA.
--  Tables:      Input  - stg_jda_st_plan_anlysis_wk_cpy
--               Output - dwh_foundation.FND_JDAFF_ST_PLAN_ANALYSIS_WK
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_169U';
...
begin
   ...
   l_text := 'LOAD OF FND_JDAFF_ST_PLAN_ANALYSIS_WK STARTED AT '||
...
--**************************************************************************************************
   ...
   FROM stg_jda_st_plan_anlysis_wk_cpy;
   ...
   l_text := 'Running GATHER_TABLE_STATS ON FND_JDAFF_ST_PLAN_ANALYSIS_WK - Needed due to the truncate that preceeded this job';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   --DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'FND_JDAFF_ST_PLAN_ANALYSIS_WK', DEGREE => 32);

   l_text := 'First GATHER_TABLE_STATS ON FND_JDAFF_ST_PLAN_ANALYSIS_WK completed';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   l_text := 'Processing..';
   dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);

   INSERT /*+ parallel(jdast,4) */
     INTO  dwh_foundation.FND_JDAFF_ST_PLAN_ANALYSIS_WK jdast
   SELECT /*+ full(stpln) parallel(stpln,4) */
     ...
     FROM   dwh_foundation.stg_jda_st_plan_anlysis_wk_cpy stpln,
            dwh_foundation.fnd_location fl,
            dwh_foundation.fnd_item fi
    WHERE stpln.item_no     = fi.item_no 
      AND stpln.location_no = fl.location_no
   --   and stpln.sys_process_code = 'N'
      ...
      l_text := 'Running GATHER_TABLE_STATS ON FND_JDAFF_ST_PLAN_ANALYSIS_WK';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
      DBMS_STATS.gather_table_stats ('DWH_FOUNDATION', 'FND_JDAFF_ST_PLAN_ANALYSIS_WK', DEGREE => 32);

      l_text := 'Second GATHER_TABLE_STATS ON FND_JDAFF_ST_PLAN_ANALYSIS_WK completed';
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
    ...
    --execute immediate 'DROP INDEX "DWH_FOUNDATION"."BS_S_JDF_ST_PLN_ANL_DWC"';
    ...
    --execute immediate 'CREATE INDEX "DWH_FOUNDATION"."BS_S_JDF_ST_PLN_ANL_DWC" ON "DWH_FOUNDATION"."STG_JDA_ST_PLAN_ANLYSIS_WK_CPY" ("SYS_PROCESS_CODE")
    --                  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS PARALLEL 8 NOLOGGING
    --                  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
    --                  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
    --                  TABLESPACE "FND_MASTER"' ;

    --execute immediate 'alter index "DWH_FOUNDATION"."BS_S_JDF_ST_PLN_ANL_DWC" NOPARALLEL LOGGING';

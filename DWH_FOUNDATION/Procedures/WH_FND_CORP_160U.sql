--------------------------------------------------------
--  DDL for Procedure WH_FND_CORP_160U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_CORP_160U" (p_forall_limit in integer,p_success out boolean) as

--**************************************************************************************************
--  Date:        February 2013
--  Author:      Quentin Smit
--  Purpose:     Create intactix loc item dimention table in the foundation layer
--               with input ex staging table from JDAFF.
--  Tables:      Input  - stg_jdaff_location
--               Output - fnd_location
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_jdaff_location_hsp.sys_process_msg%type;
g_rec_out            fnd_location%rowtype;
g_rec_in             stg_jdaff_location%rowtype;
g_found              boolean;
g_valid              boolean;
g_count              integer       :=  0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_CORP_160U';
l_name               sys_dwh_log.log_name%type                 := dwh_constants.vc_log_name_rtl_md;
l_system_name        sys_dwh_log.log_system_name%type          := dwh_constants.vc_log_system_name_rtl_fnd;
l_script_name        sys_dwh_log.log_script_name%type          := dwh_constants.vc_log_script_rtl_fnd_md;
l_procedure_name     sys_dwh_log.log_procedure_name%type       := l_module_name;
l_text               sys_dwh_log.log_text%type ;
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD SOME LOCATION MASTERDATA MEASURES EX JDAFF';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_jdaff_location%rowtype;
...
type staging_array1 is table of stg_jdaff_location.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_jdaff_location.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_jdaff_location is
   select *
    from stg_jdaff_location_cpy
...
begin
   ...
   insert into stg_jdaff_location_hsp values g_rec_in;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).wh_zone_no||
...
--**************************************************************************************************
    open c_stg_jdaff_location;
    fetch c_stg_jdaff_location bulk collect into a_stg_input limit g_forall_limit;

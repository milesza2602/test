--------------------------------------------------------
--  DDL for Procedure WH_FND_AST_030U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_AST_030U" 
...
--               Output - fnd_ast_chain_sc_plan_wk_pln
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_AST_030U';
...
begin
   ...
   l_text := 'LOAD OF FND_AST_CHAIN_SC_PLAN_WK_PLN STARTED AT '||
...
--    last 6 weeks
      ...
      execute immediate 'truncate table dwh_foundation.fnd_ast_chain_sc_plan_wk_pln';

      execute immediate 'alter session enable parallel dml';

      insert /*+ APPEND parallel (x,4) */ into fnd_ast_chain_sc_plan_wk_pln x
      select /*+ full (ast) parallel (ast,4) */
             ast.style_colour_no,
             ast.chain_no,
             ast.fin_year_no,
             ast.fin_week_no,
      ...
      where  ast.fin_week_no  = dim.fin_week_no
       ...
       and   ast.plan_type_no      in (60,61,62);

--------------------------------------------------------
--  DDL for Procedure WH_FND_MC_608U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_FOUNDATION"."WH_FND_MC_608U" (p_forall_limit in integer,p_success out boolean) as
...
--  Tables:      Input  - stg_rms_MC_rtl_purch_order_cpy
--               Output - fnd_MC_rtl_purchase_order
--  Packages:    constants, dwh_log, dwh_valid
--
--  Maintenance:
--  11 dec 2012 - qc4807 - Adding an additional measure to RMS PO interface stg_rms_MC_rtl_purch_order.
...
--                           -- This is at po level.
--  FEB 2018 - FOR mULTIcURRENCY opr AND locaL VALUES FOR AFRICA ONLY
--  Naming conventions
--  g_  -  Global variable
...
g_count              number        :=  0;
g_hospital           char(1)       := 'N';
g_hospital_text      stg_rms_MC_rtl_purch_order_hsp.sys_process_msg%type;
g_rec_out            fnd_MC_rtl_purchase_order%rowtype;
g_sys_bat            number        := 0;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_MC_608U';
...
type tbl_array_i is table of fnd_MC_rtl_purchase_order%rowtype index by binary_integer;
type tbl_array_u is table of fnd_MC_rtl_purchase_order%rowtype index by binary_integer;
...
type staging_array1 is table of stg_rms_MC_rtl_purch_order_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_rms_MC_rtl_purch_order_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_rms_MC_rtl_purch_order is
   select a.*, b.po_no rej_rec
   from dwh_foundation.stg_rms_MC_rtl_purch_order_cpy a, dwh_foundation.TEMP_DEL_PURCHASE_ORDER b
   WHERE 
   SYS_PROCESS_CODE = 'N'
   and
   a.po_no = b.po_no(+)
...
g_rec_in             c_stg_rms_MC_rtl_purch_order%rowtype;
-- For input bulk collect --
type stg_array is table of c_stg_rms_MC_rtl_purch_order%rowtype;
...
-- qc 5028
   ...
   g_rec_out.random_mass_unit_cost_local   := g_rec_in.random_mass_unit_cost_local;
   g_rec_out.random_mass_unit_cost_opr     := g_rec_in.random_mass_unit_cost_opr;
   ...
   g_rec_out.import_unit_cost_local        := g_rec_in.import_unit_cost_local;
   g_rec_out.import_unit_cost_opr          := g_rec_in.import_unit_cost_opr;
...
begin
   ...
   insert into stg_rms_MC_rtl_purch_order_hsp values 
...
, g_rec_in.CHAIN_CODE
, g_rec_in.reg_rsp_local
, g_rec_in.reg_rsp_opr
, g_rec_in.cost_price_local
, g_rec_in.cost_price_opr
, g_rec_in.RANDOM_MASS_UNIT_COST_local
, g_rec_in.RANDOM_MASS_UNIT_COST_opr
, g_rec_in.import_unit_cost_local
, g_rec_in.import_unit_cost_opr);
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_MC_rtl_purchase_order values 
...
, a_tbl_insert(i).reg_rsp_local
, a_tbl_insert(i).reg_rsp_opr
, a_tbl_insert(i).cost_price_local
, a_tbl_insert(i).cost_price_opr
, a_tbl_insert(i).RANDOM_MASS_UNIT_COST_local
, a_tbl_insert(i).RANDOM_MASS_UNIT_COST_opr
, a_tbl_insert(i).import_unit_cost_local
, a_tbl_insert(i).import_unit_cost_opr);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_MC_rtl_purchase_order
       set    into_store_date                 = a_tbl_update(i).into_store_date,
              ...
              import_unit_cost_local = a_tbl_update(i).import_unit_cost_local,
              import_unit_cost_opr   = a_tbl_update(i).import_unit_cost_opr,
              random_mass_unit_cost_local     = a_tbl_update(i).random_mass_unit_cost_local,
              random_mass_unit_cost_opr       = a_tbl_update(i).random_mass_unit_cost_opr
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_rms_MC_rtl_purch_order_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_MC_rtl_purchase_order
...
begin
    ...
    l_text := 'LOAD OF fnd_MC_rtl_purchase_order EX OM STARTED '||
...
--    from dwh_foundation.stg_rms_MC_rtl_purch_order_cpy
--    ;
...
--**************************************************************************************************
    ...
    with selstg as (select distinct po_no, chain_code 
         ...
         selfnd as (select distinct a.po_no, a.chain_code 
                    from fnd_MC_rtl_purchase_order a, selstg b
...
--**************************************************************************************************
    open c_stg_rms_MC_rtl_purch_order;
    fetch c_stg_rms_MC_rtl_purch_order bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_rms_MC_rtl_purch_order;

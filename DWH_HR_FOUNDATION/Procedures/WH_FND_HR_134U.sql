--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_134U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_134U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        June 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create HR POI Type dimension table in the foundation layer
--  Tables:      Input  - stg_ps_poi_type_cpy
--               Output - fnd_hr_poi_type
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_poi_type_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_poi_type%rowtype;
g_rec_in             stg_ps_poi_type_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_134U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR POI TYPE DATA EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_poi_type_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_poi_type%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_poi_type%rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_poi_type_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_poi_type_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_ps_poi_type is
   select *
   from stg_ps_poi_type_cpy
...
begin
   ...
   g_rec_out.poi_type_desc            := g_rec_in.poi_type_desc ;
...
begin
   ...
   insert into stg_ps_poi_type_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_poi_type values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).poi_type ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_poi_type
       set    poi_type_desc       = a_tbl_update(i).poi_type_desc,
              last_updated_date   = a_tbl_update(i).last_updated_date
       where  poi_type            = a_tbl_update(i).poi_type  ;
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).poi_type ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
   g_found := dwh_hr_valid.fnd_hr_poi_type (g_rec_out.poi_type );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).poi_type  = g_rec_out.poi_type  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_poi_type EX hr LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_poi_type;
    fetch c_stg_ps_poi_type bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_316U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_316U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        June 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create fnd_hr_bee_POI_skill_dev_learn in the foundation layer
--  Tables:      Input  - stg_ps_skill_dev_learn
--               Output - fnd_hr_bee_POI_skill_dev_learn
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_skill_dev_learn_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_bee_poi_skill_dev_learn%rowtype;
g_rec_in             stg_ps_skill_dev_learn_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_316U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR POI SKILL DEV LEARN EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_skill_dev_learn_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_bee_poi_skill_dev_learn%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_bee_poi_skill_dev_learn%rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_skill_dev_learn_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_skill_dev_learn_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_ps_skill_dev_learn is
   select *
   from DWH_HR_FOUNDATION.stg_ps_skill_dev_learn_cpy
   where sys_process_code = 'N'
   and employee_id in
      (select poi_id from dwh_hr_performance.dim_hr_poi)
...
begin
   ...
   g_hospital                                 := 'N';
   g_rec_out.poi_id                          := g_rec_in.employee_id;
   g_rec_out.training_program_code           := g_rec_in.training_program_code;
   ...
   g_rec_out.learner_status_code             := nvl(g_rec_in.learner_status_code,0);
   g_rec_out.absorption_ind                  := g_rec_in.absorption_ind;
   g_rec_out.absorption_date                 := g_rec_in.absorption_date;
   g_rec_out.absorption_type                 :=  nvl(g_rec_in.absorption_type,0);

   if not dwh_hr_valid.fnd_hr_training_program(g_rec_out.training_program_code) then
     g_hospital      := 'Y';
     g_hospital_text := 'TRAINING PROGRAM NOT FOUND';
     l_text          := 'TRAINING PROGRAM NOT FOUND'||g_rec_out.training_program_code ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;

   if not dwh_hr_valid.fnd_hr_poi(g_rec_out.poi_id) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_hr_constants.vc_poi_not_found;
     l_text          := dwh_hr_constants.vc_poi_not_found||g_rec_out.poi_id ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;
...
begin
   ...
   insert into stg_ps_skill_dev_learn_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_bee_poi_skill_dev_learn values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_bee_poi_skill_dev_learn
       set    actual_completion_date          = a_tbl_update(i).actual_completion_date,
              ...
              learner_status_code             = a_tbl_update(i).learner_status_code,
              absorption_ind                  = a_tbl_update(i).absorption_ind,
              absorption_date                 = a_tbl_update(i).absorption_date,
              absorption_type                 = a_tbl_update(i).absorption_type
       where  poi_id                     = a_tbl_update(i).poi_id  and
              training_program_code           = a_tbl_update(i).training_program_code and
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_ps_skill_dev_learn_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_hr_bee_poi_skill_dev_learn
   where  poi_id               = g_rec_out.poi_id  and
          training_program_code     = g_rec_out.training_program_code   and
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).poi_id            = g_rec_out.poi_id  and
            a_tbl_insert(i).training_program_code  = g_rec_out.training_program_code  and
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_bee_poi_skill_dev_learn EX hr STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_skill_dev_learn;
    fetch c_stg_ps_skill_dev_learn bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_ps_skill_dev_learn;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_410U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_410U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        Jan 2015
--  Author:      Kgomotso Lehabe
--  Purpose:     Create SKILLS DEV SPEND MODELLING targets in the foundation layer
--               with input ex staging table from Sharepoint.
--  Tables:      Input  - stg_excel_skills_dev_spend_cpy
--               Output - fnd_hr_mdl_skills_dev_spend
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_skills_dev_spend_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_mdl_skills_dev_spend%rowtype;
g_rec_in             stg_excel_skills_dev_spend_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_410U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE SKILLS DEV SPEND  Targets EX SHAREPOINT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_skills_dev_spend_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_mdl_skills_dev_spend%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_mdl_skills_dev_spend%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_skills_dev_spend_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_skills_dev_spend_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_skills_dev_spend is
   select *
   from stg_excel_skills_dev_spend_cpy
...
begin
       ...
       g_rec_out.total_spend                    := g_rec_in.total_spend;
    ...
    if not dwh_hr_valid.fnd_hr_bee_business_unit (g_rec_out.bee_business_unit_code) then
     ...
     g_hospital      := 'Y';
     g_hospital_text := 'INVALID BEE CATEGORY';
     l_text          := 'INVALID BEE CATEGORY' ||g_rec_out.bee_category_code ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         return;
   end if;
...
begin
   ...
   insert into stg_excel_skills_dev_spend_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_mdl_skills_dev_spend values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_mdl_skills_dev_spend
       set  total_spend                 =  a_tbl_update(i).total_spend,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_excel_skills_dev_spend_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_hr_mdl_skills_dev_spend
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_mdl_skills_dev_spend EX Excel STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_skills_dev_spend;
    fetch c_stg_excel_skills_dev_spend bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_excel_skills_dev_spend;
--**************************************************************************************************

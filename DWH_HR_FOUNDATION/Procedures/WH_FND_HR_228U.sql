--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_228U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_228U" (p_forall_limit in integer,p_success out boolean) AS

--**************************************************************************************************
--  Date:        Jul 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create Supplier contracts dimension in the foundation layer
--
--  Tables:      Input  - stg_of_contract_cpy
--               Output - fnd_hr_contract
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_of_contract_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_contract%rowtype;
g_rec_in             stg_of_contract_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_228U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE OF CONTRACTS EX ORACLE FINANCE';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_of_contract_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_contract%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_contract%rowtype index by binary_integer;
...
type staging_array1 is table of stg_of_contract_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_of_contract_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_of_contract is
   select *
   from DWH_HR_FOUNDATION.stg_of_contract_cpy
...
begin
   ...
   g_rec_out.contract_id          := g_rec_in.contract_id ;
   g_rec_out.contract_name        := g_rec_in.contract_name;
   g_rec_out.contract_no          := g_rec_in.contract_no ;
   g_rec_out.contract_status      := g_rec_in.contract_status ;
   g_rec_out.contract_start_date  := g_rec_in.contract_start_date ;
...
begin
   ...
   insert into stg_of_contract_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_contract values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).contract_id ||
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       ...
       set
           ...
           contract_id          = a_tbl_update(i).contract_id ,
           contract_name        = a_tbl_update(i).contract_name,
           contract_no          = a_tbl_update(i).contract_no ,
           contract_status      = a_tbl_update(i).contract_status ,
           contract_start_date  = a_tbl_update(i).contract_start_date ,
           contract_end_date    = a_tbl_update(i).contract_end_date,
           last_updated_date    =a_tbl_update(i).last_updated_date
        where  contract_id      = a_tbl_update(i).contract_id
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ' '||g_error_index||
                       ' '||sqlerrm(-sql%bulk_exceptions(i).error_code)||
                       ' '||a_tbl_update(g_error_index).contract_id  ||
                       ' '||a_tbl_update(g_error_index).supplier_id  ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   dwh_hr_foundation.fnd_hr_contract
    where  supplier_id      = g_rec_out.supplier_id  and
           contract_id       = g_rec_out.contract_id;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_contract EX hr STARTED AT '||
...
--**************************************************************************************************
    open c_stg_of_contract;
    fetch c_stg_of_contract bulk collect into a_stg_input limit g_forall_limit;

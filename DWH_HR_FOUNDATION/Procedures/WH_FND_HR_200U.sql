--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_200U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_200U" (p_forall_limit in integer,p_success out boolean) AS
...
--  Tables:      Input  - stg_excel_bee_bu_cpy
--               Output - fnd_hr_bee_bu
...
--     Purpose     :Add bee_parent_business_unit_code to fnd_hr_bee_bu
--
--  Naming conventions
--  g_  -  Global variable
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_bee_bu_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_bee_bu%rowtype;
g_rec_in             stg_excel_bee_bu_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_200U';
...
type stg_array is table of stg_excel_bee_bu_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_bee_bu%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_bee_bu%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_bee_bu_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_bee_bu_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_bee_bu is
   select *
   from stg_excel_bee_bu_cpy
...
begin
   ...
   insert into stg_excel_bee_bu_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_bee_bu values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_bee_bu
       set    bee_business_unit_desc        = a_tbl_update(i).bee_business_unit_desc,
              last_updated_date             = a_tbl_update(i).last_updated_date,
              bee_parent_business_unit_code = a_tbl_update(i).bee_parent_business_unit_code
...
begin
   g_found := dwh_hr_valid.fnd_hr_bee_business_unit (g_rec_out.bee_business_unit_code );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).bee_business_unit_code  = g_rec_out.bee_business_unit_code  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_bee_bu EX hr LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_bee_bu;
    fetch c_stg_excel_bee_bu bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_314U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_314U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        June 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create HR EAP TARGET table in the foundation layer
--  Tables:      Input  - stg_ps_eap_target_cpy
--               Output - fnd_hr_eap_target
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_eap_target_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_eap_target%rowtype;
g_rec_in             stg_ps_eap_target_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_314U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR EAP TARGET EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_eap_target_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_eap_target%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_eap_target%rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_eap_target_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_eap_target_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_ps_eap_target is
   select *
   from stg_ps_eap_target_cpy
...
begin
   ...
   if not dwh_hr_valid.fnd_hr_race(g_rec_out.race_code) then
    ...
    if not dwh_valid.indicator_field(g_rec_out.disabled_ind) then
     ...
     l_text          := dwh_hr_constants.vc_invalid_indicator||g_rec_out.disabled_ind ;
     return;
   end if;
...
begin
   ...
   insert into stg_ps_eap_target_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_eap_target values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_eap_target
       set    percentage                = a_tbl_update(i).percentage,
             effective_date             = a_tbl_update(i).effective_date,
              last_updated_date         = g_date
      where  occupational_level_code    = a_tbl_update(i).occupational_level_code  and
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_hr_eap_target
   where occupational_level_code 	  = g_rec_out.occupational_level_code and
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).occupational_level_code   = g_rec_out.occupational_level_code  and
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_eap_target EX hr STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_eap_target;
    fetch c_stg_ps_eap_target bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_126U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_126U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        NOV. 2011
--  Author:      Alastair de Wet
--  Purpose:     Create HR org_hierachy dimention table in the foundation layer
--               with input ex staging table from HR.
--  Tables:      Input  - stg_ps_org_hierachy_cpy
--               Output - fnd_hr_org_hierachy
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_org_hierachy_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_org_hierachy%rowtype;
g_rec_in             stg_ps_org_hierachy_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_126U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR org_hierachy EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_org_hierachy_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_org_hierachy%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_org_hierachy%rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_org_hierachy_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_org_hierachy_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_ps_org_hierachy is
   select *
   from stg_ps_org_hierachy_cpy
...
begin
   ...
   g_rec_out.level_1_code                := g_rec_in.level_1_code ;
   g_rec_out.level_2_code                := g_rec_in.level_2_code ;
   g_rec_out.level_3_code                := g_rec_in.level_3_code ;
   g_rec_out.level_4_code                := g_rec_in.level_4_code ;
   g_rec_out.level_5_code                := g_rec_in.level_5_code ;
   g_rec_out.level_6_code                := g_rec_in.level_6_code ;
...
begin
   ...
   insert into stg_ps_org_hierachy_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_org_hierachy values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_org_hierachy
       set    level_1_code       = a_tbl_update(i).level_1_code,
              level_2_code       = a_tbl_update(i).level_2_code,
              level_3_code       = a_tbl_update(i).level_3_code,
              level_4_code       = a_tbl_update(i).level_4_code,
              level_5_code       = a_tbl_update(i).level_5_code,
              level_6_code       = a_tbl_update(i).level_6_code,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_ps_org_hierachy_cpy
...
begin
   g_found := dwh_hr_valid.fnd_hr_org_hierachy(g_rec_out.department_code,g_rec_out.company_code);
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).department_code  = g_rec_out.department_code and
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_org_hierachy EX hr STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_org_hierachy;
    fetch c_stg_ps_org_hierachy bulk collect into a_stg_input limit g_forall_limit;

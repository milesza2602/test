--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_323U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_323U" 
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_323U';
...
cursor c_fnd_hr_bee_sed is
   ...
   from   dwh_hr_foundation.fnd_hr_ap_invoice ai,
          dwh_hr_performance.dim_hr_bee_supplier ds
            left join dwh_hr_performance.dim_hr_contract ct
...
begin
  ...
  if upper(g_rec_in.verification_status) = 'VERIFIED'  and g_rec_in.empowering_num=1 and  g_rec_in.local_import  <> 'I' then
     ...
     if  g_rec_in.supplier_take_on_fin_yr  = g_rec_in.fin_year_no then
          g_rec_out.enhanced_recognition_1st_time :=   g_rec_out.base_recognition  * 1.2;
     end if;

      g_rec_out.recognition_all    :=  g_rec_out.base_recognition +
                                     (0.2 * g_sd_beneficiary) +
                                     (0.2 * g_procuremnt_contract) +
                                     (0.2 * g_first_time_supplier);

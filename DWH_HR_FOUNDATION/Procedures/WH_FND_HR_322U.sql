--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_322U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_322U" 
...
--  Date:        DEC. 2011
--  Author:      Wendy Lyttle
--  Purpose:     Create HR invoice_PAYMENT fact table in the foundation layer
--               with input ex staging table from Oracle Financials.
--  Tables:      Input  - stg_of_ap_invoice_payment_cpy
--               Output - fnd_hr_ap_invoice_payment
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_of_ap_invoice_payment_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_ap_invoice_payment%rowtype;
g_rec_in             stg_of_ap_invoice_payment_cpy%rowtype;
g_found              boolean;
g_count              number;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_322U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR BEE INVOICE_PAYMENT DATA EX ORACLE FINANCIALS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_of_ap_invoice_payment_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_ap_invoice_payment%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_ap_invoice_payment%rowtype index by binary_integer;
...
type staging_array1 is table of stg_of_ap_invoice_payment_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_of_ap_invoice_payment_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_of_ap_invoice_payment is
   select *
   from stg_of_ap_invoice_payment_cpy
...
BEGIN
  ...
  g_rec_out.payment_gl_period               := g_rec_in.payment_gl_period;
  ...
  g_rec_out.paid_ind                        := g_rec_in.paid_ind;
  g_rec_out.post_date                       := g_rec_in.post_date;

  g_rec_out.last_updated_date               := g_date;

  g_rec_out.fin_week_no  := 1;
  g_rec_out.fin_month_no := 1;
  g_rec_out.fin_year_no  := 2000;

  begin
  select fin_month_no,fin_week_no,fin_year_no
  into   g_rec_out.fin_month_no,g_rec_out.fin_week_no,g_rec_out.fin_year_no
  from   dim_calendar
  where  calendar_date = g_rec_out.post_date ;
  exception
...
/*
  if g_rec_out.org_id = 81 then
  if g_rec_out.payment_gl_period is not null then
  g_rec_out.fin_week_no := substr(g_rec_out.payment_gl_period,3,2);
  g_rec_out.fin_year_no := substr(g_rec_out.payment_gl_period,6,2);
  g_rec_out.fin_year_no := g_rec_out.fin_year_no + 2000;

  begin
  select fin_month_no
  into   g_rec_out.fin_month_no
  from   dim_calendar_wk
  where  fin_week_no = g_rec_out.fin_week_no and
         fin_year_no = g_rec_out.fin_year_no;
  exception
         when no_data_found then
         g_rec_out.fin_month_no := 0;
  end;
  end if;
  end if;
*/
   if not dwh_valid.indicator_field(g_rec_out.PAID_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     l_text          := dwh_constants.vc_invalid_indicator||' '||g_rec_out.PAID_ind  ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;

   if not DWH_HR_VALID.FND_HR_BEE_SUPPLIER(g_rec_out.supplier_id) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_supplier_not_found;
     l_text          := dwh_constants.vc_supplier_not_found||g_rec_out.supplier_id  ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
     return;
   end if;
...
begin
   ...
   insert into stg_of_ap_invoice_payment_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_ap_invoice_payment values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_ap_invoice_payment
       set    org_id                    = a_tbl_update(i).org_id,
              supplier_id               = a_tbl_update(i).supplier_id,
              invoice_id                = a_tbl_update(i).invoice_id,
              payment_gl_period         = a_tbl_update(i).payment_gl_period,
              ...
              paid_ind                  = a_tbl_update(i).paid_ind,
       ...
       and    PAYMENT_date              = a_tbl_update(i).PAYMENT_date
       and    supplier_id               = a_tbl_update(i).supplier_id  ;
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_of_ap_invoice_payment_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_hr_ap_invoice_payment
   where  invoice_id      = g_rec_out.invoice_id  and
          PAYMENT_date    = g_rec_out.PAYMENT_date and
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).invoice_id    = g_rec_out.invoice_id and
            a_tbl_insert(i).supplier_id   = g_rec_out.supplier_id and
            a_tbl_insert(i).payment_date  = g_rec_out.payment_date then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_ap_invoice_payment EX oracle fin STARTED AT '||
...
--**************************************************************************************************
    open c_stg_of_ap_invoice_payment;
    fetch c_stg_of_ap_invoice_payment bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_of_ap_invoice_payment;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_130U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_130U" (p_forall_limit in integer,p_success out boolean) AS
...
--  Tables:      Input  - stg_ps_bee_category_cpy
--               Output - fnd_hr_bee_category
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_bee_category_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_bee_category %rowtype;
g_rec_in             stg_ps_bee_category_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_130U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR CATEGORY DATA EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_bee_category_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_bee_category %rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_bee_category %rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_bee_category_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_bee_category_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_ps_bee_category  is
   select *
   from stg_ps_bee_category_cpy
...
begin
   ...
   g_hospital                         := 'N';
   g_rec_out.bee_category_code        := nvl(g_rec_in.bee_category_code,0);
...
begin
   ...
   insert into stg_ps_bee_category_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_bee_category  values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_bee_category
       set    bee_category_desc  = a_tbl_update(i).bee_category_desc,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_ps_bee_category_cpy
...
begin
   g_found := dwh_hr_valid.fnd_hr_bee_category  (g_rec_out.bee_category_code );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).bee_category_code  = g_rec_out.bee_category_code  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_bee_category  EX hr LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_bee_category ;
    fetch c_stg_ps_bee_category  bulk collect into a_stg_input limit g_forall_limit;

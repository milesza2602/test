--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_406U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_406U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        Jan 2015
--  Author:      Kgomotso Lehabe
--  Purpose:     Create PARENT BU EMPLOYMENT EQUITY MODELLING table in the foundation layer
--               with input ex staging table from Sharepoint.
--  Tables:      Input  - stg_excel_mgmt_ctrl_pbu_ee_cpy
--               Output - fnd_hr_mdl_mgmt_ctrl_pbu_ee
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_mgmt_ctrl_pbu_ee_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_mdl_mgmt_ctrl_pbu_ee%rowtype;
g_rec_in             stg_excel_mgmt_ctrl_pbu_ee_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_406U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE PARENT BU EMPLOYMENT EQUITY MODELLING EX SHAREPOINT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_mgmt_ctrl_pbu_ee_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_mdl_mgmt_ctrl_pbu_ee%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_mdl_mgmt_ctrl_pbu_ee%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_mgmt_ctrl_pbu_ee_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_mgmt_ctrl_pbu_ee_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_mgmt_ctrl_pbu_ee is
   select *
   from stg_excel_mgmt_ctrl_pbu_ee_cpy
...
begin
       ...
       g_rec_out.bee_parent_business_unit_code  := g_rec_in.bee_parent_business_unit_code;
       g_rec_out.occupational_level_code        := g_rec_in.occupational_level_code;
       ...
       g_rec_out.pln_ee_count                   := g_rec_in.pln_ee_count;
       g_rec_out.pln_ee_perc                    := g_rec_in.pln_ee_perc;


       g_rec_out.last_updated_date          := g_date;


      if not dwh_hr_valid.fnd_hr_bee_parent_bu (g_rec_out.bee_parent_business_unit_code ) then
      g_hospital      := 'Y';
      g_hospital_text := dwh_hr_constants.vc_invalid_bee_parent_bu;
      l_text          := dwh_hr_constants.vc_invalid_bee_parent_bu||g_rec_out.bee_parent_business_unit_code ;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;

      if not dwh_hr_valid.fnd_hr_occupation_level(g_rec_out.occupational_level_code) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_hr_constants.vc_invalid_occupational_level;
     l_text          := dwh_hr_constants.vc_invalid_occupational_level||g_rec_out.occupational_level_code ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         return;
   end if;
...
if not dwh_valid.indicator_field(g_rec_out.disabled_ind) then
   ...
   if g_rec_in.citizenship_status_code  = '1' then
       g_rec_out.citizenship_status_code    := 'C';
     end if;
    if g_rec_in.citizenship_status_code   = '0' then
      g_rec_out.citizenship_status_code    := 'E';
    end if ;
...
begin
   ...
   insert into stg_excel_mgmt_ctrl_pbu_ee_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_mdl_mgmt_ctrl_pbu_ee values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).bee_parent_business_unit_code ||
                       ' '||a_tbl_insert(g_error_index).occupational_level_code ||
                       ...
                       ' '||a_tbl_insert(g_error_index).disabled_ind ;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_mdl_mgmt_ctrl_pbu_ee
       set  pln_ee_count                    =  a_tbl_update(i).pln_ee_perc,
            pln_ee_perc                     =  a_tbl_update(i).pln_ee_perc,
           last_updated_date                = a_tbl_update(i).last_updated_date
       where  fin_year_no                   = a_tbl_update(i).fin_year_no  and
              bee_parent_business_unit_code = a_tbl_update(i).bee_parent_business_unit_code and
              occupational_level_code       = a_tbl_update(i).occupational_level_code and
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_excel_mgmt_ctrl_pbu_ee_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_hr_mdl_mgmt_ctrl_pbu_ee
   where  fin_year_no                         = g_rec_out.fin_year_no  and
              bee_parent_business_unit_code   = g_rec_out.bee_parent_business_unit_code and
              occupational_level_code         = g_rec_out.occupational_level_code and
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).fin_year_no                    = g_rec_out.fin_year_no  and
            a_tbl_insert(i).bee_parent_business_unit_code  = g_rec_out.bee_parent_business_unit_code  and
            a_tbl_insert(i).occupational_level_code        = g_rec_out.occupational_level_code and
            ...
            a_tbl_insert(i).disabled_ind                   = g_rec_out.disabled_ind then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_mdl_mgmt_ctrl_pbu_ee EX Excel STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_mgmt_ctrl_pbu_ee;
    fetch c_stg_excel_mgmt_ctrl_pbu_ee bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_excel_mgmt_ctrl_pbu_ee;
--**************************************************************************************************

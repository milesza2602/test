--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_320U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_320U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        DEC. 2011
--  Author:      Wendy Lyttle
--  Purpose:     Create HR invoice fact table in the foundation layer
--               with input ex staging table from Oracle Financials.
--  Tables:      Input  - stg_of_ap_invoice_cpy
--               Output - fnd_hr_ap_invoice
...
--  Change      :	 Add Capital_Goods_ind to fnd_hr_ap_invoice as part of the key
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_of_ap_invoice_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_ap_invoice%rowtype;
g_rec_in             stg_of_ap_invoice_cpy%rowtype;
g_found              boolean;
g_count              number;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_320U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR BEE INVOICE DATA EX ORACLE FINANCIALS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_of_ap_invoice_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_ap_invoice%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_ap_invoice%rowtype index by binary_integer;
...
type staging_array1 is table of stg_of_ap_invoice_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_of_ap_invoice_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_of_ap_invoice is
   select *
   from stg_of_ap_invoice_cpy
...
/*
  ...
  if g_rec_out.invoice_gl_period is not null then
  g_rec_out.fin_week_no := substr(g_rec_out.invoice_gl_period,3,2);
  g_rec_out.fin_year_no := substr(g_rec_out.invoice_gl_period,6,2);
...
*/
   if not dwh_valid.indicator_field(g_rec_out.posted_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_constants.vc_invalid_indicator;
     l_text          := dwh_constants.vc_invalid_indicator||' '||g_rec_out.posted_ind  ;
...
begin
   ...
   insert into stg_of_ap_invoice_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_ap_invoice values a_tbl_insert(i);
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).invoice_id     ||
                        ''||a_tbl_insert(g_error_index).capital_goods_ind;
          dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).invoice_id  = g_rec_out.invoice_id  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_ap_invoice EX oracle fin STARTED AT '||
...
--**************************************************************************************************
    open c_stg_of_ap_invoice;
    fetch c_stg_of_ap_invoice bulk collect into a_stg_input limit g_forall_limit;

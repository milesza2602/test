--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_412U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_412U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        Jan 2015
--  Author:      Kgomotso Lehabe
--  Purpose:     Create SKILLS DEV LEARNERSHIPS MODELLING targets in the foundation layer
--               with input ex staging table from Sharepoint.
--  Tables:      Input  - stg_excel_skills_dev_learn_cpy
--               Output - fnd_hr_mdl_skills_dev_learn
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_skills_dev_learn_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_mdl_skills_dev_learn%rowtype;
g_rec_in             stg_excel_skills_dev_learn_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_412U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE SKILLS DEV LEARNERSHIPS Targets EX SHAREPOINT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_skills_dev_learn_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_mdl_skills_dev_learn%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_mdl_skills_dev_learn%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_skills_dev_learn_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_skills_dev_learn_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_skills_dev_learn is
   select *
   from stg_excel_skills_dev_learn_cpy
...
begin
       ...
       g_rec_out.learner_status_code         := g_rec_in.learner_status_code;
       ...
       g_rec_out.completed_ind               := g_rec_in.completed_ind;
       g_rec_out.absorption_ind              := g_rec_in.absorption_ind;
       g_rec_out.pln_number_of_learners      := g_rec_in.pln_number_of_learners;
    ...
    if not dwh_hr_valid.fnd_hr_bee_business_unit (g_rec_out.bee_business_unit_code) then
      ...
      l_text          := dwh_hr_constants.vc_invalid_bee_bu||g_rec_out.bee_business_unit_code;
      dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
   end if;

      if not dwh_hr_valid.fnd_hr_learner_status(g_rec_out.learner_status_code) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_hr_constants.vc_invalid_learner_status;
     l_text          := dwh_hr_constants.vc_invalid_learner_status ||g_rec_out.learner_status_code ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         return;
   end if;
...
if not dwh_valid.indicator_field(g_rec_out.completed_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_hr_constants.vc_invalid_indicator ||'COMPLETED IND:' ||g_rec_out.completed_ind;
     return;
   end if;

   if not dwh_valid.indicator_field(g_rec_out.absorption_ind) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_hr_constants.vc_invalid_indicator || 'ABSORPTION IND: ' || g_rec_out.absorption_ind;
     return;
   end if;
...
begin
   ...
   insert into stg_excel_skills_dev_learn_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_mdl_skills_dev_learn values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).bee_business_unit_code ||
                       ' '||a_tbl_insert(g_error_index).learner_status_code ||
                       ...
                       ' '||a_tbl_insert(g_error_index).completed_ind ||
                       ' '||a_tbl_insert(g_error_index).absorption_ind ;
             dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_mdl_skills_dev_learn
       set  pln_number_of_learners    =  a_tbl_update(i).pln_number_of_learners,
           last_updated_date          = a_tbl_update(i).last_updated_date
       where  fin_year_no             = a_tbl_update(i).fin_year_no  and
              bee_business_unit_code  = a_tbl_update(i).bee_business_unit_code and
              learner_status_code     = a_tbl_update(i).learner_status_code and
              race_code               = a_tbl_update(i).race_code and
              gender_code             = a_tbl_update(i).gender_code and
              citizenship_status_code = a_tbl_update(i).citizenship_status_code and
              completed_ind           = a_tbl_update(i).completed_ind and
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_excel_skills_dev_learn_cpy
...
-- Check to see if item is present on table and update/insert accordingly
   select count(1)
   into   g_count
   from   fnd_hr_mdl_skills_dev_learn
   where  fin_year_no                 = g_rec_out.fin_year_no  and
              bee_business_unit_code  = g_rec_out.bee_business_unit_code and
              learner_status_code       = g_rec_out.learner_status_code and
              race_code               = g_rec_out.race_code and
              gender_code             = g_rec_out.gender_code and
              citizenship_status_code = g_rec_out.citizenship_status_code and
              completed_ind           = g_rec_out.completed_ind and
              absorption_ind           = g_rec_out.absorption_ind;
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).fin_year_no             = g_rec_out.fin_year_no  and
             a_tbl_insert(i).bee_business_unit_code = g_rec_out.bee_business_unit_code and
            a_tbl_insert(i).learner_status_code       = g_rec_out.learner_status_code and
            a_tbl_insert(i).race_code               = g_rec_out.race_code  and
            a_tbl_insert(i).gender_code             = g_rec_out.gender_code  and
            a_tbl_insert(i).citizenship_status_code = g_rec_out.citizenship_status_code and
            a_tbl_insert(i).completed_ind           = g_rec_out.completed_ind and
            a_tbl_insert(i).absorption_ind          = g_rec_out.absorption_ind then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_mdl_skills_dev_learn EX Excel STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_skills_dev_learn;
    fetch c_stg_excel_skills_dev_learn bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_excel_skills_dev_learn;
--**************************************************************************************************

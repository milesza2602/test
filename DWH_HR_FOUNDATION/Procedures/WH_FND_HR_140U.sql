--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_140U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_140U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        June 2014
--  Author:      Kgomotso Lehabe
--  Purpose:     Create HR POI dimension table in the foundation layer
--  Tables:      Input  - stg_ps_poi_cpy
--               Output - fnd_hr_poi
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_poi_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_poi%rowtype;
g_rec_in             stg_ps_poi_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_140U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR POI DATA EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_poi_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_poi%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_poi%rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_poi_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_poi_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
begin
   ...
   g_rec_out.poi_type      	             :=nvl(g_rec_in.poi_type, '00000');
 ...
 if not dwh_hr_valid.fnd_hr_gender(g_rec_out.gender_code) then
   ...
   if not dwh_hr_valid.fnd_hr_citizenship_status(g_rec_out.citizenship_status_code) then
    ...
    if not dwh_hr_valid.fnd_hr_poi_type(g_rec_out.poi_type) then
     g_hospital      := 'Y';
     g_hospital_text := dwh_hr_constants.vc_invalid_poi_type ;
     l_text          := dwh_hr_constants.vc_invalid_poi_type ||g_rec_out.poi_type ;
     dwh_log.write_log(l_name,l_system_name,l_script_name,l_procedure_name,l_text);
         return;
   end if;
...
begin
   ...
   insert into stg_ps_poi_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_poi values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_poi
       set id_no 	                 = a_tbl_update(i).id_no,
...
begin
   g_found := dwh_hr_valid.fnd_hr_poi(g_rec_out.poi_id );
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).poi_id  = g_rec_out.poi_id  then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_poi EX hr LIAISON STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_poi;
    fetch c_stg_ps_poi bulk collect into a_stg_input limit g_forall_limit;

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_344U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_344U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        NOV. 2011
--  Author:      Alastair de Wet
--  Purpose:     Create OF npat forecast table in the foundation layer
--               with input ex staging table from excel.
--  Tables:      Input  - stg_excel_npat_forecast_cpy
--               Output - fnd_hr_bee_npat_forecast
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_npat_forecast_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_bee_npat_forecast%rowtype;
g_rec_in             stg_excel_npat_forecast_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_344U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE EXCEL NPAT FORECAST FACT EX OF';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_npat_forecast_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_bee_npat_forecast%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_bee_npat_forecast%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_npat_forecast_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_npat_forecast_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_npat_forecast is
   select *
   from stg_excel_npat_forecast_cpy
...
begin
   ...
   insert into stg_excel_npat_forecast_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_bee_npat_forecast values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_bee_npat_forecast
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_excel_npat_forecast_cpy
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_bee_npat_forecast EX hr STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_npat_forecast;
    fetch c_stg_excel_npat_forecast bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_excel_npat_forecast;
--**************************************************************************************************

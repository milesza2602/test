--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_416U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_416U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        Jan 2015
--  Author:      Kgomotso Lehabe
--  Purpose:     Create ENTERPRISE AND SUPPLIER DEVELOPMENT MODELLING targets in the foundation layer
--               with input ex staging table from Sharepoint.
--  Tables:      Input  - stg_excel_ed_sd_cpy
--               Output - fnd_hr_mdl_ed_sd
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_ed_sd_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_mdl_ed_sd%rowtype;
g_rec_in             stg_excel_ed_sd_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_416U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE  ENTERPRISE AND SUPPLIER DEVELOPMENT  Targets EX SHAREPOINT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_excel_ed_sd_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_mdl_ed_sd%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_mdl_ed_sd%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_ed_sd_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_ed_sd_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_ed_sd is
   select *
   from stg_excel_ed_sd_cpy
...
begin
       ...
       g_rec_out.pln_contribution_amount := g_rec_in.pln_contribution_amount;
...
begin
   ...
   insert into stg_excel_ed_sd_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_mdl_ed_sd values a_tbl_insert(i);
      ...
      when others then
       ...
       for i in 1 .. g_error_count loop
          ...
          l_message := dwh_hr_constants.vc_err_lb_loop||i||
                       ...
                       ' '||a_tbl_insert(g_error_index).benefit_factor_id ||
                       ' '||a_tbl_insert(g_error_index).ed_sd_indicator;
             dwh_log.record_error(l_module_name,sqlcode,l_message);
       end loop;
       raise;
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_mdl_ed_sd
       set  pln_contribution_amount   =  a_tbl_update(i).pln_contribution_amount,
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).fin_year_no             = g_rec_out.fin_year_no  and
            ...
            a_tbl_insert(i).ed_sd_indicator          =g_rec_out.ed_sd_indicator then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_mdl_ed_sd EX Excel STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_ed_sd;
    fetch c_stg_excel_ed_sd bulk collect into a_stg_input limit g_forall_limit;

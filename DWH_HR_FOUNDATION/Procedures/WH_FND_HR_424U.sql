--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_424U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_424U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        Jan 2015
--  Author:      Kgomotso Lehabe
--  Purpose:     Create COMPANY MEASURES MODELLING targets in the foundation layer
--               with input ex staging table from Sharepoint.
--  Tables:      Input  - stg_excel_company_pln_cpy
--               Output - fnd_hr_mdl_company_pln
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_excel_company_pln_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_mdl_company_pln%rowtype;
--g_rec_in             stg_excel_company_pln_cpy%rowtype;
g_found              boolean;
g_count              number        :=  0;
g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_424U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE COMPANY MEASURES MODELLING Targets EX SHAREPOINT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
--type stg_array is table of stg_excel_company_pln_cpy%rowtype;
--a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_mdl_company_pln%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_mdl_company_pln%rowtype index by binary_integer;
...
type staging_array1 is table of stg_excel_company_pln_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_excel_company_pln_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_excel_company_pln is
   select ownshp.sys_source_batch_id,
       ownshp.sys_source_sequence_no,
       ownshp.sys_load_date,
       ownshp.sys_process_code,
       ownshp.sys_load_system_name,
       ownshp.sys_middleware_batch_id,
       ownshp.sys_process_msg,
       ownshp.fin_year_no,
       ownshp.company_code,
       ownshp.pln_share_capital,
       spnd.pln_leviable_amount,
       ownshp. pln_number_of_employees,
       ownshp.pln_number_of_shares
from
    (SELECT *
     FROM  dwh_hr_foundation.stg_excel_company_pln_cpy
     where pln_share_capital is not null) ownshp
inner join
    (SELECT *
     FROM  dwh_hr_foundation.stg_excel_company_pln_cpy
    where pln_leviable_amount is not null) spnd
on  ownshp.fin_year_no = spnd.fin_year_no
    and ownshp.company_code =spnd.company_code

order by sys_source_batch_id,sys_source_sequence_no;

-- order by only where sequencing is essential to the correct loading of data
g_rec_in                   c_stg_excel_company_pln%rowtype;
-- For input bulk collect --
type stg_array is table of c_stg_excel_company_pln%rowtype;
a_stg_input      stg_array;
...
begin
       ...
       g_rec_out.pln_share_capital          := g_rec_in.pln_share_capital;
       g_rec_out.pln_leviable_amount        := g_rec_in.pln_leviable_amount;
       g_rec_out.pln_number_of_employees    := g_rec_in.pln_number_of_employees;
       g_rec_out.pln_number_of_shares       := g_rec_in.pln_number_of_shares;
...
begin
   ...
   insert into stg_excel_company_pln_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_mdl_company_pln values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_mdl_company_pln
       set    pln_share_capital        = a_tbl_update(i).pln_share_capital,
              pln_leviable_amount      = a_tbl_update(i).pln_leviable_amount,
              pln_number_of_employees  = a_tbl_update(i).pln_number_of_employees,
               pln_number_of_shares    = a_tbl_update(i).pln_number_of_shares,
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_excel_company_pln_cpy
   ...
   if a_count_i > 0 and not g_found then
      ...
      loop
         if a_tbl_insert(i).fin_year_no               = g_rec_out.fin_year_no  and
            a_tbl_insert(i).company_code              = g_rec_out.company_code then
            g_found := TRUE;
         end if;
      end loop;
   end if;
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_mdl_company_pln EX Excel STARTED AT '||
...
--**************************************************************************************************
    open c_stg_excel_company_pln;
    fetch c_stg_excel_company_pln bulk collect into a_stg_input limit g_forall_limit;

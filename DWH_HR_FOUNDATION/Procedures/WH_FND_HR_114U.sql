--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_114U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_114U" (p_forall_limit in integer,p_success out boolean) AS


--**************************************************************************************************
--  Date:        NOV. 2011
--  Author:      Alastair de Wet
--  Purpose:     Create HR employee_location dimention table in the foundation layer
--               with input ex staging table from HR.
--  Tables:      Input  - stg_ps_employee_location_cpy
--               Output - fnd_hr_employee_location
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_ps_employee_location_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_employee_location%rowtype;
g_rec_in             stg_ps_employee_location_cpy%rowtype;
g_found              boolean;

g_date               date          := trunc(sysdate);

l_message            sys_dwh_errlog.log_text%type;
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_114U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR employee_location EX PEOPLE SOFT';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_ps_employee_location_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_employee_location%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_employee_location%rowtype index by binary_integer;
...
type staging_array1 is table of stg_ps_employee_location_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_ps_employee_location_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_ps_employee_location is
   select *
   from stg_ps_employee_location_cpy
...
begin
   ...
   g_hospital                            := 'N';
   g_rec_out.employee_location_code      := nvl(g_rec_in.employee_location_code,0);
...
begin
   ...
   insert into stg_ps_employee_location_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_employee_location values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_employee_location
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_ps_employee_location_cpy
...
begin
   g_found := dwh_hr_valid.fnd_hr_employee_location(g_rec_out.employee_location_code,g_rec_out.company_code);
-- Check if insert of item already in insert array and change to put duplicate in update array
   if a_count_i > 0 and not g_found then
      for i in a_tbl_insert.first .. a_tbl_insert.last
      loop
         if a_tbl_insert(i).employee_location_code  = g_rec_out.employee_location_code and
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_employee_location EX hr STARTED AT '||
...
--**************************************************************************************************
    open c_stg_ps_employee_location;
    fetch c_stg_ps_employee_location bulk collect into a_stg_input limit g_forall_limit;
    ...
    close c_stg_ps_employee_location;
--**************************************************************************************************

--------------------------------------------------------
--  DDL for Procedure WH_FND_HR_223U
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "DWH_HR_FOUNDATION"."WH_FND_HR_223U" 
...
--  Tables:      Input  - stg_of_ed_beneficiary_cpy
--               Output - fnd_hr_of_ed_beneficiary
...
g_hospital           char(1)       := 'N';
g_hospital_text      stg_of_ed_beneficiary_hsp.sys_process_msg%type;
g_rec_out            fnd_hr_of_ed_beneficiary%rowtype;
g_rec_in             stg_of_ed_beneficiary_cpy%rowtype;
g_found              boolean;
g_count              number;
...
l_module_name        sys_dwh_errlog.log_procedure_name%type    := 'WH_FND_HR_223U';
...
l_description        sys_dwh_log_summary.log_description%type  := 'LOAD THE HR BEE ED BENEFICIARY DATA EX ORACLE FINANCIALS';
l_process_type       sys_dwh_log_summary.log_process_type%type := dwh_hr_constants.vc_log_process_type_n;


-- For input bulk collect --
type stg_array is table of stg_of_ed_beneficiary_cpy%rowtype;
a_stg_input      stg_array;

-- For output arrays into bulk load forall statements --
type tbl_array_i is table of fnd_hr_of_ed_beneficiary%rowtype index by binary_integer;
type tbl_array_u is table of fnd_hr_of_ed_beneficiary%rowtype index by binary_integer;
...
type staging_array1 is table of stg_of_ed_beneficiary_cpy.sys_source_batch_id%type
                                                       index by binary_integer;
type staging_array2 is table of stg_of_ed_beneficiary_cpy.sys_source_sequence_no%type
                                                       index by binary_integer;
...
cursor c_stg_of_ed_beneficiary is
   select *
   from stg_of_ed_beneficiary_cpy
...
begin
   ...
   insert into stg_of_ed_beneficiary_hsp values g_rec_in;
...
begin
    forall i in a_tbl_insert.first .. a_tbl_insert.last
       save exceptions
       insert into fnd_hr_of_ed_beneficiary values a_tbl_insert(i);
...
begin
    ...
    forall i in a_tbl_update.first .. a_tbl_update.last
       save exceptions
       update fnd_hr_of_ed_beneficiary
...
begin
    forall i in a_staging1.first .. a_staging1.last
       save exceptions
       update stg_of_ed_beneficiary_cpy
...
begin
    ...
    l_text := 'LOAD OF fnd_hr_of_ed_beneficiary EX oracle fin STARTED AT '||
...
--**************************************************************************************************
    open c_stg_of_ed_beneficiary;
    fetch c_stg_of_ed_beneficiary bulk collect into a_stg_input limit g_forall_limit;
